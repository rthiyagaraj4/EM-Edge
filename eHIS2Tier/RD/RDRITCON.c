...
{
    34,
    "d:\\ehis\\34065\\modified\\rdritcon.pc"
};


static unsigned long sqlctx = 1466143525;
...
36,0,0,2,0,0,30,203,0,0,0,0,0,1,0,
51,0,0,3,190,0,4,235,0,0,9,4,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,
0,0,1,9,0,0,1,9,0,0,1,9,0,0,
102,0,0,4,127,0,2,271,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
133,0,0,5,165,0,4,305,0,0,5,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,
168,0,0,6,719,0,9,370,0,0,6,6,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,
207,0,0,6,0,0,13,506,0,0,5,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,
242,0,0,7,219,0,4,596,0,0,5,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,
277,0,0,8,95,0,4,617,0,0,3,2,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,
304,0,0,9,122,0,4,673,0,0,4,3,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
335,0,0,10,322,0,6,917,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,3,3,0,0,2,9,0,0,
366,0,0,11,112,0,6,954,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
389,0,0,12,136,0,6,967,0,0,2,2,0,1,0,2,9,0,0,1,3,0,0,
};
...
struct { unsigned short len; unsigned char arr[101]; } h_uom_desc;
...
void print_detl_rec()
{
	 ...
	 /* Changed by Gopi kannan for Display the Unit Descrition (IN-32463)
	
	 fprintf(f1,"%-20s %-40s ", h_item_code.arr, h_item_desc.arr);
     fprintf(f1,"%-5s    ", h_item_units.arr);
     */
	 
	 fprintf(f1,"%-20s %-32.32s ", h_item_code.arr, h_item_desc.arr);
	 fprintf(f1,"%-15.15s  ",h_uom_desc.arr);
     fprintf(f1,"%11.2lf\n", h_actual_quantity);
...
void get_item_desc ()
  {
	...
	*/
	...
	/* Changed By Gopi Kannan for Display the Item Discription (IN-34065) 
	
	EXEC SQL SELECT A.SHORT_DESC, A.UOM_CODE , B.SHORT_DESC
	           INTO :h_item_desc, :h_item_units, :h_uom_desc 
	           FROM RD_ITEMS_LANG_VW A,AM_UOM_LANG_VW B
	          WHERE A.ITEM_CODE   = :h_item_code
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select A.SHORT_DESC ,A.UOM_CODE ,B.SHORT_DESC into :b0,:b1,:\
b2  from RD_ITEMS_LANG_VW A ,AM_UOM_LANG_VW B where (((A.ITEM_CODE=:b3 and A.L\
ANGUAGE_ID=:b4) and A.UOM_CODE=B.UOM_CODE(+)) and A.LANGUAGE_ID=B.LANGUAGE_ID(\
+))";
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqadto[1] = (unsigned short )0;
 sqlstm.sqtdso[1] = (unsigned short )0;
 sqlstm.sqhstv[2] = (         void  *)&h_uom_desc;
 ...
 sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
	 ...
	 h_uom_desc.arr [ h_uom_desc.len ] = '\0';
  }
/*----------------------------- End Exam Desc -------------------------------*/
