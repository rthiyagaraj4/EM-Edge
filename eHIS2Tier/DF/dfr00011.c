...
{
    39,
    "T:\\BL\\CRF-0120\\DF\\check-out\\DFR00011.pc"
};


static unsigned long sqlctx = 1939086899;
...
b0 and a.physician_id between NVL(:b1,'!!!!!!!!!!!!!!!') and NVL(:b2,'~~~~~~~\
~~~~~~~~')) and a.contract_ref between NVL(:b3,'!!!!!!!!!!!!!!!!!!!!') and NV\
L(:b4,'~~~~~~~~~~~~~~~~~~~~')) and b.facility_id=a.operating_facility_id) and\
 a.physician_id=b.physician_id) order by a.physician_id,a.contract_ref,a.cont\
ract_period_from,a.processing_year,a.processing_month            ";
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select PARAM1 ,PARAM2 ,PARAM3 ,PARAM4 into :b0,:b1,:b2,:\
b3  from SY_PROG_PARAM where ((PGM_ID=:b4 and SESSION_ID=:b5) and PGM_DATE=:b\
6)";
    sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('DFR00011.LEGEND_'||LTRIM(RTRIM(TO_CHA\
