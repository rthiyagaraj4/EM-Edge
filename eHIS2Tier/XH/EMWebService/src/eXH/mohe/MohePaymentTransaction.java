...
public class MohePaymentTransaction {
	...
	public String processRequest(String actionKey, String eventType) {
		...
		String l_response_str = "", l_response_err_json = "",l_tranasction_date = "";
		...
		try {
			...
			l_app_msg = " ************ Mohe Payment Traction request started ************ ";
			...
			if (l_errcd.equals("N")) {
				l_app_msg = " Retrieving Mohe Doc Type...";
				if (l_debug_YN.equals("Y")) System.out.println(l_app_msg);
				...
				if (l_practo_doc_type.indexOf("Error") >= 0) {
					l_errcd = "E";
					l_errmsg = "Error in Mohe Doc Type - " + l_practo_doc_type;
					l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  " + " } ";

					return "500#" + l_response_err_json + "#";
				}
				l_app_msg = " Mohe Doc Type : " + l_practo_doc_type;
				if (l_debug_YN.equals("Y")) System.out.println(l_app_msg);
			...
			if (l_errcd.equals("N")) {
				...
				l_app_msg = " ************ Mohe Payment Traction request Ended ************ ";
				if (l_debug_YN.equals("Y"))	System.out.println(l_app_msg);
	...
	public String insertWPReceiptMessageDetails(Map<String, String> params, String status, String err_message) {
		String insertstmt = "INSERT INTO XH_WP_RECEIPT (OPERATING_FACILITY_ID, VENDOR_TYPE, PATIENT_ID, SHORT_NAME, DATE_OF_BIRTH, GENDER,  PRACTO_DOC_TYPE_CODE,"
				...
				+ " MODIFIED_AT_WS_NO,  STATUS, PROCESS_FLAG, PRACTO_TRANS_ID)  VALUES ( ?,?,?,?,TO_DATE(?,'DD/MM/YYYY HH24:MI:SS'),?,?,?,TO_DATE(?,'DD/MM/YYYY HH24:MI:SS'),?,?,?,?,TO_DATE(?,'DD/MM/YYYY HH24:MI:SS'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		try {
			...
			l_app_msg = " :::  Mohe Payment Transaction Insert Count : " + insertCount;
			if (l_debug_YN.equals("Y"))	System.out.println(l_app_msg);

			if (insertCount == 1) {

				l_err_msg = "Mohe Payment Transaction Successfull";

				dbConn.commit();
			} else {
				l_err_msg = "Error in Mohe Payment Transaction Details";
				dbConn.rollback();
			}
