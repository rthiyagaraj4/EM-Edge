...
public class FinAppInboundTransaction {
	...
	public String processRequest(String actionKey, String eventType, String key) {
		...
		String l_message_id = "", l_application_id = "", l_facility_id = "",
				l_protocol_link_id = "", l_patient_gender = "", l_appointment_type = "";
		...
		try {
			...
			l_app_msg = " ::: Inside FinAppInboundTransactions processRequest ::: ";
			...
			{			
				...
				l_external_app_details = FetchExternalAppDetails("FINAPP");			
			...
			if (l_patient_gender.length() > 1) {
				l_errmsg = "Error in Patient Gender,Please Check the Patient Gender in the request - "+l_patient_gender;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "
						+ " } ";
			...
			if (l_appointment_type.length() > 2) {
				l_errmsg = "Error in Appointment Type, Please Check the Appointment Type in the request - "+l_appointment_type;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "
						+ " } ";
			...
			if (l_errcd.equals("S")) {
				...
				l_fetch_resp_time = FetchResponseWaitTime("FINAPP");
	...
	public static ArrayList FetchExternalAppDetails(String ExternalApplication) {
		...
		try {
				dbConn = EMDataSource.getConnection();
				query = "SELECT APPLICATION_ID,PROTOCOL_LINK_ID,FACILITY_ID FROM xh_filter_dms_vw WHERE EXTERNAL_APPLICATION = 'FINAPP'";
	...
	public String validateRequest(String key) {
		...
		String l_app_msg = "", l_errmsg = "";
		String l_key = "";
		String l_segment_id = "FWG", l_standard_code = "FINAPPSTD";
