...
{
    36,
    "T:\\BL\\ver remov\\blr00079\\BLR00079.PC"
};


static unsigned long sqlctx = 1476883155;
...
"_CODE,a.DOC_TYPE_CODE)=b.doc_type_co\
de and nvl(a.ADJ_DOC_NUM,a.DOC_NUMBER)=b.doc_num) and a.DOCUMENT_NATURE in ('\
BI','DC')) and a.doc_date<=nvl(to_date((:b0||'23:59:59'),'DD/MM/YYYY hh24:mi:\
ss'),sysdate)) and a.patient_id=c.patient_id) and CEIL((NVL(TO_DATE(:b0,'DD/M\
M/YYYY'),SYSDATE)-nvl(b.DOC_DATE,a.doc_date))) between :b2 and :b3) and NVL(b\
.blng_grp_id,'X')=d.blng_grp_id(+)) and B.BILL_TOT_AMT>0) group by SUBSTR(blc\
ommon.get_reln_code(a.patient_id),1,10),nvl(d.BLNG_GRP_CATG,'**'),a.patient_i\
d,c.short_name,nvl(b.bed_class_code,'**'),nvl(a.episode_type,'R'),nvl(a.ADJ_D\
OC_TYPE_CODE,a.DOC_TYPE_CODE),nvl(a.ADJ_DOC_NUM,a.DOC_NUMBER) having ((sum(a.\
...
,'CANC-WRITEOFF',a.DEBIT_AMT,0))))>0)  group by citizen,BLNG_GRP_CATG,patient\
_id,patient_name,bed_class_code,episode_type order by citizen,BLNG_GRP_CATG,p\
atient_id            ";
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select PARAM1 ,to_number(PARAM2) ,to_number(PARAM3) ,PAR\
AM4 ,PARAM5 into :b0,:b1,:b2,:b3,:b4  from SY_PROG_PARAM where ((PGM_ID=:b5 a\
nd SESSION_ID=:b6) and PGM_DATE=:b7)";
...

{
      ...
      sqlstm.sqladtp = &sqladt;
      sqlstm.sqltdsp = &sqltds;
      sqlstm.stmt = "select SHORT_DESC into :b0  from BL_BLNG_GRP_CATG wher\
e BLNG_GRP_CATG=:b1";
      sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00079.LEGEND_'||LTRIM(RTRIM(TO_CHA\
