...
{
    31,
    "T:\\BL\\handover\\PROC\\blrundep.pc"
};


static unsigned long sqlctx = 2038189845;
...
!!!!!!!!!!!!!!!') and NVL(:b1,'~~~~~~~~~~~~~~~~~~~~') and EPISODE_ID between \
NVL(:b2,0) and NVL(:b3,'99999999')) and RECPT_NATURE_CODE='AD') and TRUNC(DOC\
_DATE)<=TO_DATE(:b4,'DD/MM/YYYY')) and NVL(CANCELLED_IND,'N')<>'Y') group by \
PATIENT_ID,EPISODE_ID,VISIT_ID,EPISODE_TYPE having sum(DOC_AMT)>=0           ";

 static const char *sq0011 = 
"select PATIENT_ID ,EPISODE_ID ,VISIT_ID ,EPISODE_TYPE ,NVL(sum(DOC_AMT),0) \
 from BL_RECEIPT_REFUND_HDRDTL_VW where (((PATIENT_ID between NVL(:b0,'!!!!!!\
!!!!!!!!!!!!!!') and NVL(:b1,'~~~~~~~~~~~~~~~~~~~~') and RECPT_NATURE_CODE='P\
D') and trunc(DOC_DATE)<=TO_DATE(:b2,'DD/MM/YYYY')) and NVL(CANCELLED_IND,'N'\
)<>'Y') group by PATIENT_ID,EPISODE_ID,VISIT_ID,EPISODE_TYPE having sum(DOC_A\
MT)>=0           ";

 static const char *sq0013 = 
"select sum(NVL(DEPOSIT_ADJ_AMT,0))  from BL_BILL_HDR where (((((EPISODE_TYP\
E=:b0 and PATIENT_ID=:b1) and EPISODE_ID=:b2) and VISIT_ID=DECODE(:b0,'I',VIS\
IT_ID,'D',VISIT_ID,:b4)) and trunc(DOC_DATE)<=TO_DATE(:b5,'DD/MM/YYYY')) and \
NVL(BILL_STATUS,'Z')<>'C')           ";

 static const char *sq0015 = 
"select sum(NVL(DEPOSIT_ADJ_AMT_PD,0))  from BL_BILL_HDR where (((EPISODE_TY\
PE=:b0 and PATIENT_ID=:b1) and trunc(DOC_DATE)<=TO_DATE(:b2,'DD/MM/YYYY')) an\
d NVL(BILL_STATUS,'Z')<>'C')           ";
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/blrundep.c-arc   1.0   Apr 19 2007 17:07:24   vcm_th  $ sqlca.h 
 */
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/blrundep.c-arc   1.0   Apr 19 2007 17:07:24   vcm_th  $ sqlda.h 
 */
...
AM4 ,PARAM5 ,PARAM6 into :b0,:b1,:b2,:b3,:b4,:b5,:b6  from SY_PROG_PARAM wher\
e ((PGM_ID='BLRUNDEP' and SESSION_ID=TO_NUMBER(:b7)) and PGM_DATE=:b8)";
   sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select NVL(sum(DOC_AMT),0) into :b0  from BL_RECEIPT_REFUN\
D_HDRDTL_VW where ((((((EPISODE_TYPE in ('I','D') and PATIENT_ID=:b1) and EPI\
SODE_ID=:b2) and VISIT_ID=VISIT_ID) and TRUNC(DOC_DATE)<=TO_DATE(:b3,'DD/MM/Y\
YYY')) and RECPT_NATURE_CODE='AD') and NVL(CANCELLED_IND,'N')<>'Y')";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select NVL(sum(DOC_AMT),0) into :b0  from BL_RECEIPT_REFUN\
D_HDRDTL_VW where ((((EPISODE_TYPE in ('I','D') and PATIENT_ID=:b1) and TRUNC\
(DOC_DATE)<=TO_DATE(:b2,'DD/MM/YYYY')) and RECPT_NATURE_CODE='PD') and NVL(CA\
NCELLED_IND,'N')<>'Y')";
  sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select count(*)  into :b0  from IP_OPEN_EPISODE where PATIE\
NT_ID=:b1";
 sqlstm.iters = (unsigned int  )1;
...
,60),null ,' ',substr(short_name,31,60)) shortname2 ,short_name into :b0,:b1,\
...
r2 ) ; IF str2 IS NOT NULL THEN :nd_pat_name := str2 ; END IF ; blcommon . sp\
lit_words ( :d_short_name_full , 30 , '*' , 2 , str1 , str2 ) ; IF str2 IS NO\
T NULL THEN :nd_pat_name1 := str2 ; END IF ; blcommon . split_words ( :d_shor\
t_name_full , 30 , '*' , 3 , str1 , str2 ) ; IF str2 IS NOT NULL THEN :nd_pat\
_name1 := :nd_pat_name1 || str2 ; end if ; END ; END ;";
sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRUNDEP.LEGEND_'||LTRIM(RTRIM(TO_CHA\
