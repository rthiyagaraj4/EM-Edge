...
{
    23,
    "D:\\BLRLM005\\BLRLM005.pc"
};


static unsigned long sqlctx = 577824643;
...
"select A.PATIENT_ID ,TO_CHAR(A.REGN_DATE,'DD/MM/YYYY') ,decode(:b0,'en',sub\
str(A.short_name,1,60),substr(A.SHORT_NAME_LOC_LANG,1,60)) ,A.SEX ,TO_CHAR(A.\
DATE_OF_BIRTH,'DD/MM/YYYY') ,TO_CHAR(B.CLASSIFICATION_START_DATE,'DD/MM/YYYY'\
) ,TO_CHAR(B.CLASSIFICATION_END_DATE,'DD/MM/YYYY')  from MP_PATIENT_MAST A ,B\
L_PATIENT_FIN_DTLS B ,BL_CLASSFCTN_CODE C where (((((A.PATIENT_ID=B.PATIENT_I\
D and B.CLASSIFICATION_CODE=C.CLASSIFICATION_CODE) and C.OPERATING_FACILITY_I\
D=:b1) and C.CLASSIFICATION_TYPE='PV') and A.PATIENT_ID between :b2 and :b3) \
and TRUNC(A.REGN_DATE) between TO_DATE(NVL(:b4,'01/01/1800'),'DD/MM/YYYY') an\
d TO_DATE(NVL(:b5,'01/01/3000'),'DD/MM/YYYY')) order by 2,1            ";
...
230,0,0,6,0,0,27,560,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...
3  from SY_PROG_PARAM where (((OPERATING_FACILITY_ID=:b4 and PGM_ID=:b5) and \
...

{
      ...
      sqlstm.sqladtp = &sqladt;
      sqlstm.sqltdsp = &sqltds;
      sqlstm.stmt = "select ACC_ENTITY_NAME ,TO_CHAR(SYSDATE,'DD/MM/YYYY HH\
24:MI') ,USER into :b0,:b1,:b2  from SY_ACC_ENTITY_LANG_VW where (ACC_ENTITY_\
ID=:b3 and language_id=:b4)";
      sqlstm.iters = (unsigned int  )1;
