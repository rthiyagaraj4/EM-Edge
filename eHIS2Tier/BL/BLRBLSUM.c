...
{
    21,
    "C:\\10xdev\\BLRBLSUM.PC"
};


static unsigned long sqlctx = 139598259;
...
"select A.CUST_CODE ,A.BLNG_GRP_ID ,TO_CHAR(TRUNC(A.DOC_DATE),'DD-MON-YY') ,\
A.PATIENT_ID ,A.EPISODE_TYPE ,A.DOC_TYPE_CODE ,TO_CHAR(A.DOC_NUM,'00000009') \
,TO_CHAR(A.EPISODE_ID) ,TO_CHAR(A.VISIT_ID) ,NVL(A.BILL_TOT_AMT,0) ,NVL(A.BIL\
L_TOT_OUTST_AMT,0) ,CREDIT_DOC_REF_DESC ,TO_CHAR(CREDIT_DOC_REF_DATE,'DD/MM/R\
RRR') ,ADDED_BY_ID ,TO_CHAR(ADDED_DATE,'DD/MM/RRRR HH24:MI') ,NVL(BILL_TOT_AD\
JUST_AMT,0) ,NVL(BILL_TOT_EXEMPT_AMT,0) ,NVL(BILL_TOT_WRITE_OFF_AMT,0)  from \
BL_BILL_HDR A where (((((((A.OPERATING_FACILITY_ID=:b0 and TRUNC(A.DOC_DATE) \
between TO_DATE(nvl(:b1,'01/01/0001'),'dd/mm/yyyy') and TO_DATE(nvl(:b2,'31/1\
2/4712'),'dd/mm/yyyy')) and A.BILL_NATURE_CODE=:b3) and A.BILL_STATUS is null\
 ) and A.BILL_TOT_OUTST_AMT<>0) and A.BLNG_GRP_ID between nvl(:b4,'!!') and n\
vl(:b5,'~~')) and NVL(A.CUST_CODE,NVL(:b6,'!!!!!!!!')) between NVL(:b6,'!!!!!\
!!!') and NVL(:b8,'~~~~~~~~')) and NVL(A.EPISODE_TYPE,'X')=DECODE(:b9,'A',A.E\
PISODE_TYPE,:b9)) order by A.CUST_CODE,A.BLNG_GRP_ID,TRUNC(A.DOC_DATE),A.DOC_\
NUM            ";
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "select OPERATING_FACILITY_ID ,PARAM1 ,PARAM2 ,PARAM3 ,ltr\
im(rtrim(PARAM4)) ,ltrim(rtrim(PARAM5)) ,PARAM6 ,PARAM7 ,PARAM8 into :b0,:b1,\
:b2,:b3,:b4,:b5,:b6,:b7,:b8  from SY_PROG_PARAM where ((PGM_ID=:b9 and SESSIO\
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select NVL(LONG_NAME,'*****') into :b0  from AR_CUSTOMER w\
here CUST_CODE=:b1";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select substr(SHORT_NAME,1,30) ,decode(substr(short_name,3\
1,60),null ,' ',substr(short_name,31,60)) shortname1 ,short_name into :b0,:b1\
...
r2 ) ; IF str2 IS NOT NULL THEN :d_short_name := str2 ; END IF ; blcommon . s\
plit_words ( :d_short_name_full , 30 , '*' , 2 , str1 , str2 ) ; IF str2 IS N\
OT NULL THEN :d_short_name1 := str2 ; END IF ; blcommon . split_words ( :d_sh\
ort_name_full , 30 , '*' , 3 , str1 , str2 ) ; IF str2 IS NOT NULL THEN :d_sh\
ort_name1 := :d_short_name1 || str2 ; end if ; END ; END ;";
sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRBLSUM.LEGEND_'||LTRIM(RTRIM(TO_CHA\
...
:nd_temp_date , 'DD/MM/YYYY HH24:MI' ) , :p_language_id ) ; :nd_loc_date := T\
O_CHAR ( TO_DATE ( :nd_loc_date , 'DD/MM/YYYY HH24:MI' ) , 'DD/MM/YYYY HH24:M\
I' ) ; END ;";
   sqlstm.iters = (unsigned int  )1;
...
temp_date , 'DD/MM/YYYY' ) , :p_language_id ) ; END ;";
   sqlstm.iters = (unsigned int  )1;
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "begin :nd_loc_date := sm_convert_date_2t ( TO_DATE ( :nd\
_temp_date , 'DD-MON-rrRR' ) , :p_language_id ) ; END ;";
    sqlstm.iters = (unsigned int  )1;
