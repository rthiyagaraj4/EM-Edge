...
{
    30,
    "C:\\HELPDESK\\scf708\\Blrallau.pc"
};


static unsigned long sqlctx = 566253395;
...
99') ,DOC_AMT ,NARRATION ,POST_MONTH ,POST_YEAR ,POSTED_BY_ID ,TO_CHAR(POSTED_\
DATE_TIME,'DD/MM/YYYY HH24:MI:SS')  from BL_DCP_ALL_TRN_HDR where ((((operatin\
g_facility_id=:b0 and (RPAD(DOC_TYPE_CODE,6,' ')||LTRIM(TO_CHAR(DOC_NUMBER,'00\
000009'))) between (RPAD(NVL(:b1,'      '),6)||NVL(:b2,'00000000')) and (RPAD(\
NVL(:b3,'~~~~~~'),6)||NVL(:b4,'99999999'))) and TRUNC(DOC_DATE) between TO_DAT\
E(NVL(:b5,'01/01/1000'),'DD/MM/YYYY') and TO_DATE(NVL(:b6,'01/01/3000'),'DD/MM\
/YYYY')) and ADDED_BY_ID between NVL(:b7,'                    ') and NVL(:b7,'\
~~~~~~~~~~~~~~~~~~~~')) and NVL(POSTED_FLAG,'N')='Y') order by DOC_TYPE_CODE,D\
OC_NUMBER            ";
...
9,990.99') ,ALL_DOC_AMT  from BL_DCP_ALL_TRN_DTL2 where ((operating_facility_i\
d=:b0 and DOC_TYPE_CODE=:b1) and DOC_NUMBER=:b2) order by ALL_DOC_TYPE_CODE,AL\
L_DOC_NUMBER            ";
...
36,0,0,2,216,0,4,154,0,0,10,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,
91,0,0,3,128,0,4,195,0,0,4,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,
122,0,0,4,0,0,30,225,0,0,0,0,0,1,0,
137,0,0,5,91,0,2,233,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
160,0,0,6,877,0,9,331,0,0,9,9,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
211,0,0,6,0,0,13,370,0,0,14,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
282,0,0,7,0,0,13,438,0,0,11,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,9,0,0,
341,0,0,8,0,0,13,491,0,0,4,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,
372,0,0,10,92,0,9,523,0,0,1,1,0,1,0,1,9,0,0,
391,0,0,10,0,0,13,528,0,0,2,0,0,1,0,2,9,0,0,2,9,0,0,
414,0,0,11,511,0,6,544,0,0,3,3,0,1,0,1,9,0,0,2,9,0,0,3,9,0,0,
441,0,0,9,276,0,9,580,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
468,0,0,9,0,0,13,596,0,0,5,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,
503,0,0,7,335,0,9,636,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
530,0,0,8,256,0,9,667,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
557,0,0,12,245,0,4,816,0,0,8,7,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
...
struct { unsigned short len; unsigned char arr[65]; } d_acct_desc;
   ...
   /* VARCHAR d_hdr_doc_type_code                         [7],
	   ...
	   d_hdr_trx_type_desc                         [4],
           ...
           d_hdr_post_month                            [3],
           d_hdr_post_year                             [5],
	   d_hdr_posted_by_id                          [21],
	   d_hdr_posted_date_time                      [20]; */ 
...
struct { unsigned short len; unsigned char arr[21]; } d_hdr_posted_by_id;

struct { unsigned short len; unsigned char arr[20]; } d_hdr_posted_date_time;
...
struct { unsigned short len; unsigned char arr[7]; } d_dtl1_dept_code;
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/BLRALLAU.C-arc   1.0   Apr 19 2007 17:05:24   vcm_th  $ sqlca.h 
 */
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/BLRALLAU.C-arc   1.0   Apr 19 2007 17:05:24   vcm_th  $ sqlda.h 
 */
...
{
    if(argc < 5) {
      disp_message(ERR_MESG,"BLRALLAU : Usage BLRALLAU userid/password session_id pgm_date facility_id");
      proc_exit();
    }
...
RAM where ((PGM_ID='BLRALLAU' and PGM_DATE=:b8) and SESSION_ID=TO_NUMBER(:b9))";
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='BLRALLAU' and\
...
fetch_hdr()  
{
    ...
    d_hdr_posted_by_id.arr[0]                                         = '\0';
    d_hdr_posted_date_time.arr[0]                                     = '\0';
    ...
    d_hdr_posted_by_id.len                                            = 0;
    d_hdr_posted_date_time.len                                        = 0;
...

{
     ...
     sqlstm.sqhstv[12] = (         void  *)&d_hdr_posted_by_id;
     ...
     sqlstm.sqhstv[13] = (         void  *)&d_hdr_posted_date_time;
    ...
    d_hdr_posted_by_id.arr[d_hdr_posted_by_id.len]                       = '\0';
    d_hdr_posted_date_time.arr[d_hdr_posted_date_time.len]               = '\0';
...
OT NULL THEN :d_patient_name1 := str2 ; END IF ; blcommon . split_words ( :d_s\
hort_name_full , 30 , '*' , 3 , str1 , str2 ) ; IF str2 IS NOT NULL THEN :d_pa\
tient_name1 := :d_patient_name1 || str2 ; end if ; END ; END ;";
  sqlstm.iters = (unsigned int  )1;
...
put_hdr()
{
    ...
    fprintf(f1,"REP : BLRALLAU                              AUDIT TRIAL OF ALLOCATING TRANSACTIONS                                       PAGE : %4d\n",page_no);
...
put_trn_hdr() 
{
    ...
    fprintf(f1,"    POSTED BY :  %-20s    POSTED DATE : %-20s\n\n",
	d_hdr_posted_by_id.arr, d_hdr_posted_date_time.arr);
    line_no += 4;
