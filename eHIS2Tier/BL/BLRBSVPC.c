...
{
    43,
    "T:\\temp\\Bl\\robert\\Proc\\blrbsvpc\\BLRBSVPC.pc"
};


static unsigned long sqlctx = 2024256717;
...
er_rate) ,to_char(b.DAYCARE_RATE) ,b.cust_pc_ind  from BL_SERV_CUST_PC_PRICE b\
 where (((b.cust_pc_ind='S' and (b.cust_pc_code=:b0 or (b.cust_pc_code='ALL' a\
nd 0=(select count(*)   from bl_serv_cust_pc_price where ((((cust_pc_code=:b0 \
and cust_pc_ind=b.cust_pc_ind) and blng_serv_code=b.blng_serv_code) and cust_p\
c_ind='S') and rownum=1))))) and TO_DATE(:b2,'DD/MM/YYYY') between TRUNC(b.val\
id_from) and NVL(trunc(b.VALID_TO),trunc(sysdate))) and b.blng_serv_code betwe\
en NVL(:b3,'!!!!!!!!!!') and NVL(:b4,'~~~~~~~~~~')) order by 1,2            ";
...
ass_code,'**','All',a.blng_class_code) ,to_char(a.ip_rate) ,to_char(a.op_rate)\
 ,to_char(a.ref_rate) ,to_char(a.emer_rate) ,to_char(a.DAYCARE_RATE)  from bl_\
serv_cust_pc_price a where ((((((:b0='N' and a.blng_serv_code=:b1) and a.blng_\
class_code=:b2) or (:b0='Y' and a.blng_serv_code between NVL(:b4,'!!!!!!!!!!')\
 and NVL(:b5,'~~~~~~~~~~'))) and a.cust_pc_code in (:b6,:b7,:b8,:b9,:b10,:b11)\
) and TO_DATE(:b12,'DD/MM/YYYY') between NVL(a.valid_from(+),to_date('01011000\
','DDMMYYYY')) and NVL(a.valid_to(+),to_date('01014000','DDMMYYYY'))) and a.cu\
st_pc_ind(+)='P') order by a.blng_serv_code,a.cust_pc_code,a.blng_class_code d\
esc             ";

 static const char *sq0012 = 
"select distinct price_class_code  from bl_price_class where price_class_code\
 in (:b0,:b1,:b2,:b3,:b4,:b5) order by price_class_code            ";
...
244,0,0,10,698,0,9,349,0,0,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
279,0,0,13,0,0,32,355,0,0,0,0,0,1,0,
294,0,0,10,0,0,13,374,0,0,7,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,
...
403,0,0,15,0,0,32,455,0,0,0,0,0,1,0,
418,0,0,12,0,0,13,466,0,0,1,0,0,1,0,2,9,0,0,
437,0,0,16,0,0,32,479,0,0,0,0,0,1,0,
452,0,0,10,0,0,15,488,0,0,0,0,0,1,0,
467,0,0,11,716,0,9,533,0,0,13,13,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
534,0,0,17,236,0,4,539,0,0,8,7,0,1,0,2,4,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,
581,0,0,11,0,0,15,562,0,0,0,0,0,1,0,
596,0,0,11,716,0,9,570,0,0,13,13,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
663,0,0,18,236,0,4,589,0,0,8,7,0,1,0,2,4,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,
710,0,0,11,0,0,15,615,0,0,0,0,0,1,0,
725,0,0,12,143,0,9,830,0,0,6,6,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,
764,0,0,12,143,0,9,920,0,0,6,6,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,
};
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/BLRBSVPC.c-arc   1.0   Apr 19 2007 17:05:48   vcm_th  $ sqlca.h 
 */
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/BLRBSVPC.c-arc   1.0   Apr 19 2007 17:05:48   vcm_th  $ sqlda.h 
 */
...
6,:b7,:b8,:b9,:b10,:b11  from SY_PROG_PARAM where ((PGM_ID='BLRBSVPC' and SESS\
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='BLRBSVPC' and\
...
c  from (select distinct a.price_class_code pc_code  from bl_price_class a whe\
re a.price_class_code in (:b1,:b2,:b3,:b4,:b5,:b6) order by a.price_class_code\
 ) b ) c where c.pc=:b7";
    sqlstm.iters = (unsigned int  )1;
