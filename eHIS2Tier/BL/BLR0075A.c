...
static unsigned long sqlctx = 17482707;
...
{10,4274,0,0,0,
5,0,0,1,0,0,27,96,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
36,0,0,2,218,0,4,141,0,0,7,3,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,
0,0,1,9,0,0,
79,0,0,3,84,0,2,169,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
106,0,0,4,0,0,30,217,0,0,0,0,0,1,0,
121,0,0,5,384,0,9,251,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
144,0,0,5,0,0,15,259,0,0,0,0,0,1,0,
159,0,0,5,0,0,13,278,0,0,5,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,
194,0,0,6,155,0,4,432,0,0,5,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,
229,0,0,7,96,0,4,516,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
252,0,0,8,162,0,6,523,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
};
...
/*  MEDICOM VER 4.1                                                  */
/************************************************************************/
/* PROGRAM NAME          : BLR0075A.PC                                  */
/* AUTHOR                : Robert Joseph                               */  
/* DATE WRITTEN          : 18/06/2005                                   */      
/************************************************************************/
...
fprintf(fp, "%c(s12.5H", ESC);/* Reduce the font size*/
   ...
   while (fetch_hosp_cur())
     {
      fprintf(fp,"%-18s %-18s %-18s ",bl_batch_doc_type.arr,bl_doc_date.arr,bl_doc_no.arr);
 ...
 /* EXEC SQL DECLARE HOSP_CUR CURSOR FOR 
       Select BANK_IN_BATCH_DOC_TYPE,
		...
		Where 
          ...
          // to_char(bank_in_date,'MM-RRRR')  <= :nd_fm_date  and  
   ...
   open_file()
{
	...
	strcat(filename,"BLR0075A.lis");

    if ((fp = fopen(filename,"w")) == NULL)
    {
       disp_message(ERR_MESG,"Error in opening file BLR0075A.lis");
       proc_exit();
    }
}
...
print_page_title()
{
	...
	//Code for Line 3
	l_start_point=0;    
	fprintf(fp,"%-14.14s: BLR0075A",loc_legend[21]);
...
fetch_legend_value()
{
	for(i=1;i<=300;i++)
	{
		...
		/* EXEC SQL SELECT LTRIM(RTRIM('BLR0075A.LEGEND_'||LTRIM(RTRIM(TO_CHAR(:i,'009')))))
		           INTO :l_pk_value
				   FROM dual; */ 

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR0075A.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
