...
{
    27,
    "D:\\Issues\\23012\\blrsrins.pc"
};


static unsigned long sqlctx = 763486771;
...
"select A.CUST_CODE ,A.BLNG_GRP_ID ,SUBSTR(B.BLNG_SERV_CODE,1,2) ,SUBSTR(B.B\
LNG_SERV_CODE,1,3) ,B.BLNG_SERV_CODE ,B.ORG_GROSS_CHARGE_AMT ,B.ORG_DISC_AMT \
,B.ORG_NET_CHARGE_AMT  from BL_BILL_HDR A ,BL_PATIENT_CHARGES_FOLIO B where (\
(((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and A.DOC_NUM=B.BILL_DOC_NU\
M) and A.BLNG_GRP_ID=:b0) and A.CUST_CODE=:b1) and A.PATIENT_ID between :b2 a\
nd :b3) and A.DOC_TYPE_CODE between :b4 and :b5) and A.DOC_NUM between :b6 an\
d :b7) and TRUNC(A.DOC_DATE) between to_date(:b8,'DD/MM/YYYY') and to_date(:b\
9,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between :b10 and :b11) and NVL(B.PACKAG\
E_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~'\
)<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(a.operating_facility_\
id,'X')=NVL(:b12,'X')) and NVL(b.operating_facility_id,'X')=NVL(:b12,'X')) or\
der by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE            ";
...
) and A.BLNG_GRP_ID=:b0) and A.PATIENT_ID between :b1 and :b2) and A.DOC_TYPE\
_CODE between :b3 and :b4) and A.DOC_NUM between :b5 and :b6) and TRUNC(A.DOC\
_DATE) between to_date(:b7,'DD/MM/YYYY') and to_date(:b8,'DD/MM/YYYY')) and B\
.BLNG_SERV_CODE between :b9 and :b10) and NVL(B.PACKAGE_TRX_IND,'X')='N') and\
 NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_\
TRX_TYPE_CODE,'X')<>'I') and NVL(a.operating_facility_id,'X')=NVL(:b11,'X')) \
and NVL(b.operating_facility_id,'X')=NVL(:b11,'X')) order by A.BLNG_GRP_ID,A.\
CUST_CODE,B.BLNG_SERV_CODE            ";

 static const char *sq0008 = 
"select A.CUST_CODE ,A.BLNG_GRP_ID ,SUBSTR(B.BLNG_SERV_CODE,1,2) ,SUBSTR(B.B\
LNG_SERV_CODE,1,3) ,B.BLNG_SERV_CODE ,B.ORG_GROSS_CHARGE_AMT ,B.ORG_DISC_AMT \
,B.ORG_NET_CHARGE_AMT  from BL_BILL_HDR A ,BL_PATIENT_CHARGES_FOLIO B where (\
((((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and A.DOC_NUM=B.BILL_DOC_N\
UM) and A.BLNG_GRP_ID=:b0) and A.CUST_CODE=:b1) and A.EPISODE_TYPE=:b2) and A\
.PATIENT_ID between :b3 and :b4) and A.DOC_TYPE_CODE between :b5 and :b6) and\
 A.DOC_NUM between :b7 and :b8) and TRUNC(A.DOC_DATE) between to_date(:b9,'DD\
/MM/YYYY') and to_date(:b10,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between :b11 \
and :b12) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C'\
) and NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and\
 NVL(a.operating_facility_id,'X')=NVL(:b13,'X')) and NVL(b.operating_facility\
_id,'X')=NVL(:b13,'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE  \
          ";
...
,B.ORG_NET_CHARGE_AMT  from BL_BILL_HDR A ,BL_PATIENT_CHARGES_FOLIO B where (\
(((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and A.DOC_NUM=B.BILL_DOC_NU\
M) and A.BLNG_GRP_ID=:b0) and A.EPISODE_TYPE=:b1) and A.PATIENT_ID between :b\
2 and :b3) and A.DOC_TYPE_CODE between :b4 and :b5) and A.DOC_NUM between :b6\
 and :b7) and TRUNC(A.DOC_DATE) between to_date(:b8,'DD/MM/YYYY') and to_date\
(:b9,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between :b10 and :b11) and NVL(B.PAC\
KAGE_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,\
'~')<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(a.operating_facili\
ty_id,'X')=NVL(:b12,'X')) and NVL(b.operating_facility_id,'X')=NVL(:b12,'X'))\
 order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE            ";

 static const char *sq0010 = 
"select LONG_DESC  from BL_BLNG_GRP_LANG_VW where (BLNG_GRP_ID=:b0 and LANGU\
AGE_ID=:b1)           ";

 static const char *sq0013 = 
"select LONG_DESC  from BL_BLNG_SERV_GRP_LANG_VW where (SERV_GRP_CODE=:b0 an\
d language_id=:b1)           ";

 static const char *sq0014 = 
"select LONG_DESC  from BL_SERV_CLASSIFICATION_LANG_VW where (SERV_CLASSIFIC\
ATION_CODE=:b0 and language_id=:b1)           ";

 static const char *sq0012 = 
"select LONG_DESC  from BL_BLNG_SERV_LANG_VW where (BLNG_SERV_CODE=:b0 and l\
anguage_id=:b1)           ";

 static const char *sq0011 = 
"select LONG_NAME  from AR_CUSTOMER_LANG_VW where (CUST_CODE=:b0 and languag\
e_id=:b1)           ";
...
{10,4274,0,0,0,
5,0,0,1,379,0,4,253,0,0,20,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,
100,0,0,2,91,0,2,314,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
123,0,0,3,0,0,30,324,0,0,0,0,0,1,0,
138,0,0,4,0,0,27,330,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
169,0,0,5,0,0,17,380,0,0,1,1,0,1,0,1,9,0,0,
188,0,0,5,0,0,45,572,0,0,14,14,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
259,0,0,5,0,0,45,590,0,0,13,13,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
326,0,0,5,0,0,45,611,0,0,15,15,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
401,0,0,5,0,0,45,630,0,0,14,14,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
472,0,0,6,906,0,9,714,0,0,14,14,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
543,0,0,7,883,0,9,716,0,0,13,13,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
610,0,0,8,932,0,9,721,0,0,15,15,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,
685,0,0,9,909,0,9,723,0,0,14,14,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
756,0,0,6,0,0,15,859,0,0,0,0,0,1,0,
771,0,0,7,0,0,15,861,0,0,0,0,0,1,0,
786,0,0,8,0,0,15,866,0,0,0,0,0,1,0,
801,0,0,9,0,0,15,868,0,0,0,0,0,1,0,
816,0,0,5,0,0,13,901,0,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,
843,0,0,6,0,0,13,945,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,
0,0,2,4,0,0,2,4,0,0,
890,0,0,7,0,0,13,957,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,
0,0,2,4,0,0,2,4,0,0,
937,0,0,8,0,0,13,972,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,
0,0,2,4,0,0,2,4,0,0,
984,0,0,9,0,0,13,984,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,
0,0,2,4,0,0,2,4,0,0,
1031,0,0,10,97,0,9,1017,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
1054,0,0,10,0,0,13,1024,0,0,1,0,0,1,0,2,9,0,0,
1073,0,0,13,104,0,9,1038,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
1096,0,0,13,0,0,13,1045,0,0,1,0,0,1,0,2,9,0,0,
1115,0,0,14,121,0,9,1058,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
1138,0,0,14,0,0,13,1065,0,0,1,0,0,1,0,2,9,0,0,
1157,0,0,12,101,0,9,1079,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
1180,0,0,12,0,0,13,1086,0,0,1,0,0,1,0,2,9,0,0,
1199,0,0,11,95,0,9,1099,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
1222,0,0,11,0,0,13,1106,0,0,1,0,0,1,0,2,9,0,0,
1241,0,0,15,144,0,4,3442,0,0,2,1,0,1,0,2,3,0,0,1,9,0,0,
1264,0,0,16,158,0,4,3520,0,0,5,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,
1299,0,0,17,748,0,4,3545,0,0,15,14,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,
0,0,
1374,0,0,18,725,0,4,3571,0,0,14,13,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1445,0,0,19,774,0,4,3599,0,0,16,15,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,
0,0,1,9,0,0,
1524,0,0,20,751,0,4,3626,0,0,15,14,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,
0,0,
1599,0,0,21,96,0,4,3660,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
1622,0,0,22,165,0,6,3667,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
1653,0,0,23,140,0,6,3704,0,0,3,3,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,
1680,0,0,24,94,0,4,3807,0,0,2,0,0,1,0,2,9,0,0,2,9,0,0,
1703,0,0,25,39,0,4,3838,0,0,2,1,0,1,0,1,9,0,0,2,3,0,0,
1726,0,0,26,35,0,4,3841,0,0,2,1,0,1,0,1,3,0,0,2,3,0,0,
1749,0,0,27,43,0,4,3858,0,0,2,1,0,1,0,1,9,0,0,2,3,0,0,
1772,0,0,28,49,0,4,3861,0,0,3,2,0,1,0,1,9,0,0,1,3,0,0,2,9,0,0,
1799,0,0,29,43,0,4,3888,0,0,2,1,0,1,0,1,9,0,0,2,3,0,0,
1822,0,0,30,49,0,4,3891,0,0,3,2,0,1,0,1,9,0,0,1,3,0,0,2,9,0,0,
};
...
/* OCS MEDICOM VER 1.0                                                  */
/************************************************************************/
/* PROGRAM NAME          : BLRSRINS.PC                                  */
/* AUTHOR                : V.SRIKANTH                                   */
/* DATE WRITTEN          : 01/02/1999    Modified : 31/05/1999          */
/*                                                                      */
/* FUNCTION              : BUSINESS ANALYSIS BY CUSTOMER (SUMMARY)      */
/*                                                                      */
...
struct { unsigned short len; unsigned char arr[30]; } s_amt_temp;
...
RAM7 ,PARAM8 ,PARAM9 ,PARAM10 ,PARAM11 ,NVL(PARAM12,'01/01/1800') ,NVL(PARAM1\
3,'31/12/4712') ,PARAM14 ,PARAM15 ,PARAM16 ,PARAM17 ,operating_facility_id in\
to :b0,:b1,:b2,:b3,:b4,:b5,:b6,:b7,:b8,:b9,:b10,:b11,:b12,:b13,:b14,:b15,:b16\
,:b17  from SY_PROG_PARAM where ((PGM_ID='BLRSRINC' and SESSION_ID=:b18) and \
PGM_DATE=:b19)";
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='BLRSRINC' and\
...
/*--- Bill Header Cursor (Dynamic SQL) ---------------------------------*/
 ...
 strcat(d_curs.arr," DOC_TYPE_CODE BETWEEN  :nd_fr_doc_type_code and \
			   ...
			   OPERATING_FACILITY_ID = :d_facility_id AND \
...
open_files()
{
  ...
  strcat(filename,"blrsrins.lis");
   
   if ((fp = fopen(filename,"w")) == NULL)
   {
	err_mesg("\nError while opening File blrsrins.lis\n",0,"");
	proc_exit();
   }
...
print_title()
{
 ...
 /* fprintf(fp, "\n%-3.3s : BL %39s %-30s %34s  %-16s\n",loc_legend[10],
	  ...
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		...
		fprintf(fp,"\n%-5.5s : BL ",nd_desc_with_spl_chr.arr);				
																																																					
	}
	else
	   {
         fprintf(fp,"\n%-4.4s : BL ",loc_legend[10]);
        }
     fprintf(fp,"%39s "," ");
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-30.30s",hosp_name.arr);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
 ...
 /*fprintf(fp, "%-4.4s : %-8s %26s  %-28s %39s   %-5.5s : %4d\n",loc_legend[30],
    loc_legend[300], " ", "                              ", " ",loc_legend[60], pno);
	commented by saikiran*/
	  ...
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
	   ...
	   {
         fprintf(fp,"%-4.4s : ",loc_legend[30]);
        }
  /*fprintf(fp, "%-8s %26s  %-28s %39s   ",
    loc_legend[300], " ", "                              ", " "); */
	  ...
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-9s",loc_legend[300]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
 ...
 // fprintf(fp,"\n\n        %-15s \n        ----------------\n\n",loc_legend[70]);
    ...
    else if (nd_episode_type.arr[0] == 'E')
  {
	  ...
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-14.14s",loc_legend[120]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-14.14s\n\n",nd_desc_with_spl_chr.arr);				
...
else if (nd_episode_type.arr[0] == 'R')
  {
	  ...
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
	   ...
	   {
         fprintf(fp,"%-21.21s            : ",loc_legend[80]);
        }
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-20.20s",loc_legend[130]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-20.20s\n\n",nd_desc_with_spl_chr.arr);				
	...
	else 
  {
 	  ...
 	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-21.21",loc_legend[80]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-21.21s          : ",nd_desc_with_spl_chr.arr);				
	   ...
	   {
         fprintf(fp,"%-20.21s            : ",loc_legend[80]);
        }
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
  ...
  {
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-16s\n"," ",loc_legend[170],loc_legend[180],loc_legend[150]);commented by saikiran
      ...
      if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-7.7s",loc_legend[150]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-7.7s\n",nd_desc_with_spl_chr.arr);				
	...
	//fprintf(fp," %14s                          %-4.4s   : %-16s\n\n"," ",loc_legend[470],loc_legend[160]);commented by saikiran
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-4.4s",loc_legend[470]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-4.4s   : ",nd_desc_with_spl_chr.arr);				
	  ...
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-8.8s",loc_legend[160]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-8.8s\n\n",nd_desc_with_spl_chr.arr);				
  ...
  {     
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-16s\n"," ",loc_legend[170],nd_fr_pat_id.arr);commented by saikiran
    fprintf(fp," %14s "," ");
	...
	//fprintf(fp," %14s                          %-4.4s   : %-16s\n\n"," ",loc_legend[470],nd_to_pat_id.arr); commented by saikiran
  ...
  if ((strcmp(nd_fr_blng_grp.arr,"!!") == 0) && (strcmp(nd_to_blng_grp.arr,"~~") == 0))
  {
    ...
    //fprintf(fp," %14s %-20.20s     %-5.5s  : %-10s\n"," ",loc_legend[190],loc_legend[180],loc_legend[150]); commented by saikiran
	  fprintf(fp," %14s "," ");              
	 if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-8.8s",loc_legend[150]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-8.8s\n",nd_desc_with_spl_chr.arr);				
	  	...
	  	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-10.10s",loc_legend[160]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-10s\n\n",nd_desc_with_spl_chr.arr);				
  ...
  {   
 	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-3s\n"," ",loc_legend[190],loc_legend[180],nd_fr_blng_grp.arr); commented by saikiran
	fprintf(fp," %14s "," "); 
	  	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
	...
	//fprintf(fp," %14s                          %-4.4s   : %-3s\n\n"," ",loc_legend[470],nd_to_blng_grp.arr); commented by saikiran
	fprintf(fp," %14s                          "," "); 
  ...
  {
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-10s\n"," ",loc_legend[200],loc_legend[180],loc_legend[150]);commented by saikiran
	fprintf(fp," %14s "," ");              
		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
 ...
 if ((strcmp(nd_fr_doc_type_code.arr,"!!!!!!") == 0) && (strcmp(nd_to_doc_type_code.arr,"~~~~~~") == 0))
  {
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-10s\n"," ",loc_legend[210],loc_legend[180],loc_legend[150]); commented by saikiran
	fprintf(fp," %14s "," ");
	   		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
  ...
  {     
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-6s\n"," ",loc_legend[210],loc_legend[180],nd_fr_doc_type_code.arr); commented by saikiran
	fprintf(fp," %14s "," ");
   	   		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
	...
	//fprintf(fp," %14s                          %-4.4s   : %-6s\n\n"," ",loc_legend[470],nd_to_doc_type_code.arr); commented by saikiran
	fprintf(fp," %14s                          "," "); 
   	   		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
  ...
  {
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-10s\n"," ",loc_legend[220],loc_legend[180],loc_legend[150]); commented by saikiran
	fprintf(fp," %14s "," ");                            
   	   		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
    	   		...
    	   		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-9.9s",loc_legend[160]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-9.9s\n\n",nd_desc_with_spl_chr.arr);				
  ...
  {     
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-8s\n"," ",loc_legend[220],loc_legend[180],nd_fr_doc_number.arr); commented by saikiran
	fprintf(fp," %14s "," "); 
    	   		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
	...
	//fprintf(fp," %14s                          %-4.4s  : %-8s\n\n"," ",loc_legend[470],nd_to_doc_number.arr); commented by saikiran
	fprintf(fp," %14s                          %"," "); 
    	   		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
 ...
 if ((strcmp(nd_fr_bill_date.arr,"01/01/1800") == 0) && (strcmp(nd_to_bill_date.arr,"31/12/4712") == 0))
  {
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-10s\n"," ",loc_legend[230],loc_legend[180],loc_legend[150]); commented by saikiran
	fprintf(fp," %14s "," "); 
    	   		if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
  ...
  {     
		...
		fun_change_loc_date();
	    //fprintf(fp," %14s %-20.20s     %-5.5s  : %-10s\n"," ",loc_legend[230],loc_legend[180],date_convert.arr); commented by saikiran
		fprintf(fp," %14s "," "); 
         if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
         ...
         if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
	...
	else
	   {
		...
		fun_change_loc_date();
	    //fprintf(fp," %14s                          %-4.4s   : %-10s\n\n"," ",loc_legend[470],date_convert.arr); commented by saikiran
		fprintf(fp," %14s                          "," "); 
         if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
  ...
  {
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-10s\n"," ",loc_legend[240],loc_legend[180],loc_legend[150]);commetned by saikiran
	fprintf(fp," %14s "," ");    
         if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
  ...
  {    
	...
	//fprintf(fp," %14s %-20.20s     %-5.5s  : %-6s\n"," ",loc_legend[240],loc_legend[180],nd_fr_blng_serv.arr); commented by saikiran
	...
	//fprintf(fp," %14s                          %-4.4s   : %-6s\n\n"," ",loc_legend[470],nd_to_blng_serv.arr); commented by saikiran
    ...
    //fprintf(fp,  " %14s %-20.20s            : %s\n\n"," ",loc_legend[250],loc_legend[480]);commented by saikiran
  fprintf(fp,  " %14s "," ");
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-20.20s",loc_legend[250]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-20.20s           : ",nd_desc_with_spl_chr.arr);				
		  ...
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-11.11s",loc_legend[480]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-11.11s\n\n",nd_desc_with_spl_chr.arr);				
  ...
  if (nd_report_opt.arr[0] == 'G')
  {
    //fprintf(fp," %14s %-20.20s            : %s\n\n"," ",loc_legend[260],loc_legend[270]);commented by saikiran
      fprintf(fp," %14s "," ");
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-20.20s",loc_legend[260]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-20.20s           : ",nd_desc_with_spl_chr.arr);				
	   ...
	   {
         fprintf(fp,"%-20.20s            : ",loc_legend[260]);
        } 
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-14.14s",loc_legend[270]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-14.14s\n\n",nd_desc_with_spl_chr.arr);				
    ...
    {
	  //  fprintf(fp," %14s %-20.20s            : %s\n\n"," ",loc_legend[260],loc_legend[280]);commented by saikiran
	    fprintf(fp," %14s "," ");
 if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		  ...
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-23.23s",loc_legend[280]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-23.23s\n\n",nd_desc_with_spl_chr.arr);				
  ...
  {
    //fprintf(fp," %14s %-20.20s            : %s\n\n"," ",loc_legend[260],loc_legend[290]);commented by saikiran
	 fprintf(fp," %14s "," ");
 if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		  ...
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-16.16s",loc_legend[290]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-16.16s\n\n",nd_desc_with_spl_chr.arr);				
...
init_date_temp_var();                             
	  strcpy(date_convert.arr,date_time.arr);            
	  fun_change_loc_date();
  /*fprintf(fp, "\f\n%-4.4s : BL %39s %-30s %34s  %-16s\n",loc_legend[10],
      " ", hosp_name.arr," ", date_convert.arr); commented by saikiran*/
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		...
		sprintf(nd_temp_str.arr,"%-5.5s",loc_legend[10]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-5.5s : BL %39s ",nd_desc_with_spl_chr.arr," ");				
																																																					
	}
	else
	   {
         fprintf(fp,"%-4.4s : BL %39s ",loc_legend[10], " ");
        } 
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		...
		sprintf(nd_temp_str.arr,"%-30.30s",hosp_name.arr);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-30.30s %34s  ",nd_desc_with_spl_chr.arr," ");				
																																																					
	}
	else
	   {
         fprintf(fp,"%-30.30s %34s  ",hosp_name.arr," ");
        } 
   
  fprintf(fp, "%-16s\n",date_convert.arr); 


  /*fprintf(fp, "%-4.4s : %-10s                      %s",loc_legend[20],
     user_id.arr,loc_legend[40]); commented by saikiran */
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		...
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%s",loc_legend[40]);
		fun_Pad_desc_with_spl_char_1(nd_temp_str.arr);
		fprintf(fp,"%s",nd_desc_with_spl_chr.arr);				
																																																					
	}
	else
	   {
         fprintf(fp,"%s",loc_legend[40]);
        } 
...
if (nd_episode_type.arr[0] == 'I')
{
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%s",loc_legend[90]);
		fun_Pad_desc_with_spl_char_1(nd_temp_str.arr);
		fprintf(fp,"%s",nd_desc_with_spl_chr.arr);				
  ...
  else if (nd_episode_type.arr[0] == 'D')
  {
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%s",loc_legend[100]);
		fun_Pad_desc_with_spl_char_1(nd_temp_str.arr);
		fprintf(fp,"%s",nd_desc_with_spl_chr.arr);				
  ...
  else if (nd_episode_type.arr[0] == 'O')
  {
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%s",loc_legend[110]);
		fun_Pad_desc_with_spl_char_1(nd_temp_str.arr);
		fprintf(fp,"%s",nd_desc_with_spl_chr.arr);				
  ...
  else if (nd_episode_type.arr[0] == 'E')
  {
   // fprintf(fp," -- %-14.14s : %s",loc_legend[80],loc_legend[120]);
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%s",loc_legend[120]);
		fun_Pad_desc_with_spl_char_1(nd_temp_str.arr);
		fprintf(fp,"%s",nd_desc_with_spl_chr.arr);				
  ...
  else if (nd_episode_type.arr[0] == 'R')
  {
   // fprintf(fp," -- %-14.14s : %s",loc_legend[80],loc_legend[130]);
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%s",loc_legend[130]);
		fun_Pad_desc_with_spl_char_1(nd_temp_str.arr);
		fprintf(fp,"%s",nd_desc_with_spl_chr.arr);				
  ...
  else 
  {
   // fprintf(fp," -- %-14.14s : %s",loc_legend[80],loc_legend[140]);
	...
	if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%s",loc_legend[140]);
		fun_Pad_desc_with_spl_char_1(nd_temp_str.arr);
		fprintf(fp,"%s",nd_desc_with_spl_chr.arr);				
 ...
 /* fprintf(fp,"%-10.10s  : %-10s  %-6.6s : %-40s  \n",loc_legend[310],
								g_blng_grp_id.arr,loc_legend[450], g_blng_grp_desc.arr);
    COMMENTED BY SAIKIRAN */
  ...
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-14.14s",loc_legend[310]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-14.14s  : ",nd_desc_with_spl_chr.arr);				
      ...
      fprintf(fp,"%-10s  ",g_blng_grp_id.arr);

  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-6.6s",loc_legend[450]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
	   ...
	   {
         fprintf(fp,"%-6.6s : ",loc_legend[450]);
        }
  ...
  {
    /*fprintf(fp,"%-10.10s  : %-10s  %-6.6s : %-40s  \n\n",loc_legend[320],
								g_cust_code.arr,loc_legend[450], g_cust_name.arr);
      commented by saikiran */
  ...
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-10.10s",loc_legend[320]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
	   ...
	   {
         fprintf(fp,"%-10.10s  : ",loc_legend[320]);
        }
     fprintf(fp,"%-10s  ",g_cust_code.arr);
  ...
  {
	//fprintf(fp,"%-10.10s  : %-50s\n",loc_legend[320],loc_legend[460]);
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
  ...
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-50.50s",loc_legend[460]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-50s\n",nd_desc_with_spl_chr.arr);				
  ...
  if (nd_report_opt.arr[0] == 'G')
  {
	/*fprintf(fp,"%-15.15s    %-20.20s                       %17.17s   %13.13s    %15.15s\n",
	   loc_legend[270],loc_legend[330],loc_legend[340],loc_legend[350],loc_legend[360]);commented by saikiran*/
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
	   ...
	   {
         fprintf(fp,"%-15.15s    ",loc_legend[270]);
        }
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-20.20s",loc_legend[330]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-20.20s                               ",nd_desc_with_spl_chr.arr);				
	   ...
	   {
        fprintf(fp,"%-20.20s                               ",loc_legend[330]);
        }
  ...
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-10.10s",loc_legend[340]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-10.10s       ",nd_desc_with_spl_chr.arr);				
	   ...
	   {
        fprintf(fp,"%-10.10s       ",loc_legend[340]);
        }
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-9.9s",loc_legend[350]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-9.9s           ",nd_desc_with_spl_chr.arr);				
	   ...
	   {
         fprintf(fp,"%-9.9s          ",loc_legend[350]);
        }
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-8.8s",loc_legend[360]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-8.8s\n",nd_desc_with_spl_chr.arr);				
  ...
  {
	/*fprintf(fp,"%-15.15s    %-20.20s                       %17.17s   %13.13s    %15.15s\n",
	loc_legend[280],loc_legend[330],loc_legend[340],loc_legend[350],loc_legend[360]);commented by saikiran*/
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
	   ...
	   {
         fprintf(fp,"%-23.23s    ",loc_legend[280]);
        }
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-27.27s",loc_legend[330]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
	   ...
	   {
         fprintf(fp,"%-27.27s                           ",loc_legend[330]);
        }
  ...
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-11.11s",loc_legend[360]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-11.11s\n",nd_desc_with_spl_chr.arr);				
  ...
  {
	/*fprintf(fp,"%-15.15s    %-20.20s                       %17.17s   %13.13s    %15.15s\n",
	loc_legend[290],loc_legend[330],loc_legend[340],loc_legend[350],loc_legend[360]);commented by saikiran*/
  ...
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
	   ...
	   {
         fprintf(fp,"%-16.16s    ",loc_legend[290]);
        }
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
  ...
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-10.10s",loc_legend[350]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-10.10s          ",nd_desc_with_spl_chr.arr);				
	   ...
	   {
         fprintf(fp,"%-10.10s         ",loc_legend[350]);
        }
  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-10.10s",loc_legend[360]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp,"%-10.10s\n",nd_desc_with_spl_chr.arr);				
...
print_end()
{
  ...
  //fprintf(fp, "\n\n\n %52s **   %-15.15s   ** \n\n\f", " ",loc_legend[370]);commented by saikiran
  fprintf(fp, "\n\n\n %52s **   ", " ");
...
print_break_serv_grp()
{
	...
	//fprintf(fp,"%-40.40s     ",xx_grp_desc.arr);commented by saikiran
...
print_break_serv_clasfn()
{
	...
	fprintf(fp,"  %-3.3s",xx_clasfn_id.arr); fprintf(fp,"%22s"," ");
	//fprintf(fp,"%-40.40s    ",xx_clasfn_desc.arr);commented by saikiran
	  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
...
print_break_blng_serv()
{
    ...
    fprintf(fp,"  %-10.10s",xx_serv_id.arr); fprintf(fp,"%8s"," ");
	//fprintf(fp,"%-40.40s    ",xx_serv_desc.arr);commented by saikiran
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
...
print_break_blng_grp()
{
	...
	fprintf(fp,"  %-2s"," "); //fprintf(fp,"%14s"," ");
	     if (nd_report_opt.arr[0] == 'C')
	     fprintf(fp,"%21s"," ");
	 else fprintf(fp,"%14s"," ");
	//fprintf(fp," %-40.40s     ",loc_legend[380]); commeented by saikiran
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-40.40s",loc_legend[380]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp," %-40.40s     ",nd_desc_with_spl_chr.arr);				
...
print_break_cash_pat()
{
   fprintf(fp, "%s%s%s\n",
	...
	{
	   //fprintf(fp," %-40.40s     ",loc_legend[390]);commented by saikiran
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-40.40s",loc_legend[390]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp," %-40.40s     ",nd_desc_with_spl_chr.arr);				
	   ...
	   {
         fprintf(fp," %-40.40s     ",loc_legend[390]);
        }
	   }
...
print_break_cust_code()
{
	...
	//fprintf(fp," %-40.40s     ",loc_legend[400]);
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
	{
		nd_temp_str.arr[0]='\0';
		nd_temp_str.len=0;
		sprintf(nd_temp_str.arr,"%-40.40s",loc_legend[400]);
		fun_Pad_desc_with_spl_char(nd_temp_str.arr);
		fprintf(fp," %-40.40s     ",nd_desc_with_spl_chr.arr);				
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select no_of_decimal into :b0  from sm_acc_entity_param \
where acc_entity_id=(select acc_entity_code  from sy_acc_entity where acc_ent\
ity_id=:b1)";
    sqlstm.iters = (unsigned int  )1;
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select count(*)  into :b0  from BL_BILL_HDR A ,BL_PATIEN\
T_CHARGES_FOLIO B where ((((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE an\
d A.DOC_NUM=B.BILL_DOC_NUM) and A.BLNG_GRP_ID=:b1) and A.CUST_CODE=:b2) and A\
.PATIENT_ID between :b3 and :b4) and A.DOC_TYPE_CODE between :b5 and :b6) and\
 A.DOC_NUM between :b7 and :b8) and TRUNC(A.DOC_DATE) between to_date(:b9,'DD\
/MM/YYYY') and to_date(:b10,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between :b11 \
and :b12) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C'\
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select count(*)  into :b0  from BL_BILL_HDR A ,BL_PATIENT_\
CHARGES_FOLIO B where (((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and A\
.DOC_NUM=B.BILL_DOC_NUM) and A.BLNG_GRP_ID=:b1) and A.PATIENT_ID between :b2 \
and :b3) and A.DOC_TYPE_CODE between :b4 and :b5) and A.DOC_NUM between :b6 a\
nd :b7) and TRUNC(A.DOC_DATE) between to_date(:b8,'DD/MM/YYYY') and to_date(:\
b9,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between :b10 and :b11) and NVL(B.PACKA\
GE_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~\
')<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(a.operating_facility\
_id,'X')=NVL(:b12,'X')) and NVL(b.operating_facility_id,'X')=NVL(:b12,'X')) o\
rder by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE ";
  sqlstm.iters = (unsigned int  )1;
...

{
      ...
      sqlstm.sqladtp = &sqladt;
      sqlstm.sqltdsp = &sqltds;
      sqlstm.stmt = "select count(*)  into :b0  from BL_BILL_HDR A ,BL_PATI\
ENT_CHARGES_FOLIO B where (((((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE\
 and A.DOC_NUM=B.BILL_DOC_NUM) and A.BLNG_GRP_ID=:b1) and A.CUST_CODE=:b2) an\
d A.EPISODE_TYPE=:b3) and A.PATIENT_ID between :b4 and :b5) and A.DOC_TYPE_CO\
DE between :b6 and :b7) and A.DOC_NUM between :b8 and :b9) and TRUNC(A.DOC_DA\
TE) between to_date(:b10,'DD/MM/YYYY') and to_date(:b11,'DD/MM/YYYY')) and B.\
BLNG_SERV_CODE between :b12 and :b13) and NVL(B.PACKAGE_TRX_IND,'X')='N') and\
 NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_\
TRX_TYPE_CODE,'X')<>'I') and NVL(a.operating_facility_id,'X')=NVL(:b14,'X')) \
and NVL(b.operating_facility_id,'X')=NVL(:b14,'X')) order by A.BLNG_GRP_ID,A.\
CUST_CODE,B.BLNG_SERV_CODE ";
      sqlstm.iters = (unsigned int  )1;
...

{
      ...
      sqlstm.stmt = "select count(*)  into :b0  from BL_BILL_HDR A ,BL_PATI\
ENT_CHARGES_FOLIO B where ((((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE \
and A.DOC_NUM=B.BILL_DOC_NUM) and A.BLNG_GRP_ID=:b1) and A.EPISODE_TYPE=:b2) \
and A.PATIENT_ID between :b3 and :b4) and A.DOC_TYPE_CODE between :b5 and :b6\
) and A.DOC_NUM between :b7 and :b8) and TRUNC(A.DOC_DATE) between to_date(:b\
9,'DD/MM/YYYY') and to_date(:b10,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between \
:b11 and :b12) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')\
<>'C') and NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I'\
) and NVL(a.operating_facility_id,'X')=NVL(:b13,'X')) and NVL(b.operating_fac\
ility_id,'X')=NVL(:b13,'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_C\
ODE ";
      sqlstm.iters = (unsigned int  )1;
...
fetch_legend_value()
{
 	for(i=1;i<=900;i++)
	{
		...
		/* EXEC SQL SELECT LTRIM(RTRIM('BLRSRINS.LEGEND_'||LTRIM(RTRIM(TO_CHAR(:i,'009')))))
		           INTO :l_pk_value
				   FROM dual; */ 

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRSRINS.LEGEND_'||LTRIM(RTRIM(TO_CHA\
		  ...
		  if(strcmp(nd_pad_req_yn.arr,"Y")==0)
					{
						...
						fun_Pad_desc_with_spl_char_1(l_translated_value.arr);
						
					}
					else
					strcpy(nd_desc_with_spl_chr.arr,l_translated_value.arr);
					
					strcpy(loc_legend[i],nd_desc_with_spl_chr.arr);*/
	}
}
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select batch_file_name ,report_tool_ch into :b0,:b1  from s\
m_report where report_id='BLRSRINS'";
 sqlstm.iters = (unsigned int  )1;
