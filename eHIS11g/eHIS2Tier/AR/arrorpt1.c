...
{
    60,
    "c:\\madhu\\billing\\arrorpt1-print-afficial request\\arrorpt1.pc"
};


static unsigned long sqlctx = 1995208605;
...
Y') ,NARRATION ,ROWID ,NVL(PRINT_FLAG,'N') ,ADDED_BY_ID  from AR_CASHIER where\
 ((((((DOC_TYPE_CODE>=NVL(:b0,'      ') and DOC_TYPE_CODE<=NVL(:b1,'~~~~~~')) \
and DOC_NUM>=NVL(:b2,0)) and DOC_NUM<=NVL(:b3,99999999)) and OPERATING_FACILIT\
Y_ID=:b4) and NVL(DOC_DATE,SYSDATE)>=NVL(TO_DATE(:b5,'DD/MM/YYYY'),to_date('00\
010101','YYYYDDMM'))) and NVL(DOC_DATE,SYSDATE)<=NVL(TO_DATE(:b6,'DD/MM/YYYY')\
,to_date('47123112','YYYYDDMM'))) order by DOC_TYPE_CODE,DOC_NUM            ";
...
{10,4274,0,0,0,
5,0,0,1,205,0,4,193,0,0,10,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,
60,0,0,2,91,0,2,260,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
83,0,0,3,0,0,30,275,0,0,0,0,0,1,0,
98,0,0,4,0,0,27,281,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
129,0,0,5,202,0,4,307,0,0,8,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,3,0,0,1,9,0,0,
176,0,0,6,620,0,9,381,0,0,7,7,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,
219,0,0,6,0,0,13,431,0,0,13,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
4,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
286,0,0,7,118,0,9,474,0,0,1,1,0,1,0,1,9,0,0,
305,0,0,7,0,0,13,482,0,0,1,0,0,1,0,2,9,0,0,
324,0,0,8,175,0,4,597,0,0,4,3,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
355,0,0,9,111,0,5,611,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
382,0,0,10,0,0,29,620,0,0,0,0,0,1,0,
};
...
;
   ...
   facility_name.arr[facility_name.len]   = '\0';
   ...
   /* EXEC SQL DECLARE AR_CASHIER_CUR CURSOR FOR
             SELECT DOC_TYPE_CODE,
              ...
              WHERE  DOC_TYPE_CODE >= NVL(:nd_fm_doc_type,'      ')
				...
				AND OPERATING_FACILITY_ID = :nd_facility_id  // added by madhusudhan for GDOH-CRF-0107
...

{
       ...
       sqlstm.sqladtp = &sqladt;
       sqlstm.sqltdsp = &sqltds;
       sqlstm.stmt = "select ROWID into :b0  from AR_CASHIER where (((DOC_TY\
PE_CODE=:b1 and DOC_NUM=:b2) and NVL(PRINT_FLAG,'N')='N') and OPERATING_FACILI\
TY_ID=:b3) for update of PRINT_FLAG nowait ";
       sqlstm.iters = (unsigned int  )1;
