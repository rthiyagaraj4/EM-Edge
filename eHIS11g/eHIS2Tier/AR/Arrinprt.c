...
{
    66,
    "\\\\cscappche644\\currobj_32_bit_r2\\sandy\\shikha\\01032017\\arrinprt.pc"
};


static unsigned long sqlctx = 1567274331;
...
_INV_HDR where ((((((((OPERATING_FACILITY_ID=:b0 and DOC_TYPE_CODE>=NVL(:b1,' \
     ')) and DOC_TYPE_CODE<=NVL(:b2,'~~~~~~')) and DOC_NUM>=NVL(:b3,0)) and DO\
C_NUM<=NVL(:b4,99999999)) and NVL(DOC_DATE,SYSDATE)>=NVL(TO_DATE(:b5,'DD/MM/YY\
YY'),to_date('00010101','YYYYDDMM'))) and NVL(DOC_DATE,SYSDATE)<=NVL(TO_DATE(:\
b6,'DD/MM/YYYY'),to_date('47123112','YYYYDDMM'))) and NVL(POSTED_FLAG,'N')='N'\
) and NVL(PRINT_FLAG,'N')='N') order by DOC_TYPE_CODE,DOC_NUM            ";
...
LS where ((OPERATING_FACILITY_ID=:b0 and DOC_TYPE_CODE=:b1) and DOC_NUM=:b2) o\
rder by BILL_DATE,PATIENT_ID            ";

 static const char *sq0007 = 
"select distinct BILL_DOC_TYPE_CODE ,BILL_NUM ,AMT_PAID  from AR_PATIENT_DTLS\
 where ((((OPERATING_FACILITY_ID=:b0 and DOC_TYPE_CODE=:b1) and DOC_NUM=:b2) a\
nd BILL_DOC_TYPE_CODE is  not null ) and AMOUNT>0) order by BILL_DOC_TYPE_CODE\
,BILL_NUM            ";
...
{10,4274,0,0,0,
5,0,0,1,193,0,4,228,0,0,9,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
0,0,2,9,0,0,1,9,0,0,1,9,0,0,
56,0,0,2,91,0,2,334,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
79,0,0,3,0,0,30,346,0,0,0,0,0,1,0,
94,0,0,4,181,0,4,373,0,0,7,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,3,
0,0,1,9,0,0,
137,0,0,5,1007,0,9,497,0,0,7,7,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,
180,0,0,5,0,0,13,562,0,0,27,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
...
0,2,9,0,0,2,9,0,0,
303,0,0,8,516,0,9,626,0,0,1,1,0,1,0,1,9,0,0,
322,0,0,8,0,0,13,651,0,0,10,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,1,0,0,2,1,0,0,
377,0,0,6,272,0,9,711,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
404,0,0,7,253,0,9,720,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
431,0,0,6,0,0,13,747,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
0,0,2,9,0,0,2,4,0,0,
478,0,0,7,0,0,13,785,0,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,4,0,0,
505,0,0,9,71,0,9,808,0,0,1,1,0,1,0,1,9,0,0,
524,0,0,9,0,0,13,813,0,0,1,0,0,1,0,2,9,0,0,
543,0,0,10,85,0,4,1022,0,0,3,1,0,1,0,2,9,0,0,2,9,0,0,1,9,0,0,
570,0,0,11,85,0,5,1079,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
593,0,0,12,0,0,29,1086,0,0,0,0,0,1,0,
608,0,0,13,0,0,27,1202,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
};
...
declare_cursors()
{
   ...
   /* EXEC SQL DECLARE AR_PATIENT_DTLS_CUR CURSOR FOR
             SELECT PATIENT_ID,
              ...
              WHERE OPERATING_FACILITY_ID = :nd_facility_id /o ADDED BY SHIKHA FOR GDOH-CRF-0107 ON 03-01-2017o/
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "update AR_INV_HDR  set PRINT_FLAG='Y' where (ROWID=:b0 an\
d OPERATING_FACILITY_ID=:b1)";
    sqlstm.iters = (unsigned int  )1;
