...
{
    124,
    "C:\\Users\\mbothra\\Desktop\\abhinav_proc_wokr\\tocheck and change-pending for 107.6and 7\\11g_Regular_source\\ARRDBLPM\\arrdblpm.pc"
};


static unsigned long sqlctx = 1134554883;
...
_TRN where (operating_facility_id=:b0 and CUST_CODE=:b1) order by POST_YEAR,PO\
ST_MONTH,DOC_DATE,DOC_TYPE_CODE,DOC_NUM            ";

 static const char *sq0008 = 
"select TO_CHAR(BASE_DATE,'DD/MM/YYYY') ,TO_CHAR(DUE_DATE,'DD/MM/YYYY') ,MISC\
_CUST_LONG_NAME  from AR_PENDING_DOC where (((operating_facility_id=:b0 and CU\
ST_CODE=:b1) and DOC_TYPE_CODE=:b2) and DOC_NUM=:b3)           ";
...
{10,4130,0,0,0,
5,0,0,1,513,0,4,205,0,0,25,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
...
120,0,0,2,249,0,4,309,0,0,9,3,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,3,0,0,1,9,0,0,
171,0,0,3,91,0,2,390,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
194,0,0,4,0,0,30,399,0,0,0,0,0,1,0,
209,0,0,5,555,0,9,617,0,0,12,12,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
272,0,0,6,577,0,9,645,0,0,12,12,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
335,0,0,7,361,0,9,691,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
358,0,0,5,0,0,13,712,0,0,4,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
389,0,0,6,0,0,13,721,0,0,4,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
420,0,0,7,0,0,13,764,0,0,12,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
483,0,0,8,217,0,9,799,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
514,0,0,8,0,0,13,812,0,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,
541,0,0,9,0,0,27,1089,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
};
...
where ((PGM_ID='ARRDBLPM' and SESSION_ID=:b23) and PGM_DATE=:b24)";
   ...
   nd_facility_id.arr[nd_facility_id.len]        = '\0'; /*added for GDOH-CRF-107.6*/
...
open_files()   
{
   ...
   if ((fp = fopen(filename,"w")) == NULL)
   {
		...
		//err_mesg("\nError while opening File arrdblpm.lis\n",0,"");
        //proc_exit();
   }
...
"-------------------------------------------------------------------------------------------------------------------------------------\n");
fprintf(fp,
"DOC DATE   MM/YYYY DOC TYPE & NO   DUE DATE   CUSTOMER REF                       DEBIT AMT          CREDIT AMT         BALANCE AMT\n");
fprintf(fp,
"-------------------------------------------------------------------------------------------------------------------------------------\n\n");
  fflush(fp);
  lctr = 7;
...
{ 
  ...
  VARCHAR s_tot_dr_amt[20], s_tot_cr_amt[20], s_tot_bal_amt[20];
...

{
   ...
   sqlstm.sqadto[0] = (unsigned short )0;
   sqlstm.sqtdso[0] = (unsigned short )0;
   sqlstm.sqhstv[1] = (         void  *)&ar_customer_cust_code;
...

{
   ...
   sqlstm.sqadto[1] = (unsigned short )0;
   sqlstm.sqtdso[1] = (unsigned short )0;
   sqlstm.sqhstv[2] = (         void  *)&cust_org_doc_type_code;
   sqlstm.sqhstl[2] = (unsigned int  )9;
   sqlstm.sqhsts[2] = (         int  )0;
   sqlstm.sqindv[2] = (         void  *)0;
   sqlstm.sqinds[2] = (         int  )0;
   ...
   sqlstm.sqadto[2] = (unsigned short )0;
   sqlstm.sqtdso[2] = (unsigned short )0;
   sqlstm.sqhstv[3] = (         void  *)&cust_org_doc_num;
   sqlstm.sqhstl[3] = (unsigned int  )11;
   sqlstm.sqhsts[3] = (         int  )0;
   sqlstm.sqindv[3] = (         void  *)0;
   sqlstm.sqinds[3] = (         int  )0;
...
print_details()
{
  ...
  fprintf(fp,"%-10s %-2s/%-4s %6s/%-8.8s %-10s %-15s %-7s ",
		doc_date_prn.arr,nd_post_month.arr,nd_post_year.arr,cust_doc_type_code.arr,cust_doc_num.arr,
		misc_flag == 'Y'?due_date.arr:" ",cust_ref.arr," ");
