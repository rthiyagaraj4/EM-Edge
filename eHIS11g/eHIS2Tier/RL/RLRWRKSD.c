...
{
    30,
    "D:\\WORK\\PROCOTHERS\\RLRWRKSD.pc"
};


static unsigned long sqlctx = 1671424949;
...
"WORK_GROUP_CODE)))) order by 1,2,6            ";

 static const char *sq0010 = 
".AH13,0))+NVL(B.AH14,0))+NVL(B.AH15,0))+NVL(B.AH16,0))+NVL(B.AH17,0))+NVL(B.\
AH18,0))+NVL(B.AH19,0))+NVL(B.AH20,0))+NVL(B.AH21,0))+NVL(B.AH22,0))+NVL(B.AH2\
3,0))+NVL(B.AH24,0))) ADD_TOTAL  from RL_TEST_CODE A ,RL_RESULT_WORKLOAD_DAILY\
 B ,RL_PARAM_FOR_FACILITY D where (((((((A.TEST_CODE=B.TEST_CODE and B.RESULT_\
DATE>=TO_DATE(:b0,'DD/MM/YYYY')) and B.RESULT_DATE<(TO_DATE(:b1,'DD/MM/YYYY')+\
1)) and B.OPERATING_FACILITY_ID=:b2) and NVL(A.STATISTICS_YN,'N')='Y') and NVL\
(d.dept_restr_qure,'N')='Y') and b.operating_facility_id=d.operating_facility_\
id) and a.section_code in (select section_code  from rl_user_dept where ((user\
_id=:b8 and operating_facility_id=:b2) and section_code between :b3 and :b4)))\
 order by 1,2,6            ";
...
201,0,0,9,3118,0,9,351,0,0,16,16,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,
280,0,0,10,2775,0,9,403,0,0,12,12,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
343,0,0,11,0,0,32,413,0,0,0,0,0,1,0,
358,0,0,12,70,0,4,525,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
381,0,0,13,0,0,32,537,0,0,0,0,0,1,0,
396,0,0,9,0,0,13,579,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
0,0,2,9,0,0,2,9,0,0,
443,0,0,10,0,0,13,589,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,
490,0,0,14,0,0,32,615,0,0,0,0,0,1,0,
505,0,0,15,44,0,4,680,0,0,3,2,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,
532,0,0,16,44,0,4,695,0,0,3,2,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,
559,0,0,17,44,0,4,760,0,0,3,2,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,
586,0,0,18,44,0,4,775,0,0,3,2,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,
613,0,0,19,0,0,32,810,0,0,0,0,0,1,0,
628,0,0,20,101,0,6,993,0,0,1,1,0,1,0,2,3,0,0,
647,0,0,21,49,0,4,1006,0,0,1,0,0,1,0,2,9,0,0,
666,0,0,22,120,0,6,1029,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
689,0,0,23,136,0,6,1039,0,0,2,2,0,1,0,2,9,0,0,1,3,0,0,
};
...
void dclr_result_workload()
{
  ...
  if (d_workgroup.arr[0] != '\0')
  {
   /* EXEC SQL DECLARE RESULT_WORKLOAD CURSOR FOR
     ...
     FROM   RL_TEST_CODE A, RL_RESULT_WORKLOAD_DAILY B,rl_param_for_facility d  
	 ...
	 /o -- from here added by Ooha on 26/11/2020 as per ML-MMOH-CRF-1497 --- o/
     AND NVL (d.dept_restr_qure, 'N') = 'N'
     AND b.operating_facility_id = d.operating_facility_id 
	 /o -- UPTO here added by Ooha on 26/11/2020 as per ML-MMOH-CRF-1497 --- o/	
	 ...
	 /* -- UPTO here added by Ooha on 26/11/2020 as per ML-MMOH-CRF-1497 --- */	
...
TOTAL  from RL_TEST_CODE A ,RL_RESULT_WORKLOAD_DAILY B ,rl_param_for_facilit\
y d where ((((((((A.TEST_CODE=B.TEST_CODE");
  sqlbuft((void **)0, 
    " and B.RESULT_DATE>=TO_DATE(:b0,'DD/MM/YYYY')) and B.RESULT_DATE<(TO_DA\
TE(:b1,'DD/MM/YYYY')+1)) and B.OPERATING_FACILITY_ID=:b2) and A.SECTION_CODE\
 between :b3 and :b4) and NVL(A.STATISTICS_YN,'N')='Y') and NVL(d.dept_restr\
_qure,'N')='N') and b.operating_facility_id=d.operating_facility_id) and A.T\
EST_CODE in (select C.TEST_CODE  from RL_OUTST_WORK_GROUP_DTL C where (C.OPE\
RATING_FACILITY_ID=:b2 and C.OUTST_WORK_GROUP_CODE=nvl(:b6,C.OUTST_WORK_GROU\
P_CODE)))) union select A.SECTION_CODE ,A.TEST_CODE ,A.LONG_DESC ,NVL(A.WORK\
_UNITS,0) ,nvl(a.WORK_UNIT_ADD_FACTOR,0) ,TO_CHAR(B.RESULT_DATE,'DD/MM/YYYY'\
) RESULT_DATE ,TO_CHAR((((((((((((((((((((((((NVL(B.H1,0)+NVL(B.H2,0))+NVL(B\
.H3,0))+NVL(B.H4,0))+NVL(B.H5,0))+NVL(B.H6,0))+NVL(B.H7,0))+NVL(B.H8,0))+NVL\
(B.H9,0))+NVL(B.H10,0))+NVL(B.H11,0))+NVL(B.H12,0))+NVL(B.H13,0))+NVL(B.H14,\
0))+NVL(B.H15,0))+NVL(B.H16,0))+NVL(B.H17,0))+NVL(B.H18,0))+NVL(B.H19,0))+NV\
L(B.H20,0))+NVL(B.H21,0))+NVL(B.H22,0))+NVL(B.H23,0))+NVL(B.H24,0))) DAILY_T\
OTAL ,TO_CHAR((((((((((((((((((((((((NVL(");
  sqlbuft((void **)0, 
    "B.AH1,0)+NVL(B.AH2,0))+NVL(B.AH3,0))+NVL(B.AH4,0))+NVL(B.AH5,0))+NVL(B.\
AH6,0))+NVL(B.AH7,0))+NVL(B.AH8,0))+NVL(B.AH9,0))+NVL(B.AH10,0))+NVL(B.AH11,\
0))+NVL(B.AH12,0))+NVL(B.AH13,0))+NVL(B.AH14,0))+NVL(B.AH15,0))+NVL(B.AH16,0\
))+NVL(B.AH17,0))+NVL(B.AH18,0))+NVL(B.AH19,0))+NVL(B.AH20,0))+NVL(B.AH21,0)\
)+NVL(B.AH22,0))+NVL(B.AH23,0))+NVL(B.AH24,0))) ADD_TOTAL  from RL_TEST_CODE\
 A ,RL_RESULT_WORKLOAD_DAILY B ,rl_param_for_facility d where ((((((((A.TEST\
_CODE=B.TEST_CODE and B.RESULT_DATE>=TO_DATE(:b0,'DD/MM/YYYY')) and B.RESULT\
_DATE<(TO_DATE(:b1,'DD/MM/YYYY')+1)) and B.OPERATING_FACILITY_ID=:b2) and NV\
L(A.STATISTICS_YN,'N')='Y') and NVL(d.dept_restr_qure,'N')='Y') and b.operat\
ing_facility_id=d.operating_facility_id) and a.section_code in (select secti\
on_code  from rl_user_dept where ((user_id=:b10 and operating_facility_id=:b\
2) and section_code between :b3 and :b4))) and A.TEST_CODE in (select C.TEST\
_CODE  from RL_OUTST_WORK_GROUP_DTL C where (C.OPERATING_FACILITY_ID=:b2 and\
 C.OUTST_WORK_GROUP_CODE=nvl(:b6,C.OUTST_");
  sqlstm.stmt = sq0009;
  sqlstm.iters = (unsigned int  )1;
  ...
  sqlstm.sqadto[6] = (unsigned short )0;
  sqlstm.sqtdso[6] = (unsigned short )0;
  sqlstm.sqhstv[7] = (         void  *)&d_date_from;
  ...
  sqlstm.sqadto[7] = (unsigned short )0;
  sqlstm.sqtdso[7] = (unsigned short )0;
  sqlstm.sqhstv[8] = (         void  *)&d_date_to;
  ...
  sqlstm.sqhstv[12] = (         void  *)&d_section_code_fm;
  ...
  sqlstm.sqhstv[13] = (         void  *)&d_section_code_to;
  ...
  sqlstm.sqhstv[15] = (         void  *)&d_workgroup;
...
Y D where (((((((A.TEST_CODE=B.TEST_CODE ");
  sqlbuft((void **)0, 
    "and B.RESULT_DATE>=TO_DATE(:b0,'DD/MM/YYYY')) and B.RESULT_DATE<(TO_DAT\
E(:b1,'DD/MM/YYYY')+1)) and B.OPERATING_FACILITY_ID=:b2) and A.SECTION_CODE \
between :b3 and :b4) and NVL(A.STATISTICS_YN,'N')='Y') and NVL(d.dept_restr_\
qure,'N')='N') and b.operating_facility_id=d.operating_facility_id) union se\
lect A.SECTION_CODE ,A.TEST_CODE ,A.LONG_DESC ,NVL(A.WORK_UNITS,0) ,nvl(a.WO\
RK_UNIT_ADD_FACTOR,0) ,TO_CHAR(B.RESULT_DATE,'DD/MM/YYYY') RESULT_DATE ,TO_C\
HAR((((((((((((((((((((((((NVL(B.H1,0)+NVL(B.H2,0))+NVL(B.H3,0))+NVL(B.H4,0)\
)+NVL(B.H5,0))+NVL(B.H6,0))+NVL(B.H7,0))+NVL(B.H8,0))+NVL(B.H9,0))+NVL(B.H10\
,0))+NVL(B.H11,0))+NVL(B.H12,0))+NVL(B.H13,0))+NVL(B.H14,0))+NVL(B.H15,0))+N\
VL(B.H16,0))+NVL(B.H17,0))+NVL(B.H18,0))+NVL(B.H19,0))+NVL(B.H20,0))+NVL(B.H\
21,0))+NVL(B.H22,0))+NVL(B.H23,0))+NVL(B.H24,0))) DAILY_TOTAL ,TO_CHAR((((((\
((((((((((((((((((NVL(B.AH1,0)+NVL(B.AH2,0))+NVL(B.AH3,0))+NVL(B.AH4,0))+NVL\
(B.AH5,0))+NVL(B.AH6,0))+NVL(B.AH7,0))+NVL(B.AH8,0))+NVL(B.AH9,0))+NVL(B.AH1\
0,0))+NVL(B.AH11,0))+NVL(B.AH12,0))+NVL(B");
  sqlstm.stmt = sq0010;
  sqlstm.iters = (unsigned int  )1;
  ...
  sqlstm.sqadto[4] = (unsigned short )0;
  sqlstm.sqtdso[4] = (unsigned short )0;
  sqlstm.sqhstv[5] = (         void  *)&d_date_from;
  ...
  sqlstm.sqadto[5] = (unsigned short )0;
  sqlstm.sqtdso[5] = (unsigned short )0;
  sqlstm.sqhstv[6] = (         void  *)&d_date_to;
  ...
  sqlstm.sqadto[9] = (unsigned short )0;
  sqlstm.sqtdso[9] = (unsigned short )0;
  sqlstm.sqhstv[10] = (         void  *)&d_section_code_fm;
  ...
  sqlstm.sqadto[10] = (unsigned short )0;
  sqlstm.sqtdso[10] = (unsigned short )0;
  sqlstm.sqhstv[11] = (         void  *)&d_section_code_to;
