...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/DF/DFR00002.pc"
};


static unsigned int sqlctx = 1288451627;
...
") and TO_DATE(((A.PROCESSING_MONTH||'/')||A.PROCESSING_YEAR),'MM/YYYY') betw\
een to_date(:b6,'MM/YYYY') and TO_DATE(:b7,'MM/YYYY')) and A.REVENUE_CATEGORY \
between NVL(:b8,'!!!!!!!!!!') and NVL(:b9,'~~~~~~~~~~')) and A.BLNG_GRP_ID bet\
ween NVL(:b10,'!!') and NVL(:b11,'~~')) and NVL(A.REVENUE_CUST_CODE,'X') betwe\
en NVL(:b12,'!!!!!!!!') and NVL(:b13,'~~~~~~~~')) and A.EPISODE_TYPE=DECODE(:b\
2,'A',A.EPISODE_TYPE,:b2)) group by A.PHYSICIAN_ID,B.SHORT_NAME,DECODE(:b0,'D'\
,(((A.PHYSICIAN_ID||TO_CHAR(A.PROCESSING_YEAR))||'/')||LPAD(TO_CHAR(A.PROCESSI\
NG_MONTH),2,'0')),((((TO_CHAR(A.PROCESSING_YEAR)||'/')||LPAD(TO_CHAR(A.PROCESS\
ING_MONTH),2,'0'))||'/')||A.PHYSICIAN_ID)),((A.PROCESSING_MONTH||'/')||A.PROCE\
SSING_YEAR),A.REVENUE_CATEGORY,C.SHORT_DESC,DECODE(:b1,'Y',DECODE(A.EPISODE_TY\
PE,'I','Inpatient','D','Daycare','O','Outpatient','E','Emergency','R','Externa\
l'),DECODE(:b2,'I','Inpatient','D','Daycare','O','Outpatient','E','Emergency',\
'R','External','A','   All')) order by 3            ";
...
{12,4274,178,0,0,
5,0,0,1,91,0,2,171,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
28,0,0,2,0,0,30,181,0,0,0,0,0,1,0,
43,0,0,0,0,0,27,193,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
74,0,0,4,287,0,4,237,0,0,17,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
1,9,0,0,1,9,0,0,
157,0,0,5,2010,0,9,354,0,0,19,19,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
248,0,0,5,0,0,15,368,0,0,0,0,0,1,0,
263,0,0,5,0,0,13,407,0,0,10,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,3,0,0,2,4,0,0,2,4,0,0,
318,0,0,6,128,0,4,1506,0,0,4,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,
349,0,0,7,96,0,4,1586,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
372,0,0,8,165,0,6,1593,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
};
...
/* OCS MEDICOM VER 2.0                                                  */
/************************************************************************/
/* PROGRAM NAME          : DFR00002.PC                                  */
/* CALLED FROM           : DFR00002.FMB                                 */
/*                                                                      */
/************************************************************************/
...
struct { unsigned short len; unsigned char arr[6]; } fr_blng_grp;

struct { unsigned short len; unsigned char arr[6]; } to_blng_grp;
...
struct { unsigned short len; unsigned char arr[100]; } leg6_leg13;
...
double  d_hosp_reve, d_dr_reve ,
		...
		grand_tot_dr_reve, dr_rev_perc = 0;
...

{
 ...
 sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='DFR00002' and SE\
...
RAM7 ,PARAM8 ,PARAM9 ,PARAM10 ,PARAM11 ,PARAM12 ,PARAM13 ,PARAM14 ,PARAM15 int\
o :b0,:b1,:b2,:b3,:b4,:b5,:b6,:b7,:b8,:b9,:b10,:b11,:b12,:b13,:b14  from SY_PR\
OG_PARAM where ((PGM_ID='DFR00002' and SESSION_ID=:b15) and PGM_DATE=:b16)";
    sqlstm.iters = (unsigned int  )1;
    ...
    sqlstm.sqhstv[12] = (         void  *)&nd_sort_order;
    ...
    sqlstm.sqhstv[13] = (         void  *)&nd_repeat_grp;
    ...
    sqlstm.sqhstv[14] = (         void  *)&nd_percent_col;
...
declare_cursor()
{
   ...
   SELECT A.PHYSICIAN_ID, 
          B.SHORT_NAME,
		  DECODE(:nd_sort_order,'D',A.PHYSICIAN_ID||TO_CHAR(A.PROCESSING_YEAR)||'/'||LPAD(TO_CHAR(A.PROCESSING_MONTH),2,'0'),TO_CHAR(A.PROCESSING_YEAR)||'/'||LPAD(TO_CHAR(A.PROCESSING_MONTH),2,'0')||'/'||A.PHYSICIAN_ID) MTH_YR_PHY,
		  A.PROCESSING_MONTH||'/'||A.PROCESSING_YEAR MTH_YR, 
		  A.REVENUE_CATEGORY, 
		  C.SHORT_DESC REV_CATG_DESC,
          -- DECODE(:nd_brk_episode_type, 'Y', DECODE(A.EPISODE_TYPE , 'I' , 'Inpatient' , 'D' , 'Daycare' , 'O' , 'Outpatient' , 'E' , 'Emergency' , 'R' , 'External' ), 'ALL') EPISODE_TYPE , 
...
AND    A.OPERATING_FACILITY_ID =:lv_facility_id
AND    A.PHYSICIAN_ID = B.PHYSICIAN_ID
AND    A.REVENUE_CATEGORY = C.REVENUE_CATEGORY
AND    A.PHYSICIAN_ID BETWEEN NVL(:fr_doctor, '!!!!!!!!!!!!!!!') 
                      AND NVL(:to_doctor, '~~~~~~~~~~~~~~~')
AND    TO_DATE(A.PROCESSING_MONTH||'/'||A.PROCESSING_YEAR , 'MM/YYYY') 
        BETWEEN to_date(:fr_period , 'MM/YYYY') AND TO_DATE(:to_period , 'MM/YYYY')
...
AND    NVL(A.REVENUE_CUST_CODE, 'X') BETWEEN NVL(:fr_revenu_cust, '!!!!!!!!') 
...
GROUP BY A.PHYSICIAN_ID, 
         B.SHORT_NAME,
		 DECODE(:nd_sort_order,'D',A.PHYSICIAN_ID||TO_CHAR(A.PROCESSING_YEAR)||'/'||LPAD(TO_CHAR(A.PROCESSING_MONTH),2,'0'),TO_CHAR(A.PROCESSING_YEAR)||'/'||LPAD(TO_CHAR(A.PROCESSING_MONTH),2,'0')||'/'||A.PHYSICIAN_ID),
         A.PROCESSING_MONTH||'/'||A.PROCESSING_YEAR, 
		 A.REVENUE_CATEGORY, 
		 C.SHORT_DESC, 
         -- DECODE(:nd_brk_episode_type, 'Y', DECODE(A.EPISODE_TYPE , 'I' , 'Inpatient' , 'D' , 'Daycare' , 'O' , 'Outpatient' , 'E' , 'Emergency' , 'R' , 'External'), 'ALL')
...

{
 ...
 sqlstm.sqltdsp = &sqltds;
 sqlbuft((void **)0, 
   "select A.PHYSICIAN_ID ,B.SHORT_NAME ,DECODE(:b0,'D',(((A.PHYSICIAN_ID||T\
O_CHAR(A.PROCESSING_YEAR))||'/')||LPAD(TO_CHAR(A.PROCESSING_MONTH),2,'0')),(\
(((TO_CHAR(A.PROCESSING_YEAR)||'/')||LPAD(TO_CHAR(A.PROCESSING_MONTH),2,'0')\
)||'/')||A.PHYSICIAN_ID)) MTH_YR_PHY ,((A.PROCESSING_MONTH||'/')||A.PROCESSI\
NG_YEAR) MTH_YR ,A.REVENUE_CATEGORY ,C.SHORT_DESC REV_CATG_DESC ,DECODE(:b1,\
'Y',DECODE(A.EPISODE_TYPE,'I','Inpatient','D','Daycare','O','Outpatient','E'\
,'Emergency','R','External'),DECODE(:b2,'I','Inpatient','D','Daycare','O','O\
utpatient','E','Emergency','R','External','A','   All')) EPISODE_TYPE ,sum(N\
VL(A.SERV_QTY,0)) QTY ,sum(NVL(A.NET_REVENUE_AMT,0)) HOSP_REV ,sum(NVL(A.DR_\
REVENUE_AMT,0)) DR_REV  from df_rev_folio_slmt_vw A ,SY_PHYSICIAN_MAST B ,DF\
_REVENUE_CATEGORY C where (((((((((A.OPERATING_FACILITY_ID=B.FACILITY_ID and\
 A.OPERATING_FACILITY_ID=:b3) and A.PHYSICIAN_ID=B.PHYSICIAN_ID) and A.REVEN\
UE_CATEGORY=C.REVENUE_CATEGORY) and A.PHYSICIAN_ID between NVL(:b4,'!!!!!!!!\
!!!!!!!') and NVL(:b5,'~~~~~~~~~~~~~~~')");
 sqlstm.stmt = sq0005;
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqhstv[0] = (         void  *)&nd_sort_order;
 ...
 sqlstm.sqadto[0] = (unsigned short )0;
 sqlstm.sqtdso[0] = (unsigned short )0;
 sqlstm.sqhstv[1] = (         void  *)&nd_brk_episode_type;
 ...
 sqlstm.sqadto[3] = (unsigned short )0;
 sqlstm.sqtdso[3] = (unsigned short )0;
 sqlstm.sqhstv[4] = (         void  *)&fr_doctor;
 ...
 sqlstm.sqadto[4] = (unsigned short )0;
 sqlstm.sqtdso[4] = (unsigned short )0;
 sqlstm.sqhstv[5] = (         void  *)&to_doctor;
 ...
 sqlstm.sqadto[5] = (unsigned short )0;
 sqlstm.sqtdso[5] = (unsigned short )0;
 sqlstm.sqhstv[6] = (         void  *)&fr_period;
 ...
 sqlstm.sqadto[6] = (unsigned short )0;
 sqlstm.sqtdso[6] = (unsigned short )0;
 sqlstm.sqhstv[7] = (         void  *)&to_period;
 ...
 sqlstm.sqadto[7] = (unsigned short )0;
 sqlstm.sqtdso[7] = (unsigned short )0;
 sqlstm.sqhstv[8] = (         void  *)&fr_revenu_cat;
 ...
 sqlstm.sqadto[8] = (unsigned short )0;
 sqlstm.sqtdso[8] = (unsigned short )0;
 sqlstm.sqhstv[9] = (         void  *)&to_revenu_cat;
 ...
 sqlstm.sqadto[9] = (unsigned short )0;
 sqlstm.sqtdso[9] = (unsigned short )0;
 sqlstm.sqhstv[10] = (         void  *)&fr_blng_grp;
 ...
 sqlstm.sqadto[10] = (unsigned short )0;
 sqlstm.sqtdso[10] = (unsigned short )0;
 sqlstm.sqhstv[11] = (         void  *)&to_blng_grp;
 ...
 sqlstm.sqadto[11] = (unsigned short )0;
 sqlstm.sqtdso[11] = (unsigned short )0;
 sqlstm.sqhstv[12] = (         void  *)&fr_revenu_cust;
 ...
 sqlstm.sqadto[12] = (unsigned short )0;
 sqlstm.sqtdso[12] = (unsigned short )0;
 sqlstm.sqhstv[13] = (         void  *)&to_revenu_cust;
 ...
 sqlstm.sqadto[15] = (unsigned short )0;
 sqlstm.sqtdso[15] = (unsigned short )0;
 sqlstm.sqhstv[16] = (         void  *)&nd_sort_order;
 ...
 sqlstm.sqadto[16] = (unsigned short )0;
 sqlstm.sqtdso[16] = (unsigned short )0;
 sqlstm.sqhstv[17] = (         void  *)&nd_brk_episode_type;
 ...
 sqlstm.sqhstv[18] = (         void  *)&nd_episode_type;
...

{
 ...
 sqlstm.sqadto[1] = (unsigned short )0;
 sqlstm.sqtdso[1] = (unsigned short )0;
 sqlstm.sqhstv[2] = (         void  *)&d_sort_order_mth_yr_phy;
 ...
 sqlstm.sqadto[2] = (unsigned short )0;
 sqlstm.sqtdso[2] = (unsigned short )0;
 sqlstm.sqhstv[3] = (         void  *)&d_process_mth_year;
 ...
 sqlstm.sqadto[3] = (unsigned short )0;
 sqlstm.sqtdso[3] = (unsigned short )0;
 sqlstm.sqhstv[4] = (         void  *)&d_revenue_category;
 sqlstm.sqhstl[4] = (unsigned int  )13;
 sqlstm.sqhsts[4] = (         int  )0;
 sqlstm.sqindv[4] = (         void  *)0;
 sqlstm.sqinds[4] = (         int  )0;
 ...
 sqlstm.sqadto[4] = (unsigned short )0;
 sqlstm.sqtdso[4] = (unsigned short )0;
 sqlstm.sqhstv[5] = (         void  *)&d_revenue_cate_desc;
 sqlstm.sqhstl[5] = (unsigned int  )18;
 sqlstm.sqhsts[5] = (         int  )0;
 sqlstm.sqindv[5] = (         void  *)0;
 sqlstm.sqinds[5] = (         int  )0;
 ...
 sqlstm.sqadto[7] = (unsigned short )0;
 sqlstm.sqtdso[7] = (unsigned short )0;
 sqlstm.sqhstv[8] = (         void  *)&d_hosp_reve;
 ...
 sqlstm.sqadto[8] = (unsigned short )0;
 sqlstm.sqtdso[8] = (unsigned short )0;
 sqlstm.sqhstv[9] = (         void  *)&d_dr_reve;
...
open_file()
{
	...
	strcat(filename,"DFR00002.lis");

    if ((fp = fopen(filename,"w")) == NULL)
    {
       disp_message(ERR_MESG,"Error in opening file DFR00002.lis");
       proc_exit();
    }
}
...
print_head();
  ...
  if ((strcmp(nd_sort_order.arr, "D") ==0)&& (strcmp(nd_brk_episode_type.arr, "Y") ==0))
       process_for_doctor();
  
  if ((strcmp(nd_sort_order.arr, "D") ==0)&& (strcmp(nd_brk_episode_type.arr, "N") ==0))
	  process_for_doctor_no_brk();
	  
	  	
 if ((strcmp(nd_sort_order.arr, "P") ==0) && (strcmp(nd_brk_episode_type.arr, "Y") ==0))
       process_for_mth_yr();
 if ((strcmp(nd_sort_order.arr, "P") ==0) && (strcmp(nd_brk_episode_type.arr, "N") ==0))
	  process_for_mth_yr_no_brk();
...
process_for_doctor()
{
	...
	check_line(1);
		...
		if ((strcmp(temp_revenue_category.arr,d_revenue_category.arr)!=0)  || (strcmp(temp_process_mth_year.arr,d_process_mth_year.arr)!=0)  || (strcmp(temp_physician_id.arr,d_physician_id.arr)!=0))
		{
			if (first == 1) 
			{ 
				print_rev_sub_tot();
		...
		if((strcmp(temp_process_mth_year.arr,d_process_mth_year.arr)!=0)  || (strcmp(temp_physician_id.arr,d_physician_id.arr)!=0))
		{
			if (first == 1)
			{
				print_month_sub_tot();
		...
		strcpy(temp_process_mth_year.arr,d_process_mth_year.arr);
		...
		sub_tot_mth_yr_hosp_reve = sub_tot_mth_yr_hosp_reve + d_hosp_reve ;
		sub_tot_mth_yr_dr_reve   =  sub_tot_mth_yr_dr_reve + d_dr_reve;
...
process_for_mth_yr()
{
	...
	{
		check_line(1);
		if ((strcmp(temp_revenue_category.arr,d_revenue_category.arr)!=0)  || (strcmp(temp_physician_id.arr,d_physician_id.arr)!=0) || (strcmp(temp_process_mth_year.arr,d_process_mth_year.arr)!=0))
		{
			if (first == 1) 
			{ 
				print_rev_sub_tot();
		...
		if((strcmp(temp_physician_id.arr,d_physician_id.arr)!=0) || (strcmp(temp_process_mth_year.arr,d_process_mth_year.arr)!=0) )
		{
			if (first == 1)
			{
				print_doc_sub_tot();
		...
		if (strcmp(temp_process_mth_year.arr,d_process_mth_year.arr)!=0)
	    {
			if (first == 1)
			{
				print_month_sub_tot();
			...
			{
				next_page();
				fprintf(fp,"%-10s " ,  d_process_mth_year.arr);
...
print_rev_sub_tot() 
{
	...
	fprintf(fp , "%52s Revenue Category - %-10s - Total  " , " " ,temp_revenue_category.arr) ;
...
{
   check_line(3);
   if ((strcmp(nd_brk_episode_type.arr, "N") ==0) &&(strcmp(nd_sort_order.arr, "D") ==0))
   {
		horz_skip(69) ;
 		if(strcmp(nd_percent_col.arr,"Y")==0)	
			print_line('-',79);
	...
	fprintf(fp , "%45s Month/Year  - %-7s - Total " , " " ,temp_process_mth_year.arr) ;
	fprintf(fp , "%21s" , " ");
	print_formated(sub_tot_mth_yr_hosp_reve);
	fprintf(fp , "%2s"," ");
	print_formated(sub_tot_mth_yr_dr_reve);
    fprintf(fp, "\n");
    ...
    sub_tot_mth_yr_hosp_reve = 0;
	sub_tot_mth_yr_dr_reve = 0;
...
check_line(3);
	 ...
	 if ((strcmp(nd_brk_episode_type.arr, "N") ==0) &&(strcmp(nd_sort_order.arr, "P") ==0))
	 {
		horz_skip(69) ;
		if(strcmp(nd_percent_col.arr,"Y")==0)	
		    print_line('-',79);
	...
	if (strcmp(nd_sort_order.arr, "P") ==0)
	{
	    fprintf(fp , "%49s Doctor  - %-15s - Total " , " " ,temp_physician_id.arr) ;
		fprintf(fp , "%13s" , " ");
	...
	{
		fprintf(fp , "%41s Doctor  - %-15s - Total " , " " ,temp_physician_id.arr) ;
		fprintf(fp , "%21s" , " ");
		print_formated(doct_tot_hosp_reve);
		fprintf(fp , "%2s"," ");
		print_formated(doct_tot_dr_reve);
...
lctr=0;
  ...
  fprintf(fp, "\n");
	...
	fprintf(fp,"%-4s: %s",loc_legend[3],"DFR00002");
  ...
  // leg6_leg13.arr[0]  = '\0';
   ...
   //strcpy(leg6_leg13.arr,strcat(loc_legend[6],loc_legend[34]));

   //leg6_leg13.arr[leg6_leg13.len] ='\0';
 


   fprintf(fp,"%s %s ",loc_legend[6],loc_legend[34]);

      
	if (strcmp(nd_sort_order.arr, "D") ==0)
...
{
fprintf(fp,"%24s  %-7s  : %4.4s",loc_legend[14],loc_legend[26],fr_blng_grp.arr);
}
...
fprintf(fp,"%24s  %-7s  : %4.4s",loc_legend[14],loc_legend[27],to_blng_grp.arr);
...
if (strcmp(nd_sort_order.arr,"D")==0)
fprintf(fp,"%33s  : %s",loc_legend[18],loc_legend[19]);
else
fprintf(fp,"%33s  : %s",loc_legend[18],loc_legend[12]);
...
if (strcmp(nd_sort_order.arr, "D") ==0)
{
    if(strcmp(nd_percent_col.arr,"Y")==0)	
	    {
		  fprintf(fp , "%-29s %-11s %-26s %-14s%8s %20s %17s %17s",loc_legend[36],loc_legend[20],loc_legend[34],loc_legend[35],loc_legend[21],loc_legend[22],loc_legend[23],loc_legend[37]);
		 } 
    else
	    {
		  fprintf(fp , "%-29s %-11s %-26s %-14s%8s %20s %17s",loc_legend[36],loc_legend[20],loc_legend[34],loc_legend[35],loc_legend[21],loc_legend[22],loc_legend[23]);
         }  
}
else
{
    if(strcmp(nd_percent_col.arr,"Y")==0)
       {
	     fprintf(fp , "%-10s %-32s  %-26s %-13s%8s %18s  %16s  %16s",loc_legend[20],loc_legend[36],loc_legend[34],loc_legend[35],loc_legend[21],loc_legend[22],loc_legend[23],loc_legend[37]);
        }
    else
	   {
	    fprintf(fp , "%-10s %-32s  %-26s %-13s%8s %18s  %16s",loc_legend[20],loc_legend[36],loc_legend[34],loc_legend[35],loc_legend[21],loc_legend[22],loc_legend[23]); 
		}
	//fprintf(fp , "Mth/Yr  Doctor                            Revenue Category           Episode Type    Qty    Hospital Revenue Amt  Doctor Share Amt");
}
...
fetch_legend_value()
{
	for(i=1;i<=300;i++)
	{
		...
		/* EXEC SQL SELECT LTRIM(RTRIM('DFR00002.LEGEND_'||LTRIM(RTRIM(TO_CHAR(:i,'009')))))
		           INTO :l_pk_value
				   FROM dual; */ 

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('DFR00002.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
