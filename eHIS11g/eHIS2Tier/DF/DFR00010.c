...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/DF/DFR00010.pc"
};
...
(a.processing_year||TO_CHAR(a.processing_month,'FM00')) between (:b1||TO_CHAR(\
:b2,'FM00')) and (:b3||TO_CHAR(:b4,'FM00'))) and a.physician_id between NVL(:b\
5,'!!!!!!!!!!!!!!!') and NVL(:b6,'~~~~~~~~~~~~~~~')) and b.facility_id=a.opera\
ting_facility_id) and a.physician_id=b.physician_id) order by a.processing_yea\
r,a.processing_month,a.physician_id,a.contract_ref,a.contract_period_from     \
       ";
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select TO_NUMBER(PARAM1) ,TO_NUMBER(PARAM2) ,TO_NUMBER(PA\
RAM3) ,TO_NUMBER(PARAM4) ,PARAM5 ,PARAM6 into :b0,:b1,:b2,:b3,:b4,:b5  from SY\
_PROG_PARAM where ((PGM_ID=:b6 and SESSION_ID=:b7) and PGM_DATE=:b8)";
    sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('DFR00010.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
