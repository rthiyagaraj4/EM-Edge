...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR00083.PC"
};


static unsigned int sqlctx = 1288460979;
...
"select A.FACILITY_ID ,A.EPISODE_TYPE ,A.EPISODE_ID ,A.VISIT_ID ,A.PATIENT_ID\
 ,NVL(B.NATIONALITY_CODE,'zzzz') ,B.LEGAL_YN ,DECODE(C.VISIT_TYPE_IND,'F','F',\
'L')  from OP_VISIT A ,MP_PATIENT B ,OP_VISIT_TYPE C where (((((((A.FACILITY_I\
D=NVL(:b0,A.FACILITY_ID) and A.VISIT_REGN_DATE_TIME>=to_date(:b1,'dd/mm/yyyy h\
h24:mi:ss')) and A.VISIT_REGN_DATE_TIME<to_date(:b2,'dd/mm/yyyy hh24:mi:ss')) \
and NVL(A.VISIT_STATUS,'X')<>'9') and A.PATIENT_ID=B.PATIENT_ID) and A.VISIT_T\
YPE_CODE=C.VISIT_TYPE_CODE) and NVL(A.EPISODE_TYPE,'O')=DECODE(:b3,'O','O','E'\
,'E',NVL(A.EPISODE_TYPE,'O'))) and NVL(B.CITIZEN_YN,'N')=DECODE(:b4,'C','Y','N\
','N',NVL(B.CITIZEN_YN,'N'))) order by A.FACILITY_ID,NVL(B.NATIONALITY_CODE,'z\
zzz'),B.LEGAL_YN,DECODE(C.VISIT_TYPE_IND,'F','F','L')            ";

 static const char *sq0005 = 
"select A.FACILITY_ID ,A.EPISODE_TYPE ,A.EPISODE_ID ,A.PATIENT_ID ,NVL(B.NATI\
ONALITY_CODE,'zzzz') ,B.LEGAL_YN ,'F'  from IP_EPISODE A ,MP_PATIENT B where (\
(((((A.FACILITY_ID=NVL(:b0,A.FACILITY_ID) and A.ADMISSION_DATE_TIME>=to_date(:\
b1,'dd/mm/yyyy hh24:mi:ss')) and A.ADMISSION_DATE_TIME<to_date(:b2,'dd/mm/yyyy\
 hh24:mi:ss')) and A.PATIENT_ID=B.PATIENT_ID) and NVL(A.EPISODE_STATUS,'X')<>'\
9') and NVL(A.EPISODE_TYPE,'I')=DECODE(:b3,'I','I','D','D',NVL(A.EPISODE_TYPE,\
'I'))) and NVL(B.CITIZEN_YN,'N')=DECODE(:b4,'C','Y','N','N',NVL(B.CITIZEN_YN,'\
N'))) order by A.FACILITY_ID,NVL(B.NATIONALITY_CODE,'zzzz'),B.LEGAL_YN        \
    ";
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,142,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...
RAM7 ,PARAM8 ,PARAM9 ,PARAM10 ,PARAM11 into :b0,:b1,:b2,:b3,:b4,:b5,:b6,:b7,:b\
8,:b9,:b10  from SY_PROG_PARAM where ((PGM_ID=:b11 and SESSION_ID=:b12) and PG\
M_DATE=:b13)";
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select sum(NVL(DEBIT_AMT,0)) into :b0  from BL_PATIENT_STAT\
EMENT_VW A where ((((((A.EPISODE_TYPE=DECODE(:b1,'O','O','E','E',A.EPISODE_TYP\
E) and A.EPISODE_ID=:b2) and A.VISIT_ID=:b3) and A.PATIENT_ID=:b4) and (((:b5=\
'P' and A.DOC_DATE>=to_date(:b6,'dd/mm/yyyy hh24:mi:ss')) and A.DOC_DATE<to_da\
te(:b7,'dd/mm/yyyy hh24:mi:ss')) or :b5='A')) and TRX_TYPE in ('CANC-RECEIPT',\
'RECEIPT')) and TRX_NATURE_CODE in ('AD','PR'))";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select sum(NVL(DEBIT_AMT,0)) into :b0  from BL_PATIENT_STAT\
EMENT_VW A where (((((A.EPISODE_TYPE=DECODE(:b1,'I','I','D','D',A.EPISODE_TYPE\
) and A.EPISODE_ID=:b2) and A.PATIENT_ID=:b3) and (((:b4='P' and A.DOC_DATE>=t\
o_date(:b5,'dd/mm/yyyy hh24:mi:ss')) and A.DOC_DATE<to_date(:b6,'dd/mm/yyyy hh\
24:mi:ss')) or :b4='A')) and TRX_TYPE in ('CANC-RECEIPT','RECEIPT')) and TRX_N\
ATURE_CODE in ('AD','PR'))";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select sum(NVL(DEBIT_AMT,0)) into :b0  from BL_PATIENT_STAT\
EMENT_VW A where (((((A.EPISODE_TYPE=DECODE(:b1,'O','O','E','E',A.EPISODE_TYPE\
) and A.EPISODE_ID=:b2) and A.VISIT_ID=:b3) and A.PATIENT_ID=:b4) and (((:b5='\
P' and A.DOC_DATE>=to_date(:b6,'dd/mm/yyyy hh24:mi:ss')) and A.DOC_DATE<to_dat\
e(:b7,'dd/mm/yyyy hh24:mi:ss')) or :b5='A')) and TRX_TYPE in ('BILL','CANC-BIL\
L'))";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.stmt = "select sum(NVL(DEBIT_AMT,0)) into :b0  from BL_PATIENT_STAT\
EMENT_VW A where ((((A.EPISODE_TYPE=DECODE(:b1,'I','I','D','D',A.EPISODE_TYPE)\
 and A.EPISODE_ID=:b2) and A.PATIENT_ID=:b3) and (((:b4='P' and A.DOC_DATE>=to\
_date(:b5,'dd/mm/yyyy hh24:mi:ss')) and A.DOC_DATE<to_date(:b6,'dd/mm/yyyy hh2\
4:mi:ss')) or :b4='A')) and TRX_TYPE in ('BILL','CANC-BILL'))";
  sqlstm.iters = (unsigned int  )1;
...
P' and A.DOC_DATE>=to_date(:b6,'dd/mm/yyyy hh24:mi:ss')) and A.DOC_DATE<to_dat\
e(:b7,'dd/mm/yyyy hh24:mi:ss')) or :b5='A')) and TRX_NATURE_CODE in ('BI','DC'\
))";
  sqlstm.iters = (unsigned int  )1;
...
EMENT_VW A where ((((A.EPISODE_TYPE=DECODE(:b1,'I','I','D','D',A.EPISODE_TYPE)\
 and A.EPISODE_ID=:b2) and A.PATIENT_ID=:b3) and (((:b4='P' and A.DOC_DATE>=to\
_date(:b5,'dd/mm/yyyy hh24:mi:ss')) and A.DOC_DATE<to_date(:b6,'dd/mm/yyyy hh2\
4:mi:ss')) or :b4='A')) and TRX_NATURE_CODE in ('BI','DC'))";
  sqlstm.iters = (unsigned int  )1;
...
P' and A.DOC_DATE>=to_date(:b6,'dd/mm/yyyy hh24:mi:ss')) and A.DOC_DATE<to_dat\
e(:b7,'dd/mm/yyyy hh24:mi:ss')) or :b5='A')) and TRX_TYPE in ('ADJ-BILL','CANC\
-ADJ-BILL','EXM-BILL','CANC-EXM-BILL','WRITEOFF','CANC-WRITEOFF','DCP-ADJUST')\
)";
  sqlstm.iters = (unsigned int  )1;
...
EMENT_VW A where ((((A.EPISODE_TYPE=DECODE(:b1,'I','I','D','D',A.EPISODE_TYPE)\
 and A.EPISODE_ID=:b2) and A.PATIENT_ID=:b3) and (((:b4='P' and A.DOC_DATE>=to\
_date(:b5,'dd/mm/yyyy hh24:mi:ss')) and A.DOC_DATE<to_date(:b6,'dd/mm/yyyy hh2\
4:mi:ss')) or :b4='A')) and TRX_TYPE in ('ADJ-BILL','CANC-ADJ-BILL','EXM-BILL'\
,'CANC-EXM-BILL','WRITEOFF','CANC-WRITEOFF','DCP-ADJUST'))";
  sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select SHORT_NAME into :b0  from MP_COUNTRY where COUNTRY_CO\
DE=:b1";
 sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00083.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
