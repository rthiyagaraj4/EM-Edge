...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR00009.pc"
};
...
"select b.ln1_add ,b.ln2_add ,b.ln3_add ,b.tel_num ,TO_CHAR(sysdate,'dd/mm/yy\
yy') ,a.payer_name ,TO_CHAR(a.doc_date,'dd/mm/yyyy') ,c.short_desc ,b.base_cur\
rency ,a.doc_type_code ,a.doc_number ,a.doc_amt ,a.slmt_doc_ref_desc ,TO_CHAR(\
a.slmt_doc_ref_date,'dd/mm/yyyy') ,a.patient_id ,a.doc_srno  from bl_receipt_r\
efund_HDRdtl_VW a ,sy_acc_entity b ,bl_slmt_type c where (((((((a.operating_fa\
cility_id=b.acc_entity_id and b.acc_entity_code=c.acc_entity_code) and b.acc_e\
ntity_id=:b0) and a.slmt_type_code=c.slmt_type_code) and a.episode_type=:b1) a\
nd a.DOC_TYPE_CODE between :b2 and NVL(:b3,:b2)) and a.DOC_NUMBER between :b5 \
and NVL(:b6,:b5)) and trunc(doc_date) between NVL(TO_DATE(:b8,'DD/MM/YYYYHH24:\
MI:SS'),TO_DATE('01/01/100000:00:00','DD/MM/YYYYHH24:MI:SS')) and NVL(TO_DATE(\
:b9,'DD/MM/YYYYHH24:MI:SS'),TO_DATE('01/01/400023:59:59','DD/MM/YYYYHH24:MI:SS\
'))) order by 10            ";
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select substr(:b0,0,(instr(:b0,'/')-1)) into :b2  from dual\
 ";
  sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "begin TRACE_DURATION_BL ( :lv_pgm_id , 'Acknowledgement Lett\
er for Receipt Generation' , :P_START_TIME , :uid ) ; END ;";
 sqlstm.iters = (unsigned int  )1;
...
m sy_prog_param where ((pgm_id=:b9 and PGM_DATE=:b10) and SESSION_ID=:b11)";
 sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select acc_entity_name into :b0  from sy_acc_entity where a\
cc_entity_id=:b1";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select acc_entity_name_loc_lang into :b0  from sy_acc_entit\
y where acc_entity_id=:b1";
  sqlstm.iters = (unsigned int  )1;
...
sqlstm.sqladtp = &sqladt;
sqlstm.sqltdsp = &sqltds;
sqlstm.stmt = "select upper(ADDR_LINE1) ,UPPER(ADDR_LINE2) ,UPPER(ADDR_LINE3\
) ,UPPER(ADDR_LINE4) ,UPPER(ADDR_POSTAL_CODE) ,RES_AREA_CODE ,RES_TOWN_CODE ,R\
EGION_CODE into :b0,:b1,:b2,:b3,:b4,:b5,:b6,:b7  from SM_FACILITY_PARAM where \
FACILITY_ID=:b8";
sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select UPPER(LONG_DESC) into :b0  from MP_REGION where REGIO\
N_CODE=:b1";
 sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select UPPER(LONG_DESC) into :b0  from MP_RES_TOWN where RES\
_TOWN_CODE=:b1";
 sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select UPPER(LONG_DESC) into :b0  from MP_RES_AREA where RES\
_AREA_CODE=:b1";
 sqlstm.iters = (unsigned int  )1;
...
where (((operating_facility_id=:b1 and episode_type=:b2) and PATIENT_ID=:b3) a\
nd (doc_type_code,doc_num) in (select all_doc_type_code ,all_doc_num  from bl_\
patient_ledger where (((operating_facility_id=:b1 and episode_type=:b2) and or\
g_doc_type_code=:b6) and org_doc_num=:b7)))";
   sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sm_lang_legend where legend_id like ('MLBLR00009.LEGEND_'||:b1)";
  sqlstm.iters = (unsigned int  )1;
...
end_id like ('MLBLR00009.LEGEND_'||:b1)) and language_id=:b2)";
  sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "begin TRACE_DURATION_BL ( :lv_pgm_id , 'Acknowledgement Lett\
er for Receipt Printing' , :P_START_TIME , :uid ) ; END ;";
 sqlstm.iters = (unsigned int  )1;
