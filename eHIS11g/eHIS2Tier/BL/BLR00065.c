...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR00065.PC"
};


static unsigned int sqlctx = 1288460963;
...
"ode_type not  in ('I','D') and B.Episode_type=a.Episode_type) and B.Episode_\
type=:b4) or (B.Episode_type in ('I','D') and (a.episode_type,a.episode_id) in\
 (select episode_type ,episode_id  from ip_episode where (trunc(DISCHARGE_DATE\
_TIME) between TRUNC(TO_DATE(nvl(:b5,to_char((sysdate-36500),'DD/MM/YYYY')),'D\
D/MM/YYYY')) and TRUNC(TO_DATE(nvl(:b6,to_char(sysdate,'DD/MM/YYYY')),'DD/MM/Y\
YYY')) and Episode_type=:b4)))) or a.EPISODE_TYPE=decode(:b4,'A',a.episode_typ\
e,'X'))) and a.PATIENT_ID=d.PATIENT_ID) and a.EXT_ACCT_CODE=NVL(:b9,a.EXT_ACCT\
_CODE)) and NVL(a.CANCELLED_IND,'N')<>'Y') order by a.ext_acct_code,a.patient_\
id,((((a.doc_type_code||'-')||to_char(a.doc_number))||'-')||to_char(a.doc_srno\
))            ";

 static const char *sq0005 = 
"select ((DOC_TYPE_CODE||'-')||to_char(DOC_NUMBER)) refund_doc ,to_char(DOC_D\
ATE,'DD/MM/YYYY') refund_date ,ABS(DOC_AMT) REFUND_AMT  from BL_RECEIPT_REFUND\
_HDRDTL_VW where ((RECPT_REFUND_IND='F' and ((BILL_DOC_TYPE_CODE||'-')||TO_CHA\
R(BILL_DOC_NUMBER))=:b0) and NVL(CANCELLED_IND,'N')<>'Y') union select ((DOC_T\
YPE_CODE||'-')||to_char(DOC_NUMBER)) refund_doc ,to_char(DOC_DATE,'DD/MM/YYYY'\
) refund_date ,ABS(DOC_AMT) REFUND_AMT  from BL_RECEIPT_REFUND_HDRDTL_VW where\
 ((RECPT_REFUND_IND='F' and ((BILL_DOC_TYPE_CODE||'-')||TO_CHAR(BILL_DOC_NUMBE\
R))=:b1) and NVL(CANCELLED_IND,'N')<>'Y') union select null  ,null  ,abs(bill_\
tot_outst_amt)  from bl_bill_hdr where ((((doc_type_code||'-')||To_char(doc_nu\
m))=:b1 and bill_tot_outst_amt<0) and NVL(Bill_status,'X')<>'C') order by 1   \
         ";
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,114,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00065.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
