...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/Blrbouls.pc"
};
...
"select SLMT_DOC_REF_DESC ,TO_CHAR(SLMT_DOC_REF_DATE,'DD/MM/YYYY') ,SLMT_DOC_\
REMARKS ,TO_CHAR(DOC_AMT,'999,999,990.99') ,DOC_AMT ,PATIENT_ID ,EPISODE_TYPE \
,LPAD(TO_CHAR(EPISODE_ID),8,'0') ,TO_CHAR(VISIT_ID) ,DOC_TYPE_CODE ,TO_CHAR(DO\
C_NUMBER) ,TO_CHAR(DOC_DATE,'DD/MM/YYYY')  from BL_RECEIPT_REFUND_HDRDTL_VW wh\
ere ((((RECPT_REFUND_IND='R' and PATIENT_ID between NVL(:b0,'!!!!!!!!!!') and \
NVL(:b0,'~~~~~~~~~~')) and NVL(BOUNCED_YN,'N')='Y') and BOUNCED_DATE between T\
O_DATE(NVL(:b2,'01/01/1000'),'DD/MM/YYYY') and TO_DATE(NVL(:b3,'31/12/4712'),'\
DD/MM/YYYY')) and operating_facility_id=:b4) order by PATIENT_ID            ";
...
:b0,:b1,:b2,:b3  from SY_PROG_PARAM where ((PGM_ID='BLRBOULS' and PGM_DATE=:b4\
) and SESSION_ID=TO_NUMBER(:b5))";
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select ACC_ENTITY_NAME ,TO_CHAR(SYSDATE,'DD/MM/YYYY') ,TO\
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='BLRBOULS' and\
...
str2 ) ; IF str2 IS NOT NULL THEN :h_patient_name := str2 ; END IF ; blcommon \
. split_words ( :d_short_name_full , 30 , '*' , 2 , str1 , str2 ) ; IF str2 IS\
 NOT NULL THEN :h_patient_name1 := str2 ; END IF ; blcommon . split_words ( :d\
_short_name_full , 30 , '*' , 3 , str1 , str2 ) ; IF str2 IS NOT NULL THEN :h_\
patient_name1 := :h_patient_name1 || str2 ; END IF ; END ; END ;";
    sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRBOULS.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
