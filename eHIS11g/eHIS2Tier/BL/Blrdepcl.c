...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/Blrdepcl.pc"
};
...
"DATE(:b8,'DD/MM/YYYY'),to_date('47123112','YYYYDDMM'))) and TRX_TYPE_CODE='5\
') and RECPT_NATURE_CODE='RD') and NVL(DOC_OUTST_AMT,0)<>0) and OPERATING_FACI\
LITY_ID=:b9) order by EPISODE_TYPE,PATIENT_ID,EPISODE_ID,VISIT_ID,ORG_DOC_TYPE\
_CODE,ORG_DOC_NUM            ";

 static const char *sq0005 = 
"select DOC_TYPE_CODE ,DOC_NUM ,TRUNC(DOC_DATE) ,TO_CHAR(DOC_AMT,'999,999,990\
.99') ,DOC_AMT ,TRX_TYPE_CODE ,BILL_TRX_TYPE_CODE ,RECPT_NATURE_CODE ,SUBSTR(D\
ECODE(BILL_TRX_TYPE_CODE,'D','Inpatients Discharge Bill','A','Inpatients After\
 Dischg Bill','I','Inpatients Inhouse Bill','O','Outpatients Bill','E','Outpat\
ients Episode Level Bill','R','Referral Patients Bill',''),1,30)  from BL_PATI\
ENT_LEDGER_TRN where ((((((EPISODE_TYPE=:b0 and EPISODE_ID=:b1) and PATIENT_ID\
=:b2) and VISIT_ID=:b3) and ORG_DOC_TYPE_CODE=:b4) and ORG_DOC_NUM=:b5) and OP\
ERATING_FACILITY_ID=:b6) order by DOC_TYPE_CODE,DOC_NUM            ";
...
str2 ) ; IF str2 IS NOT NULL THEN :bl_patient_name := str2 ; END IF ; blcommon\
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRDEPCL.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
