...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR0071B.PC"
};


static unsigned int sqlctx = 1288461211;
...
"select BANK_IN_BATCH_DOC_TYPE ,BANK_IN_DATE ,BANK_IN_DOC_NO ,decode(DR_CR_IN\
D,'DR',ACCT_CODE_AMT,0) ,decode(DR_CR_IND,'CR',ACCT_CODE_AMT,0)  from BL_BANK_\
IN_BY_ACCT_CODE where (((NVL(EXT_ACCT_CODE,'ZZ')=:b0 and TRUNC(nvl(bank_in_dat\
e,sysdate))<=TRUNC(LAST_DAY(TO_DATE(:b1,'MM-YYYY')))) and nvl(reconciled_ind,'\
N')='N') and bank_in_date is  not null ) order by BANK_IN_DATE            ";
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,98,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
36,0,0,2,223,0,4,143,0,0,7,3,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,
0,0,1,9,0,0,
79,0,0,3,84,0,2,171,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
106,0,0,4,0,0,30,242,0,0,0,0,0,1,0,
121,0,0,5,384,0,9,276,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
144,0,0,5,0,0,15,284,0,0,0,0,0,1,0,
159,0,0,5,0,0,13,303,0,0,5,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,
194,0,0,6,190,0,4,457,0,0,6,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,
9,0,0,
233,0,0,7,84,0,4,487,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
256,0,0,8,96,0,4,562,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
279,0,0,9,162,0,6,569,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
};
...
M2,'MM-YYYY'),'fmMonth YYYY') ,to_char(to_date(PARAM2,'MM-YYYY'),'YYYY') into \
...

{
          ...
          sqlstm.sqhstv[0] = (         void  *)&bl_batch_doc_type;
...
_CHAR(SYSDATE,'HH24:MI:SS') ,USER ,(BASE_CURRENCY||')') currency into :b0,:b1,\
:b2,:b3,:b4  from SY_ACC_ENTITY where ACC_ENTITY_ID=:b5";
    sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR0071B.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
