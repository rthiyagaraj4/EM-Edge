...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR00010.pc"
};


static unsigned int sqlctx = 1288460843;
...
"select doc_srno ,patient_id ,episode_id ,episode_type ,slmt_type_code ,recpt\
_type_code ,recpt_nature_code ,TO_CHAR(doc_date,'DD/MM/RRRR') ,slmt_doc_ref_de\
sc ,TO_CHAR(slmt_doc_ref_date,'DD/MM/RRRR') ,added_by_id ,((-1)* doc_amt) ,((b\
ill_doc_type_code||' ')||bill_doc_number) ,payer_name  from bl_receipt_refund_\
HDRDTL_VW where (((doc_type_code=:b0 and doc_number=:b1) and NVL(cancelled_ind\
,'N')<>'Y') and recpt_type_code=(select misc_rcpt_type_for_med_rep  from bl_pa\
rameters where OPERATING_FACILITY_ID=:b2))           ";
...
36,0,0,2,0,0,29,178,0,0,0,0,0,1,0,
51,0,0,4,0,0,32,211,0,0,0,0,0,1,0,
66,0,0,3,519,0,9,218,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
93,0,0,5,90,0,4,232,0,0,3,1,0,1,0,2,9,0,0,2,9,0,0,1,9,0,0,
120,0,0,6,161,0,4,247,0,0,3,2,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,
147,0,0,7,308,0,4,273,0,0,12,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,
210,0,0,8,71,0,4,317,0,0,1,0,0,1,0,2,9,0,0,
229,0,0,9,205,0,4,328,0,0,5,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,
264,0,0,10,72,0,4,351,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
287,0,0,3,0,0,15,368,0,0,0,0,0,1,0,
302,0,0,3,0,0,13,390,0,0,14,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,9,0,0,2,9,0,0,
373,0,0,11,0,0,32,413,0,0,0,0,0,1,0,
388,0,0,12,96,0,4,446,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
411,0,0,13,165,0,6,453,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
};


/************************************************************************/
/* PROGRAM NAME          :	BLR00010.PC                                 */
/* PROGRAM DESC          :	Receipt Letter for Medical report Payment   */
/* AUTHOR                :	HARISH. A                                   */
...
struct { unsigned short len; unsigned char arr[10]; } l_recpt_type_code;
...
int argc;
{
	...
	if (argc != 8) 
	{
        disp_message (ERR_MESG,"Usage : BLR00010 username/pwd@connect_string session_id pgm_date facility_id Calling_Pgm_Id doc_type_Code doc_number\n");
        proc_exit();
	} 
...
get_data_for_report()
{
	...
	/* EXEC SQL DECLARE receipt_cursor CURSOR FOR
			   ...
			   WHERE doc_type_code = :l_doc_type_code
				 ...
				 AND recpt_type_code = (SELECT misc_rcpt_type_for_med_rep
										  FROM bl_parameters where OPERATING_FACILITY_ID = :nd_facility_id); */ 
...
ere acc_entity_id=:b2))";
 ...
 sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
	...
	/* EXEC SQL SELECT UPPER(acc_entity_name), 
				    ...
				    tlx_num,
				    fax_num,
					INITCAP(head_of_company_name),
					INITCAP(head_of_company_desig),
				    base_currency
...
(ln2_add)||',') ,(UPPER(ln3_add)||'.') ,zip_code ,tel_num ,tlx_num ,fax_num ,I\
NITCAP(head_of_company_name) ,INITCAP(head_of_company_desig) ,base_currency in\
to :b0,:b1,:b2,:b3,:b4,:b5,:b6,:b7,:b8,:b9,:b10  from sy_acc_entity where acc_\
entity_id=:b11";
 sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
	...
	/* EXEC SQL SELECT UPPER(substr(ln1_mail_add,1,40)),
					UPPER(substr(ln2_mail_add,1,40)),
					UPPER(substr(ln3_mail_add,1,40)),
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select UPPER(substr(ln1_mail_add,1,40)) ,UPPER(substr(ln2_ma\
il_add,1,40)) ,UPPER(substr(ln3_mail_add,1,40)) ,substr(mail_country_code,1,40\
) into :b0,:b1,:b2,:b3  from mp_pat_other_dtls where patient_id=:b4";
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
	...
	if(l_country_code.len > 0)
	{
		...
		/* EXEC SQL SELECT UPPER(long_name)
				   INTO :l_country
				   FROM mp_country
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select UPPER(long_name) into :b0  from mp_country where cou\
ntry_code=:b1";
  sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqadto[2] = (unsigned short )0;
 sqlstm.sqtdso[2] = (unsigned short )0;
 sqlstm.sqhstv[3] = (         void  *)&l_episode_type;
 ...
 sqlstm.sqadto[3] = (unsigned short )0;
 sqlstm.sqtdso[3] = (unsigned short )0;
 sqlstm.sqhstv[4] = (         void  *)&l_slmt_type_code;
 ...
 sqlstm.sqadto[4] = (unsigned short )0;
 sqlstm.sqtdso[4] = (unsigned short )0;
 sqlstm.sqhstv[5] = (         void  *)&l_recpt_type_code;
 ...
 sqlstm.sqadto[5] = (unsigned short )0;
 sqlstm.sqtdso[5] = (unsigned short )0;
 sqlstm.sqhstv[6] = (         void  *)&l_recpt_nature_code;
 ...
 sqlstm.sqadto[7] = (unsigned short )0;
 sqlstm.sqtdso[7] = (unsigned short )0;
 sqlstm.sqhstv[8] = (         void  *)&l_slmt_doc_ref_desc;
 ...
 sqlstm.sqadto[8] = (unsigned short )0;
 sqlstm.sqtdso[8] = (unsigned short )0;
 sqlstm.sqhstv[9] = (         void  *)&l_slmt_doc_ref_date;
 ...
 sqlstm.sqadto[9] = (unsigned short )0;
 sqlstm.sqtdso[9] = (unsigned short )0;
 sqlstm.sqhstv[10] = (         void  *)&l_added_by_id;
 ...
 sqlstm.sqadto[10] = (unsigned short )0;
 sqlstm.sqtdso[10] = (unsigned short )0;
 sqlstm.sqhstv[11] = (         void  *)&l_doc_amt;
 ...
 sqlstm.sqadto[11] = (unsigned short )0;
 sqlstm.sqtdso[11] = (unsigned short )0;
 sqlstm.sqhstv[12] = (         void  *)&l_org_doc_type_code;
 ...
 sqlstm.sqhstv[13] = (         void  *)&l_payers_name;
...

{
	...
	l_recpt_type_code.arr[l_recpt_type_code.len]		= '\0';
...
fetch_legend_value()
{
	for(i=1;i<=100; i++)
	{
		...
		/* EXEC SQL SELECT LTRIM(RTRIM('BLR00010.LEGEND_'||LTRIM(RTRIM(TO_CHAR(:i,'009')))))
		           INTO :l_pk_value
				   FROM dual; */ 

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00010.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
...
print_letter_header()
{
	...
	fprintf(fp,"\t                %-40.40s          %-3.3s: %-s\n",l_ln1_add.arr,loc_legend[1],l_telephone_num.arr);
	fprintf(fp,"\t                %-40.40s          %-3.3s  %-s\n",l_ln2_add.arr," ",l_telex_num.arr);
	fprintf(fp,"\t                %-40.40s          %-3.3s: %-s\n",l_ln3_add.arr,loc_legend[2],l_fax_num.arr);
	fprintf(fp,"    ______________________________________________________________________________________\n");
	fprintf(fp,"\n");
}

print_letter_reference()
{
	fprintf(fp,"%-62.62s%-s:%s\n\n"," ",loc_legend[3],l_doc_date.arr);
...
print_letter_body()
{
	...
	//This line should be Bold and underlined
	fprintf(fp,"    %s\n\n",loc_legend[5]);

	fprintf(fp,"    %s %s %s %s %s %s %s %s\n",loc_legend[6],l_slmt_desc.arr,loc_legend[7],l_slmt_doc_ref_date.arr,loc_legend[8],l_slmt_desc.arr, loc_legend[9], l_slmt_doc_ref_desc.arr);
	fprintf(fp,"    %s \n",loc_legend[10],l_doc_type_code.arr,l_doc_num.arr,l_doc_srno.arr);
	fprintf(fp,"    %s/%s/%s ",l_doc_type_code.arr,l_doc_num.arr,l_doc_srno.arr);
	fprintf(fp,"%s %s %s\n\n",loc_legend[11],l_doc_date.arr,loc_legend[12]);

	fprintf(fp,"    2.  %s\n",loc_legend[13]);
	fprintf(fp,"        %s %s %s\n",loc_legend[16], l_acc_entity_name.arr,loc_legend[14]);
	fprintf(fp,"        %s\n",loc_legend[15]);
	fprintf(fp,"        %s %s\n\n",loc_legend[16],l_acc_entity_name.arr);
...
print_letter_tail()
{
	...
	fprintf(fp,"    %s\n",loc_legend[23]);
	fprintf(fp,"    %s\n",loc_legend[24]);
	fprintf(fp,"    %s, %s\n",l_head_of_company_desig.arr,l_acc_entity_name.arr);
	fprintf(fp,"    %s\n\n",l_acc_entity_name.arr);
	fprintf(fp,"    %s\n",loc_legend[25]);
}
