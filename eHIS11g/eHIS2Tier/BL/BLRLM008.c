...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLRLM008.pc"
};


static unsigned int sqlctx = 1288466331;
...
"d) and b.blng_grp_id=d.blng_grp_id) and d.settlement_ind='X') and d.adm_rec_\
flag=2) and b.blng_grp_id between nvl(:b2,'!!!!') and nvl(:b3,'~~~~')) and b.p\
atient_id between nvl(:b4,'!!!!!!!!!!!!!!!!!!!!') and nvl(:b5,'~~~~~~~~~~~~~~~\
~~~~~')) and b.cust_code between nvl(:b6,'!!!!!!!!') and nvl(:b7,'~~~~~~~~')) \
and c.cur_ward_code between nvl(:b8,'!!!!') and nvl(:b9,'~~~~')) and trunc(c.a\
dmission_date_time) between trunc(to_date(nvl(:b10,'01/01/1000'),'dd/mm/yyyy')\
) and trunc(to_date(nvl(:b11,'01/01/4712'),'dd/mm/yyyy'))) group by a.patient_\
id,c.cur_ward_code,c.episode_id,b.blng_grp_id,b.cust_code,b.BED_BILL_BED_TYPE_\
CODE,to_char(c.admission_date_time,'dd/mm/yyyy hh24:mi'),(nvl(b.discharge_date\
_time,sysdate)-b.admission_date_time),substr(a.ADDED_BY_ID,1,15),b.encounter_i\
d order by b.blng_grp_id,(sum(a.APPROVED_AMT)-sum((NVL(b.tot_billed_amt,0)+NVL\
(b.tot_unbld_amt,0)))) desc             ";

 static const char *sq0009 = 
"episode_id) and a.episode_id=c.open_episode_id) and a.episode_id=d.episode_i\
d) and b.blng_grp_id=a.blng_grp_id) and b.blng_grp_id=e.blng_grp_id) and e.set\
tlement_ind='X') and e.adm_rec_flag=2) and b.blng_grp_id between nvl(:b2,'!!!!\
') and nvl(:b3,'~~~~')) and b.patient_id between nvl(:b4,'!!!!!!!!!!!!!!!!!!!!\
') and nvl(:b5,'~~~~~~~~~~~~~~~~~~~~')) and b.cust_code between nvl(:b6,'!!!!!\
!!!') and nvl(:b7,'~~~~~~~~')) and d.cur_ward_code between nvl(:b8,'!!!!') and\
 nvl(:b9,'~~~~')) and trunc(c.admission_date_time) between trunc(to_date(nvl(:\
b10,'01/01/1000'),'dd/mm/yyyy')) and trunc(to_date(nvl(:b11,'01/01/4712'),'dd/\
mm/yyyy'))) group by a.patient_id,d.cur_ward_code,c.open_episode_id,b.blng_grp\
_id,b.cust_code,b.BED_BILL_BED_TYPE_CODE,to_char(c.admission_date_time,'dd/mm/\
yyyy hh24:mi'),(sysdate-b.admission_date_time),substr(a.ADDED_BY_ID,1,15),b.en\
counter_id order by b.blng_grp_id,(sum(a.APPROVED_AMT)-sum((NVL(b.tot_billed_a\
mt,0)+NVL(b.tot_unbld_amt,0)))) desc             ";

 static const char *sq0012 = 
") and d.settlement_ind='X') and d.adm_rec_flag=2) and b.blng_grp_id between \
nvl(:b2,'!!!!') and nvl(:b3,'~~~~')) and b.patient_id between nvl(:b4,'!!!!!!!\
!!!!!!!!!!!!!') and nvl(:b5,'~~~~~~~~~~~~~~~~~~~~')) and b.cust_code between n\
vl(:b6,'!!!!!!!!') and nvl(:b7,'~~~~~~~~')) and c.clinic_code between nvl(:b8,\
'!!!!') and nvl(:b9,'~~~~')) and trunc(c.visit_regn_date_time) between trunc(t\
o_date(nvl(:b10,'1/1/1000'),'dd/mm/yyyy')) and trunc(to_date(nvl(:b11,'1/1/471\
2'),'dd/mm/yyyy'))) group by a.patient_id,c.clinic_code,c.episode_id,c.visit_i\
d,b.blng_grp_id,b.cust_code,to_char(c.visit_regn_date_time,'dd/mm/yyyy hh24:mi\
'),substr(a.ADDED_BY_ID,1,15),b.encounter_id order by b.blng_grp_id,(sum(a.APP\
ROVED_AMT)-sum((NVL(b.tot_billed_amt,0)+NVL(b.tot_unbld_amt,0)))) desc        \
     ";
...
1131,0,0,0,0,0,27,1965,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...
AM16 ,PARAM17 ,PARAM18 ,PARAM19 ,OPERATING_FACILITY_ID into :b0,:b1,:b2,:b3,:b\
4,:b5,:b6,:b7,:b8,:b9,:b10,:b11,:b12,:b13,:b14,:b15,:b16,:b17,:b18,:b19  from \
SY_PROG_PARAM where (((OPERATING_FACILITY_ID=:b20 and PGM_ID=:b21) and SESSION\
_ID=:b22) and PGM_DATE=:b23)";
   sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select sum(nvl(bill_amt,0)) into :b0  from BL_BILL_HDR wher\
e ((((PATIENT_ID=:b1 and blng_grp_id=:b2) and episode_id=:b3) and episode_type\
=:b4) and NVL(visit_id,0)=:b5)";
  sqlstm.iters = (unsigned int  )1;
...
MI') ,USER into :b0,:b1,:b2  from SY_ACC_ENTITY where (ACC_ENTITY_ID=:b3 and l\
anguage_id=:b4)";
    sqlstm.iters = (unsigned int  )1;
...

{
     ...
     sqlstm.sqladtp = &sqladt;
     sqlstm.sqltdsp = &sqltds;
     sqlstm.stmt = "select decode(:b0,'en',substr(short_name,1,60),substr(SH\
ORT_NAME_LOC_LANG,1,60)) into :b1  from MP_PATIENT_MAST where PATIENT_ID=:b2";
     sqlstm.iters = (unsigned int  )1;
...

{
     ...
     sqlstm.sqladtp = &sqladt;
     sqlstm.sqltdsp = &sqltds;
     sqlstm.stmt = "select PROC_CODE into :b0  from MR_PROCEDURE_RECODING_DT\
L where ((((PATIENT_ID=:b1 and Stage_no='0') and Stage_Srl_no=1) and FACILITY_\
ID=:b2) and Encounter_id=:b3)";
     sqlstm.iters = (unsigned int  )1;
...

{
     ...
     sqlstm.sqladtp = &sqladt;
     sqlstm.sqltdsp = &sqltds;
     sqlstm.stmt = "select bill_amt into :b0  from BL_BILL_HDR where ((((PAT\
IENT_ID=:b1 and blng_grp_id=:b2) and episode_id=:b3) and episode_type=:b4) and\
 NVL(visit_id,0)=:b5)";
     sqlstm.iters = (unsigned int  )1;
