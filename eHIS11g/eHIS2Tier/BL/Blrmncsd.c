...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/Blrmncsd.pc"
};


static unsigned int sqlctx = 1288467787;
...
"select to_char(TRUNC(A.DOC_DATE),'dd/mm/yyyy') ,A.CASH_COUNTER_CODE ,A.SHIFT\
_ID ,sum(DECODE(B.CASH_SLMT_FLAG,'Y',DECODE(A.SLMT_TYPE_CODE,'CO',0,DOC_AMT),0\
)) CASH ,sum(DECODE(B.CASH_SLMT_FLAg,'B',DOC_AMT,0)) CHEQUE ,sum(DECODE(B.CASH\
_SLMT_FLAG,'C',DOC_AMT,0)) AUTOPAY ,sum(DECODE(B.CASH_SLMT_FLAG,'A',DOC_AMT,0)\
) CREDIT ,sum(DECODE(A.SLMT_TYPE_CODE,'CO',DOC_AMT,0)) CO ,CASH_COUNTER_FACILI\
TY_ID ,OPERATING_FACILITY_ID  from BL_RECEIPT_REFUND_DTL_VW A ,BL_SLMT_TYPE_LA\
NG_VW B where (((((A.SLMT_TYPE_CODE=B.SLMT_TYPE_CODE and language_id=:b0) and \
TRUNC(A.DOC_DATE) between TO_DATE(:b1,'DD/MM/YYYY') and TO_DATE(:b2,'DD/MM/YYY\
Y')) and NVL(A.CANCELLED_IND,'N')<>'Y') and CASH_COUNTER_FACILITY_ID between N\
VL(:b3,OPERATING_FACILITY_ID) and NVL(:b4,OPERATING_FACILITY_ID)) and CASH_COU\
NTER_CODE between NVL(:b5,CASH_COUNTER_CODE) and NVL(:b6,CASH_COUNTER_CODE)) g\
roup by A.CASH_COUNTER_FACILITY_ID,A.CASH_COUNTER_CODE,A.OPERATING_FACILITY_ID\
,TRUNC(A.DOC_DATE),A.CASH_COUNTER_CODE,A.SHIFT_ID           ";
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRMNCSD.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
