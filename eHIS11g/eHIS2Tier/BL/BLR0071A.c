...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR0071A.PC"
};


static unsigned int sqlctx = 1288461203;
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,99,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
36,0,0,2,222,0,4,144,0,0,7,3,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,
0,0,1,9,0,0,
79,0,0,3,84,0,2,172,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
106,0,0,4,0,0,30,239,0,0,0,0,0,1,0,
121,0,0,5,350,0,9,273,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
144,0,0,5,0,0,15,281,0,0,0,0,0,1,0,
159,0,0,5,0,0,13,300,0,0,5,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,
194,0,0,6,190,0,4,454,0,0,6,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,
9,0,0,
233,0,0,7,84,0,4,482,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
256,0,0,8,96,0,4,555,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
279,0,0,9,162,0,6,562,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
};
...
struct { unsigned short len; unsigned char arr[18]; } nd_month;

struct { unsigned short len; unsigned char arr[5]; } nd_year;

struct { unsigned short len; unsigned char arr[11]; } cost_centre;

struct { unsigned short len; unsigned char arr[9]; } d_currency;
    ...
    if (sqlca.sqlcode < 0 )
	   {
        ...
        /* EXEC SQL SELECT	nvl(PARAM1,'ZZ'),PARAM2,to_char(to_date(PARAM2,'MM-YYYY'),'fmMonthYYYY'),to_char(to_date(PARAM2,'MM-YYYY'),'YYYY')
			INTO :nd_acct_code,
                 //:nd_dept_code,
				 :nd_fm_date,
				 :nd_month,
...
M2,'MM-YYYY'),'fmMonthYYYY') ,to_char(to_date(PARAM2,'MM-YYYY'),'YYYY') into :\
   ...
   open_file()
{
	...
	strcat(filename,"BLR0071A.lis");

    if ((fp = fopen(filename,"w")) == NULL)
    {
       disp_message(ERR_MESG,"Error in opening file BLR0071A.lis");
       proc_exit();
    }
}
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select ACC_ENTITY_NAME ,TO_CHAR(SYSDATE,'DD/MM/YYYY') ,TO_CH\
AR(SYSDATE,'HH24:MI:SS') ,USER ,(BASE_CURRENCY||')') currency into :b0,:b1,:b2\
,:b3,:b4  from SY_ACC_ENTITY where ACC_ENTITY_ID=:b5";
 ...
 sqlstm.sqadto[3] = (unsigned short )0;
 sqlstm.sqtdso[3] = (unsigned short )0;
 sqlstm.sqhstv[4] = (         void  *)&d_currency;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select COST_CENTRE_CODE into :b0  from BL_PARAMETERS where \
OPERATING_FACILITY_ID=:b1";
  sqlstm.iters = (unsigned int  )1;
  ...
  sqlstm.sqlety = (unsigned short)4352;
  sqlstm.occurs = (unsigned int  )0;
  sqlstm.sqhstv[0] = (         void  *)&cost_centre;
  sqlstm.sqhstl[0] = (unsigned int  )13;
  sqlstm.sqhsts[0] = (         int  )0;
  sqlstm.sqindv[0] = (         void  *)0;
  sqlstm.sqinds[0] = (         int  )0;
...
fetch_legend_value()
{
	for(i=1;i<=300;i++)
	{
		...
		/* EXEC SQL SELECT LTRIM(RTRIM('BLR0071A.LEGEND_'||LTRIM(RTRIM(TO_CHAR(:i,'009')))))
		           INTO :l_pk_value
				   FROM dual; */ 

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR0071A.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
