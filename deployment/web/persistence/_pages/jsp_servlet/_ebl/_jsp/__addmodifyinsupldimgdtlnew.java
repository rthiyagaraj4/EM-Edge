...
public final class __addmodifyinsupldimgdtlnew extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/ebl/jsp/AddModifyInsUpldImgDtlNew.jsp", 1743565500636L ,"10.3.6.0","Asia/Calcutta")) return true;
    ...
    private final static java.lang.String  _wl_block23 ="<!-- Added V191114-Aravindh/GHL-CRF-0550 -->\n\t\t\t\t\n\t\t\t\t\t<td class=\'fields\' nowrap>&nbsp;<a href=\"javascript:updateDetails1(";
    ...
    private final static java.lang.String  _wl_block31 =",this)\" nowrap>";
    ...
    private final static java.lang.String  _wl_block50 ="\"  id=\"line_\" value =\"";
...
---------------------------------------------------------------------------------------------------------
1         V220818      	  35244         BL-RBU-GHL-CRF-0005 US001/01- 
						Change Patient Details-Upload Documents       Mohanapriya
---------------------------------------------------------------------------------------------------------
*/
	...
	*/
	...
	if(siteSpecUploadDocAll) {
		//	if (blInsuranceImageBean.getSize() == 0) { //29-09-2020
				blInsuranceImageBean.loadInsuranceDBvalues(strPatientId,strModuleId,patView,strepisode_id,strvisit_id);//Added against 9994
				System.err.println("137, Data is not Present in Cache - load Data from the DB==>"+blInsuranceImageList.keySet());
	//	} //29-09-2020
	}
	else
	{
		if (blInsuranceImageBean.getSize() == 0) {
				...
				System.err.println("143, Data is not Present in Cache - load Data from the DB for other site==>"+blInsuranceImageList.keySet());
			}
	}	
	//Added 03-feb-20 palaniNarayanan/GHL-CRF-0550
	...
	System.err.println("150, Data loaded_AddModifyInsUpldImgDtl.jsp==>"+blInsuranceImageList.keySet());
	/*Added 03-feb-20 palaniNarayanan/GHL-CRF-0550
	try
	{
		siteSpecUploadDocAll = eCommon.Common.CommonBean.isSiteSpecific(con, "BL","BL_UPLOAD_DOC_ALL");//Added V191028-Aravindh/GHL-CRF-0550
...
try{
		...
		pstmt3 = con.prepareStatement("select long_desc blng_grp_desc from bl_blng_grp_lang_vw where blng_grp_id = ? and language_id = '"+locale+"'");//Added 
		pstmt4 = con.prepareStatement("select SETTLEMENT_IND from BL_BLNG_GRP where SHORT_DESC=?");
	
//		for(index;index<blInsuranceImageList.size();index++)

		System.out.println("BLInsurance image size Keyset "+ ob);
		for(index=0;index<ob.length;index++)
		{
			System.err.println("ob.length===>"+ob.length);
			System.out.println("BLInsurance image size Keyset --> "+ ob[index]);
			...
			{		
					...
					strPolicyNo		=			blInsuranceImage.getPolicyNo();
			...
			//		byte imByte[]=blInsuranceImage.getImage();
					...
					if(rs.next())					
						strPayerGroupDesc=rs.getString(1);		
							...
							pstmt1.setString(2,strPayerCode);
		...
		/*		try
				{
					if(blInsuranceImage.getImage()==null)
						len  = 0;
					else 
						len  = 1;
				//	long len = imByte.length;
				}catch(Exception ee1)
				{
					len = 0;
				}
