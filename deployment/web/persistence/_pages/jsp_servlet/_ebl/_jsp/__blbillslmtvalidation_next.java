...
public final class __blbillslmtvalidation_next extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/ebl/jsp/BLBillSlmtValidation_NEXT.jsp", 1737915729320L ,"10.3.6.0","Asia/Calcutta")) return true;
	...
	{
		struser = (String) session.getAttribute("login_user");  //V210212
	}	
	...
	try
	{
		...
		{			
			...
			{		
				...
				call = con.prepareCall("{ call blopin.BILL_SLMT_HDR_PROC (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"); //V210113
...
//					intslmt_doc_sr_num=intslmt_doc_sr_num+i;
					...
					if(proc_success.equals("Y"))
					{
						error_flag="";
						//Commented by Sethu for GHL-SCF-1545 on 11/01/2021-->
					      //call = con.prepareCall("{ call blopin.BILL_SLMT_DTL_PROC (?,?,?,?,?,?,?,?,?,?,?,?,?,?,to_date(?,'dd/mm/yyyy'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"); //V210113			
						//Added by Sethu for GHL-SCF-1545 on 11/01/2021-->
						call = con.prepareCall("{ call blopin.BILL_SLMT_DTL_PROC (?,?,?,?,?,?,?,?,?,?,?,?,?,?,to_date(?,'dd/mm/yyyy'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, to_date(?,'dd/mm/yyyy hh24:mi:ss'),?)}");			
						call.setString(1, strEncounterFacilityId); //V210211
						//call.setString(1,strfacilityid); //V210211 commented
						call.setString(2,strpatientid);
						call.setString(3,strepisodetype);
