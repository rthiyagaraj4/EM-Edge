...
public final class __nursunitspecialityqueryresult extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eip/jsp/NursUnitSpecialityQueryResult.jsp", 1742446732383L ,"10.3.6.0","Asia/Calcutta")) return true;
 ...
 String sStyle = checkForNull((String)session.getAttribute("PREFERRED_STYLE"), "IeStyle.css"); 
	...
	if ( (whereClause == null || whereClause.equals("")) )
	{
		...
		specialty_code = specialty_code.toUpperCase();
		...
		if ( !(specialty_code == null || specialty_code.equals("")) )
		{
			...
			if ( andCheck == 1 )
			{	sql.append(" and ");
				sql.append("upper(specialty_code) like  upper('"+specialty_code+"%')" );
			}
			else
			{
				if(sql.length() > 0) sql.delete(0,sql.length());
		...
		if ( !(specialty_short_desc == null || specialty_short_desc.equals("")) )
		{
			if ( andCheck == 1 )
			{	sql.append(" and ");
				sql.append("upper(specialty_short_desc) like  upper('"+specialty_short_desc+"%')" );
			}
			else
			{
				if(sql.length() > 0) sql.delete(0,sql.length());
				sql.append("where upper(specialty_short_desc) like upper('"+specialty_short_desc+"%')" );
				andCheck = 1;
			}
		 }							
	...
	try{		
			...
			strsql1.append("select count(*) as total from ip_nurs_unit_for_specialty_vw  "+sql.toString());
	...
	out.println("<script>alert(getMessage('NO_RECORD_FOUND_FOR_CRITERIA','common'));history.go(-1);</script>");
			...
			strsql2.append("select  * from ip_nurs_unit_for_specialty_vw  "+sql.toString());
		 ...
		 for( int j=1; j<start; i++,j++ ){
		  ...
		  while (  i<=end && rs.next())
            ...
            _bw.write(_wl_block7Bytes, _wl_block7);
					...
					if ( !(start <= 1) )
						out.println("<td align ='right' id='prev'><A HREF='../jsp/NursUnitSpecialityQueryResult.jsp?from="+(start-14)+"&to="+(end-14)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A></td>");

					//if ( !( (start+14) > maxRecord ) )
						out.println("<td align ='right' id='next' style='visibility:hidden'><A HREF='../jsp/NursUnitSpecialityQueryResult.jsp?from="+(start+14)+"&to="+(end+14)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A></td>");
            ...
            _bw.write(_wl_block11Bytes, _wl_block11);
				...
				//{
						...
						String specialtycode	    = rs.getString("specialty_code");
						String specialtyshortname   = rs.getString("specialty_short_desc");
						
						out.println("<tr><td class='" + classValue + "'>");
						out.println(nursingunitcode+"</a></td><td class='" + classValue + "'>");
						out.println(nursingunitshortdesc+"</a></td><td class='" + classValue + "'>");
						out.println(specialtycode+"</a></td><td class='" + classValue + "'>");
						out.println(specialtyshortname+"</a></td><td class='" + classValue + "'>");	
					i++;
					maxRecord++;
			}//endwhile
