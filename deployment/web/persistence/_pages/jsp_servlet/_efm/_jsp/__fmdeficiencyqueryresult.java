...
public final class __fmdeficiencyqueryresult extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/efm/jsp/FMDeficiencyQueryResult.jsp", 1709116835669L ,"10.3.6.0","Asia/Calcutta")) return true;
    ...
    private final static java.lang.String  _wl_block6 ="\n\t  \t<script>\n\t//\talert(getMessage(\'NO_RECORD_FOUND_FOR_CRITERIA\',\'common\'));\n\t//\tparent.frames[1].location.href =\"../../eFM/jsp/FMDeficiencyQueryCriteria.jsp\" ;\n\t\t</script>\n\t ";
...
(session.getAttribute("PREFERRED_STYLE")!=null)||(session.getAttribute("PREFERRED_STYLE")!="")?(String)session.getAttribute("PREFERRED_STYLE"):"IeStyle.css";
	...
	{
		String deficiency_code		= request.getParameter("deficiency_code");
		String short_desc			= request.getParameter("long_desc");
		String enabled				= request.getParameter("eff_status");
				

		if (!(deficiency_code == null || deficiency_code.equals("")) )
		{
			deficiency_code=deficiency_code.toUpperCase();
			sql.append(" where deficiency_code like '"+deficiency_code+"%' ");
			andCheck = 1;
		} 
...
//	int maxRecord = 0;
	...
	//	String facilityId = (String) session.getValue( "facility_id" ) ;
		...
		/*strsql1.append(" select count(*) as total from fm_deficiency ");
		strsql1.append(sql);
	  ...
	  //}
		if(rs!=null)	rs.close();
		strsql2.append(" select deficiency_code, long_desc, short_desc, eff_status from fm_deficiency ");
		strsql2.append(sql);
		...
		while ( i<=end && rs.next())
		{
            ...
            _bw.write(_wl_block8Bytes, _wl_block8);
				...
				if ( !(start <= 1) )
				out.println("<td align ='right' id='prev'><A HREF='../jsp/FMDeficiencyQueryResult.jsp?from="+(start-14)+"&to="+(end-14)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A></td>");
				
            _bw.write(_wl_block9Bytes, _wl_block9);


				//if ( !( (start+14) > maxRecord ) )
				out.println("<A HREF='../jsp/FMDeficiencyQueryResult.jsp?from="+(start+14)+"&to="+(end+14)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
			...
			String deficiency_code	=	rs.getString("deficiency_code");
			...
			out.println("<a href='../jsp/FMDeficiencyAddModify.jsp?deficiency_code="+deficiency_code+ "&target=f_query_add_mod&operation=modify'>");
			out.println(deficiency_code+"</a></td><td class='" + classValue + "' >");
