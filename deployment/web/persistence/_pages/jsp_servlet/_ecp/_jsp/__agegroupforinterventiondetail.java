...
public final class __agegroupforinterventiondetail extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/ecp/jsp/AgeGroupForInterventionDetail.jsp", 1709116558356L ,"10.3.6.0","Asia/Calcutta")) return true;
    ...
    private final static java.lang.String  _wl_block10 =")\' onKeyDown=\"lockKey()\" class=\'CONTENT\' >\n<form name=\'facility_form_result\' id=\'facility_form_result\' action=\'../../eCP/jsp/AgeGroupForInterventionDetail.jsp\' method=\'post\' >\n<table  width=\'90%\' border=0 align=\'center\'>\n<tr>\n<td align=\'right\' class=\'NONURGENT\'>\n<input type=hidden name=\'age_group_code\' id=\'age_group_code\' value=\'";
	...
	localeName 		 = (localeName == null )?"en":localeName.toLowerCase();
            ...
            _bw.write(_wl_block2Bytes, _wl_block2);
            webbeans.eCommon.RecordSet AgeIntervention= null;synchronized(session){
                AgeIntervention=(webbeans.eCommon.RecordSet)pageContext.getAttribute("AgeIntervention",javax.servlet.jsp.PageContext.SESSION_SCOPE);
                if(AgeIntervention==null){
                    AgeIntervention=new webbeans.eCommon.RecordSet();
                    pageContext.setAttribute("AgeIntervention",AgeIntervention,javax.servlet.jsp.PageContext.SESSION_SCOPE);
	...
	String flagSelect = request.getParameter("fromSearch");
				...
				if(flagSelect !=null)
	        	{
	        		AgeIntervention.clearAll();
	        		  String sqlinsVals= " select intervention_code from ca_age_grp_for_intervention where age_group_code=?";
			
				 stmt3=con.prepareStatement(sqlinsVals);
				 stmt3.setString(1,age_group_code);
				 rstlRs3 = stmt3.executeQuery();
				if(rstlRs3 !=null)
				{
					 while(rstlRs3.next())
					 {
						AgeIntervention.putObject(rstlRs3.getString(1));
...
//	String   sql="select ?,intervention_code, short_desc intervention_desc,'I' db_action from ca_intervention where intervention_code not in (select intervention_code from ca_age_grp_for_intervention where age_group_code = ?) and eff_status = 'E' union all select ?,a.intervention_code intervention_code, b.short_desc intervention_desc, 'U' db_action from ca_age_grp_for_intervention a, ca_intervention b where a.age_group_code = ? and b.intervention_code  = a.intervention_code order by 3";
	String   sql="SELECT ?,intervention_code, short_desc intervention_desc,'I' db_action FROM ca_intervention_lang_vw WHERE intervention_code NOT IN (SELECT intervention_code FROM ca_age_grp_for_intervention WHERE age_group_code = ?) AND eff_status = 'E' and language_id=? UNION ALL SELECT ?,a.intervention_code intervention_code, b.short_desc intervention_desc, 'U' db_action FROM ca_age_grp_for_intervention a, ca_intervention_lang_vw b WHERE a.age_group_code = ? AND b.intervention_code  = a.intervention_code and b.language_id = ? ORDER BY 3";
    ...
    try
	{
	 ...
	 stmt.setString(4,age_group_code);
	 stmt.setString(5,age_group_code);
    ...
    if ( !((start+12) >= maxRecord )){
		...
		if (start != 0){
			...
			String intervention_desc=rs.getString("intervention_desc");

			if(AgeIntervention.containsObject(intervention_code))  chkAttribute = "CHECKED";
			else
			chkAttribute="";

			int recordIndex = AgeIntervention.indexOfObject(intervention_code);
			if(recordIndex!=-1)
			if(chkAttribute.equals("CHECKED"))    AgeIntervention.removeObject(recordIndex);
			...
			i++;
			  ...
			  finally
			  {
				...
				session.setAttribute("AgeIntervention", AgeIntervention);
			  }
