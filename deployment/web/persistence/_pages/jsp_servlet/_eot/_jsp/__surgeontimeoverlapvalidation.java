...
public final class __surgeontimeoverlapvalidation extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eot/jsp/SurgeonTimeoverLapValidation.jsp", 1709117288000L ,"10.3.6.0","Asia/Calcutta")) return true;
...
//		String day_code=day_code.splitString("@@");
	...
	String retVal="Y";
	String check_with_session_data_yn =  request.getParameter("check_with_session_data_yn");
	String old_start_time	=  request.getParameter("old_start_time");
	String old_end_time		= request.getParameter("old_end_time");
	...
	try{
		...
		if(check_with_session_data_yn.equals("Y") && !st_time.equals(old_end_time) && !end_time.equals(old_start_time)){
			// compare the times with the existing times for that speciality and surgeon codes
				pstmt1=con.prepareStatement(SQL_OVERLAP);
				pstmt1.setString(1,st_time);
				pstmt1.setString(2,old_start_time);
				pstmt1.setString(3,old_end_time);
				pstmt1.setString(4,end_time);
				pstmt1.setString(5,old_start_time);
				pstmt1.setString(6,old_end_time);
		...
		if(!retVal.equals("X")){
			...
			String SQL_TIMESLOTS ="SELECT TO_CHAR(FROM_TIME,'HH24:MI'),TO_CHAR(TO_TIME,'HH24:MI'),SCHEDULE_TYPE,FIRST_WEEK_YN,SECOND_WEEK_YN,THIRD_WEEK_YN,FOURTH_WEEK_YN,FIFTH_WEEK_YN FROM OT_DIARY_TIMETABLE WHERE OPERATING_FACILITY_ID=?  AND OPER_ROOM_CODE =?  AND SPECIALITY_CODE= ? AND RESOURCE_ID=?";
			...
			String SQL_TIMESLOTS_1 ="SELECT TO_CHAR(FROM_TIME,'HH24:MI'),TO_CHAR(TO_TIME,'HH24:MI'),SCHEDULE_TYPE,FIRST_WEEK_YN,SECOND_WEEK_YN,THIRD_WEEK_YN,FOURTH_WEEK_YN,FIFTH_WEEK_YN FROM OT_DIARY_TIMETABLE WHERE OPERATING_FACILITY_ID=?  AND OPER_ROOM_CODE =?  AND SPECIALITY_CODE=? AND RESOURCE_ID=? AND DAY_CODE=?  ";
			...
			if(!day_flag) pstmt.setString(5,day_code);
