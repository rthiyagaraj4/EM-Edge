...
public final class __requeststatusqueryresult extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/ess/jsp/RequestStatusQueryResult.jsp", 1709121858117L ,"10.3.6.0","Asia/Calcutta")) return true;
    ...
    private final static java.lang.String  _wl_block12 ="</th>\n\t\t<!--Added by Rabbani #INC no:43292 on 12/12/2013 -->\n\t\t<th>Requestor name</th>\n\t\t<th>Time of Request</th>\n\t\t<th>";
...
//	java.util.HashMap defaultParameter			=		null;
	String bean_id								=		"ssrequestStatusBean";
...
try{
	...
	//sqlMap.put( "sqlData", SsRepository.getSsKeyValue( "SQL_SS_REQUEST_STATUS_QUERY"));
	//sqlMap.put( "sqlData","SELECT sr.facility_id, sr.doc_type_code, TO_CHAR (sr.doc_no) doc_no,       TO_CHAR (doc_date, 'dd/mm/yyyy') doc_date, sr.doc_ref,       ms.short_desc req_by_store_desc,       ms1.short_desc req_on_store_desc, sr.entry_completed_yn,       DECODE (authorized_by_id, NULL, 'N', 'Y') authorized_yn,       DECODE (ss_req_issue_chk ((?), sr.doc_type_code, doc_no),               '1', 'Not Issued', '2', 'Fully Issued', '3',              'Partially Issued') issued,       sr.cancelled_yn, sy.short_desc doc_type_desc , SS_Req_Ack_Status1 (sr.facility_id, sr.doc_type_code, sr.doc_no) Ack_status FROM ss_request_hdr sr,       mm_store_lang_vw ms,       mm_store_lang_vw ms1,       sy_doc_type_master_lang_vw sy,       ss_user_access_for_store ss1,       ss_user_access_for_store ss2 WHERE	sy.doc_type_code = sr.doc_type_code   AND sr.facility_id = (?)    AND sr.req_by_store_code = ms.store_code    AND ss1.store_code = 		sr.req_by_store_code   AND ss2.store_code = sr.req_on_store_code    AND ss1.req_allowed_yn = 'Y'   AND ss2.iss_allowed_yn = 'Y'		AND ss1.user_id = (?)   AND ss2.user_id = (?)   AND sr.req_on_store_code = ms1.store_code   AND TO_DATE (TO_CHAR (doc_date, 		'dd/mm/yyyy'), 'dd/mm/yyyy')          BETWEEN TO_DATE (NVL ((?), '01/01/1472'), 'dd/mm/yyyy')              AND TO_DATE (NVL 		((?), '31/12/5000'), 'dd/mm/yyyy')   AND sr.entry_completed_yn LIKE (?)   AND DECODE (sr.authorized_by_id, NULL, 'N', 'Y') LIKE 		(?)   AND ss_req_issue_chk ((?), sr.doc_type_code, doc_no) LIKE (?)   AND sr.cancelled_yn LIKE (?)   AND sr.req_by_store_code 		LIKE (?)   AND sr.req_on_store_code LIKE (?)   AND sr.request_type LIKE (?)   AND ms.language_id = sy.language_id    AND 		ms.language_id = ?   AND ms1.language_id =?");
	
	//Added by Rabbani #INC no:43292 on 12/12/2013 
	sqlMap.put( "sqlData","SELECT sr.facility_id, sr.doc_type_code, TO_CHAR (sr.doc_no) doc_no,       TO_CHAR (doc_date, 'dd/mm/yyyy') doc_date, sr.doc_ref,       ms.short_desc req_by_store_desc,       ms1.short_desc req_on_store_desc, sr.entry_completed_yn,       DECODE (authorized_by_id, NULL, 'N', 'Y') authorized_yn,       DECODE (ss_req_issue_chk ((?), sr.doc_type_code, doc_no),               '1', 'Not Issued', '2', 'Fully Issued', '3',              'Partially Issued') issued,       sr.cancelled_yn, sy.short_desc doc_type_desc , ss_req_ack_status (sr.facility_id, sr.doc_type_code, sr.doc_no) Ack_status,TO_CHAR (sr.ADDED_DATE, 'DD/MM/YYYY HH24:MI:SS') ADDED_DATE,(SELECT APN.APPL_USER_NAME FROM  sm_appl_user_LANG_VW APN WHERE  SR.ADDED_BY_ID = APN.APPL_USER_ID and MS.LANGUAGE_ID = APN.LANGUAGE_ID)APPL_USER_NAME FROM ss_request_hdr sr,       mm_store_lang_vw ms,       mm_store_lang_vw ms1,       sy_doc_type_master_lang_vw sy,       ss_user_access_for_store ss1,       ss_user_access_for_store ss2 WHERE	sy.doc_type_code = sr.doc_type_code   AND sr.facility_id = (?)    AND sr.req_by_store_code = ms.store_code    AND ss1.store_code = 		sr.req_by_store_code   AND ss2.store_code = sr.req_on_store_code    AND ss1.req_allowed_yn = 'Y'   AND ss2.iss_allowed_yn = 'Y'		AND ss1.user_id = (?)   AND ss2.user_id = (?) AND sr.req_on_store_code = ms1.store_code   AND TO_DATE (TO_CHAR (doc_date, 		'dd/mm/yyyy'), 'dd/mm/yyyy')          BETWEEN TO_DATE (NVL ((?), '01/01/1472'), 'dd/mm/yyyy')              AND TO_DATE (NVL 		((?), '31/12/5000'), 'dd/mm/yyyy')   AND sr.entry_completed_yn LIKE (?)   AND DECODE (sr.authorized_by_id, NULL, 'N', 'Y') LIKE 		(?)   AND ss_req_issue_chk ((?), sr.doc_type_code, doc_no) LIKE (?)   AND sr.cancelled_yn LIKE (?)   AND sr.req_by_store_code 		LIKE (?)   AND sr.req_on_store_code LIKE (?)   AND sr.request_type LIKE (?)   AND ms.language_id = sy.language_id     AND 		ms.language_id = ?   AND ms1.language_id =?");
	...
	displayFields.add( "Ack_status"	); //Added by Rabbani #INC no:32068 on 06/12/2012
	displayFields.add("APPL_USER_NAME"); //Added by Rabbani #INC no:43292 on 12/12/2013
	displayFields.add("ADDED_DATE"); //Added by Rabbani #INC no:43292 on 12/12/2013
...
//				String link_columns = "1";
 ...
 if(AckForReqyn) {
	...
	out.println(CommonBean.setForm ( request ,"../../eSS/jsp/RequestStatusQueryResult.jsp", searched) );
} catch(Exception e) {
    ...
    private boolean _jsp__tag10(javax.servlet.ServletRequest request, javax.servlet.ServletResponse response, javax.servlet.jsp.PageContext pageContext, javax.servlet.jsp.tagext.JspTag activeTag, javax.servlet.jsp.tagext.JspTag parent) throws java.lang.Throwable
    {
        ...
        __tag10.setKey(( java.lang.String ) weblogic.jsp.internal.jsp.utils.JspRuntimeUtils.convertType("eSS.AcknowledgementStatus.label", java.lang.String .class,"key"));
