...
public final class __salesreturnqueryresult extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/est/jsp/SalesReturnQueryResult.jsp", 1720509525838L ,"10.3.6.0","Asia/Calcutta")) return true;
...
String locale			= (String)session.getAttribute("LOCALE");
		...
		try{
		  ...
		  //sqlMap.put("sqlData", eST.Common.StRepository.getStKeyValue("SQL_ST_SAL_RET_HDR_QUERY_RESULT"));
	      //sqlMap.put("sqlData","SELECT hdr.doc_type_code, sy.short_desc doc_type_desc,hdr.doc_no, TO_CHAR(hdr.doc_date,'dd/mm/yyyy') doc_date, hdr.store_code,  fms.short_desc to_store_desc, hdr.patient_id,NVL(hdr.encounter_id,'') encounter_id FROM st_sal_ret_hdr hdr, mm_store_lang_vw fms, sy_doc_type_master_lang_vw sy, st_user_access_for_store uaca /* ,  st_acc_entity_param pa */ WHERE hdr.doc_type_code = sy.doc_type_code AND hdr.store_code= fms.store_code AND uaca.store_code = hdr.store_code AND hdr.finalized_yn='N' AND hdr.module_id='ST' AND uaca.srt_allowed_yn = 'Y' AND hdr.doc_type_code LIKE ? AND TO_CHAR(hdr.doc_no) LIKE  ? AND TO_CHAR(hdr.doc_date,'dd/mm/yyyy') LIKE ? AND hdr.store_code LIKE ? AND  NVL(hdr.item_class_code,'X') LIKE ? AND uaca.user_id = ? AND hdr.facility_id=?  AND sy.language_id = fms.language_id AND fms.language_id = ?  and NVL(hdr.LOCN_CODE,'%') like(?) and NVL(hdr.patient_id,'%') like (?)" );
		 	//sqlMap.put("sqlData","SELECT hdr.doc_type_code, sy.short_desc doc_type_desc,hdr.doc_no, TO_CHAR(hdr.doc_date,'dd/mm/yyyy') doc_date, hdr.store_code,  fms.short_desc to_store_desc, hdr.patient_id,NVL(hdr.encounter_id,'') encounter_id FROM st_sal_ret_hdr hdr, mm_store_lang_vw fms, sy_doc_type_master_lang_vw sy, st_user_access_for_store uaca /* ,  st_acc_entity_param pa */ WHERE hdr.doc_type_code = sy.doc_type_code AND hdr.store_code= fms.store_code AND uaca.store_code = hdr.store_code AND hdr.finalized_yn='N' AND hdr.module_id='ST' AND uaca.srt_allowed_yn = 'Y' AND hdr.doc_type_code LIKE ? AND TO_CHAR(hdr.doc_no) LIKE  ? AND hdr.doc_date BETWEEN  TO_DATE (?, 'dd/mm/yyyy')  AND NVL (TO_DATE (?, 'dd/mm/yyyy'), SYSDATE) AND hdr.store_code LIKE ? AND  NVL(hdr.item_class_code,'X') LIKE ? AND uaca.user_id = ? AND hdr.facility_id=?  AND sy.language_id = fms.language_id AND fms.language_id = ?  and NVL(hdr.LOCN_CODE,'%') like(?) and NVL(hdr.patient_id,'%') like (?)" );
			
			
			String sql = "SELECT hdr.doc_type_code, sy.short_desc doc_type_desc,hdr.doc_no, TO_CHAR(hdr.doc_date,'dd/mm/yyyy') doc_date, hdr.store_code,  fms.short_desc to_store_desc, hdr.patient_id,NVL(hdr.encounter_id,'') encounter_id FROM st_sal_ret_hdr hdr, mm_store_lang_vw fms, sy_doc_type_master_lang_vw sy, st_user_access_for_store uaca /* ,  st_acc_entity_param pa */ WHERE hdr.doc_type_code = sy.doc_type_code AND hdr.store_code= fms.store_code AND uaca.store_code = hdr.store_code AND hdr.finalized_yn='N' AND hdr.module_id='ST' AND uaca.srt_allowed_yn = 'Y' AND hdr.doc_date BETWEEN  TO_DATE (?, 'dd/mm/yyyy')  AND NVL (TO_DATE (?, 'dd/mm/yyyy'), SYSDATE) AND   uaca.user_id = ? AND hdr.facility_id=?  AND sy.language_id = fms.language_id AND fms.language_id = ?  ";
...
//			displayFields.add("ITEM_CLASS_DESC");
		...
		out.println(CommonBean.setForm (request ,"../../eST/jsp/SalesReturnQueryResult.jsp", searched));
	}
	catch(Exception e) {
