...
public final class __cancelrequestvalidate extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/est/jsp/CancelRequestValidate.jsp", 1719305688101L ,"10.3.6.0","Asia/Calcutta")) return true;
	...
	case 3:
		{
			request_by_store				=		(String) hash.get("request_by_store");
				...
				//To populate the requisition_on_stores
					out.println("clearListItems('formCancelRequestQueryCriteria.request_on_store'); ");
					...
					for(int i=0; i< req_by_stores.size(); i++) {
						...
						out.println("addListItem(\"formCancelRequestQueryCriteria.request_on_store\", \"" + record.get("PARENT_STORE_CODE") + "\",\"" + record.get("SHORT_DESC") + "\");");
		...
		case 4:
		{
			...
			out.println("clearListItems('formCancelRequestQueryCriteria.item_class_code'); ");
			...
			for(int i=0; i< itemClasses.size(); i++) {
				...
				out.println("addListItem(\"formCancelRequestQueryCriteria.item_class_code\", \"" + record.get("ITEM_CLASS_CODE") + "\",\"" + record.get("SHORT_DESC") + "\");");
			}
		}
		break;
		case 5:
		{
			...
			out.println("clearListItems('formCancelRequestQueryCriteria.request_by_store'); ");

			ArrayList request_by_store_list = bean.getRequestOnStore(request_type);
			HashMap record =  new HashMap();
			for(int i=0; i< request_by_store_list.size(); i++) {
				record = (java.util.HashMap)request_by_store_list.get(i);
				out.println("addListItem(\"formCancelRequestQueryCriteria.request_by_store\", \"" + record.get("STORE_CODE") + "\",\"" + record.get("SHORT_DESC") + "\");");
			}
		}
		break;
