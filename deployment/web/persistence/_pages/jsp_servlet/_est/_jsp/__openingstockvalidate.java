...
public final class __openingstockvalidate extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/est/jsp/OpeningStockValidate.jsp", 1720499387099L ,"10.3.6.0","Asia/Calcutta")) return true;
...
bean.setLanguageId(locale);
	...
	try {
		...
		if (validate.equals("ADD_TO_LIST")) {
			...
			String gen_uom					=	request.getParameter("gen_uom");
			flag="";
			...
			hashmapDtl.put("gen_uom",			gen_uom		);
		...
		// from here
			...
			result = bean.isEntryAlreadyExists (item_code, store_code);	
	...
	if(item_store_exists>0){
			...
			if(item_code.equals("")){
			  ...
			  out.println("document.formOpeningStockListHeader.item_desc.value = '"+bean.checkForNull(Item_desc_val,"")+"';");
			  out.println("document.formOpeningStockListHeader.barcode.value = ''");
			    result=true;
			}else{
				out.println("document.formOpeningStockListHeader.barcode.value = ''");
				if(item_code.equals(barcode_item)){
					for(int i=0;i<batchData.size();i+=3){ 
				     out.println("parent.parent.frameOpeningStockList.frameOpeningStockListDetail.document.formOpeningStockListDetail.expiry_date_or_receipt_date_"+j+".value = '"+batchData.get(i+1)+"';");
					 out.println("parent.parent.frameOpeningStockList.frameOpeningStockListDetail.document.formOpeningStockListDetail.batch_id_"+j+".value = '"+batchData.get(i)+"';");
					 out.println("parent.parent.frameOpeningStockList.frameOpeningStockListDetail.document.formOpeningStockListDetail.trade_id_"+j+".value = '"+batchData.get(i+2)+"';");
					 j++;
					}
				}
			 }
