...
public final class __queryalternateid extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/emp/jsp/queryAlternateID.jsp", 1709118699340L ,"10.3.6.0","Asia/Calcutta")) return true;
...
//	String sql ;									// will be deleted once code walkthrough is done
	...
	{
//	sql = "";
	String cd = request.getParameter("alt_id_type");
	if(cd != null)
		cd=cd.toUpperCase();
	...
	if ( !(cd == null || cd.equals("")) )
		{
		sbQuery.append("where upper(alt_id_type) like '");
		sbQuery.append(cd);
		sbQuery.append("%'");
//		sql = "where alt_id_type like '"+cd+"%'";
		andCheck = 1;
		}
...
try{
	...
	/*pstmt = conn.prepareStatement("select count(*) as total from mp_alternate_id_type"+sbQuery.toString());
	...
	if (maxRecord==0) 			out.println("<script>alert(getMessage('NO_RECORD_FOUND_FOR_CRITERIA','COMMOM'));history.go(-1);</script>");*/
	
	pstmt = conn.prepareStatement("select alt_id_type, long_desc, short_desc, eff_Status from mp_alternate_id_type "+sbQuery.toString());
	
//	pstmt = conn.prepareStatement("select alt_id_type, long_desc, short_desc, eff_Status from mp_alternate_id_type order by alt_id_type");
	rs = pstmt.executeQuery(); 
...
while (i<=end && rs.next())
{
	if(cnt==0)
	{
		...
		if ( !(start <= 1) )
			out.println("<td align ='right' id='prev'><A HREF='../../eMP/jsp/queryAlternateID.jsp?from="+(start-14)+"&to="+(end-14)+"&whereclause="+URLEncoder.encode(sbQuery.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</td></A>");
		
            _bw.write(_wl_block9Bytes, _wl_block9);

		//if ( !( (start+14) > maxRecord ) )
			out.println("<A HREF='../../eMP/jsp/queryAlternateID.jsp?from="+(start+14)+"&to="+(end+14)+"&whereclause="+URLEncoder.encode(sbQuery.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
	...
	String code = rs.getString("alt_id_type");

	out.println("<a href='../../eMP/jsp/alternateIDModify.jsp?alternate_id_code="+ code + "' target='f_query_add_mod' >");
