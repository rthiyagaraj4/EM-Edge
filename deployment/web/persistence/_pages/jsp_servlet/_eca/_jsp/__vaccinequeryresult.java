...
public final class __vaccinequeryresult extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eca/jsp/VaccineQueryResult.jsp", 1709116414901L ,"10.3.6.0","Asia/Calcutta")) return true;
	...
	if ( !(vaccine_id == null || vaccine_id.equals("")) )
        {
			sql.append(" where upper( VACCINE_ID) like upper('"+vaccine_id +"%')");
			count =  1;
        }
...
ResultSet rset = null;
ResultSet rs = null;
String strsql="select count(*) as total from CA_IM_VACCINE "+sql.toString();

String strsql1="select VACCINE_ID,LONG_DESC,SHORT_DESC,EFF_STATUS from  CA_IM_VACCINE "+sql.toString();
...
if ( !(start <= 1) )
    out.println("<A class='gridLink'  href='../../eCA/jsp/VaccineQueryResult.jsp?from="+(start-pageCnt)+"&to="+(end-pageCnt)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A>");

if ( !( (start+pageCnt) > maxRecord ) )
    out.println("&nbsp;<A class='gridLink'  href='../../eCA/jsp/VaccineQueryResult.jsp?from="+(start+pageCnt)+"&to="+(end+pageCnt)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
...
rset = stmt1.executeQuery();
      ...
      while ( rset.next() && i<=end  )
  {
		...
		String vaccine_id1		=(rset.getString("VACCINE_ID")==null)?"":rset.getString("VACCINE_ID");
		...
		out.println("<a class='gridLink'  href='../../eCA/jsp/VaccineAddModify.jsp?mode=update&vaccine_id="+ URLEncoder.encode(vaccine_id1)+"' target='f_query_add_mod' >");
		out.println(vaccine_id1 + "</a></td><td class='" + classValue + "'>");
