...
public final class __ormdrointermediate extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eOR/jsp/ORMDROIntermediate.jsp", 1721626133492L ,"10.3.6.0","Asia/Calcutta")) return true;
		...
		String mdro_reason = (String) hashObj.get("mdro_reason") == null ? "": (String) hashObj.get("mdro_reason");
		 if("POPUP_ENCOUNTER".equals(action))
		 {
		    ...
		    pstmt.setString(2,p_facility_id);
			 ...
			 int enctr_count=0;
			 pstmt = con.prepareStatement( "SELECT COUNT(A.ENCOUNTER_ID) ENCRT_COUNT FROM PR_ENCOUNTER A, MP_PATIENT B WHERE A.PATIENT_ID = B.PATIENT_ID AND A.PATIENT_ID = ? AND ( ( A.PATIENT_CLASS IN ('IP', 'DC') AND A.ADT_STATUS NOT IN ('00', '09') ) OR ( A.PATIENT_CLASS IN ('OP', 'EM') AND A.VISIT_STATUS NOT IN ('01', '02', '99') ) ) AND A.FACILITY_ID = ?" ) ;
				pstmt.setString(1,p_patient_id);
				pstmt.setString(2,p_facility_id);
				rs = pstmt.executeQuery() ;
				while ( rs != null && rs.next() ) {
		 ...
		 else if("REASON_CODE_VALID".equals(action)){
			 String action_reason_desc="";
			 pstmt = con.prepareStatement( "select ACTION_REASON_CODE,ACTION_REASON_DESC,ACTION_DESCRIPTION from or_order_action_reason where ACTION_TYPE='MD' AND EFF_STATUS='E' AND ACTION_REASON_CODE=?" ) ;
				pstmt.setString(1,mdro_reason);
				rs = pstmt.executeQuery() ;
				while ( rs != null && rs.next() ) {
					action_reason_desc =  rs.getString("ACTION_REASON_DESC");
					  out.println(action_reason_desc);
					  System.out.println("action_reason_desc====ORMDROIntermediate.jsp============>"+action_reason_desc);
          }
		 }
		 if(rs!=null) rs.close();
