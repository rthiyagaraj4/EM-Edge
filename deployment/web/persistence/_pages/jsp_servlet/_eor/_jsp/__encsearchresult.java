...
public final class __encsearchresult extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eor/jsp/EncSearchResult.jsp", 1723118612000L ,"10.3.6.0","Asia/Calcutta")) return true;
    ...
    private final static java.lang.String  _wl_block16 ="\t\t\n\t\t<table border=\"1\" width=\"100%\" cellspacing=\'0\' cellpadding=\'0\' class=\"grid\">\n\t\t";
    ...
    private final static java.lang.String  _wl_block39 ="</TABLE>\n";
    ...
    private final static java.lang.String  _wl_block80 ="\'></input>  <!-- IN057196 -->\n\t<br><center>\n";
...
24/06/2014	  IN056062	 	Nijitha S			KDAH-SCF-0310
...
//Check for bl_interfaced_yn condition.......added by kishore
	...
	//IN051968 - Start
	//IN056062 Start
	//String actionOnPendingBill = "";
	String actionOnPendingBill_OP = "";
	String actionOnPendingBill_AE = "";
	//if("OP".equals(Enc_Type) || "EM".equals(Enc_Type))
	//{
		//String query_actionBill	   = "";  
		//if("OP".equals(Enc_Type))
		//{
			 //query_actionBill="SELECT ACTION_ON_PENDING_BILL FROM OP_PARAM WHERE OPERATING_FACILITY_ID='"+FacilityId+"'";
		//} 
		//else if("EM".equals(Enc_Type))	
		//{
			 //query_actionBill="SELECT ACTION_ON_PENDING_BILL FROM AE_PARAM WHERE OPERATING_FACILITY_ID='"+FacilityId+"'";
		//} 
		String query_actionBill_OP	   = "";  
		String query_actionBill_AE	   = "";  

		query_actionBill_OP="SELECT ACTION_ON_PENDING_BILL FROM OP_PARAM WHERE OPERATING_FACILITY_ID='"+FacilityId+"'";
		query_actionBill_AE="SELECT ACTION_ON_PENDING_BILL FROM AE_PARAM WHERE OPERATING_FACILITY_ID='"+FacilityId+"'";
		try
		{
			stmt_bl=conn.createStatement();
			//rs_bl=stmt_bl.executeQuery(query_actionBill);
			rs_bl=stmt_bl.executeQuery(query_actionBill_OP);
			if(rs_bl.next())
			{
				//actionOnPendingBill=rs_bl.getString("ACTION_ON_PENDING_BILL");
				actionOnPendingBill_OP=rs_bl.getString("ACTION_ON_PENDING_BILL");
			}
			if(rs_bl != null) rs_bl.close();
			rs_bl=stmt_bl.executeQuery(query_actionBill_AE);
			if(rs_bl.next())
			{
				actionOnPendingBill_AE=rs_bl.getString("ACTION_ON_PENDING_BILL");
			}
		//IN056062 Ends	
		if(rs_bl != null) rs_bl.close();
		if(stmt_bl != null) stmt_bl.close(); 
...
if ( !(orderBy == null || orderBy.equals("")) )
{
	...
	else
	{
		...
		//sqlbuff.append(" (CASE WHEN a.Attend_Practitioner_Id IS NOT NULL  THEN AM_GET_DESC.AM_PRACTITIONER(a.Attend_Practitioner_Id,'"+locale+"','1') END) Practitioner_Name, a.assign_room_num room_no, a.assign_bed_num bed_no,AM_GET_DESC.AM_CARE_LOCN_TYPE(a.Assign_Care_Locn_Type,'"+locale+"','1') Locn_Type, a.assign_care_locn_type, b.alt_id1_no alt_id1_no, b.alt_id2_no alt_id2_no, b.alt_id3_no alt_id3_no, b.alt_id4_no alt_id4_no,AM_GET_DESC.AM_PATIENT_CLASS(a.Patient_Class,'"+locale+"','1') Patient_Class, a.patient_class patient_class_code, TO_CHAR (a.discharge_date_time,'dd/mm/yy hh24:mi') discharge_date_time, a.OP_EPISODE_VISIT_NUM visit_id, a.EPISODE_ID EPISODE_ID FROM PR_ENCOUNTER a, MP_PATIENT b ");//IN051968 //common-icn-0180
		sqlbuff.append(" (CASE WHEN a.Attend_Practitioner_Id IS NOT NULL  THEN AM_GET_DESC.AM_PRACTITIONER(a.Attend_Practitioner_Id,?,'1') END) Practitioner_Name, a.assign_room_num room_no, a.assign_bed_num bed_no,AM_GET_DESC.AM_CARE_LOCN_TYPE(a.Assign_Care_Locn_Type,?,'1') Locn_Type, a.assign_care_locn_type, b.alt_id1_no alt_id1_no, b.alt_id2_no alt_id2_no, b.alt_id3_no alt_id3_no, b.alt_id4_no alt_id4_no,AM_GET_DESC.AM_PATIENT_CLASS(a.Patient_Class,?,'1') Patient_Class, a.patient_class patient_class_code, TO_CHAR (a.discharge_date_time,'dd/mm/yy hh24:mi') discharge_date_time, a.OP_EPISODE_VISIT_NUM visit_id, a.EPISODE_ID EPISODE_ID FROM PR_ENCOUNTER a, MP_PATIENT b ");//IN051968 //common-icn-0180
		...
		if(maxRecord>0 && start >0)
		{
		
            _bw.write(_wl_block17Bytes, _wl_block17);

					if ( !(start <= 1) )
						out.println("<A class='gridLink'  HREF=\"javascript:submitPrevNext("+(start-10)+","+(end-10)+")\" text-decoration='none'>"+"   "+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A>");
					if ( !( (start+10) > maxRecord ) )
						out.println("<A class='gridLink' HREF=\"javascript:submitPrevNext("+(start+10)+","+(end+10)+")\" text-decoration='none'>"+"   "+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
		...
		if ( rs != null )
		{
			...
			for( int j=1; j<start; i++,j++ )
				...
				while ( rs.next() && i<=end )
				{
					...
					String visit_id= checkForNull(rs.getString("visit_id"));//IN051968
					String episode_id = checkForNull(rs.getString("EPISODE_ID"));//IN051968
		...
		{	
			...
			out.println("<td align='left' class='"+classValue+"'>");
			out.print("<a href=\"javascript:closeWin('"+Encounter_ID+"','"+Patient_ID+"','"+"N"+"','"+""+"','"+call_function+"','"+install_yn+"')\">");	
		...
		else
		{
			...
			{	
				...
				if (!PatientId.equals(""))
			   {
					...
					//IN056062 Starts
					if("OP".equals(Patient_Class_code))
					{
						out.println("<td nowrap class='"+classValue+"'><a href=javascript:callWin('"+Patient_Class_code+"','ORDER_ENTRY','"+Encounter_ID+"','"+Patient_ID+"','"+visit_adm_type+"','"+P_DEP_APPL_YN+"','"+FacilityId+"','"+visit_id+"','"+actionOnPendingBill_OP+"','"+episode_id+"','"+cnsnt_with_order_yn+"')>"+Encounter_ID+"</a></td>"); //IN051968 included visit_id,actionOnPendingBill,episode_id
					}
					else if("EM".equals(Patient_Class_code))
					{
						out.println("<td nowrap class='"+classValue+"'><a href=javascript:callWin('"+Patient_Class_code+"','ORDER_ENTRY','"+Encounter_ID+"','"+Patient_ID+"','"+visit_adm_type+"','"+P_DEP_APPL_YN+"','"+FacilityId+"','"+visit_id+"','"+actionOnPendingBill_AE+"','"+episode_id+"','"+cnsnt_with_order_yn+"')>"+Encounter_ID+"</a></td>"); //IN051968 included visit_id,actionOnPendingBill,episode_id
					}
					else
					{
						out.println("<td nowrap class='"+classValue+"'><a href=javascript:callWin('"+Patient_Class_code+"','ORDER_ENTRY','"+Encounter_ID+"','"+Patient_ID+"','"+visit_adm_type+"','"+P_DEP_APPL_YN+"','"+FacilityId+"','"+visit_id+"','','"+episode_id+"','"+cnsnt_with_order_yn+"')>"+Encounter_ID+"</a></td>"); //IN051968  included visit_id,actionOnPendingBill,episode_id
					}
					//IN056062 Ends
			    } 
			     ...
			     { 
					  //out.println("<td nowrap class='"+classValue+"'><a href=javascript:callWin('"+Patient_Class_code+"','ORDER_ENTRY','"+Encounter_ID+"','"+Patient_ID+"','"+visit_adm_type+"','"+P_DEP_APPL_YN+"','"+FacilityId+"')>"+Encounter_ID+"</a></td>"); 
					  //out.println("<td nowrap class='"+classValue+"'><a href=javascript:callWin('"+Patient_Class_code+"','ORDER_ENTRY','"+Encounter_ID+"','"+Patient_ID+"','"+visit_adm_type+"','"+P_DEP_APPL_YN+"','"+FacilityId+"','"+visit_id+"','"+actionOnPendingBill_OP+"','"+actionOnPendingBill_AE+"','"+episode_id+"')>"+Encounter_ID+"</a></td>"); //IN051968 included visit_id,actionOnPendingBill,episode_id
					  if("OP".equals(Patient_Class_code))
					  {
						out.println("<td nowrap class='"+classValue+"'><a href=javascript:callWin('"+Patient_Class_code+"','ORDER_ENTRY','"+Encounter_ID+"','"+Patient_ID+"','"+visit_adm_type+"','"+P_DEP_APPL_YN+"','"+FacilityId+"','"+visit_id+"','"+actionOnPendingBill_OP+"','"+episode_id+"','"+cnsnt_with_order_yn+"')>"+Encounter_ID+"</a></td>");
					  }
					  else if("EM".equals(Patient_Class_code))
					  {
						out.println("<td nowrap class='"+classValue+"'><a href=javascript:callWin('"+Patient_Class_code+"','ORDER_ENTRY','"+Encounter_ID+"','"+Patient_ID+"','"+visit_adm_type+"','"+P_DEP_APPL_YN+"','"+FacilityId+"','"+visit_id+"','"+actionOnPendingBill_AE+"','"+episode_id+"','"+cnsnt_with_order_yn+"')>"+Encounter_ID+"</a></td>");
					  }
					  else
					  {
						out.println("<td nowrap class='"+classValue+"'><a href=javascript:callWin('"+Patient_Class_code+"','ORDER_ENTRY','"+Encounter_ID+"','"+Patient_ID+"','"+visit_adm_type+"','"+P_DEP_APPL_YN+"','"+FacilityId+"','"+visit_id+"','','"+episode_id+"','"+cnsnt_with_order_yn+"')>"+Encounter_ID+"</a></td>");
					  }
					  out.println("<td nowrap class='"+classValue+"'>"+Patient_ID+"</td>");
				  }
			}
	...
	if(!finalSelect.equals("SELECTALL"))
	{
		...
		while(rs.next())
		{
			if(ssm.containsKey(new String(i+"")))
			{
				...
				if(visit_id==null	      || visit_id.equals("null"))		    
					visit_id = "";
				//IN051968 - End
