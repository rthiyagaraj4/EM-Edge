...
public class OperationBillingManager  
{
	...
	private  OperationBillingManager(HashMap commonData)
	{
		audit_flds = (ArrayList)commonData.get("mandatory_flds");
		comon_hdr = (HashMap)commonData.get("checkin_hdr");
		oper_map = (HashMap)commonData.get("post_oper_dtls");
	}

public boolean processBilling(Connection connection) 
{
		//checks billing is installed
		if(!isBillingInstalled()) 
			return true;//true
		//return "SUCCESS" string if procedure is executed successfully.
//		if (!"RECORD_INSERTED".equals(GeneratePatientBill(connection)))//50700
//			return false;
		if (("RECORD_INSERTED").equals(GeneratePatientBill(connection)))
		{
				System.err.println("cmg inside call billing");
			if (!"RECORD_INSERTED".equals(callBillRecord(connection)))
			{
				tearDown();
				return false;
			}
	...
	public static OperationBillingManager intializeOperationBillingManager(HashMap commonData)
	{
		return new OperationBillingManager(commonData);
	}
	...
	private boolean isBillingInstalled()
	{
		...
		String status="";
		try{
			con = ConnectionManager.getConnection();// Changed on 03/07/13 against UAT5 - Connection for checking whether the Billing module is installed or not
...
public String GeneratePatientBill(Connection con) 
{
	...
	String oper_num	    =comon_hdr.get("oper_num") ;
	String episode_id		= comon_hdr.get("episode_id") ;
	String episode_type	=comon_hdr.get("episode_type") ;
	String patient_class	= comon_hdr.get("patient_class") ;
	String ws_no = audit_flds.get(1);
	String oper_date		=comon_hdr.get("surgery_date") ;
	String surgeon_code		=comon_hdr.get("surgeon_code") ;
	String visit_id			=comon_hdr.get("visit_id") ;
	String v_stage = "REG";// if it is from booking -- "ORD" check-In--"REG"
	String oper_code = "";
	...
	try 
	{
			...
			System.err.println("cmg size :" +size);
	for (int i = 1; i <= size; i++) 
		{
			map = oper_map.get(""+i);
			oper_code=map.get("operation_code");
	...
	//if(panel_str!= null || !panel_str.equals("") || !panel_str.equals("null"))//50700
	if(panel_str != null && panel_str.trim().length() > 0 && !panel_str.equals("null")) //50700
		{
	...
	if(pract_arr.length >5)
		...
		String bill_str=map.get("bill_str");
		if(!bill_str.equals("") || bill_str !="")
		{
		  ...
		  cstmt.setString(5,map.get("order_id") );	//v_order_id                                    VARCHAR2,
		  cstmt.setString(6,map.get("order_line_num"));//v_ord_line_no                                VARCHAR2,
		  // map.get("operation_code") //modified for chekcing error
		  // message in front end
		  cstmt.setString(7, map.get("operation_code") );	//v_oper_code				  VARCHAR2,
		  ...
		  cstmt.setString(12, v_stage); // v_stage VARCHAR2, modified for chekcing error message in front end
          ...
          cstmt.setString(30,"OT");
		   if(("null").equals(overridden_excl_incl_ind))
			...
			if(("null").equals(action_reason_code))
				...
				cstmt.setString(33,map.get("fpp_yn")) ;	    //fpp_yn	      VARCHAR2 default N
				cstmt.setString(34,map.get("fpp_category")) ;	//fpp_category	  VARCHAR2 default null
				cstmt.execute();
	...
	private String callBillRecord(Connection con) 
	{
		...
		String episode_type			  =comon_hdr.get("episode_type") ;
		String patient_class	      = comon_hdr.get("patient_class") ;
		String visit_id			      =checkForNull(comon_hdr.get("visit_id") );
		String order_id               = comon_hdr.get("order_id");
		String oper_num               = comon_hdr.get("oper_num");
		String bl_episode_type = BillingUtil.getBillingEpisodeType(patient_class,episode_type);
		//Connection con = ConnectionManager.getConnection();// Changed on 03/07/13 against UAT5 - Connection
		CallableStatement cstmt = null;
		try 
		{
			...
			{
				System.err.println("Operation Billing Manger Err Msg in callBillRecord "+e);
			}
		finally 
		{
			try 
