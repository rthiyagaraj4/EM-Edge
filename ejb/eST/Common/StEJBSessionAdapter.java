...
public abstract class StEJBSessionAdapter extends CommonEJBSessionAdapter {
...
//	protected String RECORD_ALREADY_LOCKED  =	"RECORD_ALREADY_LOCKED" ;
	...
	*/
	...
	//	String locale				= checkForNull (hmParameters.get("language_id"));
	   // String close_req_yn         = checkForNull (hmParameters.get("close_req_yn")); //commented by Rabbani on 24-Mar-2014 for chkstyles
		...
		for(int i=0;i<iterations;i++) {
			...
			*/
			...
			if(mode.equals("MODE_MODIFY"))
			{
				...
				//above query is commented by ganga to include conv factor Thursday, April 19, 2012
				stSQL	= "SELECT SUM (NVL (a.qty_on_hand, 0))/ST_GET_UOM_CONV_FAC_SING_STR(b.item_code,b.store_code) qty_on_hand,NVL (b.item_value, 0) item_value, stock_item_yn , sum(NVL (a.qty_on_hand, 0))/ST_GET_UOM_CONV_FAC_SING_STR(b.item_code,b.store_code) store_qty_on_hand  FROM st_item_batch a, st_item_store b WHERE a.store_code(+) = b.store_code AND a.item_code(+)= b.item_code AND b.item_code = ? AND b.store_code = ? GROUP BY b.item_code, b.store_code, item_value, stock_item_yn";
			}
			else
			{
				...
				//above query is commented by ganga to include conv factor on Thursday, April 19, 2012
				stSQL	= "SELECT SUM (NVL (a.qty_on_hand, 0) - NVL (committed_qty, 0))/ST_GET_UOM_CONV_FAC_SING_STR(b.item_code,b.store_code) qty_on_hand,NVL (b.item_value, 0) item_value, stock_item_yn ,sum(NVL (a.qty_on_hand, 0))/ST_GET_UOM_CONV_FAC_SING_STR(b.item_code,b.store_code) store_qty_on_hand FROM st_item_batch a, st_item_store b WHERE a.store_code(+) = b.store_code AND a.item_code(+)= b.item_code AND b.item_code = ? AND b.store_code = ? GROUP BY b.item_code, b.store_code, item_value, stock_item_yn";
			}
...
//			hmResult =new HashMap();
			...
			if(curr_trn_stk_item_yn.equals("Y")) {
				...
				if( ( l_qty_on_hand+curr_trn_qty < 0) || (l_stk_value+curr_trn_value < 0)) {
					System.err.println("l_qty_on_hand = > "+l_qty_on_hand+" curr_trn_qty = > "+curr_trn_qty+" l_stk_value = > "+l_stk_value+" curr_trn_value = > "+curr_trn_value);
					System.err.println("StEjbSessionAdapter.java 545,hmParameters "+hmParameters);
					...
					//NegativeItemsDesc.append(" , ");		 */
					//throw new Exception("NEGATIVE_STOCK"); //Commented by suresh.r on 08-12-2014 against Inc 52457[MMS-QF-SCF-0458]
				}
...
//Changed By Mahesh to avoid Invalid Column Index in error log.....this is because PH has not completed their Variable UOM..					
			...
			//Added by suresh.r on 10-12-2014 against Inc 52457[MMS-QF-SCF-0458] beg
			stSQL = "select st_display_unit_cost(?,sysdate,'CURRENT_STOCK') UNIT_COST from dual";
			alData.clear();
			alData.add(p_item_code);
			hmResult = fetchRecord(connection,stSQL,alData);
			l_cur_unit_cost = Double.parseDouble((String)hmResult.get("UNIT_COST"));
			l_cur_unit_cost=  Double.parseDouble(setNumber(""+l_cur_unit_cost,noOfDecimalsForCost));
			//Added by suresh.r on 10-12-2014 against Inc 52457[MMS-QF-SCF-0458] end

			alData.clear();
			
			double l_mm_unit_cost = 0;

			stSQL = " select unit_cost from mm_item where ITEM_CODE = ?";
			alData.add(p_item_code);
			hmResult = fetchRecord(connection,stSQL,alData);

			l_mm_unit_cost = Double.parseDouble(checkForNull((String)hmResult.get("UNIT_COST"),"0"));
			l_mm_unit_cost=  Double.parseDouble(setNumber(""+l_mm_unit_cost,noOfDecimalsForCost));

				if(l_mm_unit_cost != l_cur_unit_cost)  {
					...
					if (l_cur_unit_cost<0){						
						...
						System.err.println("=NEGATIVE_STOCK CHECK2 l_sum_cur_stk_value="+l_sum_cur_stk_value+"==curr_trn_value="+curr_trn_value+"===l_sum_of_qty_on_hand="+l_sum_of_qty_on_hand+"====curr_trn_qty="+curr_trn_qty+"===l_sum_of_qty_on_hand="+l_sum_of_qty_on_hand+"===curr_trn_qty"+curr_trn_qty+"==l_cur_unit_cost="+l_cur_unit_cost);     // SEP for Line 694
						System.err.println("StEjbSessionAdapter.java 545,hmParameters "+hmParameters);
...
//				}
				...
				/* This is handled for table lock during in st_item_store udpate
				Added on Monday, April 29, 2013 by Ganga*/

				alData.clear();
				
				stSQL = " select * from st_item_store where item_code = ? and store_code = ? for update nowait ";
				alData.add(p_item_code);
				alData.add(curr_store_code);
				hmResult = fetchRecord(connection,stSQL,alData);
				...
				/*
				System.out.println("StEJBS 821=>stSQL="+stSQL);
				...
				//commented due to pending_req_qty in st_item_store updating at the time of Issue no need to update in Acknowledgment time 
				...
				alData.add(p_item_code);									 
				alData.add(curr_store_code);
				//Added against MMS-JU-SCF-0072 starts
					System.err.println("testing for negative stock"+l_qty_on_hand);    
				if (l_qty_on_hand<0){						
					System.err.println("=NEGATIVE_STOCK CHECK l_qty_on_hand="+l_qty_on_hand);    
					System.err.println("StEjbSessionAdapter.java 883,alData "+alData);
					hmResult.put(MESSAGE,"NEGATIVE_STOCK");
					throw new Exception("NEGATIVE_STOCK");
				}
				//Added against MMS-JU-SCF-0072 ends
				hmArgData.put(DATA,alData);
				hmArgSQL.put(UPDATE_SQL,stSQL);
				hmReturn	=	update(connection, hmArgData, hmArgSQL);
				throwExceptionWhenFailed(hmReturn);
	...
	*/
	...
	public void updateBatchOnLine (Connection connection, HashMap hmSQL, HashMap hmParameters) throws Exception {
		...
		*/
		...
		String p_supp_code						= checkForNull (hmParameters.get("supp_code"),""); 
		//added ended
		//added by Ganga Monday, November 26, 2012 IN035661	PMG2012-CRF-0012		
		String p_pur_uom_code						= checkForNull (hmParameters.get("pur_uom_code")); 
		String p_pur_uom_conv						= checkForNull (hmParameters.get("pur_uom_conv")); 
		//added ended  IN035661	PMG2012-CRF-0012
		//Added by suresh.r on 03-01-2014 against KDAH for update batch_cost into st_batch_control beg
		String p_batch_cost						= "";
		double temp_batch_cost=0;
		int noOfDecimalsForCost=Integer.parseInt(getNoOfDecimalsCost(connection));
		if(p_trn_type.equals("GRN")){
			p_batch_cost=checkForNull (hmParameters.get("grn_unit_cost_in_stk_uom"),"0");
			temp_batch_cost=Double.parseDouble(p_batch_cost);
		}
		...
		*/
		...
		if(batch_count == 0) { // Insert into batch control
			...
			alData.add(p_supp_code); 		
			//added ended
			
			//added by Ganga Monday, November 26, 2012 IN035661	PMG2012-CRF-0012	
			alData.add(p_pur_uom_code); 			
			alData.add(p_pur_uom_conv); 
			//added by ended IN035661	PMG2012-CRF-0012
			alData.add(p_batch_cost);//Added by suresh.r on 03-01-2014 against KDAH for update batch_cost into st_batch_control

			System.out.println("alData======1003"+alData);
			...
			try {
				...
				System.err.println("hmParameters=STEJB=945==>"+hmParameters);
				System.err.println("alData-BC->"+alData);
				System.err.println("stSQL-BC-->"+stSQL);
				throw ex;
			}
		}

		//Added by suresh.r on 03-01-2014 against KDAH for update batch_cost into st_batch_control beg
		if(batch_count > 0 && p_trn_type.equals("GRN")) {
			double prev_qty	=0;
			stSQL = eST.Common.StEJBRepository.getStKeyValue("SQL_ST_ITEM_BATCH_SELECT_QTY_ONHAND");
			alData.clear();
			alData.add(p_from_store_code);
			alData.add(p_item_code);
			alData.add(p_batch_id);
			alData.add(p_from_bin_location_code);
			alData.add(p_expiry_date_or_receipt_date);
			try{
				hmResult		= fetchRecord(connection, stSQL, alData);
				if(!hmResult.isEmpty()){
					prev_qty	= Double.parseDouble(checkForNull(hmResult.get("QTY_ON_HAND"),"0"));
				}
			}catch(Exception e){
				System.out.println("NO QTY_ONHAND FOR==>"+alData);
			}
			stSQL = eST.Common.StEJBRepository.getStKeyValue("SQL_ST_BATCH_CONTROL_SELECT_BATCH_COST");
			alData.clear();
			alData.add(p_item_code);
			alData.add(p_batch_id);
			alData.add(p_expiry_date_or_receipt_date);
			...
			String prev_batch_cost=setNumber(""+checkForNull(hmResult.get("BATCH_COST"),"0"),noOfDecimalsForCost);
			double tmp_prev_batch_cost	= Double.parseDouble(prev_batch_cost);
			double total_batch_cost=(prev_qty*tmp_prev_batch_cost)+(p_trn_qty*temp_batch_cost);
			double avg_batch_cost =total_batch_cost/(prev_qty+p_trn_qty);
			p_batch_cost=setNumber(""+avg_batch_cost,noOfDecimalsForCost);
			
			alData.clear();
			alData.add(p_batch_cost);
			alData.add(prev_batch_cost);
			alData.add(p_login_by_id);
			alData.add(p_login_at_ws_no);
			alData.add(p_login_facility_id);
			alData.add(p_item_code);
			alData.add(p_batch_id);
			alData.add(p_expiry_date_or_receipt_date);
			stSQL = eST.Common.StEJBRepository.getStKeyValue("SQL_ST_BATCH_CONTROL_UPDATE_FOR_BATCH_COST");
			hmArgData.put(DATA,alData);
			hmArgSQL.put(UPDATE_SQL,stSQL);
			...
			try {
				...
				System.err.println("Error st_batch_control Update==>"+alData);
				throw e;
			}
			...
			if(item_batch_count == 0) {
				if(curr_trn_qty >= 0) {
					...
					try {
						...
						System.err.println("alData-ITM->"+alData);
						System.err.println("stSQL-ITM-->"+stSQL);
						throw e;
					}
				}
				else {
					//throw new Exception("RECORD_UPDATED_BY_OTH_USER"); // BATCH_RECORD_NOT_FOUND
					//Comented above line and added below line against MMS-SCF-0306 by sakti 
					throw new Exception("INSUFFICIENT_BATCH_QTY"); // INSUFFICIENT_BATCH_QTY
				}
			}
			else {		
				...
				alData.add(curr_bin_location_code);
				alData.add(p_expiry_date_or_receipt_date);
				System.err.println("hmResult   in batch level Data SQL ******1261:"+stSQL);//added for AMS-SCF-0648 AND GHL-SCF-1158.1
				hmResult		=	fetchRecord(connection, stSQL, alData);	  
				...
				double l_qty_on_hand			=	0;
				double l_commited_qty_onhand			=	0;// added for AMS-SCF-0648 AND GHL-SCF-1158.1
				double l_avail_qty			=	0;//addrd for AMS-SCF-0648 AND GHL-SCF-1158.1

				if(hmResult.get("QTY_ON_HAND") != null) {
					
					l_qty_on_hand = Double.parseDouble((String)(hmResult.get("QTY_ON_HAND")));
					l_commited_qty_onhand=Double.parseDouble((String)(hmResult.get("COMMITTED_QTY")));//added for AMS-SCF-0648 AND GHL-SCF-1158.1
					l_avail_qty=l_qty_on_hand-l_commited_qty_onhand;// added for AMS-SCF-0648 AND GHL-SCF-1158.1
				}
...
System.err.println("l_commited_qty_onhand for GHL-SCF-1158.1======="+l_commited_qty_onhand+"l_qty_on_hand======"+l_qty_on_hand);
				if (l_qty_on_hand < 0) { 
				...
				//	System.err.println("=NEGATIVE_STOCK CHECK3 Query Executed for l_qty_on_hand stSQL="+stSQL+"== Data="+alData+"==== Result="+hmResult);
				...
				else if(l_qty_on_hand == 0 && l_commited_qty_onhand==0) {//added l_commited_qty_onhand AMS-SCF-0648	 AND GHL-SCF-1158.1				
					...
					try {
					/* commanded for AMS-SCF-0648 AND GHL-SCF-1158.1
						hmReturn		=	delete(connection, hmArgData, hmArgSQL);
						throwExceptionWhenFailed(hmReturn);
						*/
					// added for AMS-SCF-0648 AND GHL-SCF-1158.1
						if((((String)result_multi_chk.get("MODIFIED_DATE")).equals(curr_modified_date)) || ((curr_modified_date.equals(""))))
						{
							System.err.println("*****updateBatchOnLine******delete*"+p_login_at_ws_no);
						hmReturn		=	delete(connection, hmArgData, hmArgSQL);
						}else
						{
	                           hmReturn.put(RESULT,FALSE);//added during merging
			                   hmReturn.put(MSGID,"BATCH_RECORD_NOT_FOUND");
			                   hmReturn.put(MESSAGE,"BATCH_RECORD_NOT_FOUND");                     
							}
						throwExceptionWhenFailed(hmReturn);
					}// end AMS-SCF-0648 AND GHL-SCF-1158.1
					catch (Exception e) {
						throw e;	
					}
				}
...
// select modified_by_id and To_char(modified_date,'dd/mm/yyyy hh24:mm:ss')  
					...
					try {
						...
						//	hmReturn		=	update(connection, hmArgData, hmArgSQL); commanded for AMS-SCF-0648	AND GHL-SCF-1158.1
							//start AMS-SCF-0648 AND GHL-SCF-1158.1
							System.err.println("l_avail_qty  GHL-SCF-1158.1===="+l_avail_qty+"curr_release_qty====="+curr_release_qty);
							if(l_avail_qty-curr_release_qty>=0)
						hmReturn		=	update(connection, hmArgData, hmArgSQL);	
							else
								throw new Exception("NEGATIVE_STOCK");// end AMS-SCF-0648 AND GHL-SCF-1158.1
						}else
						{
                           hmReturn.put(RESULT,FALSE);//added during merging
			   ...
			   hmSelect	=	fetchRecord(connection, eST.Common.StEJBRepository.getStKeyValue("SQL_ST_CHECK_ITEM_BATCH_EXISTS"),alparamcount);
				...
				}else
				{
		              ...
		              hmResult.put(MESSAGE,"RECORD_UPDATED_BY_OTH_USER");
                       System.err.println("hmResult:ejbsession===1479=>" +hmResult);  						
                       System.err.println("hmParameter:ejbsession===1479=>" +hmParameter);  						
					  throwExceptionWhenFailed(hmResult);
...
//added during merging ends here
			...
			System.err.println("hmResult:ejbsession===1499=>" +hmResult);
			System.err.println("hmParameter:ejbsession===1454=>" +hmParameter); 
			exception.printStackTrace();
		}
		return hmResult;		
	}
...
*/
	...
	public void updateBatchHistoryOnLine (Connection connection, HashMap hmSQL, HashMap hmParameters) throws Exception {
		...
		String p_item_code						= checkForNull (hmParameters.get("item_code")); 
		String p_store_code						= checkForNull (hmParameters.get("store_code")); 
		String p_trn_type						= checkForNull (hmParameters.get("trn_type")); 
		String p_batch_id						= checkForNull (hmParameters.get("batch_id")); 
		String p_expiry_date_or_receipt_date	= checkForNull (hmParameters.get("expiry_date_or_receipt_date"));
		String p_trade_id						= checkForNull (hmParameters.get("trade_id")); 		
		...
		String p_login_at_ws_no					= checkForNull (hmParameters.get("login_at_ws_no"));
		String p_login_facility_id				= checkForNull (hmParameters.get("login_facility_id"));
		String p_doc_date						= checkForNull (hmParameters.get("last_receipt_date"),(String)hmParameters.get("doc_date")); 
		String p_doc_type_code					= checkForNull (hmParameters.get("grn_doc_type_code"),(String)hmParameters.get("doc_type_code"));
		String p_doc_no							= checkForNull (hmParameters.get("grn_doc_no"),(String)hmParameters.get("doc_no"));  	
		String p_po_no							= checkForNull (hmParameters.get("po_no")); 		
		String p_sale_price						= checkForNull (hmParameters.get("sale_price"));
		String p_barcode_id						= checkForNull (hmParameters.get("barcode_id")); 
		String p_supp_code						= checkForNull (hmParameters.get("supp_code"));	
		...
		//System.out.println("hmParameters=STEJB=1722==>"+hmParameters);
		
				
			stSQL	=	"SELECT COUNT	(*)	COUNT FROM ST_ITEM_BATCH_HIST WHERE STORE_CODE=?  AND	ITEM_CODE=?	AND	BATCH_ID =?	AND	TRUNC(EXPIRY_DATE_OR_RECEIPT_DATE) =TO_DATE(?,'DD/MM/YYYY') AND DOC_TYPE_CODE = ? AND DOC_NO = ? AND TRN_TYPE= ? ";

			alData.clear();
			alData.add(p_store_code);
			alData.add(p_item_code);
			alData.add(p_batch_id);
			alData.add(p_expiry_date_or_receipt_date);
			alData.add(p_doc_type_code);
			alData.add(p_doc_no);
			alData.add(p_trn_type);
			...
			//System.out.println("item_batch_count=====>>>>1112"+item_batch_count);
			
			if(item_batch_count == 0) {
				
					 stSQL = "INSERT INTO ST_ITEM_BATCH_HIST ( ITEM_CODE,STORE_CODE, BATCH_ID, EXPIRY_DATE_OR_RECEIPT_DATE,TRN_TYPE, DOC_NO, DOC_TYPE_CODE, TRADE_ID, DOC_DATE, SUPP_CODE, PO_NO,SALE_PRICE, BARCODE_ID, ADDED_BY_ID, ADDED_DATE,	ADDED_AT_WS_NO,	ADDED_FACILITY_ID, MODIFIED_BY_ID, MODIFIED_DATE, MODIFIED_AT_WS_NO, MODIFIED_FACILITY_ID) VALUES (?,?,?,TO_DATE(?,'DD/MM/YYYY'),?,?,?,?,TO_DATE(?,'DD/MM/YYYY'),?,?,?,?,?,SYSDATE,?,?,?,SYSDATE,?,?)";
	...
	public double getConvFactTemp(Connection connection,String item_code,String store_code){
		...
		try {
		   ...
		   conv_fac_temp=Double.parseDouble((String)fetchRecord(connection,"SELECT ST_GET_UOM_CONV_FAC_SING_STR(?,?) CONV_FACTOR FROM DUAL",	alParameters).get("CONV_FACTOR"));
		}	catch(Exception e){
			e.printStackTrace();
		}
