...
<BODY leftmargin=0 topmargin=0 onMouseDown="CodeArrest()" onKeyDown='lockKey()' onLoad="initSetup();">
<%
Connection con = null;
	...
	<tr>
		...
		<fieldset style=" border-color:black;"><legend style="color: black;"><b><fmt:message key="Common.to.label" bundle="${common_labels}"/></b></legend>		
			<table cellspacing=0 cellpadding=3 width='100%' align=center>
				...
				<td class=label><fmt:message key="Common.date.label" bundle="${common_labels}"/></td>
				<td class='fields'>
				<input type=text name= msg_dt2 value="<%=msg_dt2%>" size=11 onBlur='isAfterCurrDate(this);'> 
...
//            System.out.println("queryApplication : "+queryApplication);
			...
			//	System.out.println("ViewEventsOutboundHeader.jsp tempApp : "+tempApp );
				if(!tempApp.equals("") && tempApp.equalsIgnoreCase("SRREBL")) select = "selected";
		%>				 
		...
		<td class='fields'>
		<select name="eventtype" id="eventtype" id="eventtype" style="width: 200px" onchange="changeProtocolLink();">
		<option value = ''><fmt:message key="Common.all.label" bundle="${common_labels}"/></option>
		 </select>  <img name="myImage1" src='../../eCommon/images/empty.gif'></td>
		<td class=label><fmt:message key="eXH.CommMode.Label" bundle="${xh_labels}"/></td>		    
		<td class='fields'>
			<select id="comm_mode" name="comm_mode" id="comm_mode" onChange="onChangeCommMode1()" style="width: 200px" disabled>  
			<%
			// Check to populate list item Interactive			
			if(function_id != null && (function_id.equalsIgnoreCase("XH_J_ADMINISTER_EVENTS")
										|| function_id.equalsIgnoreCase("XI_J_ADMINISTER_EVENTS")
			...
			<!--  Commented as per new changes	<option value="inbound" <%=comm_mode.equalsIgnoreCase("inbound")?"selected":""%>><fmt:message key="eXH.Inbound.Label" bundle="${xh_labels}"/></option>-->
			<%}
			else{%>
		...
		{  
			stmt=con.createStatement();
//			if(function_id != null && function_id.equalsIgnoreCase("XH_J_ADMINISTER_EVENTS"))
			if(function_id != null && function_id.equalsIgnoreCase("XB_J_ADMINISTER_EVENTS"))
			{				
		...
		finally
		{
			...
			catch(Exception e)
			{
			 ...
			 %>
		<input type=hidden  name="event_status" id="event_status" id="event_status" >
<!--  Commented EventStatus as per new changes
...
<table cellspacing=0 cellpadding=3 width='100%' align=center border=0>
    ...
    <tr>
	<td class=label width="10%"><fmt:message key="eXH.Gateway.Label" bundle="${xh_labels}"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT TYPE="text" id="protocol_link_name" name="protocol_link_name" id="protocol_link_name" onBlur='searchProtocolID();changeProtocolVal()'  value="<%=protocol_link_name%>"  style="width: 160px">
	...
	<% if(strXlsAuth != null && strXlsAuth.equalsIgnoreCase("Y")){ %>
	 <td class=label > &nbsp;<fmt:message key="eXH.OutputType.Label" bundle="${xh_labels}"/> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<select id="dispMode" name="dispMode" id="dispMode" style="width: 200px" >				
	...
	<% }else{ %>
		<td>&nbsp;<INPUT TYPE="hidden" name="dispMode" id="dispMode" value="display">&nbsp;</td>
	<%}%>
		<td width="30%">
	</tr>
  
    <INPUT TYPE="hidden" name="sub_module" id="sub_module" id="sub_module" value="XB">
	<INPUT TYPE="hidden" name="interface_module" id="interface_module" id="interface_module" value="XB">
	...
	%>
		...
		<select name="sub_module" id="sub_module" id="sub_module" style="width: 150px" onChange="onChangeApplication()">				
			<option value='XB'>eBilling Interface</Option>
		<% 
		try
		{  
...
<div id="auditional" style="display:none"> 
		<table cellspacing=0 cellpadding=3 width='100%' align=center>
			...
			<td class='fields'><input type=text name=msg_date id="msg_date" value="<%=msg_date%>" size=15 onBlur='CheckDate(this);' 
			...
			<td class='fields'><input type=text size=25 id="p_patient_id1" name="pat_id" id="pat_id" value="<%=pat_id%>" maxlength="<%=pat_id_length%>" onBlur="makeValidString(this);ChangeUpperCase(this);document.forms[0].clear.onclick();" onKeypress='return CheckForSpecChars(event);'><input class='BUTTON' type="button"  name=reset value='?' onClick="callSearchScreen3()"></td>
...
<!--********************************************COMMON***********************************-->
	...
	<tr align="right">
<td colspan=4 class='BUTTON'><input class='BUTTON' type="button"  name=search id=search value='Search' onClick="func1()"></td>
</tr>
	</table>
	<input type='hidden' name='orderBy' id='orderBy' value='1,3,9'>
