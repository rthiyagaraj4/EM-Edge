...
/*END : Patient Series*/	
	...
	rdonly = rdonly_temp; //Added by Ashwini on 14-May-2019 for TBMC-CRF-0010.1
			...
			<%	}/*End TBMC-CRF-0010.1*/
			%>
				<img src='../../eCommon/images/mandatory.gif' <%=disp_nat_man_gif%> name='nat_gif' id='nat_gif'></img>
				<input type='hidden' name='aadhaarStatus' id='aadhaarStatus' value="<%=aadhaar_card_status%>">
				<input type='hidden' name='aadhaarID' id='aadhaarID' value="<%=aadhaar_id%>">
				<input type='hidden' name='aadhaarTransId' id='aadhaarTransId' value="<%=aadhaar_trans_id%>">
	...
	<!-- Alternate IDs-->	  
			...
			//&& isForm60Available
				if((!Alt_Id3_Type.equals("")) && !CalledFromFunction.equals("EmergencyReg") && isForm60Available ) { 
					...
					<tr>
						...
						<td class='fields'  >			
							<select onchange='form60OnChange();' name='formno60' id='formno60' <%=isDisabledForm60%>>
				...
				<%}%>
				<input type='hidden' name='isForm60Available' id='isForm60Available'  value="<%=isForm60Available%>" />
				<input type='hidden' name='isForm60Validation' id='isForm60Validation' value="<%=isForm60Validation%>" />
				<input type='hidden' name='isDisabledForm60' id='isDisabledForm60'value="<%=isDisabledForm60%>" />
		...
		if((!Alt_Id2_Type.equals(""))) { %>
			...
			if(((String)hash_Map.get("Alt_Id2_Reqd_Yn")).equals("Y")) { 
				...
				<input type="checkbox"  name="gccchk" id="gccchk"   <%if("Y".equals(gcc)){%>checked<%}%>  onclick='checkGCC()'/>	</span>		
		...
		<%	}%>			
			...
			%><input name="gcc" id="gccId" type="hidden" value="<%=gcc%>"/>
			<%
		...
		<%if(family_org_id_accept_yn.equals("Y")){%>
			...
			<%if(CalledFromFunction.equals("ChangePatDtls")){
				...
				<%out.print("<td  class='fields'   ><input type='text' name='family_link_no' id='family_link_no' maxlength=\""+(String)hash_Map.get("Patient_Id_Length")+"\" size=\""+(String)hash_Map.get("Patient_Id_Length")+"\"   ");
					...
					<%}else{%><input type='button' id='head_pat_search_reg' name='head_pat_search_reg' class='Button' value='?' onclick='get_patient_id()' >
					<%} %>  </td>
...
<% String emerg_sex_event = "";
	...
	String sel_unknown = "";
			...
			<tr>
				...
				<input type='text' name='b_age' id='b_age' style='width:35px' maxlength='3' onblur='gotoNext(this);disableEnableYMDH(this);fnMakeMandNatIdAltId();' <%=disable_age%>  value='<%=year_value%>' onKeyPress="return ChkNumberInput(this,event,0)">&nbsp;Y
				<input type='text' name='b_months' id='b_months' style='width:25px' maxlength='2' onblur='gotoNext(this);disableEnableYMDH(this);fnMakeMandNatIdAltId();' <%=disable_age%>  value='<%=month_value%>' onKeyPress="return ChkNumberInput(this,event,0)">&nbsp;M
				<input type='text' name='b_days' id='b_days' style='width:25px' maxlength='2' onblur='gotoNext(this);disableEnableYMDH(this);fnMakeMandNatIdAltId();' <%=disable_age%>  value="<%=day_value%>" onKeyPress="return ChkNumberInput(this,event,0)">&nbsp;D
				...
				<% } %> 
				&nbsp;<input type='text' name='date_of_birth' id='date_of_birth' maxlength='10' size='8' <%=disable_dob%>    value="" onblur='gotoNext(this);disableEnableYMDH(this);calc_hij(this);fnMakeMandNatIdAltId();'>
				<%-- <img id='imgdateofbirth' src='../../eCommon/images/CommonCalendar.gif'  onClick="document.forms[0].date_of_birth.select();return showCalendar('date_of_birth','','','');"  <%=disable_dob%>> --%>
				<img id='imgdateofbirth' src='../../eCommon/images/CommonCalendar.gif'  onClick="if (!document.getElementById('date_of_birth').disabled) {document.forms[0].date_of_birth.select();return showCalendar('date_of_birth','','','');}"  <%=disable_dob%>>
			...
			%>
				...
				<%
					...
					<%}else{%><td class='fields' colspan='2'><input type='hidden' name='exp_val_appl_yn' id='exp_val_appl_yn' value=''><input type='hidden' name='pat_cat_code_exp_date' id='pat_cat_code_exp_date'  value=""><img id='imgCatExpDateMan' src='../../eCommon/images/mandatory.gif' style="visibility:hidden" ></img></td><%}%>
					<%if(CalledFromFunction.equals("ChangePatDtls")){
				String pat_cat_code = checkForNull((String)hash_Map1.get("pat_cat_code"));%>
		...
		<tr>
					...
					<td   >
						<table width='100%'>
						  <tr>
							<td class="oaBorder"   >
								<input type='radio' name='citizen' id='citizen_y' value='Y' <%=check%> <%=checkenable%> onClick='changestatdis();' ><fmt:message key="Common.citizen.label" bundle="${common_labels}"/>
								<input type='radio' name='citizen' id='citizen_n' value='Y' <%=checkNon%>	<%=checkenable%> onClick='changestatus();' ><fmt:message key="Common.noncitizen.label" bundle="${common_labels}"/></td>
				...
				<%} else if(CalledFromFunction.equals("PatientRegistration") || CalledFromFunction.equals("RapidReg") || CalledFromFunction.equals("EmergencyReg")){%>
					...
					<td    >
						<table width='100%'>
							<tr>
								<td class="oaBorder"  >
								<input type='radio'  name='citizen' id='citizen' value='Y' checked onClick='change_stat_dis();' ><label for='citizenid' id='lbl1'><fmt:message key="Common.citizen.label" bundle="${common_labels}"/></label>
								<%
								...
								<%}else{%><input type='radio' name='citizen' id='citizen' value='Y' onClick='change_status_emer();' ><%}%><label for='citizenid2' id='lbl2'><fmt:message key="Common.noncitizen.label" bundle="${common_labels}"/></label></td>
			...
			%>
				...
				<input type = 'hidden' name = 'default_race_desc' id = 'default_race_desc' value='<%=checkForNull((String)hash_Map6.get("race_desc"))%>'>
				</td>
				...
				if(CalledFromFunction.equals("PatientRegistration") || CalledFromFunction.equals("ChangePatDtls"))  { %>
					...
					<% if(disableRegnDate) {
						if(CalledFromFunction.equals("ChangePatDtls") && downtime_regn_yn.equals("Y") && checkForNull((String)hash_Map1.get("regn_date")).equals("")) {  %>
							...
							<td  class='fields'><input type='text' name="disas_regn_date_time" id="disas_regn_date_time"  value="<%=regn_date%>" maxLength='19' size='19' onBlur='chkRegDateTime(this);' ><img src="../../eCommon/images/CommonCalendar.gif"  onClick="return showCalendar('disas_regn_date_time','dd/mm/yyyy' ,'hh:mm',null );" style='cursor:pointer'></img>&nbsp;<img src='../../eCommon/images/mandatory.gif' ></img></td>
						<%}else{ %>
					<td   class="label" ><fmt:message key="Common.RegistrationDate.label" bundle="${common_labels}"/> <fmt:message key="Common.time.label" bundle="${common_labels}"/></td>
							<td class='fields' ><input type='text'  name="disas_regn_date_time" id="disas_regn_date_time"  value="<%=regn_date%>" maxLength='19' size='19' disabled><img src="../../eCommon/images/CommonCalendar.gif" style='cursor:pointer' disabled></img></td>  
						<%} 
					}
					else{
					%>
