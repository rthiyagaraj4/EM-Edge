...
%>
		...
		<jsp:include page="PatNamesComp.jsp" flush="true"/>
	...
	<table cellpadding=0 cellspacing=0 border=0 width='100%' tabindex=-1>
	 <tr><td colspan='8' class='white'></td></tr>
	 	<input type='hidden' name='op_call' id='op_call' value="<%=op_call%>">
			...
			<td  class='FIELDS'>
			<input type="text" name="date_of_birth" id="date_of_birth"  maxlength="10" size="8" tabIndex='32' onblur="makeValidString(this);dateCheck(this,'<%=strBodyFrame%>','<%=strSubmitFrame%>')" value = ''><img name="dob" id="dob" src="../../eCommon/images/CommonCalendar.gif" onclick="if (!document.getElementById('date_of_birth').disabled) { document.forms[0].date_of_birth.focus(); return showCalendar('date_of_birth'); }">
			</td>
		</tr>
		<%}%>
		...
		<tr><td colspan='8' class='white'></td></tr>
			...
			<input type='radio' name='citizen_yn' id='citizen_yn' value='Y' tabIndex='33'  onclick='disable_Legal_If_Citzen(this)' checked><fmt:message key="Common.citizen.label" bundle="${common_labels}"/>
			<input type='radio' name='citizen_yn' id='citizen_yn' value='N' onclick='disable_Legal_If_Citzen(this)'><fmt:message key="Common.noncitizen.label" bundle="${common_labels}"/></td><td class=label>&nbsp;</td>

			</table></td>

			<td class='label'  ><fmt:message key="Common.status.label" bundle="${common_labels}"/></td>
			<td  class='FIELDS' >
			<table cellpadding='2' cellspacing=0 border=0 width='100%' tabindex=-1><tr><td class=oaborder nowrap WIDTH='17%'><input type='radio' name='legal_yn' id='legal_yn' value="Y" checked onclick='chkLegal(this)' disabled><fmt:message key="Common.legal.label" bundle="${common_labels}"/><input type='radio' name='legal_yn' id='legal_yn' value="N"  onclick='chkLegal(this)' tabIndex='34' disabled><fmt:message key="Common.illegal.label" bundle="${common_labels}"/></td><td class=label>&nbsp;</td>
			</table></td>
			</tr>
				<tr>
		...
		<td class="label"  width='240pt'><fmt:message key="Common.race.label" bundle="${common_labels}"/></td>
		<td class='FIELDS'>
		<input type='text' name='race_desc' id='race_desc' size='20' maxlength='20' value="<%=checkForNull((String)hash_Map6.get("race_desc"))%>" onblur='if(this.value!=""){searchRace(race_code,race_desc,"<%=locale%>");loadEthnicSubGroup(this);}else{clearfields();}'  tabindex='2' ><input type='button'    name='race_desc_id' id='race_desc_id' value='?' class='button' onclick='searchRace(this,race_desc,"<%=locale%>")' tabindex='2'>
		<input type='hidden' name='race_code' id='race_code' value="<%=default_race_code%>">
...
<input type='hidden' name='isVirtualConsultation' id='isVirtualConsultation' value='<%=isVirtualConsultation%>'>
