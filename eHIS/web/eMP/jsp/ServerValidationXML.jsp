<%@ page contentType="text/html;charset=UTF-8" import="java.sql.Connection, java.sql.PreparedStatement, java.sql.ResultSet, java.util.StringTokenizer, java.util.HashMap,java.util.Hashtable, java.util.ArrayList, java.io.InputStreamReader, java.io.ByteArrayInputStream, java.io.BufferedReader, webbeans.eCommon.ConnectionManager,com.ehis.util.*,eCommon.Common.*,eCommon.XSSRequestWrapper" %>
...
//						out.println ( "var obj = eval(document.all."+nme+");" );
			...
			//Added by kumar on 9/9/2002 to remove other series types if QID no is not null
							...
							if(retVal =='Y'){
								...
								*/
								...
								if(obj.name == 'mar_status_code'){
									if('<%=mar_sts_desc%>'!='') {
										document.getElementById("mar_status_desc").value="<%=mar_sts_desc%>";
									} else {
								...
								if(obj.name == 'relgn_code'){
									if('<%=relgn_desc%>'!='') {
										document.getElementById("relgn_desc").value="<%=relgn_desc%>";
									} else {
								...
								if(obj.name == 'language_code'){
								    if('<%=lang_desc%>'!='') {
										document.getElementById("language_desc").value="<%=lang_desc%>";
									} else {
								...
								if(obj.name == 'pref_facility_id'){
									if('<%=fcy_desc%>'!='') {
										document.getElementById("pref_facility_desc").value="<%=fcy_desc%>";
									} else {
				...
				if ( result.equals("TRUE") ){
					...
					document.getElementById("b_age").value="<%=age_in_years%>";
					document.getElementById("b_months").value="<%=age_in_months%>";
					document.getElementById("b_days").value="<%=age_in_days%>";
					document.getElementById("place_of_birth_code").value="<%=place_of_birth_code%>";
					document.getElementById("sex").value="<%=sex%>";
					document.getElementById("place_of_birth_desc").value="<%=place_of_birth_desc%>";
					<%
		...
		if(!race_code.equals("")){	/*Changed Long description to short description - 4/4/2006 : Vinod*/
			...
			if(rs!=null)
			{
				...
				while (rs.next())
				{
					...
					element.value= "<%=rs.getString("ethnic_group_code")%>";
				//	document.getElementById("ethnic_group").add(element);	

					if("<%=rs.getString("ethnic_group_code")%>" == "<%=a_ethnic_group_code%>")
					{
				//		document.getElementById("ethnic_group").options[<%=i%>].selected = true;
					}
	...
	if ( process == 15 ){
		...
		while(rs != null && rs.next()){
			%>			
				...
				var element = document.createElement('OPTION');
				element.text = "<%=rs.getString("short_desc")%>";
				element.value= "<%=rs.getString("ocpn_code")%>";
	...
	*/
	...
	if(process == 25){
		...
		%>
			...
			document.getElementById("relationship_level").value="<%=relationship_level%>";
		<%
		...
		if(!relationship_level.equals("0")){
			...
			%>
				...
				document.getElementById("relntext").innerHTML="<input type='text' name='relationship_level1_desc' id='relationship_level1_desc' size='25' maxlength='25' value='<%=relationship_level1_description%>' onblur='searchRelationshiplevel1(relationship_level1_code,relationship_level1_desc);clearfields();'   ><input type='button'  id='relationship_level1_id'  name='relationship_level1_code_id' id='relationship_level1_code_id' value='?' class='button' onclick='searchRelationshiplevel1(this,relationship_level1_desc)' ><img src='../../eCommon/images/mandatory.gif'></img><input type='hidden' name='relationship_level1_code' id='relationship_level1_code' value='<%=relation_level1_code%>'>";					
	...
	if(process == 30){
		...
		%>
			if(<%=frame_reference%>.family_org_membership){
				if(<%=frame_reference%>.family_org_membership.value=="4")
					alert(getMessage("NON_ORGMEM_NOT_PAT_SER","MP"));
				else if(<%=frame_reference%>.family_org_membership.value=="3")
					alert(getMessage("NON_FAMILY_NOT_PAT_SER","MP"));

				<%=frame_reference%>.family_org_membership.value="";
				<%=frame_reference%>.alt_id1_no.value="";
				if(<%=frame_reference%>.relationship_to_head_desc){
					<%=frame_reference%>.relationship_to_head_desc.value="";
					<%=frame_reference%>.relationship_to_head.value="";
				}
			}			
		<%
		}	
	...
	while(rs.next()){
    	altCount++;	  
	  	...
	  	document.forms[0].isAlternateIdCount.value="<%=altCount%>" ;
	  <%}
	//End HSA-CRF-0289
	...
	%> 
	if(document.getElementById('nationality_id')!=null){document.getElementById('nationality_id').innerHTML="<%=nat_other_alter_legend%>";}
	if(document.getElementById('oth_alternate_id')!=null){document.getElementById('oth_alternate_id').innerHTML="<%=nat_other_alter_id%>";}
	  <%
	  if(rs!=null) rs.close();
	  if(stmt!=null) stmt.close();
	  }	
