...
function create()
{
	...
	f_query_add_mod.location.href='../../eOT/jsp/SurgeonPrefFrameForCreate.jsp?source='+source;
}
...
function chkForDuplicates()
{
	...
	var surgeon_desc=HdrformObj.surgeon_code.value;
	...
	for (var i=0;i<arr.length;i++ )
	{
		var chkString=arr[i];
		if(appl_desc=="AOPR"){
			/*Added by Suma against 18874 on feb 4 2010*/
			if(chkString.indexOf(surgeon_desc)>=0)
			{
		   ...
		   if(speciality_code==''){
	      	...
	      	if(chkString.indexOf(appl_desc)>=0 && chkString.indexOf(speciality_code)>=0 && chkString.indexOf(surgeon_desc)>=0){
			err_txt=getMessage("CODE_ALREADY_EXISTS","common");
		    }
		   }
		   ...
		   if(speciality_code==''){
			...
			if(chkString.indexOf(appl_desc)>=0 && chkString.indexOf(speciality_code)>=0 && chkString.indexOf(category_code)>=0 && chkString.indexOf(surgeon_desc)>=0){
			err_txt=getMessage("CODE_ALREADY_EXISTS","common");
		    }
		   }
		     ...
		     if(speciality_code==''){
	      	...
	      	if(chkString.indexOf(appl_desc)>=0 && chkString.indexOf(speciality_code)>=0 && chkString.indexOf(category_code)>=0 && chkString.indexOf(surgeon_desc)>=0 && chkString.indexOf(subCategory_code)>=0){
			err_txt=getMessage("CODE_ALREADY_EXISTS","common");
		    }
		   }
		   ...
		   if(operation_code==''){
	      	...
	      	if(chkString.indexOf(appl_desc)>=0 && chkString.indexOf(operation_code)>=0 &&  chkString.indexOf(surgeon_desc)>=0){
			err_txt=getMessage("CODE_ALREADY_EXISTS","common");
		    }
		   }
	   }
...
{  
	...
	var xmlStr="<root>";
	var action = '../../servlet/eOT.SurgeonPreferenceServlet?mode='+mode;
		xmlStr+=hiddenframe.formXMLString();
	if (hiddenframe.formXMLString().length!=0){
	xmlStr+="</root>";
	...
	var msg_split=retVal.split(" ");
	var msg="APP-OT0082";
	if(msg_split[0]==msg){
		alert(getMessage("RECORD_MODIFIED","SM"));
	}else{
		alert(retVal);
	}	window.parent.frames[2].frames[2].location.href="../../eCommon/jsp/MstCodeError.jsp?err_num=";
} else{
	...
	f_query_add_mod.document.location.reload();	//Added for IN:049258
	return false;
}
else
	{
...
function reset() {
if(window.parent.parent.frames[2].frames[1].document.location.href.indexOf("SurgeonPrefFrameForCreate.jsp") != -1){
     if(f_query_add_mod.document.location.href.indexOf("SurgeonPrefMain.jsp") != -1)
	 return false;
	 else
	 f_query_add_mod.document.location.reload();
}else if(window.parent.parent.frames[2].frames[1].document.location.href.indexOf("SurgeonPreferencesQueryCriteria.jsp") != -1){
...
function query()
{
	...
	var source=document.forms[0].source.value;
	f_query_add_mod.location.href='../../eOT/jsp/SurgeonPreferencesQueryCriteria.jsp?source='+source;
}
