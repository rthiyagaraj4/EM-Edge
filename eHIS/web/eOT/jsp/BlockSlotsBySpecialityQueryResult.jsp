...
//String strsql1="SELECT  OPER_ROOM_CODE,SHORT_DESC, TO_CHAR(WD_START_TIME,'HH24:MI'), TO_CHAR(WD_END_TIME,'HH24:MI'), TO_CHAR(HD_START_TIME,'HH24:MI'), TO_CHAR(HD_END_TIME,'HH24:MI') FROM OT_OPER_ROOM WHERE NVL(STATUS,'E') = 'E' AND NVL(RESTRICT_BY_SPLTY_YN,'N')='Y' "+sql;
		...
		String strsql="SELECT  OPER_ROOM_CODE,SHORT_DESC, TO_CHAR(WD_START_TIME,'HH24:MI'), TO_CHAR(WD_END_TIME,'HH24:MI'), TO_CHAR(HD_START_TIME,'HH24:MI'), TO_CHAR(HD_END_TIME,'HH24:MI') FROM OT_OPER_ROOM_LANG_VW WHERE OPERATING_FACILITY_ID = ? AND LANGUAGE_ID='"+locale+"' AND NVL(STATUS,'E') = 'E' AND NVL(RESTRICT_BY_SPLTY_YN,'N')='Y' "+sql;
		String strsql1="SELECT  OPER_ROOM_CODE,SHORT_DESC, TO_CHAR(WD_START_TIME,'HH24:MI'), TO_CHAR(WD_END_TIME,'HH24:MI'), TO_CHAR(HD_START_TIME,'HH24:MI'), TO_CHAR(HD_END_TIME,'HH24:MI') FROM OT_OPER_ROOM_LANG_VW WHERE OPERATING_FACILITY_ID = ? AND LANGUAGE_ID='"+locale+"' AND NVL(STATUS,'E') = 'E' AND NVL(RESTRICT_BY_SPLTY_YN,'N')='Y' "+sql;
...
<%
if ( !(start <= 1) )
    out.println("<A HREF='../../eOT/jsp/BlockSlotsBySpecialityQueryResult.jsp?from="+(start-14)+"&to="+(end-14)+"&whereclause=" + URLEncoder.encode(sql) + "'" + " text-decoration='none'>Prev</A>");

if ( !( (start+14) > maxRecord ) )
    out.println("<A HREF='../../eOT/jsp/BlockSlotsBySpecialityQueryResult.jsp?from="+(start+14)+"&to="+(end+14)+"&whereclause="+URLEncoder.encode(sql)+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
%>
</td>
</tr>
</table>
