		...
		<form name="formCancelRequestDetail" id="formCancelRequestDetail" >
			<table border="1" cellpadding="0" cellspacing="0" width='80%' align='center'>
				<tr>
					...
					<th><fmt:message key="eSS.RequestedQuantity.label" bundle="${ss_labels}"/></th>
...
%>
	<tr>
		...
		<td class="<%=className%>"><%=(String)hmRecord.get("request_qty")%></td>
		<td class="<%=className%>"><%=bean.checkForNull(issue_qty)%></td>
		<td class="<%=className%>" > <input type="checkbox" name="group_code_<%=i%>" id="group_code_<%=i%>"  value="<%=(String)hmRecord.get("req_group_code")%>-<%=(String)hmRecord.get("request_qty")%>" onclick="updateSelection();" <%=checked%> <%=disabled%>></td>	
	</tr>
<%
}
%>
