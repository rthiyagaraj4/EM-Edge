		...
		<form name="formTrayDiscrepancyTrayQueryCriteria" id="formTrayDiscrepancyTrayQueryCriteria" action="../../eSS/jsp/TrayDiscrepancyTrayQueryResult.jsp" method="post" target="frameTrayDiscrepancyTrayQueryResult" onreset="FocusFirstElement();">
			<table border="0" cellpadding="1" cellspacing="0" width='100%' height='100%' align='center'>
				<tr>
					...
					<td class="label">&nbsp;
					<input name="group_desc" id="group_desc" maxLength='15' size='15' readOnly value="<%=group_desc%>" <%=disabled%>>
					<input type="hidden" value="<%=group_code%>" name="group_code">
					<input type="button" class="BUTTON" value='?' onClick="searchGroupCode(group_code, group_desc);" <%=disabled%>>	
					</td>
					<td  class="label"><fmt:message key="eSS.TrayNo.label" bundle="${ss_labels}"/></td>
					<td class="label">&nbsp;
					<input type='text' name="tray_no" id="tray_no" value="<%=bean.checkForNull(tray_no)%>" size='<%=bean.getSsRepositoryValue("INTEGER_PRECISION")%>' maxLength='<%=bean.getSsRepositoryValue("INTEGER_PRECISION")%>' class="NUMBER" onKeyPress="return isValidInteger();">
					</td>
					<td class="label"><fmt:message key="eSS.OwnerStore.label" bundle="${ss_labels}"/></td>
					<td class="label">&nbsp;
					<%-- 
					<select name="store_code<%=(store_code==null)?" id="store_code<%=(store_code==null)?"":"_disabled"%>" <%=(store_code==null)?"":"disabled"%>>
					<%=bean.getStore_code_List()%></select>
					<%=(store_code==null)?"":"<input type='hidden' name='store_code' id='store_code' value='"+store_code+"'>"%> --%> 
					 <%	String storeCodeId = "store_code"
					 			+ (store_code == null ? "_disabled" : "");
					 	String disabledAttribute = (store_code == null) ? "" : "disabled";
					 %> <select name="store_code" id="<%=storeCodeId%>"
										<%=disabledAttribute%>>
