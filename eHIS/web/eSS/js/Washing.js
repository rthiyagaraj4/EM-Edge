...
function apply() {
	...
	var count=0;
	if(formObjDtlData!=null){
	if((formObjDtlData.mode.value==MODE_MODIFY)&&(parseInt(formObjDtlData.totalRecords.value)>=1))
		{

			for(var i=0;i<parseInt(formObjDtlData.totalRecords.value);i++)
			{
			  //Added by Rabbani on 10/12/2012
		       if (eval("formObjDtlData.checkbox" + i) != null && eval("formObjDtlData.checkbox" + i + ".checked")) {
	...
	if(blankObject==null) {
		if(formObjDtlData==undefined){message=getMessage("ONE_RECORD_NEEDED","SS");
    ...
    var countCheck =0;
		if(formObjDtl!=-1){
			...
			//else if((formObjDtlData.mode.value==MODE_MODIFY)&&(count>0)&&(formObj.entry_completed_yn.value=="Y")){
				else if((formObjDtlData.mode.value==MODE_MODIFY)&&(formObj.entry_completed_yn.value=="Y")){
				...
				else if((formObjDtlData.mode.value==MODE_MODIFY)&&(formObj.entry_completed_yn.value="N"))
					{
...
function goNextOne(source_mode) {	
	formObj=parent.frameWashingDetail.document.formWashingDetail;
	...
	if(source_mode=='select'){
		...
		var formObjWashing		=	parent.frameWashingAddModify.document.forms[0] ;
		parent.frameWashingDetail.location.href="../../eSS/jsp/WashingResult.jsp?autoclave_wash_unit_code="+formObjWashing.autoclave_wash_unit_code.value+"&mode="+MODE_MODIFY+"&load_no="+formObjWashing.load_no.value+"&washing_type="+formObjWashing.washing_type.value+"&from="+start+"&to="+end;
	}
}	

function goPrevOne(source_mode) {
...
function checkSelect(start,end,maxRec){
	var checkFlag="";
	if(maxRec>0){
	//for(var ap=start;(ap<end && ap<maxRec);ap++){ //commented by Rabbani INC NO:32068 on 13/12/2012
	 for(var ap=parseInt(start);(ap<parseInt(end) && ap<parseInt(maxRec));ap++){ //Added by Rabbani INC NO:32068 on 13/12/2012
	 if((eval("document.formWashingDetail.checkbox"+ap).value)!='Y'){
			checkFlag='N';
		  }
	 }
		if(checkFlag!='N'){
