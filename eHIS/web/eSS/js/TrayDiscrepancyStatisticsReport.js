...
function run()
{
	formObj= parent.f_query_add_mod.document.formDispatch;
		var messageFrame	=	parent.messageFrame;	
			...
			if( isValidFromToField(formObj.p_fm_doc_date1,formObj.p_to_doc_date1,DATE,"Date" , messageFrame )  )
			if( isValidFromToField(formObj.p_fm_store_code,formObj.p_to_store_code,STRING,"Store" ,  messageFrame )  )
			if( isValidFromToField(formObj.p_fm_tray_no,formObj.p_to_tray_no,LONG,"Tray Number" , messageFrame )  ){
...
async function searchCode( objCode,label ) {
	...
	if(objCode.name=="p_fm_group_code" || objCode.name=="p_to_group_code" ){
		argumentArray[0] =document.formDispatch.SQL_SS_GROUP_LOOKUP.value;
	dataNameArray  [0]="LANGUAGE_ID"
	dataValueArray [0]=document.formDispatch.locale.value;
	dataTypeArray  [0]=STRING;
