...
function create(){
	f_query_add_mod.location.href = "../../eSS/jsp/LoanRequestGroupFrame.jsp?mode="+MODE_INSERT+"&menu_id="+menu_id+"&module_id="+module_id+"&function_id="+function_id ;
}

function query(){
	f_query_add_mod.location.href = "../../eSS/jsp/LoanRequestGroupQueryFrame.jsp?function_id="+function_id ;
}
...
function onSuccess(){
	...
	f_query_add_mod.frameLoanRequestGroupHeader.document.location.href="../../eCommon/html/blank.html";
	//f_query_add_mod.frameLoanRequestGroupDetail.document.location.href="../../eCommon/html/blank.html";//Commented by sakti against BRU-HIMS-CRF-373
	f_query_add_mod.frameLoanRequestGroupList.document.location.href="../../eCommon/html/blank.html";
	create();
	}else if(module_id=="OT"){
		if(!f_query_add_mod.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.doc_no.disabled){
			var doc_no = f_query_add_mod.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.doc_no.value;
...
function reset() {
	if(f_query_add_mod.frameLoanRequestGroupQueryCriteria!=null){
		f_query_add_mod.frameLoanRequestGroupQueryCriteria.document.formLoanRequestGroupQueryCriteria.reset();
		f_query_add_mod.frameLoanRequestGroupQueryResult.document.location.href="../../eCommon/html/blank.html";
	}
	else if(f_query_add_mod.frameLoanRequestGroupHeader!=null){
...
async function deleterecord(){
	if( f_query_add_mod.frameLoanRequestGroupList==undefined){
		messageFrame.location.href ="../../eCommon/jsp/error.jsp?err_num=";
		return;
	}
	var formObj	=	f_query_add_mod.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader;
...
function apply(){
	...
	var formObject		=	f_query_add_mod.frameLoanRequestGroupList.document.formLoanRequestGroupList ;//Added by sakti against BRU-HIMS-CRF-373
	...
	if(blankObject==null) {
		...
		bean_id = f_query_add_mod.frameLoanRequestGroupList.document.formLoanRequestGroupList.bean_id.value;
		bean_name =f_query_add_mod.frameLoanRequestGroupList.document.formLoanRequestGroupList.bean_name.value;
		var index = f_query_add_mod.frameLoanRequestGroupList.document.formLoanRequestGroupList.total_checkboxes.value;
		xmlDom = new ActiveXObject( "Microsoft.XMLDom" );
		xmlHttp = new ActiveXObject( "Microsoft.XMLHTTP" );
		xmlStr = getEnhancedXMLString (formObject);
		xmlDom.loadXML( xmlStr );
		xmlHttp.open("POST", "LoanRequestGroupValidate.jsp?prev_index=" + index  +"&bean_id="+bean_id+"&bean_name="+bean_name+ "&validate=DTL_FINAL_DATA_STORE", false);
		xmlHttp.send( xmlDom );
		...
		//if (formObj.SS_REQUEST_GROUP_AUTHORIZE!=null) { Commented by Ganga  since we have authorize in function level ML-BRU-SCF-0813 [IN:039420]
				...
				var confirmation = confirmAuthorize();//window.confirm("Current record will be deleted!!");
...
function checkDoc_type_code(){
	if(document.formLoanRequestGroupHeader.doc_type_code.value=="") return;
	...
	xmlHttp.open("POST", "../../eSS/jsp/LoanRequestGroupValidate.jsp?validate=DOC_TYPE_CODE&doc_type_code=" + document.formLoanRequestGroupHeader.doc_type_code.value, false);
	//xmlHttp.send(xmlDoc);
...
function clearList(){
	...
	var xmlDoc="";
	xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../eSS/jsp/LoanRequestGroupValidate.jsp?validate=CLEAR_LIST", false);
	xmlHttp.send(xmlDoc);
	responseText=xmlHttp.responseText;
	refreshList();
}
...
function enableDocNo(bValue){
	if(!bValue){
		parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.doc_no.value="";	
	}
	parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.doc_no.disabled=!bValue;
}

function addToList(){
	var formObj=  document.formLoanRequestGroupDetail;
	var errorPage	 = "../../eCommon/jsp/error.jsp";
	if((!formObj.description.readOnly)&&(!formObj.description.disabled)&&(formObj.description.value!="")){
		var xmlHttp = new XMLHttpRequest();
		var xmlDoc="";
		xmlDoc.loadXML(getXMLString(null));
		xmlHttp.open("POST", "../../eSS/jsp/LoanRequestGroupValidate.jsp?req_group_code=" + formObj.description.value + "&validate=GROUP_DESC", false);
		xmlHttp.send(xmlDoc);
		responseText=xmlHttp.responseText;
		eval(responseText);
		if(! result ) {
	...
	if(obj==null) {
		...
		var xmlDoc="";
		xmlDoc.loadXML(getXMLString(null));
		//xmlHttp.open("POST", "../../eSS/jsp/LoanRequestGroupValidate.jsp?req_group_code=" + formObj.req_group_code.value +"&auth_group_code=" + formObj.auth_group_code.value + "&request_qty=" + formObj.request_qty.value +"&group_type=" + formObj.group_type.value +"&index="+formObj.index.value+ "&description="+formObj.description.value+ "&auth_group_desc="+auth_group_desc+ "&issue_store_code="+parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.req_on_store_code.value+ "&validate=ADD_TO_LIST", false);
		xmlHttp.open("POST", "../../eSS/jsp/LoanRequestGroupValidate.jsp?surg_name=" + encodeURIComponent(formObj.ot_surg_description.value,"UTF-8") +"&surg_description=" + encodeURIComponent(formObj.surg_description.value,"UTF-8") +"&req_group_code=" + formObj.req_group_code.value +"&auth_group_code=" + formObj.auth_group_code.value + "&request_qty=" + formObj.request_qty.value +"&group_type=" + formObj.group_type.value +"&index="+formObj.index.value+ "&description="+formObj.description.value+ "&auth_group_desc="+auth_group_desc+ "&issue_store_code="+parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.req_on_store_code.value+ "&validate=ADD_TO_LIST", false);
		xmlHttp.send(xmlDoc);
		responseText=xmlHttp.responseText;
		eval(responseText);
		if( result ) {
...
/*function searchGroupCode( objCode, objDesc ) {
	...
	var formObj		=	parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader ;
	...
	dataValueArray[0]= document.formLoanRequestGroupDetail.group_type.value;
	dataTypeArray[0] = STRING;
	
	dataNameArray[1] = "language_id";
	dataValueArray[1]= document.formLoanRequestGroupDetail.locale.value;
	dataTypeArray[1] = STRING;

	dataNameArray[2] = "store_code";
	dataValueArray[2]= parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.req_on_store_code.value;
	dataTypeArray[2] = STRING;
	
	
	argumentArray[0] = document.formLoanRequestGroupDetail.SQL_SS_GROUP_LOOKUP_BASED_ON_TYPE_FOR_REQUEST.value;
	...
	if((returnedValues != null) && (returnedValues != "") )  {
		...
		var xmlDoc="";
		xmlDoc.loadXML(getXMLString(null));
		xmlHttp.open("POST", "../../eSS/jsp/LoanRequestGroupValidate.jsp?req_group_code=" + formObj.req_group_code.value+ "&req_by_store="+parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.req_by_store_code.value+ "&validate=CHECK_REQ_STORE", false);
		xmlHttp.send(xmlDoc);
		responseText=xmlHttp.responseText;
		eval(responseText);
		if(!result){
		...
		var xmlDoc="";
		xmlDoc.loadXML(getXMLString(null));
		xmlHttp.open("POST", "../../eSS/jsp/LoanRequestGroupValidate.jsp?req_group_code=" + formObj.req_group_code.value + "&validate=POPULATE_AUTH_GROUP_CODE", false);
		xmlHttp.send(xmlDoc);
		responseText=xmlHttp.responseText;
		eval(responseText);
...
/*function searchGroupCode( objCode, objDesc ) {
	...
	dataValueArray[3]= document.formLoanRequestGroupDetail.surgery_type.value;
...
/*function searchGroupCode( objCode, objDesc ) {
	...
	var fieldNames	=	new Array();
	var index=0;
	var surgery_type = document.formLoanRequestGroupDetail.surgery_type.value;
...
//Added ends
	...
	if(surgery_type == ""){
		...
		argumentArray[0] = document.formLoanRequestGroupDetail.SQL_SS_GROUP_LOOKUP_BASED_ON_TYPE_SURGERY_TYPE_FOR_REQUEST.value;
	}
	argumentArray[1] = dataNameArray;
	argumentArray[2] = dataValueArray;
	argumentArray[3] = dataTypeArray;
...
function searchGroupCode( objCode, objDesc ) {
	...
	var formObj		=	document.formLoanRequestGroupHeader ;
	
	var blankObject	=	null;
	var fields		=	new Array();
	var fieldNames	=	new Array();
	var index=0;
	var surgery_type = document.formLoanRequestGroupHeader.surgery_type.value;
	...
	dataNameArray[0] = "group_type";
	dataValueArray[0]= document.formLoanRequestGroupHeader.group_type.value;
	dataTypeArray[0] = STRING;
	
	dataNameArray[1] = "language_id";
	dataValueArray[1]= document.formLoanRequestGroupHeader.locale.value;
	dataTypeArray[1] = STRING;

	dataNameArray[2] = "store_code";
	dataValueArray[2]= document.formLoanRequestGroupHeader.req_on_store_code.value;
	dataTypeArray[2] = STRING;
	//Added By Sakti against KDAH-CRF-0247
	if(surgery_type != ""){
	dataNameArray[3] = "surgery_type";
...
//Added ends
	...
	if(surgery_type == ""){
		argumentArray[0] = document.formLoanRequestGroupHeader.SQL_SS_GROUP_LOOKUP_BASED_ON_TYPE_FOR_REQUEST.value;
	}else{
		argumentArray[0] = document.formLoanRequestGroupHeader.SQL_SS_GROUP_LOOKUP_BASED_ON_TYPE_SURGERY_TYPE_FOR_REQUEST.value;
	}
	argumentArray[1] = dataNameArray;
	argumentArray[2] = dataValueArray;
	argumentArray[3] = dataTypeArray;
	...
	if((returnedValues != null) && (returnedValues != "") )  {
		...
		var xmlDoc="";
		xmlDoc.loadXML(getXMLString(null));
		xmlHttp.open("POST", "../../eSS/jsp/LoanRequestGroupValidate.jsp?req_group_code=" + formObj.req_group_code.value+ "&req_by_store="+document.formLoanRequestGroupHeader.req_by_store_code.value+ "&validate=CHECK_REQ_STORE", false);
		xmlHttp.send(xmlDoc);
		responseText=xmlHttp.responseText;
		eval(responseText);
		if(!result){
...
function clearGroupDetail(){
	var formObj = "";
	if(parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.module_id != undefined && parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.module_id.value == "OT")
		formObj = document.formLoanRequestGroupDetail;
	else
		formObj = top.content.f_query_add_mod.frameLoanRequestGroupDetail.document.formLoanRequestGroupDetail;
...
function resetGroupDetail(){
	...
	clearListItems("parent.frameLoanRequestGroupDetail.document.formLoanRequestGroupDetail.auth_group_code");
}
...
function refreshList(){
	if(parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.module_id != undefined && parent.frameLoanRequestGroupHeader.document.formLoanRequestGroupHeader.module_id.value == "OT"){
		parent.frameLoanRequestGroupList.location.href="../../eSS/jsp/LoanRequestGroupList.jsp";
	}
	else	
		top.content.f_query_add_mod.frameLoanRequestGroupList.document.location.reload();
}

function listModify(index){
	parent.frameLoanRequestGroupDetail.location.href="../../eSS/jsp/LoanRequestGroupDetail.jsp?index="+index;
}
...
function Modify(obj){
	...
	top.content.f_query_add_mod.document.location.href="../../eSS/jsp/LoanRequestGroupFrame.jsp?mode="+MODE_MODIFY+"&function_id="+parent.parent.function_id+"&doc_type_code="+doc_type_code+"&menu_id="+parent.parent.menu_id+"&module_id="+parent.parent.module_id+"&doc_no="+doc_no+"&req_on_store_code="+req_on_store_code+"&req_by_store_desc="+req_by_store_code;
}
...
function Setsurgdesc(surg_description,surgery_type){
	document.formLoanRequestGroupDetail.surg_description.value = decodeURIComponent(surg_description.value,"UTF-8");
	document.formLoanRequestGroupDetail.surg_description.value = document.formLoanRequestGroupDetail.surg_description.value.replace(/\+/g,' ');
	document.formLoanRequestGroupDetail.ot_surg_description.value = decodeURIComponent(surgery_type.value,"UTF-8");
	document.formLoanRequestGroupDetail.ot_surg_description.value = document.formLoanRequestGroupDetail.ot_surg_description.value.replace(/\+/g,' ');
}
...
async function callGroupSearch() {
	var formObject 								= parent.frameLoanRequestGroupList.document.formLoanRequestGroupList;
	...
	if (blankObject == true) {
		...
		if(req_on_store_code==req_by_store_code){
			alert(getMessage("REQUEST_BOTH_STORE_SAME","SS"));
			return false;
		}
		
		var listbox 							= document.formLoanRequestGroupHeader.req_by_store_code;
...
//		xmlHttp = new ActiveXObject( "Microsoft.XMLHTTP" );
		...
		//xmlDom.loadXML( xmlStr );
		xmlHttp.open("POST", "LoanRequestGroupValidate.jsp?bean_id="+formObject.bean_id.value+"&bean_name="+formObject.bean_name.value+ "&validate=DTL_DATA_STORE", false);
//		xmlHttp.send( xmlDom );
		xmlHttp.setRequestHeader("Content-Type", "text/xml");
	    xmlHttp.send(new XMLSerializer().serializeToString(xmlDoc));	
		responseText=xmlHttp.responseText;
		...
		retval =await window.showModalDialog("../../eSS/jsp/LoanRequestGroupSearchFrame.jsp?req_on_store_code=" + req_on_store_code+"&mode="+formObj.mode.value+"&req_by_store_code="+req_by_store_code ,"", "dialogHeight:60vh; dialogWidth:60vw; center:0; status: no; dialogLeft :200; dialogTop :100;");
		...
		if (retval != null && retval!=undefined) {
			...
			parent.frameLoanRequestGroupList.location.href="../../eSS/jsp/LoanRequestGroupLists.jsp?mode="+formObj.mode.value+"&req_on_store_code="+formObj.req_on_store_code.value+"&req_by_store_code="+formObj.req_by_store_code.value+"&req_by_store_desc="+selText;
		}
		//else part added  for clearing checked values oncancel and clicking on cross or close
			...
			xmlHttp.open("POST", "LoanRequestGroupValidate.jsp?bean_id="+formObject.bean_id.value+"&bean_name="+formObject.bean_name.value+"&validate=CLEAR_DATA", false);
			xmlHttp.send( xmlDom );
			responseText=xmlHttp.responseText;
			eval( responseText );
...
function closeActionPerformed(frmObj) {
	...
	var seach_type = "";
	var exist = false;
	var formObj = parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm;
//	var formObj = parent.frameLoanRequestGroupSearchResult.document.getElementById('GroupsearchForm');
	...
	for (var count = 0;count<total_trays;count++) {
			if(eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.group_code"+count+".checked")) {
				exist = true;
				returnvals += formObj.elements["group" + count].value + "`" + formObj.elements["desc" + count].value +  "~";
				
				arrayObject[index] = eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.group"+count);
				arrayObject[index+1] = eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.desc"+count);
				
				arrayObject[index+2] = eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.group_type"+count);
				arrayObject[index+3] = eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.auth_group"+count);
				arrayObject[index+4] = eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.surgery_type"+count);
				arrayObject[index+5] = eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.surgery_name"+count);
				arrayObject[index+6] = eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.qty"+count);
				arrayObject[index+7] = eval("parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.avlbl_ster_qty"+count);
				index += 8;
	...
	parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.index.value = new_index;
	parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.mode.value = document.GroupSearchCriteria_Form.mode.value;
	parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.unchecked_index.value = unchecked_index;
	
	
	arrayObject[arrayObject.length] = parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.unchecked_index;
	arrayObject[arrayObject.length] = parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.index;
	arrayObject[arrayObject.length] = parent.frameLoanRequestGroupSearchResult.document.GroupsearchForm.mode;
	...
	xmlHttp.open("POST", "LoanRequestGroupValidate.jsp?validate=ADD_GROUPS_TO_LIST"+"&search_type=search", false);
	xmlHttp.send( xmlDom );
	responseText=xmlHttp.responseText;
	eval( responseText );
...
function SubmitLink(from, to) {
	...
	xmlDom.loadXML( xmlStr );
	xmlHttp.open("POST", "LoanRequestGroupValidate.jsp?validate=ADD_GROUPS_TO_LIST"+"&search_type=next_prev", false);
	xmlHttp.send( xmlDom );
	responseText=xmlHttp.responseText;
	eval( responseText );
	}
...
function updateSelection() {
	var frmObject		= document.formLoanRequestGroupList;
	...
	var exist=true;
		for (var count = 0;count<total_check_boxes;count++) {
			...
			if(exist){
			document.formLoanRequestGroupList.selectAll.checked=true;
			}else{
			document.formLoanRequestGroupList.selectAll.checked=false;
			}
}
...
function toggleCheckforDetail(selAllObj){
	...
	var formObj = document.formLoanRequestGroupList;
...
function checkstrqty(i,pgroup_code,req_by_store_code,req_on_store_code) {
	...
	xmlHttp.open("POST", "LoanRequestGroupValidate.jsp?group_code="+group_code+"&req_by_store_code="+req_by_store_code+"&index="+i+"&language_id="+language_id+"&req_on_store_code="+req_on_store_code+"&validate=CHECK_STER_QTY", false);
	xmlHttp.send( xmlDom );
	responseText=xmlHttp.responseText;
	eval( responseText );
}
