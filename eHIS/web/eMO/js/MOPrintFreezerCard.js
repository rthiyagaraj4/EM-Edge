function reset() 
{
	if(f_query_add_mod.document.repFreezerCard)
	{
	   f_query_add_mod.location.href = '../../eMO/jsp/MOrepFreezerCard.jsp?param=';
	}
}
/*function run() 
{ 	
...
if (f_query_add_mod.CheckString(getLabel('Common.registrationno.label','Common'),f_query_add_mod.document.forms[0].P_FM_REGISTRATION_NO,f_query_add_mod.document.forms[0].P_TO_REGISTRATION_NO,messageFrame))
	...
	if(f_query_add_mod.document.forms[0].patient_id.value=='' && f_query_add_mod.document.forms[0].valid.value=='Y')
		{
			var errors='APP-000001 Registration Number Cannot be Blank'; 
			messageFrame.document.location.href="../../eCommon/jsp/error.jsp?err_num="+errors ;
		}
	...
	if (valid=='Y')
	{
		...
		else if(f_query_add_mod.document.forms[0].adesc.value=='')
			{
				alert('APP-MO0010 Location is not yet assigned');
			}
			else
			{*/
...
/*function calljsp1(obj)
{
	...
	if(az!='')
	{
		f_query_add_mod.location.href = '../../eMO/jsp/MOrepFreezerCard.jsp?param='+az+'&first=N';
	}
}*/
...
async function searchCode(obj,target)
{
	...
	if(obj.name=="patient_search"||obj.name=="patient_search1")
	{
		...
		sql="select REGISTRATION_NO code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and area_code is not null and upper(REGISTRATION_NO) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}
	...
	
	{
        ...
        fieldLegend["DESC_LEGEND"] = getLabel("Common.PatientName.label","Common");
		sql="select PATIENT_ID code , decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and area_code is not null and upper(PATIENT_ID) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}
	argumentArray[0] =sql;
	...
	if(retVal != null && retVal != "" )
	{
		...
		var arr=ret1.split(",");
			...
			target.value= arr[0];
			//parent.f_query_add_mod.location.href = '../../eMO/jsp/MOrepBodyTag.jsp?param='+arr[0];
	}
...
/*function calljsp(obj)
{
	...
	if(az!='')
	{
		parent.f_query_add_mod.location.href = '../../eMO/jsp/MOrepFreezerCard.jsp?param='+az+'&first=N';
	}
}*/

/*function callfun()
{
	var error=getMessage('AREA_IS_NOT_ASSIGNED');
	alert(error);
