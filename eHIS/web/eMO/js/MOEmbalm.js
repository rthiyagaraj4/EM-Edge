...
/*	if(eval(endnum)<=startnum)		
	...
	if(eval(nextnum) >= eval(endnum))
	{
		...
		error = error.replace('$',getLabel("eMO.EndEmbalmNo.label","MO"));
		error = error.replace('#',getLabel("eMO.NextEmbalmNo.label","MO"));
		alert(error);
...
function apply()
{
  var fields = new Array ( f_query_add_mod.document.PostmortemForm.start_num,
	...
	var names = new Array ( getLabel('eMO.StartingEmbalmNo.label','MO'),
						getLabel('eMO.NextEmbalmNo.label','MO'),
						getLabel('eMO.EndEmbalmNo.label','MO')
    				      );

	
	var splField = new Array (f_query_add_mod.document.PostmortemForm.start_num);
	var splFieldName = new Array (getLabel('eMO.StartingEmbalmNo.label','MO'));
