...
document.forms[0].NAT_ID_NO.readOnly=false;
document.forms[0].EMAIL_ID.readOnly=false
document.forms[0].EMAIL_ID.disabled=false; //Modified by Ashwini on 11-Oct-2018 for ML-MMOH-CRF-1095
...
document.forms[0].copydet.disabled=true; //Added by Ashwini on 11-Oct-2018 for ML-MMOH-CRF-1095
...
// Below Condition Modified Aginst PAS-INC-MO -> Release Deceased/Body Part [IN:043269] By Saanthaakumar
	...
	var SiteApplicaple = document.forms[0].SiteApplicaple.value; // Added by Shagar ML-MMOH-CRF-0690
	var i = 0;
	
	fields[i++] = document.forms[0].patient_id;
	
	if(SiteApplicaple=="false")
		fields[i++] = document.forms[0].bodrelby;
	fields[i++] = document.forms[0].bodreldate;
	if(document.forms[0].bodrelby.disabled)
	 document.forms[0].bodrelby.disabled=false;

 if(document.forms[0].burial_issued.checked)
	{
	fields[i++] = document.forms[0].burial_dtime;
	if(SiteApplicaple=="true")	
		var names = new Array (getLabel("Common.patientId.label","Common"),getLabel("Common.BodyReleaseDateTime.label","common"),getLabel("eMO.BurialPermitIssueDateTime.label","mo"));
	else
	...
	{
	if(SiteApplicaple=="true")	
		var names = new Array (getLabel("Common.patientId.label","Common"),getLabel("Common.BodyReleaseDateTime.label","common"));
	else
	...
	{
	  fields[i++] = document.forms[0].CLAIMANT_TYPE_CODE;
	  names.push(getLabel("eMO.Claimant.label","mo"));
	}//else{alert(1); document.forms[0].submit();}
	
// 43269 Over Here.	
	//Added by Santhosh for ML-MMOH-CRF-1948
	if(document.forms[0].datetime_claim_mand.value != null && document.forms[0].datetime_claim_mand.value =="R" && (trimString(document.forms[0].claimdatetime.value) == null || trimString(document.forms[0].claimdatetime.value)== "")){
		fields[i++] = document.forms[0].claimdatetime;
		names.push(getLabel("eMO.DateTimeOfClaiming.label","mo"));
	}
	//END
