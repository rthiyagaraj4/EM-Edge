...
text-align: left;
	...
	<script>
		...
		{	
			...
			if("<%=call_function%>" == "TransferPatientIn"){
				dialogHeight	= "700px";
			}
			if("<%=call_function%>" == "TransferPatient" || "<%=call_function%>" == "RequestforTransfer" || "<%=call_function%>" == 'EMERGENCY_TRANSFER')
			{
				var call_function="<%=call_function%>"
				if(tfr_req_status != "")
				{
		...
		//10/16/2008 5089_MF-CRF-0017 enhancement start
				...
				{
					if("<%=call_function%>" == "TransferPatient"|| "<%=call_function%>" == "RequestforTransfer")
					{
						if(bed_no=="")
						{
	...
	//10/16/2008 5089_MF-CRF-0017 enhancement start
			if("<%=call_function%>" =='TransferPatientOut'||"<%=call_function%>" =='PrepareDischargeAdvice' || "<%=call_function%>" == 'EMERGENCY_TRANSFER')
			{ 
				// Added for the CRF -KDAH-CRF-0104  by Dharma Start
				if("<%=call_function%>" =='PrepareDischargeAdvice'){
	...
	//10/16/2008 5089_MF-CRF-0017 enhancement end
			...
			{
				var temp		= '<%=targetURL%>'+"&Patient_ID="+patid+"&nursing_unit_code="+IPval+"&bed_no="+bed_no+"&<%=paramsValue%>" 
				document.forms[0].targetURL.value = document.forms[0].targetURL.value+ "&Patient_ID="+patid+"&nursing_unit_code="+IPval+"&bed_no="+bed_no+"&<%=paramsValue%>" ;
				document.forms[0].jsp_name.value = '<%=targetURL%>';
			...
			{	
				...
				if(finalize_yn == "Y") {
					...
					HTMLVal = "<HTML><BODY onKeyDown='lockKey()'><form name='form1' id='form1' method='post' action='../../eIP/jsp/FindCancelDischargeAdvice.jsp?IP_val="+IPval+"&enable_height_bmi_pat_cls="+enable_height_bmi_pat_cls+"&encounter_Id="+enc+"&patient_Id="+patid+"&Practitioner_Id="+practid+"&callfrom=IP&call_function=CancelDischargeAdviceMain.jsp&jsp_name="+chk_jsp+"&Sex="+sex+"&p_service_type="+"<%=p_service_type%>"+"&mode=modal&function_id=<%=function_id%>'></form></BODY></HTML>";
					parent.messageFrame.document.body.insertAdjacentHTML('afterbegin',HTMLVal);
					parent.messageFrame.document.form1.submit();			
				 }
			}
			else if("<%=call_function%>" == "CANCEL_DISCHARGE_PATIENT")
			{
				dialogHeight	= "56";
				dialogTop		= "10";
				var HTMLVal		= new String();
				//Added enable_height_bmi_pat_cls for ML-MMOH-SCF-1643 by Kamatchi S
				HTMLVal			= "<HTML><BODY onKeyDown='lockKey()'><form name='form1' id='form1' method='post' action='../../eIP/jsp/FindPatientDischarge.jsp?IP_val="+IPval+"&encounter_Id="+enc+"&enable_height_bmi_pat_cls="+enable_height_bmi_pat_cls+"&patient_Id="+patid+"&Practitioner_Id="+practid+"&callfrom=IP&call_function=CANCEL_DISCHARGE_PATIENT&jsp_name="+chk_jsp+"&Sex="+sex+"&p_service_type="+"<%=p_service_type%>"+"&bl_install_yn="+instl_yn+"&mode=modal&function_id=<%=function_id%>&mo_install_yn=<%=mo_install_yn%>'></form></BODY></HTML>";
				parent.messageFrame.document.body.insertAdjacentHTML('afterbegin',HTMLVal);
				parent.messageFrame.document.form1.submit();
			}
			...
			{				
				...
				if(proc_flg=="Y"){
					HTMLVal = "<HTML><BODY onKeyDown='lockKey()'><form name='form1' id='form1' method='post' action='../../eIP/jsp/FindPrepareDischarge.jsp?weight_on_admn_mandatory="+weight_on_admn_mandatory+"&height_on_admn_mandatory="+height_on_admn_mandatory+"&enable_height_bmi_pat_cls="+enable_height_bmi_pat_cls+"&IP_val="+IPval+"&encounter_Id="+enc+"&patient_Id="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function=DischargeAdviceMain.jsp&jsp_name="+chk_jsp+"&Sex="+sex+"&p_service_type="+"<%=p_service_type%>"+"&patient_class="+patientclass+"&leave_exp_dt_flag="+leave_exp_dt_flag+"&mode=modal&setup_bl_dtls_in_ip_yn=<%=setup_bl_dtls_in_ip_yn%>&allow_discharge_date=<%=allow_discharge_date%>&max_disch_period_for_dc=<%=max_disch_period_for_dc%>&function_id=<%=function_id%>&bl_interfaced_yn=<%=bl_interfaced_yn%>&allow_discharge_date_predc=<%=allow_discharge_date_predc%>&max_disch_period_for_predc=<%=max_disch_period_for_predc%>&mother_cut_off_age_pda=<%=mother_cut_off_age%>&mother_max_age_pda=<%=mother_max_age%>&pat_dtls_unknown_yn_pda="+pat_dtls_unknown_yn+"&contact1_name_pda="+contact1_name+"&contact2_name_pda="+contact2_name+"'></form></BODY></HTML>";//Added By Kamatchi S on 23-JUN-2020 for ML-MMOH-CRF-1527
				...
				if(result_count > 0 && pen_orders_chk =="Y"){
					...
					//sam1=errmsge2.replace("$",orders);
					 ...
					 confirm=window.confirm("Pending orders exist. Click Ok to view and coninue" )
						...
						if(confirm==true ){
							HTMLVal = "<HTML><BODY onKeyDown='lockKey()'><form name='form1' id='form1' method='post' action='../../eIP/jsp/FindPrepareDischarge.jsp?weight_on_admn_mandatory="+weight_on_admn_mandatory+"&IP_val="+IPval+"&encounter_Id="+enc+"&patient_Id="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function=DischargeAdviceMain.jsp&jsp_name="+chk_jsp+"&Sex="+sex+"&p_service_type="+"<%=p_service_type%>"+"&patient_class="+patientclass+"&leave_exp_dt_flag="+leave_exp_dt_flag+"&mode=modal&setup_bl_dtls_in_ip_yn=<%=setup_bl_dtls_in_ip_yn%>&allow_discharge_date=<%=allow_discharge_date%>&max_disch_period_for_dc=<%=max_disch_period_for_dc%>&function_id=<%=function_id%>&bl_interfaced_yn=<%=bl_interfaced_yn%>&allow_discharge_date_predc=<%=allow_discharge_date_predc%>&max_disch_period_for_predc=<%=max_disch_period_for_predc%>&mother_cut_off_age_pda=<%=mother_cut_off_age%>&mother_max_age_pda=<%=mother_max_age%>&pat_dtls_unknown_yn_pda="+pat_dtls_unknown_yn+"&contact1_name_pda="+contact1_name+"&contact2_name_pda="+contact2_name+"'></form></BODY></HTML>";
			...
			{ 
				var HTMLVal		= new String();
				pat_check_in_allowed_yn = "<%=pat_check_in_allowed_yn%>";
			...
			else if(chk_jsp=="CheckinLodgerFrames.jsp") 				
			{
				...
				var features		= "dialogHeight:" + dialogHeight + "; scroll=no; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
				var arguments		= "" ;
				retVal				= 	await top.window.showModalDialog("../../eIP/jsp/CheckinLodgerFrames.jsp?location_code="+IPval+"&EncounterId="+enc+"&patientId="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function="+call_function+"&Sex="+sex+"&oper_stn_id=<%=oper_stn_id%>&mode=insert&Category=L",arguments,features);
				if(tot_count >= 1)
				parent.frames[2].document.location.reload();
				else
				parent.frames[2].document.location.href="../../eCommon/html/blank.html";
			...
			else if(chk_jsp=="CheckinAccmpPersonFrames.jsp") 				
			{
				...
				var features	= "dialogHeight:" + dialogHeight + "; scroll=no; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
				var arguments		= "" ;
				retVal				= 	await window.showModalDialog("../../eIP/jsp/CheckinAccmpPersonFrames.jsp?location_code="+IPval+"&EncounterId="+enc+"&patientId="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function="+call_function+"&Sex="+sex+"&oper_stn_id=<%=oper_stn_id%>&mode=insert&Category=A&Lodger_Max_RefNo=<%=Lodger_Max_RefNo%>&Lodger_Next_RefNo=<%=Lodger_Next_RefNo%>&Lodger_Period=<%=Lodger_Period%>&Acc_Period=<%=Acc_Period%>&Acc_Period_unit=<%=Acc_Period_unit%>&IP_param_No_Acc_Persons=<%=IP_param_No_Acc_Persons%>",arguments,features);
				if(tot_count >= 1)
					parent.frames[2].document.location.reload();
				else
					parent.frames[2].document.location.href="../../eCommon/html/blank.html";
			...
			else if(chk_jsp=="ConfirmBedSideReferralDetail.jsp") 				
			{
				...
				var features		= "dialogHeight:" + dialogHeight + "; scroll = no; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
				var arguments		= "" ;
				retVal = 	await window.showModalDialog("../../eIP/jsp/PaintReferralPatientDetails.jsp?location_code="+IPval+"&encounterId="+enc+"&patientId="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function=<%=call_function%>&Sex="+sex+"&mode=modal&bl_instal="+instl_yn+"&p_service_type=<%=p_service_type%>&bed_block_period_dis=<%=bed_block_period_dis%>&dis_date_chk_days=<%=dis_date_chk_days%>&oper_stn_id=<%=oper_stn_id%>",arguments,features);
				if(tot_count >= 1)
					parent.frames[2].document.location.reload();
				else
					parent.frames[2].document.location.href="../../eCommon/html/blank.html";
			...
			else if(chk_jsp=="PaintReferralPatientDetails.jsp") 				
			{
				...
				var features		= "dialogHeight:" + dialogHeight + "; scroll=no; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
				var arguments		= "" ;
				retVal = 	await top.window.showModalDialog("../../eIP/jsp/PaintReferralPatientDetails.jsp?function_id="+function_id+" 	&location_code="+IPval+"&encounterId="+enc+"&patientId="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function=<%=call_function%>&Sex="+sex+"&mode=modal&bl_instal="+instl_yn+"&p_service_type=<%=p_service_type%>&bed_block_period_dis=<%=bed_block_period_dis%>&dis_date_chk_days=<%=dis_date_chk_days%>",arguments,features);
			...
			else
			{
				if( document.forms[0].modal.value != 'yes')
				{
					...
					{
						condition = 'pat_check_in_allowed_yn="<%=pat_check_in_allowed_yn%>"';
					}
					else
					{
					...
					if( model_window == 'NO')
					{
							parent.parent.frames[1].document.location.href="../../"+module+"/jsp/"+jsp_name+"?encounterId="+enc+"&function_name="+function_name+"&patientId="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function=<%=call_function%>&Sex="+sex+"&bl_instal="+instl_yn+"&"+condition;
					}
					else if( model_window == 'N')
					{
					parent.frames[3].document.location.href="../../"+module+"/jsp/"+jsp_name+"?encounterId="+enc+"&function_name="+function_name+"&patientId="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function=<%=call_function%>&Sex="+sex+"&bl_instal="+instl_yn+"&"+condition;
					}
					else
					{
						var retVal		=    new String();
						...
						{
							retVal =    await parent.window.showModalDialog("../../"+module+"/jsp/DischargeCheckListFrames.jsp?location_code="+IPval+"&encounterId="+enc+"&patientId="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function=<%=call_function%>&menu_id=<%=menu_id%>&Sex="+sex+"&mode=modal&bl_instal="+instl_yn+"&confirmIP=<%=confirmIP%>&p_service_type=<%=p_service_type%>&disc_to_date=<%=disc_to_date%>&disc_from_date=<%=disc_from_date%>",arguments,features);
						}
						else
						{
							if('<%=call_function%>' != 'MODIFY_SERVICE_FOR_INPATIENT') {   
								...
								alert(getMessage("MDR_FINALIZED_DISCH_CANT_CANCEL","MR"));
										...
										retVal =    await top.window.showModalDialog("../../"+module+"/jsp/"+jsp_name+"?weight_on_admn_mandatory="+weight_on_admn_mandatory+"&height_on_admn_mandatory="+height_on_admn_mandatory+"&enable_height_bmi_pat_cls="+enable_height_bmi_pat_cls+"&location_code="+IPval+"&encounterId="+enc+"&function_id="+function_id+"&function_name="+function_name+"&patientId="+patid+"&Practitioner_Id="+escape(practid)+"&callfrom=IP&call_function=<%=call_function%>&Sex="+sex+"&mode=modal&bl_instal="+instl_yn+"&status_value="+status_value+"&confirmIP=<%=confirmIP%>&p_service_type=<%=p_service_type%>&chg_nurs_unit_in_assign_bed_yn="+chg_nurs_unit_in_assign_bed_yn+"&oper_stn_id=<%=oper_stn_id%>&pat_check_in_allowed_yn=<%=pat_check_in_allowed_yn%>&setup_bl_dtls_in_ip_yn=<%=setup_bl_dtls_in_ip_yn%>&bl_interfaced_yn=<%=bl_interfaced_yn%>&priotity_status=<%=priotity_status%>&change_bed_class_yn=<%=bed_class_allowed%>&deactivate_pseudo_bed_yn=<%=deactivate_pseudo_bed_yn%>&dflt_blk_prd_for_tfr_out=<%=dflt_blk_prd_for_tfr_out%>&bed_block_period_normal=<%=bed_block_period_normal%>&reserve_bed_on_tfr_yn=<%=reserve_bed_on_tfr_yn%>&res_reserve_bed_intraward_yn=<%=res_reserve_bed_intraward_yn%>&bed_block_period_max=<%=bed_block_period_max%>&bed_block_period_max_type=<%=bed_block_period_max_type%>&allow_multiple_bed_for_resv_yn=<%=allow_multiple_bed_for_resv_yn%>&mother_cut_off_age_pda=<%=mother_cut_off_age%>&mother_max_age_pda=<%=mother_max_age%>&max_disch_period_for_predc=<%=max_disch_period_for_predc%>&transfer_wo_delink_yn=<%=transfer_wo_delink_yn%>&allowCancelAdmsn="+allowCancelAdmsn+"&contact1_name_pda="+encodeURIComponent(contact1_name)+"&contact2_name_pda="+encodeURIComponent(contact2_name)+""+condition,arguments,features);
...
<Script src=\"../../eCommon/js/showModalDialog.js\" language=\"JavaScript\"></Script>
		...
		*/
		...
		<tr>
			...
			<td width = "25%" class='white' align="right"><input type='button' name='search' id='search' value='<%=com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.cancel.label","common_labels")%>' class='button' >
			</td>
			</tr>
			</table>
			<%}
...
<!-- 0-7/09/2009    end-->
			...
			<input type='hidden' name='height_on_admn_mandatory' id='height_on_admn_mandatory' value="<%=height_on_admn_mandatory%>" >  <!-- Added by KAMATCHI S on 23-JUN-2020 fro ML-MMOH-CRF-1527 -->
			...
			<!--Added By Dharma on 28th Mar 2019 AMRI-CRF-0380 [IN:067892] Start-->
			<input type='hidden' name='chk_pen_sale_rtn_bfr_dis_adv' id='chk_pen_sale_rtn_bfr_dis_adv' value="<%=chk_pen_sale_rtn_bfr_dis_adv%>" >
			<input type='hidden' name='isRestrictDisAdvSaleRetPend' id='isRestrictDisAdvSaleRetPend' value="<%=isRestrictDisAdvSaleRetPend%>" >
			<!--Added By Dharma on 28th Mar 2019 AMRI-CRF-0380 [IN:067892] End-->

			<input type='hidden' name='allowCancelAdm' id='allowCancelAdm' value="<%=allowCancelAdm%>" >  <!-- Added by Thamizh selvi on 20th Feb 2017 against MMS-QH-CRF-0182 -->
			<!--Added by Shanmukh on 23rd-MAY-2018 for ML-MMOH-CRF-1135 -->
            <input type='hidden' name='chk_dis_pat_bt_order_pend' id='chk_dis_pat_bt_order_pend' value="<%=chk_dis_pat_bt_order_pend%>">
			<input type='hidden' name='blood_consump_status' id='blood_consump_status' value="<%=blood_consump_status%>">
			<input type='hidden' name='isDisPatBTorderPendAppl' id='isDisPatBTorderPendAppl' value="<%=isDisPatBTorderPendAppl%>">
			<!--end-->
			<input type='hidden' name='isNewBornChngsAppl' id='isNewBornChngsAppl' value="<%=isNewBornChngsAppl%>">
			<input type='hidden' name='trans_yn_unfnzed_newborn' id='trans_yn_unfnzed_newborn' value="<%=trans_yn_unfnzed_newborn%>">
