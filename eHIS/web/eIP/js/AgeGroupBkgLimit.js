function apply()
{
	...
	{
		var age_group_code=frames[1].frames[0].document.forms[0].age_group.value;
		...
		if(age_group_code=="")
		{
			...
			err= err.replace("$",getLabel('Common.agegroup.label','common'));
		...
		s="<html><body class='message' onKeyDown='lockKey()'><form name='final' id='final' method='post' action= '../../servlet/eIP.AgeGroupBkgLimitServlet'><input type=hidden name=age_group_code value='"+age_group_code+"'>";
	...
	//end addition
		...
		if(bool)
		{
			//var frm1=parent.frames[1].frames[1].frames[2].document.forms[0];// frame2
 ...
 function create() 
  {
	f_query_add_mod.location.href = "../../eIP/jsp/AgeGroupMain.jsp?" ;
  } 

function query()
  {
	f_query_add_mod.location.href ="../../eIP/jsp/AgeGroupBkgLimitQreryCriteria.jsp?param=query" ;
  }
...
/*function reset()
{
	...
	var loc=frames[1].frames[1].document.forms[0];
...
function popDynamicValues(obj)
{
	...
	HTMLval+= "<body class='message' onKeyDown='lockKey()'><form name='f' id='f' action='../../eIP/jsp/FetValForAgeGroupBkgLimits.jsp'>"
	HTMLval+= "<input type='hidden' name='age_grp_code' id='age_grp_code' value='"+obj.value+"'>"
	HTMLval+= "</form></body>"
...
function calTotal(obj)
{
	if(obj.value != '' && obj.value != '0' )
		self.document.AgeGroupBookingLimit_Form.max_total.value = obj.value;
}


// pasted from.... addModifyAgeGroupBkgLimit.jsp
...
function first()
{
	var M=parent.Dummy.document.getElementById('M').value;
	var F=parent.Dummy.document.getElementById('F').value;
	var U=parent.Dummy.document.getElementById('U').value;
	var T=parent.Dummy.document.getElementById('T').value;
	var S=parent.Dummy.document.getElementById('S').value;
...
function chkExists(mode, chkStr)
{
    ...
    /*function setTotal(id)
    {
        ...
        var max_male_dly_booking=eval("document.forms[0].M"+i+".value");
        var max_female_dly_booking=eval("document.forms[0].F"+i+".value");
        var max_unknown_dly_booking=eval("document.forms[0].U"+i+".value");
        var max_total_dly_booking=eval("document.forms[0].T"+i+".value");
...
function bookingLimitChk(obj,i){
	
	if(eval("document.forms[0].F"+i+".value")=='0'||eval("document.forms[0].M"+i+".value")=='0'||eval("document.forms[0].U"+i+".value")=='0'){

		if(eval("document.forms[0].T"+i+".value")=='0'){

		var err1= getMessage("CANNOT_BE_ZERO","IP");		
		err1= err1.replace("$",getLabel('Common.total.label','Common') +" "+getLabel('eIP.MaxDailyBookingLimit.label','IP') );
		eval("document.forms[0].T"+i+".value='0'");
		alert(err1);
		eval("document.forms[0].F"+i+".value=''");
		eval("document.forms[0].M"+i+".value=''");
		eval("document.forms[0].U"+i+".value=''");
		
		eval("document.forms[0].T"+i+".value=''");
		}
		else
		{
			if(eval("document.forms[0].F"+i+".value")=='0')
				eval("document.forms[0].F"+i+".value=''");
			if(eval("document.forms[0].M"+i+".value")=='0')
				eval("document.forms[0].M"+i+".value=''");
			if(eval("document.forms[0].U"+i+".value")=='0')
				eval("document.forms[0].U"+i+".value=''");
