...
%>
<form name="formBillToDeliverToDetails" id="formBillToDeliverToDetails">
...
<%	if (alItemKeys != null && (alItemKeys.size()>0)) {
				...
				//	for (int i=0;	i<hmList.size();	i++){
						...
						if(bean.getBtDRecords() != null && bean.getBtDRecords().size() > 0 ){
						ArrayList bDArrayList	=	(ArrayList)bean.getBtDRecords().get(0);
						if(bDArrayList.size()>0){
							for(int j=0;j<bDArrayList.size();j++){
							HashMap hm = (HashMap)bDArrayList.get(j);
							if(facility_code.equals(bean.checkForNull((String)hm.get("FACILITY_CODE")))){
							  	facility_code = bean.checkForNull((String)hm.get("FACILITY_CODE"));
								bill_to = bean.checkForNull((String)hm.get("BILL_TO"));
								deliver_to  = bean.checkForNull((String)hm.get("DELIVER_TO"));
	...
	%>
		...
		<tr id="tr_<%=i%>">
			<th colspan=4 align=left><%=facility_name%></th>
