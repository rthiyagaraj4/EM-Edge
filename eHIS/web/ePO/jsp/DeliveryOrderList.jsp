<!DOCTYPE html>
<%@ page import="ePO.DeliveryOrderBean, ePO.DeliveryOrderDetailBean,java.sql.*,java.util.*,webbeans.eCommon.*,java.io.UnsupportedEncodingException,
...
<script src='../../eCommon/js/showModalDialog.js' language='JavaScript'></script>
	...
	%>
<body onLoad="loadExRate();onLoadCurrCode('<%=dflt_currency_desc%>','<%=currency_desc_lc%>');">
	<form name='formDeliveryOrderList' id='formDeliveryOrderList'>
		...
		<table name ="tblItemList" border='1' width='100%' cellpadding='0' cellspacing='0' > 
			<tr colspan=11>
				<th colspan=11 nowrap><fmt:message key="ePO.DeliveryOrderDetails.label" bundle="${po_labels}"/></th>
			</tr>
		</table>
		...
		<table name ="tblItemList" border='1' width='100%' cellpadding='0' cellspacing='0' > 
			...
			<tr style='height:40px;' style='position:relative' style='top:expression(this.offsetParent.scrollTop-2)' >
				...
				<td class='COLUMNHEADER' nowrap>
				<fmt:message key="ePO.ScheduledDate.label" bundle="${po_labels}"/></td>
				<td class='COLUMNHEADER' nowrap> 
				<fmt:message key="ePO.PONo.label" bundle="${po_labels}"/></td>
				<td class='COLUMNHEADER' nowrap>
				...
				<td class='COLUMNHEADER' nowrap>
				<fmt:message key="ePO.POUOM.label" bundle="${po_labels}"/>
				</td>
				<td class='COLUMNHEADER' nowrap>
				<fmt:message key="ePO.DoQty.label" bundle="${po_labels}"/></td>
				<td class='COLUMNHEADER' nowrap>
				<fmt:message key="ePO.DoUOM.label" bundle="${po_labels}"/>
				</td>
				...
				<td class='COLUMNHEADER' nowrap>
				<fmt:message key="ePO.MultiBatchDetails.label" bundle="${po_labels}"/></td>	
				<td class='COLUMNHEADER' nowrap>
			...
			<tr id="tr_<%=i%>">
				...
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right'><%=bean.checkForNull(detailBean.getDelv_date(),"&nbsp;")%></td>
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right' onClick = "MoreDetails('<%=bean_id%>','<%=bean_name%>','<%=mode%>','<%=bean.checkForNull(detailBean.getPo_no())%>','<%=bean.checkForNull(detailBean.getItem_code())%>','<%=bean.checkForNull(detailBean.getDelv_date())%>','<%=bean.checkForNull(detailBean.getRequest_no())%>')" onMouseOver="changeCursor(this);"><font class="HYPERLINK"><%=bean.checkForNull(po_no,"&nbsp;")%></font></td>
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right'><%=bean.checkForNull(detailBean.getRequest_no(),"&nbsp;")%></td>
				
				
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right'><%=bean.checkForNull(detailBean.getOrder_qty(),"&nbsp;")%></td>
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right'><%=bean.checkForNull(detailBean.getUomDesc(),"&nbsp;")%></td>
				<% if (detailBean.getDelivery_qty() != "") { %>
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right'><%=bean.checkForNull(detailBean.getDelivery_qty(),"&nbsp;")%></td>
			...
			<% if (detailBean.getDelivery_uom_desc() != "") { %>
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right'><%=bean.checkForNull(detailBean.getDelivery_uom_desc(),"&nbsp;")%></td>
			...
			<%  bdUOM = bean.checkForNull(detailBean.getUom());
				...
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right'><%=bean.checkForNull(detailBean.getDelivery_price(),"&nbsp;")%></td>
			<% if (detailBean.getDelivery_qty() != "") { %>	
			...
			<% } else { 
				...
				net_amt = n;
				%>	
				<td class="<%=className%>" nowrap style="WORD-BREAK:BREAK-ALL;" align='right'><%=n%></td>
			<% } %>
		<!--		<td class="<%=className%>" nowrap align='center' onMouseOver="changeCursor(this);"   onClick = "BatchDetails1('<%=bean_id%>','<%=bean_name%>','<%=mode%>','<%=bean.checkForNull(detailBean.getPo_no())%>','<%=bean.checkForNull(detailBean.getItem_code())%>','<%=bean.checkForNull(detailBean.getItem_desc())%>','<%=bean.checkForNull(detailBean.getDelivery_price())%>','<%=bean.checkForNull(detailBean.getDelv_date())%>','<%=bean.checkForNull(detailBean.getRequest_no())%>','<%=bdQty%>','<%=bdUOMDesc%>','<%=bdUOM%>','<%=detailBean.getPo_srl_no()%>','<%=bean.checkForNull(detailBean.getFacility_code())%>','<%=bean.checkForNull(detailBean.getStore_code())%>');"><font class='HYPERLINK'><fmt:message key="ePO.MultiBatchDetails.label" bundle="${po_labels}"/></font></td> modified by B.Badmavathi against ML-BRU-SCF-1616 -->
			 <td class="<%=className%>" nowrap align='center' onMouseOver="changeCursor(this);"   onClick = "BatchDetails1('<%=bean_id%>','<%=bean_name%>','<%=mode%>','<%=bean.checkForNull(detailBean.getPo_no())%>','<%=bean.checkForNull(detailBean.getItem_code())%>','<%=item_desc%>','<%=bean.checkForNull(detailBean.getDelivery_price())%>','<%=bean.checkForNull(detailBean.getDelv_date())%>','<%=bean.checkForNull(detailBean.getRequest_no())%>','<%=bdQty%>','<%=bdUOMDesc%>','<%=bdUOM%>','<%=detailBean.getPo_srl_no()%>','<%=bean.checkForNull(detailBean.getFacility_code())%>','<%=bean.checkForNull(detailBean.getStore_code())%>');"><font class='HYPERLINK'><fmt:message key="ePO.MultiBatchDetails.label" bundle="${po_labels}"/></font></td>
				...
				<input type="hidden" name="delv_price_<%=i%>" id="delv_price_<%=i%>" value="<%=bean.checkForNull(detailBean.getDelivery_price())%>">
		...
		<table name ="tblItemList" border='0' width='98%' cellpadding='0' cellspacing='0' >
			<tr>
				...
				<th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<fmt:message key="ePO.NetDOAmount.label" bundle="${po_labels}"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label id='currency_label_7' class="label" >&nbsp;<B></B></label></th>
			...
			<tr id="netDoAmtHidden" style=visibility:hidden>
				...
				<th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<fmt:message key="ePO.NetDOAmount.label" bundle="${po_labels}"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label id='currency_label_8' class="label" >&nbsp;<B></B></label></th>
				<% if(bean.checkForNull(bean.getNet_poAmt()) == "") {%>
