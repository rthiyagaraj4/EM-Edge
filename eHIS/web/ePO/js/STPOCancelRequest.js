...
function Modify(obj){
	var req_no =obj.cells[1].innerText;
	var req_code =obj.cells[5].innerText;
	parent.document.location.href="../../ePO/jsp/STPOCancelRequestFrame.jsp?mode="+MODE_MODIFY+"&function_id="+parent.parent.function_id+"&req_code="+req_code+"&req_no="+req_no;
}
...
function query() {
		f_query_add_mod.location.href="../../ePO/jsp/STPOCancelRequestQueryFrame.jsp?function_id="+function_id+"&mode="+MODE_MODIFY;
}

function apply() {

	if( f_query_add_mod.STPOCancelRequestQueryResultFrame==undefined){
		messageFrame.location.href ="../../eCommon/jsp/error.jsp?err_num=";
		return;
	}
	var errorPage = "../../eCommon/jsp/error.jsp";
	var formObject = f_query_add_mod.STPOCancelRequestQueryResultFrame.document.formSTPOCancelRequestQueryResult;
...
//	xmlStr = getXMLString(formObject);
	...
	xmlHttp.open("POST", "../../ePO/jsp/STPOCancelRequestValidate.jsp?bean_id="+bean_id+"&bean_name="+bean_name+"&function_type=4", false);
	xmlHttp.send(xmlDom);
...
async function displayPending(trObj) {
	changeRowColor(trObj,7);
	var req_no =trObj.cells[1].innerText;
	var req_code = trObj.cells[7].innerText
	retVal= await window.showModalDialog("../../ePO/jsp/STPOCancelRequestList.jsp?function_id="+parent.parent.function_id+"&req_code="+req_code+"&req_no="+req_no,'','dialogHeight:15; dialogWidth:40; center:0; status: no; scroll: no; dialogLeft :75; dialogTop :250;');
...
async function showReason(trObj,index) {
	...
	var req_no =trObj.cells[1].innerText;
	var req_code = trObj.cells[7].innerText
	if (eval ("document.formSTPOCancelRequestQueryResult.doc_"+index+".checked")){
		retVal= await window.showModalDialog("../../ePO/jsp/STPOCancelRequestRemarks.jsp?function_id="+parent.parent.function_id+"&req_code="+req_code+"&req_no="+req_no,'','dialogHeight:10; dialogWidth:32; center:0; status: no; dialogLeft :75; dialogTop :250;');
		
		while(retVal == undefined){
			alert(getMessage("PLS_PRESS_CLOSE_BUTTON","PO")); 
			retVal=await window.showModalDialog("../../ePO/jsp/STPOCancelRequestRemarks.jsp?function_id="+parent.parent.function_id+"&req_code="+req_code+"&req_no="+req_no,'','dialogHeight:10; dialogWidth:32; center:0; status: no; dialogLeft :75; dialogTop :250;');
		}
		(eval("document.formSTPOCancelRequestQueryResult.rem_"+index)).value=retVal;	
	}
}
function updateRemarks() {
	formObject = document.formSTPOCancelRequestRemarks;
	...
	//xmlDom.loadXML(xmlStr);
	xmlDom = new DOMParser().parseFromString(xmlStr, "text/xml");
	xmlHttp.open("POST", "../../ePO/jsp/STPOCancelRequestValidate.jsp?bean_id="+bean_id+"&bean_name="+bean_name+"&function_type=1", false);
	xmlHttp.send(xmlDom);
	responseText=xmlHttp.responseText;
	eval(responseText);
...
function SubmitLink(from, to) {
	var formObject = document.formSTPOCancelRequestQueryResult;
	var selectedIndices = "";
	var index = 0;
	var total_check_boxes = formObject.total_check_boxes.value;
	for (count = 0;count<total_check_boxes;count++) {
	...
	assignBean(formObject);
//	xmlStr = getXMLString(formObject);
	xmlStr = getXMLStringForReqCancel(formObject);
	//xmlDom.loadXML(xmlStr);
	xmlDom = new DOMParser().parseFromString(xmlStr, "text/xml");
	xmlHttp.open("POST", "../../ePO/jsp/STPOCancelRequestValidate.jsp?bean_id="+bean_id+"&bean_name="+bean_name+"&function_type=2", false);
	xmlHttp.send(xmlDom);
	responseText=xmlHttp.responseText;
	eval(responseText);
...
function changeMyRowColor(index,n,chkObj) { 
	...
	//	xmlStr = getXMLString(formObject);
		...
		//xmlDom.loadXML(xmlStr);
		xmlDom = new DOMParser().parseFromString(xmlStr, "text/xml");
		xmlHttp.open("POST", "../../ePO/jsp/STPOCancelRequestValidate.jsp?bean_id="+bean_id+"&bean_name="+bean_name+"&key="+chkObj.value+"&function_type=3", false);
		xmlHttp.send(xmlDom);
		responseText=xmlHttp.responseText;
		eval(responseText);
	}
...
function submitQueryCriteria(){
	if (isValidFromToField(document.formSTPOCancelRequestQueryCriteria.from_doc_date,document.formSTPOCancelRequestQueryCriteria.to_doc_date,DATE,"Doc Date",parent.parent.messageFrame,"../../eCommon/jsp/error.jsp")) {
		document.formSTPOCancelRequestQueryCriteria.submit();
	}
}
function toggleCheck(formObject){
	...
	for (i=0; i<allElements.length; i++){
		...
		//	alert('allElements['+ i +'] . type = ' +allElements[i].type);
...
/*function populateStores(request_by_store) {
	...
	assignBean(document.formSTPOCancelRequestQueryCriteria);
	xmlStr												=		getXMLString(arrayObject);
	xmlDom.loadXML(xmlStr);
	xmlHttp.open("POST", "../../ePO/jsp/STPOCancelRequestValidate.jsp?bean_id="+bean_id+"&bean_name="+bean_name+"&function_type=3", false);
	xmlHttp.send(xmlDom);
	responseText										=		xmlHttp.responseText;
	eval(responseText);
...
function frmdate(){
   var from_date    =  document.formSTPOCancelRequestQueryCriteria.from_doc_date.value ;
   var to_doc_date =  document.formSTPOCancelRequestQueryCriteria.to_doc_date.value ;
   var locale=document.formSTPOCancelRequestQueryCriteria.p_language_id.value;
   if(locale != "en") {
   ...
   document.formSTPOCancelRequestQueryCriteria.from_doc_date.focus();
   return ;
   }
   else if(to_doc_date==""){
   alert(getMessage("TO_DATE_NOT_BLANK", "PO"));
   document.formSTPOCancelRequestQueryCriteria.to_doc_date.focus();
   return ;
   }
...
function getXMLStringForReqCancel(frmObj) {
