...
var prevColor = null;
function reset() {
f_query_add_mod.criteria.formSTPORequestStatusQueryCriteria.document.formSTPORequestStatusQueryCriteria.reset();
...
parent.frames[1].location.href = "../../eCommon/html/blank.html" ;
	...
	xmlHttp.open("POST", "../../ePO/jsp/PORequestStatusValidate.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name + "&request_by_store=" + request_by_store.value + "&&Validate=GETDEPARTMENT", false);
	xmlHttp.send(xmlDom);
...
function populateItemClasses(request_by_store, request_on_department) {
	...
	xmlStr = getXMLString(arrayObject);
	xmlDom.loadXML(xmlStr);
	xmlHttp.open("POST", "../../ePO/jsp/STPORequestStatusValidate.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name + "&Validate=GETITEMCLASSES", false);
	xmlHttp.send(xmlDom);
	responseText = xmlHttp.responseText;
	eval(responseText);
}

function searchItemCode(objCode) {
	var search_string = "";
	search_string = document.formSTPORequestStatusQueryCriteria.item_code.value;
	var store_code = "";
	var category = "A";
	var sole_source = "A";
	var ps_item = "A";
	...
	if (retVal != null) {
		document.formSTPORequestStatusQueryCriteria.item_code.value = retVal[0];
	}
}
...
function displayPending(trObj) {
	...
	changeColor(trObj, 7);
	retVal = window.showModalDialog("../../ePO/jsp/STPORequestStatusFrameList.jsp?function_id=" + parent.parent.function_id + "&doc_type_code=" + doc_type_code + "&doc_no=" + doc_no + "&doc_type_desc=" + doc_type_desc, "", "dialogHeight:17; dialogWidth:50; center:0; status: no; dialogLeft :75; dialogTop :250;");
	retainColor(trObj, 7);
}
...
function displayAuditInfo(trObj) {
	...
	retVal						=	window.showModalDialog("../../ePO/jsp/STPORequestStatusAuditInfo.jsp?function_id=" + parent.parent.function_id + "&added_by_id=" + added_by_name + "&modified_by_id=" + modified_by_name + "&modified_date=" + modified_date + "&request_status=" + request_status + "&doc_no=" + doc_no + "&doc_type_desc=" + doc_type_desc, "", "dialogHeight:10; dialogWidth:20; center:1; status: no; dialogLeft :475; dialogTop :250;scroll:no");
	retainColor(trObj, 7);
}
