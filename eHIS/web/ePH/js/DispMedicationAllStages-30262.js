...
async function callReason(obj,order_id,drug_code,no,order_line_no,curr_stage,scope)	{
    if(!obj.disabled){
		...
		var arguments	= "";
		retVal = await window.showModalDialog("../../ePH/jsp/DispMedicationHoldReason.jsp?recno="+no+"&order_id="+order_id+"&drug_code="+drug_code+"&order_line_no="+order_line_no+"&fromdb="+fromdb+"&held="+held,arguments,features);
	...
	if (frmObj.reason.disabled){
		...
		//if (fromdb=="Y"){
				...
				//if(checkFlds( fields, names)){
					...
					xmlStr +=" /></root>";
					var temp_jsp="DispMedicationValidation.jsp?func_mode=release";
					var xmlDoc = "" ;
...
function callActiveOrders(patient_id,encounter_id,generic_id,order_id,order_line_no,allergy_cnt,patient_class,disp_medication)	{
	//order_id,order_line_no,allergy_cnt,patient_class,disp_medication addded for IN070605
	...
	var arguments	= "" ;
	//order_id,order_line_no,allergy_cnt,patient_class,disp_medication addded for IN070605
	retVal = window.showModalDialog("../../ePH/jsp/PrescriptionActiveOrderDetailsFrame.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&generic_id="+generic_id+"&order_id="+order_id+"&order_line_no="+order_line_no+"&allergy_cnt="+allergy_cnt+"&patient_class="+patient_class+"&disp_medication="+disp_medication,arguments,features);
	//added for IN070605 start
	if(retVal!="" && retVal != null){
...
function showMonoGraph(drug_desc,mono_graph,daily_dose,unit_dose,min_unit_dose,min_daily_dose,dosage_unit,std_dose,dose_unit_desc,order_id,order_line_no,allergy_cnt,patient_class,disp_medication,max_daily_ceeling_dose,min_daily_ceeling_dose,max_unit_ceeling_dose,min_unit_ceeling_dose,dose_unit_desc1){//dose_unit_desc added for ML-MMOH-SCF-0863
	//order_id,order_line_no,allergy_cnt,patient_class,disp_medication addded for IN070605
...
function showDrugDetails(patient_class,locn_code,order_id,drug_code,ord_line_no,called_from,disp_locn_code,from_sliding_scale_yn,mfr_yn, patient_id,amend_Dtls,generic_id,order_line_status){  // patient_id  added for [IN:035667]  (AmendDtls,generic_id,order_line_status added for JD-CRF-0198 [IN058599])
...
function showOverRideReason(allergy,exceed_dose,duplicate) {
	var dialogHeight = "18";
	...
	var center			= "1" ;
 	...
 	var arguments		= "";
 	retVal 				= window.showModalDialog("../../ePH/jsp/DispMedicationOverRideReasons.jsp?allergy="+encodeURIComponent(allergy,"UTF-8")+"&exceed_dose="+encodeURIComponent(exceed_dose,"UTF-8")+"&duplicate="+encodeURIComponent(duplicate,"UTF-8"),arguments,features);
}


function callFilling(patient_id,order_id,drug_code,disp_stage,sub_disp_stage) {
	...
	var center			= "1" ;
 	var features		= 'Width:' + dialogWidth +'; dialogHeight:' + dialogHeight + '; dialogWidth:' + dialogWidth +'; center:' + center +';  dialogTop:'+dialogTop +'; scroll=no; status=no';
 	var arguments		= "";
 	retVal 				= window.showModalDialog("../../ePH/jsp/DispMedicationAllocateFrames.jsp?patient_id="+patient_id+"&order_id="+order_id+"&chosen_drug="+drug_code+"&source=Fill",arguments,features);
...
function callAllocate(patient_id,order_id,drug_code,disp_stage,sub_disp_stage) {
	var retVal;
	var dialogTop		= ""; 
 	var dialogHeight	= "" ;
	if(disp_stage=="A")
...
//var durationVal =frmObj.durationVal.value; // Added for ML-MMOH-SCF-0690 if elseadded for fill withfilllist stage not working issue
	...
	//window.open("../../ePH/jsp/DispMedicationAllocateFrames.jsp?patient_id="+patient_id+"&order_id="+order_id+"&chosen_drug="+drug_code+"&source="+source+"&orderIDS_trxRefIDS="+OrderIds);
 	retVal 				= window.showModalDialog("../../ePH/jsp/DispMedicationAllocateFrames.jsp?patient_id="+patient_id+"&order_id="+order_id+"&chosen_drug="+drug_code+"&source="+source+"&durationVal="+durationVal+"&orderIDS_trxRefIDS="+OrderIds,arguments,features); // "&durationVal="+durationVal Added for ML-MMOH-SCF-0690
...
function verifyAuthorization(drug_class,disp_narcotic_yn,disp_auth_yn, disp_controlled_yn) { // disp_controlled_yn added for SKR-SCF-0688 [IN:036036]
var return_val	=	true;
	...
	if( (drug_class=="N" && disp_narcotic_yn=="N") || (drug_class=="C" && disp_controlled_yn=="N")) { //if condition changed from above for SKR-SCF-0688 [IN:036036]
		...
		if( disp_narcotic_yn=="Y") {			  
			...
			var features	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
			var arguments	= "" ;
			retVal = window.showModalDialog("../../ePH/jsp/DispMedicationAuthorize.jsp",arguments,features);
			if(retVal=="OK") {
				return_val	=	true;
			}
			else {
...
function callStockAdd(det,oper_mode) {
	...
	if(verifyAuthorization(drug_class,disp_narcotic_yn,disp_auth_yn, disp_controlled_yn)) { // disp_controlled_yn added for SKR-SCF-0688 [IN:036036]
		if(checkTotQty(frmObj)) {
			...
			xmlStr +=" /></root>";
			xmlDoc = new DOMParser().parseFromString(xmlStr, "text/xml");
			xmlHttp.open("POST","DispMedicationValidation.jsp?func_mode=stock_items",false);
			xmlHttp.send(xmlDoc);
...
function evaluateBaseOnForm(obj,frmObj,barcode_scan_for_alt_app,drug_yn,cunt,main_screen_yn,disp_non_preference_rem_window,patient_id,encounter_id,alt_drug_code)	{ //barcode_scan_for_alt_app,drug_yn,cunt,main_screen_yn added for MMS-KH-CRF-0030 and ,disp_non_preference_rem_window,patient_id,encounter_id,alt_drug_code added for GHl-ICN-075
	if(obj.checked==true)	{
		//GHL-ICN-0075
		if(disp_non_preference_rem_window==true || disp_non_preference_rem_window=="true"){
		  var allow_proc = formularyNonPreference(alt_drug_code,patient_id,encounter_id);
		  if(allow_proc==false){
			  obj.checked=false;
		    return false;
		  }
...
function changeRowColor(no) {
	...
	var len	=	eval(document.getElementById("tr_"+no).cells.length");
		for(i=1; i<len; i++)
		eval(document.getElementById("tr_"+no).cells(i).style.backgroundColor='#ffffcc'");
...
// To enable/disable next stage button
	for(i=1; i<tot_recs; i++){
		var val	=	trimString(eval("document."+curr_but_name+i+".innerText"));
...
function controlAllocButton() {
	...
	//Enable or Diable delivery button (if present )
	for(i=1; i<tot_recs; i++){
		var val	=	trimString(eval(document.getElementById("alloc_detail"+i).innerText"));
		if(val=="Detail" || val=="Det"){
			detail	=	true;
			break;
		}
...
function callDelivery(patient_id,issue_token_on_regn_yn){
	...
	var features	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "" ;
	retVal = window.showModalDialog("../../ePH/jsp/DispMedicationCollectorDetails.jsp?patient_id="+patient_id+"&token_no="+tokenNo,arguments,features);
...
function callStockEntry(patient_id,order_id,order_line_no,drug_code,drug_desc,multi_strength,alt_drug_code,alt_drug_desc,color,qty_uom,qty,source,no,drug_strength,alt_drug_strength,end_date,uom_code,drug_class,stk_qty,min_end_date,main_strength_per_value_pres_uom, PRNOrAbsOrder,alt_drg_no,alt_strength_uom,alt_drug_line_no,facility_id,disp_locn_code) {// alt_drug_line_no added for  ML-BRU-SCF-0474[IN035407]// facility_id,disp_locn_code parameters added for KDAH-CRF-0231 [IN-034551] .
	...
	parent.parent.f_stock_dtls.location.href="../../ePH/jsp/DispMedicationStockDetails.jsp?drug_code="+drug_code+"&drug_desc="+drug_desc+"&patient_id="+patient_id+"&order_id="+order_id+"&order_line_no="+order_line_no+"&alt_drug_status="+multi_strength+"&source="+source+"&alt_drug_code="+alt_drug_code+"&alt_drug_desc="+encodeURIComponent(alt_drug_desc,"UTF-8")+"&drug_color="+escape(color)+"&qty_uom="+escape(qty_uom)+"&qty="+qty+"&no="+no+"&drug_strength="+drug_strength+"&alt_drug_strength="+alt_drug_strength+"&end_date="+end_date+"&uom_code="+uom_code+"&bms_strength="+bms_strength+"&oper_mode="+oper_mode+"&drug_class="+drug_class+"&stk_qty="+stk_qty+"&min_end_date="+min_end_date+"&main_strength_per_value_pres_uom="+main_strength_per_value_pres_uom+"&resetQty="+resetQty+"&PrnAbs="+PRNOrAbsOrder+"&alt_drg_no="+alt_drg_no+"&alt_strength_uom="+alt_strength_uom+"&prescribed_strength="+prescribed_strength+"&facility_id="+facility_id+"&disp_locn_code="+disp_locn_code;//facility_id,disp_locn_code parameters added for KDAH-CRF-0231 [IN-034551] 
	
	if(parent.parent.f_drug_buttons.location.href.indexOf(".jsp")==-1){	
	      parent.parent.f_drug_buttons.location.href="../../ePH/jsp/DispMedicationAllocationButtons.jsp?patient_id="+patient_id+"&order_id="+order_id+"&order_line_no="+order_line_no+"&drug_code="+drug_code+"&alt_drug_status="+multi_strength+"&source="+source+"&called_frm=stock"+"&no="+no+"&resetQty="+resetQty+"&PrnAbs="+PRNOrAbsOrder;
	}
}
...
function clearItemDetail() {
	parent.parent.f_batch_title.location.href="../../eCommon/html/blank.html";
	parent.parent.f_drug_entry.location.href="../../eCommon/html/blank.html";
	parent.parent.f_buttons.location.href="../../eCommon/html/blank.html";
}


function displayDescription(obj,frmObj,drug_code,patient_id,order_id,order_line_no,drug_color,alt_drug_code,qty,uom,uom_code,source,recno,store_code,end_date,drug_strength,alt_drug_strength,called_from,bms_strength,oper_mode,drug_class,stk_qty,min_end_date,drug_desc,alt_drug_desc,main_strength_per_value_pres_uom,alt_drg_no,alt_strength_uom,prescribed_strength,barcode_flg) {//barcode_flg parameter added for KDAH-CRF-0231 [IN-034551]
	...
	if(obj.value!="")
		showItemDetail(drug_code,patient_id,order_id,order_line_no,drug_color,alt_drug_code,qty,uom,uom_code,source,recno,store_code,end_date,drug_strength,alt_drug_strength,called_from,bms_strength,oper_mode,drug_class,stk_qty,min_end_date,drug_desc,selectedText,alt_drug_desc,main_strength_per_value_pres_uom,alt_drg_no,alt_strength_uom,prescribed_strength,barcode_flg);//barcode_flg parameter added for KDAH-CRF-0231 [IN-034551]
}
...
function callBatchSearch(params,store_code,end_date,trade_code,allowchangetradenameyn,allowmultitradeyn,allow_short_expiry_drugs_yn) {
	frmObj			=	parent.parent.f_stock_dtls.document.DispMedicationStockDetails;
	...
	if(item_val=="")
		...
		var retval=callBatchSearchWindow(item_code,store_code,includeZeroStockBatches,includeExpiredBatches,includeSuspendedBatches,searchFunction,multipleSelect,trade_code,allowchangetradenameyn,allowmultitradeyn,end_date,allow_short_expiry_drugs_yn);
...
function callBatchSearchWindow(item_code,store_code,includeZeroStockBatches,includeExpiredBatches,includeSuspendedBatches,searchFunction,multipleSelect,trade_code,allowchangetradenameyn,allowmultitradeyn,end_date,allow_short_expiry_drugs_yn){
	var dialogHeight	= "30" ;
	var dialogWidth		= "60";
	var dialogTop		= "100" ;
	var dialogLeft		= "200" ;
...
function refreshStockWindow(patient_id,order_id,order_line_no,drug_code,drug_desc,multi_strength,alt_drug_code,alt_drug_desc,color,qty_uom,qty,source,no,oper_mode,alt_drg_no,alt_drg_sel) {
	if(multi_strength=="Y") {
		...
		if(parent.frames[0].DispMedicationAllocateDrugsTitle!="undefined")
			durationVal = parent.frames[0].DispMedicationAllocateDrugsTitle.durationVal.value; // // Added for ML-MMOH-SCF-0690 - End
		parent.parent.f_drug_detail.location.href="../../ePH/jsp/DispMedicationAllocationDetails.jsp?patient_id="+patient_id+"&order_id="+order_id+"&order_line_no="+order_line_no+"&drug_code="+drug_code+"&source="+source+"&recno="+no+"&alt_drg_no="+alt_drg_no+"&alt_drg_sel="+alt_drg_sel+"&durationVal="+durationVal; // durationVal Added for ML-MMOH-SCF-0690
	} 
	else {
		if(qty!="" && qty!=0) {
		   ...
		   var bms_det = " "+pres_qty +"<B>("+bms_qty+")</B>"+" "+qty_uom;
			 ...
			 eval(parent.parent.f_drug_detail.document.getElementById("bms_det_"+no).innerHTML=bms_det");
		}
		no = parseInt(no);
		parent.parent.f_drug_detail.document.DispMedicationAllocateDetails.oper_mode.value=oper_mode;
		clickNextDrug(++no,parent.parent.f_drug_detail.document,window,alt_drg_no,patient_id,'',source);
	}
}
...
function callPreview(patient_id,order_id) {
	var dialogHeight	= "18" ;
	var dialogWidth		= "60";
	var dialogTop		= "278";
	var center			= "1" ;
	...
	var features		= "dialogHeight:" + dialogHeight +"; dialogLeft"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments		= "";
	retVal = window.showModalDialog("../../ePH/jsp/DispMedicationAllocatePreview.jsp?patient_id="+patient_id+"&order_id="+order_id,arguments,features);
}
...
function controlFillingButton() {
	...
	var found	=	false;
	for(i=1; i<tot_cnt; i++) {
		if(eval(formObj.document.getElementById("alloc_qty"+i))!=undefined) {
			val	=	eval(formObj.document.getElementById("alloc_qty"+i).innerText);
...
function controlAllocationButton(obj) {
   if(parent.f_stage==undefined){
	...
	for(i=1; i<tot_cnt; i++) {
		if(eval(formObj.document.getElementById("alloc_qty"+i))!=undefined) {
			val	=	eval(formObj.document.getElementById("alloc_qty"+i).innerText");
			if(eval(formObj.document.getElementById("hold_remarks_"+i))!=undefined){
				hld_val	=	eval(formObj.document.getElementById("hold_remarks_"+i).innerText");
			}
			if(!isNaN(parseInt(trimString(val))) || hld_val==on_hold_legend){
				found	= true;
...
function showDrugInfo(external_product_id,drug_desc){
	var dialogHeight	="85" ;
	var dialogWidth	="70";
	var dialogTop		= "110" ;
	var dialogLeft		= "70" ;
	...
	var features		= "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments		= "";
	var retVal			= window.showModalDialog("../../ePH/jsp/DisplayDrugInfoFrames.jsp?extprodid="+external_product_id+"&drugdesc="+drug_desc+"&JSFile=../../ePH/js/Prescription_1.js",arguments,features);
}
...
function callFillingRemarks(obj,order_id,drug_code,recno,order_line_no,chk,id){	// code added by senthil kumar j on 16/4/2004
	var dialogHeight	= "13" ;
	var dialogWidth		= "31";
	var dialogTop		= "300" ;
	var dialogLeft		= "350" ;
	...
	var arguments	= "";
	if(chk=="F" ){
		retVal = window.showModalDialog("../../ePH/jsp/DispMedicationFillingRemarks.jsp?title="+getLabel("ePH.FillingStageFindings.label","PH")+"&chk="+chk+"&order_id="+order_id+"&drug_code="+drug_code+"&recno="+recno+"&order_line_no="+order_line_no+"&obj="+obj,arguments,features);
	}
	else if(chk=="D" || chk=="A"){
		var title="";
		if(chk=="A"){
		...
		retVal = window.showModalDialog("../../ePH/jsp/DispMedicationFillingRemarks.jsp?title="+title+"&chk="+chk+"&order_id="+order_id+"&drug_code="+drug_code+"&recno="+recno+"&order_line_no="+order_line_no+"&obj="+obj,arguments,features);
	}
	if(retVal=="OK") {
		changeColor(obj);
	} 
...
function callFillVal(frmObj,obj){	
	if(frmObj.remarks.value.length < 500){
		if(frmObj.chk.value=="O"){
			validateAmendReason(frmObj);
		}
		else{
			validateFillRemarks(frmObj);
		}
	}
	else{
...
var click = 1;
function callEditDetails(obj,order_id,drug_code,recno,order_line_no,chk,id,patientid,order_date,pres_by,drug_code,drug_name,alt_drug_flag,pres_qty,uom){	// added alt_drug_flag for ML-BRU-SCF-0179 [IN:032578]
...
function validateEditDetails(frmObj,calledFrom) {
	...
	var noOfDrugs = frmObj.document.DispMedicationEditLabel.totalRecordds.value;
	var allow_edit_disp_label = frmObj.document.DispMedicationEditLabel.allow_edit_disp_label.value;//Added for Bru-HIMS-CRF-414 [IN045554]
	var IncludeZeroQtyForIP=frmObj.document.DispMedicationEditLabel.IncludeZeroQtyForIP.value;//Added for ML-BRU-CRF-0473
	var IncludeZeroQtyForOP=frmObj.document.DispMedicationEditLabel.IncludeZeroQtyForOP.value;//Added for ML-BRU-CRF-0473
	var patient_class=frmObj.document.DispMedicationEditLabel.patient_class.value;//Added for ML-BRU-CRF-0473
...
function callAmendReasonRemarks(obj,order_id,drug_code,order_line_no,chk,patient_id,encounter_id,locn_type,locn_code,patientclass,iv_prep_yn){	
   if(iv_prep_yn==null || iv_prep_yn=="null")
	...
	var arguments		= "";
	if(chk=="O" ){
		retVal = window.showModalDialog("../../ePH/jsp/DispMedicationFillingRemarks.jsp?title="+getLabel("ePH.ReasonforAmend.label","PH")+"&chk="+chk+"&order_id="+order_id+"&drug_code="+drug_code+"&order_line_no="+order_line_no+"&patient_id="+patient_id+"&encounter_id="+encounter_id,arguments,features);
	}
	...
	if(retVal=="OK"){
		if(iv_prep_yn == '-1')
			iv_prep_yn = '';
		callAmmendFunctions1('Amend','','',order_id,'',order_line_no,drug_code,'',patient_id,encounter_id,'',locn_type,locn_code,patientclass,'PH','','','','','','',iv_prep_yn);
...
function viewInteraction(patientid,orderid,encounterid){
	var dialogHeight	="18" ;
	var dialogWidth	="40";
	var dialogTop	= "270" ;
	var dialogLeft	= "200" ;
	...
	var features	= "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "";
	retVal = window.showModalDialog("../../ePH/jsp/DispDisplayInteractions.jsp",arguments,features);
}
...
function displayConst(nRow){
	...
	var status		= "no";
	var features	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	window.showModalDialog("../../ePH/jsp/TPNRegStdRegConsFrame.jsp?RegimenCode="+vDrugVal,arguments,features);
}
...
function showTPNDetails(order_id){
	...
	var title           ="aaaa";
 	...
 	window.showModalDialog("../../ePH/jsp/TPNConstituentDetails.jsp?order_id="+order_id,arguments,features);
...
function displayInstructions(order_id,order_line_no,dialogheight,drug_name,from_disp){
	...
	var retVal=window.showModalDialog("../../ePH/jsp/InstructionsFrame.jsp?order_id="+order_id+"&order_line_no="+order_line_no+"&drug_name="+drug_name+"&from_disp="+from_disp,arguments,features);
}
...
function showchargedetails(message){
	if(message.length > 0){
		var insind			 =	"";
		var baseqty			 =	"";
		var baserate		 =	"";
...
function callverifyremarks(order_id,order_line_no){
	...
	var features	= "dialogHeight:" + dialogHeight +"; dialogLeft"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "";
	retVal = window.showModalDialog("../../ePH/jsp/DispMedicationVerficationRemarks.jsp?&order_id="+order_id+"&order_line_no="+order_line_no,arguments,features);
...
function callIncludeExclude(called_from,recnum,drug_code,qty,batch_str,order_id,order_line_no){
...
function showImage(drug_code, trade_code){
	var dialogHeight= "30" ;
	var dialogWidth	= "50" ;
	var dialogTop = "111" ;
	var center = "1" ;
	...
	var arguments	= "" ;
	var retVal = window.showModalDialog("../../ePH/jsp/DrugTradeImage.jsp?drug_code="+drug_code+"&trade_code="+trade_code,arguments,features);
}
...
//-- For issue qty should be the multiples of def UOM ---- end -----
	...
	setQtyBsdOnIssueUOMinFrontScreen(obj,recCount,conv_factor,"FROMBATCH");	//Added for to revert the display of qty interms of issue UOM when chaning issue UOM failed
  	if(parseInt(alloc_qty)==0 ){
		...
		else if(eval(document.getElementById("divIncludeZeroQty"+recCount))!= undefined && eval(q"document.DispMedicationAllStages.IncludeZeroQty").value== 'Y'){ // Added for [IN:045700]
...
function callbatchdetails(patient_id,order_id,order_line_no,drug_code,alt_drug_yn,drug_name){
	...
	var features	    = "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop+ ";scroll "+scroll;
	var arguments	    = "";
	retVal          = window.showModalDialog("../../ePH/jsp/BatchDetails.jsp?order_id="+order_id+"&order_line_no="+order_line_no+"&drug_code="+drug_code+"&alt_drug_yn="+alt_drug_yn+"&patient_id="+patient_id+"&drug_name="+drug_name ,arguments,features);
}

function CallPreparationChargeDetails(gross_admx_prep_charge,groos_pat_payable_admx_prep_charge,obj3,obj4,disp_stage){
	...
	var features	    = "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop+ ";scroll "+scroll;
	var arguments	    = "";
	retVal          = window.showModalDialog("../../ePH/jsp/AdmixturePreparationChargeDetails.jsp?gross_admx_prep_charge="+gross_admx_prep_charge+"&groos_pat_payable_admx_prep_charge="+groos_pat_payable_admx_prep_charge+"&MarkupDown="+obj3.value+"&BL_decimal_format_str="+escape(obj4.value)+"&disp_stage="+disp_stage,arguments,features);
...
function callMedicalItemdetails(patient_id,encounter_id,disp_stage,billing_interface_yn,disp_locn_code,bl_patient_class,orderIDS_trxRefIDS,sysdate,store_desc){
	var dialogHeight	= "30" ;
	var dialogWidth	    = "90";
	var dialogTop	    = "650" ;
	var dialogLeft	    = "30" ;
 ...
 function showDrug(drug_code){
	...
	var retVal = window.showModalDialog("../../ePH/jsp/ViewDrugInfo.jsp?DrugCode="+drug_code,arguments,features);
}
...
function loadPage(obj){
	...
	var status = "no" ;
	var drug_code = obj;
	var dialogUrl		= "../../ePH/jsp/DrugDetailFrames.jsp?drug_code="+drug_code;
	var dialogFeatures	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth +" ; scroll=no; status:" + status;
	window.showModalDialog(dialogUrl,"",dialogFeatures);	
}
	...
	for(i=0;i<totalRecords;i++){
		...
		if(issueUom_code_value != ""){
			...
			if(base_to_def_uom_equl_val!=base_to_disp_uom_equl_val){
				...
				issue_qty_val				= Math.ceil(issue_btch_qty*base_to_disp_uom_equl_val);
				var conv_factor				= lcm(iss_qty_eq_val, iss_qty_eq_val);
				conv_factor					= lcm(conv_factor, base_to_disp_uom_equl_val);
				issue_qty_val_2				= Math.ceil(((Math.ceil(issue_qty_val/conv_factor))*(conv_factor))/base_to_disp_uom_equl_val);
				issue_qty.value				= issue_qty_val_2; 
				issue_qty_val_1				= Math.ceil(((Math.ceil(issue_qty_val/conv_factor))*(conv_factor))/act_stk_equval_value);
...
function setQtyBsdOnIssueUOM(obj,no){
	...
	avail_qty	   =parseInt(eval("document.DispMedicationAllocateDrugsWithStock.alloc_qty"+no+".value"));
	if(issueUom_code_value != ""){
        	var conv_factor =lcm(base_to_def_uom_equl_val, act_stk_equval_value); //iss_qty_eq_val changed act_stk_equval_value for 
	    	conv_factor     =lcm(conv_factor, base_to_disp_uom_equl_val);
	     	if(base_to_disp_uom_equl_val!=act_stk_equval_value){ //Commented for  MMS-SCF-0040 [IN:041888] -Start  -- uncommented for the same issue - on change of Uissue OM
...
function calculateIssueUOM(obj,i,key,updateStock,approval_no){//approval_no added for AAKH-CRF-0117
	...
	issue_uom_lbl  =  eval(document.getElementById("issue_uom_lb"+i));
	issue_uom_qty  =  eval("formObj.issue_uom_qty"+i);
	issue_uom_uom  =  eval("formObj.issue_uom_uom"+i);
	issue_uom_desc =  eval("formObj.issue_uom_desc"+i);
	issue_uom_equl =  eval("formObj.issue_uom_equl"+i);
...
function viewTokenDetails(patientId){
	...
	window.showModalDialog("../../ePH/jsp/DispMedicationTokenDeatilsl.jsp?patient_id="+patientId,arguments,features);	
}
...
function includeToBms(obj,recCount,act_bms_qty){//KAUH-SCF-0096 [IN:035355]. Added Prescribed Quantiy parameter.
	if(obj.value == '' || parseInt(obj.value) == 0){
		...
		if(document.DispMedicationAllStages.patient_class!= undefined && document.DispMedicationAllStages.patient_class.value=="IP"){//Adding start for ML-BRU-CRF-0473
			if(eval("document.DispMedicationAllStages.IncludeZeroQtyForIP")!=undefined && eval("document.DispMedicationAllStages.IncludeZeroQtyForIP").value== 'Y'){
               if(eval(document.getElementById("alloc_bms_chk_"+recCount))!=undefined){
				   eval("document.DispMedicationAllStages.alloc_bms_chk_"+recCount).checked=true;
                   eval("document.DispMedicationAllStages.alloc_bms_chk_"+recCount).value= 'Y';
...
function openEditLabel(patientId,mode, patient_class){
	...
	var features	= "dialogHeight:" + dialogHeight +"; dialogLeft"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "";
	retVal = window.showModalDialog("../../ePH/jsp/DispMedicationEditLabelFrame.jsp?patientid="+patientId+"&mode="+mode+"&patient_class="+patient_class,arguments,features);
...
function dispDrugAuthCheck(checkYN){
	if(document.getElementById("userAuthPINYN").value=='Y'){   //RUT-CRF-0035 [IN029926] -- begin
		...
		var retVal=window.showModalDialog("../../ePH/jsp/UserPINAuthorization.jsp?call_from="+stage+"&disp_category="+disp_locn_catg+"&disp_locn_code="+disp_locn_code,"",features);
...
function callADRdetails(patient_id,drug_code){//newly added for RUT-SCF-0144 [IN:033371]
	var dialogHeight= "60" ;
	var dialogWidth	= "60" ;
	var dialogTop = "305" ;
	var dialogLeft = "180" ;
	...
	var arguments	= "" ;
	var retVal = window.showModalDialog("../../ePH/jsp/AdrReportingFrame_1.jsp?type=query "+"&status=S"+"&drug_code="+drug_code+"&patient_id="+patient_id ,arguments,features);
}
...
function GetLookup(target, call_from){//RUT-CRF-0035 [IN029926] Added for PIN Validation  - begin
	var sql="";			
...
function openAltDrugRemarks(order_id, order_line_num, alt_drug_remarks_ind,callFrom ){// Added For Bru-HIMS-CRF-082 [IN:029948]-start
	var dialogHeight= "15" ;
	var dialogWidth	= "40" ;
	var dialogTop = "195" ;
	var center = "1" ;
	...
	var features	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "" ;
	retRem = window.showModalDialog("../../ePH/jsp/DispMedicationRemarks.jsp?alt_drug_remarks_ind="+alt_drug_remarks_ind+"&order_id="+order_id+"&order_line_num="+order_line_num,arguments,features);
	if(callFrom!='LINK'){
...
function displayPayeeIndicator(display_yn){ //function added for Bru-HIMS-CRF-094-DD1 [IN:047224]
	var lblpayingPatient = parent.parent.frames[1].patient_details.frmPatientDetails.document.getElementById("payingPatient");
	if(display_yn=='Y')
...
function callMedicationPlanner(patient_id,encounter_id,med_plan_check,mediplan_seq_no){//Added for Bru-HIMS-CRF-072.1[IN 049144] start
	var dialogHeight	= "30" ;
	var dialogWidth	    = "80";
	var dialogTop	    = "150" ;
	var dialogLeft	    = "30" ;
	...
	var features	    = "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop+ ";scroll "+scroll;
	var arguments	    = "";
	retVal          = window.showModalDialog("../../ePH/jsp/MedicationPlannerFrame.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&med_plan_check="+med_plan_check+"&mediplan_seq_no="+mediplan_seq_no,arguments,features);
}
function callActiveOrders_Plan(patient_id,encounter_id,generic_id,patient_class,med_plan_check)	{
	...
	var features	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "" ;
	retVal = window.showModalDialog("../../ePH/jsp/PrescriptionActiveOrderDetailsFrame.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&generic_id="+generic_id+"&called_frm=PH"+"&patient_class="+patient_class+"&called_from=MediPlan",arguments,features);
	if(retVal=='OK'){
...
function recordMediPlan(called_from,mediplan_seq_no){
	...
	if(curr_drug_count){
		...
		if(called_from=="printDialog"){
			updateMedPlan(mediplan_seq_no,eval("formObj.patient_id").value,eval("formObj.encounter_id").value);
		}
			...
			var features	    = "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop+ ";scroll "+scroll;
			var arguments	    = "";
			retVal          = window.showModalDialog("../../ePH/jsp/MedicationPlannerPrint.jsp?patient_id="+formObj.patient_id.value,arguments,features);
		}
		window.close();
	}
	else{
...
function callMedplanCheck(formObj,patient_id,patient_class){
	if(formObj.med_plan!=null || formObj.med_plan!=undefined){
		if(formObj.med_plan.checked==true && patient_class=="OP")
			callMedicationPlanner(patient_id,'','Y', ''); // ,'' ML-BRU-SCF-1490 [IN:052770]
	}
}
function callMergeMedPlan(patient_id,encounter_id,med_plan_id_max,med_plan_check){
	var sel_lang = parent.f_medication_plan.document.formMedicationPlanner.sel_Language.value;
	var dialogHeight= "25" ;
	var dialogWidth	= "75" ;
	var dialogTop = "200" ;
	...
	var features	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "" ;
	retVal = window.showModalDialog("../../ePH/jsp/MedicationPlannerMerge.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&med_plan_id_max="+med_plan_id_max+"&sel_lang="+sel_lang,arguments,features);
	if(retVal=='OK'){
		parent.f_medication_plan.document.location.href="../../ePH/jsp/MedicationPlanner.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&med_plan_check="+med_plan_check+"&sel_lang="+sel_lang;
	}
}
...
function amendAltDrug(order_id,order_line_no,drug_code,patient_id,encounter_id,locn_type,locn_code,patientclass,iv_prep_yn){ // Added for JD-CRF-0198 [IN058599] - Start              	 
	if(iv_prep_yn==null || iv_prep_yn=="null")
          iv_prep_yn="";
...
function showDispInractionDtl(patient_id,encounter_id,drug_code,form_code,generic_id,start_date,end_date,drug_name,form_desc,order_id,order_line_no,allergy_cnt,patient_class,disp_medication){ // Added for  MMS-KH-CRF-0013 [IN:067952] - Start	
	//order_id,order_line_no,allergy_cnt,patient_class,disp_medication addded for IN070605
	var retVal;
	var dialogTop		= "170";
 	var dialogHeight	= "32" ;
...
//Adding start for MMS-DM-CRF-0157.1
function callManualScanRemarks(patient_id,encounter_id,disp_stage,order_id,order_line_no,item_code,batch_id,act_patient_class,remarks_required_yn){
...
function displayAlternateHeaderBarcode(rec_no,classvalue,barcode_flg){
	...
	if(barcode_flg=="Y")
	{	
	 cell1.innerHTML ="<font style='font-size:10;vertical-align: text-top;'>Barcode</font> <input type='text class='fields' name='barcode_id_' id='barcode_id_'"+rec_no+" onkeydown='if (event.keyCode==13) {}' maxlength='50'></td>";
	}
...
function validateEmptyCheck(order_id,order_line_no,patient_id,disp_stage,store_code,end_date){
		valid_flag = true;
		...
		if(!valid_flag){
			...
			setAlterDrugDetail(order_id,order_line_no,patient_id,alloc_qty,alt_drug_code,form_code,strength_value,strength_uom,alt_drug_desc,store_code,end_date);
			if(disp_stage=="A"){
			parent.f_disp_medication_allocation.location.href="../../ePH/jsp/DispMedicationAllocation.jsp?patient_id="+document.DispMedicationAllStages.patient_id.value+"&encounter_id=DUMMYENCOUNTERID&order_id="+document.DispMedicationAllStages.order_id.value+"&resetQty=true";
			}else if(disp_stage=="F"){
				parent.f_disp_medication_filling.location.href="../../ePH/jsp/DispMedicationFilling.jsp?patient_id="+patient_id+"&encounter_id=DUMMYENCOUNTERID&order_id="+order_id;
...
function setAlterDrugDetail(order_id,order_line_no,patient_id,alloc_qty,alt_drug_code,form_code,strength_value,strength_uom,alt_drug_desc,store_code,end_date){
...
function enableAllocQty(obj,order_id,order_line_no,alt_drug_code,cunt){
	if(obj.checked==true){
...
function displayAlternateDrugDtls(drug_name,alt_drug_code,fmtQty,order_id,order_line_no,cunt,recCount,strength_value, strength_uom,tot_alt_drug_qty,form_code,strength_uom,classvalue,allow_more_qty){
...
function formularyNonPreference(drug_code,patient_id,encounter_id){
		...
		var arguments	= "" ;
		retVal = window.showModalDialog("../../ePH/jsp/PhNonPreferableRemarks.jsp?item_code="+drug_code+"&patient_id="+patient_id+"&encounter_id="+encounter_id+"&called_from=A",arguments,features);
...
function saveNonPreRemForDispAlternate(drug_code,patient_id,encounter_id){
