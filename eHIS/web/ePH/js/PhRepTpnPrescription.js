...
function reset() {
	...
	f_query_add_mod.frmTpnPrescriptionQueryCriteria.reset();
    f_query_add_mod.frmTpnPrescriptionQueryCriteria.btnPatientID.disabled=false;
...
function callPatientSearch(){
	...
	var formObj=document.frames.frmTpnPrescriptionQueryCriteria;
...
if(pat_id=="") {
	...
	if(patient_id !="" || orig_order_id != ""){
		if(iv_type!="" ){
		parent.messageFrame.location.href="../../eCommon/jsp/error.jsp?err_num=";
		parent.f_query_results.location.href="../../ePH/jsp/PhRepTpnPrescripResult.jsp?patient_id="+patient_id+"&orig_order_id="+orig_order_id+"&order_date_from="+order_date_from+"&encounter_id="+encounter_id+"&pres_practitioner_id="+pres_practitioner_id+"&locn_type="+locn_type+"&locn_code"+locn_code+"&iv_type="+iv_type;
		parent.f_query_status.location.href="../../ePH/jsp/PhRepPrintPrescriptionStatus.jsp";
		}else{
			parent.messageFrame.document.location.href="../../eCommon/jsp/error.jsp?err_num="+getMessage("TPN_TYPE_CANT_BE_BLANk","PH") ;
		}
	}else{
	...
	var locn_code=formObj.locn_code.value;
		...
		parent.f_query_results.location.href="../../ePH/jsp/PhRepTpnPrescriptionResult.jsp?patient_id="+patient_id+"&orig_order_id="+orig_order_id+"&order_date_from="+order_date_from+"&encounter_id="+encounter_id+"&pres_practitioner_id="+pres_practitioner_id+"&locn_type="+locn_type+"&locn_code"+locn_code;
		parent.f_query_status.location.href="../../ePH/jsp/PhRepPrintPrescriptionStatus.jsp";
	}
}

function SubmitLink(from,to,patient_id,orig_order_id,order_date_from,order_date_to,encounter_id,pres_practitioner_id){
	var obj=document.frames.frmTpnPrescriptionReportResult;
	obj.from.value=from;
	obj.to.value=to;
...
async function showReportPrescription(order_id,patient_id,encounter_id,patient_class,locn_code,iv_type){
	...
	var arguments	= "" ;
	var retVal = await window.showModalDialog("../../ePH/jsp/PhRepPrintIVPrescriptionWindow.jsp?order_id="+order_id+"&patient_id="+patient_id+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&locn_code="+locn_code+"&iv_type="+iv_type,arguments,features);
}
...
function showPrintReport(order_id,patient_class,locn_code,iv_type){
	...
	var formObj = parent.f_query_add_mod.frmTpnPrescriptionQueryCriteria;
	...
	parent.f_query_add_mod.frmTpnPrescriptionQueryCriteria.submit();
