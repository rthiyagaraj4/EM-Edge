...
function validateQtyDesc(mode,status){
	...
	if(mode=="S"){
		document.prescription_detail_form.dosage_label.innerText=getLabel("Common.Strength.label",'Common');	
		document.prescription_detail_form.qty_desc.disabled	=	false;
		if(status!="R") {
		...
		document.prescription_detail_form.dosage_label.innerText=getLabel("ePH.Dose.label",'PH');	
		if(parent.f_header==null) {
		frameobj	=	parent.parent.f_header;
		} 
		else {
	...
	//else if(mode=="A" && document.prescription_detail_form.patient_class.value=="IP" && document.prescription_detail_form.freq_nature.value=="P") {
		document.prescription_detail_form.dosage_label.innerText=getLabel("Common.Quantity.label",'Common');	
	}
	if(document.prescription_detail_form.sliding_scale_yn.value == 'Y') {
		document.prescription_detail_form.dosage_label.innerText="";
	}
	if(document.prescription_detail_form.qty_desc.value== ''){
...
//		document.prescription_detail_form.durn_desc.disabled=true;
	...
	if(formObject.alternateOrder.value=="Y"){//added for  JD-CRF-0198 [IN:058599] parent.f_header.document.prescription_header_form replaed with formObject for ARYU-SCF-0033
		document.prescription_detail_form.allow_refill.disabled=true;
...
function assignRefillDates(refill_start,refill_end){
	...
	parent.refill_detail.location.href="../../ePH/jsp/PrescriptionRefillDetails.jsp?bean_id="+bean_id+"&bean_name="+bean_name+"&presBean_id="+presBean_id+"&presBean_name="+presBean_name+"&end_date="+end_date+"&repeat_durn_type="+repeat_durn_type+"&durn_value="+durn_value+"&refill_no="+refill_no+"&max_refills="+max_refills+"&max_durn_for_refill_pres="+max_durn_for_refill_pres+"&srl_no="+srl_no;
}
...
function loadRefillDetails(max_refills){
	var end_date			=	document.prescription_detail_form.end_date.value;
	...
	if(end_date!=""){
		...
		var arguments	= "" ;
		retVal = window.showModalDialog("../../ePH/jsp/PrescriptionRefillDetailsFrame.jsp?bean_id="+bean_id+"&bean_name="+bean_name+"&presBean_id="+presBean_id+"&presBean_name="+presBean_name+"&end_date="+end_date+"&repeat_durn_type="+repeat_durn_type+"&durn_value="+durn_value+"&refill_no="+refill_no+"&max_refills="+max_refills+"&max_durn_for_refill_pres="+max_durn_for_refill_pres+"&srl_no="+srl_no,arguments,features);
...
function callOrderRemarks(){
	...
	var features		= "dialogLeft:" + dialogLeft + ";dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + ";scroll "+scroll+"; status: " + status + "; dialogTop :" + dialogTop;
	var arguments		= "" ;
	retVal = window.showModalDialog("../../ePH/jsp/PrescriptionRemarksFrame.jsp?order_remarks="+order_remarks+"&bean_id="+bean_id+"&bean_name="+bean_name,arguments,features);
	if(retVal!=null)
...
function validateRemarks(format_id)	{
	...
	document.prescription_detail_form.DrugRemarksMandImg.style.display = 'none';
	document.prescription_detail_form.DrugRemarksMandatory.value =mandatory;
	if(format == null || format=='' || formatId==null || formatId==''){
		document.prescription_detail_form.drug_remarks.style.display = 'none';
	}
	else{
		document.prescription_detail_form.drug_remarks.style.display = 'inline';
		if(mandatory == 'Y'){
			document.prescription_detail_form.DrugRemarksMandImg.style.display = 'inline';
		}
	}
...
function callRemarks(format_id,catalog_code,catalog_codeSRLNO,order_category,row_value,called_from)	{
	...
	var arguments	= "" ;
	retVal			= window.showModalDialog("../../eOR/jsp/OrderEntryOrderFormatFrameSet.jsp?bean_id="+bean_id+"&bean_name="+bean_name+"&format_id="+format_id+"&p_catalog_code="+catalog_code+"&catalog_code="+catalog_codeSRLNO+"&p_form_code="+form_code+"&p_route_code="+route_code+"&order_category="+order_category+"&row_value=0&called_from=PH"+"&order_id="+order_id+"&order_line_num="+order_line_no,arguments,features);//order_id & order_line_no Added for ML-BRU-SCF-1333 [IN:049124]
	if(retVal!=undefined ){
...
function callAmendRemarks(format_id,catalog_code,catalog_codeSRLNO,order_category,row_value,called_from,order_id,order_line_no)    {
   	patient_id		=document.prescription_detail_form.patient_id.value;
	encounter_id	=document.prescription_detail_form.encounter_id.value;
	srl_no			=document.prescription_detail_form.srl_no.value;
	sliding_scale_yn	=document.prescription_detail_form.sliding_scale_yn.value;
...
function callSchedule(patient_id,encounter_id,bean_id,bean_name,drug_code,srl_no,freq_code,row_value,facility_id,max_qty,unit_qty,split_dose_yn,split_qty,qty_reqd_yn,called_from,uom_desc) {
	var chk_max			= "N";
	var chk_unit		= "N";
...
function makeScheduleLink(admin_time,admin_qty,total_qty,freq_size,split_chk,calling_mode){
	...
	document.prescription_detail_form.dosage_label.innerHTML="<label nowrap style='color:red;font-size:10' onmouseover='displayToolTipDataChange(\"" + title + "\");' onmouseout='disasbleToolTipDataChange();'>"+legend+"</label>";
...
function loadOtherFrames(params){
...
function defaultDrug(drug_code,mode,srl_no,sliding_scale_yn,recomm_yn,order_idC,modeC,parent_stat_yn,stat_yn){  //AAKH-CRF-0091
	...
	if(sliding_scale_yn=="Y" && (recomm_yn == '' || recomm_yn == 'N') ){
		...
		parent.parent.f_detail.location.href="../../ePH/jsp/PrescriptionDetail_1.jsp?srl_no="+srl_no+"&drug_code="+drug_code+"&priority="+priority+"&start_date="+start_date+"&mode="+mode+"&patient_id="+patient_id+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&bsa="+bsa+"&bed_num="+bed_num+"&room_num="+room_num+"&drug_db_interface_yn="+drug_db_interface_yn+"&bl_install_yn="+bl_install_yn+"&bl_disp_charge_dtl_in_rx_yn="+bl_disp_charge_dtl_in_rx_yn+"&MAR_app_yn="+MAR_app_yn+"&take_home_medication="+take_home_medication+"&order_idC="+order_idC+"&modeC="+modeC+"&trade_display_yn="+trade_display_yn+"&drug_db_allergy_check_yn="+drug_db_allergy_check_yn+"&drug_db_duptherapy_yn="+drug_db_duptherapy_yn+"&alternateOrder="+alternateOrder;//added for  JD-CRF-0198 [IN:058599]
		parent.parent.f_button.location.href="../../ePH/jsp/PrescriptionButton_1.jsp?patient_id="+patient_id+"&mode="+mode+"&encounter_id="+encounter_id+"&sliding_scale_yn="+sliding_scale_yn+"&bed_num="+bed_num+"&room_num="+room_num+"&drug_db_interface_yn="+drug_db_interface_yn+"&take_home_medication="+take_home_medication+"&parent_stat_yn="+parent_stat_yn+"&stat_yn="+stat_yn;   //AAKH-CRF-0091
	}
}
...
function drugSearch(drugCode){
	...
	retVal = DrugSearchForPres("D",document.prescription_detail_form.drug_name,order_type_code,drugCode,bean_id,service_search_yn,servide_code, patient_class,priority,discharge_yn,pat_id,encounter_id,date_of_birth,gender,source_type,source_code,"","","","",home_leave_yn_chk,alternateOrder,"Y");//added home_leave_yn_chk for Bru-HIMS-CRF-093[29960] -->//added alternateOrder for  JD-CRF-0198 [IN:058599] and 'Y' added for GHL-CRF-0549
...
function chkOverRideRemarks(){
	...
	if(parent.parent.f_detail.document.prescription_detail_form!=undefined){//SKR-SCF-0648 [IN-35984] - if condition and else block Added to check Override remarks on Sliding Scale. 
		if(parent.parent.f_detail.document.prescription_detail_form.dosage_limit.style.visibility=="visible"){ // added if condition for the incident no:MO-GN-5406(31896) by chandrashekar raju
			if((parent.parent.f_detail.document.prescription_detail_form.override_remarks.style.visibility=="visible") && (override_remarks == "N") ){
	...
	var xmlStr=formXMLString(formobj,fldString);
	fieldValidation(xmlStr,"chkOverRideRemarks");
}

function allowRelease(allow_1,allow_2,allow_3,allow_4,allow_5){
...
//added for GHL-CRF-0482 - end
	...
	if(can_allow_1 && can_allow_2){
		can_allow=true;
		if(parent.frames[2].document.forms[0].rx_button_disable.value=="true"){//Added for KDAH-SCF-0382
				...
				if(checkForDuplicate()){
					if(oper_mode=="taper" || oper_mode=="tapered" || oper_mode =='delete'){
						...
						if(refresh_screen){
							parent.f_prescription.location.href="../../ePH/jsp/PrescriptionFormFrame.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&bed_num="+bed_num+"&room_num="+room_num+"&weight="+weight+"&bsa="+bsa+"&drug_db_interface_yn="+drug_db_interface_yn+"&bl_install_yn="+bl_install_yn+"&bl_disp_charge_dtl_in_rx_yn="+bl_disp_charge_dtl_in_rx_yn+"&MAR_app_yn="+MAR_app_yn+"&adr_count="+adr_count+"&act_patient_class="+act_patient_class+"&take_home_medication="+take_home_medication+"&order_set_code="+order_set_code+"&drug_db_allergy_check_yn="+drug_db_allergy_check_yn+"&drug_db_duptherapy_yn="+drug_db_duptherapy_yn+"&conf_drug_code="+drug_code+"&conf_srl_no="+srl_no;
							var mode="blank";
							parent.f_detail.location.href="../../ePH/jsp/PrescriptionDetail_1.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&mode="+mode+"&start_date="+order_date_time+"&bed_num="+bed_num+"&room_num="+room_num+"&drug_db_interface_yn="+drug_db_interface_yn+"&bl_install_yn="+bl_install_yn+"&bl_disp_charge_dtl_in_rx_yn="+bl_disp_charge_dtl_in_rx_yn+"&MAR_app_yn="+MAR_app_yn+"&act_patient_class="+act_patient_class+"&take_home_medication="+take_home_medication+"&trade_display_yn="+trade_display_yn+"&drug_db_allergy_check_yn="+drug_db_allergy_check_yn+"&drug_db_duptherapy_yn="+drug_db_duptherapy_yn+"&alternateOrder="+alternateOrder;//added for  JD-CRF-0198 [IN:058599]
							parent.f_button.location.href="../../ePH/jsp/PrescriptionButton_1.jsp?patient_id="+patient_id+"&mode="+mode+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&bed_num="+bed_num+"&room_num="+room_num+"&drug_db_interface_yn="+drug_db_interface_yn+"&act_patient_class="+act_patient_class+"&take_home_medication="+take_home_medication;
						}
					}
				}
...
function enableDosageLimit(limit_ind,daily_dose,unit_dose,mono_graph, min_daily_dose, min_unit_dose,dose_unit, srl_no,dose_unit_desc){  // dose_unit_desc added for ML-MMOH-SCF-0633 [IN:063271]
	// srl_no in parameter list added for	SRR20056-SCF-7690 IN 27798 & 28440 - Start
	var drug_desc ='';
	var dosage_std='';
	var dose_override = ''
//added for	SRR20056-SCF-7690 IN 27798 & 28440 - End
	if(document.prescription_detail_form.dosage_limit==null) {
		...
		//frmObj.dosage_limit.style.display="inline";
...
//added for	SRR20056-SCF-7690 IN 27798 & 28440 - start
	...
	dosagemit_ind.onclick = new Function( "showMonoGraph('"+drug_desc+"','"+mono_graph+"','"+daily_dose+"','"+unit_dose+"','"+min_daily_dose+"','"+min_unit_dose+"','"+dose_unit+"','"+dosage_std+"','"+dose_unit_desc+"');"); // dose_unit_desc Added for ML-MMOH-SCF-0633 [IN:063271]
	if(dose_override!=null && dose_override=="N")
...
function disableDosageLimit(srl_no,drug_code){
	try{
		...
		//var dosagemit_ind = frmObjform_1.document.getElementById('td_dosage_override'+srl_no);
		var drugDescTD = eval('frmObjform_1.document.getElementById("drug_")'+drug_code+'_'+srl_no);
...
function showMonoGraph(drug_desc,mono_graph,daily_dose,unit_dose,min_daily_dose,min_unit_dose,dosage_unit, dosage_std,dose_unit_desc){ // dose_unit_desc Added for ML-MMOH-SCF-0633 [IN:063271]
	var retVal;
	var dialogTop		= "310";
 	var dialogHeight	= "17" ;
 	var dialogWidth		= "36" ;
	var dialogLeft		= "180" ;
 	...
 	retVal 				=    window.showModalDialog("../../ePH/jsp/PrescriptionDosageLimitMonoGraph.jsp?drug_desc="+decodeURIComponent(drug_desc)+"&mono_graph="+mono_graph+"&daily_dose="+daily_dose+"&unit_dose="+unit_dose+"&min_daily_dose="+min_daily_dose+"&min_unit_dose="+min_unit_dose+"&dosage_unit="+dosage_unit+"&dosage_std="+dosage_std+"&dose_unit_desc="+dose_unit_desc,arguments,features); // encodeURIComponent(drug_desc) changed to decodeURIComponent(drug_desc) and dose_unit_desc added for ML-MMOH-SCF-0633 [IN:063271]
}

function showAllergyDetails(generic_id,patient_id,encounter_id, generic_name, drug_name ){
	var retVal;
	var dialogTop		= "310";
 	var dialogHeight	= "12" ;
 	var dialogWidth		= "25" ;
	var dialogLeft		= "240" ;
 	...
 	var arguments		= "";
 	retVal 				= window.showModalDialog("../../ePH/jsp/PrescriptionAllergyDetailsFrames.jsp?generic_name="+generic_name+"&encounter_id="+encounter_id+"&generic_id="+generic_id+"&patient_id="+patient_id+"&drug_name="+ drug_name,arguments,features);
}

function showMonoGraphDetail(){
	...
	var dialogLeft		= "180" ;
 	...
 	var arguments		= "";
 	retVal 				= window.showModalDialog("../../ePH/jsp/PrescriptionDosageLimitMonoGraph.jsp?drug_desc="+drug_desc+"&mono_graph="+mono_graph+"&daily_dose="+daily_dose+"&unit_dose="+unit_dose,arguments,features);
}
...
function reloadPage(print_ord_sht_rule_ind){//print_ord_sht_rule_ind,Added for Bru-HIMS-CRF-393_1.0
...
function loadDrugInfo(prodid,drugdesc){
	var dialogHeight	="85" ;
	var dialogWidth	="70";
	var dialogTop	= "110" ;
	var dialogLeft	= "80" ;
	...
	var features	= "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "";
	retVal = window.showModalDialog("../../ePH/jsp/DisplayDrugInfoFrames.jsp?extprodid="+prodid+"&drugdesc="+drugdesc+"&JSFile=../../ePH/js/Prescription_1.js",arguments,features);
}
...
function viewInteraction(patientid,orderid,encounterid){
	...
	var features	= "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "";
	retVal = window.showModalDialog("../../ePH/jsp/DisplayInteractionsFrames.jsp?patientid="+patientid+"&encounterid="+encounterid+"&orderid="+orderid+"&JSFile=../../ePH/js/Prescription_1.js",arguments,features);
}
...
function showDrugProfile(patient_id,called_frm,encounter_id){ // encounter_id added for AAKH-CRF-0088.2
	...
	var dialogHeight	= "40";//"36" ;
	var dialogWidth		= "100";//"60" ;
	var dialogTop		= "100";//"152" ;
	var center			= "1" ;
	var status			= "no";
	var features		= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center:" + center +"; status:" + status + "; dialogTop:" + dialogTop;
	var arguments		= "" ;
	retVal				= window.showModalDialog( "../../ePH/jsp/QueryPatientDrugProfileFrames.jsp?patient_id="+encodeURIComponent(patient_id)+"&pat_class="+pat_class+"&called_frm=Prescription"+"&encounter_id="+encounter_id, arguments, features ); // encounter_id added for AAKH-CRF-0088.2
		...
		if( document.prescription_detail_form.freq_nature.value=="P" || document.prescription_detail_form.freq_nature.value=="O" || document.prescription_detail_form.sliding_scale_yn.value == 'Y'){  /*(document.prescription_detail_form.dosage.value=="A" && (document.prescription_detail_form.patient_class.value=="OP" || document.prescription_detail_form.patient_class.value=="EM") ) || */ // 'A' & 'OP' or 'EM' condition commented for IN27197 --17/05/2011-- priya
			...
			if(document.prescription_detail_form.sliding_scale_yn.value == 'Y' ) {
				...
				if(document.prescription_detail_form.freq_nature.value!="O") {
					document.prescription_detail_form.absol_qty.style.display='inline';
					document.getElementById("lgd_ord_qty").style.display='inline';
					document.getElementById("lgd_qty_uom").style.display='inline';
					document.prescription_detail_form.btnSchedule.style.display='inline'
				}
			}
		}
		else {
...
function checkForPRN(obj) {
	if(document.prescription_detail_form.freq_nature.value=="P") {
		...
		if(document.prescription_detail_form.prn_dose!=null) {
			...
			document.prescription_detail_form.absol_qty.style.display ="inline";
			document.getElementById("ord_qty_lbl").style.display	= "inline";
			document.getElementById("ord_qty_uom").style.display	= "inline";
...
function showPreviousOrders(){
	...
	var features	= "dialogLeft:" + dialogLeft + ";dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + ";scroll "+scroll+"; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "" ;
	retVal = window.showModalDialog("../../ePH/jsp/PrescriptionPrevOrders.jsp?patient_class="+patient_class+"&pract_id="+pract_id+"&patient_id="+patient_id+"&encounter_id="+encounter_id+"&rx_type="+iv_prep_yn+"&pract_type="+pract_type+"&discharge_ind="+discharge_ind+"&call_from=COPYPREVORDERS",arguments,features);
...
function recommendDosage(drug_code,modify,srl_no,order_idC,modeC, called_from) { //frmObj, removed for Bru-HIMS-CRF-079 IN[029945]
	...
	if(srl_no!=null && srl_no!='null') 
		defaultDrug(drug_code,'modify',srl_no,'','',order_idC,modeC);
}
...
//				var mode="blank";
		...
		parent.parent.f_detail.location.href="../../ePH/jsp/PrescriptionDetail_1.jsp?srl_no="+srl_no+"&drug_code="+drug_code+"&priority="+priority+"&start_date="+start_date+"&mode="+mode+"&patient_id="+patient_id+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&bsa="+bsa+"&bed_num="+bed_num+"&room_num="+room_num+"&drug_db_interface_yn="+drug_db_interface_yn+"&bl_install_yn="+bl_install_yn+"&bl_disp_charge_dtl_in_rx_yn="+bl_disp_charge_dtl_in_rx_yn+"&MAR_app_yn="+MAR_app_yn+"&act_patient_class="+act_patient_class+"&take_home_medication="+take_home_medication+"&from_sliding_scale_yn=Y";
		parent.parent.f_button.location.href="../../ePH/jsp/PrescriptionButton_1.jsp?patient_id="+patient_id+"&mode="+mode+"&encounter_id="+encounter_id+"&sliding_scale_yn="+sliding_scale_yn+"&bed_num="+bed_num+"&room_num="+room_num+"&drug_db_interface_yn="+drug_db_interface_yn+"&act_patient_class="+act_patient_class+"&take_home_medication="+take_home_medication;
	}
}
...
function refreshSlidingScaleFrame(){
	...
	parent.f_prescription.location.href="../../ePH/jsp/PrescriptionFormFrame.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&bed_num="+bed_num+"&room_num="+room_num+"&weight="+weight+"&bsa="+bsa+"&drug_db_interface_yn="+drug_db_interface_yn+"&bl_install_yn="+bl_install_yn+"&bl_disp_charge_dtl_in_rx_yn="+bl_disp_charge_dtl_in_rx_yn+"&MAR_app_yn="+MAR_app_yn+"&adr_count="+adr_count+"&act_patient_class="+act_patient_class+"&take_home_medication="+take_home_medication+"&order_set_code="+order_set_code+"&drug_db_allergy_check_yn="+drug_db_allergy_check_yn+"&drug_db_duptherapy_yn="+drug_db_duptherapy_yn;
	var mode="blank";
	parent.f_detail.location.href="../../ePH/jsp/PrescriptionDetail_1.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&mode="+mode+"&start_date="+order_date_time+"&bed_num="+bed_num+"&room_num="+room_num+"&drug_db_interface_yn="+drug_db_interface_yn+"&bl_install_yn="+bl_install_yn+"&bl_disp_charge_dtl_in_rx_yn="+bl_disp_charge_dtl_in_rx_yn+"&MAR_app_yn="+MAR_app_yn+"&act_patient_class="+act_patient_class+"&take_home_medication="+take_home_medication+"&drug_db_allergy_check_yn="+drug_db_allergy_check_yn+"&drug_db_duptherapy_yn="+drug_db_duptherapy_yn;
	parent.f_button.location.href="../../ePH/jsp/PrescriptionButton_1.jsp?patient_id="+patient_id+"&mode="+mode+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&bed_num="+bed_num+"&room_num="+room_num+"&drug_db_interface_yn="+drug_db_interface_yn+"&act_patient_class="+act_patient_class+"&take_home_medication="+take_home_medication;
	if(top.content != null)
		top.content.messageFrame.location.href = "../../eCommon/jsp/error.jsp?err_num=";
}
...
function disableoverride(limit_ind){
	...
	if((limit_ind=='Y') &&(document.prescription_detail_form.allergy_yn.value =='N')&& (document.prescription_detail_form.current_rx.value == 'N') && interaction_exists!="Y"){
		document.prescription_detail_form.dosage_limit.style.visibility='hidden';
		//document.prescription_detail_form.dosage_limit.style.display="none";
		document.prescription_detail_form.override_remarks.style.visibility="hidden";
	}
	else{	 
      if((limit_ind=='N')||(document.prescription_detail_form.allergy_yn.value =='Y')|| (document.prescription_detail_form.current_rx.value == 'Y') || interaction_exists=="Y"){	 
        document.prescription_detail_form.dosage_limit.style.visibility='visible';
		//document.prescription_detail_form.dosage_limit.style.display="inline";		 
 		document.prescription_detail_form.override_remarks.style.visibility="visible";
	  }
...
function viewReasonforRequest(orderId,drug_code){
	...
	var retVals = window.showModalDialog("../../eOR/jsp/TDMReasonforrequest.jsp?"+finalString,arguments,features);
}
...
function displayDosageCheckResult(patient_id, pract_name, encounter_id, ext_prod_id, drug_code, srl_no, drug_db_dosage_check_flag, drug_db_duptherapy_flag, drug_db_contraind_check_flag, drug_db_interact_check_flag,  drug_db_allergy_flag, dsg_reason, dup_reason, con_reason, int_reason, alg_reason, callFrom){
...
function getChargeDetails(in_formulary_yn, called_from){
	...
	if(episode_type!='')
		episode_type					= episode_type.substring(0,1);
	var durn_value						=	document.prescription_detail_form.tmp_durn_value.value;//changed temp_drug_value to durn_value for MMS-DM-CRF-126.1 reverted for tbmc-scf-0051
...
/*	//commented By Naveen
		...
		if(document.prescription_detail_form.billable_item_yn.value == "N"){
			...
			//added for GHL-CRF-0508 start
		var default_freq_not_found =document.prescription_detail_form.default_freq_not_found.value;
	    var freq_code  =document.prescription_detail_form.frequency.value;
		...
		document.prescription_detail_form.bl_override_excl_incl_ind.style.display="inline";
...
function defDispLocnForIV(){
	if(document.prescription_header_form.MAR.value!='MAV'){
		try{
			if(document.getElementById("ivrx").className=="CASECONDSELECTHORZ"){
...
//	try{
		parent.f_prescription.f_ivbutton.document.formIVPrescriptionButtons.time_flag =time_flag;
		parent.f_prescription.f_ivbutton.document.getElementById("dflt_disp_locn_lbl").innerText = ord_disp_location;
//	}
//	catch(e){}
}
...
function callPregnancyRemarks(){
	...
	var features		= "dialogLeft:" + dialogLeft + ";dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + ";scroll "+scroll+"; status: " + status + "; dialogTop :" + dialogTop;
	var arguments		= "" ;
	window.showModalDialog("../../ePH/jsp/PregnancyRemarks.jsp",arguments,features);
}
...
function durationConversn(obj){
    ...
    if(document.prescription_detail_form.calling_mode!=undefined){ 
...
function getBuildMARDefaulting(){
	...
	var xmlHttp = new XMLHttpRequest() ;
		xmlDoc = new DOMParser().parseFromString(xmlStr, "text/xml");
		xmlHttp.open("POST","PrescriptionValidation_1.jsp?func_mode=getMARDefaulting",false);
		xmlHttp.send(xmlDoc);
		responseText=xmlHttp.responseText;
		eval(responseText);
	}
...
function callAllergies(patient_id){
	...
	retVal = window.showModalDialog("../../eCA/jsp/AllergyList.jsp?called_from_ip=Y&patient_id="+patient_id,arguments,features);
}
...
function showDuplicateInd(srlNo, currentrx, slidingScale_yn, drug_code, callFrom, allow_duplicate){ // allow_duplicate added for MO-GN-5490 [IN:054620]
	...
	if(callFrom == 'CHANGEDATE' || callFrom =='ChangeFreq' || callFrom =='DATECHANGE')// DATECHANGE added for MO-GN-5490 [IN:054620] - [IN:055921]
		formObj = document.prescription_detail_form;
	else{
		if(currentrx == 'R')
			formObj = parent.f_detail.document.prescription_detail_form;
		else
	...
	if(currentrx == 'Y' || currentrx == 'R'){
		if(formObj!=undefined){//SKR-SCF-0648 [IN-35984] - if condition Added to check Override remarks on Sliding Scale.
			...
			if(allow_duplicate!='Y'){ // if block and else condition added for MO-GN-5490 [IN:054620]
				...
				formObj.trDupDrugInfo.style.display="inline";
		...
		if(slidingScale_yn==undefined || slidingScale_yn=='N'){
			...
			if(ATC_Duplicate_yn!="Y" && doseOverride!="N" && allergy!="Y" && interaction_exists!="Y"){ //doseOverride changed to  != 'N'  for MO-GN-5490 [IN:054620]
...
function drugindication_remarks(){//Added for  ML-BRU-CRF-072[Inc:29938] start
	var formobj		=	document.prescription_detail_form;
	if(parent.f_header==null) {//Adding start for ARYU-SCF-0033
		var formObject	=	parent.parent.f_header.document.prescription_header_form;
	}
	...
	var arguments	= "" ;
	retVal = window.showModalDialog("../../ePH/jsp/DrugIndicationRemarks.jsp?&DrugIndicationRemarks="+encodeURIComponent(DrugIndicationRemarks)+"&drug_code="+drug_code+"&bean_id="+bean_id+"&bean_name="+bean_name,arguments,features);
...
function alertIntr(drug_code, srl_no, patient_id, encounter_id, intr_restric_trn){
	...
	var status		="no";
 	...
 	retVal 				=    window.showModalDialog("../../ePH/jsp/PrescriptionOverrideReasonFrameset.jsp?called_from=normal&calledFrom=INTRACTIONDTLS&drug_code="+drug_code+"&srl_no="+srl_no+"&patient_id="+patient_id+"&encounter_id="+encounter_id+"&calledFor=Alert&intr_restric_trn="+intr_restric_trn,arguments,features);
...
function showInractionDtl(drug_code, srl_no, patient_id, encounter_id, intr_restric_trn){
	var retVal;
	var dialogTop		= "170"; // dialogTop and dialogHeight modified for FD-RUT-CRF-0066.1 [IN036975]
 	var dialogHeight	= "32" ;
 	var dialogWidth		= "50" ;
	var dialogLeft		= "375" ;
 	var features		= 'Width:' + dialogWidth +'; dialogHeight:' + dialogHeight + '; dialogWidth:' + dialogWidth +'; dialogLeft:' + dialogLeft +';  dialogTop:'+dialogTop +'; scroll=auto; status=no';
 	var arguments		=getLabel("Common.DrugInteraction.label",'Common'); 
 	retVal 				=    window.showModalDialog("../../ePH/jsp/PrescriptionOverrideReasonFrameset.jsp?called_from=normal&calledFrom=INTRACTIONDTLS&drug_code="+drug_code+"&srl_no="+srl_no+"&patient_id="+patient_id+"&encounter_id="+encounter_id,arguments,features);
}
...
//added for CRF RUT-CRF-0063[29601]--start 
 function chkPregnencyAlert(){
	...
	if(pregnency_yn=="Y" &&  preg_effect!="" ){
		...
		var features	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop+"; dialogLeft:"+dialogLeft;
		var arguments	= "" ;
		retVal = window.showModalDialog("../../ePH/jsp/PregnencyDrugEffectAlert.jsp?pregnency_cat="+pregnency_cat+"&preg_effect="+encodeURIComponent(preg_effect)+"&pregnency_cat_desc="+pregnency_cat_desc+"&preg_remarks="+preg_remarks+"&drug_desc="+drug_desc+"&preg_category_code="+preg_category_code+"&trimester="+trimester,arguments,features);
		if(retVal!=undefined || retVal!=null){
			pregRemarks = retVal;
			if(document.prescription_detail_form!=undefined || document.prescription_detail_form!=null)
...
function assignpregdrug(preg_effect,preg_warn_req_yn,preg_category_code,trimester){
	...
	if(preg_effect!="" && preg_warn_req_yn=="Y"  ){
		document.prescription_detail_form.preg_alert.style.display = 'inline';
		chkPregnencyAlert();
	}
	else
...
function callWitnessAuth(){ //Added for AAKH-CRF-0023[IN:038259] start
	var hdrFormObj = parent.parent.f_header.document.prescription_header_form;
	var patient_id=hdrFormObj.patient_id.value;
	var encounter_id=hdrFormObj.encounter_id.value;
	var dialogHeight	= "10";
	...
	var status			= "no";	
	var features		= "dialogHeight:" + dialogHeight +"; dialogLeft:"+dialogLeft+ "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var retVal=window.showModalDialog("../../ePH/jsp/UserPINAuthorization.jsp?call_from=MAR&Witness=Y&patient_id="+patient_id+"&encounter_id="+encounter_id,"",features);
...
function dispPrevRecs(tdObj){
	var disp_prev_vital_signs_yn = document.prescription_header_form.disp_prev_vital_signs_yn.value;
  ...
  //  else
   ...
   // else
    ...
    document.getElementById("divPrevHtWtDtls").style.display='inline';
}

function hidePrevRecs(){
	if(document.getElementById("divPrevHtWtDtls").style.display=='inline')
...
function medicationReconciliation(){ // Added for MMS-KH-CRF-0016 - Start
	...
	var features	    =   "dialogLeft:" + dialogLeft + ";dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + ";scroll: "+scroll+"; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	    =   "" ;
	retVal              =   window.showModalDialog("../../ePH/jsp/MedicationReconciliation.jsp?&patient_id="+patient_id+"&encounter_id="+encounter_id+"&patient_class="+patient_class+"&pract_id="+pract_id+"&iv_prep_yn="+iv_prep_yn+"&pract_type="+pract_type+"&discharge_ind="+discharge_ind+"&gender="+gender+"&patient_age="+patient_age+"&patient_name="+patient_name+"&log_pract_id="+log_pract_id,arguments,features);
