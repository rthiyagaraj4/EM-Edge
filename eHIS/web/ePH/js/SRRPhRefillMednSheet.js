function showResults(){
	...
	var p_tok = formObj.p_tok.value;
	var p_locn_code = formObj.p_locn_code.value;
	...
	var names= new Array (getLabel("Common.OrderDateFrom.label","common"), getLabel("ePH.OrderDateTo.label","PH"), getLabel("ePH.DispenseLocation.label","PH"));

	if(checkFields(fields,names,parent.messageFrame)) 
	{
		
		if(doDateCheck(formObj.p_fr_disp_date,formObj.p_to_disp_date,parent.messageFrame) )
		parent.f_query_results.location.href="../../ePH/jsp/SRRPhRefillMednSheetResult.jsp?patient_id="+patient_id+"&locale="+locale+"&fr_dispdate="+fr_dispdate+"&to_dispdate="+to_dispdate+"&p_pres_id="+p_pres_id+"&p_pract_id="+p_pract_id+"&p_disp_locn_code="+p_disp_locn_code+"&p_locn_type="+p_locn_type+"&p_encounter="+p_encounter+"&p_locn_code="+locn_code+"&p_tok="+p_tok;
...
function SubmitLink(from,to,patient_id,p_pres_id,fr_dispdate,to_dispdate,p_encounter,p_pract_id,p_facility_id,p_disp_locn_code,p_tok,p_locn_type,p_locn_code){
	var obj=document.frames.SRRPhRefillMednSheetResult;
...
function searchPractitionerName(obj) {
	...
	var lan_id=document.SRRPhRefillMednSheetForm.p_language_id.value;
	

	dataNameArray[0]	= "APPL_USER_ID" ;
	dataValueArray[0]	= document.SRRPhRefillMednSheetForm.p_user_name.value;
	dataTypeArray[0]	= STRING ;
	dataNameArray[1]	= "FACILITY_ID" ;
	dataValueArray[1]	= document.SRRPhRefillMednSheetForm.p_facility_id.value;
...
function resetLocation(){
	var formObj = document.frames.SRRPhRefillMednSheetForm;
...
function showPrintReport( p_disp_date,p_token_no,p_patient_id,p_trn_group_ref){
	var formObj = parent.f_query_add_mod.SRRPhRefillMednSheetForm;
	 
		formObj.p_trn_group_ref.value = p_trn_group_ref;
		formObj.p_disp_date.value = p_disp_date;
		formObj.p_token_no.value = p_token_no ;
		formObj.p_patient_id.value = p_patient_id ;
		formObj.p_report_id.value = "PHPRRELMS_SRR" ;
		
	  parent.f_query_add_mod.document.SRRPhRefillMednSheetForm.submit();
   }
