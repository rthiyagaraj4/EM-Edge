...
%>
	...
	<tr>
			...
			<th  class="small" width="8%"><fmt:message  key="Common.Select.label" bundle="${common_labels}"/><input type="checkbox" name="selectall_yn" id="selectall_yn" value=""  onClick="return chkall_durg(activeOrderDetailsForm);"></th>
...
<%
			  ...
			  {
%>
					<td  class="<%=class_name%>"nowrap><fmt:message key="Common.SlidingScale.label" bundle="${common_labels}"/></td>
<%
			  }
%>
...
%>
			<td  class="<%=class_name%>" width="10%" ><label onClick="callFreqDet('<%=patient_id%>','<%=encounter_id%>','<%=order_id%>','<%=order_line_no%>','<%=java.net.URLEncoder.encode(freq_desc,"UTF-8")%>','<%=start_date%>','<%=sliding_scale_yn%>')" style="cursor:pointer;color:blue" nowrap><%=freq_desc%></label></td>
<%			}	%>
...
%>
	 ...
	 <input type="checkbox" name="p_print_yn<%=i%>" id="p_print_yn<%=i%>" value="<%=flag%>"  onClick="assignValue_inner(this);calvalidation(activeOrderDetailsForm,this,'<%=facility_id%>','<%=i%>','<%=order_id%>');" >
		</td>
		<input type="hidden" name="prescription_<%=i%>" id="prescription_<%=i%>" value="<%=order_id%>">
