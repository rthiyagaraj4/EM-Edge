...
%>
					<tr>
						<td class="COLUMNHEADERCENTER" class="data">&nbsp;<fmt:message key="ePH.AllergicOverrideReason.label" bundle="${ph_labels}"/></td>
					<!-- Added for IN:072715 START --> 
					...
					<%} %>
					<!-- Added for IN:072715 END -->
						<td class="label" ><label onClick="showAllergyDetails('<%=generic_id%>','<%=patient_id%>','<%=encounter_id%>','<%=java.net.URLEncoder.encode(generic_name,"UTF-8")%>','<%=drug_desc%>');" style="cursor:pointer;color:blue;font-size:9"><fmt:message key="ePH.AllergicDetails.label" bundle="${ph_labels}"/></label>&nbsp;&nbsp;&nbsp;</td> <!-- // java.net.URLEncoder.encode(drug_desc,"UTF-8") changed to drug_desc Added for ICN-65327 -->
					</tr>
					<tr>
						<td   colspan="2">&nbsp;
						<!--<textarea rows="3" cols="80" name="allergy" onKeyPress="CheckForSpecCharsAllCaseAlwSpc(event); checkMaxLimit(this,255);"  <%=disable_status%>><%=allergy%></textarea>-->
						<textarea rows="3" cols="80" name="allergy" onKeyPress="javascript:if(event.keyCode != '32'){ return checkMaxLimit(this,255)};"  <%=disable_status%>><%=allergy%></textarea>&nbsp;<!-- Modified for IN:073485 START -->
<%
						if(Override_called_from!=null && !Override_called_from.equals("RENEW_ORDER")) { 
%>
...
%>
					<tr>
						<td colspan="2"><marquee bgcolor="#FFCC66" scrollamount="2"><font color="black" style="font-weight:bold" size=1><fmt:message key="ePH.DailyDose.label" bundle="${ph_labels}"/>: <fmt:message key="Common.maximum.label" bundle="${common_labels}"/>-<%=daily_dose%>,<fmt:message key="Common.Minimum.label" bundle="${common_labels}"/>-<%=min_daily_dose%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<fmt:message key="Common.UnitDose.label" bundle="${common_labels}"/>: <fmt:message key="Common.maximum.label" bundle="${common_labels}"/>-<%=unit_dose%>,<fmt:message key="Common.Minimum.label" bundle="${common_labels}"/>-<%=min_unit_dose%></font></marquee></td>
					</tr>
					<tr>
						<td class="COLUMNHEADERCENTER"  class="data">&nbsp;<fmt:message key="ePH.BeyondDoseLimitOverrideReason.label" bundle="${ph_labels}"/>&nbsp;&nbsp;</td>
					...
					<%} %>
					<!-- Added for IN:072715 END -->
						<td class="label"><label onClick="showMonoGraph('<%=drug_desc%>','<%=mono_graph%>','<%=daily_dose%>','<%=unit_dose%>','<%=min_daily_dose%>','<%=min_unit_dose%>','<%=dosage_unit%>','<%=dosage_std%>','<%=dose_unit_desc%>','<%=max_daily_ceeling_dose%>','<%=min_daily_ceeling_dose%>','<%=max_unit_ceeling_dose%>','<%=min_unit_ceeling_dose%>','<%=dose_unit_desc1%>')" style="cursor:pointer;color:blue;font-size:9"><fmt:message key="ePH.OverdoseDetails.label" bundle="${ph_labels}"/></label>&nbsp;&nbsp;&nbsp;</td><!-- Modified for IN:071108  -->
					</tr>
					<tr>
						<td  colspan="2">&nbsp;<textarea rows="3" cols="80" name="exceed_dose" onKeyPress="javascript:if(event.keyCode != '32'){ return checkMaxLimit(this,255)};" <%=disable_status%>><%=exceed_dose%></textarea>&nbsp;
<% 
						if(Override_called_from!=null && !Override_called_from.equals("RENEW_ORDER")) { 
%>
...
%>
					...
					<tr>
						<td class="COLUMNHEADERCENTER" class="data">&nbsp;<fmt:message key="ePH.DuplicateDrugOverrideReason.label" bundle="${ph_labels}"/>&nbsp;&nbsp;</td>
					<!-- Added for IN:072715 START -->
					<%if(over_ride_remarks_select_appl){ %>
						<td class="fields">&nbsp;
					...
					<%} %>
					<!-- Added for IN:072715 END -->
						<td class="label">&nbsp;&nbsp;<label onClick="callCurrentMedication('<%=patient_id%>','<%=encounter_id%>','<%=generic_id%>')" style="cursor:pointer;color:blue;font-size:9"><fmt:message key="ePH.DuplicateDrugs.label" bundle="${ph_labels}"/></label>&nbsp;&nbsp;&nbsp;</td>
					</tr>

					<tr>
						<td colspan="2">&nbsp;<textarea rows="3" cols="80" name="dup_drug" onKeyPress="javascript:if(event.keyCode != '32'){ return checkMaxLimit(this,255)}" <%=disable_status%>><%=dup_drug%></textarea>&nbsp;
<% 
						if(Override_called_from!=null  && !Override_called_from.equals("RENEW_ORDER")) {
%>
							<img src="../../eCommon/images/mandatory.gif" >
...
%>
					...
					<%if(over_ride_remarks_select_appl){ %>
						<td class="fields">&nbsp;
					<select name="interaction_remarks_code" id="interaction_remarks_code"  onChange="checkOverrideRemarks(this,'DD',document.DispQueueMgmtHoldReason,document.DispQueueMgmtHoldReason.interaction_remarks,'interaction_remarks')"> <%=bean.getOverrideListValues(sql,inter_override_params,inter_override_select) %> </select>
					</td>
					<%} %>
					<!-- Added for IN:072715 END -->
...
%>
				...
				<%if(over_ride_remarks_select_appl){ %>
						<td class="fields">&nbsp;
					<select name="lab_interaction_remarks_code" id="lab_interaction_remarks_code"  onChange="checkOverrideRemarks(this,'DL',document.DispQueueMgmtHoldReason,document.DispQueueMgmtHoldReason.lab_interaction_remarks,'lab_interaction_remarks')"> <%=bean.getOverrideListValues(sql,lab_inter_override_params,lab_inter_override_select) %> </select>
					</td>
					<%} %>
					<!-- Added for IN:072715 END -->
...
%>
						...
						<!-- Added for IN:072715 END -->
							<td class="label"><label onClick="showAbuseDtl('<%=drug_code%>','<%=srl_no%>','<%=patient_id%>','<%=encounter_id%>');" style="cursor:pointer;color:blue;font-size:9;>">Drug Abuse Dtls</label>&nbsp;</td> 
						</tr>
						<tr >
							<td  colspan="2">&nbsp;<textarea rows="3" cols="80" name="abuse_override_remarks" onKeyPress="javascript:if(event.keyCode != '32'){ return checkMaxLimit(this,255)};" onblur="checkMaxLimit(this,255);" ><%=abuse_override_remarks %></textarea>&nbsp;
	<% 
							if(Override_called_from!=null && !Override_called_from.equals("RENEW_ORDER")) { 
	%>
								<img src="../../eCommon/images/mandatory.gif" >
...
<%
						if(Override_called_from!=null && Override_called_from.equals("RENEW_ORDER")) { 
%>
							<input type="button" value='<fmt:message key="Common.close.label" bundle="${common_labels}"/>' class="button" onClick="window.close()"></td>
<%
						}
						else {
%>
							<input type="button" value='<fmt:message key="Common.ok.label" bundle="${common_labels}"/>' class="button" onClick="validate(document.DispQueueMgmtHoldReason)">
<%
						}
%>
						</td>
...
%>
			...
			<input type="hidden" name="Override_called_from" id="Override_called_from" value="<%=Override_called_from%>">
		</form>
