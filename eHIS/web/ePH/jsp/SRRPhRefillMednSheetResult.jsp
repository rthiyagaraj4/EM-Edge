...
<form name="SRRPhRefillMednSheetResult" id="SRRPhRefillMednSheetResult">
...
<%
   try{
    ...
    String p_tok = CommonBean.checkForNull(request.getParameter("p_tok"));
	...
	result=bean.getRefillMedicationList( locale,  p_pres_id, patient_id, fr_dispdate, to_dispdate, p_encounter, p_pract_id, p_disp_locn_code, p_locn_type, p_locn_code, from, to, p_tok, p_facility_id);
...
//Corrected  for the incident num:23847 on 26/Nov/2010===By Sandhya
				if(record[8]!= null  )
				{
					//tokn = tokn+ "/"+ record[8];
					tokn =  record[8];
	...
	%>
			...
			<td class="<%=classvalue%>" style="font-size=9"><font class="hyperlink" style="cursor:pointer" onClick="showPrintReport('<%=record[5]%>','<%=tokn%>','<%=record[1]%>','<%=record[0]%>')" >&nbsp;<%=com.ehis.util.DateUtils.convertDate(record[5],"DMY","en",locale)%></font></td>
			<td class="<%=classvalue%>" style="font-size=9"><font class="hyperlink" style="cursor:pointer" onClick="showPrintReport('<%=record[5]%>','<%=tokn%>','<%=record[1]%>','<%=record[0]%>')">&nbsp;<%=tokn%></font></td>
	...
	<input type="hidden" name="p_tok" id="p_tok" value="">
