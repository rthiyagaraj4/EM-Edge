...
<%
	...
	if(ord_disp_location != null & ord_disp_location.size()>0){ 
		...
		if(dflt_disp_locn == null) dflt_disp_locn ="";
	}
	
	        HashMap record1=bean.ChkAdMixtureAppl();
			String alert_practitioner_yn=(String)record1.get("ALERT_PRACTITIONER_YN");
			String time_flag =bean.ChkWorkingHours((String)ord_disp_location.get(0));
		 if(!time_flag.equals("X")  )
	     {
...
<form name="TpnButtons" id="TpnButtons" id="TpnButtons">
	<table width="100%">
	<TR>
		...
		<TD align="right">
			...
			<INPUT TYPE="button" class="button" name="Release" id="Release" value='<fmt:message key="ePH.ReleaseRx.label" bundle="${ph_labels}"/>' onClick="apply('<%=encounter_id%>','<%=tpn_screening_type%>')"  >	
		</TD>
	</TR>
	</TABLE>
