   ...
   try{
	...
	%>
		...
		<table cellpadding="0" cellspacing="0" width="98%" align="center" border="1" id="reprint_rep">
			...
			<th  nowrap><fmt:message key="ePH.MedicationReturnNo.label" bundle="${ph_labels}"/></th>
...
<%		
	...
	%>
		<tr>
			<td class="<%=classvalue%>" style="font-size=9"><font class="hyperlink" style="cursor:pointer" onClick="showPrintReport('<%=result.get(i+1)%>','<%=result.get(i+8)%>','<%=result.get(i+9)%>')"><%=result.get(i+8)%></font></td>
			...
			<td class="<%=classvalue%>" style="font-size=9"><%=result.get(i+2)%></td>
