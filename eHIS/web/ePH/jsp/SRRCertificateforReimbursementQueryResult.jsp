...
//out.println("maxRecord-----> " +maxRecord);
	...
	<%
	    if ( !(start <= 1)) {
	%>
			 <a href='javascript:SubmitLink("<%=(start-query_result_size)%>","<%=(end-query_result_size)%>","<%=(patient_id)%>","<%=(p_fr_pre)%>","<%=(p_to_pre)%>","<%=(adr_reported_date_from)%>","<%=adr_reported_date_to%>","<%=(p_encounter)%>","<%=(p_disp_locn_code)%>","<%=(token_no_from)%>","<%=(token_no_to)%>","<%=locn_type%>","<%=locn_code%>")' text-decoration='none'><fmt:message key="Common.previous.label" bundle="${common_labels}"/></a>	
	<%
	    }
	    if ( !((start+14) > result.size() )){
	%>
	    <a href='javascript:SubmitLink("<%=(start+query_result_size)%>","<%=(end+query_result_size)%>","<%=(patient_id)%>","<%=(p_fr_pre)%>","<%=(p_to_pre)%>","<%=(adr_reported_date_from)%>","<%=adr_reported_date_to%>","<%=(p_encounter)%>","<%=(p_disp_locn_code)%>","<%=(token_no_from)%>","<%=(token_no_to)%>","<%=locn_type%>","<%=locn_code%>")' text-decoration='none'><fmt:message key="Common.next.label" bundle="${common_labels}"/></a>
	<%
	    }
	%>
		</td>
		...
		<table>
			...
			<th  nowrap><fmt:message key="ePH.Select.label" bundle="${ph_labels}"/><input type="checkbox" name="selectall_yn" id="selectall_yn" value="N" onClick="return chkall(SRRCertiforReimbursementQueryResultForm);" ></th>
				</tr>
	...
	%>
		<tr>
			...
			<td class="<%=classvalue%>" style="font-size=9" nowrap ><%=pre_id%></td>			
			<td class="<%=classvalue%>" style="font-size=9" nowrap ><%=record[6]%></td>	
			<td  class="<%=classvalue%>"nowrap>  <input type="checkbox" name="p_print_yn<%=i%>" id="p_print_yn<%=i%>" value="<%=record[7]%>" onClick="assignValue(this);calvalidation(this,'<%=p_facility_id%>','<%=i%>','<%=pre_id%>');"   
			<%if(record[7].equals("Y")   ){count++;%>checked<%}%>></td>
