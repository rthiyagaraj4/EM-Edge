...
%>
								<TD CLASS="ALLERGY1" COLSPAN="" NOWRAP WIDTH="10%" TITLE="Allergy Drug" onClick="showAllergyDetails('<%=generic_id%>','<%=patient_id%>','<%=encounter_id%>','<%=java.net.URLEncoder.encode(generic_name,"UTF-8")%>','<%=java.net.URLEncoder.encode(drug_desc,"UTF-8")%>');" onMouseOver="changeCursor(this);">&nbsp;<%=allergy_override_gif%>&nbsp;</TD>
<%
							}
							else {
%>
...
%>
								<TD CLASS="DOSELIMIT" COLSPAN="" NOWRAP WIDTH="10%" TITLE="Exceed Dosage Limit" onMouseOver="changeCursor(this);" onClick="showMonoGraph('<%=drug_desc%>','<%=mono_graph%>','<%=daily_dose%>','<%=unit_dose%>','<%=min_daily_dose%>','<%=min_unit_dose%>');">&nbsp;<%=overdose_override_gif%>&nbsp;</TD>
...
%>
								<TD CLASS="CURRENTRX1" COLSPAN="" NOWRAP WIDTH="10%" TITLE="Duplicate Medication" onMouseOver="changeCursor(this);" onClick="callCurrentMedication('<%=patient_id%>','<%=encounter_id%>','<%=generic_id%>')">&nbsp;<%=currrx_override_gif%>&nbsp;</TD>
<%
							}
							else {
%>
