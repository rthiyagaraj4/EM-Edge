...
<body OnMouseDown="CodeArrest()" onKeyDown="lockKey()">
	<ibaHTMLEl:form action="/IssueOfLinenPageAction.do">
...
<ibaLogic:iterate id="result" name="PageForm"
	...
	<!-- 	 <td class="<%=style%>" align='left'>
		...
		LinenIssue.getSourceDesc("<%=locale%>","<%=facilityId%>",stype,scode,callBackSourceDesc);
			function callBackSourceDesc(data){
				for(prop in data)
					document.getElementById("sDesc_<%=rowId%>").value=data[prop];
					desc=document.getElementById("sDesc_<%=rowId%>").value;
				}
		...
		<ibaBean:message key="com.iba.ehis.lc.department" bundle="lcLabels" /></ibaLogic:equal>
		</td>

<td class="<%=style%>" align='right'> <a id="<%=rowId%>"  href='<%=request.getContextPath()%>/eLC/IssueOfLinen/jsp/link.jsp?&sourceCode="+<ibaBean:write name="result" property="sourceCode" />+"&sourceType="+<ibaBean:write name="result" property="sourceType" />+"&requestNum="+<ibaBean:write name="result" property="requestNo" />+"&requestDate="+<ibaBean:write name="result" property="requestDate" bundle="lcLabels" />+"&raisedBy="+<ibaBean:write name="result" property="raisedBy" />+"&requestStatus="+<ibaBean:write name="result" property="requestStatus" />+"&linenItem="+<ibaBean:write name="result" property="linenItem" />+"&linenDesc="+<ibaBean:write name="result" property="shortDesc" />+"&requestedQty="+<ibaBean:write name="result" property="requestedQty" bundle="lcLabels" />' onclick="return show_hide_box(this,100,35,'2px dashed')"> <ibaBean:write name="result" property="requestNo" /> </a>
</td>
...
<ibaBean:write name="result" property="raisedBy" /> 
	...
	<td class="<%=style%>" align='left'> 
		...
		<ibaBean:message key="com.iba.ehis.lc.requestForLinen.issued" bundle="lcLabels" /></ibaLogic:equal>
