...
function run() 
{
	...
	var function_id=f_query_add_mod.document.forms[0].function_id.value; //Added by Ashwini on 08-Mar-2019 for ML-MMOH-CRF-1296
	...
	//Added by Ashwini on 08-Mar-2019 for ML-MMOH-CRF-1296
	if(function_id == 'AE_PAT_DEPARTMENT_DEAD')
	{
		var names  = new Array ( f_query_add_mod.getLabel("Common.DeceasedDate.label","Common")+" "+f_query_add_mod.getLabel("Common.from.label","Common"),f_query_add_mod.getLabel("Common.DeceasedDate.label","Common")+" "+f_query_add_mod.getLabel("Common.to.label","Common"));
	}else
	{
...
if(parent.parent.frames[2].frames[1].checkFields( fields, names, messageFrame))
{
		...
		if(f_query_add_mod.isBefore(f_query_add_mod.document.forms[0].p_fm_date.value,f_query_add_mod.document.forms[0].p_to_date.value,"DMY",locale)){
			//Added by Ashwini on 08-Mar-2019 for ML-MMOH-CRF-1296
			var dec_visit_type = "";
			if(function_id == 'AE_PAT_DEPARTMENT_DEAD')
			{
				dec_visit_type = f_query_add_mod.getLabel("Common.deceased.label","Common")+" "+f_query_add_mod.getLabel("Common.code.label","Common")
			}else
			{
				dec_visit_type = f_query_add_mod.getLabel("Common.VisitTypeCode.label","Common")
			}
				if(f_query_add_mod.CheckString( dec_visit_type,f_query_add_mod.document.forms[0].p_fm_visit_type,f_query_add_mod.document.forms[0].p_to_visit_type,messageFrame))
				if(f_query_add_mod.CheckString( f_query_add_mod.getLabel("Common.locationcode.label","Common"),f_query_add_mod.document.forms[0].p_fm_locn_code,f_query_add_mod.document.forms[0].p_to_locn_code,messageFrame))
				if(f_query_add_mod.CheckString( f_query_add_mod.getLabel("Common.SpecialityCode.label","Common"),f_query_add_mod.document.forms[0].p_fm_specialty_code,f_query_add_mod.document.forms[0].p_to_specialty_code,messageFrame))
				if(f_query_add_mod.CheckString( f_query_add_mod.getLabel("Common.ServiceCode.label","Common"),f_query_add_mod.document.forms[0].p_fm_service_code,f_query_add_mod.document.forms[0].p_to_service_code,messageFrame))
				if(f_query_add_mod.CheckString( f_query_add_mod.getLabel("Common.practitionerid.label","Common"),f_query_add_mod.document.forms[0].p_fm_pract_id,f_query_add_mod.document.forms[0].p_to_pract_id,messageFrame))
				{
			...
			if(function_id == 'AE_PAT_DEPARTMENT_DEAD')
			{
				var visit=f_query_add_mod.getLabel("Common.DeceasedDate.label","Common");
			}else
			{
...
async function searchCode(obj,target)
{
	...
	var sqlSecond="";
	var tit="";
	//var p_faclid = "<%=p_facility_id%>";
	//var p_usr = "<%=p_user_name%>";
	//var p_resp = "<%=p_resp_id%>";
	...
	var function_id = document.forms[0].function_id.value; //Added by Ashwini on 08-Mar-2019 for ML-MMOH-CRF-1296
	...
	if((obj.name=="tovtype" || obj.name=="fromvtype") && function_id == 'AE_PAT_BROUGHT_DEAD')
	{
	...
	}else if((obj.name=="tovtype" || obj.name=="fromvtype") && function_id == 'AE_PAT_DEPARTMENT_DEAD')
	{
		tit=getLabel("Common.DispType.label","Common");
        argumentArray[0]="select disp_type code, short_desc description from am_disposition_type_lang_vw where deceased_yn = 'Y' and language_id = "+locale+" and  '1' like ? and upper(disp_type) like upper(nvl(?,disp_type)) and upper(short_desc) like upper(nvl(?,short_desc)) order by 2";
