...
<th nowrap  width='5%' ><fmt:message key="Common.Disaster.label" bundle="${common_labels}"/> <fmt:message key="Common.CCategory.label" bundle="${common_labels}"/><img src='../../eAE/images/Ascending.gif'  border=none  align=right name='disaster_category' style='visibility:hidden'></th>
<%} else if (StrHdr.equals("disaster_type") && enable_disaster_categ.equals("Y")) {%>
<th nowrap  width='5%' ><fmt:message key="Common.Disaster.label" bundle="${common_labels}"/> <fmt:message key="Common.Type.label" bundle="${common_labels}"/><img src='../../eAE/images/Ascending.gif'  border=none  align=right name='disaster_type' style='visibility:hidden'></th>
<%} else if(StrHdr.equals("sec_triage_queue_no") && queueProceed){%>
<th onclick ="sort('queue_no','<%=priority_zone%>');" style='cursor:pointer' nowrap  width='7%' ><fmt:message key="Common.QueueNo.label" bundle="${common_labels}"/><img src='../../eAE/images/Ascending.gif'  border=none  align=right name='queue_no_gif' style='visibility:hidden'></th>
<%}
}%>
</tr>
	...
	for (int m=0; m < size; ++m)
	{
      ...
      <% } else if (str.equals("patient_name")) {
		...
		if(checkout_yn.equals("N"))
			...
			<td class='<%=classValue%>' width='9%'  id='desc<%=i%>'onMouseover="onMMove(msg,event,'<%=brought_dead%>','<%=brought_dead_yn%>');hideToolTip(this,'imgArrow<%=i%>','<%=i%>');" onMouseout="onMOut(msg,'<%=brought_dead_yn%>');" onClick="showAllowedOperations('<%=clinic_code%>','<%=java.net.URLEncoder.encode(p_clinic_desc)%>','<%=clinic_type%>','<%=patient_id%>','<%=queue_status%>','<%=encounter_id%>','<%=triage_area%>','<%=trauma_yn%>','<%=treatment_area_code%>','<%=practitioner_id%>','<%=java.net.URLEncoder.encode(practitioner_full_name)%>','<%=treatment_area_desc%>','<%=open_to_all_prac_for_clinic_yn%>', '<%=queue_date_time%>','<%=sys_date_time%>','<%=queue_date%>','<%=patient_gender%>','<%=brought_dead_yn%>','<%=assign_tmt_area_time%>','<%=open_to_all_pract_yn%>','<%=patient_class%>','<%=priority_zone%>','<%=bed_no%>','<%=visit_type_code%>','<%=pract_type%>','<%=record_vital_signs_yn%>','<%=pract_type1%>','<%=enable_rights_yn%>','<%=sel_triage_yn%>','<%=sel_cons_yn%>','<%=end_date%>','<%=start_date%>','<%=enable_rights_yn%>','<%=min_age_for_lmp%>','<%=year_value%>','<%=allow_chkout_with_bill_stmt_yn%>','<%=episode_id%>','<%=visit_num%>',this,'imgArrow<%=i%>','<%=i%>','link<%=i%>','<%=reg_new_born_yn%>','<%=inv_reg_new_born_yn%>','<%=pendngAdmDateTime%>','<%=sel_admission_yn%>' , '<%=sel_cancel_yn%>' , '<%=sel_referral_yn%>', '<%=enable_admission_tab%>' , '<%=referral_id%>', '<%=assign_bed_cnt%>');onMOut(msg,'<%=brought_dead_yn%>');"><a href='javascript:DummyRef()' name='link<%=i%>' ><font size='1'><%=img_temp%><%=patient_name%></a><img align='right' src='../../eCommon/images/inactiveArrow.gif' name='imgArrow<%=i%>' id='imgArrow<%=i%>'></td><!--Modified by Thamizh selvi on 18th Apr 2018 against ML-MMOH-CRF-0658-->
						...
						<% }else{ %>
							<table>
								...
								<tr>
									...
									<fmt:message key="Common.Pending.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.for.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.admission.label" bundle="${common_labels}"/>&nbsp; -</font></td>
...
<table>
					<%if(admission_cnt.equals("1") && cancelDateTime.equals("")){%>
								...
								<tr>
									<td nowrap>
									<font size=1>
									<fmt:message key="eIP.LengthofStay.label" bundle="${ip_labels}"/>
									-</font>
									</td>
									<td nowrap><font size=1><%=forAdmWaitngTime%></font></td>
									<%}%>
									...
									<tr><td nowrap width='6%' ><font size=1><a href='javascript:showReferralDetails("<%=patient_id%>","<%=encounter_id%>","<%=facility_id %>")'><fmt:message key="Common.view.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.details.label" bundle="${common_labels}"/></a></font></td></tr>
									<%}%>
...
<%}
		...
		<td  class='<%=classValue%>' width='7%'><font size=1><%=disaster_category%></td>
			<% }else {%>
			  <td nowrap class='<%=classValue%>'  width='7%'><font size=1>&nbsp;</td>
		 <%}}else if (str.equals("disaster_type")) { 
			if(enable_disaster_categ.equals("Y") && !disaster_type.equals("")) { %>
		<td  class='<%=classValue%>' width='7%'><font size=1><%=disaster_type%></td>
		<% }else {%>
			  <td nowrap class='<%=classValue%>'  width='7%'><font size=1>&nbsp;</td>
        ...
        <%}%>	        
		 ...
		 <a href="javascript:getMenstrualHistoryDtl('<%=patient_id%>','<%=facility_id%>')" style="white-space: nowrap;"><fmt:message key="eAE.ViewMenstrualHistoryDetails.label" bundle="${ae_labels}"/></a>
		 <%} %>
		 <INPUT TYPE="hidden" NAME="txtComm<%=i%>" value="<%=dis_regn_remarks%>">
		 </a>
			   </font>
		</td>
		<%
		...
		<td class='<%=classValue%>' width='5%'><font size=1>  <!-- //68508 -->
		 <%if(existActiveMensDtlYN > 0 && invokeMenstrualHistYN.equals("Y")){ %>
		 <a href="javascript:getMenstrualHistoryDtl('<%=patient_id%>','<%=facility_id%>')" style="white-space: nowrap;"><fmt:message key="eAE.ViewMenstrualHistoryDetails.label" bundle="${ae_labels}"/></a>
		 <%}else{ %>&nbsp;<%} %>
		 </td>
 ...
 <%} else if (str.equals("case_of_trauma")) {%>
		...
		<%}else if(str.equals("sec_triage_queue_no") && queueProceed){ %>
			<td  class='<%=classValue%>' width='7%'><font size=1><%=queue_no==""?"&nbsp;":queue_no %></td>
...
<!--Added By Kamatchi S for ML-MMOH-CRF-1972-->
<input type="hidden" name="enable_admission_tab" id="enable_admission_tab" value="<%=enable_admission_tab%>">
<input type="hidden" name="sel_admission_yn" id="sel_admission_yn" value="<%=sel_admission_yn%>">
<input type="hidden" name="sel_cancel_yn" id="sel_cancel_yn" value="<%=sel_cancel_yn%>">
<input type="hidden" name="referral_id" id="referral_id" value="<%=referral_id%>">
<input type="hidden" name="sel_referral_yn" id="sel_referral_yn" value="<%=sel_referral_yn%>">
<input type="hidden" name="isForAdmission" id="isForAdmission" value="<%=isForAdmission%>">
<input type="hidden" name="enableSubTabForIPAdm" id="enableSubTabForIPAdm" value="<%=enableSubTabForIPAdm%>">
<input type='hidden' name='toggleId1' id='toggleId1' value="">

<input type='hidden' name='queueProceed' id='queueProceed' value="<%=queueProceed %>">
...
<input type='hidden' name='p_ws_no' id='p_ws_no' value='<%=p_ws_no%>'> 
<input type='hidden' name='unbilled_yn' id='unbilled_yn' value='<%=unbilled_yn%>'> 
<input type='hidden' name='session_id' id='session_id' value='<%=sessionId%>'> 
<input type='hidden' name='isSiteUnbilledorNot' id='isSiteUnbilledorNot' value='<%=isSiteUnbilledorNot%>'> <!--Added By Gunasekar R for  GHL-CRF-0517.4 End-->
