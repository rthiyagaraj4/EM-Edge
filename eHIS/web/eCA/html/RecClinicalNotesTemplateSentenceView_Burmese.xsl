...
<!--                     BORDER-STYLE:dashed; -->
	...
	<xsl:template match="USER-TEMPLATE">
		...
		<table border="0" cellpadding='0' cellspacing='4' width="100%" style="font-family:Segoe UI">
		<!--IN066793 ENDS -->
...
<!-- 					<td align="left" style="border-style:dashed"> -->
				...
				<xsl:otherwise>
					<xsl:if test="not(@TXTALLIGNYN) or @TXTALLIGNYN='N'">
						<tr>
							<td align="left" colspan="1" valign="top" width="20%">
								<xsl:attribute name="nowrap"></xsl:attribute>
								<!-- <xsl:attribute name="class"><xsl:value-of select="@CLASS"/></xsl:attribute> -->
								<font style="COLOR:NAVY;FONT-SIZE:9pt;font-weight:bold;hyphens:auto;FONT-FAMILY: Segoe UI;"><!--IN066793 -->
									<xsl:value-of select="@PROMPT"/>
								</font>
							</td>
							<td width="3%" valign="top">
					...
					<xsl:if test="@TXTALLIGNYN='Y'">
						<tr>
							...
							<td width="3%" valign="top">
								<font style="COLOR:NAVY;FONT-SIZE:9pt;font-weight:bold;FONT-FAMILY: Segoe UI;"><!--IN066793 -->
									:&#160;&#160;&#160;
							</font>
							</td>
							<td  align="left">&#160;</td>
	...
	<xsl:param name="text" select="." /> 
		<font>
			<xsl:if test="@GRID-COMP|@MATRIX-COMP">
				<xsl:attribute name="style">
            COLOR:NAVY;FONT-SIZE:9pt;font-weight:normal;hyphens:auto;FONT-FAMILY: Segoe UI ; <!--IN066793 -->
            </xsl:attribute>
			</xsl:if>
			<xsl:if test="not(@GRID-COMP|@MATRIX-COMP)">
				<xsl:attribute name="style">
	...
	<xsl:template match="LONG-TEXT">
		<xsl:if test="(not(/USER-TEMPLATE/@ONLYFILLED) or /USER-TEMPLATE/@ONLYFILLED = 'N' or (/USER-TEMPLATE/@ONLYFILLED = 'Y' and text() != '' )) and (not(@TXTALLIGNYN) or @TXTALLIGNYN='N') ">
			<tr>
				...
				<td  align="left">				 
				<xsl:attribute name="style">hyphens:auto</xsl:attribute>
				<font style="COLOR:NAVY;FONT-SIZE:9pt;font-weight:normal; FONT-FAMILY: Segoe UI ;">	<!--IN066793 -->				
