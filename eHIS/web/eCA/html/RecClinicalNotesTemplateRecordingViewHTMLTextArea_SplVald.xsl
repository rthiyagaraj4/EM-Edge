...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"> 
     ...
     <html>
        <head>            
	 	...
	 	<script>
			...
			setTimeout(function(){ //IN055628
				for(i=0;i&lt;len;i++){
					var input = inputs[i];
					document.getElementById(input.id).contentWindow.document.body.innerHTML= eval("document.forms[0]."+input.name+".value");
					document.getElementById(input.id).contentWindow.document.body.contentEditable = "True";
					document.getElementById(input.id).contentWindow.document.body.style.fontFamily = "verdana";
					document.getElementById(input.id).contentWindow.document.body.style.fontSize = "10PT";
					document.getElementById(input.id).contentWindow.document.body.style.lineHeight="15px";
				}
			},100); //IN055628
		</script>
    </xsl:template>
        ...
        <xsl:choose>
            <xsl:when test="../../@LIST_SELECTION='M'">
                ...
                <input type="checkbox">
					...
					<xsl:attribute name="id"><xsl:value-of select="@NAME"></xsl:value-of></xsl:attribute>
                </input>
				<xsl:value-of select="."/>
            </xsl:when>
            <xsl:otherwise>
 ...
 <!--   <xsl:choose>
	   ...
	   <xsl:attribute name="title">
	    ...
	    <xsl:apply-templates select="POST-ITEM-ACTIONS"/>
				...
				<!--<input type="text" onKeyDown='restrictBackSpace(this)'>-->
					<input type="text">
						...
						<xsl:attribute name="onBlur">
						  return lookUpOnBlur<xsl:value-of select="@NAME"/>(this);
						</xsl:attribute>
					</input>
					<!-- ONBLUR LOOKUP FUNCTION -->
			
			<script>
						async function lookUpOnBlur<xsl:value-of select="@NAME"/>(inputElement)
						{ 
							//var str = "<xsl:value-of select="@NAME"/>";
							const search_text = inputElement.value;
							let targetField = inputElement.closest('tr').querySelector(`input[name='${inputElement.name}']`);
							if(search_text!=""){
							var encounter_id ="";
							var patient_id ="";
							var tempTitle ="";
							...
							if(returnArray != null)
							{
								...
								//document.getElementById(str).value = returnArray[1];
								targetField.value = returnArray[1];
			...
			<!-- ONBLUR LOOKUP FUNCTION END -->
					<script>
					async function lookUp<xsl:value-of select="@NAME"/>(inputElement)
						{
						let targetField = inputElement.closest('tr').querySelector(`input[name='${inputElement.name}']`);
							<xsl:variable name="str" select="@NAME"/>
							var encounter_id ="";
							var patient_id ="";
							var tempTitle ="";
							var speciality_code = "";
					...
					<input type="image" src = "../../eCA/images/Lookup.gif" style="padding-top: 3px;background: url(../../eCA/images/Lookup.gif) no-repeat center;background-size: contain; border: none; width: 15px; min-height: 18px; cursor: pointer;">
						<xsl:if test="@GRID-COMP='Y'">
   ...
   <xsl:template name="LIST-BOX-WTD" match="LIST-BOX-WTD">
	   ...
	   <xsl:choose>
				<xsl:when test="@STD_COMP_RESULT_TYPE ='L'">
					...
					<script>
						async function lookUp<xsl:value-of select="@NAME"/>()
						{
							...
							if(returnArray != null)
							{
								document.getElementById(<xsl:value-of select="@NAME"/>).value = unescape(returnArray[1]);
								document.getElementById(<xsl:value-of select="@NAME"/>_desc).value = returnArray[0];
  ...
  <!--<textarea rows="1">  68801 by Twinkle-->   
   	<textarea rows="2">         
            <xsl:if test="@GRID-COMP = 'Y'">
...
<!-- <xsl:attribute name="OnBlur">CheckMaximumLength('<xsl:value-of select="@PROMPT"/>',this,'<xsl:value-of select="@SIZE"/>',top.messageFrame)</xsl:attribute>  68801 by Twinkle-->         
<xsl:attribute name="OnBlur">CheckMaximumLength('<xsl:value-of select="@PROMPT"/>',this,'<xsl:value-of select="@SIZE"/>',top.messageFrame,'<xsl:value-of select="@DISCRID"/>')</xsl:attribute> 
  ...
  <xsl:template match="LONG-TEXT">    
			...
			<table  width="100%" border="0">
				<tr>
					<td align="left" nowrap="nowrap" width="25%">
						...
						<xsl:if test="@RECORD-TYPE!=''">
							<xsl:call-template name="HTML-TEXT-WTD"/>
						</xsl:if>
					</td>	
					<td align="left" valign="bottom" nowrap="nowrap" width="10%">
						<xsl:if test="@RECORD-TYPE!=''">
							<input type="button" class="button" value="Link Doc" name = "" onClick="" >
								...
								<xsl:attribute name="id">Link_Doc_<xsl:value-of select="@NAME"/></xsl:attribute>
								<!--IN061907 START-->
								...
								<xsl:attribute name="onClick">showLinkDocumentsHTMLFromTemplate('<xsl:value-of select="@RECORD-TYPE"/>','<xsl:value-of select="@NAME"/>','<xsl:value-of select="@LINKDOC-YN"/>')</xsl:attribute>
								<!--IN061907 End-->
							</input>
						</xsl:if>
						<input type="button" class="button" value="Canned Text" name = "" onClick="" >
							...
							<xsl:attribute name="id">Can_Txt_<xsl:value-of select="@NAME"/></xsl:attribute>
				...
				<!--64275 Starts-->
						<xsl:if test="@RECORD-TYPE='CHIC'">	
							<td align="left" valign="bottom" nowrap="nowrap" width="10%">
							 <xsl:text>&#xA0;</xsl:text>
							<IMAGE SRC="../../eCA/images/MI_RecordChiefComplaints.gif">
								<xsl:attribute name="onClick">showRecordCheifComplaints()</xsl:attribute>
							</IMAGE>
							</td>
						</xsl:if>
				<!--64275 ENDS-->
   ...
   <xsl:template name="LONG-TEXT-WTD" match="LONG-TEXT-WTD">
        <textarea rows="3" cols="80" style="width:546px;">
                <xsl:attribute name="name"><xsl:value-of select="@NAME"/></xsl:attribute>
        ...
        <!--<xsl:attribute name="onBlur">chkMaxLenForLongTest('<xsl:value-of select="@NAME"/>','<xsl:value-of select="@PROMPT"/>')</xsl:attribute> 68801 by Twinkle-->		     
		<xsl:attribute name="onBlur">chkMaxLenForLongTest('<xsl:value-of select="@NAME"/>','<xsl:value-of select="@PROMPT"/>','','<xsl:value-of select="@DISCRID"/>');isSpclChar('<xsl:value-of select="@NAME"/>')</xsl:attribute>
                <xsl:value-of select="." disable-output-escaping="yes"/>
           </textarea>

   </xsl:template>
   <xsl:template name="HTML-TEXT-WTD" match="HTML-TEXT-WTD">      
		<DIV style="background-color: white;border: 1px solid #CCC;box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075);width:546px;">
		  <iframe width="546" height="100" frameborder="0">
			<xsl:attribute name="name"><xsl:value-of select="@NAME"/></xsl:attribute>
			<xsl:attribute name="id">IFRAME<xsl:value-of select="@NAME"/></xsl:attribute>
   <!-- <xsl:attribute name="onBlur">chkMaxLenForHTMLLongTest('<xsl:value-of select="@NAME"/>','<xsl:value-of select="@PROMPT"/>');isSpclCharHTMLLongTest('<xsl:value-of select="@NAME"/>')</xsl:attribute>  68801 by Twinkle -->		  
   <xsl:attribute name="onBlur">chkMaxLenForHTMLLongTest('<xsl:value-of select="@NAME"/>','<xsl:value-of select="@PROMPT"/>','','<xsl:value-of select="@DISCRID"/>');isSpclCharHTMLLongTest('<xsl:value-of select="@NAME"/>')</xsl:attribute>
		...
		<xsl:choose>
			<xsl:when test="@NUM_PREFIX_REQUIRED_YN = 'Y'">
				<input type="text" size="1" maxlength="1">
					...
					<xsl:attribute name="id"><xsl:value-of select="@NAME"/>_number_prefix</xsl:attribute>
       ...
       <input>
		...
		<xsl:choose>
			...
			<xsl:if test="not(@GRID-COMP)"> 
				...
				<!--<xsl:attribute name="onBlur">if(CheckDate(this)){getFormulaResult();<xsl:value-of select="@ONBLUR"/>}</xsl:attribute>--><!--INC61905--><!-- Commented for IN071708-->
			...
			<xsl:if test="not(@GRID-COMP)">
				...
				<xsl:attribute name="onBlur">if(callCheckDateTime(this)){<xsl:value-of select="@ONBLUR"/>}</xsl:attribute><!--INC61905-->
				<xsl:attribute name="value"><xsl:value-of select="@OTHLANGVAL"/></xsl:attribute>
	      ...
	      <xsl:otherwise>
			...
			<input type="image" src = "../../eCommon/images/CommonCalendar.gif">
				...
				<xsl:if test="@COMPTYPE='E'">
				 ...
				 <xsl:if test="not(@GRID-COMP|@MATRIX-COMP)">              
					<xsl:attribute name="name">calender_for_<xsl:value-of select="@NAME"/></xsl:attribute>	
					<xsl:attribute name="id">calender_for_<xsl:value-of select="@NAME"/></xsl:attribute>
				</xsl:if>
		    </input>
		    </xsl:if>
	      </xsl:otherwise>
