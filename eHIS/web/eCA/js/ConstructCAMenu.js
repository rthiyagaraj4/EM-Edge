...
async function callpriview()
{
	...
	for(i=0;i<obj.length;i++)
	{
		...
		arr[i] = trimtxt+add;
				...
				var obj = document.getElementById('Menu').selectedIndex;
				//var menuid = Construct_ca_menu_form.Menu.options[obj].text;
				var menuid = document.getElementById('Menu').options[obj].text;
				...
				var reportURL 	= await window.showModalDialog( url_mod, arguments, features ) ;
...
function putVal()
{
	...
	{	
		...
		if(obj1.selectedIndex!=-1)
		{
			...
			if(toplen!= 0)
			{
				var topval = obj3.options[toplen-1].value;
				var topchk = topval.split("#");
			}
...
function MoveIndex(num)
{
	...
	if(index3!=-1)           //it checks whether the MainMenus listbox is been selected r not..
	{
		...
		if(num==1 && index3!=-1 && index3!=0  ) //moving upwards.. 1 is passed on click of Top button... 
		{
			...
			var trimbel = belowtxt.replace(/\u00A0/g,'');
			var Bval = belowval;
			var Btxt = belowtxt;
			}
			else
			...
			var toplevel= val3.split("#");
				//alert("top"+toplevel[1]+"bel"+belval[1]);	
...
/*	if(toplevel[2] == 'F' && toplevel[1]<=level[1] && level[2] == 'M')
	{

		if(toplevel[1] == '2')
		{
			toptxt = spc+spc+trimtxt;
			topval = toplevel[0]+"#3#"+toplevel[2];
		}
		else if(toplevel[1] == '3')
		{
			toptxt = spc+spc+spc+trimtxt;
			topval = toplevel[0]+"#4#"+toplevel[2];
		}

	}*/
//alert(level[2]+toplevel[1]+level[1]+toplevel[2]+belval[2]);
	/*if(level[2] == 'M' && toplevel[2] == 'F' && level[1]<=toplevel[1] && belval[2] == 'F')
	{
		alert("below");
		Btxt = spc+trimbel;
		Bval = belval[0]+"#2#"+belval[2];
	...
	//moving downwards.....2 is passed on click of Bottom button... 
	...
	else if(num==2 && index3!=-1 && index3!=len1-1 )
		{
			...
			/*	var curtxt = obj3.options(index3).text;
				var curval = obj3.options(index3).value;
				var cur = curval.split("#");*/
	...
	{   
		// var obj1 =Construct_ca_menu_form.Menu.selectedIndex;
		 var obj1 =document.getElementById('Menu').selectedIndex;
		// var menuid = Construct_ca_menu_form.Menu.options[obj1].value;
		 var menuid = document.getElementById('Menu').options[obj1].value;

				
				//var trimval=trimString(val2);
				var trimval = val2.replace(/\u00A0/g,'');
				if(index3!=0)
				{
...
function create()
{
	construct_ca_menu.location.href='../../eCA/jsp/ConstructCAMenu.jsp?';
}
	...
	for(i=0;i<obj.length;i++)
	{
		...
		{	
			for(p=0;p<((level3.length)-1);p++)
			{
				...
				from=level3[p];
				to=level3[p+1];
				//if(from<level3[((level3.length)-1)])
				//{
					//alert("from"+(from+1));
					//alert("to"+(to-1));
...
//				for(s=(from+1);s<=(to-1);s++)
	...
	{	
		...
		for(f=(level3[(level3.length)-1]+1);f<((obj.length)-1);f++)
		{
			...
			//alert(lvlno[f]);
			//alert(lvlname[f]);
			//if( f > (level3[(level3.length)-1]+1) && lvlname[f-1]!='F'){
			if(lvlname[f]=='M')
						{
							//alert("q"+q);
...
function apply()
{
	if(construct_ca_menu.document.Construct_ca_menu_form!=null)
	{
	var fields = new Array(construct_ca_menu.document.Construct_ca_menu_form.Menu);
    var names = new Array (construct_ca_menu.getLabel("Common.MenuID.label",'COMMON'));
...
{	
	...
	var HTMLVal="<html><script src='../../eCommon/js/ValidateControl.js' language='javascript'></script><script src='../../eCommon/js/showModalDialog.js' language='JavaScript'></script><body onKeyDown='lockKey()'><form name='formtemp' id='formtemp' method='post' action='../../servlet/eCA.ConstructCAMenuServlet'>";
	var rowcnt = obj.length;//alert("here"+rowcnt);
	for(i=0;i<obj.length;i++)
	{
		var val = obj.options[i].value;
		//alert('val...'+val);
...
HTMLVal+="<input type='hidden' name='mode' id='mode' value='"+construct_ca_menu.document.Construct_ca_menu_form.mode.value+"'>";
HTMLVal+="<input type='hidden' name='desktop_chart' id='desktop_chart' value='"+construct_ca_menu.document.Construct_ca_menu_form.desktop_chart.value+"'>";
...
{
	//alert("here1");
//construct_ca_menu.document.Construct_ca_menu_form.submit();
//loadTree();
//parent.frames[1].location.href = '../../eCA/jsp/ConstructCAMenuMain.jsp?module_id=CA&function_id=CONSTRUCTCAMENU&function_name=Construct CA Menu&function_type=F&access=YNNNN';
...
function dTree(objName) {
	...
	function loadTree()
	{
		...
		var main_menu_id = document.getElementById('Menu').options[obj].value;
