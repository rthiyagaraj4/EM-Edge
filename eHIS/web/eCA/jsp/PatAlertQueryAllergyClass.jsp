...
<%
	...
	<tr>
		<th align='left' nowrap colspan='6'><fmt:message key="eMR.AdeverseEventsAllgSens.label" bundle="${mr_labels}"/></th></th>
	</tr>
	<tr>
		...
		<%}else{%>
		<th width='15%'><div  id='head5' class='myClass' nowrap width='23%'><fmt:message key="Common.status.label" bundle="${common_labels}"/></div></th>
		<th width='15%'><div  id='head6' class='myClass' nowrap width='23%'><fmt:message key="Common.reason.label" bundle="${common_labels}"/></div></th>
		<%} //--[IN034975] - End
		%>
	</tr>
	<%
		...
		%>
			...
			<td  class='<%=classValue1%>' nowrap id='detail<%=m%>'><a class='classValue1' href="javascript:onclick=display_records1('<%=m%>','<%=patient_id%>','<%=causative_code%>','<%=adv_event_type_ind_code%>','<%=adv_event_type_code%>','<%=reaction_date1%>','<%=status_pass%>','<%=error_status%>','<%=onset_date%>')">&nbsp;-&nbsp;</a>
			</td>
			
		<%//}else{%>	
			
		<!--	<td  class='<%=classValue1%>' nowrap id='detail<%=m%>'>&nbsp;&nbsp;
			</td> -->
		...
		<%//}%>
			...
			<td   class='gridData' nowrap id='header'><a class='gridLink' href="javascript:onclick=display_header('<%=patient_id%>','<%=adv_event_type_ind_code%>','<%=adv_event_type_code%>','<%=causative_code%>','<%=adv_event_srl_no%>')">
			<%=allergen%>
			</td>
			...
			<td  class='gridData' nowrap ><%=status%></td>
				...
				<td class='gridData'  nowrap width= '15%'><%=error_remark%></td><%}%>
				</tr>
		<%//if(!error_status.equals("E")){
		%>
