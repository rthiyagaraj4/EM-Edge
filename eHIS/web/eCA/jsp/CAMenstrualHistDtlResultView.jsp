...
<title><fmt:message key="eCA.MenstrualHistoryDetails.label" bundle="${ip_labels}"/></title>
    ...
    // Assuming eventStatus has already been set based on the delivery_miscarrage_type and delivery_miscarrage_date
		...
		%>
                ...
                <span><%= eventStatus%></span>
       			 <a href="#" onclick="openErrorRemarksView('<%= patient_id %>','<%= srl_no %>')">R</a>
               <% } else { %>
               <span><%= eventStatus %></span>
              <% } %>
                 </td> <!-- Event Status -->
    			...
    			<% if ((remarks != null && !remarks.equals("")) || ("Auto Closed".equals(eventStatus))) { %>
       			 <a href="#" onclick="openRemarksPopup('<%=remarks %>','<%=event_remarks%>','<%=srl_no %>','<%= patient_id %>')">R</a>
   				 <% } else { %>
       				 &nbsp;
   					 <% } %>
				</td>
