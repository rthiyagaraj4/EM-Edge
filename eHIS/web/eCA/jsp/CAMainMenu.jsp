	...
	//detl_sql.append("  CONNECT BY PRIOR  HEADER_OPTION_ID =OPTION_ID and HEADER_OPTION_OCCURANCE=OPTION_OCCURANCE ) B   , CA_OPTION_LANG_VW C   WHERE C.LANGUAGE_ID = ?  AND A.OPTION_ID=B.OPTION_ID AND A.HEADER_OPTION_ID=B.HEADER_OPTION_ID AND A.HEADER_OPTION_OCCURANCE=B.HEADER_OPTION_OCCURANCE AND A.OPTION_OCCURANCE=B.OPTION_OCCURANCE AND C.OPTION_ID(+)=B.OPTION_ID   ORDER BY A.ORDER_SEQ_NO ");//52413
	  detl_sql.append("  CONNECT BY PRIOR  HEADER_OPTION_ID =OPTION_ID and HEADER_OPTION_OCCURANCE=OPTION_OCCURANCE ) B   , CA_OPTION_LANG_VW C   WHERE C.LANGUAGE_ID = ? AND B.OPTION_ID!='PAT_WITHOUT_ENCOUNT' AND A.OPTION_ID=B.OPTION_ID AND A.HEADER_OPTION_ID=B.HEADER_OPTION_ID AND A.HEADER_OPTION_OCCURANCE=B.HEADER_OPTION_OCCURANCE AND A.OPTION_OCCURANCE=B.OPTION_OCCURANCE AND C.OPTION_ID(+)=B.OPTION_ID   ORDER BY A.ORDER_SEQ_NO ");//52413
		...
		%>
			...
			%>
				obj.add('<%=option_id%>','<%=header_option_id%>','<%=option_desc%>','<%=executable_name%>','<%=function_name%>','workAreaFrame','','','','<%=function_type%>','<%=only_option_id%>');
			<%
				}
		...
		%>
			if('<%=limit_function_id%>' == "")
			{
				if('<%=homeValue%>' == "home"){
					...
					parent.parent.CommonToolbar.totMenuDisplay.value='<%=totMenuDisplay%>'; //IN0052635
					//IN058755 starts
					//parent.CommonToolbar.menuId.value='<%=menu_id%>'; //ICON_COUNT_ISSUE
					parent.CommonToolbar.homePageParam.menuId.value='<%=menu_id%>';
					//IN058755 ends
					//alert('<%=homeValue%>');
				}
				else{
				...
				parent.CommonToolbar.totMenuDisplay.value='<%=totMenuDisplay%>'; //IN0052635
				//IN058755 starts
				//parent.CommonToolbar.menuId.value='<%=menu_id%>'; //ICON_COUNT_ISSUE
				parent.CommonToolbar.homePageParam.menuId.value='<%=menu_id%>';
				//IN058755 ends
