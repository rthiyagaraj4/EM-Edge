...
String practitionerId ="", practitionerName = "", access_from_date = "", access_to_date = "", accessRightsIU="", access_res_pat_yn="", classValue="";
...
try{
	...
	if(dispMode.equals("")){
		...
		ArrayList  resArray = bean.getAccessRightsViewClinicalDataRestrictedPatients(facilityId,practitionerId,search_by,search_txt,locale);
		...
		hMapRecs.put("access_res_pat_yn"	, array.get(5));
		...
		for(int j=from_val; j<=to_val; j++){			
			...
			access_res_pat_yn 	=	request.getParameter("access_res_pat_yn"+j)==null?"N":request.getParameter("access_res_pat_yn"+j);
...
<form name='CAAccessRightsForViewRestrictPatResultform' id='CAAccessRightsForViewRestrictPatResultform' >
...
<tr height='19px'>
		...
		<th colspan="2"><fmt:message key="eCA.AccessRightsclinicaldataRestrictedPatients .label" bundle="${ca_labels}"/></th>
		<th rowspan="2"><input type="checkbox" id="allChecked" name="allChecked"  onClick="selectAll(this);" />
		</th>		
</tr>  
<tr>
...
<%
	for(int i=fm_disp; i<=to_disp; i++){
		...
		access_res_pat_yn		=	(String)	hMapRecords.get("access_res_pat_yn");
		
		if(access_from_date.equals(""))	
		{
			disb="disabled";
			disbImg="style='pointer-events: none;'";
		}
		else
		{
			disb="";
		...
		{
			disb1="disabled";
			disbImg1="style='pointer-events: none;'";
		}
		else
		{
			disb1="";
...
<td>&nbsp;&nbsp;&nbsp;&nbsp;<input type='textbox' id='access_from_date<%=i%>' name='access_from_date<%=i%>' value='<%=access_from_date%>' <%=disb%> onBlur="DateValidation(this,'access_from_date<%=i%>','<%=i%>');validate_date1(this,access_to_date<%=i%>);" size='14' maxlength='16' >&nbsp;<img id='imgfromDate<%=i%>' <%=disbImg%> src="../../eCommon/images/CommonCalendar.gif" onclick="return showCalendarValidate('access_from_date<%=i%>')"></img></td>
<td>
&nbsp;&nbsp;&nbsp;&nbsp;
<input type='textbox' id='access_to_date<%=i%>' name='access_to_date<%=i%>' value='<%=access_to_date%>' <%=disb1%> onBlur='toFromValidate(this,access_from_date<%=i%>);validate_date(this,access_from_date<%=i%>);' size='14' maxlength='16' >&nbsp;<img <%=disbImg1%> id='imgToDate<%=i%>' src="../../eCommon/images/CommonCalendar.gif" onclick="return showCalendarValidate('access_to_date<%=i%>')"></img>
&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:showAccessRightsHistory('<%=facilityId%>','<%=practitionerId%>','<%=practitionerName%>')">H</a>
</td>	
<td align="center"><input type="checkbox" id='access_res_pat_yn<%=i%>' name='access_res_pat_yn<%=i%>' onclick="selectChkBox(this,<%=i%>);enable_date('<%=i%>')" value="<%=access_res_pat_yn%>" <%if("Y".equals(access_res_pat_yn)){%> checked <%}%> /></td>
</tr>
		<input type='hidden' id='checkallDateEnable' name='checkallDateEnable' value="<%=i%>">	
	<%
	}
}
catch(Exception e)
