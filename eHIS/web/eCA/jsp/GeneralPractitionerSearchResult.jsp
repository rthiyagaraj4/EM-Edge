...
<%	}
			...
			<%} else {%>
			<th width='<%=plusSymbol_width%>' style='visibility:hidden' >+</th>
			...
			<th width='<%=other_fields_width%>' ><fmt:message key="Common.primaryspeciality.label" bundle="${common_labels}"/></th>
...
//out.println("<tr style='background-color:;'><td class='"+rowclass+"'>");	
	%>
	<input type='hidden' name="desc<%=i%>" value="<%=pract_name1%>" >
	<%
		if(dispDescFirst.equals("dispDescFirst")) {
		me=desc+"::"+code;
