...
<form name="ChartSummResult_Form" id="ChartSummResult_Form" action="ChartSummaryLabNotificationsD.jsp?<%=request.getQueryString()%>" method="post">
...
<%
		...
		*/
		...
		{
%>
		<!--<tr><td colspan='6' class='gridData'><a class='gridLink' href="javascript:callResults('<%=patient_id%>','<%=title%>','<%=reln_id%>','<%=enc_id%>','<%=facility_id%>','<%=content_ID%>','<%=p_called_from%>')"><fmt:message key="eCA.MoreDtlRef.label" bundle="${ca_labels}"/></a></td></tr>--> <!--added arguments facility_id and content_ID for IN038454, added p_called_from for IN037701 --> 
		<tr><td colspan='6' class='gridData'><a class='gridLink' href="javascript:callResults('<%=patient_id%>','<%=title%>','<%=reln_id%>','<%=enc_id%>','<%=facility_id%>','<%=content_ID%>','<%=p_called_from%>','<%=called_from%>','<%=RepDb%>','<%=clob_data%>')"><fmt:message key="eCA.MoreDtlRef.label" bundle="${ca_labels}"/></a></td></tr><!--IN065341-->
<%
		}
	}	
	catch(Exception e)
