...
public class LineGraphApplet extends Applet implements MouseMotionListener,
		MouseListener {
	boolean started;
	...
	public void mousePressed(MouseEvent mouseevent) {
		/*
		 * int k = mouseevent.getX(); int l = mouseevent.getY(); if(started) {
		 * for(int i1 = 0; i1 < IntArray[0]; i1++) { for(int j1 = 0; j1 <
		 * IntArray[3]; j1++) if(!valuenull[i1][j1] && k > PointArea[i1][j1][0] &&
		 * k < PointArea[i1][j1][1] && l > PointArea[i1][j1][3] && l <
		 * PointArea[i1][j1][2] && (PointURL[i1][j1][0] != null &&
		 * !PointURL[i1][j1][0].equals("")))
		 * if(PointURL[i1][j1][1].equals("javascript")) try {
		 * JSObject.getWindow(this).eval(PointURL[i1][j1][0]); } catch(Throwable
		 * _ex) { try { URL url = new URL(getDocumentBase(), "javascript:" +
		 * PointURL[i1][j1][0]); getAppletContext().showDocument(url); }
		 * catch(MalformedURLException malformedurlexception) {
		 * malformedurlexception.printStackTrace(); } } else try { URL url1 =
		 * new URL(getDocumentBase(), PointURL[i1][j1][0]);
	...
	public final void paint(Graphics g) {
		...
		linecolor[0] = linecolor[0].red.darker();
		linecolor[1] = linecolor[1].green.darker();
		linecolor[2] = linecolor[2].blue;
		linecolor[3] = linecolor[3].magenta;
		linecolor[4] = linecolor[4].black;
		linecolor[5] = linecolor[5].cyan;
		linecolor[6] = linecolor[6].orange;
		linecolor[7] = linecolor[7].pink;
		linecolor[8] = linecolor[8].yellow;
		linecolor[9] = linecolor[9].gray;

		if (!started)
			startUp(g);
		buffer = createImage(WIDTH, HEIGHT);
		pad = buffer.getGraphics();
		linechart.linechart(buffer, value, valuenull, displayValue, labels,
				labelsoffset, ImageXY, Images, chartStartY, chartScale,
				linecolor, PointStyle, PointFill, BoolArray, IntArray,
				StringArray, PosArray, FontArray, ColorArray, legendlabels,
				ttcolor, ttint, ttval, ttlabel, ttfont);
	...
	public void startUp(Graphics g) {
		String s = verifystr(getParameter("LOADINGMESSAGE"),
				"Loading Please Wait...");
		FontMetrics fontmetrics = getFontMetrics(g.getFont());
		g.setColor(verifyclr(getParameter("STEXTCOLOR"), "0,0,0", ","));
		g.drawString(s, (size().width - fontmetrics.stringWidth(s)) / 2,
		...
		CalcArea(IntArray[0], IntArray[3], IntArray[2], IntArray[4],
				IntArray[6], IntArray[5], chartScale, chartStartY);
		started = true;
	}
	...
	private void get_chartprops() {
		...
		for (i = 13; i < 15; i++)
			FontArray[i] = verifyfnt(getParameter("font" + (i + 1)),
					"Courier,N,12", ",");

		for (i = 13; i < 19; i++)
			ColorArray[i] = verifyclr(getParameter("color" + (i + 1)),
					"255,255,255", ",");

		ColorArray[19] = SBGCOLOR;
		ColorArray[20] = verifyclr(getParameter("LegendBackground"),
				"255,255,255", ",");
		...
		for (j = 0; j < IntArray[3]; j++) {
			...
			if (sTemp != null) {
				st = new StringTokenizer(sTemp, "|");
				if (st.hasMoreTokens())
					color = verifyclr(st.nextToken(), "0,0,125", ",");
				if (st.hasMoreTokens())
		...
		for (j = 0; j < IntArray[3]; j++)
			for (i = 0; i < IntArray[0]; i++) {
				...
				if ((sTemp != "") & (sTemp != null)) {
					st = new StringTokenizer(sTemp, ",");
					if (st.hasMoreTokens()) {
						value[i][j] = Double.valueOf(st.nextToken())
								.floatValue();
						valuenull[i][j] = false;
					}
					if (st.hasMoreTokens())
	...
	public void CalcArea(int k, int l, int i1, int j1, int k1, int l1,
			double d, double d1) {
		...
		for (int k2 = 0; k2 < k; k2++) {
			for (int l2 = 0; l2 < l; l2++) {
				...
				if (!valuenull[k2][l2]) {
					int j2 = k1
							- (int) Math.round((d2 * (value[k2][l2] - d1)) / d);
					int i2 = l1 + k2 * j1 + j1 / 2;
					PointArea[k2][l2][0] = i2 - PointStyle[l2][1] / 2;
					PointArea[k2][l2][1] = (i2 + PointStyle[l2][1])
							- PointStyle[l2][1] / 2;
					PointArea[k2][l2][2] = (j2 + PointStyle[l2][1])
							- PointStyle[l2][1] / 2;
	...
	public Image downloadImage(String s) {
		...
		pixs = new int[k * l];
		PixelGrabber pixelgrabber = new PixelGrabber(image, 0, 0, k, l, pixs,
				0, k);
		try {
	...
	public void init() {
		// new Evaluation(this, "2D / 3D Line Graph, version 3.2 - by
		// Jpowered.", "http://www.jpowered.com/line_graph/index.htm");
	...
	public static Font verifyfnt(String s, String s1, String s2) {
		...
		try {
			...
			else if (s4.equals("BI") || s4.equals("IB"))
	...
	private void get_chartpropsfromfile(String s) {
		...
		try {
			...
			for (Enumeration enumeration = parameters.elements(); enumeration
					.hasMoreElements();) {
				...
				if (!sTemp.startsWith("<!-")) {
					...
					for (i = 0; i < 10; i++)
						if (s1.equals("text" + (i + 1)) && s3 != ""
							...
							if (st.hasMoreTokens())
								FontArray[i] = verifyfnt(st.nextToken(),
										"Courier,N,10", ",");
							if (st.hasMoreTokens())
								ColorArray[i] = verifyclr(st.nextToken(),
										"0,0,0", ",");
						}
					...
					if (s1.equals("title") && s3 != "" && s3 != null) {
						...
						if (st.hasMoreTokens())
							FontArray[10] = verifyfnt(st.nextToken(),
									"Courier,N,10", ",");
						if (st.hasMoreTokens())
					...
					if (s1.equals("xtitle") && s3 != "" && s3 != null) {
						...
						if (st.hasMoreTokens())
							FontArray[11] = verifyfnt(st.nextToken(),
									"Courier,N,10", ",");
						if (st.hasMoreTokens())
					...
					if (s1.equals("ytitle") && s3 != "" && s3 != null) {
						...
						if (st.hasMoreTokens())
							FontArray[12] = verifyfnt(st.nextToken(),
									"Courier,N,10", ",");
						if (st.hasMoreTokens())
					...
					for (i = 0; i < 10; i++)
						if (s1.equals("trend" + (i + 1)) && s3 != ""
							...
							if (st.hasMoreTokens())
								ttlabel[i] = st.nextToken();
							if (st.hasMoreTokens())
								ttfont[i] = verifyfnt(st.nextToken(),
										"Arial,N,10", ",");
						}

					for (i = 10; i < 20; i++)
						if (s1.equals("target" + (i - 9)) && s3 != ""
								&& s3 != null) {
							st = new StringTokenizer(s3, "|");
							if (st.hasMoreTokens())
								ttcolor[i] = verifyclr(st.nextToken(), "0,0,0",
			...
			for (Enumeration enumeration1 = parameters.elements(); enumeration1
					.hasMoreElements();) {
				...
				if (!sTemp.startsWith("<!-")) {
					...
					for (i = 0; i < IntArray[0]; i++)
						if (s2.equals("series" + (i + 1)) && s4 != ""
								&& s4 != null) {
							st = new StringTokenizer(s4, "|");
							Color color = new Color(125, 125, 125);
							legendlabels[i] = "";
							if (st.hasMoreTokens())
								color = verifyclr(st.nextToken(),
										"125,125,125", ",");
							if (st.hasMoreTokens())
								PointStyle[i][0] = Integer.parseInt(st
										.nextToken());
							else
								PointStyle[i][0] = 0;
							if (st.hasMoreTokens())
								PointStyle[i][1] = Integer.parseInt(st
										.nextToken());
							else
								PointStyle[i][1] = 0;
							if (st.hasMoreTokens())
								PointFill[i] = verifybool(st.nextToken(),
										"false");
							else
								PointFill[i] = false;
							if (st.hasMoreTokens())
