 ...
 * 
 */
public class CachePermanentRegionInvalidator extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext jobContext)
		...
		logger.info(this.getClass(), jobName + " executing at " + new Date(),
				null);
		if ( (((CacheManager) ServiceLocator.getInstance().getBean("CacheManager"))).isRunning()) {
			(((CacheManager) ServiceLocator.getInstance().getBean("CacheManager"))).invalidateOtherRegion();
		} else
			logger.info(this.getClass(),
					"Cache service is down. Cleanup job failed.", null);
	}
}
