...
public class FDBDrugDuplicateTherapy
{

    public FDBDrugDuplicateTherapy(FDBDataManager fwConn)throws IOException
    {
        this.fwConn = fwConn;
    }
   ...
   public String getDuplicateDrugTherapyResult(ArrayList drugList,String prod_id) throws FDBException, Exception
    {
        screenDrugs = new ScreenDrugs();
      //  DispensableDrug drug = null;  Removed for IN063877
		drug_det= new HashMap();
		int pres_drug_count =0;
        for(int i = 0; i < drugList.size(); i++)
        {
			...
			if(prod_id.equals(drugList.get(i).toString()))
				pres_drug_count++;
        }
        setDuplicateDrugTherapyResult(prod_id, pres_drug_count);
		return sbDuplicateDescription.toString();
    }

    public void setDuplicateDrugTherapyResult(String prod_id, int pres_drug_count) throws FDBException, Exception
    {
		sbDuplicateDescription = new StringBuffer();
        Screening screening = new Screening(fwConn);
     //   DTScreenDrugItems drugItems = null;  Removed for IN063877
       ...
       if(messages!=null && messages.count() > 0)
        {
			...
			drugMap = new HashMap();
			HashMap drugClassNameMap = new HashMap();
			ArrayList classNameAL = null;
			String pres_drug_desc =(String)drug_det.get(prod_id);
			String className, drugName, drugClasses;
			for (int i = 0; i < resultCount; i++)
			{
				DTScreenResult scrngResult = dtScreenResults.item(i);
				className = scrngResult.getClassDescription();
				...
				for (int d=0; d<objDTScreenDrugItems.count(); d++)
				{
					...
					drugName = objDTScreenDrugItem.getDrugDescription();							
					drugClasses = (String)drugMap.get(drugName);
					...
					if(drugClassNameMap!=null && drugClassNameMap.containsKey(drugName)){
						classNameAL = (ArrayList)drugClassNameMap.get(drugName);
						classNameAL.add(className);
						drugClassNameMap.put(drugName,classNameAL);
					}
					else{
						classNameAL = new ArrayList();
						classNameAL.add(className);
						drugClassNameMap.put(drugName,classNameAL);
					}
			...
			if (duplicate_found == true)
			{
				...
				Set keyset = drugMap.keySet() ;
				Object[] keyslist = keyset.toArray();
				ArrayList tempClassNameAL = null;
				boolean dupClassFound = false;
				for (int i=0;i<keyslist.length ;i++ )
				{
					drugName = (String)keyslist[i];
					drugClasses = (String)drugMap.get(drugName);
					dupClassFound = false;

					 if (pres_drug_desc.equalsIgnoreCase(drugName) && pres_drug_count>1){ //Condition added for IN23988 --22/11/2010-- priya
						...
						if(drugClassNameMap!=null && drugClassNameMap.containsKey(drugName)){
							if(!pres_drug_desc.equalsIgnoreCase(drugName)){
								classNameAL = (ArrayList)drugClassNameMap.get(pres_drug_desc);
								tempClassNameAL = (ArrayList)drugClassNameMap.get(drugName);
								if(classNameAL!=null && tempClassNameAL!=null){
									for(int pDrugIndex=0; pDrugIndex< classNameAL.size(); pDrugIndex++){
										for(int dupDrugIndex=0; dupDrugIndex< tempClassNameAL.size(); dupDrugIndex++){
											if(((String)classNameAL.get(pDrugIndex)).equalsIgnoreCase(((String)tempClassNameAL.get(dupDrugIndex)))){
												dupClassFound=true;
												break;
											}
										}
...
*/
  ...
  //  private long drugid;  Removed for IN063877
