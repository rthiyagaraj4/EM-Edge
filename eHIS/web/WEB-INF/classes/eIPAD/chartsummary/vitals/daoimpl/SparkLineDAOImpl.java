...
public class SparkLineDAOImpl extends GenericDAOImpl implements SparkLineDAO{
	public SparkLineChartResponse getData(VitalsRequest oVitalsReq)
	{
		SparkLineChartResponse oSparkLineResponse = new SparkLineChartResponse();
		...
		List<SparkLineDM> lstSparkLineDM = new ArrayList<SparkLineDM>();
		...
		if(oVitalsReq != null && oVitalsReq.getChartId() != null && !oVitalsReq.getChartId().equals("") )
		{
			...
			//	ChartAxisContainer oChartAxisContainer;
				LineChartAxisContainer oLineChartAxisContainer;
				//ArrayList<ChartAxisConatiner> listChartAxisConatiner;
				//Map<SparkLineDM,ArrayList<GraphXAxis>> mapSparkLineDM = new LinkedHashMap<SparkLineDM,ArrayList<GraphXAxis>>();
				//Map<SparkLineDM,ChartAxisContainer> mapSparkLineDM = new LinkedHashMap<SparkLineDM,ChartAxisContainer>();
				Map<SparkLineDM,LineChartAxisContainer> mapSparkLineDM = new LinkedHashMap<SparkLineDM,LineChartAxisContainer>();
				...
				try
				{
					...
					if(resultSet != null)
					{
						...
						{
							SparkLineDM oSparkLineDM = new SparkLineDM();
							DiscreteValue oDiscreteValue = null ;//= new DiscreteValue();
							//ChartAxisContainer oChartAxisContainer = new ChartAxisContainer();
							GraphXAxis oGraphXAxis = new GraphXAxis();
							GraphYAxis oGraphYAxis = new GraphYAxis();
							
							oSparkLineDM.setEventCode(checkForNull(resultSet.getString("event_code")));
							oSparkLineDM.setEventDescription(checkForNull(resultSet.getString("event_desc")));
							oSparkLineDM.setNormalHigh(checkForNull(resultSet.getString("normal_high")));
							oSparkLineDM.setNormalLow(checkForNull(resultSet.getString("normal_low")));
							oSparkLineDM.setCriticalLow(checkForNull(resultSet.getString("critical_low")));
							oSparkLineDM.setCriticalHigh(checkForNull(resultSet.getString("critical_high")));
							oSparkLineDM.setUOM(checkForNull(resultSet.getString("uom")));
							oSparkLineDM.setHistRecType(checkForNull(resultSet.getString("hist_rec_type")));
							oSparkLineDM.setEventGroupDesc(checkForNull(resultSet.getString("event_group_desc")));
							...
							if(checkForNull(resultSet.getString("rankk")).equals("1"))
							{
								...
								oSparkLineDM.setCurrentDiscreteValue(oDiscreteValue);
							...
							if(mapSparkLineDM.containsKey(oSparkLineDM))
							{
								//mapSparkLineDM.get(oSparkLineDM).add(oGraphXAxis);
								mapSparkLineDM.get(oSparkLineDM).getoGraphXAxisList().add(oGraphXAxis);
								mapSparkLineDM.get(oSparkLineDM).getoGraphYAxisList().add(oGraphYAxis);
								/*
								if(oDiscreteValue != null) // to assign the current discrete value to the sparkline object present in the map.
								{
									LineChartAxisContainer objLineChartAxisContainer = mapSparkLineDM.get(oSparkLineDM);
									//mapSparkLineDM.put(oSparkLineDM, objLineChartAxisContainer);
									mapSparkLineDM.remove(oSparkLineDM);
									mapSparkLineDM.put(oSparkLineDM, objLineChartAxisContainer);
							...
							{
								//oChartAxisContainer = new ChartAxisContainer();
								...
								mapSparkLineDM.put(oSparkLineDM, oLineChartAxisContainer);
							}
						}//while end
							...
							Iterator<Entry<SparkLineDM,LineChartAxisContainer>> iter = mapSparkLineDM.entrySet().iterator();
							if(iter != null)
							{
								while (iter.hasNext()) {
									Map.Entry<SparkLineDM,LineChartAxisContainer> mEntry = (Map.Entry<SparkLineDM,LineChartAxisContainer>)iter.next();
									SparkLineDM oSparkLineDMEntry = mEntry.getKey();
									LineChartAxisContainer oLineChartAxisContainerEntry = mEntry.getValue();
									//ArrayList<GraphXAxis> oGraphXAxisListEntry = mEntry.getValue();
									//oSparkLineDMEntry.setLstXAxisValues(oGraphXAxisListEntry);
									//oSparkLineDMEntry.setLstXAxisValues(oChartAxisContainerEntry.getoGraphXAxisList());
									//oSparkLineDMEntry.setSparkLineAxisContainer(oLineChartAxisContainerEntry);
									oSparkLineDMEntry.setLineChartAxisContainer(oLineChartAxisContainerEntry);
									lstSparkLineDM.add(oSparkLineDMEntry);
								}
							}
						//}//while end
					}
					...
					oSparkLineResponse.setLstSparkLineDM(lstSparkLineDM);
				}
				catch(Exception e)
				{
					e.printStackTrace();
