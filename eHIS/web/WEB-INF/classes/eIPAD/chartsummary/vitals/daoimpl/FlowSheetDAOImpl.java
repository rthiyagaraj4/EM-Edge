...
public class FlowSheetDAOImpl extends GenericDAOImpl implements FlowSheetDAO {
	public FlowSheetResponse getFlowSheetValues(VitalsRequest oVitalsReq)
	{
		FlowSheetResponse oFlowSheetResponse = new FlowSheetResponse();
		...
		List<FlowSheetDM> lstFlowSheetDM = new ArrayList<FlowSheetDM>();
		List<FlowSheetData> flowSheetDataList = new ArrayList<FlowSheetData>();
		...
		if(oVitalsReq != null && oVitalsReq.getChartId() != null && !oVitalsReq.getChartId().equals("") )
		{
			...
			if(oPatContext != null)
			{
				...
				Map<FlowSheetDM,ArrayList<DiscreteValue>> discreteMeasureMap = new LinkedHashMap<FlowSheetDM, ArrayList<DiscreteValue>>();
				...
				try
				{
					...
					if(resultSet != null)
					{
						...
						{
							FlowSheetDM oFlowSheetDM = new FlowSheetDM();
							DiscreteValue oDiscreteValue = new DiscreteValue();
							//FlowSheetData oFlowSheetData = new FlowSheetData();
							
							
							oFlowSheetDM.setEventCode(checkForNull(resultSet.getString("event_code")));
							oFlowSheetDM.setEventDescription(checkForNull(resultSet.getString("event_desc")));
							oFlowSheetDM.setNormalHigh(checkForNull(resultSet.getString("normal_high")));
							oFlowSheetDM.setNormalLow(checkForNull(resultSet.getString("normal_low")));
							oFlowSheetDM.setCriticalLow(checkForNull(resultSet.getString("critical_low")));
							oFlowSheetDM.setCriticalHigh(checkForNull(resultSet.getString("critical_high")));
							oFlowSheetDM.setUOM(checkForNull(resultSet.getString("uom")));
							oFlowSheetDM.setHistRecType(checkForNull(resultSet.getString("hist_rec_type")));
							oFlowSheetDM.setEventGroupDesc(checkForNull(resultSet.getString("event_group_desc")));
							...
							//oFlowSheetData.setEventDateTime(oDiscreteValue.getEventDateTimestamp());
							//flowSheetDataList.add(oFlowSheetData);
							if(!flowSheetDataList.contains(oDiscreteValue.getEventDateTimestamp()))
							{
								FlowSheetData oFlowSheetData = new FlowSheetData();
								oFlowSheetData.setEventDateTime(oDiscreteValue.getEventDateTimestamp());
								flowSheetDataList.add(oFlowSheetData);
							...
							{
								oFlowSheetDM.setCurrentDiscreteValue(oDiscreteValue);
							}
							if(discreteMeasureMap.containsKey(oFlowSheetDM))
							{
								discreteMeasureMap.get(oFlowSheetDM).add(oDiscreteValue);
							}
							else
							{
								discreteValueList = new ArrayList<DiscreteValue>();
								discreteValueList.add(oDiscreteValue);
								discreteMeasureMap.put(oFlowSheetDM, discreteValueList);
							}
						...
						Iterator<Entry<FlowSheetDM,ArrayList<DiscreteValue>>> iter = discreteMeasureMap.entrySet().iterator();
						if(iter != null)
						{
							while (iter.hasNext()) {
								Map.Entry<FlowSheetDM,ArrayList<DiscreteValue>> mEntry = iter.next();
								FlowSheetDM oFlowSheetDMEntry = mEntry.getKey();
								ArrayList<DiscreteValue> discreteValueListEntry = mEntry.getValue();
								oFlowSheetDMEntry.setDiscreteValueList(discreteValueListEntry);
								lstFlowSheetDM.add(oFlowSheetDMEntry);
								
							}
						}
					}
					if(lstFlowSheetDM != null && lstFlowSheetDM.size() == 0)
					{
		...
		oFlowSheetResponse.setFlowSheetDMList(lstFlowSheetDM);
		oFlowSheetResponse.setFlowSheetDataList(flowSheetDataList);
