...
/**
 * Servlet implementation class RecordConsentInitPatContext
 */
public class RecordConsentInitPatContext extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RecordConsentInitPatContext() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		bean.setLanguageId(patientContext.getLocale());
		bean.clear() ;
		bean.setMode( mode ) ;
		bean.clearHash();
		
		PersistenceHelper.putObjectInBean(bean_id,bean,request);
			RequestDispatcher rd = session.getServletContext().getRequestDispatcher("/eIPAD/jsp/chartwidgets/recordconsent/RecordConsentBasePage.jsp");
			rd.forward(request, response);	
	}
