...
public class EncounterSummaryXMLHTML {
	...
	public static StringBuffer getXMLHtml(String xmlData) {
	...
	public static void dataList(Element Node){	
		...
		//try {
			...
			if (Node.getName().equalsIgnoreCase("group")){
				//htmlData.append(brStr);
				subGroupCount = 0;
				subGroupTotal = Node.getChildren("SubGroup").size();
			}				
			...
			templateInt = 0;
	   		...
	   		for (int k=0;k<childList.size();k++){
				...
				//templateInt = 0; //Commented by Sethu on 21/04/2014
	   			...
	   			if (childNode.getName().equalsIgnoreCase("subgroup")){
	   				kSubGroupFlag = true;
	   				subGroupData = null;
	...
	public static String[][] returnTable(Element Node, int colCount){
		...
		if (columnTypeList.size() > 0)		
			tableArray = new String[2+rowList.size()][colCount+1];
		else
			tableArray = new String[1+rowList.size()][colCount+1];
...
public static void drawMainHead(String[][] main, int mGroupCount){
	...
	public static void drawTable(Element Node, String[][] text, int template){
		...
		if (text.length>0){		
			...
			for (int i=0;i<text.length-2;i++){
				...
				for (int j=0;j<text[i+2].length-1;j++){					
					...
					if (text[0][j].equalsIgnoreCase("v")){
						htmlData.append("<td class=\"thickBorder\" style=\"border-bottom: "+text[i+2][text[i+2].length-1]+" 3px solid; text-align: center\"  valign=\"center\" width=125px height=30px>");
					}
					...
					if (j==2){
						htmlData.append("<td class=\"thickBorder\" style=\"border-bottom: "+text[i+1][text[i+1].length-1]+" 3px solid; text-align: center\"  valign=\"center\" width=125px height=30px>");
					}
					if (j==3){
						htmlData.append("<td class=\"thickBorder\" style=\"text-align: center\" valign=\"center\" width=150px height=30px>");
					}
		...
		if (values.length>0){			
			...
			htmlData.append(tdValignStr); //changes made here  
				...
				htmlData.append(trStrLC);
					...
					htmlData.append("<td width=\"25px\" height=\"25px\" align=\"right\" style=\"border:5px solid "+values[0][1]+";\" text-align:center\">");
					htmlData.append(strongStr);
	...
	public static String[][] returnLGraphRangeValues(Element Node){
		...
		for (int t=0;t<rangeList.size();t++){
			...
			lineRangeArray[t][5] = rangeNode.getAttributeValue("yAxisLabel");				
		}
