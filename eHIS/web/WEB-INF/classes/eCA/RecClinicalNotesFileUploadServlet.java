...
public class RecClinicalNotesFileUploadServlet extends HttpServlet
{
...
response.addHeader("X-Content-Type-Options", "nosniff"); //MMS-ME-SCF-0085
				...
				previewMode				=	(String)		request.getParameter("previewMode");
	...
	public void NotesFileUpload(HttpServletRequest request, HttpServletResponse response) throws FileUploadException, java.io.IOException
	{
		...
		if((docName.equals("") && record_status== false) || (!docName.equals("") && record_status== true))
		{
			...
			catch(Exception e)
			{
				e.printStackTrace();
				record_status = false;
				error.append("<br>Exception @ RecClinicalNotesSectionsFileUpload while checking for the file size"+e);
			}
			finally
			{
				if(rs!=null)
			...
			{*/
				...
				if(operation_mode.equals("Record") || operation_mode.equals("UpdateRecord"))
				{
					uploadresults =	recordNotesFileUpload(request,response);
				}
				else if(operation_mode.equals("Sign") || operation_mode.equals("UpdateSign"))
				{
					uploadresults = signRecClinicalNotesFileUpload(request,response);
				}
				else if(operation_mode.equals("Delete"))
				{
					uploadresults = deleteRecClinicalNotesFileUpload(request,response);
				}
			...
			response.sendRedirect("../eCA/jsp/RecClinicalNotesFileUploadError.jsp?err_num=" + java.net.URLEncoder.encode( error.toString() ,"UTF-8") +"&err_value="+error_value+"&operation_mode="+operation_mode+"&accession_num="+accession_num+"&called_from="+called_from);
		}
		catch(Exception e)
		{
			e.printStackTrace();
			//out.println("Exception @ NotesFileUpload "+e.toString());//commom//common-icn-0181
		}

	}


	public java.util.HashMap recordNotesFileUpload(HttpServletRequest request, HttpServletResponse response) throws FileUploadException, java.io.IOException
	{
			...
			try
			{
				if(accession_num.equals(""))
				{
					...
					catch(Exception e)
					{
						e.printStackTrace();
						commit_flag = false;
						strbuf.append("<br>Exception@ RecClinicalNotesFileUploadServlet - Inserting note header "+e);
					}
					finally
					{
				...
				else
				{
					try
					{
						...
						if(operation_mode.equals("UpdateRecord") || operation_mode.equals("UpdateSign") ||  operation_mode.equals("Sign"))
						{

							if(operation_mode.equals("UpdateRecord"))
							{
								update_ca_encntr_note = "update ca_encntr_note set authorized_by_id = ?, authorized_date_time = to_char(?,'mm/dd/yyyy hh24:mi'),  last_amended_by_id=? , last_amended_date_time=SYSDATE, modified_by_id = ?, modified_date = SYSDATE, modified_at_ws_no = ?, modified_facility_id = ?, event_title_desc= ?, service_code = ? where facility_id = ? and accession_num = ? ";
							}
							else if(authorized_date_time.equals("SYSDATE") && (!operation_mode.equals("UpdateRecord")))
							{
							...
							{
								update_ca_encntr_note = "update ca_encntr_note set authorized_by_id = ?, authorized_date_time = to_char(?,'mm/dd/yyyy hh24:mi'), event_status_caption = ?,  event_status = ?, last_amended_by_id=? , last_amended_date_time=SYSDATE, modified_by_id = ?, modified_date = SYSDATE, modified_at_ws_no = ?, modified_facility_id = ?, event_title_desc= ?, service_code = ? where facility_id = ? and accession_num = ? ";
							}
							...
							pstInsEnctrNote = con.prepareStatement(update_ca_encntr_note);
							if(operation_mode.equals("UpdateRecord"))
							{
								pstInsEnctrNote.setString(1,authorized_by_id);
								pstInsEnctrNote.setString(2,authorized_date_time);
								pstInsEnctrNote.setString(3,authorized_by_id);
								pstInsEnctrNote.setString(4,login_user_id);
								pstInsEnctrNote.setString(5,login_at_ws_no);
								pstInsEnctrNote.setString(6,facilityId);
								pstInsEnctrNote.setString(7,event_title_desc);
								pstInsEnctrNote.setString(8,service_code);
								pstInsEnctrNote.setString(9,facilityId);
								pstInsEnctrNote.setString(10,accession_num);
							}
							else
								if(authorized_date_time.equals("SYSDATE") && (!operation_mode.equals("UpdateRecord")))
								{
									pstInsEnctrNote.setString	(1,authorized_by_id);
									pstInsEnctrNote.setString	(2,event_status_caption);
									pstInsEnctrNote.setString	(3,event_status);
									pstInsEnctrNote.setString	(4,authorized_by_id);
									pstInsEnctrNote.setString	(5,login_user_id);
									pstInsEnctrNote.setString	(6,login_at_ws_no);
									pstInsEnctrNote.setString	(7,facilityId);
									pstInsEnctrNote.setString	(8,event_title_desc);
									pstInsEnctrNote.setString	(9,service_code);
									pstInsEnctrNote.setString	(10,facilityId);
									pstInsEnctrNote.setString	(11,accession_num);
								}
								else
								{
									pstInsEnctrNote.setString	(1,authorized_by_id		);
									pstInsEnctrNote.setString	(2,authorized_date_time	);
									pstInsEnctrNote.setString	(3,event_status_caption	);
									pstInsEnctrNote.setString	(4,event_status			);
									pstInsEnctrNote.setString	(5,authorized_by_id		);
									pstInsEnctrNote.setString	(6,login_user_id			);
									pstInsEnctrNote.setString	(7,login_at_ws_no			);
									pstInsEnctrNote.setString	(8,facilityId				);
									pstInsEnctrNote.setString	(9,event_title_desc		);
									pstInsEnctrNote.setString	(10,service_code			);
									pstInsEnctrNote.setString	(11,facilityId				);
									pstInsEnctrNote.setString	(12,accession_num			);
								}
								...
								catch(Exception e)
								{
										e.printStackTrace();
										commit_flag = false;
										strbuf.append("<br>Exception@ RecClinicalNotesFileUploadServlet - updating the ca_enctnr_note"+e);
								}
								finally
								{
									if(pstInsEnctrNote!=null)
							...
							*/
							...
							/* if( (operation_mode.equals("UpdateRecord") && change_file_ref.equals("N") )  || (operation_mode.equals("UpdateSign") && change_file_ref.equals("N")) && insert_result > 0 && commit_flag ==true)
							{
								try
								{
									String insert_enctr_dtl_obj = "update CR_ENCOUNTER_DETAIL_OBJ  set EVENT_TITLE = ? where HIST_REC_TYPE=? AND CONTR_SYS_ID=? AND CONTR_SYS_EVENT_CODE = ? and ACCESSION_NUM = ?";
										...
										pstEnctrDtlObj.setString(5,accession_num);
								...
								catch(Exception ex)
								{
										ex.printStackTrace();
										commit_flag = false;
										strbuf.append("<br>Exception@ RecClinicalNotesFileUploadServlet - updating the remarks"+ex);
								}
								finally
								{
									if(pstEnctrDtlObj!=null)
							...
							else if((operation_mode.equals("UpdateRecord") && change_file_ref.equals("Y") && insert_result > 0)|| (operation_mode.equals("UpdateSign") && change_file_ref.equals("Y"))   || operation_mode.equals("Sign"))
							{
							...
							} */ // end of the else condition operation_mode.equals("UpdateRecord") || operation_mode.equals("UpdateSign") ) )
						}
					...
					catch(Exception e)
					{
						e.printStackTrace();
						commit_flag = false;
						strbuf.append("<br>Exception @ RecClinicalNotesFileUploadManager: while updating the blob value : "+e);
					}
				}
	...
	private boolean caBuildNotes(String facilityId, String accessionNum,Connection con)
	{
			...
			//String strTempSel = ca_labels.getString("eCA.MedAncService.label");
	...
	public java.util.HashMap signRecClinicalNotesFileUpload (	HttpServletRequest request, HttpServletResponse response) throws FileUploadException, java.io.IOException
	{
			java.util.HashMap results	=	new java.util.HashMap() ;
			...
			try
			{
				...
				if(accession_num.equals("") || (operation_mode.equals("UpdateSign")))
				{
					try
					{
						java.util.HashMap record_results	=	recordNotesFileUpload(request, response);
						boolean record_status				=	((Boolean)record_results.get("status")).booleanValue();
						...
						{
							commit_flag = false;
							strbuf.append((String)record_results.get("error"));
						}
					...
					{
						e.printStackTrace();
						strbuf.append("<br>Exception@ RecClinicalNotesFileUploadManager sign -1: "+e);
					}
				} // end of if(accession_num.equals(""))
				...
				if(commit_flag)
				{
					...
					if(privilege_type.equals("1"))
					{
						...
						catch(Exception e)
						{
                            e.printStackTrace();
							commit_flag = false;
							strbuf.append("<br>Exception@ RecClinicalNotesFileUploadManager sign privilege_type 1:1 : "+e);
						}
						finally
						{
							if(pstmt!=null)
						...
						if(insert_result > 0 )
						{
							try
							{
								insert_ca_encntr_note_audit_log = "insert into ca_encntr_note_audit_log ( facility_id, accession_num, action_date_time, action_type, action_status, action_by_id, added_by_id, added_date, added_at_ws_no, added_facility_id, modified_by_id, modified_date, modified_at_ws_no, modified_facility_id ) values ( ?, ?, to_date(?,'dd/mm/yyyy HH24:MI'), 'TR', 'CO', ?, ?, SYSDATE, ?, ?, ?, SYSDATE, ?, ? ) ";
							...
							catch(Exception e)
							{
								e.printStackTrace();
								commit_flag = false;
								strbuf.append("<br>Exception@ RecClinicalNotesFileUploadManager sign:privilege_type 1:2 "+e);
							}
							finally
							{
								if(pstmt!=null)
							...
							if( insert_result > 0 )
							{
								try
								{
									insert_ca_encntr_note_audit_log = "insert into ca_encntr_note_audit_log ( facility_id, accession_num, action_date_time, action_type, action_status, action_by_id, request_date_time, request_by_id, added_by_id, added_date, added_at_ws_no, added_facility_id, modified_by_id, modified_date, modified_at_ws_no, modified_facility_id ) values ( ?, ?, SYSDATE, 'SN', 'RQ', ?, SYSDATE, ?, ?, SYSDATE, ?, ?, ?, SYSDATE, ?, ? ) ";

									pstmt	=	con.prepareStatement(insert_ca_encntr_note_audit_log);

									pstmt.setString	(	1,	facilityId				);	// facilityId
									pstmt.setString	(	2,	accession_num			);
									pstmt.setString	(	3,	performed_by_id			);
									pstmt.setString	(	4,	clinician_id			);
									pstmt.setString	(	5,	login_user_id			);
									pstmt.setString	(	6,	login_at_ws_no			);
									pstmt.setString	(	7,	facilityId				);
								...
								catch(Exception e)
								{
									e.printStackTrace();
									commit_flag = false;
									strbuf.append("<br>Exception@ RecClinicalNotesFileUploadManager sign : privilege_type 1:3 "+e);
								}
								finally
								{
									if(pstmt!=null)
					...
					else if(privilege_type.equals("2"))
					{
						...
						if(forwarded_clinician_id.equals("") || action_comment.equals("") )
						{
							event_status			=	"2";
							//commit_flag				=	false;
							//message					=	msgmngr.getMessage( con, "FORWARD_CLINICIAN_REQUIRED" ) ;

							//strbuf.append( (String) message.get("message") ) ;
							strbuf.append("FORWARD_CLINICIAN_REQUIRED") ;
						} // end of if(forwarded_clinician_id.equals(""))
						else
						{
							event_status			=	"3";
							...
							catch(Exception e)
							{
								e.printStackTrace();
								commit_flag = false;
								strbuf.append("<br>Exception@signOn: privilege_type 2:1 "+e);
							}
							finally
							{
								if(pstmt!=null)
							...
							if(update_result<=0)
							{
								...
								catch(Exception e)
								{
									e.printStackTrace();
									commit_flag = false;
									strbuf.append("<br>Exception@sign : privilege_type 2:2  "+e);
								}
								finally
								{
									if(pstmt!=null)
								...
								if( insert_result > 0 )
								{
									try
									{
										insert_ca_encntr_note_audit_log = "insert into ca_encntr_note_audit_log ( facility_id, accession_num, action_date_time, action_type, action_status, action_by_id, added_by_id, added_date, added_at_ws_no, added_facility_id, modified_by_id, modified_date, modified_at_ws_no, modified_facility_id ) values ( ?, ?, SYSDATE, 'SN', 'CO', ?, ?, SYSDATE, ?, ?, ?, SYSDATE, ?, ? ) ";
									...
									catch(Exception e)
									{
										e.printStackTrace();
										commit_flag = false;
										strbuf.append("<br>Exception@sign:  privilege_type 2:3 "+e);
									}
									finally
									{
										if(pstmt!=null)
							...
							try
							{
								insert_ca_encntr_note_audit_log = "insert into ca_encntr_note_audit_log ( facility_id, accession_num, action_date_time, action_type, action_status, action_by_id, request_date_time, request_by_id, action_comment, added_by_id, added_date, added_at_ws_no, added_facility_id, modified_by_id, modified_date, modified_at_ws_no, modified_facility_id ) values ( ?, ?, NULL, 'SN', 'RQ', ?, SYSDATE, ?, ?, ?, SYSDATE, ?, ?, ?, SYSDATE, ?, ? ) ";
								pstmt	=	con.prepareStatement(insert_ca_encntr_note_audit_log);

								pstmt.setString	(	1,	facilityId				);	// facilityId
								pstmt.setString	(	2,	accession_num			);
								pstmt.setString	(	3,	forwarded_clinician_id	);
								pstmt.setString	(	4,	performed_by_id			);
								pstmt.setString	(	5,	action_comment			);
							...
							catch(Exception e)
							{
								e.printStackTrace();
								commit_flag = false;
								strbuf.append("<br>Exception@sign: privilege_type 2:4 "+e);
							}
							finally
							{
								if(pstmt!=null)
					...
					else if(privilege_type.equals("3") || privilege_type.equals("4") )
					{
						...
						catch(Exception e)
						{
							e.printStackTrace();
							commit_flag = false;
							strbuf.append("<br>Exception@sign: privilege_type 3:1"+e);
						}
						finally
						{
							if(pstmt!=null)
						...
						if(update_result<=0)
						{
							...
							catch(Exception e)
							{
								e.printStackTrace();
								commit_flag = false;
								strbuf.append("<br>Exception@sign: privilege_type 3:2 "+e);
							}
							finally
							{
								if(pstmt!=null)
							...
							if( insert_result > 0 )
							{
								...
								catch(Exception e)
								{
									e.printStackTrace();
									commit_flag = false;
									strbuf.append("<br>Exception@sign:privilege_type 3:3 "+e);
								}
								finally
								{
									if(pstmt!=null)
						...
						}//end of else if( privilege_type.equals("3") || privilege_type.equals("4") )
					} // end of if(commit_flag)
				...
				if((!(accession_num.equals("")) && commit_flag == true) || operation_mode.equals("UpdateSign"))
				{
					try
					{
					//	hmNotesFileupload.put(	"authorized_by_id",		  authorized_by_id		   );
					//	hmNotesFileupload.put(	"authorized_date_time", authorized_date_time);
					//	hmNotesFileupload.put(	"event_status",			      event_status			   );
					...
					catch(Exception e)
					{
						e.printStackTrace();
						commit_flag = false;
						strbuf.append("<br>Exception@sign: ass_num ! = EMPTY" +e);
					}
				}//END OF if(!(accession_num.equals("")))
			}// END OF MAIN TRY
			catch(Exception e)
			{
				commit_flag = false;
				strbuf.append("<br>Exception@signCN-12: "+e);
			}
			finally
			{
				try
				...
				{
					e.printStackTrace();
					strbuf.append("<br>Exception@signCN-13: "+e);
					...
					try
					{
						...
						e1.printStackTrace();
						strbuf.append("<br>Exception@signCN while rollingback: "+e1);
					}
				}
				finally
				{
	...
	//end of signRecClinicalNotesFileUpload

	public java.util.HashMap deleteRecClinicalNotesFileUpload(HttpServletRequest request, HttpServletResponse response)
	{
		...
		try
		{
			...
			pstEnctrNoteAudLog	=	con.prepareStatement(delete_ca_encntr_note_audit_log);

			pstEnctrNoteAudLog.setString	(1,	facilityId		);
			pstEnctrNoteAudLog.setString	(2,	accession_num	);
			delete_result = pstEnctrNoteAudLog.executeUpdate();

			if(pstEnctrNoteAudLog!=null) pstEnctrNoteAudLog.close();

			String delete_ca_encntr_note = "delete ca_encntr_note where facility_id = ? and accession_num = ? ";

			pstEnctrNoteAudLog	=	con.prepareStatement(delete_ca_encntr_note);
			pstEnctrNoteAudLog.setString	(1,	facilityId		);
			pstEnctrNoteAudLog.setString	(2,	accession_num	);
			delete_result = pstEnctrNoteAudLog.executeUpdate();

			if(pstEnctrNoteAudLog!=null)
					pstEnctrNoteAudLog.close();

			if(delete_result > 0)
			{
				String delete_cr_encounter_detail_obj = "delete cr_encounter_detail_obj  where HIST_REC_TYPE=? AND CONTR_SYS_ID=? AND accession_num = ? ";
				pstEnctrNoteAudLog	=	con.prepareStatement(delete_cr_encounter_detail_obj);

				pstEnctrNoteAudLog.setString	(1,	"CLNT"	);
				pstEnctrNoteAudLog.setString	(2,	"CA"	);
				pstEnctrNoteAudLog.setString	(3,	accession_num	);
			   delete_result = pstEnctrNoteAudLog.executeUpdate();
		...
		catch(Exception e)
		{
			e.printStackTrace();
			commit_flag = false;
			strbuf.append("<br>Exception@deleteCN-1: "+e);
		}
		finally
		{
			try
			...
			{
				e.printStackTrace();
				strbuf.append("<br>Exception@deleteCN-2: "+e);
				try
				{
					con.rollback();
				}
		...
		results.put( "error", strbuf.toString() ) ;
		return results ;
	} // end of deleteRecClinicalNotesFileUpload
	...
	private void preview(HttpServletRequest req, HttpServletResponse res, PrintWriter out)throws FileUploadException, java.io.IOException
    {
		...
		try{
			...
			if(fcre || flag )
			{
				...
				out.println("var doc_type='"+docType+"';");

				/*   Modified by kishore kumar N on 13/01/2010 for SRR200910-CRF-0500,
					[To dispaly window title]
					RecClinicalNotesFileUploadShowDoc.jsp to RecClinicalNotesFileUploadShowDocMain.jsp
				*/
				out.println("var filename='../eCA/jsp/RecClinicalNotesFileUploadShowDocMain.jsp?previewMode=Y&image_name='+escape(imageName)+'&docType='+doc_type;");
				/*ends here.*/
				...
				//out.println("top.content.frames(1).messageFrame.location.href='../eCommon/jsp/error.jsp';");
				out.println("</script>");
