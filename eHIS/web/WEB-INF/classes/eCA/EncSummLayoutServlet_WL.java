...
public class EncSummLayoutServlet_WL extends javax.servlet.http.HttpServlet 
{
	...
	public void insert(HttpServletRequest request, HttpServletResponse response, PrintWriter out)
	{
		...
		int es_footer_sign_flag = 0, es_footer_sign_posn = 0, es_footer_digital_sig = 0; 
		...
		Clob headerValueclob = null;
		Clob footerValueclob = null;
		...
		try 
		{
			...
			headerValueclob = con.createClob();
			footerValueclob = con.createClob();
			...
			headerValueclob.setString(1, headerValue);
			footerValueclob.setString(1, footerValue);
			...
			if (!finalString.equals("")) 
			{
				...
				{					
					...
					pstmt_upd_hdr.setClob(19, headerValueclob);
					pstmt_upd_hdr.setClob(20, footerValueclob);
					...
					else// if above condition fails then the Values are roll backed from the Database
					{
						...
						{
							out.println("Exception in Main Block  : "+ ee.toString());
							ee.printStackTrace();
						}
					}
				}
		...
		try 
		{
			...
			if (!headerValue.equals("") || !footerValue.equals("")) 
			{
				...
				{					
					...
					if (footerValue.equals("update")){
						pstmt_upd_hdr.setClob(1, headerValueclob);
					}
					else
					{
						pstmt_upd_hdr.setClob(1, footerValueclob);
					}
