...
public class CAContSpecialtyPositionPatListByLocServlet extends javax.servlet.http.HttpServlet
{
	/** Multi Record Code **/
	public void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
		...
		//String locale = p.getProperty("LOCALE");//54391
		...
		try{
			...
			if(insertStuAllHashMap.size() >0){
				String insertStuAllQuery = "INSERT INTO CA_CNTRL_SPL_POSITION_BY_LOC (POSITION_CODE,POSITION_DESC,POSITION_RESTR_YN,ADDED_BY_ID,ADDED_DATE,ADDED_AT_WS_NO,ADDED_FACILITY_ID,MODIFIED_BY_ID,MODIFIED_DATE,MODIFIED_AT_WS_NO,MODIFIED_FACILITY_ID,FACILITY_ID,PRACT_TYPE) VALUES(?, ?, ?,?,SYSDATE,?,?,?,SYSDATE,?,?,?,?)";
				try
				{
			...
			if(updateStuAllHashMap.size() >0){
				String updateStuAllQuery = "UPDATE CA_CNTRL_SPL_POSITION_BY_LOC SET POSITION_RESTR_YN=?,MODIFIED_BY_ID=?,MODIFIED_DATE=SYSDATE,MODIFIED_AT_WS_NO=?,MODIFIED_FACILITY_ID=? WHERE POSITION_CODE=? AND PRACT_TYPE=?";
				try
				{
			...
			if(deleteStuAllHashMap.size() >0){
				String deleteStuAllQuery = "DELETE FROM CA_CNTRL_SPL_POSITION_BY_LOC WHERE POSITION_CODE=? AND FACILITY_ID=? AND PRACT_TYPE=?";
				try
				{
