...
import eCA.SectionChartBean;
public class SectionChartServlet extends javax.servlet.http.HttpServlet	{
	...
	{   HttpSession session = request.getSession(false);
		...
		{   request.setCharacterEncoding("UTF-8"); 
			...
			{   p = (java.util.Properties)session.getValue( "jdbc" ) ;
				...
				ps1 = con.prepareStatement("delete from ca_chart_section where CHART_ID = ? ");
				ps1.setString(1,chartId);
				ps1.executeUpdate();
			...
			{
				//out.println("Exception raise by Sections For  Chart Servlet ...So Record Not inserted  : "+ee.toString());//common-icn-0181
				ee.printStackTrace();
			}
			finally
			{
...
*/
	public void insertModify(HttpServletRequest request,HttpServletResponse response,int arrayLength,Connection con)
	{
		...
		for(index = 0;index < arrayLength;index++)
		{
			...
			sectionCode[index] =  request.getParameter("sectionCode"+index);
		}
		try
		{
			out = response.getWriter();
			...
			while(token.hasMoreTokens())
				{
					bean.deleteRow(token.nextToken(),con,chartId,p);
				}
			}
			...
			sqlUpdate = "update ca_chart_section set  DISPLAY_ORDER_SEQ=?, MODIFIED_BY_ID=?, MODIFIED_DATE=sysdate, MODIFIED_AT_WS_NO=?, MODIFIED_FACILITY_ID=? where CHART_ID= ? and  PANEL_ID =?";
			sqlSelect = "select PANEL_ID, DISPLAY_ORDER_SEQ from ca_chart_section where CHART_ID=? and  PANEL_ID=?";
			...
			for(rowCount = 0 ; rowCount < arrayLength;rowCount ++)
			{
				...
				ps.setString(2,sectionCode[rowCount]);
				try
				{
					rs = ps.executeQuery();
					while(rs.next())
				...
				if(count !=0)
				{
					...
					else
					{
						...
						psUpdate.setString(5,chartId);
						psUpdate.setString(6,sectionCode[rowCount]);
						rows =rows +  psUpdate.executeUpdate();
					}
				}
				else
				{
					psInsert.setString(1,chartId);
					psInsert.setString(2,sectionCode[rowCount]);
			...
			{
				//out.println("Exception raise by Sections For  Chart Insert Modify Function Servlet ...So Record Not inserted  : "+ee.toString());//common-icn-0181
				ee.printStackTrace();
			}
			finally
			{				
