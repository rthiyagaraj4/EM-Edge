...
---------------------------------------------------------------------------------------------------------------------------
?             	100            	?           										created
26/10/2012    IN032816       Chowminya G     										Result Reporting > Record Clinical Note >
																					Mark Error 
08/09/2015	  IN057248		 Vijayakumar K 			08/09/2015		Dinesh T		SKR-SCF-1105 [IN:057248]
13/06/2021	  15305			NijithaS				02/04/2021						MOHE-SCF-0087-TF
28/06/2021	  	29803			Ramesh													ML-BRU-CRF-0635
---------------------------------------------------------------------------------------------------------------------------
*/
package eOR; 
...
import eOR.ORResultEntry.*;

public class ResultEntryBean extends eOR.Common.OrAdapter implements java.io.Serializable {
...
*/
	public String[] viewMainDetails(String order_id) throws Exception
	{
		...
		try 
		{
			...
			//pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_MAIN_DETAILS") ) ;//IN45937
			pstmt = connection.prepareStatement("SELECT a.order_id,TO_CHAR (a.ord_date_time,'DD/MM/YYYY HH24:MI') ord_date_time,or_get_desc.or_order_category (a.order_category,?,'2') order_categ_desc,or_get_desc.or_order_type (a.order_type_code, ?,'2') order_type_desc,DECODE (?,'en', b.patient_name,NVL (b.patient_name_loc_lang, patient_name)) patient_name,b.sex, get_age (b.date_of_birth) age, a.patient_id patient_id,am_get_desc.am_practitioner (a.ord_pract_id, ?, 1) ordering_pract_name,nvl(source_type,'') location_type,CASE WHEN a.patient_class IN ('IP', 'DC') THEN ip_get_desc.ip_nursing_unit(a.ordering_facility_id,a.source_code,?,'2') ELSE op_get_desc.op_clinic (a.ordering_facility_id,a.source_code,?,'2')  END location_desc,Nvl(a.encounter_id,'') encounter_id,nvl(a.parent_order_id,'')parent_order_id FROM or_order a, mp_patient b WHERE order_id = ? AND b.patient_id = a.patient_id") ;//IN45937
		...
		{
			System.err.println("ResultEntryBean.java,102,message=>"+e.getMessage());
			e.printStackTrace() ;
			throw e ;
		} 
		finally 
...
public ArrayList getSearchDetails(String order_id,String order_line_num) throws Exception {
		...
		try {
				connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_SEARCH_DETAILS") ) ;
			...
			while ( resultSet != null && resultSet.next() ) {
				...
				if(getEmptyStrIfNull(resultSet.getString( "auth_by_id" )).equals(""))
 				...
 				record[36] = resultSet.getString("order_catalog_desc");
...
*/
	...
	/**
		modified by Deepa on 5/17/2010 at 3:57 PM for IN0201020
		added sex and dob as parameters.
		using resRowNum variable to chk if no records are retrieved for the passes gender of the patient.
		in case of single reference range also, the gender is passed as A.
	**/
	public String[] getNumericResultTypeDetails(String msr_id,String sex, String dob) throws Exception {
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 			= null;
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_NUMERIC_DETAILS") ) ;
				pstmt.setString( 1, msr_id.trim() ) ;
				pstmt.setString( 2, sex.trim() ) ;
				pstmt.setString( 3, dob.trim() ) ;
			...
			while (resultSet != null && resultSet.next() )
			{
				...
				/*Added by Uma on 4/29/2010 for IN020986*/
			...
			int resRowNum = resultSet.getRow();
			closeResultSet(resultSet) ;
			//int resSize = resultSet.getFetchSize();
...
*/
	...
	public String[] getHeaderLineDetails(String order_id, String report_srl_no) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_HEADER_DETAIL") ) ;
				pstmt.setString( 1, language_id ) ;
			...
			while ( resultSet != null && resultSet.next() )
			{
				...
				else if(record[1].trim().equals("4"))record[0] = "Prelim";  //29803
...
*/
	...
	public ArrayList getListResultTypeDetails(String discr_msr_id) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_LIST_DETAILS") ) ;
	...
	public ArrayList getTemplateBlocks(String panel_msr_id) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_TEMPLATE_BLOCK") ) ;
				pstmt.setString( 1, language_id);
...
*/
	public String getResultTemplate(String template_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_TEMPLATE") ) ;
...
/*	public String[] getTemplate(String template_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_TEMPLATE_SELECT") ) ;
 			pstmt.setString( 1, template_id.trim() ) ;
			resultSet = pstmt.executeQuery() ;
...
*/
	...
	public String getAuthorization(String pract_id, String resp_id, String order_id,String order_line_num) throws Exception {
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 			= null;
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_REPORT_OR_ORDER_LINE_AUTHORIZATION") ) ;
			pstmt.setString( 1, order_id.trim() ) ;
			pstmt.setString( 2, order_line_num.trim() ) ;
			resultSet = pstmt.executeQuery() ;
			...
			while( resultSet != null && resultSet.next() )
			{
				...
				{
					pstmt1 = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_REPORT_AUTHORIZATION_CHK_VALID_USER") ) ;
	...
	public String getAuthorization1(String pract_id, String resp_id, String order_id, String order_category,String order_line_num) throws Exception {
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 			= null;
		PreparedStatement pstmt1 = null;
		...
		try {
			...
			while( resultSet != null && resultSet.next() )
			{
				...
				if(auth!=null && auth.trim().equals("Y"))
				{
					pstmt1 = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_REPORT_AUTHORIZATION_CHK_VALID_USER1") ) ;
					pstmt1.setString( 1, pract_id.trim() ) ;
					pstmt1.setString( 2, resp_id.trim() ) ;
					pstmt1.setString(3,order_category.trim());
					rs = pstmt1.executeQuery() ;
...
*/
	...
	public boolean frequencyCodeExists(String order_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_FREQ_CODE") ) ;
			pstmt.setString( 1, order_id.trim() ) ;
			resultSet = pstmt.executeQuery() ;
...
*/
	...
	public String[] getReportSrlNo(String order_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_REPORT_SRL_NO") ) ;
			pstmt.setString( 1, order_id.trim() ) ;
			resultSet = pstmt.executeQuery() ;
...
public String[] getReportSrlNo(String order_id,String order_line_num) throws Exception {
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 			= null;
		String[] reportSrlNo = {"1","1"};
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement("SELECT MAX(nvl(REPORT_SRL_NO,1))  REPORT_SRL_NO, result_status FROM OR_RESULT_HEADER WHERE ORDER_ID = ? and REPORT_SRL_NO=? group by report_srl_no, result_status" ) ;
			pstmt.setString( 1, order_id.trim() ) ;
			pstmt.setString( 2, order_line_num.trim() ) ;
			resultSet = pstmt.executeQuery() ;
...
*/
	public ArrayList getOrderStatus() throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORDER_STATUS_SELECT") ) ;
			pstmt.setString( 1, language_id);
			resultSet = pstmt.executeQuery() ;
...
*/
	public ArrayList getFreqExplOrders(String order_id) throws Exception
	{
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_FREQ_EXPL_ORDERS") ) ;
...
*/
	public boolean checkFreqExplosion(String order_id) throws Exception
	{
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_FREQ_EXPL_CHECK") ) ;
			pstmt.setString(1, order_id);
			resultSet = pstmt.executeQuery() ;
...
*/
	...
	{		
		...
		{
			child_reporting_date = DateUtils.convertDate(recordSet.get("child_reporting_date").toString(),"DMYHM",language_id,"en");
			commonRecord.put("child_reporting_date",child_reporting_date);
		}
		...
		//String tempStr1 = (String) recordSet.get("rpt_pract_id");
		String tempStr2 = "";
		...
		resultRecord.put("result_modify_yn"		,getEmptyStrIfNull(recordSet.get("result_modify_yn")));
		...
		for(int i=0; i<totalLineRecords; i++)
		{
			...
			tempStr2 = (String) recordSet.get("practitioner_id"+line);
...
*/
	...
	public String returnMode(String order_id, String report_srl_no) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_CHECK_UPDATE_OR_INSERT") ) ;
				pstmt.setString( 1, order_id.trim() ) ;
			...
			if(result_status==null || result_status.equals(""))
				mode = "1";
			else if(result_status.equals("1") || result_status.equals("2") || result_status.equals("3") || result_status.equals("4"))  // Added result_status.equals("4") for 29803
				mode = "2"; 
...
*/
	public String[] getUser(String appl_user_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_DFLT_PRACTITIONER") ) ;
			pstmt.setString(1, language_id);
...
public HashMap insert()	{
	...
	try{
		...
		String bill_message=OrRepository.getOrKeyValue("SQL_OR_ORDER_ENTRY_BILL_MESSAGE");
		...
		sqlMap.put("SQL_OR_RESULT_REPORTING_GET_STATUS_FUNC",OrRepository.getOrKeyValue("SQL_OR_RESULT_REPORTING_GET_STATUS_FUNC"));
		...
		sqlMap.put("SQL_OR_RESULT_ENTRY_NOTE_DATA_CLOB",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_NOTE_DATA_CLOB"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_UPDATE_OR_ORDER_LINE_STATUS",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_UPDATE_OR_ORDER_LINE_STATUS"));
		...
		sqlMap.put("SQL_OR_RESULT_ENTRY_PARENT_ORDERS_SELECT",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_PARENT_ORDERS_SELECT"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_FREQUENCY_EXPLOSION",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_FREQUENCY_EXPLOSION"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_ABNORMAL_CHILD_CHECK",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ABNORMAL_CHILD_CHECK"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_BL_RESULT_CATALOG",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_BL_RESULT_CATALOG"));
		...
		sqlMap.put("SQL_OR_RESULT_ENTRY_ENCOUNTER_ID_DETAILS",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ENCOUNTER_ID_DETAILS"));
        ...
        sqlMap.put("SQL_OR_RESULT_ENTRY_ORGANISM_INSERT",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORGANISM_INSERT"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_ORGANISM_DELETE",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORGANISM_DELETE"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_INSERT",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_INSERT"));
		sqlMap.put("SQL_OR_BL_WITHOUT_ORDER_CATALOG",OrRepository.getOrKeyValue("SQL_OR_BL_WITHOUT_ORDER_CATALOG"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_FREQ_AUTO_CLOSE",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_FREQ_AUTO_CLOSE"));
		...
		sqlMap.put("SQL_OR_ORDER_GET_CHART_RESULT",OrRepositoryExt.getOrKeyValue("SQL_OR_ORDER_GET_CHART_RESULT") );
		sqlMap.put("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_ENCOUNTER_DELETE",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_ENCOUNTER_DELETE") ) ;
		sqlMap.put("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_ENCOUNTER_INSERT",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_ENCOUNTER_INSERT") ) ;
		sqlMap.put("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_ENCOUNTER_SRL_NO",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_ENCOUNTER_SRL_NO") ) ;
...
public HashMap modify()	{
	...
	try{
		...
		sqlMap.put("SQL_OR_RESULT_REPORTING_UPDATE_DETAIL",OrRepository.getOrKeyValue("SQL_OR_RESULT_REPORTING_UPDATE_DETAIL"));
		sqlMap.put("SQL_OR_RESULT_REPORTING_GET_STATUS_FUNC",OrRepository.getOrKeyValue("SQL_OR_RESULT_REPORTING_GET_STATUS_FUNC"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_UPDATE_OR_ORDER",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_UPDATE_OR_ORDER"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_UPDATE_OR_ORDER_LINE",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_UPDATE_OR_ORDER_LINE"));
		sqlMap.put("SQL_OR_RESULT_REPORTING_UPDATE_CLOB",OrRepository.getOrKeyValue("SQL_OR_RESULT_REPORTING_UPDATE_CLOB"));
		...
		sqlMap.put("SQL_OR_RESULT_ENTRY_ORGANISM_DELETE",OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORGANISM_DELETE"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_INSERT",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_INSERT"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_DELETE",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_DELETE"));
		sqlMap.put("SQL_OR_BL_WITHOUT_ORDER_CATALOG",OrRepository.getOrKeyValue("SQL_OR_BL_WITHOUT_ORDER_CATALOG"));
		sqlMap.put("SQL_OR_RESULT_ENTRY_FREQ_AUTO_CLOSE",OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_FREQ_AUTO_CLOSE"));
		sqlMap.put("SQL_OR_ORDER_ENTRY_GET_ORDER_SET_STATUS_TYPE",OrRepositoryExt.getOrKeyValue("SQL_OR_ORDER_ENTRY_GET_ORDER_SET_STATUS_TYPE") );
		sqlMap.put("SQL_OR_ORDER_ENTRY_GET_ORDER_SET_STATUS_TYPE_CODE",OrRepositoryExt.getOrKeyValue("SQL_OR_ORDER_ENTRY_GET_ORDER_SET_STATUS_TYPE_CODE") );
	...
	try
		{
			...
			Object object		   = context.lookup(OrRepository.getOrKeyValue("OR_RESULT_ENTRY_JNDI"));
			...
			Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome(OrRepository.getOrKeyValue("OR_RESULT_ENTRY_JNDI"), ORResultEntryHome.class, getLocalEJB());
...
*/
	...
	public Hashtable getReportDetails(String order_id) throws Exception
	{
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_REPORT_DETAILS") ) ;
				pstmt.setString( 1, order_id.trim() ) ;
	...
	public int getFormatCount(String order_id, String called_from) throws Exception {
		...
		try{
				...
				{
					pstmt		= connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_FORMAT_LINE_DETAILS"));
					pstmt.setString(1, order_id.trim());
				}
...
/*****************************************************************************/
    ...
    public String getReferralDetails(String order_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_REFERRAL_DETAILS") ) ;
			pstmt.setString( 1, language_id);
...
/*****************************************************************************/
	public String[] getProcedureCode(String order_id,String order_line_num) {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_CHART_DESC_SELECT") ) ;
			pstmt.setString(1, language_id);
			pstmt.setString(2, order_id.trim());
	...
	public ArrayList getOrganismDetails(String chart_result_type) throws Exception {
		...
		try {
			...
			{
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORGANISM_ALL_SELECT") ) ;
				pstmt.setString( 1, language_id);
				pstmt.setString(2,chart_result_type);
			} /*else {
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORGANISM_SELECT") ) ;
	...
	public ArrayList getAntibioticDetails(String chart_result_type) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ANTIBIOTIC_SELECT") ) ;
				//pstmt = connection.prepareStatement("select OR_GET_DESC.OR_CHART_VERTICAL(a.CHART_RESULT_TYPE, a.CHART_VERTI_CODE,?,'2') short_desc, a.chart_verti_code chart_verti_code, a.seq_no seq_no,a.discr_msr_id discr_msr_id,b.result_type result_type,a.copy_prev_vert_yn copy_prev_vert_yn,b.num_digits_max num_digits_max,b.num_digits_min num_digits_min,b.num_digits_dec num_digits_dec from or_chart_vertical a, am_discr_msr b where chart_result_type = ? and a.eff_Status = 'E' and a.discr_msr_id = b.discr_msr_id(+) order by a.seq_no,a.short_desc ") ;
				pstmt.setString(1, language_id);
				pstmt.setString(2, chart_result_type);
				resultSet = pstmt.executeQuery() ;
			...
			while ( resultSet != null && resultSet.next() )
			{
				...
				record[9] = resultSet.getString( "discr_msr_uom" )  ;
				record[10] = resultSet.getString( "num_dflt_result" )  ;
				record[11] = resultSet.getString( "min_value" )  ;
				record[12] = resultSet.getString( "max_value" )  ;
	...
	public ArrayList getChartHeadingDetails(String chart_result_type) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_CHART_HDG_SELECT") ) ;
				pstmt.setString(1, language_id);
				pstmt.setString(2, language_id);
	...
	public ArrayList getOrganismDiscreteDetails(String organism_id) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORGANISM_DISCR_SELECT") ) ;
	...
	public String getOrganismDesc(String organism_id,String chart_result_type) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORGANISM_DESC") ) ;
				pstmt.setString(1, language_id);
	...
	public ArrayList getPreviewListResultTypeDetails(String discr_msr_id,String result_value) throws Exception {
		...
		try {
			connection = getConnection() ;
				pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_PREVIEW_LIST_VALUES") ) ;
				pstmt.setString( 1, discr_msr_id.trim() ) ;
...
//		result_html_data		= new HashMap();
	...
	public void CultureTestRecordSync(String order_id,String report_srl_no) {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_ORGANISM_MODIFY_SELECT") ) ;
			pstmt.setString(1,order_id);
			...
			while ( resultSet != null && resultSet.next() )	{
                ...
                ArrayList headingDetails	= getChartHeadingDetails(chart_result_type);
                ...
                if (resultCounter == 0) {
					ArrayList antibioticValues	= getAntibioticDetails(chart_result_type);
	...
	public String getParentOrderId(String parent_order_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_PARENT_ORDER_ID") ) ;
	...
	public String getCatalogDesc(String order_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_PH_CATALOG_DESC") ) ;
			pstmt.setString( 1, order_id ) ;
	...
	public ArrayList getProcedureDetails(String order_id,int count,String order_line_num) throws Exception 
	{
...
//		int index = 0;
		...
		try 
		{
			...
			if(count==0)
			{
				tmpsql=OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK_CATALOG");				
				tmpsql=tmpsql+" order by order_catalog_code";				
				pstmt = connection.prepareStatement(tmpsql);			
			}
			else
			{
				tmpsql=OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURES_LINK");
				tmpsql=tmpsql+" order by order_catalog_code";				
				pstmt = connection.prepareStatement(tmpsql);
			}
			//pstmt = connection.prepareStatement( OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_LINK"));
				pstmt.setString( 1, order_id.trim() ) ;
				pstmt.setString( 2, order_line_num.trim() ) ;
				pstmt.setString( 3, order_id.trim() ) ;
				pstmt.setString( 4, order_line_num.trim() ) ;
				pstmt.setString( 5, order_id.trim() ) ;
				pstmt.setString( 6, order_line_num.trim() ) ;
				pstmt.setString( 7, order_id.trim() ) ;
				pstmt.setString( 8, order_line_num.trim() ) ;
				pstmt.setString( 9, order_id.trim() ) ;				
				pstmt.setString( 10, order_line_num.trim() ) ;				
				resultSet = pstmt.executeQuery() ;
				String[] record = null;
			...
			while ( resultSet != null && resultSet.next() ) 
			{
				...
				record[15] = resultSet.getString( "catalog_desc" )  ;
	...
	public String[] getProcScheme() throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_PROCEDURE_SCHEME_SELECT") ) ;
	...
	public String getTDMReason(String order_id) throws Exception {
		...
		try {
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_TDMREASON") ) ;
			pstmt.setString(1,order_id);
...
// Added By Sridhar Reddy For PMG20089-CRF-0804 to get RESTRICT_AUTO_COMPLETE_NOTE_YN value for OR_PARAM
	public String getAutoCompleteNoteYN() throws Exception {
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 			= null;
		String auto_complete_note_yn = "";
		try {
			connection = getConnection() ;			
			pstmt = connection.prepareStatement( "SELECT RESTRICT_AUTO_COMPLETE_NOTE_YN FROM OR_PARAM " ) ;
						
			resultSet = pstmt.executeQuery() ;

			while( resultSet != null && resultSet.next() )
			{
				auto_complete_note_yn = resultSet.getString("RESTRICT_AUTO_COMPLETE_NOTE_YN")==null?"N":resultSet.getString("RESTRICT_AUTO_COMPLETE_NOTE_YN");
	...
	public String getValueForImg(String login_facility_id,String order_id,String order_line_num) throws Exception{
		...
		try{
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_VALUE_FOR_IMG") ) ;
	...
	public HashMap validate() throws Exception 
	{
		...
		/*Added by Uma on 11/9/2010 for IN024845*/
		resulted_date_yn	    = getResultedDate();
		...
		/*Added by Uma on 11/9/2010 for IN024845*/
		if(resulted_date_yn.equals("Y"))
		{
			map.put( "result", new Boolean( false ) ) ;
			map.put( "message", getMessage(language_id, "RESULTING_LESS_PREV_REC","OR") ) ;
		}
		/*Ends Here*/
        return map ;
    }

    /*Added by Uma on 11/9/2010 for IN024845 As discussed with Rajan*/
	public String getResultedDate () throws Exception
	{
		String resultedyn = "N";
		String query = "select 1 from or_result_detail where reporting_date =  to_date(?,'dd/mm/yyyy hh24:mi') and order_id in  (select order_id from or_order where parent_order_id=?)";
		String parent_order_id 		= "";
		Connection connection   = null;
		PreparedStatement pstmt = null;
		ResultSet resultSet     = null;
		String child_order_index = "";
		String child_reporting_date = "";
		...
		try
		{
			...
			if(!parent_order_id.equals(""))
			{
				...
				child_order_index				    = (String)commonRecord.get("child_order_index");
				child_reporting_date				= (String)commonRecord.get("child_reporting_date");	

				if(!child_order_index.trim().equals(""))
				{
					pstmt.setString(1 , child_reporting_date); // Enter Child Reporting Time in case of Frequency Explosion.
					pstmt.setString(2 , parent_order_id); // parent_order_id.
					resultSet = pstmt.executeQuery();
					while(resultSet.next())
					{
						resultedyn = "Y";
	...
	*/
	public String getChartStatus(String order_id,String order_line_num) throws Exception {
		...
		try {
			...
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_CHART_STATUS") ) ;
...
public int getRecordedProcedureCount(String order_id,String order_line_num) throws Exception {
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 		= null;
		...
		try {
			...
			connection = getConnection() ;
			pstmt = connection.prepareStatement( OrRepositoryExt.getOrKeyValue("SQL_OR_RESULT_ENTRY_COUNT_RESULT_DETAIL_PROC") ) ;
	...
	//[IN032816]
public String getNtStat(String acc_num) throws Exception 
	{
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 			= null;
		...
		try {
				connection = getConnection() ;
				pstmt = connection.prepareStatement( "select event_status from ca_encntr_note where accession_num =?" ) ;
				pstmt.setString( 1,acc_num.trim());
				resultSet = pstmt.executeQuery() ;

				while ( resultSet != null && resultSet.next() ) 
				{
					nt_stat = resultSet.getString("event_status")  ;
				}
			} catch ( Exception e )	{
				e.printStackTrace() ;
				throw e ;
	...
	public String getOrderStat(String order_id, String order_line_no) throws Exception 
	{ 
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 			= null;
		...
		try {
				connection = getConnection() ;
				pstmt = connection.prepareStatement( "select order_line_status from or_order_line where order_id =? AND order_line_num =?" ) ;//15305
				pstmt.setString( 1,order_id.trim());
				pstmt.setString( 2,order_line_no);//15305
				resultSet = pstmt.executeQuery() ;

				while ( resultSet != null && resultSet.next() ) 
				{
					order_stat = resultSet.getString("order_line_status")  ;
				}
			} catch ( Exception e )	{
				e.printStackTrace() ;
				throw e ;
...
/*public String getUnitDetails(String msr_id) throws Exception {
		Connection connection 		= null;
		PreparedStatement pstmt 	= null;
		ResultSet resultSet 			= null;
		String record = "" ;
		try {
			connection = getConnection() ;
				//pstmt = connection.prepareStatement( OrRepository.getOrKeyValue("SQL_OR_RESULT_ENTRY_UNIT_DETAILS") ) ;
				pstmt = connection.prepareStatement("SELECT a.num_uom FROM am_discr_msr_ref_rng_num  a,am_discr_msr b WHERE a.discr_msr_id = ? AND a.discr_msr_id = b.discr_msr_id ") ;
				pstmt.setString( 1, msr_id.trim() ) ;				
				resultSet = pstmt.executeQuery() ;				

			while ( resultSet != null && resultSet.next() )
			{
				record = resultSet.getString( "num_uom" )  ;				
			}
		} catch ( Exception e )	{
			e.printStackTrace() ;
			throw e ;
...
//IN057248 starts
public String getChildOrderIndex(String order_id,String order_line_num) throws Exception
{
	Connection connection = null;
	PreparedStatement pstmtCO = null;
	ResultSet resultSetCO = null;
	String childOrderIndex;
	int count = 0;

	try 
	{
		connection = getConnection() ;
		pstmtCO = connection.prepareStatement( "SELECT COUNT (*) INCOMPLETE_ORDER  FROM OR_ORDER_LINE WHERE PARENT_ORDER_ID = (SELECT PARENT_ORDER_ID  FROM OR_ORDER_LINE  WHERE ORDER_ID = ?) AND COMPLETE_DATE_TIME IS NULL" ) ;
		pstmtCO.setString( 1,order_id.trim());
		resultSetCO = pstmtCO.executeQuery() ;

		while ( resultSetCO != null && resultSetCO.next() ) 
		{
			count = Integer.parseInt(resultSetCO.getString("incomplete_order")) ;
		}
		childOrderIndex = (count == 1)?"L":(count>1)?order_line_num:"";
	} catch ( Exception e )	
	{
		e.printStackTrace() ;
		throw e ;
	...
	{
		closeResultSet( resultSetCO ) ;
		closeStatement( pstmtCO ) ;
		closeConnection(connection);
	}
