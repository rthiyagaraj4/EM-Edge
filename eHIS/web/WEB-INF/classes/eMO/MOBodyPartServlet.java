...
public class MOBodyPartServlet extends HttpServlet {
	...
	try {
			...
			BODY_PART_CODE = httpservletrequest.getParameter("body_part_code");
			if(BODY_PART_CODE == null) BODY_PART_CODE = "";
			...
			if(s.equals("insert"))
			insertBodyPart(httpservletresponse);
			if(s.equals("modify"))
			modifyBodyPart(httpservletresponse);
		}
    ...
    private void modifyBodyPart(HttpServletResponse httpservletresponse) {
        try {
            ...
            hashmap1.put("BODY_PART_CODE", BODY_PART_CODE);
            
		   String tabname="MO_BODY_PART";
		...
		results.clear();
            ...
            out.println(BODY_PART_CODE + " " + short_desc + " " + effectiveStatus);
        }
    }

    private void insertBodyPart(HttpServletResponse httpservletresponse) {
        try {
            ...
            java.sql.Date date1 = java.sql.Date.valueOf(s3);
            HashMap hashmap = new HashMap();
            hashmap.put("BODY_PART_CODE", BODY_PART_CODE);
            hashmap.put("LONG_DESC", long_desc);
            hashmap.put("SHORT_DESC", short_desc);
            hashmap.put("EFF_STATUS", effectiveStatus);
            hashmap.put("added_by_id", s);
