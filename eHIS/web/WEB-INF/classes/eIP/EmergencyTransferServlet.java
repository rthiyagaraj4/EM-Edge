 ...
 ******************************************************************************/
/*
OPEr - change_bed_class_yn
IP param - ipparam_bed_yn
change_bed_YN = bed_change ch box val
*/
package eIP;
...
import eIP.IPEmergencyTransfer.* ;
import eIP.IPConfirmBedClass.* ;
import com.ehis.util.*;
import eCommon.XSSRequestWrapper;

public class EmergencyTransferServlet extends javax.servlet.http.HttpServlet 
{
	//PrintWriter out;//Uncommented on 20-2-2012 by shanthi for the incident ML-BRU-SCF-0047 IN031024
	...
	//MMS-ME-SCF-0096 vulnerability Issue
		PrintWriter out				= null;  //--  commented on 20-2-2012 by shanthi for the incident ML-BRU-SCF-0047 IN031024
		...
		try 
		{
			...
			try
			{
				...
				entitlement_by_pat_cat_yn					= checkForNull(req.getParameter("entitlement_by_pat_cat_yn"),"N");
				/*Friday, April 30, 2010 , modified for PE , query executed in EmergencyTransfer.jsp*/
				/*
				...
				*/
				...
				else if(status.equals("2"))
				{
					...
					if(msg.lastIndexOf("<br>") >= 0)
						msg=msg.substring(0,msg.lastIndexOf("<br>"));
					out.println("<script>alert('"+msg+"');</script>");
					out.println("<script>if(parent.frames[1].document.EmergencyTransfer_form.transfer.disabled==true)parent.frames[1].document.EmergencyTransfer_form.transfer.disabled=false;</script>");
					message.clear();
				}
			...
			//String reserved_bed_no = "";
			//String to_bed_no	= checkForNull(req.getParameter("to_bed_no"));

/*			
			String str_reserve_bed = " select reserved_bed_no from IP_RESERVED_BED_DTL where facility_id = '"+facilityId+"' and  encounter_id  = '"+encounter_id+"' and  reserved_to_time is null ";
			pstmt = con.prepareStatement(str_reserve_bed);
			try
			{
				rset  = pstmt.executeQuery();
				if(rset!=null && rset.next())
				{
					reserved_bed_no = rset.getString("reserved_bed_no");
				}
				if(rset !=null) rset.close(); if(pstmt !=null) pstmt.close();
		
				if(reserved_bed_no.equals("to_bed_no"))
				{
					proceed =  true;
				}
				else
				{
					proceed = false;
					msg = "cannot proced as its reserved by another patient ... ";
					out.println("<script>alert('"+msg+"');</script>");
					out.println("<script>if(parent.frames(1).document.EmergencyTransfer_form.transfer.disabled==true)parent.frames(1).document.EmergencyTransfer_form.transfer.disabled=false;</script>");
					//message.clear();
				}
...
*/
			...
			if(proceed)
			{
				IPEmergencyTransfer(req,res, facilityId, patientid);
			}
		}catch (Exception e)	
		{
			e.printStackTrace();
	...
	private void IPEmergencyTransfer(HttpServletRequest req, HttpServletResponse res ,String facilityId, String patientid)	 throws java.io.IOException
	{
		...
		String transfer_date=""; //Changes for the SCF SRR20056-SCF-9406 on 6-2-2013
		...
		String mother_bed_status = checkForNull(req.getParameter("mother_bed_status"),"N");
		if(mother_bed_status.equals(""))
			mother_bed_status="N";
				
		
		frdc						= checkForNull(req.getParameter("from_daily_rate"));
		if(!frdc.equals(""))
			frdailycharge			= Integer.parseInt(frdc);

		tonursingunitcode			= checkForNull(req.getParameter("nursing_unit"));
		/*Changes for the SCF SRR20056-SCF-9406 on 6-2-2013 Begin*/
		transfer_date			= checkForNull(req.getParameter("transfer_date"));
		if(!transfer_date.equals("")){
			transfer_date=DateUtils.convertDate(transfer_date,"DMYHM",locale,"en");
		}
		/*Changes for the SCF SRR20056-SCF-9406 on 6-2-2013 End*/
		todc						= checkForNull(req.getParameter("to_daily_rate"));
		if(!todc .equals(""))
			todailycharge			= Integer.parseInt(todc);

		change_bed_YN				= checkForNull(req.getParameter("bed_change"));
		if(!change_bed_YN.equals("Y"))
			change_bed_YN="N";
		call_function				= checkForNull(req.getParameter("call_function"));
		reason_request				= checkForNull(req.getParameter("reqstat"));
		tran_type					= checkForNull(req.getParameter("tfr_type"));
		...
		try 
		{
			...
			hashData.put("transferdate",transfer_date);//Changes for the SCF SRR20056-SCF-9406 on 6-2-2013
			hashData.put("transfertype",checkForNull(req.getParameter("transfer_type_code")));
			...
			hashData.put("mother_bed_status", mother_bed_status);
			...
			hashData.put("transfer_date",transfer_date);//Changes for the SCF SRR20056-SCF-9406 on 6-2-2013
			hashData.put("retain_res_bed",checkForNull(req.getParameter("retain_res_bed")));
			...
			hashData.put("exp_discharge_date_time",exp_discharge_date_time);
			...
			if(call_function.equals("Transfer_Patient") || reason_request.equals("1") && tran_type.equals("ET"))
			{
				boolean local_ejbs = false;
		
				if ((getServletConfig().getServletContext().getInitParameter("LOCAL_EJBS") !=null) &&(getServletConfig().getServletContext().getInitParameter("LOCAL_EJBS").equalsIgnoreCase("TRUE"))) local_ejbs = true;

				Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome("java:comp/env/IPEmergencyTransfer",IPEmergencyTransferHome.class,local_ejbs);
				...
				results = (java.util.HashMap)(busObj.getClass().getMethod("insertIPEmergencyTransfer",paramArray)).invoke(busObj,argArray);
			...
			else
			{
				...
				Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome("java:comp/env/IPConfirmBedClass",IPConfirmBedClassHome.class,local_ejbs);
				Object busObj = (home.getClass().getMethod("create",null)).invoke(home,null);
				...
				results = (java.util.HashMap)(busObj.getClass().getMethod("insertIPConfirmBedClass",paramArray)).invoke(busObj,argArray);
			...
			if(inserted)
			{
				...
				if(call_function.equals("Transfer_Patient"))
				{
//					if(CALL_REPORT) //commented on 15-3-2012 for the incident ML-BRU-SCF-0047 IN031024
						doOnlineReports(req,res,eid,tonursingunitcode, facilityId, patientid,CALL_REPORT,year);
				}
				else
				{
	...
	private void doOnlineReports(HttpServletRequest req,HttpServletResponse res,String encounter_id, String nursing_unit, String facilityId, String patientid,boolean CALL_REPORT,String year) throws ServletException, IOException, SQLException
	{
		Connection con				= null; 
		java.util.Properties p		= null;
		HttpSession session			= null;
	 ...
	 /* CheckStyle Violation Commented by Munisekhar */ 	
		//String sqlString			= "";		//CheckStyle Violation 
		...
		sqlBuffer.append("select a.report_id report_id, b.report_desc report_desc, 1 no_of_copies, a.module_id from ip_online_report a, sm_report_lang_vw b  where b.language_id='"+locale+"' and a.module_id = 'IP' and b.internal_request_yn='Y' and a.facility_id='");
        sqlBuffer.append(facilityId);
		sqlBuffer.append("' and ");
		if(CALL_REPORT){
		sqlBuffer.append("a.print_on_transfer_request_yn='Y'");
		}
		else{
		sqlBuffer.append("a.print_on_conform_transfer_yn = 'Y'");
		}
		sqlBuffer.append(" and a.nursing_unit_code ='");
		sqlBuffer.append(nursing_unit);
		sqlBuffer.append("' and a.report_id = b.report_id and a.report_id not in ('");
		sqlBuffer.append(wristBandLbl);
	...
	/*	sqlString = "select a.report_id report_id, b.report_desc report_desc, 1 no_of_copies, a.module_id from ip_online_report a, sm_report_lang_vw b  where b.language_id='"+locale+"' and a.module_id = 'IP' and b.internal_request_yn='Y' and a.facility_id='"+facilityId+"' and a.print_on_conform_transfer_yn='Y' and a.nursing_unit_code ='"+nursing_unit+"' and a.report_id = b.report_id and a.report_id not in ('"+wristBandLbl+"') order by 2 ";  */
		...
		reportParamValues			= ""+encounter_id+","+nursing_unit+","+clinic_type+","+patientid+"";
		...
		htmlFor.append("  var getUrl		=	'../eCommon/jsp/InternalReportsPrint.jsp?P_function_id=TRANSFER_PATIENT&step=1&sqlString="+java.net.URLEncoder.encode(sqlBuffer.toString())+"&EncounterId="+encounter_id+"&dest_locn_type="+clinic_type+"&dest_locn_code="+nursing_unit+"&reportParamNames="+reportParamNames+"&reportParamValues="+reportParamValues+"';");//modified on 20-2-2012 by shanthi for the incident ML-BRU-SCF-0047 IN031024
		htmlFor.append(" var features    = 'dialogHeight:' + dialogHeight + '; dialogWidth:' + dialogWidth +'; status=no;' ;");
		htmlFor.append("  retVal			=	window.showModalDialog(getUrl,arguments,features);");//modified on 20-2-2012 for the incident ML-BRU-SCF-0047 IN031024
