...
public class IngredientsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("insert".equals(mode)){
			...
			if(!(commonChk.checkForPk(request, response, DlQuery.DL_INGREDIENTS_CHECK_PK, request.getParameter("ingredients")))){
	...
	private int update(HttpServletRequest request,HttpServletResponse response) {
		...
		int err_no = 0;
		try{
			ingredients = request.getParameter("updIngredients");
			...
			pstmt = con.prepareStatement(DlQuery.DL_UPDATE_INGREDIENTS);
			...
			con.commit();
				...
				System.err.println("eDS Exception Ingredients->"+e);
				err_no = 2;
			}
			finally{
				try{
	...
	private int insert(HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			...
			con = ConnectionManager.getConnection(request);
			pstmt = con.prepareStatement(DlQuery.DL_INSERT_INGREDIENTS);
