 ...
 */
public class DeliveryInstructionsServlet extends HttpServlet {
     ...
     */
    public DeliveryInstructionsServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("insert".equals(mode)){
			...
			if(!(commonChk.checkForPk(request, response, DlQuery.DL_DELIVERY_INSTRUCTIONS_CHECK_PK, request.getParameter("del_Instr")))){
	...
	private int update(HttpServletRequest request,HttpServletResponse response) {
		...
		int err_no = 0;
		try{
			delInstr = request.getParameter("updDelInstr");
			...
			pstmt = con.prepareStatement(DlQuery.DL_UPDATE_DELIVERY_INSTRUCTIONS);
			pstmt.setString(1, longDesc);
			pstmt.setString(2, enabledYn);
			pstmt.setString(3, login_user);
			pstmt.setString(4, facility_id);
			pstmt.setString(5, client_ip_address);
			pstmt.setString(6, delInstr);
			rslt = pstmt.execute();
			err_no = 1;
			con.commit();
			}
			catch(Exception e){
				System.err.println("eDS Exception Delivery Instr->"+e);
				err_no = 2;
			}
			finally{
				try{
					...
					System.err.println("Exception in Delivery Instr:"+e);
				}
			}
			return err_no;
	}
	...
	private int insert(HttpServletRequest request, HttpServletResponse response) {
		...
		int err_no = 0;
		try{
			delInstr = request.getParameter("del_Instr");
			longDesc = request.getParameter("long_Desc");
			enabledYn = request.getParameter("enabled_Yn");
			...
			con = ConnectionManager.getConnection(request);
			pstmt = con.prepareStatement(DlQuery.DL_INSERT_DELIVERY_INSTRUCTIONS);
			pstmt.setString(1, delInstr);
