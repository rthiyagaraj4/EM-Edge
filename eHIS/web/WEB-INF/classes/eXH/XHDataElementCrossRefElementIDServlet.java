 ...
 *
 */
public class XHDataElementCrossRefElementIDServlet extends HttpServlet 
{
...
//	  request.getParameter("jspmode");
     ...
     try
     {
      ostmt = connection.prepareCall("{ CALL Xg_prc.xh_appl_derivation_rules(?,?,?,?,?,?,?,?,?,?,?,?,?) }" ); 
    	  ...
    	  String  mode1=(String)arrFilterInfo.get(0);
   	  	  String  applicationid=(String)arrFilterInfo.get(1);
	...
	//	  System.out.println("values"+mode1+"applicationid,"+applicationid+"elementid,"+elementid+"derivation_type,"+derivation_type+"cond_exprsn,"+cond_exprsn+"expected_value,"+expected_value+"execution_order,"+execution_order+"position,"+position+"usr_added_by_id,"+usr_added_by_id+"usr_added_at_ws_no,"+usr_added_at_ws_no);
		  ...
		  ostmt.setString(2,elementid);
		  ostmt.setString(3,derivation_type);
		  ostmt.setString(4,cond_exprsn);
		  ostmt.setString(5,expected_value);
		  ostmt.setString(6,execution_order);
		  ostmt.setString(7,expected_value2);
...
ostmt.setString(13,applicationid);
        ostmt.execute();
