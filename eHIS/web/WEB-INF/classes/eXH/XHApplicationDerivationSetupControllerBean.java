...
public class XHApplicationDerivationSetupControllerBean implements java.io.Serializable
{
...
/**
   ...
   */  public String[][] getDatabaseData()
  {
    ...
    XHApplicationDerivationElementsBean xhBean = null;
...
/**
   ...
   */   
  public XHApplicationDerivationElementsBean getCodeDesc(String key)
  {
    return((XHApplicationDerivationElementsBean)treemp.get(key));
  }
/**
...
/**
  ...
  public void getAllParameterValues(HttpServletRequest request,Connection conn)
  {
    ...
    String strKey = "",strFormat = "",strExceorder="",strExpvalue="",strExpvalue2="",strCondi="",strPosi="";
    XHApplicationDerivationElementsBean xhCode = null;
	try{
...
strExpvalue2=XHDBAdapter.checkNull(request.getParameter(strKey+"EV2"));
strPosi=XHDBAdapter.checkNull(request.getParameter(strKey+"P"));
  strExpvalue=XHDBAdapter.checkNull(request.getParameter(strKey+"EV"));
 xhCode = getCodeDesc(strKey);

	

	  xhCode.setExpectedValue(strExpvalue);
	  xhCode.setCondition(strCondi);
    xhCode.setExpectedValue2(strExpvalue2);
   xhCode.setPosition(strPosi);
...
/**
 ...
 public static XHApplicationDerivationSetupControllerBean getBean(String strBeanName,
														  HttpServletRequest request,
														  HttpSession session)
 {
     String action  = XHDBAdapter.checkNullZero(request.getParameter("action_type"));
     XHApplicationDerivationSetupControllerBean xhBean = null;
     if(action.equals("S")||action.equals("0"))
     {
      session.removeAttribute(strBeanName); 
  xhBean = new XHApplicationDerivationSetupControllerBean();
      session.setAttribute(strBeanName,xhBean); 
    }
    else
    {
      xhBean = (XHApplicationDerivationSetupControllerBean) session.getAttribute(strBeanName);
    }
    return(xhBean);
 }
...
PreparedStatement pstmt = null;
    ...
    { 
      ...
      while(rset.next())
      {
        treemp.put(rset.getString(1)+rset.getString(3),new XHApplicationDerivationElementsBean(
			                                ...
			                                XHDBAdapter.checkNull(rset.getString(9)),  XHDBAdapter.checkNull(rset.getString(10)),XHDBAdapter.checkNull(rset.getString(11)))) ;
      }
    ...
    {
       System.out.println("Error in populatexhcodedesc method of XHApplicationDerivationSetupControllerBean :"+exp.toString());
	   exp.printStackTrace(System.err);
    }
  }
