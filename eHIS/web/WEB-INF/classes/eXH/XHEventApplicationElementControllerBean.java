...
public class XHEventApplicationElementControllerBean implements java.io.Serializable
{
...
/**
   ...
   */  public String[][] getDatabaseData()
  {
    ...
    XHEventForApplicationElementsBean xhBean = null;
...
/**
   * Method  getCodeDesc return bean element XHEventForApplicationElementsBean  depending ob the key 
   * @param  String key
   * @return returns object type  XHEventForApplicationElementsBean
   */   
  public XHEventForApplicationElementsBean getCodeDesc(String key)
  {
    return((XHEventForApplicationElementsBean)treemp.get(key));
  }
/**
...
/**
 ...
 //   Iterator iterator = enumerate();
    ...
    XHEventForApplicationElementsBean xhCode = null;
	try{
...
/**
 ...
 public static XHEventApplicationElementControllerBean getBean(String strBeanName,
														  HttpServletRequest request,
														  HttpSession session)
 {
     String action  = XHDBAdapter.checkNullZero(request.getParameter("action_type"));
     XHEventApplicationElementControllerBean xhBean = null;
     if(action.equals("S")||action.equals("0"))
     {
      
      session.removeAttribute(strBeanName); 
      xhBean = new XHEventApplicationElementControllerBean();
      session.setAttribute(strBeanName,xhBean); 
    }
    else
    {
      xhBean = (XHEventApplicationElementControllerBean) session.getAttribute(strBeanName);
    }
    return(xhBean);
 }
...
/**
   * Method to populate the  XHEventForApplicationElementsBean Bean 
   * @param  Connection conn
   * @param  HttpServletRequest request
   * @param  String locale
  */
  public void populateXHCodeDesc(Connection conn,String locale)
  {
     ...
     String sqlString="SELECT A.EVENT_TYPE,A.EVENT_NAME,A.STANDARD_CODE,A.MESSAGE_TYPE,A.INTERFACE_YN,A.COMMUNICATION_TYPE,C.STANDARD_DESC,A.GENERIC_YN,A.SOLICITED_YN FROM XH_EVENT_TYPE_LANG_VW A,XH_STANDARD C WHERE      DECODE(A.MESSAGE_TYPE,NULL,'XX',A.MESSAGE_TYPE) = 	 	 NVL('"+msg_type+"',DECODE(A.MESSAGE_TYPE,NULL,'XX',A.MESSAGE_TYPE)) AND A.LANGUAGE_ID='en' and A.STANDARD_CODE=C.STANDARD_CODE(+)      AND DECODE(a.standard_code,NULL,'XX',a.standard_code) = 	 	 NVL('"+standard_code+"',DECODE(a.standard_code,NULL,'XX',a.standard_code))  ORDER BY 4,1 ";
  //   System.out.println("XHEventApplicationElementControllerBean sql"+sqlString); 
    ...
    { 
      ...
      while(rset.next())
      {
		   listA.add(rset.getString(1));
              treemp.put(rset.getString(1),new XHEventForApplicationElementsBean(XHDBAdapter.checkNull(rset.getString(1)),XHDBAdapter.checkNull(rset.getString(2)),XHDBAdapter.checkNull(rset.getString(3)),XHDBAdapter.checkNull(rset.getString(4)),XHDBAdapter.checkNull(rset.getString(5)),XHDBAdapter.checkNull(rset.getString(6)),XHDBAdapter.checkNull(rset.getString(7)),XHDBAdapter.checkNull(rset.getString(8)),XHDBAdapter.checkNull(rset.getString(9)),"N")) ;
      }
