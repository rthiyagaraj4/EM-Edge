...
public class XHEventControllerBean implements java.io.Serializable
{
  ...
  public static  String element_desc="";
	...
	public static String prtcl_link_id="";
/**
...
/**
   * Method  getCodeDesc return bean element XHEventBean  depending ob the key 
   * @param  String key
   * @return returns object type  XHEventBean
   */   
  public XHEventBean getCodeDesc(String key)
  {
    return((XHEventBean)treemp.get(key));
  }
/**
...
/**
 ...
 public static XHEventControllerBean getBean(String strBeanName,
														  HttpServletRequest request,
														  HttpSession session)
 {
     String action  = XHDBAdapter.checkNullZero(request.getParameter("action_type"));
     XHEventControllerBean xhBean = null;
     if(action.equals("S")||action.equals("0"))
     {
      
      session.removeAttribute(strBeanName); 
      xhBean = new XHEventControllerBean();
      session.setAttribute(strBeanName,xhBean); 
    }
    else
    {
      xhBean = (XHEventControllerBean) session.getAttribute(strBeanName);
    }
    return(xhBean);
 }
 ...
 public void action(HttpServletRequest request,Connection conn)
  {
    ...
    String action  = XHDBAdapter.checkNullZero(request.getParameter("action_type"));
	whereCaluse=(String)request.getAttribute("whereCaluse");
	prtcl_link_id=(String)request.getAttribute("protocol_link");
	prtcl_link_id="";
...
/**
   * Method to populate the  XHEventBean Bean 
   * @param  Connection conn
   * @param  HttpServletRequest request
   * @param  String locale
  */
  ...
  {
        String sql="SELECT distinct A.event_type code,f.event_name description FROM xh_data_group_detail A,xh_protocol_link B,xh_protocol c,xh_application d,xh_interface e,xh_event_type f,sm_facility_param g WHERE A.data_group_id=B.data_group_id(+) AND b.protocol_id=c.protocol_id(+) AND A.application_id=D.application_id(+) AND c.interface_module_id= e.interface_module_id(+)	 AND A.event_type=f.event_type(+) AND a.facility_id=g.facility_id(+) AND B.protocol_link_id=nvl('"+prtcl_link_id+"',B.protocol_link_id) "+whereCaluse;
    //String sql="SELECT  EVENT_TYPE code,EVENT_NAME description FROM XH_EVENT_TYPE "+whereCaluse;    
//	System.out.println("XHEventControllerBean sql"+sql); 
    ...
    { 
      ...
      while(rset.next())
      {
        treemp.put(rset.getString(1),new XHEventBean(XHDBAdapter.checkNull(rset.getString(1)),XHDBAdapter.checkNull(rset.getString(2)),"N")) ;
      }
