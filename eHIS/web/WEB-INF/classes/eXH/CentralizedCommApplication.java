 ...
 * 
 */
class CentralizedCommApplication {
	String appln;
	String commClient;
	String commType;
	String sql;
	...
	CentralizedCommApplication(int rowKey) {
		setParams(rowKey);
	}
	...
	public void setParams(int rowKey) {
		try {
			commType = CentralizedDataBaseAdapter.communicationType;
			...
			if (commType.equalsIgnoreCase("O")) {
				...
				commClient = (String) outbound[rowKey][0];
				protocolLinkID = (String) outbound[rowKey][4];
			} else {
				commType = (String) inbound[rowKey][3];
				appln = (String) inbound[rowKey][1];
				commClient = (String) inbound[rowKey][0];
