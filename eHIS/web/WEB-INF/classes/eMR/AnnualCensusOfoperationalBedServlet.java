...
public class AnnualCensusOfoperationalBedServlet extends javax.servlet.http.HttpServlet 
{
...
//	String bed_class_group_code			= "";
	...
	//	bed_class_group_code		= checkForNull(request.getParameter("bed_class_group_code"));
	    System.err.println("AnnualCensusOfoperationalBedServlet====>"+request.getQueryString());
	...
	private void insertTabulation(HttpServletRequest request, HttpServletResponse response)
	{
		...
		int count = 0;
		count=Integer.parseInt(maxrecord); System.err.println("AnnualCensusOfoperationalBedServlet count====>"+count);
		try
		{
			...
			System.err.println("Test-100->"+request.getQueryString());
			int i=1;
			try
			{
				String sql350="";
				for(i=1;i<count;i++)
				{
					...
					if(inc.equals("") || inc.equals("N"))
					{
						//String sql22="";
						try{
							sql350 ="delete from mr_annual_census_of_opr_bed where to_char(CENSUS_YEAR,'yyyy')= ? and GROUP_CODE=?  and SUB_GROUP_CODE=?  and BED_CLASS_GROUP_CODE=?";
					...
					if(inc.equals("Y") )//&& !(text.equals(""))
					{
						...
						String sp = "insert into mr_annual_census_of_opr_bed (FACILITY_ID , CENSUS_YEAR, GROUP_CODE ,SUB_GROUP_CODE, BED_CLASS_GROUP_CODE, COMM_BED_CNT, MALE_BED_CNT, FEMALE_BED_CNT,CHILD_BED_CNT,  added_by_id, added_date, added_at_ws_no, added_facility_id, modified_by_id, modified_date, modified_at_ws_no, modified_facility_id ) values  (?, to_date(?,'yyyy'), ?, ?, ?, ?, ?, ?, ?,?, sysdate, ?, ?, ?, sysdate, ?, ?)";
						// MODIFIED by Mujafar on 25th June 2019 against ML-MMOH-CRF-1345
						...
						//modified by Mujafar on 25th June 2019 against ML-MMOH-CRF-1345 start
						...
						//added by Mujafar on 25th June 2019 against ML-MMOH-CRF-1345 end
						cnt=pstmt.executeUpdate();	 System.err.println("AnnualCensusOfoperationalBedServlet.java cnt===>"+cnt);					
