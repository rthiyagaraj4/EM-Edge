...
public class GenerateCS_AU extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleCS_AU js =new JobScheduleCS_AU();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createCS_AU function which will create the JSON file*/
			GenerateCS_AU Gjson= new GenerateCS_AU();

			String CS_AU_fileCreated_flag =Gjson.createCS_AU(facility_id,from_date,to_date);

			String error="";
			if(CS_AU_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (CS_AU_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (CS_AU_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + CS_AU_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createCS_AU(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='CS_AU'");
				...
				/*The View which is created for this createCS_AUJson purpose*/
				...
				String sqlQuery		  = "SELECT * FROM MR_CS_AUD_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				
				//String sqlQuery		  = "SELECT * FROM MR_CS_AUD_VW  where rownum<6";// WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('06/10/2021','dd/mm/yyyy') AND TO_DATE ('06/10/2021', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String pat_class="";
				 ...
				 while(rset!=null && rset.next()) 	{  	 // looping each ip encounter
					...
					StringBuffer audiology= new StringBuffer(" \"audiology\": {	");
				
					audiology.append("\"refDisciplineCode\": \""+checkForNull(rset.getString("refDisciplineCode"))+"\",");
					audiology.append("\"refSpecialityCode\": \""+checkForNull(rset.getString("refSpecialityCode"))+"\",");
					audiology.append("\"refSubSpecialityCode\": \""+checkForNull(rset.getString("refSubSpecialityCode"))+"\",");
					audiology.append("\"refMainDiagnosisCodes\": "+checkForNull(rset.getString("refMainDiagnosiscode"))+",");
					audiology.append("\"diagnosisOthers\": \""+checkForNull(rset.getString("mainDiagnosis"))+"\",");
					//audiology.append("\"diagnosisOthers\": \""+checkForNull(rset.getString("diagnosisOthers"))+"\",");				
					
					
					System.out.println("enc_id-->"+ enc_id);
					pstmt_1 =  con.prepareStatement("select MR_CS_AUD_DTLS("+enc_id+",'A') AU_DTLS from dual");
					
					String hasAssessment="false";
					String hasIntervention="false";
					...
					while(rset_1!=null && rset_1.next()){
						...
						AU_dtls=checkForNull(rset_1.getString("AU_DTLS"));
					}
					
					//AU_dtls="TRIP00000621379~446~451~A~FEES~00^**~**@TRIP00000621380~446~452~A~Videofloroscopic Swallowing Study(VFSS)~00^**~**@TRIP00000621382~479~482~A~Speech language assessment~00^**~**@TRIP00000621382~479~483~A~Clinical Swallowing Evaluation (CSE)~00^**~**@TRIP00000621382~496~497~A~Instrumental assessment of voice~00^**~**";
					
					//TROP00000551807~00~369~A~Visual Reinforcement Audiometry~00^**~**@TROP00000551808~00~370~A~Speech Test~00^**~**@TROP00000551821~365~370~A~Speech Test~00^**~**@TROP00000551822~365~369~A~Visual Reinforcement Audiometry~00^**~**";
					//AU_dtls="";
					
					System.out.println("AU_dtls-->"+ AU_dtls);
					temp_tkn=null;
					temp_tkn_inner=null;
					
					temp_tkn		= new StringTokenizer(AU_dtls,"@" );
					...
					while (temp_tkn.hasMoreTokens()){
						...
						while (temp_tkn_inner.hasMoreTokens()){
							...
							if(!activity_type.equals("00") && !catalog_codes.equals("00")){	
								...
								if (! olc_act_typ.equals(activity_type)) {
									if (ii>0){
										assessments.append("]},");								
									} 
									...
									assessments.append("{");
									assessments.append("\"refDataCode\": \""+catalog_codes+"\",");
									assessments.append("\"values\": []");	
									assessments.append("}");
									olc_act_typ= activity_type;
								}else{	
									assessments.append(",{");
									assessments.append("\"refDataCode\": \""+catalog_codes+"\",");
									assessments.append("\"values\": []");	
					...
					if (hasAssessment.equals("true")){
						assessments.append("]}");
					}
					
					StringBuffer findings= new StringBuffer(" \"findings\": [	");
					findings.append("{");
					findings.append("\"refDataCode\":  \"\",");
					findings.append("\"values\": [],");					
					findings.append("\"findings\":[]}],");
					
					StringBuffer amplifications= new StringBuffer(" \"amplifications\": [	");
					amplifications.append("{");
					amplifications.append("\"refDataCode\":  \"\",");
					amplifications.append("\"values\": [],");					
					amplifications.append("\"amplifications\":[]}],");
					
					StringBuffer vestibulars= new StringBuffer(" \"vestibulars\": [	");
					vestibulars.append("{");
					vestibulars.append("\"refDataCode\":  \"\",");
					vestibulars.append("\"values\": [],");					
					vestibulars.append("\"vestibulars\":[]}],");
					
					
					StringBuffer capds= new StringBuffer(" \"capds\": [	");
					capds.append("{");
					capds.append("\"refDataCode\":  \"\",");
					capds.append("\"values\": [],");					
					capds.append("\"capds\":[]}],");
					
					
					StringBuffer tinnitus= new StringBuffer(" \"tinnitus\": [	");
					tinnitus.append("{");
					tinnitus.append("\"refDataCode\":  \"\",");
					tinnitus.append("\"values\": [],");					
					tinnitus.append("\"tinnitus\":[]}],");
					
					
					StringBuffer cochlearImplants= new StringBuffer(" \"cochlearImplants\": [	");
					cochlearImplants.append("{");
					cochlearImplants.append("\"refDataCode\":  \"\",");
					cochlearImplants.append("\"values\": [],");					
					cochlearImplants.append("\"cochlearImplants\":[]}],");
					
					StringBuffer auralRehabilitations= new StringBuffer(" \"auralRehabilitations\": [	");
					auralRehabilitations.append("{");
					auralRehabilitations.append("\"refDataCode\":  \"\",");
					auralRehabilitations.append("\"values\": [],");					
					auralRehabilitations.append("\"auralRehabilitations\":[]}],");
					
					StringBuffer neonatalHearingScreenings= new StringBuffer(" \"neonatalHearingScreenings\": [	");					
					neonatalHearingScreenings.append("{");
					neonatalHearingScreenings.append("\"refDataCode\":  \"\",");
					neonatalHearingScreenings.append("\"values\": [],");					
					neonatalHearingScreenings.append("\"neonatalHearingScreenings\":[]}],");
					
					
					therapists.append("]");
					assessments.append("]");
					
					clinicalsupport.append(audiology.toString());
					clinicalsupport.append(assessments.toString());
					clinicalsupport.append(",");	
					clinicalsupport.append(findings.toString());
					clinicalsupport.append(amplifications.toString());
					clinicalsupport.append(vestibulars.toString());
					clinicalsupport.append(capds.toString());
					clinicalsupport.append(tinnitus.toString());
					clinicalsupport.append(cochlearImplants.toString());
					clinicalsupport.append(auralRehabilitations.toString());
					clinicalsupport.append(neonatalHearingScreenings.toString());
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_CS_AU.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- CS_AU generate SERVLET-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File CS_AUjson = new File (dir);
						FileWriter file = new FileWriter(CS_AUjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("CS_AUjson.getPath()-->"+CS_AUjson.getPath());
						//System.out.println("CS_AUjson.getAbsolutePath()-->"+CS_AUjson.getAbsolutePath());
						//System.out.println("CS_AUjson.getName()-->"+CS_AUjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_AU",CS_AUjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  CS_AU generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_AU","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createCS_AU function Inside try:"+e.toString());
						System.out.println("Error while file obj  CS_AU generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_AU","IO Error. Unable to create File for CS_AU generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for CS_AU generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_AU","No data for CS_AU generation");
				}
	}	 //check in function control				

		
		}catch (Exception e) {
			  System.err.println("Exception in createCS_AU function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_AU",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleCS_AU extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleCS_AU = new JobScheduleCS_AU();
    ...
    timer.scheduleAtFixedRate(JobScheduleCS_AU, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobScheduleCS_AU, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createCS_AU(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleCS_AU Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
