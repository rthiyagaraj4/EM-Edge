...
public class GeneratePD301 extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobSchedulePD301 js =new JobSchedulePD301();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createPD301 function which will create the JSON file*/
			GeneratePD301 Gjson= new GeneratePD301();

			String PD301_fileCreated_flag =Gjson.createPD301(facility_id,from_date,to_date);

			String error="";
			if(PD301_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (PD301_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (PD301_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + PD301_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createPD301(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='PD301'");
				...
				/*The View which is created for this PD301Json purpose*/
				...
				String fl_name=filename+"_PD301";
				...
				if(i>0){
					...
					try{
						...
						//System.out.println("<-- PD301-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File PD301json = new File (dir);
						FileWriter file = new FileWriter(PD301json);/*File is Created with Json object as data in specific folder*/
						//System.out.println("PD301json.getPath()-->"+PD301json.getPath());
						//System.out.println("PD301json.getAbsolutePath()-->"+PD301json.getAbsolutePath());
						//System.out.println("PD301json.getName()-->"+PD301json.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD301",PD301json.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  PD301 generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD301","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createPD301 function Inside try:"+e.toString());
						System.out.println("Error while file obj  PD301 generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD301","IO Error. Unable to create File for PD301 generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for PD301 generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD301","No data for PD301 generation");
				}
				
			}	 //check in function control				
		
		}catch (Exception e) {
			  System.err.println("Exception in createPD301 function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD301",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobSchedulePD301 extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobSchedulePD301 = new JobSchedulePD301();
    ...
    timer.scheduleAtFixedRate(JobSchedulePD301, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobSchedulePD301, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createPD301(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobSchedulePD301 Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
