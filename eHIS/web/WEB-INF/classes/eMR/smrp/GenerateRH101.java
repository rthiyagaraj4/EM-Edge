...
public class GenerateRH101 extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleRH101 js =new JobScheduleRH101();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createRH101 function which will create the JSON file*/
			GenerateRH101 Gjson= new GenerateRH101();

			String RH101_fileCreated_flag =Gjson.createRH101(facility_id,from_date,to_date);

			String error="";
			if(RH101_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (RH101_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (RH101_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + RH101_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createRH101(String facility_id,String start_date,String end_date)					
	{ 
		
		Connection con = null;
		ResultSet rset = null;
		Statement stmt =  null;
		//ResultSet rset_drug = null;
		...
		try {			
			...
			//String sql_drug="select PRESCRIBE_NO,PRESCRIBE_DATE,ORDERED_BY,PH_DAIGNOSIS,LOCA_TION,CO_ST,REFDRUGCODE,REFDOSAGECODE,REFROUTECODE,REFFREQUENCYCODE,DISPENSEUOM,ORDERUOM,DURATIONTYPE,STATUS,STARTDATE,ENDDATE,DISPENSENO,DISPENSEDDATE,EXPIRYDATE,DISP_LOCATION,DISP_COST,REDISPENSEQTY,PENDINGQTY,BATCH_NO,REF_DRUG_CODE,DISPENSE_UOM,ORDER_UOM,'' SUPPY_DURATION, '' SUPPLY_END_DATE,DISPENSEDQTY,VERIFIEDBY,VERIFIEDDATE,PREPAREDBY,PREPAREDDATE,DISPENSEDBY,ORDER_LINE_STATUS from PH_PDRH_101 where encounter_id=? and ORDERING_FACILITY_ID=? ;
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='RH101'");
				...
				/*The View which is created for this RH101Json purpose*/
				...
				// facility id to be used from mapping table -- 15/feb/2018
				String refServiceTypeCode="02";
					

				/*This query will return the service code for which JSON file need to be generated*/
				
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select EXT_PK_ID from  XH_OTH_APPL_DATA_SKEY WHERE table_id='AM_PATIENT_CLASS' and APPLICATION_ID='SMRP' and PK_VALUE='DC'");
				...
				//String sqlQuery		  = "SELECT * FROM MR_PDRH101_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999 and patient_class='DC'  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				String sqlQuery		  = "SELECT * FROM MR_PDRH101_VW  WHERE trunc(VISIT_ADM_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999 and patient_class='DC'  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String fl_name=filename+"_RH101";
				...
				str_head.append("{\"filename\": \""+fl_name+"\",\"refServiceTypeCode\": \""+refServiceTypeCode+"\",\"formType\": \"101\",\"facilityCode\": \""+mapped_fac_id+"\",");
				...
				int i=0;
				 ...
				 while(rset!=null && rset.next()) 	{  	 // looping each ip encounter
					...
					//admission.append("\"totalDurationDay\": \""+rset.getString("TOTAL_DAY_DURATION")+"\" ,");

					admission.append(" \"daycare\": {");
					temp_tkn=null;

					temp_tkn		= new StringTokenizer(checkForNull(rset.getString("DC_DETAILS")),"~" );
					String refUnplannedAdmissionReasonCode="";
								
					while (temp_tkn.hasMoreTokens()){
						admission.append("\"admissionDate\": \""+temp_tkn.nextToken()+"\" ,");
						admission.append("\"refDisciplineCode\": \""+temp_tkn.nextToken()+"\" ,");
						admission.append("\"refSpecialityCode\": \""+temp_tkn.nextToken()+"\" ,");
						admission.append("\"refSubSpecialityCode\": \""+temp_tkn.nextToken()+"\" ,");
						//admission.append("\"refUnplannedAdmissionReasonCode\": \""+temp_tkn.nextToken()+"\" ");
						refUnplannedAdmissionReasonCode=temp_tkn.nextToken();
						admission.append("\"refUnplannedAdmissionReasonCode\": \"\" ");
					}
					//admission.append("},");
					admission.append("}");
					...
					/*System.out.println("enc"+rset.getString("encounter_id"));
					System.out.println("fac"+ rset.getString("facility_code"));
					System.out.println("sql_drug"+ sql_drug);

					pstmt=con.prepareStatement(sql_drug);
					pstmt.setString(1,rset.getString("encounter_id"));
					pstmt.setString(2,rset.getString("facility_code"));
					rset_drug = pstmt.executeQuery();
					...
					StringBuffer drug_1= new StringBuffer(" ");
					StringBuffer drug_2= new StringBuffer(" ");
					StringBuffer drug_3= new StringBuffer(" ");
					StringBuffer drug_4= new StringBuffer(" ");

					StringBuffer drug= new StringBuffer(" ");
					while(rset_drug!=null && rset_drug.next()) 	{
							System.out.println("insaie");
						if( prescribe_No.equals(rset_drug.getString("PRESCRIBE_NO"))){						
							flg_1=false;
							drug_1.append("}	");
						}else{
							flg_1=true;
							drug_1.append(" \"drugPrescriptions \": {	");
							prescribe_No=rset_drug.getString("PRESCRIBE_NO");
							drug_1.append("\"prescribeNo\": \""+checkForNull(rset_drug.getString("PRESCRIBE_NO"))+"\" ,");
							drug_1.append("\"prescribeDate\": \""+checkForNull(rset_drug.getString("PRESCRIBE_DATE"))+"\" ,");
							drug_1.append("\"orderBy\": \""+checkForNull(rset_drug.getString("ORDERED_BY"))+"\" ,");
							drug_1.append("\"diagnosis\": \""+rset_drug.getString("PH_DAIGNOSIS")+"\" ,");
							drug_1.append("\"location\": \""+rset_drug.getString("LOCA_TION")+"\" ,");
							drug_1.append("\"cost\": \""+rset_drug.getString("CO_ST")+"\" ,");
						}

						if(refDrug_Code.equals(rset_drug.getString("refDrugCode"))){
							flg_2=false;
							//drug_2.append("}	");
						}else{
							flg_2=true;
							drug_2.append(" \"prescriptionDetails  \": {	");
							refDrug_Code=rset_drug.getString("refDrugCode");
							drug_2.append("\"refDrugCode\": \""+checkForNull(rset_drug.getString("REFDRUGCODE"))+"\" ,");
							drug_2.append("\"refDosageCode\": \""+checkForNull(rset_drug.getString("REFDOSAGECODE"))+"\" ,");
							drug_2.append("\"refRouteCode\": \""+checkForNull(rset_drug.getString("REFROUTECODE"))+"\" ,");
							drug_2.append("\"refFrequencyCode\": \""+rset_drug.getString("REFFREQUENCYCODE")+"\" ,");
							drug_2.append("\"dispenseUom\": \""+rset_drug.getString("DISPENSEUOM")+"\" ,");
							drug_2.append("\"orderUom\": \""+rset_drug.getString("ORDERUOM")+"\" ,");
							drug_2.append("\"durationType\": \""+rset_drug.getString("DURATIONTYPE")+"\" ,");
							drug_2.append("\"status\": \""+rset_drug.getString("STATUS")+"\" ,");
							drug_2.append("\"startDate\": \""+rset_drug.getString("STARTDATE")+"\" ,");
							drug_2.append("\"endDate\": \""+rset_drug.getString("ENDDATE")+"\"} ");
						}

						if(dispense_No.equals(rset_drug.getString("dispenseNo"))){
							flg_3=false;
							//drug_3.append("}	");
						}else{
							flg_3=true;
							dispense_No=rset_drug.getString("dispenseNo");

							drug_3.append(" \"dispensing  \": {	");
							drug_3.append("\"dispenseNo\": \""+checkForNull(rset_drug.getString("DISPENSENO"))+"\" ,");
							drug_3.append("\"dispensedDate\": \""+checkForNull(rset_drug.getString("DISPENSEDDATE"))+"\" ,");
							drug_3.append("\"expiryDate\": \""+checkForNull(rset_drug.getString("EXPIRYDATE"))+"\" ,");
							drug_3.append("\"location\": \""+rset_drug.getString("DISP_LOCATION")+"\" ,");
							drug_3.append("\"cost\": \""+rset_drug.getString("DISP_COST")+"\" ,");
							drug_3.append("\"reDispenseQty\": \""+rset_drug.getString("REDISPENSEQTY")+"\" ,");
							drug_3.append("\"pendingQty\": \""+rset_drug.getString("PENDINGQTY")+"\" ,");
							drug_3.append("\"orderBy\": \""+rset_drug.getString("ORDERED_BY")+"\" }");
							
						}

						if(batch_No.equals(rset_drug.getString("batch_No"))){
							flg_4=false;
							//drug_4.append("}	");
						}else{
							flg_4=true;
							batch_No=rset_drug.getString("batch_No");
							drug_4.append(" \"dispensingDetails  \": {	");
							drug_4.append("\"batchNo\": \""+checkForNull(rset_drug.getString("batch_No"))+"\" ,");
							drug_4.append("\"refDrugCode\": \""+checkForNull(rset_drug.getString("REF_DRUG_CODE"))+"\" ,");
							drug_4.append("\"dispenseUom\": \""+checkForNull(rset_drug.getString("DISPENSE_UOM"))+"\" ,");
							drug_4.append("\"orderUom\": \""+rset_drug.getString("ORDER_UOM")+"\" ,");
							drug_4.append("\"supplyDuration\": \""+rset_drug.getString("SUPPY_DURATION")+"\" ,");
							drug_4.append("\"supplyEndDate\": \""+rset_drug.getString("SUPPLY_END_DATE")+"\" ,");
							drug_4.append("\"dispensedQty\": \""+rset_drug.getString("DISPENSEDQTY")+"\" ,");
							drug_4.append("\"verifiedBy\": \""+rset_drug.getString("VERIFIEDBY")+"\" ,");
							drug_4.append("\"verifiedDate\": \""+rset_drug.getString("VERIFIEDDATE")+"\" ,");
							drug_4.append("\"preparedBy\": \""+rset_drug.getString("PREPAREDBY")+"\" ,");
							drug_4.append("\"preparedDate\": \""+rset_drug.getString("PREPAREDDATE")+"\" ,");
							drug_4.append("\"dispensedBy \": \""+rset_drug.getString("DISPENSEDBY")+"\" ,");
							drug_4.append("\"status\": \""+rset_drug.getString("ORDER_LINE_STATUS")+"\" } ");

							
						}		
						
						
						drug.append(drug_1.toString());
						drug.append(drug_2.toString());
						drug.append(drug_3.toString());
						drug.append(drug_4.toString());
				...
				filename=filename+"_RH101.json";
				if(i>0){
				
					try{
						
						//System.out.println("dir-->"+dir);
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_PD101.json");/*File is Created with Json object as data in specific folder*/
						//System.out.println("<-- RH101-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File RH101json = new File (dir);
						FileWriter file = new FileWriter(RH101json);/*File is Created with Json object as data in specific folder*/
						//System.out.println("RH101json.getPath()-->"+RH101json.getPath());
						//System.out.println("RH101json.getAbsolutePath()-->"+RH101json.getAbsolutePath());
						//System.out.println("RH101json.getName()-->"+RH101json.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH101",RH101json.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  RH101 generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH101","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createRH101 function Inside try:"+e.toString());
						System.out.println("Error while file obj  RH101 generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH101","IO Error. Unable to create File for PD101 generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for RH101 generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH101","No data for RH101 generation");
				}
			  ...
			  System.err.println("Exception in createRH101 function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH101",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleRH101 extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleRH101 = new JobScheduleRH101();
    ...
    con =   ConnectionManager.getConnection();
		...
		rset =  stmt.executeQuery("select nvl(to_char(LAG_TIME,'HH24'),'00') hr,nvl(to_char(LAG_TIME,'mi'),'00') min from MR_SMRP_CONFIG where SMRP_ID='RH101'");
		String hr_lag = "00";
		String min_lag = "00";
		if(rset!=null && rset.next()){
    ...
    //timer.scheduleAtFixedRate(JobScheduleRH101, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	timer.scheduleAtFixedRate(JobScheduleRH101, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createRH101(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleRH101 Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
	  ...
	  private final static int fZERO_MINUTES = 15; 
