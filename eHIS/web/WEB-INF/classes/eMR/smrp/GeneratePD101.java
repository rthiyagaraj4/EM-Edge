...
public class GeneratePD101 extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobSchedulePD101 js =new JobSchedulePD101();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createPD101 function which will create the JSON file*/
			GeneratePD101 Gjson= new GeneratePD101();

			String PD101_fileCreated_flag =Gjson.createPD101(facility_id,from_date,to_date);

			String error="";
			if(PD101_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (PD101_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (PD101_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + PD101_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createPD101(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='PD101'");
				...
				/*The View which is created for this PD101Json purpose*/
				...
				//String sqlQuery		  = "SELECT * FROM MR_PDRH101_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999 and patient_class='IP' and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				String sqlQuery		  = "SELECT * FROM MR_PDRH101_VW  WHERE trunc(VISIT_ADM_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999 and patient_class='IP' and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String fl_name=filename+"_PD101";
				...
				if(i>0){
					...
					try{
						...
						//System.out.println("<-- PD101-->"+str_head.toString());
						
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File PD101json = new File (dir);
						FileWriter file = new FileWriter(PD101json);/*File is Created with Json object as data in specific folder*/
						//System.out.println("PD101json.getPath()-->"+PD101json.getPath());
						//System.out.println("PD101json.getAbsolutePath()-->"+PD101json.getAbsolutePath());
						//System.out.println("PD101json.getName()-->"+PD101json.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD101",PD101json.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  PD101 generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD101","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createPD101 function Inside try:"+e.toString());
						System.out.println("Error while file obj  PD101 generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD101","IO Error. Unable to create File for PD101 generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for PD101 generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD101","No data for PD101 generation");
				}
				
			}	 //check in function control				
		
		}catch (Exception e) {
			  System.err.println("Exception in createPD101 function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PD101",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobSchedulePD101 extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobSchedulePD101 = new JobSchedulePD101();
    ...
    con =   ConnectionManager.getConnection();
		...
		rset =  stmt.executeQuery("select nvl(to_char(LAG_TIME,'HH24'),'00') hr,nvl(to_char(LAG_TIME,'mi'),'00') min from MR_SMRP_CONFIG where SMRP_ID='PD101'");
		String hr_lag = "00";
		String min_lag = "00";
		if(rset!=null && rset.next()){
    ...
    timer.scheduleAtFixedRate(JobSchedulePD101, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobSchedulePD101, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createPD101(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobSchedulePD101 Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
