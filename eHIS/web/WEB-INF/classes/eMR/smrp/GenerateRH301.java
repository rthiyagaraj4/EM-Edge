...
public class GenerateRH301 extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleRH301 js =new JobScheduleRH301();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createRH301 function which will create the JSON file*/
			GenerateRH301 Gjson= new GenerateRH301();

			String RH301_fileCreated_flag =Gjson.createRH301(facility_id,from_date,to_date);

			String error="";
			if(RH301_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (RH301_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (RH301_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + RH301_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createRH301(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='RH301'");
				...
				/*The View which is created for this RH301Json purpose*/
				...
				String sqlQuery		  = "SELECT * FROM MR_PDRH301_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999 and patient_class='DC'  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String fl_name=filename+"_RH301";
				...
				filename=filename+"_RH301.json";
				if(i>0){
				
					try{
						
						//System.out.println("dir-->"+dir);
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_PD301.json");/*File is Created with Json object as data in specific folder*/
						//System.out.println("<-- RH301-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File RH301json = new File (dir);
						FileWriter file = new FileWriter(RH301json);/*File is Created with Json object as data in specific folder*/
						//System.out.println("RH301json.getPath()-->"+RH301json.getPath());
						//System.out.println("RH301json.getAbsolutePath()-->"+RH301json.getAbsolutePath());
						//System.out.println("RH301json.getName()-->"+RH301json.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH301",RH301json.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  RH301 generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH301","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createRH301 function Inside try:"+e.toString());
						System.out.println("Error while file obj  RH301 generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH301","IO Error. Unable to create File for PD301 generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for RH301 generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH301","No data for RH301 generation");
				}
			  ...
			  System.err.println("Exception in createRH301 function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"RH301",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleRH301 extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleRH301 = new JobScheduleRH301();
    ...
    con =   ConnectionManager.getConnection();
		...
		rset =  stmt.executeQuery("select nvl(to_char(LAG_TIME,'HH24'),'00') hr,nvl(to_char(LAG_TIME,'mi'),'00') min from MR_SMRP_CONFIG where SMRP_ID='RH301'");
		String hr_lag = "00";
		String min_lag = "00";
		if(rset!=null && rset.next()){
    ...
    timer.scheduleAtFixedRate(JobScheduleRH301, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobScheduleRH301, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createRH301(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleRH301 Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
