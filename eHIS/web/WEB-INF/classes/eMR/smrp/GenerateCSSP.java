...
public class GenerateCSSP extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleCSSP js =new JobScheduleCSSP();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createCSSP function which will create the JSON file*/
			GenerateCSSP Gjson= new GenerateCSSP();

			String CSSP_fileCreated_flag =Gjson.createCSSP(facility_id,from_date,to_date);

			String error="";
			if(CSSP_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (CSSP_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (CSSP_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + CSSP_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createCSSP(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				/*The View which is created for this CSSPJson purpose*/
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_CSSP.json");/*File is Created with Json object as data in specific folder*/
						
						System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File CSSPjson = new File (dir);
						FileWriter file = new FileWriter(CSSPjson);/*File is Created with Json object as data in specific folder*/
						System.out.println("CSSPjson.getPath()-->"+CSSPjson.getPath());
						System.out.println("CSSPjson.getAbsolutePath()-->"+CSSPjson.getAbsolutePath());
						System.out.println("CSSPjson.getName()-->"+CSSPjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CSSP",CSSPjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  CSSP generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CSSP","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createCSSP function Inside try:"+e.toString());
						System.out.println("Error while file obj  CSSP generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CSSP","IO Error. Unable to create File for CSSP generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for CSSP generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CSSP","No data for CSSP generation");
				}
				
			}	 //check in function control				
		
		}catch (Exception e) {
			  System.err.println("Exception in createCSSP function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CSSP",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleCSSP extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleCSSP = new JobScheduleCSSP();
    ...
    con =   ConnectionManager.getConnection();
		...
		rset =  stmt.executeQuery("select nvl(to_char(LAG_TIME,'HH24'),'00') hr,nvl(to_char(LAG_TIME,'mi'),'00') min from MR_SMRP_CONFIG where SMRP_ID='CSSP'");
		String hr_lag = "00";
		String min_lag = "00";
		if(rset!=null && rset.next()){
    ...
    timer.scheduleAtFixedRate(JobScheduleCSSP, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
  //  timer.scheduleAtFixedRate(JobScheduleCSSP, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createCSSP(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleCSSP Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
