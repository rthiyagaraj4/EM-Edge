...
public class 	GeneratePLOPSPL extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobSchedulePLOPSPL js =new JobSchedulePLOPSPL();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createPLOPSPL function which will create the JSON file*/
			GeneratePLOPSPL Gjson= new GeneratePLOPSPL();

			String PLOPSPL_fileCreated_flag =Gjson.createPLOPSPL(facility_id,from_date,to_date);

			String error="";
			if(PLOPSPL_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (PLOPSPL_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (PLOPSPL_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + PLOPSPL_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createPLOPSPL(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				/*The View which is created for this PLOPSPLJson purpose*/
				...
				*/
				...
				String sqlQuery		  = "SELECT * FROM MR_PLOP_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999 and patient_class ='OP' and FACILITY_CODE='"+facility_id+"' and refServiceSubTypeCode='08' ORDER BY VISIT_ADM_DATE_TIME";
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_PLOPSPL.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- PLOPSPL-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						//System.out.println("json content-->"+str_head.toString());
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File PLOPSPLjson = new File (dir);
						FileWriter file = new FileWriter(PLOPSPLjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("PLOPSPLjson.getPath()-->"+PLOPSPLjson.getPath());
						//System.out.println("PLOPSPLjson.getAbsolutePath()-->"+PLOPSPLjson.getAbsolutePath());
						//System.out.println("PLOPSPLjson.getName()-->"+PLOPSPLjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPSPL",PLOPSPLjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  PLOPSPL generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPSPL","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createPLOPSPL function Inside try:"+e.toString());
						System.out.println("Error while file obj  PLOPSPL generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPSPL","IO Error. Unable to create File for PLOPSPL generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for PLOPSPL generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPSPL","No data for PLOPSPL generation");
				}
				
			}	 //check in function control				
		
		}catch (Exception e) {
			  System.err.println("Exception in createPLOPSPL function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPSPL",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobSchedulePLOPSPL extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobSchedulePLOPSPL = new JobSchedulePLOPSPL();
    ...
    con =   ConnectionManager.getConnection();
		...
		rset =  stmt.executeQuery("select nvl(to_char(LAG_TIME,'HH24'),'00') hr,nvl(to_char(LAG_TIME,'mi'),'00') min from MR_SMRP_CONFIG where SMRP_ID='PLOPSPL'");
		String hr_lag = "00";
		String min_lag = "00";
		if(rset!=null && rset.next()){
    ...
    timer.scheduleAtFixedRate(JobSchedulePLOPSPL, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
    //timer.scheduleAtFixedRate(JobSchedulePLOPSPL, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createPLOPSPL(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobSchedulePLOPSPL Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
