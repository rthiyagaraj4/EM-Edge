...
public class 	GeneratePLOPD extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobSchedulePLOPD js =new JobSchedulePLOPD();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createPLOPD function which will create the JSON file*/
			GeneratePLOPD Gjson= new GeneratePLOPD();

			String PLOPD_fileCreated_flag =Gjson.createPLOPD(facility_id,from_date,to_date);

			String error="";
			if(PLOPD_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (PLOPD_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (PLOPD_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + PLOPD_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createPLOPD(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				/*The View which is created for this PLOPDJson purpose*/
				...
				*/
				...
				String sqlQuery		  = "SELECT * FROM MR_PLOP_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999 and patient_class ='OP' and FACILITY_CODE='"+facility_id+"' and refServiceSubTypeCode='07' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String fl_name=filename+"_PLGOPD";
				...
				filename=filename+"_PLGOPD.json"; 
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_PLOPD.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- PLOPD-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						//System.out.println("json content-->"+str_head.toString());
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File PLOPDjson = new File (dir);
						FileWriter file = new FileWriter(PLOPDjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("PLOPDjson.getPath()-->"+PLOPDjson.getPath());
						//System.out.println("PLOPDjson.getAbsolutePath()-->"+PLOPDjson.getAbsolutePath());
						//System.out.println("PLOPDjson.getName()-->"+PLOPDjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPD",PLOPDjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  PLOPD generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPD","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createPLOPD function Inside try:"+e.toString());
						System.out.println("Error while file obj  PLOPD generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPD","IO Error. Unable to create File for PLOPD generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for PLOPD generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPD","No data for PLOPD generation");
				}
				
			}	 //check in function control				
		
		}catch (Exception e) {
			  System.err.println("Exception in createPLOPD function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPD",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobSchedulePLOPD extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobSchedulePLOPD = new JobSchedulePLOPD();
    ...
    con =   ConnectionManager.getConnection();
		...
		rset =  stmt.executeQuery("select nvl(to_char(LAG_TIME,'HH24'),'00') hr,nvl(to_char(LAG_TIME,'mi'),'00') min from MR_SMRP_CONFIG where SMRP_ID='PLOPD'");
		String hr_lag = "00";
		String min_lag = "00";
		if(rset!=null && rset.next()){
    ...
    timer.scheduleAtFixedRate(JobSchedulePLOPD, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
    //timer.scheduleAtFixedRate(JobSchedulePLOPD, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createPLOPD(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobSchedulePLOPD Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
