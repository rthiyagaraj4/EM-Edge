...
public class PERPL206SetupServlet extends javax.servlet.http.HttpServlet 
{
		...
		try
		{
			...
			try
			{
				...
				for(i=0;i<count;i++)
				{
					...
					{
						sqlDel ="delete from mr_report_per_pl206 where facility_id = ? and service_code = ? and clinic_code = ?";
					...
					{ 
						...
						String sql = "insert into mr_report_per_pl206 (facility_id, service_code, clinic_code, added_by_id, added_date, added_at_ws_no, added_facility_id, modified_by_id, modified_date, modified_at_ws_no, modified_facility_id) values  (?, ?, ?, ?, sysdate, ?, ?, ?, sysdate, ?, ?)";
				...
				//out.println("Exceptioina at PERPL206SetupServlet"+x.getMessage());
				status=false;
				con.rollback();
				error=x.getMessage();
				x.printStackTrace();
