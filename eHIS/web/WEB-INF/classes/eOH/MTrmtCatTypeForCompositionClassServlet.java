...
public class MTrmtCatTypeForCompositionClassServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	public void modifyData(HttpServletRequest req, HttpServletResponse res)	throws javax.servlet.ServletException,IOException{
		...
		try{
			...
			String composition_class_code= req.getParameter("composition_class_code");
			...
			pstmt=con.prepareStatement("DELETE FROM OH_COMPOSN_CLASS_TRMT_CAT_TYPE WHERE COMPOSITION_CLASS_CODE =? ");
			...
			pstmt=con.prepareStatement("INSERT INTO OH_COMPOSN_CLASS_TRMT_CAT_TYPE(COMPOSITION_CLASS_CODE, TRMT_CATEGORY_TYPE, ADDED_BY_ID, ADDED_DATE, ADDED_AT_WS_NO, ADDED_FACILITY_ID, MODIFIED_BY_ID, MODIFIED_DATE, MODIFIED_AT_WS_NO, MODIFIED_FACILITY_ID) VALUES(?, ?, ?, SYSDATE, ?, ?, ?, SYSDATE, ?, ?)");
			...
			for(int i=1;i<=row_count_int;i++){
				...
				if(chk_trtmt_yn.equals("Y")){
					...
					pstmt.setString(1,composition_class_code);			// COMPOSITION_CLASS_CODE
			...
			try{
				System.err.println("Rollback performed, Err Msg in MTrmtCatTypeForCompositionClassServlet"+e.getMessage());
				con.rollback();
			}catch(Exception e_1){
				System.err.println("MTrmtCatTypeForCompositionClassServlet :Caught Exception during rollback"+e_1);
			}
			System.err.println("MTrmtCatTypeForCompositionClassServlet:Err Msg in MTrmtCatTypeForCompositionClassServlet"+e.getMessage());
