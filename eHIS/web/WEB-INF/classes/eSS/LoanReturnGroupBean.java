...
import eSS.LoanReturnGroup.*;
import eSS.Common.*;
import eCommon.Common.CommonRepository;


public class LoanReturnGroupBean extends SsTransaction implements java.io.Serializable {
	...
	private Hashtable htDiscrepancyDetails;
	private Hashtable htSelectDiscrepancyDetail;
	private Hashtable htMaxIssuedQty;
	private Hashtable htDiscrepancyMain;
	private Hashtable htDiscrepancyKey;
	private Hashtable htSelectedDiscrepancyKey;
	private Hashtable htGroupTrayDiscrepancyKey;
	private ArrayList alDiscrepancyDetailsKey;
	// Used for editing the selected discrepancy detail
	private String currentEditingDiscrepancy = null;
	// Default Sterile Maps
	private HashMap hmSterileTypes = null;
	...
	public LoanReturnGroupBean() {
		alTrayDetails = new ArrayList();
		alSelectedTrayDetails = new ArrayList();
		alDiscrepancyDetailsKey = new java.util.ArrayList(); 
		htMaxIssuedQty = new Hashtable();
		htDiscrepancyDetails = new Hashtable();
		htDiscrepancyMain = new Hashtable();
		htDiscrepancyKey = new Hashtable();
		htSelectedDiscrepancyKey = new Hashtable();
		htGroupTrayDiscrepancyKey = new Hashtable();
		hmSterileTypes = new HashMap();
		alAddedTrayDetails = new ArrayList();
	}
	...
	public void selectDiscrepancyDetails(String key) {
		if (htDiscrepancyMain.containsKey(key)) {			
			htDiscrepancyDetails = (Hashtable) htDiscrepancyMain.get(key);
			alDiscrepancyDetailsKey = (ArrayList) htDiscrepancyKey.get(key);
		}
		else {
			
			setCurrentEditingDiscrepancy(null);
			htDiscrepancyDetails = new Hashtable();
			alDiscrepancyDetailsKey = new ArrayList();
		}
	}
	...
	public void setAll(Hashtable htResult) {
		...
		setFinalized_yn(((String)htResult.get("finalize_yn")).equals(SsRepository.getSsKeyValue("YES"))?"Y":"N");
		setIsAutoNo(htResult.get("autono_yn").equals((String)SsRepository.getSsKeyValue("YES"))?true:false);
	...
	public String getDocTypes() {
		try {
			HashMap defaultParameter = getSSParameter();
			String defaultLoanReturnGroup = (String) defaultParameter.get("DEF_RTL_DOC_TYPE_CODE");
			return getDoc_type_code_ListOptions("RTL", defaultLoanReturnGroup);
		}
		catch (Exception exception) {
			exception.printStackTrace();
			return null;
	...
	//Added by Sakti to avoid problem of multiple document type above method is not required
	public String getDoc_type_code_List() {
		return getDoc_type_code_ListOptions("RTL", getDoc_type_code());
	}
	...
	//SQL_SS_ITEM_FOR_GROUP_SELECT_LIST
		...
		ArrayList alOptionValues = getListOptionArrayList(sql,group_code);
	...
	public String getReturnFrom(String dflt_store) {
		String sql = (String) getSsRepositoryValue("SQL_SS_LOAN_RETURN_FROM_STORE_SELECT_LIST");
		ArrayList alParam = new ArrayList();
		alParam.add(login_facility_id);
		alParam.add(login_by_id);
		alParam.add(getLanguageId());
		try{
			int cnt = Integer.parseInt((String)fetchRecord(getSsRepositoryValue("SQL_SS_LOAN_RETURN_FROM_STORE_SELECT_LIST_COUNT"),alParam).get("COUNT"));
		...
		ArrayList alOptionValues = getListOptionArrayList(sql, alParam);
			return getListOptionTag(alOptionValues,dflt_store);
	}

	public String getReturnTo(String dflt_store) {
		String sql = (String) getSsRepositoryValue("SQL_SS_LOAN_RETURN_TO_STORE_SELECT_LIST");
		ArrayList alParam = new ArrayList();
		alParam.add(login_facility_id);
		alParam.add(login_by_id);
		alParam.add(getLanguageId());
		try{
			int cnt = Integer.parseInt((String)fetchRecord(getSsRepositoryValue("SQL_SS_LOAN_RETURN_TO_STORE_SELECT_LIST_COUNT"),alParam).get("COUNT"));
	...
	public String getOptionValues(String sql,String defaultValue) {
		...
		ArrayList alOptionValues = getListOptionArrayList(sql,defaultValue);
	...
	public ArrayList getSelectTray() {
		ArrayList alTrays = null;
		Connection connection = null;
		PreparedStatement preparedStatement	= null;
		ResultSet resultSet = null;
		String sql = (String)SsRepository.getSsKeyValue("SQL_SS_TRAY_DTLS_RTL_QUERY_RESULT");
		int index = 0;
		try {
			connection = getConnection();
			...
			preparedStatement.setString(3, getLanguageId ());
			preparedStatement.setString(4, getLanguageId ());
			preparedStatement.setString(5, getLanguageId ());
			resultSet = preparedStatement.executeQuery();
			alTrays = new ArrayList();
...
//Added by sakti against inc#43893
	...
	public void updateSelectedTraysValues(Hashtable htTrays) {
		int total = Integer.parseInt((String) htTrays.get("total_checkboxes"));
		
		for (int i=0;i<total;i++) {
		String[] updateTrayValue = (String[]) alTrayDetails.get(i);

		updateTrayValue[0] = (String) htTrays.get("group_code_"+i); //group_code
		updateTrayValue[1] = (String) htTrays.get("group_desc_"+i); //group_desc
		updateTrayValue[2] = (String) htTrays.get("tray_no_"+i); //tray_no
		updateTrayValue[3] = (String) htTrays.get("bin_location_code_"+i); //bin_location_Code
		//updateTrayValue[4] = (String) htTrays.get("expiry_date"); //expiry_date
		updateTrayValue[4] = (String) htTrays.get("expired_yn_"+i); //expiry_date
		updateTrayValue[5] = (String) htTrays.get("non_returnable_yn_"+i); //nonreturnable_yn
		updateTrayValue[6] = (String) htTrays.get("patient_id_"+i); // patient_id
		updateTrayValue[7] = (String) htTrays.get("expired_yn_"+i); //Expired_yn
		updateTrayValue[8] = (String) htTrays.get("used_yn_"+i); //used_yn
		updateTrayValue[9] = (String) htTrays.get("sterilize_yn_"+i);//sterilize_yn
		updateTrayValue[10] = (String) htTrays.get("wash_yn_"+i);//wash_yn
		updateTrayValue[11] = (String) htTrays.get("pack_yn_"+i);//pack_yn
		updateTrayValue[12] = (String) htTrays.get("contaminated_yn_"+i);//contaminated_yn
		updateTrayValue[13] = (String) htTrays.get("charge_consumable_yn_"+i);//
		updateTrayValue[15] = (String) htTrays.get("updated_"+i); //can_update ?
		updateTrayValue[16] = (String) htTrays.get("patient_name_"+i); //patient_name?
		//updateTrayValue[21] = bin_desc; //patient_name?
		updateTrayValue[22] = (String) htTrays.get("remarks_code_"+i); // Remarks code #Inc no:39364 (Bru-HIMS-CRF-360) on 30-Apr-2013
		updateTrayValue[23] = (String) htTrays.get("remarks_"+i); // Remarks #Inc no:39364 (Bru-HIMS-CRF-360) on 30-Apr-2013

	}
		
}
//Added by Sakti to get and set all previously selected values if any from the list part
	public void updateSelectedList(Hashtable htTrays) throws Exception{
		...
		for (int i=0;i<total;i++) {
			//String[] trayInfo = new String[22];
			String[] trayInfo = new String[24];
			trayInfo[0] = (String) htTrays.get("group_code_"+i); //group_code
			trayInfo[1] = (String) htTrays.get("group_desc_"+i); //group_desc
			trayInfo[2] = (String) htTrays.get("tray_no_"+i); //tray_no
			trayInfo[3] = (String) htTrays.get("bin_location_code_"+i); //bin_location_Code
			trayInfo[4] = (String) htTrays.get("expiry_date_"+i); //expiry_date
			trayInfo[5] = (String) htTrays.get("non_returnable_yn_"+i); //nonreturnable_yn
			trayInfo[6] = (String) htTrays.get("patient_id_"+i);// patient_id

			trayInfo[7] = (String) htTrays.get("expired_yn_"+i); //Expired_yn
			trayInfo[8] = (String) htTrays.get("used_yn_"+i); //used_yn
			trayInfo[9] = (String) htTrays.get("sterilize_yn_"+i);//sterilize_yn
			trayInfo[10] = (String) htTrays.get("wash_yn_"+i);//wash_yn
			trayInfo[11] = (String) htTrays.get("pack_yn_"+i);//pack_yn
			trayInfo[12] = (String) htTrays.get("contaminated_yn_"+i);//contaminated_yn

			
			trayInfo[13] = (String) htTrays.get("charge_consumable_yn_"+i);//charge_consumable_yn default unchecked Added by Sakti against BRU-HIMS-CRF-0360 requirement
			trayInfo[14] = "N";//
			trayInfo[15] = "N";//can_update ?
		
			trayInfo[16] = (String) htTrays.get("patient_name_"+i);//patient_name?
			trayInfo[17] = "0";//returnable_inst_cost
			trayInfo[18] = "0";//missing_used_inst_cost
			trayInfo[19] = (String) htTrays.get("iss_doc_type_code_"+i);//iss_doc_type_code
			trayInfo[20] = (String) htTrays.get("iss_doc_no_"+i);//iss_doc_no
			trayInfo[21] = (String) htTrays.get("bin_desc_"+i);//bin_desc
			//trayInfo[22] = (String) htTrays.get("remarks_code"+i);//Remarks code 
			trayInfo[22] = (String) htTrays.get("remarks_"+i);//Remarks desc 

			alTrayDetails.set(i,trayInfo); //Setting all the prviously selected values index wise
	...
	public String getBinLocationsList(String group_code,String tray_no,String deft_bin_location) {
		
		String owner_store_code = "";
		try{
			ArrayList storeData = new ArrayList();			
			storeData.add(group_code);
			storeData.add(tray_no);
			owner_store_code = (String)fetchRecord("select owner_store_code from ss_tray_dtls where group_code=? and tray_no=?",storeData).get("OWNER_STORE_CODE");			
		}
		catch(Exception e){
			e.printStackTrace();
		}
		
		String sql = (String) getSsRepositoryValue("SQL_MM_BIN_LOCATION_SELECT_LIST");
		ArrayList binLocationData = new ArrayList();
		binLocationData.add(owner_store_code);
		binLocationData.add(getLanguageId());
		
		ArrayList alOptionValues = getListOptionArrayList(sql,binLocationData);
		return getListOptionTag(alOptionValues,deft_bin_location);
	}
	...
	public String getDiscrepancyReasons() {
		...
		ArrayList alOptionValues = getListOptionArrayList(sql,getLanguageId());
	...
	public boolean validateEncounter(String encounter_id,String patient_id) {
		...
		PreparedStatement preparedStatement	= null;
		ResultSet resultSet = null;
		String sql = (String)SsRepository.getSsKeyValue("SQL_PR_ENCOUNTER_COUNT");
		int value = -1;
		try {
			connection = getConnection();
			preparedStatement = connection.prepareStatement(sql);
	...
	public void loadDataTrayDetails() {
		...
		try {
			...
			connection = getConnection();
			preparedStatement = connection.prepareStatement(SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DTL_SELECT_MULTIPLE")); 
			...
			preparedStatement.setString(7, getLanguageId().trim());
			resultSet = preparedStatement.executeQuery();
			while (resultSet.next()) {
	...
	public String[] getTrayDetailArray(ResultSet rs) {
			String[] trayInfo = new String[24];
			try {
	...
	public void loadData() {
		...
		try {
			setFacility_id(login_facility_id);
			connection = getConnection();
			preparedStatement = connection.prepareStatement(SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_HDR_SELECT_SINGLE")); 
	...
	public HashMap updateDiscrepancyDetail(Hashtable hash) {
		...
		try {
			...
			key.append('|');//replaced - with | for 21052
	...
	public HashMap insert() {
		...
		try {
			...
			//alHDRInsertData.add(getFinalized_yn().substring(0,1));
			...
			hmSQLMap.put("InsertHDRSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_HDR_INSERT"));
			...
			hmSQLMap.put("InsertDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DTL_INSERT"));
			...
			if ((htSelectedDiscrepancyKey != null) && (htSelectedDiscrepancyKey.size()>0)) {
				...
				hmSQLMap.put("InsertDISDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DISCREPANCY_DTL_INSERT"));
			...
			hmSQLMap.put("UpdateSQL", SsRepository.getSsKeyValue("SQL_SY_ACC_ENTITY_DOC_TYPE_UPDATE"));
		...
		LoanReturnGroupRemote LoanReturnGroupRemote = null ;
		try {

			Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome(SsRepository.getSsKeyValue("JNDI_SS_LOAN_RETURN_GROUP") ,LoanReturnGroupHome.class,getLocalEJB());
		...
		if( ((Boolean) hmResult.get("result")).booleanValue() )	
			...
			try {
				if(LoanReturnGroupRemote != null)
					LoanReturnGroupRemote.remove() ;
			}
			catch(Exception ee) {
	...
	public HashMap modify() {
		...
		if ((db_entry_completed_yn.equals("Y")) && (finalized_yn.equals("Y"))) {
			...
			hmSQLMap.put("UpdateSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_HDR_FINALIZE_UPDATE"));
			hmSQLMap.put("UpdateDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DTL_AUDIT_UPDATE"));
			...
			String count_sql = (String)SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DISCREPANCY_DTL_COUNT");
			if (hasDiscrepancyRecords(alDTLCheckData,count_sql)) {
				hmSQLMap.put("UpdateDISDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DISCREPANCY_DTL_AUDIT_UPDATE"));
			...
			try {
				...
				alHDRUpdateData.add(getDoc_type_code());
				alHDRUpdateData.add(getDoc_no());
				hmSQLMap.put("UpdateHDRSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_HDR_UPDATE"));
				...
				if (deleteDetailIndex != null && deleteDetailIndex.length>0) {
					for (int index=0;index<deleteDetailIndex.length;index++) {
						...
						String count_sql = (String)SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DISCREPANCY_DTL_ITEM_COUNT");
						if (hasDiscrepancyRecords(alDTLDeleteRecord,count_sql)) {
							alDTSDTLDeleteData.add(alDTLDeleteRecord);
						}
					}
				}
				hmSQLMap.put("DeleteDTSDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DISCREPANCY_DTL_ITEM_DELETE"));
				hmTableData.put("DeleteDTSDTLData", alDTLDeleteData);
				
				hmSQLMap.put("DeleteDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DTL_ITEM_DELETE"));
				hmTableData.put("DeleteDTLData", alDTLDeleteData);
				hmSQLMap.put("UpdateDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DTL_UPDATE"));
				hmTableData.put("UpdateDTLData", alDTLUpdateData);

				hmSQLMap.put("UpdateDISDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DISCREPANCY_DTL_UPDATE"));
	...
	public HashMap delete() {
		...
		if (hasDiscrepancyRecords(alDeleteData,count_sql)) {
				...
				hmSQLMap.put("DeleteDISDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DISCREPANCY_DTL_DELETE"));
		}
		hmTableData.put("DeleteDTLData",alDeleteData);
		hmSQLMap.put("DeleteDTLSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DTL_DELETE"));
		hmTableData.put("DeleteHDRData",alDeleteData);
		hmSQLMap.put("DeleteHDRSQL", SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_HDR_DELETE"));
	...
	public java.util.ArrayList getDTLUpdateValue(String[] tempTrayDTL) {
		...
		alTempDTLRecord.add(checkForNull(tempTrayDTL[23]));//Remarks
	...
	public java.util.ArrayList getDTLValue(String[] tempTrayDTL) {
		...
		alTempDTLRecord.add(checkForNull(tempTrayDTL[23]));
		return alTempDTLRecord;
	}
	...
	public HashMap validate() {
		...
		try {
			...
			if ((!isAutoNo) && (mode.equals(CommonRepository.getCommonKeyValue("MODE_INSERT")))) {
				preparedStatement = connection.prepareStatement(SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_HDR_COUNT"));
			...
			preparedStatement = connection.prepareStatement(SsRepository.getSsKeyValue("SQL_SS_GROUP_DEF_STERILE_SELECT"));
			resultSet = preparedStatement.executeQuery();
			while (resultSet.next()) {
	...
	public ArrayList getDiscrepancyDetail(String[] stringArray) {
		...
		try {
			...
			HashMap hmRecord = fetchRecord(getSsRepositoryValue("SQL_SS_TRAY_DTLS_LOAN_ISSUE_DATE_SELECT"),alParam);
	...
	public java.util.ArrayList getDiscrepancyDetailsKey(String group_code,String tray_no) {
		...
		if(alDiscrepancyDetailsKey.size()>0){
			...
			for(int i=0;i<alDiscrepancyDetailsKey.size();i++){
				java.util.StringTokenizer  strTokens=new java.util.StringTokenizer(((String)alDiscrepancyDetailsKey.get(i)),"|");//replaced - with | for 21052
				while (strTokens.hasMoreElements()) {
				...
				da.add(tokens[0] + "|"+ tokens[1]+"|"+tokens[2]+"|"+tokens[3]);//replaced - with | for 21052
				}
			   }
			}
			return da;
	...
	public void loadIssueQty(String group_code) {
		...
		PreparedStatement preparedStatement	= null;
		ResultSet resultSet = null;
		String sql = (String)SsRepository.getSsKeyValue("SQL_SS_ITEM_FOR_GROUP_QTY_SELECT");
	...
	public void loadDiscrepancyDetails(String group_code,String tray_no) {
		...
		PreparedStatement preparedStatement	= null;
		ResultSet resultSet = null;
		String sql = (String)SsRepository.getSsKeyValue("SQL_SS_LOAN_RETURN_DISCREPANCY_DTL_MULTIPLE_SELECT");
		...
		try {
			...
			while(resultSet.next()){
				...
				alDiscrepancyDetailsKey.add(discrepancyInfo[0] + "|"+ discrepancyInfo[1]+"|"+discrepancyInfo[4]+"|"+discrepancyInfo[2]);//replaced - with | for 21052
				htDiscrepancyDetails.put (discrepancyInfo[0] + "|"+ discrepancyInfo[1]+"|"+discrepancyInfo[4]+"|"+discrepancyInfo[2],discrepancyInfo);//end 
			...
			sql = (String)SsRepository.getSsKeyValue("SQL_SS_DISCREPANCY_DTL_RETURN_GROUP_MULTIPLE_SELECT");
