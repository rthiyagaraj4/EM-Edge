...
----------------------------------------------------------------------------------------------------------------------------------------------------------
04/09/2015	IN057333		Badmavathi B 								Stock updated wrongly in Request Vendor Loan for pending request quantity
...
import eSS.VendorLoanRequest.*;
import javax.servlet.http.HttpSession;

public class VendorLoanRequestBean extends SsTransaction implements Serializable {

	private String doc_ref;
	private String trn_type;
	private String req_by_store_code;
	private String authorized_by_id;
	private String req_on_vendor_code;
	...
	public void setDoc_ref(String doc_ref) {
		this.doc_ref = (doc_ref != null)?doc_ref.toUpperCase():""; //checkForNull(doc_ref); changed against AAKH-CRF-0058.1 by B.Badmavathi
	}
	...
	public void setReq_on_vendor_code(String req_on_vendor_code) {
		this.req_on_vendor_code = req_on_vendor_code;
	}

	public String getReq_on_vendor_code() {
		return req_on_vendor_code;
	}
	...
	public String getDoc_type_code_List() {
		return getDoc_type_code_ListOptions("RQV", getDoc_type_code());
	}
	...
	public String getReq_on_vendor_code_List() {
		return getListOptionTag(
				getListOptionArrayList(
						...
						getLanguageId()), getReq_on_vendor_code());
	}
	...
	public String getReq_by_store_code_List() {
		...
		return getListOptionTag(
				getListOptionArrayList(
						getSsRepositoryValue("SQL_SS_VENDOR_LOAN_REQ_BY_STORE_SELECT_LIST"),
	...
	public void selectedBatches(Hashtable htTrays) {
		...
		while (st.hasMoreTokens()) {
			...
			dataList.put("TOTAL_QTY",(String) htTrays.get("total_qty" + i));
		...
		while (ss.hasMoreTokens()) {
			...
			dataList.put("TOTAL_QTY",(String) htTrays.get("total_qty" + j));
	...
	public void setAll(Hashtable htRecordSet) {
		...
		setReq_on_vendor_code((String) htRecordSet.get("req_on_vendor_code"));
		...
		try {
			if (!getMode().equals(getCommonRepositoryValue("MODE_MODIFY")))
				if (!isDocNoAutoGenYN(getDoc_type_code())) {
					...
					if (!fetchRecord(
							getSsRepositoryValue("SQL_SS_EXT_REQ_HDR_COUNT"),
	...
	public HashMap insert() {
		...
		try {
			...
			alHdrInsertData.add(getReq_on_vendor_code());
			...
			hmSQLMap.put("HdrInsertSQL",
					getSsRepositoryValue("SQL_SS_EXT_REQ_HDR_INSERT"));
			hmSQLMap.put("DtlInsertSQL",
					getSsRepositoryValue("SQL_SS_EXT_REQ_DTL_INSERT"));
			hmSQLMap.put("NextDocNoUpdateSQL",
					getSsRepositoryValue("SQL_SY_ACC_ENTITY_DOC_TYPE_UPDATE"));
		/*	hmSQLMap.put(Commented against IN057333
		...
		return VendorLoanRequestMethodCall(hmTableData, hmSQLMap,
				FUNCTION_INSERT);
	}
	...
	public HashMap modify() {
		...
		alCommonData.add(getReq_on_vendor_code());

		hmSQLMap.put("HdrModifySQL",
				getSsRepositoryValue("SQL_SS_EXT_REQ_HDR_UPDATE"));
		hmSQLMap.put("DtlModifySQL",
				getSsRepositoryValue("SQL_SS_EXT_REQ_DTL_UPDATE"));
		hmSQLMap.put("DtlInsertSQL",
				getSsRepositoryValue("SQL_SS_EXT_REQ_DTL_INSERT"));
		hmSQLMap.put("DtlDeleteSQL",
				getSsRepositoryValue("SQL_SS_EXT_REQ_DTL_DELETE_GROUP"));
	/*	hmSQLMap.put(Commented against IN057333
				"UpdateGroupForStoreSQL",
				getSsRepositoryValue("SQL_SS_GROUP_FOR_STORE_PEND_REQ_QTY_UPDATE"));
		hmSQLMap.put(
		...
		hmSQLMap.put("RequestQtySelectSQL",
				getSsRepositoryValue("SQL_SS_EXT_REQ_DTL_REQ_QTY_SELECT"));
	...
	public HashMap delete() {
		...
		hmSQLMap.put("HdrDeleteSQL",
				getSsRepositoryValue("SQL_SS_EXT_REQ_HDR_DELETE"));
		hmSQLMap.put("DtlDeleteSQL",
				getSsRepositoryValue("SQL_SS_EXT_REQ_DTL_DELETE"));
	/*	hmSQLMap.put(Commented against IN057333
				"UpdateGroupForStoreReduceSQL",
				getSsRepositoryValue("SQL_SS_GROUP_FOR_STORE_PEND_REQ_QTY_REDUCE_UPDATE"));
*/
	...
	public HashMap VendorLoanRequestMethodCall(HashMap hmTableData,
			HashMap hmSQLMap, int functionID) {

		HashMap hmResult = new HashMap();
		VendorLoanRequestRemote VendorLoanRequestRemote = null;
		hmResult.put(RESULT, FALSE);
		hmResult.put(MESSAGE, "Initial Message");

		try {
			Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome(
					getSsRepositoryValue("JNDI_SS_VENDOR_LOAN_REQUEST"),
					VendorLoanRequestHome.class, getLocalEJB());
			...
			default:
				hmResult.put("message",
						"Invalid Function ID passed to VendorLoanRequestMethodCall()");
			}
	...
	public void loadData() throws Exception {
		...
		HashMap hmRecord = fetchRecord(
				getSsRepositoryValue("SQL_SS_EXT_REQ_HDR_SELECT_SINGLE"),
				alParameters);

		setDoc_ref((String) hmRecord.get("DOC_REF"));
		setReq_by_store_code((String) hmRecord.get("REQ_BY_STORE_CODE"));
		setReq_on_vendor_code((String) hmRecord.get("REQ_TO_SUPP_CODE"));
	...
	public void loadDtlData() {
		...
		try {
			...
			stSQL = eSS.Common.SsRepository
					.getSsKeyValue("SQL_SS_EXT_REQ_DTL_SELECT_MULTIPLE");
			...
			for (int i = 0; i < listRecord.size(); i++) {
				...
				dataList.put("TOTAL_QTY",
						(String) dataList.get("TOTAL_QTY"));
				dataList.put(
						"SURGERY_TYPE",
						checkForNull(
