...
import java.util.*;
import java.io.*;
public class LoanRequestGroupListBean extends eSS.Common.SsTransaction implements Serializable { 
	...
	public String getAuth_group_code_List(){
		...
		alParameters.add(getReq_group_code());
		return getListOptionTag( getListOptionArrayList(getSsRepositoryValue("SQL_SS_ALTERNATE_GROUP_SELECT_LIST_FOR_LOAN"),alParameters), getAuth_group_code());
	}
	...
	public boolean isGroupInArrayList(ArrayList alLoanRequestGroupList, String stReq_group_code, int exceptionalIndex){
		boolean result=false;
		for (int i=0;i<alLoanRequestGroupList.size();i++) {
			if (exceptionalIndex == i) continue;
			if ( ((HashMap)alLoanRequestGroupList.get(i)).get("req_group_code").toString().equals(stReq_group_code) ) {
				result=true;
				break;
			}
		}
	...
	public void initialize(HashMap hmLoanRequestGroupItem){
		setGroup_type	 (hmLoanRequestGroupItem.get("group_type").toString()	);
		setReq_group_code(hmLoanRequestGroupItem.get("req_group_code").toString()		);
		setAuth_group_code(hmLoanRequestGroupItem.get("auth_group_code").toString()		);
		setAuth_group_desc(hmLoanRequestGroupItem.get("auth_group_desc").toString()		);
		setDescription   (hmLoanRequestGroupItem.get("description").toString()	);
		setRequest_qty   (hmLoanRequestGroupItem.get("request_qty").toString()			);
		setAvailable_qty (hmLoanRequestGroupItem.get("available_qty").toString()			);
		setMode          (hmLoanRequestGroupItem.get("mode").toString()			);
	}
}
