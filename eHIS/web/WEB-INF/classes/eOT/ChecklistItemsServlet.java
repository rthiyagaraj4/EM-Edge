...
public class ChecklistItemsServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	String login_user="";
	String table_name="OT_CHECKLIST_ITEMS";
	...
	private HashMap populateTabData(HttpServletRequest req, HttpServletResponse res){
		...
		if("insert".equals(mode)){
			String checklist_item_code=checkForNull(req.getParameter("checklist_item_code"));
			String description=checkForNull(req.getParameter("description"));
			String checklist_type=checkForNull(req.getParameter("checklist_type"));
			String list_id=checkForNull(req.getParameter("list_id"));
			String accept_remarks_yn=checkForNull(req.getParameter("accept_remarks_yn"),"N");
			String item_level=checkForNull(req.getParameter("item_level"));
			String status = checkForNull(req.getParameter("status"),"D");
			tabdata.put("checklist_item_code",checklist_item_code);
			tabdata.put("description",description);
			tabdata.put("checklist_type",checklist_type);
			tabdata.put("list_id",list_id);
			tabdata.put("accept_remarks_yn",accept_remarks_yn);
			tabdata.put("item_level",item_level);
			...
			String description=checkForNull(req.getParameter("description"),checkForNull(req.getParameter("description1")));
			String checklist_type=checkForNull(req.getParameter("checklist_type"));
			String list_id=checkForNull(req.getParameter("list_id"));
			String accept_remarks_yn=checkForNull(req.getParameter("accept_remarks_yn"),"N");
			String item_level=checkForNull(req.getParameter("item_level"));
	...
	private void insert(HttpServletRequest req, HttpServletResponse res){
		...
		String dupflds[]={"checklist_item_code"};
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		String checklist_item_code=checkForNull(req.getParameter("checklist_item_code1"));
		HashMap tabData = populateTabData(req,res);
		HashMap condflds=new HashMap();
		condflds.put("checklist_item_code",checklist_item_code);
		Object argArray[]  = new Object[4];
		argArray[0] = p;
		argArray[1] = tabData;				   
		argArray[2] = condflds;
