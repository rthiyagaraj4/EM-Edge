...
public class NoteTypeForSpecialitiesServlet extends javax.servlet.http.HttpServlet {
...
//		String modified_date="";
		...
		try{
				...
				String select_value = "";
				String msg="";
                String sqlDelete="DELETE FROM OT_NOTE_TYPE_FOR_SPECIALTIES WHERE SPECIALTY_CODE=? AND LOCN_TYPE=? AND NOTE_TYPE=?";
				String sqlInsert="INSERT INTO OT_NOTE_TYPE_FOR_SPECIALTIES (LOCN_TYPE,SPECIALTY_CODE,NOTE_TYPE,ADDED_BY_ID,ADDED_DATE,ADDED_AT_WS_NO,ADDED_FACILITY_ID,MODIFIED_BY_ID,MODIFIED_DATE,MODIFIED_AT_WS_NO,MODIFIED_FACILITY_ID) VALUES (?,?,?,?,SYSDATE,?,?,?,SYSDATE,?,?)";
				...
				if(tabData!=null && tabData.size()>0){
					...
					locn_type = (String )  map.get("location_type_desc");
					note_type = (String )  map.get("note_type");
					select_value = (String) map.get("select_value");
					//pstmt = select_value.equals("N")?pstmt1:pstmt2;
					if(select_value.equals("N")){
						// pstmt = pstmt1;
					     pstmt1.setString(1,speciality_code);
						 pstmt1.setString(2,locn_type);
						 pstmt1.setString(3,note_type);
			             flag = pstmt1.executeUpdate();
						 flag_arr.add(""+flag);
						//pstmt.addBatch();
					}else{
						//pstmt=pstmt2;
						pstmt2.setString(1,locn_type);
						pstmt2.setString(2,speciality_code);
						pstmt2.setString(3,note_type);
