...
public class OrderTypesForSpecialityServlet extends javax.servlet.http.HttpServlet {
...
//		String modified_date="";
		...
		try{
				...
				String select_value = "";
				String msg="";
                String sqlDelete="DELETE FROM OT_SPECIALITY_ORDER_TYPES WHERE SPECIALITY_CODE=? AND ORDER_TYPE_CODE=?";
				String sqlInsert="INSERT INTO OT_SPECIALITY_ORDER_TYPES (SPECIALITY_CODE,ORDER_TYPE_CODE,ADDED_BY_ID,ADDED_DATE,ADDED_AT_WS_NO,ADDED_FACILITY_ID,MODIFIED_BY_ID,MODIFIED_DATE,MODIFIED_AT_WS_NO,MODIFIED_FACILITY_ID) VALUES (?,?,?,SYSDATE,?,?,?,SYSDATE,?,?)";
				...
				if(tabData!=null && tabData.size()>0){
					...
					order_type_code = (String )  map.get("order_type_code");
					...
					if(select_value.equals("N")){
						// pstmt = pstmt1;
						 ...
						 pstmt1.setString(2,order_type_code);
			             flag = pstmt1.executeUpdate();
						 flag_arr.add(""+flag);
						//pstmt.addBatch();
					}else{
						//pstmt=pstmt2;
						pstmt2.setString(1,speciality_code);
						pstmt2.setString(2,order_type_code);
			    ...
			    System.err.println("Err Msg in OrderTypesForSpecialityServlet"+e.getMessage());
				e.printStackTrace();
	}finally{
		try{
