...
public class ComponentsServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	String login_user="";
	String table_name="OT_SS_COMPONENTS";
...
//		String locale = p.getProperty("LOCALE");
	...
	private HashMap populateTabData(HttpServletRequest req, HttpServletResponse res){
		...
		if("insert".equals(mode)){
			String item_code=checkForNull(req.getParameter("item_code"));
			String item_desc=checkForNull(req.getParameter("item_desc"));
//			String short_desc=checkForNull(req.getParameter("short_desc"));
//			String long_desc=checkForNull(req.getParameter("long_desc"));
			String location=checkForNull(req.getParameter("location"));//Added by MuthuN against RUT-CRF-0091
	//Added by Muthu for RUT-CRF-0057
			String image_file=checkForNull(req.getParameter("image_file"));
	//Added by Muthu for RUT-CRF-0057
			String cross_ref_st_item_code=checkForNull(req.getParameter("code"),"null");
			String consumable_yn = checkForNull(req.getParameter("status"),"N");
			tabdata.put("item_code",item_code);
			tabdata.put("item_desc",item_desc);
			tabdata.put("location",location);//Added by Muthu against RUT-CRF-0091
			tabdata.put("consumable_yn",consumable_yn);
			tabdata.put("cross_ref_st_item_code",cross_ref_st_item_code);
	//Added by Muthu for RUT-CRF-0057
			tabdata.put("image_file",image_file);
	//Added by Muthu for RUT-CRF-0057
			...
			String item_desc=checkForNull(req.getParameter("item_desc"),req.getParameter("item_desc1") );
			String cross_ref_st_item_code=(checkForNull(req.getParameter("desc")).equals(""))?"null":req.getParameter("code");
			String consumable_yn = checkForNull(req.getParameter("status"),"N");
			String location=checkForNull(req.getParameter("location"));//Added by Muthu against RUT-CRF-0091
//Added by Muthu for RUT-CRF-0057
		//Modified by Rajesh to store empty Image
		//String image_file=checkForNull(req.getParameter("image_file"),req.getParameter("image_file1"));
		String image_file=checkForNull(req.getParameter("image_file"));
		//Modified by Rajesh to store empty Image
//Added by Muthu for RUT-CRF-0057
			tabdata.put("item_desc",item_desc);
...
//Added by Muthu for RUT-CRF-0057
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		String item_code=checkForNull(req.getParameter("item_code1"));
		HashMap tabData = populateTabData(req,res);
		HashMap condflds=new HashMap();
		condflds.put("item_code",item_code);
		Object argArray[]  = new Object[4];
		argArray[0] = p;
		argArray[1] = tabData;				   
		argArray[2] = condflds;
