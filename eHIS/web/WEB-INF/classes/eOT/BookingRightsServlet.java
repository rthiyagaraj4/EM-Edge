...
public class BookingRightsServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	String login_user="";
	String table_name="OT_BOOKING_ACCESS_RIGHTS";
	...
	private HashMap populateTabData(HttpServletRequest req, HttpServletResponse res){
		...
		if("insert".equals(mode)){
			String appl_user_id=checkForNull(req.getParameter("appl_user_name"));
			String book_schedule_yn=checkForNull(req.getParameter("book_schedule_yn"),"N");
			String cancel_schedule_yn=checkForNull(req.getParameter("cancel_schedule_yn"),"N");
			String transfer_schedule_yn=checkForNull(req.getParameter("transfer_schedule_yn"),"N");
			String overbooking_allowed_yn=checkForNull(req.getParameter("overbooking_allowed_yn"),"N");//Added by lakshmi against MO-CRF-20058
			String overide_splty_restriction_yn=checkForNull(req.getParameter("overide_splty_restriction_yn"),"N");
			//Added on 9/16/2010 for crf
			String restrict_bkg_display_to_self=checkForNull(req.getParameter("restrict_bkg_display_to_self"),"N");

			tabdata.put("appl_user_id",appl_user_id);
			tabdata.put("operating_facility_id",facility_id);
			tabdata.put("book_schedule_yn",book_schedule_yn);
			tabdata.put("cancel_schedule_yn",cancel_schedule_yn);
			tabdata.put("transfer_schedule_yn",transfer_schedule_yn);
			tabdata.put("over_book_yn",overbooking_allowed_yn);//Added by lakshmi against MO-CRF-20058
			tabdata.put("overide_splty_restriction_yn",overide_splty_restriction_yn);	
			...
			tabdata.put("restrict_bkg_display_to_self",restrict_bkg_display_to_self );
...
//			tabdata.put("operating_facility_id",facility_id);
	...
	private void insert(HttpServletRequest req, HttpServletResponse res){
		...
		String dupflds[]={"operating_facility_id","appl_user_id"};
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		String appl_user_id=checkForNull(req.getParameter("appl_user_id1"));
		HashMap tabData = populateTabData(req,res);
		HashMap condflds=new HashMap();
		condflds.put("appl_user_id",appl_user_id);
		condflds.put("operating_facility_id",facility_id);
		Object argArray[]  = new Object[4];
		argArray[0] = p;
		argArray[1] = tabData;				   
		argArray[2] = condflds;
