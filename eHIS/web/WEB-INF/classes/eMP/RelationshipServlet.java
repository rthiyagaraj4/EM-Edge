 ...
 ******************************************************************************/
/****** Last Modified Date Time 9/22/2005 11:51 AM ******/
package eMP;
...
public synchronized void doPost(HttpServletRequest req, HttpServletResponse res)
	...
	if ( operation.equals("insert") )
		insertRelationship(req, res);
	if ( operation.equals("modify"))
			modifyRelationship(req, res);
	}
	catch (Exception e)	{
...
public void doGet(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException {
	try	{
		...
		out.println("<html><head><link rel='stylesheet' type ='text/css' href='../eCommon/html/IeStyle.css'></link><script language='javascript' src='../eMP/js/Relationship.js'></script></head><frameset rows='42,*,50'><frame name='commontoolbarFrame'	src='"+source+"' frameborder=0 scrolling='no' noresize><frame name='f_query_add_mod' src='../eCommon/html/blank.html' frameborder=0><frame name='messageFrame' src='../eCommon/jsp/MstCodeError.jsp' frameborder=0 noresize scrolling='auto'></frameset></html> ");
	} catch(Exception e)	{	}
}


private void modifyRelationship(HttpServletRequest req, HttpServletResponse res)	{
	try {
	
	RelationshipCode = req.getParameter("relationship_code");
	longDesc = req.getParameter("long_desc");
	shortDesc = req.getParameter("short_desc");
	Sex = req.getParameter("sex_ind");
	Relationship = req.getParameter("relationship_level");
...
String modifiedAtWorkstation = addedAtWorkstation ;
			...
			if(Sex.equals("Male"))
				...
				Sex="U";
			/*else if(Sex.equals("Specify"))
				Sex="S";*/

			HashMap tabdata=new HashMap();
			tabdata.put("relationship_code",RelationshipCode);
			tabdata.put("long_desc",longDesc);
			tabdata.put("short_desc",shortDesc);
			tabdata.put("sex_ind",Sex);
			tabdata.put("relationship_level",Relationship);
			...
			condflds.put("relationship_code",RelationshipCode);
		...
		results.clear();
    	 res.sendRedirect("../eCommon/jsp/MstCodeError.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8") + "&err_value=" + error_value );
			...
			out.println( RelationshipCode + " "+longDesc + " "+shortDesc +" "+ effectiveDateFrom+" "+ effectiveDateTo + " "+ effectiveStatus ); */
			e.printStackTrace();
			}
	}
private void insertRelationship(HttpServletRequest req, HttpServletResponse res)	{
	try {
	...
	String modifiedAtWorkstation = addedAtWorkstation ;
			...
			String dupflds[]={"relationship_code"};
