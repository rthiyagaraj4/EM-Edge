 ...
 ******************************************************************************/
/****** Last Modified Date Time 9/22/2005 11:24 AM

 * @filename NameSuffixServlet.java
 *
...
public class NameSuffixServlet extends HttpServlet	
{
	PrintWriter out;
	Properties p;
...
/*** @todo Synchronization can be removed once variables are moved to method level ***/
	public synchronized void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			if ( operation.equals("insert") )
				insertNameSuffix(req, res);
			if ( operation.equals("modify"))
					modifyNameSuffix(req, res);
		}catch (Exception e){
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException,IOException{
		try	{
			...
			out.println("<html><head><link rel='stylesheet' type ='text/css' href='../eCommon/html/IeStyle.css'></link><script language='javascript' src='../eMP/js/NameSuffix.js'></script></head><frameset rows='42,*,50'><frame name='commontoolbarFrame'	src='"+source+"' frameborder=0 scrolling='no' noresize> <frame name='f_query_add_mod' src='../eCommon/html/blank.html' frameborder=0><frame name='messageFrame'	src='../eCommon/jsp/MstCodeError.jsp' frameborder=0 noresize scrolling='auto'></frameset></html> ");
		}catch(Exception e)	{
			e.printStackTrace();
		}
	}
	private void modifyNameSuffix(HttpServletRequest req, HttpServletResponse res)	{
		try {
			nameSuffix=req.getParameter("name_suffix");
			babySex= req.getParameter("baby_sex");
			...
			String name_suffix_loc_lang = req.getParameter("suffix_in_oth_lang")==null?"":req.getParameter("suffix_in_oth_lang");

			//InputStreamReader reader = new InputStreamReader( new ByteArrayInputStream( name_suffix_loc_lang.getBytes()), "windows-1256" ) ;
			  //name_suffix_loc_lang = (new BufferedReader(reader)).readLine() ;

            if ( name_suffix_loc_lang == null || name_suffix_loc_lang.equals("null"))
			...
			{
				babySuffixYn=req.getParameter("baby_suffix_yn");
					if(babySuffixYn == null)
						babySuffixYn="N";
			}
			...
			if( babySex.equals("Unknown"))
					babySex="U";
			else if (babySex.equals("Male"))
					babySex="M";
			else if (babySex.equals("Female"))
					babySex="F";

			HashMap tabdata=new HashMap();
			tabdata.put("name_suffix",nameSuffix);
			tabdata.put("baby_sex",babySex);
			tabdata.put("baby_suffix_yn",babySuffixYn);
			...
			condflds.put("name_suffix",nameSuffix);
			...
			out.println( nameSuffix + " "+babySuffixYn + " "+babySex +" "+ effectiveDateFrom+" "+ effectiveDateTo + " "+ effectiveStatus ); */
			e.printStackTrace();
		}
	}

	private void insertNameSuffix(HttpServletRequest req, HttpServletResponse res)	{
		try {
			nameSuffix = req.getParameter("name_suffix");
			babySuffixYn = req.getParameter("baby_suffix_yn");
			if(babySuffixYn==null)babySuffixYn="N";
			babySex = req.getParameter("baby_sex");
			effectiveDateFrom = req.getParameter("eff_date_from");
			if(effectiveDateFrom==null)effectiveDateFrom="null";
			effectiveDateTo = req.getParameter("eff_date_to");
			if(effectiveDateTo==null)effectiveDateTo="null";
			effectiveStatus = req.getParameter("eff_status");
			...
			try{
				...
				pstmt=con.prepareStatement("Select 'x' from mp_name_suffix where upper(name_suffix) = upper(?)" );
				pstmt.setString(1, nameSuffix);
