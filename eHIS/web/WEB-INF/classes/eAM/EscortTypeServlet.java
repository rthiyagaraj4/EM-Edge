 ...
 */
public class EscortTypeServlet extends HttpServlet{
	PrintWriter out;
	Properties p;
	String escorttypecode;
	String longDesc ;
	String shortDesc ;
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		try	{
			escorttypecode	= req.getParameter("escort_type");
	...
	//		java.sql.Date modified_date1 = java.sql.Date.valueOf(modifiedDate ) ;
		//	java.sql.Date modified_date = new java.sql.Date(modified_date1.parse(modified_date1.toLocaleString())-(modified_date1.getTimezoneOffset()*60*1000) ) ;
			...
			condflds.put("escort_type_code",escorttypecode);

			out.println(escorttypecode);
...
/*			InitialContext context = new InitialContext();
			...
			java.util.HashMap results = singleTabHandlerManagerRemote.update(p,tabdata,condflds,"AM_ESCORT_TYPE");
*/
		...
		/*	Object argArray[] = new Object[1];			
			...
			hmParams.put("table_name","AM_ESCORT_TYPE");
			argArray[0] = hmParams;
			...
			paramArray[0] = hmParams.getClass();*/
				...
				//argArray[0] =hashmap;
				String table_name="AM_ESCORT_TYPE";
				argArray[0] =p;
				argArray[1] =tabdata;
				argArray[2] =condflds;
...
//			java.sql.Date modified_date1 = java.sql.Date.valueOf(modifiedDate ) ;
		//	java.sql.Date modified_date = new java.sql.Date(modified_date1.parse(modified_date1.toLocaleString())-(modified_date1.getTimezoneOffset()*60*1000) ) ;
			...
			HashMap tabdata=new HashMap();
			tabdata.put("escort_type_code",escorttypecode);
			tabdata.put("long_desc",longDesc);
			tabdata.put("short_desc",shortDesc);
			tabdata.put("eff_status",effectiveStatus);
			...
			String dupflds[]={"escort_type_code"};
/*
			InitialContext context = new InitialContext();
			Object homeObject = context.lookup("java:comp/env/SingleTabHandler");
			SingleTabHandlerManagerHome singleTabHandlerManagerHome  = (SingleTabHandlerManagerHome) PortableRemoteObject.narrow(homeObject ,SingleTabHandlerManagerHome.class);
			SingleTabHandlerManagerRemote singleTabHandlerManagerRemote = singleTabHandlerManagerHome.create();
			java.util.HashMap results = singleTabHandlerManagerRemote.insert(p,tabdata,dupflds,"AM_ESCORT_TYPE");
*/
			boolean local_ejbs = false;
			ServletContext context = getServletConfig().getServletContext();
			if ((context.getInitParameter("LOCAL_EJBS") !=null) && (context.getInitParameter("LOCAL_EJBS").equalsIgnoreCase("TRUE"))) 
...
//			hmParams.clear();
			...
			out.println(escorttypecode+longDesc+shortDesc);
