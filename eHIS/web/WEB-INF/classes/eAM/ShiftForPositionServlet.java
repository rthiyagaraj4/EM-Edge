...
public class ShiftForPositionServlet extends HttpServlet
{
		...
		throws ServletException,IOException
	{
		...
		try{
			...
			if ( operation.equals("showshifts") )   insertOperation(request, response);
			if ( operation.equals("showpositions")) modifyOperation(request, response);
	...
	public void insertOperation(HttpServletRequest request,HttpServletResponse response)
	{
		...
		String sql_insert = "insert into am_shift_for_position(position_code,shift_code,added_by_id,added_date,added_at_ws_no,added_facility_id,modified_by_id,modified_date,modified_at_ws_no,modified_facility_id) values (?,?,?,sysdate,?,?,?,sysdate,?,?)";
		String sql_update = "update am_shift_for_position set eff_status=? where position_code = ? and shift_code = ?";	
		...
		if(!(tabData.isEmpty()) && !(sqlMap.isEmpty())){
			...
			response.sendRedirect("../eCommon/jsp/MstCodeError.jsp?err_num="+java.net.URLEncoder.encode(err_num,"UTF-8")+"&err_value="+error_value);
	} catch ( Exception e ) {
