...
/**
 *		FileName : DepartmentServlet.java
 *		Version	 : 3
 *		Date	 : 14-2-05	
**/
import java.sql.*;
...
public class DepartmentServlet extends javax.servlet.http.HttpServlet
{
	...
	public synchronized void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException
	{
		...
		try
		{
			...
			if ( operation.equals("insert") )   insertDepartment(req, res);
			if ( operation.equals("modify"))    modifyDepartment(req, res);
	...
	private void modifyDepartment(HttpServletRequest req, HttpServletResponse res)
	{
		try
		{
			...
			final java.util.Hashtable results = singleTabHandlerManagerRemote.update(p,tabdata,condflds,"am_dept");*/
		...
		catch ( Exception e )
		{
			...
			out.println( deptcode+ " "+longDesc + " "+shortDesc + " "+ effectiveStatus );
		}
	}

	private void insertDepartment(HttpServletRequest req, HttpServletResponse res)
	{
		try
		{
			deptcode=req.getParameter("dept_code");
			longDesc = req.getParameter("long_desc");
			shortDesc = req.getParameter("short_desc");
			acc_entity= req.getParameter("acc_entity");
			effectiveStatus = req.getParameter("eff_status");
			if ( effectiveStatus == null )
				effectiveStatus="D";
			...
			HashMap tabdata=new HashMap();
			tabdata.put("acc_entity_id",acc_entity);
			...
			final SingleTabHandlerManagerHome singleTabHandlerManagerHome  = (SingleTabHandlerManagerHome) PortableRemoteObject.narrow(homeObject ,SingleTabHandlerManagerHome.class);
			final SingleTabHandlerManagerRemote singleTabHandlerManagerRemote = singleTabHandlerManagerHome.create();
			final java.util.Hashtable results = singleTabHandlerManagerRemote.insert(p,tabdata,dupflds,"am_dept");*/
