...
public class BLDepositRecRefundServlet extends javax.servlet.http.HttpServlet {
	...
	String rcpt_rfnd_ind="", rcpt_nature_code="", cash_counter_code="",	shift_id="", doc_amt="", narration="", ip_adm_trf_dep_ind="", rec_ref_format_ind="", rfnd_dep_in_use_yn="", pmnt_thru_mail="",ext_acc_facility_id="", ext_acc_code="", ext_acc_dept_code="", mult_slmt_yn="", downtime_entry_yn="", downtime_reason_code="", downtime_doc_type_code="", downtime_doc_num="", downtime_doc_date="", downtime_reason="", cc_machine_id="", payers_add1="", payers_add2="", payers_add3="", payers_tel_num="", consolidated_receipt_yn="",rcpt_type_code="",currdate="",customercode="",remark="",origdocnumber="",ref="",totalbill="",billpaid="",total_settled_amt="",requestid="",curmnth="",curryr="",refnumber="",orgdoctypecode="",orgdocnum="",orgdocsrno="";
    ...
    String splAgnDpstYN="",splAgnDepEffFrom="",splAgnDepEffTo="";
	...
	public synchronized void doPost(HttpServletRequest req,HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		//EstimateDeposit depositbean = (EstimateDeposit)PersistenceHelper. getBeanObject(beanId, beanName, req);
			
		//List<EstimateDeposit> depositlist = new ArrayList<EstimateDeposit>();  
		...
		str_slmt_mode_wise_rcpt_yn_for_rcpt_refnd = req.getParameter("str_slmt_mode_wise_rcpt_yn_for_rcpt_refnd");
		if(str_slmt_mode_wise_rcpt_yn_for_rcpt_refnd == null) str_slmt_mode_wise_rcpt_yn_for_rcpt_refnd="";

		hand_without_counter_chk_out = req.getParameter("hand_without_counter_chk_out");
	...
	private void insertServiceRequest(HttpServletRequest req,HttpServletResponse res) throws SQLException
	{
		try 
		{		
...
//			System.out.println("ejb call 6");	
			...
			java.util.HashMap results = (java.util.HashMap) (busObj.getClass().getMethod("insertDepositData", paramArray)).invoke(busObj, argArray);
			...
			String final_msg_doc_type_code = (String) results.get("final_msg_doc_type_code");
//Added by muthu on 25-7-12 against 33868 - Receipt Print
			System.out.println("final_msg_doc_type_code servlet:::"+final_msg_doc_type_code);
			
			String gen_prt_format = (String) results.get("gen_prt_format");
			System.out.println("gen_prt_format in Servlet ==> "+gen_prt_format);
			
			str_rec_prt_format_vals = (String) results.get("str_rec_prt_format_vals");
			System.out.println("str_rec_prt_format_vals in Servlet ==> "+str_rec_prt_format_vals);

			rec_print_format_enabled_yn = (String) results.get("rec_print_format_enabled_yn");
			System.out.println("rec_print_format_enabled_yn in Servlet ==> "+rec_print_format_enabled_yn);
//Added by muthu on 25-7-12 against 33868 - Receipt Print

			 String date=currdate;
			 String[] items1 = date.split("/");
			// String date1=items1[0]; //commented checkstyle
			 String month=items1[1];
			 String year=items1[2].substring(0,4);
			...
			if(inserted)
			{			
				System.out.println("in if inserted");		
				warningMess = (String) results.get("sys_warning");
				if(warningMess == null)	warningMess="";
				/*out.println("<script>parent.frames(4).document.forms(0).postmthyr.value='"+month+"';</script>");
				out.println("<script>parent.frames(4).document.forms(0).postmonthyr.value='"+year+"';</script>");						
				out.println("<script>parent.frames(4).document.forms(0).doc_date.value='"+currdate+"';</script>");
				out.println("<script>parent.frames(4).document.forms(0).cash_ctr.value='"+cash_counter_code+"';</script>");
				out.println("<script>parent.frames(4).document.forms(0).shift.value='"+shift_id+"';</script>");*/
				responseUrl = "../eBL/jsp/BLDepositErrorMessage.jsp?month="+java.net.URLEncoder.encode(month, "UTF-8" )+
							"&year="+java.net.URLEncoder.encode(year, "UTF-8" )+
							"&currdate="+java.net.URLEncoder.encode(currdate, "UTF-8" )+
							"&cash_counter_code="+java.net.URLEncoder.encode(cash_counter_code, "UTF-8" )+
							"&shift_id="+java.net.URLEncoder.encode(shift_id, "UTF-8" )+
							"&sys_warning="+java.net.URLEncoder.encode(warningMess, "UTF-8" );
				
				 if(!message1.equals(""))
				{
					messageHashtable=MessageManager.getMessage(locale,message1,"BL");
				...
				//String final_mssage=gen_receipt+" "+gen_doc_type+"/"+gen_doc_num;
				System.out.println("final_message="+final_message);
				//out.println("<script>alert('"+final_message+"')</script>");
				errorMessage = errorMessage + final_message;
...
//Added by muthu on 25-7-12 against 33868 - Receipt Print Starts Here
		System.out.println("rec_print_format_enabled_yn  "+rec_print_format_enabled_yn+"str_rec_prt_format_vals "+str_rec_prt_format_vals);
			if(rec_print_format_enabled_yn.equals("Y") && !str_rec_prt_format_vals.equals("") && strerrortext.equals(""))
				{
				HashMap<String,String> asyncPrinterAttrib=new HashMap<String,String> ();
				str_rec_prt_format_vals = str_rec_prt_format_vals+"/"+"1"+"/"+"1";
				if("ar".equals(locale)){
				...
				asyncPrinterAttrib.put(str_rec_prt_format_vals,reportId);
				session.setAttribute("ASYNC_PRINTER_ATTRIB", asyncPrinterAttrib);
				printYn = "Y";
				/*try
					{
						StringTokenizer st1 = new StringTokenizer(str_rec_prt_format_vals,"/");				
						...
						while(st1.hasMoreTokens())
						{
							...
							System.out.println("248 session_id in Slmt:"+session_id);
							pgm_date = st1.nextToken();
							System.out.println("248 pgm_date in Slmt:"+pgm_date);
						}
						System.out.println("248 locale in SlmtValidation "+locale);				
						OnlineReport onlinereportParam = null;						
						if(locale.equals("ar"))
							{
							 System.out.println("Inside If condition 248");
							 onlinereportParam = new OnlineReport(facility_id, "BL", "BLRECPRTAKAR");	
							 System.out.println("Inside If condition 248 "+onlinereportParam);
							}
							else
							{
							 System.out.println("Else If condition 248 ");
							 onlinereportParam = new OnlineReport(facility_id, "BL", "BLRECPRT");
							 System.out.println("Else If condition  248 "+onlinereportParam);
							}

						onlinereportParam.addParameter("p_facility_id",facility_id);
						System.out.println("facility_id in Slmt 248:"+facility_id);
						...
						System.out.println("session_id in Slmt 248:"+session_id);

						System.out.println("Online Report onlinereportParam in Slmt 248:"+onlinereportParam);
						OnlineReports onlinereports = new OnlineReports();
						onlinereports.add(onlinereportParam);
						System.out.println("Online Report Added in Slmt 248:"+onlinereports);
						String strResults =  onlinereports.execute(req,res);
						System.out.println("Online Report Executed in Slmt 248");
					...
					{
						System.err.println("Error while submitting Print Job to Report Server Servlet:"+e);
					}*/
				}
//Added by muthu on 25-7-12 against 33868 - Receipt Print Ends Here
			}
			...
			{
				responseUrl = "../eBL/jsp/BLDepositErrorMessage.jsp?";
				System.out.println("in else inserted");
				if(!sys_error.equals(""))
				{
					err_val = 2;
					errorMessage = sys_error.toString();
					//out.println("<script>parent.frames(5).location.href='../eCommon/jsp/error.jsp?err_num=" +java.net.URLEncoder.encode( sys_error.toString(), "UTF-8" )+ "<br>'</script>");
				}
				else if(!message.equals(""))
				{
					...
					if(	message.equals("BL7340")){
						...
						//out.println("<script>parent.frames(5).location.href='../eCommon/jsp/error.jsp?err_num=" +java.net.URLEncoder.encode( msg, "UTF-8" )+ "<br>'</script>");
						errorMessage = msg;
					}
				...
				{	
					err_val = 2;
					errorMessage = error;
					//out.println("<script>parent.frames(5).location.href='../eCommon/jsp/error.jsp?err_num=" +java.net.URLEncoder.encode( error, "UTF-8" )+ "<br>'</script>");
				}
			//con.rollback();
			}
			responseUrl = responseUrl + "&printYn="+printYn+
							"&err_val="+err_val+
							"&errorMessage="+java.net.URLEncoder.encode(errorMessage, "UTF-8" );
			res.sendRedirect(responseUrl);
		} 
		catch (Exception e) 
		{
			//con.rollback();
			out.println("<script>parent.frames(5).location.href='../eCommon/jsp/error.jsp?err_num=" + e.toString()+ "<br>'</script>");
			System.out.println("exception in main=" + e.toString());
			e.printStackTrace();
			//out.println("<h3> Exception raise by Servlet ...So Record Not inserted </h3> Foll: Error Received : "
			//				+ e.toString() + sb.toString()); commented by ram and added above line error.jsp
	...
	{     
		...
		visit_id = replaceNull(req.getParameter("visit_id"));	
		System.out.println("Visit_id="+visit_id);
		
		refnumber=replaceNull(req.getParameter("refnumber"));
		System.out.println("refnumber="+refnumber);
		
		
		episode_id=replaceNull(req.getParameter("episode_id"));
		...
		System.out.println("episode type="+episode_type);

		encounter_id=replaceNull(req.getParameter("encounter_id"));
		...
		System.out.println("rcpt_rfnd_ind="+rcpt_rfnd_ind);
		
		rcpt_nature_code=req.getParameter("rcpt_nature_code");
		if(rcpt_nature_code==null) rcpt_nature_code="";
		System.out.println("rcpt_nature_code="+rcpt_nature_code);
		
		cash_counter_code=req.getParameter("cash_counter_code");
		if(cash_counter_code==null) cash_counter_code="";
		System.out.println("cash_counter_code="+cash_counter_code);
		
		total_settled_amt=req.getParameter("total_settled_amt");
		if(total_settled_amt==null) total_settled_amt="";
		System.out.println("total_settled_amt="+total_settled_amt);
		...
		System.out.println("requestid="+requestid);
				...
				orgdoctypecode=req.getParameter("orgdoctype");
				if(orgdoctypecode==null) orgdoctypecode="";
				
				System.out.println("orgdoctypecode"+orgdoctypecode);
				
				orgdocnum=req.getParameter("orgdocnumber");
				if(orgdocnum==null) orgdocnum="";
				System.out.println("orgdocnum"+orgdocnum);
				
				
				orgdocsrno=req.getParameter("orgdocsrno");
				if(orgdocsrno==null) orgdocsrno="";
				System.out.println("orgdocsrno"+orgdocsrno);
				
				
		
		splAgnDpstYN=req.getParameter("splAgnDpstYN");
		if(splAgnDpstYN==null) splAgnDpstYN="";
		splAgnDepEffFrom=req.getParameter("splAgnDepEffFrom");
		if(splAgnDepEffFrom==null) splAgnDepEffFrom="";
		splAgnDepEffTo=req.getParameter("splAgnDepEffTo");
	...
	/*	origdocnumber=req.getParameter("origdocnumber");
		if(origdocnumber==null) origdocnumber="";
		System.out.println("origdocnumber="+origdocnumber);
		*/
...
//		System.out.println("totalRecords servlet" +totalRecords);
	...
	public void InsertSupportdata(HttpServletRequest req) 
	{
		support_data.clear();
		support_data.put("facility_id",facility_id);
		support_data.put("customercode",customercode);
		...
		support_data.put("downtime_reason",downtime_reason);
		...
		//support_data.put("ref",ref);
		//support_data.put("origdocnumber",origdocnumber);
		...
		support_data.put("requestid",requestid);
		support_data.put("total_settled_amt",total_settled_amt);
		support_data.put("splAgnDpstYN",splAgnDpstYN);
		support_data.put("splAgnDepEffFrom",splAgnDepEffFrom);
		support_data.put("splAgnDepEffTo",splAgnDepEffTo);
		support_data.put("refnumber",refnumber);
		support_data.put("orgdoctypecode",orgdoctypecode);
		support_data.put("orgdocnum",orgdocnum);
		support_data.put("orgdocsrno",orgdocsrno);
	...
	//	System.out.println("support_data kdfdklfksl;"+support_data);
		...
		//EstimateDeposit depositbean=null;
		//BLEstimateDepositBean depositbean_test =null;
		
		
		BLEstimateDepositBean depositbean=(BLEstimateDepositBean)PersistenceHelper. getBeanObject(beanId, beanName, req);
		support_data.put("depositbean",depositbean);
		
		/*depositbean = (BLEstimateDepositBean)PersistenceHelper.getBeanObject(beanId, beanName, req);
		List<BLEstimateDepositBean> depositlist1 = depositbean.getDepositList();
		
	 	//int size=depositlist1.size();
		
		
		System.out.println("Bean Add in servlet");
...
//				System.out.println("rtn_str_slmt_doc_ref_date mult:"+rtn_str_slmt_doc_ref_date);
				...
				String str_slmt_doc_remarks = req.getParameter("str_slmt_doc_remarks"+i);
...
//					System.out.println("rtn_str_app_ref_no mult:"+rtn_str_app_ref_no);
				...
				/*String str_cash_slmt_flag = req.getParameter("str_cash_slmt_flag"+i);
...
//						System.out.println("rtn_str_cc_batch_no mult:"+rtn_str_cc_batch_no);
				...
				String str_cc_sale_draft_no = req.getParameter("saledraftno"+i);
...
//				System.out.println("rtn_str_cc_sale_draft_no mult:"+rtn_str_cc_sale_draft_no);
				...
				String str_cc_swipped_yn = req.getParameter("str_cc_swipped_yn"+i);
...
//				System.out.println("rtn_str_cash_amt_given: "+rtn_str_cash_amt_given);
				...
				String str_cash_amt_return = req.getParameter("cash_return"+i);
...
//					System.out.println("rtn_str_rcpt_rfnd_id_no mult:"+rtn_str_rcpt_rfnd_id_no);
				...
				String str_nominee_name = req.getParameter("str_nominee_name"+i);
