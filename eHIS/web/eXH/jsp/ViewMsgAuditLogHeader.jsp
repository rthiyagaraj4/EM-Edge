...
{
	System.out.println("(ViewMsgAuditLogHeader:Exception)Connection: "+e1.toString());
}
%>
...
<input type="hidden" name=appl_name id=appl_name value='<%=applicationId%>' />
<input type="hidden" name=facility id=facility value='<%=facilityId%>' />
<input type="hidden" name=msg_id id=msg_id value='<%=msgId%>' />
<input type="hidden" name=event_type id=event_type value='<%=eventType%>' />
<input type="hidden" name=purge_status id=purge_status value='<%=purge_status%>' />
<input type="hidden" name=commMode id=commMode value='<%=commMode%>' />
...
<tr>
	<td class=label><fmt:message key="Common.Application.label" bundle="${common_labels}"/>: <B><%= applicationName %></B></td>
	<td class=label><fmt:message key="Common.facility.label" bundle="${common_labels}"/>: <B><%= facilityName %></B></td>
	<td class=label><fmt:message key="Common.EventType.label" bundle="${common_labels}"/>: <B><%= eventName %></B></td>
	<td class=label><fmt:message key="eXH.MessageID.Label" bundle="${xh_labels}"/>: <B><%= msgId %></B></td>
</tr>
