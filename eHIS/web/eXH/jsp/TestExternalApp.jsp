...
<c:set var="sessionTest" value="DOCU_SESS_ID" scope="session" />
<TABLE ID="mydiv">
<TR>
<TH>${param.EM_INT_HEADER}</TH>
</TR>
<c:forEach var="currentParam" items="${reqParams}">
<TR>
	<c:if test="${currentParam.key != 'EM_INT_HEADER'}">
		<TD>${currentParam.key}</TD>
		<TD>${currentParam.value}</TD>
	</c:if>
