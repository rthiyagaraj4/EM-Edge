...
<%	sql.setLength(0);
	...
	//System.out.println(" XHTranslationManagerResultN.jsp sql:"+sql.toString());
	...
	{																			 						   
//		System.out.println("COLUMN_ID NPK : "+rs.getString("COLUMN_ID"));
...
*/
//	System.out.println("Others start : "+start+" end : "+end); 
...
//				System.out.println("XHTranslationManagerResultN.jsp Dec 30 columnVal.size() : "+columnVal.size());
				 if(columnVal.size()>0){
						sql.append(pkval.get(startVal)+",");	  
				 }
				 else{
...
//		System.out.println(" trans_table_id sql 241 : "+sql);
//		System.out.println(" trans_table_id : 242"+trans_table_id);
	...
	// Non- Translated
	 ...
	 {	 
		  ...
		  if(trans_table_id.equals("XH_OTH_APPL_DATA_SKEY"))
		 {
			 ...
			 if(columnVal.size()>0)
			{
				   sql.append("	 and (desc2_value is null AND ext_pk_id is null ) ) AND UPPER(NVL("+pkval.get(0)+",'*')) like upper('"+searchText+"')");
			 }
			 else
			{
...
//			sql.append(" AND ("+pkval.get(0)+","+pkval.get(1)+","+pkval.get(2)+") IN (SELECT "+pkval.get(0)+",");
	 ...
	 System.out.println("XHTranslationManagerResultN.jsp 832sql : "+sql.toString());
	pstmt= con.prepareStatement(sql.toString());
	rs = pstmt.executeQuery() ;
...
/*	
	 ...
	 */
	 if(!masterName.equals("SM_MASTER_CODE"))
			{
//		 		   System.out.println("910 SM_MASTER_CODE ");
	 for( startVal=0;startVal<pkval.size();startVal++)
	  {
	    sql.append("a.");
		  sql.append(pkval.get(startVal)+",");
...
for( startVal=0;startVal<pkval.size();startVal++)
  {
  %>
<input type=hidden name=pk<%=colCount% id=pk<%=colCount%><%=startVal%> value='<%=pkData[startVal]%>'>  
<%
	}
%>
...
<%
			...
			<input type="hidden" name="engLang<%=engColCount%><%=colCount%>" id="engLang<%=engColCount%><%=colCount%>" id='<%="engLang"+(colDispCount)+"0"%>' value='<%=rs.getString((String)columnVal.get(colDispCount))==null?"":rs.getString((String)columnVal.get(colDispCount))%>'> 
 ...
 <%
	...
	if(lang.equals("en"))
	 {																  
			...
			<input type="hidden" name="engLang<%=engColCount%><%=colCount%>" id="engLang<%=engColCount%><%=colCount%>" id='<%="engLang"+(engColCount)+(colCount)%>' value='<%=rs.getString((String)columnVal.get(startVal))==null?"":rs.getString((String)columnVal.get(startVal))%>'> 
	 	 
	 <%engColCount++ ;}else {
		 
		 %>
				 <td class='fields' style='<%=classValue%>' ><INPUT TYPE="text" name=desc<%=colCount1++% id=desc<%=colCount1++%><%=colCount%>  onblur='return makeValidString(this);'
