<!DOCTYPE html>
<html>
<%@ page import="java.sql.*,java.net.*,java.util.*,eXH.*,webbeans.eCommon.ConnectionManager" %>
...
<%
	String application_id = URLDecoder.decode(XHDBAdapter.checkNull(request.getParameter("application_id")),"utf-8");
...
//System.out.println("application_id"+application_id+"element_id"+element_id+"derivation_type"+derivation_type+"condition_exp1_val"+condition_exp1_val+"expected_value"+expected_value+"execution_order"+execution_order+"expected_value"+expected_value+""+execution_order+""+position);
...
<input type=hidden name=derivation_type size=10 value='<%=derivation_type%>' >
	...
	onBlur=="TextFocus(ocument.DATAELEMENTCROSSREF.execution_order);" >--><input type=text name=cond_exprsn1 size=10 value='<%=condition_exp1_val%>' disabled > <input type=hidden name=cond_exprsn size=10 value='<%=condition_exp1_val%>' ></td>
</tr>
<tr>
	<td class=label width='10%'><fmt:message key="eXH.ExpectedValue.Label" bundle="${xh_labels}"/></td>
	<td align=left><input type=text name=expected_value1 size=10 value='<%=expected_value%>' disabled><input type='hidden' name='expected_value' id='expected_value' value='<%=expected_value%>'></td>
	<td class=label width='10%'><fmt:message key="eXH.ExecOrder.Label" bundle="${xh_labels}"/></td>
	<td align=left><input type=text name=execution_order1 size=10 value='<%=execution_order%>' onKeyPress='return CheckForSpecCharsforID(event);' disabled ><input type='hidden' name='execution_order' id='execution_order' value='<%=execution_order%>' ></td>
</tr>
<tr>

	<td class=label  id="Expected_value_id"  ><fmt:message key="eXH.ExpectedValue2.Label" bundle="${xh_labels}"/></label>	</td>

	<td align=left><input type=text name=expected_value21 size=10 value='<%=expected_value2%>' disabled >	<input type='hidden' name='expected_value2' id='expected_value2' value='<%=expected_value2%>'>	</td>
	

	<td class=label  width='10%'><fmt:message key="Common.Position.label" bundle="${common_labels}"/></td><td align=left><input type=text name=Position_value1 size=10 value='<%=position%>' disabled> <input type=hidden name=Position_value size=10 value='<%=position%>'> 
</td>
</tr>
</table>
<table cellspacing=0 cellpadding=0 width='100%' border=0 align=center >
<tr>
<td colspan=6 align=right ><input class='button' type='button'  name='modify' id='modify' value='<fmt:message key="Common.delete.label" bundle="${common_labels}"/>' onClick="deleterecord('D')"></td>
</tr>
</table>
...
*/
	%>
<input type='hidden' name='actualmode' id='actualmode' value='D'>	
