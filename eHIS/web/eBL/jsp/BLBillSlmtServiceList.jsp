<!DOCTYPE html>
<%@ page import="eBL.*,java.sql.*, webbeans.eCommon.*,webbeans.op.CurrencyFormat,java.io.*,java.util.*" %>
...
<%-- 	<tr><td class='columnHeader' width='100%'><fmt:message key="eBL.SETTLEMENT_BY_SERVICES.label" bundle="${bl_labels}"/></td></tr> --%>
	</table></div>
...
<%
		...
		{
%>
			<td class='COLUMNHEADER' width='10%'><fmt:message key="Common.date.label" bundle="${common_labels}"/></td>
			...
			<td width="5%" class='COLUMNHEADER'><fmt:message key="Common.ChargeAmount.label" bundle="${common_labels}"/></td>			
			<td width="5%" class='COLUMNHEADER'><fmt:message key="eBL.AMT_SETTLED.label" bundle="${bl_labels}"/></td>	
			<td width="*" class='COLUMNHEADER'><fmt:message key="eBL.PENDING_SETTLEMENT.label" bundle="${bl_labels}"/></td>	
<%-- 			<td width="5%" class='COLUMNHEADER'><fmt:message key="eBL.CURRENT_SETTLEMENT.label" bundle="${bl_labels}"/></td>			 --%>
<%
...
%>
			...
			<td class='COLUMNHEADER' width='10%'><fmt:message key="Common.date.label" bundle="${common_labels}"/></td>
			<td class='COLUMNHEADER' width='10%'><fmt:message key="Common.BillingService.label" bundle="${common_labels}"/></td>
			<td class='COLUMNHEADER' width='20%'><fmt:message key="Common.description.label" bundle="${common_labels}"/></td>
			<td width="5%" class='COLUMNHEADER'><fmt:message key="Common.ChargeAmount.label" bundle="${common_labels}"/></td>			
			<td width="5%" class='COLUMNHEADER'><fmt:message key="eBL.AMT_SETTLED.label" bundle="${bl_labels}"/></td>	
			<td width="*" class='COLUMNHEADER'><fmt:message key="eBL.PENDING_SETTLEMENT.label" bundle="${bl_labels}"/></td>	
...
<!--<td class=<%=classval%> width='2%'><input type="checkbox" name='selected_row<%=i%>' id='selected_row<%=i%>' value='<%=i%>' ></td>	-->
	<td class=<%=classval%> width="14%"><%=com.ehis.util.DateUtils.convertDate(strServiceDate,"DMYHMS","en",locale)%><INPUT TYPE="hidden" name='service_date<%=i%>' id='service_date<%=i%>' SIZE="15" MAXLENGTH="20" 	VALUE='<%=com.ehis.util.DateUtils.convertDate(strServiceDate,"DMYHMS","en",locale)%>'  onBlur='' readonly><input type='hidden' name='trx_doc_ref<%=i%>' id='trx_doc_ref<%=i%>' value='<%=strTrxDocRef%>' > </td>
...
<!--Added by muthu against Drug Penalty Issue on 04-06-12 Starts here -->
	...
	%>
		<td class=<%=classval%> width="20%" class="fields"><%=strblngservdesc%>'<input type='hidden' name='trx_doc_ref_seq_num<%=i%>' id='trx_doc_ref_seq_num<%=i%>' value="<%=strTrxDocRefSeqNum%>" > </td>
	...
	<%
		...
		else
		{
			...
			//Added by MuthuN against 33583 on 27-9-12
	%>
		<td class=<%=classval%> width="20%" class="fields"><div name='description<%=i%>' size='40' maxlength='100'  value='<%=strLongDesc%>' readonly>
	...
	<td class=<%=classval%> width="11%" class="fields"><input type='text' name='curr_set<%=i%>' id='curr_set<%=i%>' size='11' maxlength='11'  value='' style='text-align:right' onKeyPress='return(ChkNumberInput1(this,event,document.forms[0].noofdecimal.value))' onBlur='putdeci(this);funCurStlmt()'> </td>
<%
		}
		else
		{
%>
	<td class="fields" width="14%"><%=com.ehis.util.DateUtils.convertDate(strServiceDate,"DMYHMS","en",locale)%><INPUT TYPE="hidden" name='service_date<%=i%>' id='service_date<%=i%>' SIZE="15" MAXLENGTH="20" 	VALUE='<%=com.ehis.util.DateUtils.convertDate(strServiceDate,"DMYHMS","en",locale)%>'  onBlur='' readonly><input type='hidden' name='trx_doc_ref<%=i%>' id='trx_doc_ref<%=i%>' value='<%=strTrxDocRef%>' > </td>	
	
	<td class="fields" width="14%" ><%=strBlngservCode%>
			<%		
	...
	<td width="20%" class="fields"><%=strLongDesc%><input type='hidden' name='trx_doc_ref_seq_num<%=i%>' id='trx_doc_ref_seq_num<%=i%>' value="<%=strTrxDocRefSeqNum%>" > </td>
...
%>	
	...
	<td  width="13%" class="fields"><%=strUpdNetChargeAmt%><input type='hidden' name='charge_amt<%=i%>' id='charge_amt<%=i%>'  value='<%=strUpdNetChargeAmt%>' style='text-align:right' onFocus='' readonly> </td>
	
	<td  width="13%" class="fields"><%=strTotalSettAmt%><input type='hidden' name='settled_amt<%=i%>' id='settled_amt<%=i%>'   value='<%=strTotalSettAmt%>' style='text-align:right'  onFocus='' readonly> </td>
	
	<td  width="13%" class="fields"><%=strPendStlmt%><input type='hidden' name='pend_set<%=i%>' id='pend_set<%=i%>'  value='<%=strPendStlmt%>' style='text-align:right' onFocus='' readonly> <input type="hidden" name='pend_set_no_roundoff<%=i%>' id='pend_set_no_roundoff<%=i%>'  value="<%=strPendStlmtNoRoundOff%>"> </td>

<%-- 	<td class=<%=classval%> width="13%" class="fields"> --%>
	<input type='hidden' name='curr_set<%=i%>' id='curr_set<%=i%>'  value='' style='text-align:right' onKeyPress='return(ChkNumberInput1(this,event,document.forms[0].noofdecimal.value))' onBlur='putdeci(this);funCurStlmt()'> 
<!-- 	</td> -->
...
%>	
		<td width="*" class="fields"><%=strTrxDocRef%></td>
	
	

<%-- 	<td class="fields"><a class='depadjlinkclass' name='depAdj_<%=i%>' id='depAdj_<%=i%>' value='<%=i%>' href="#" >Adjust</a></td> --%>
...
<input type="hidden" name="all_service_criteria" id="all_service_criteria" value='<%=request.getParameter("all_service_criteria")%>'>
<input type="hidden" name="all_services" id="all_services" value='<%=optionParam%>'>
<input type="hidden" name="slmt_amt_disabled_YN" id="slmt_amt_disabled_YN" value='<%=slmt_amt_disabled_YN%>'>
<input type="hidden" name="billslmtamtdet" id="billslmtamtdet" value='<%=billslmtamtdet%>'>
<input type="hidden" name="total_settled_amt" id="total_settled_amt" value='<%=total_settled_amt%>'>
<input type="hidden" name="billLevelTax" id="billLevelTax" value='<%=billLevelTax%>'>
<input type="hidden" name="addlChargeAmtForBill" id="addlChargeAmtForBill" value='<%=addlChargeAmtForBill%>'>
<input type="hidden" name="billpayableamt" id="billpayableamt" value='<%=strbillpayableamt%>'
<input type="hidden" name='optionParam' id='optionParam'  value='<%=optionParam%>'>
