...
*/ %>
<%@page import="eBL.MultiFactorDeductRuleBean"%>
...
<form name="frmMultiFactorDeductRule" id="frmMultiFactorDeductRule">	
      	<table class='grid' border='1' width='100%' cellpadding='3'  cellspacing=0 align="center" id='MultiFactorDeductRule'>  
			...
			<tr>
				...
				<td class='COLUMNHEADER' nowrap><fmt:message key="ePH.DrugCategory.label" bundle="${ph_labels}"/>&nbsp;<img src='../../eCommon/images/mandatory.gif'></td>
			...
			<tr>
				...
				<!-- V221113 -->
				<td class='fields' nowrap>
					<select  id='drug_category_<%= tableIndex%>' name='drug_category_<%= tableIndex%>' <%=disabled %> <%=totalDisabled %>   onClick="chkForMandFields('D',<%= tableIndex%>)" > 
						
						<option value=""><fmt:message key="Common.defaultSelect.label" bundle="${common_labels}"/></option>	
						
						<option value='AG' <%="AG".equals(policyBean1.getDrugCategory()) ? "selected" : "" %>> <fmt:message key="eBL.Agreement.label" bundle="${bl_labels}"/></option>
						
						<option value='GN' <%="GN".equals(policyBean1.getDrugCategory()) ? "selected" : "" %>> <fmt:message key="eBL.Generic.label" bundle="${bl_labels}"/></option>
						
						<option value='BA' <%="BA".equals(policyBean1.getDrugCategory()) ? "selected" : "" %>> <fmt:message key="eBL.BrandWithAlternate.label" bundle="${bl_labels}"/> </option>

						<option value='BN' <%="BN".equals(policyBean1.getDrugCategory()) ? "selected" : "" %>> <fmt:message key="eBL.BrandWithoutAlternate.label" bundle="${bl_labels}"/> </option>
				...
				<td class='fields' nowrap>
					<select name='patLimitAmt_<%= tableIndex%>' id='patLimitAmt_<%= tableIndex%>'>
					<option value=""><fmt:message key="Common.defaultSelect.label" bundle="${common_labels}"/></option>

						<% if(("Y").equals(policyBean1.getPatLimitAmt())){ %>
							<option value='Y' selected><fmt:message key="Common.yes.label" bundle="${common_labels}"/></option>
						<%} else{ %>
