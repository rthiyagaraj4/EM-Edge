...
<%@  page import="java.sql.*, org.apache.commons.lang.StringUtils,   webbeans.eBL.*,webbeans.eCommon.*,webbeans.op.CurrencyFormat,java.io.*,java.util.*,eBL.Common.*,com.ehis.util.*,eBL.*,eXH.*" contentType="text/html;charset=UTF-8" %>
...
function payerstatus_MemIDPolNum()
{
	...
	if(settlement_ind !='C' && settlement_ind !='R' )
	{
		...
		var param="called_from=memIDPolId&cust_code="+cust_code.value+"&rtn_total_records="+rtn_total_records+"&siteSpecMemberID="+siteSpecMemberID+"&facility_id="+facility_id+"&patientId="+patient_id+"&siteSpecPolicyNo="+siteSpecPolicyNo;
		var temp_jsp="../../eBL/jsp/BLInsuranceValidations.jsp?"+param;
		var xmlDoc = "" ;
		var xmlHttp = new XMLHttpRequest() ;
		xmlDoc = new DOMParser().parseFromString(xmlStr, "text/xml");
...
//		var param = "title="+title+"&"+"column_sizes="+column_sizes+"&"+"column_descriptions="+column_descriptions+"&"+"sql="+sql+"&"+"message="+message+"&target="+encodeURIComponent(target.value)+"&srch_by_clmn_index=2"+"&col_show_hide=YYYNNNNNNNN";
	...
	if (retVal != null || retVal!="")
	{
	    ...
	    if(retVal != null && retVal.length>0)
		{
			...
			var siteSpecValidateBlngGrp = $('#siteSpecValidateBlngGrp').val();
			var blngGrpValid ="";
...
%>
	<tr>
		<td class='fields' width="25%" nowrap>
			<fmt:message key="eBL.BillingGroupCategory.Label" bundle="${bl_labels}"/>
		</td>
		<td class='fields' width="25%" nowrap>
		...
		<td class='fields' width="25%" nowrap><fmt:message key="eBL.SLMT_INDICATOR.label" bundle="${bl_labels}"/>
		<td class='fields' width="25%" nowrap>
			<select name='slmtIndicator' id='slmtIndicator'
					id='slmtIndicator'>
						<option value=''></option>
						<option value='C'>
							<fmt:message key="eBL.CASH_SETTLEMENT.label"
								bundle="${bl_labels}" />
						</option>
						<option value='R'>
							<fmt:message key="eBL.CREDIT_PATIENT.label" bundle="${bl_labels}" />
						</option>
						<option value='X'>
							<fmt:message key="eBL.SPONSORED_PATIENT.label"
								bundle="${bl_labels}" />
						</option>
				</select>
				</td>
...
<%
	...
	<% } else if(((strModuleId.equals("MP")) && (strFunctionId.equals("PAT_REG")) || (strModuleId.equals("MP")) && (strFunctionId.equals("RAPID_REG")) 
		...
		<span id ="linkNew"><%=txtDisp%></span>   <!-- Added By palani narayanan - for CRF-0550 -->	
		
		<!--  <a href="javascript:uploadImage()" id='insImage' ><fmt:message key="eBL.UploadDocuments.label" bundle="${common_labels}"/></a>-->
	...
	<input type= "hidden" name="billing_flag" id="billing_flag"  value="<%=billing_reset_flag%>">	<!--Added Against GHL-SCF-1478-->
...
<input type=button class='button' name='close_button' id='close_button' value="<%=com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.apply.label","common_labels")%>" onClick='payerstatus();return sendBackFinDetails();'></td>
<%
}
...
%>
	...
	<input type=button class='button' name='close_button' id='close_button' value="<%=com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.close.label","common_labels")%>" onClick='payerstatus_MemIDPolNum();'></td>
...
<td width="25%"  class='fields' ><input type='text' name="emp_id_mand_YN_dis" id="emp_id_mand_YN_dis" maxlength='30' size='20' tabindex='0'  value="<%=memployer_code%>" onChange="if(this.value != '') {return emplIdLookup(emp_id_mand_YN_dis,document.forms[0].billing_group)}" ><input type='button' class='button' name="emplIdLookupbut" value='?' onClick="return emplIdLookup(emp_id_mand_YN_dis,document.forms[0].billing_group)" tabindex='0' >
...
<tr>
	...
	<td width="25%" class='fields' colspan='3'><input type="text" maxlength='2' size="2" name='no_of_dependants' id='no_of_dependants' style='text-align:right' onKeyPress='return(ChkNumberInput(this,event,0))' value='<%=strNoDependants%>'></input></td>
<%
			if(!patient_id.equals(""))
			{
...
<tr>
	...
	<td width="25%"  class="fields" ><input type=checkbox name='upd_fin_dtls' id='upd_fin_dtls' style='text-align:right' onClick='chng_class_for_inv_pyrs_on_rfrsh();checkAlways(this);' <%=upd_pat_fin_dtls_YN%>></input></td>
...
<tr>
	<td width="25%"  class="label" ><fmt:message key="eBL.UPDATE_PATIENT_FIN_DTL.label" bundle="${bl_labels}"/></td>
	<td width="25%"  class="fields" ><input type=checkbox name='upd_fin_dtls' id='upd_fin_dtls' style='text-align:right' onClick='checkAlways(this);' <%=upd_pat_fin_dtls_YN%>></input></td>
	<td width="25%"  class="label" >
<%
				if (!(ref_id.equals("")))
				{	
...
%>
		...
		%>
			<td width="25%" class='label'><fmt:message key="eBL.PATIENT_BED_CATEGORY.label" bundle="${bl_labels}"/></td>
			<td width="25%"  class="fields" >
				<input type='text' name="patient_bed_category" id="patient_bed_category" maxlength='30' size='20' tabindex='0'  value='<%=patBedCatgDesc%>' onKeyPress='return CheckForSpecChars_local(event)' 
					onBlur="return patbedcat(document.forms[0].patient_bed_category)" />
				<input type='hidden' name='rtn_patient_bed_category' id='rtn_patient_bed_category' id='rtn_patient_bed_category' value='<%=pat_bed_cat%>'>
...
<input type='hidden' name='Site_Spec' id='Site_Spec' value="<%=Site_Spec%>">
<input type='hidden' name='new_module_id' id='new_module_id' value="<%=new_module_id%>">
...
<%-- <input type='hidden' name ='class_typ_change_authYN' value='<%=classTypChangeAuthYN%>'> --%>
<!-- Added By Vijay For MLMMOH-SCF-0745 -->
<input type='hidden' name='siteSpecStlmtType' id='siteSpecStlmtType' value='<%=siteSpecStlmtType%>'>
<input type='hidden' name='BlngGrpFacility' id='BlngGrpFacility' value='<%=BlngGrpFacility%>'>

<!-- Added V181105-Subha/KDAH-SCF-0425/Starts -->
<input type='hidden' name='site_specific_blg_grp_chng' id='site_specific_blg_grp_chng' value='<%=site_specific_blg_grp_chng%>'> 
<input type='hidden' name='Emplr_Blngrp_mapping' id='Emplr_Blngrp_mapping' value='<%=Emplr_Blngrp%>'> 
...
<input type='hidden' name='KDAHSite_finTab' id='KDAHSite_finTab' value='<%=KDAHSite_finTab%>'> 
<input type='hidden' name ='siteSpecDispBlngGrpCatg' id ='siteSpecDispBlngGrpCatg' value='<%=siteSpecDispBlngGrpCatg%>'><!-- Added V190508-Aravindh/GHL-CRF-0553 -->
<input type='hidden' name='practo_appt_yn' id='practo_appt_yn' value='<%=practo_appt_yn%>'> 
<input type='hidden' name='capture_pat_bed_cat' id='capture_pat_bed_cat' value='<%=capture_pat_bed_cat%>'><!-- Added V190801 Gayathri/GHL-CRF-0523--> 
<input type='hidden' name='siteSpecPatBedCategory' id='siteSpecPatBedCategory' value='<%=GHL_Pat_bed_Category%>'><!-- Added V190820 Aravindh/GHL-CRF-0523--> 
<input type='hidden' name='siteSpecValidateBlngGrp' id='siteSpecValidateBlngGrp' value='<%=siteSpecValidateBlngGrp%>'><!-- Added V191218 Aravindh/GHL-CRF-0599-->
<input type='hidden' name='siteSpecUploadDocAll' id='siteSpecUploadDocAll' value='<%=siteSpecUploadDocAll%>'><!-- Added V191218 Aravindh/GHL-CRF-0550--> 
<input type='hidden' name='siteSpecMemberID' id='siteSpecMemberID' value='<%=siteSpecMemberID%>'><!-- Added Against MMS-DM-CRF-0164.1-->
<input type='hidden' name='siteSpecPolicyNo' id='siteSpecPolicyNo' value='<%=siteSpecPolicyNo%>'>
<input type='hidden' name='policyNoMandateflag' id='policyNoMandateflag' value=""><!-- Added Against NMC-JD-CRF-0054-->

<input type=hidden name="cg_patdet_patient_id" id="cg_patdet_patient_id" value="<%=cg_patdet_patient_id %>" /> 
<input type=hidden name="cg_patdet_national_id" id="cg_patdet_national_id" value="<%=cg_patdet_national_id %>" /> 
<input type=hidden name="cg_patdet_alt_id1_type" id="cg_patdet_alt_id1_type" value="<%=cg_patdet_alt_id1_type %>" /> 
<input type=hidden name="cg_patdet_alt_id1_no" id="cg_patdet_alt_id1_no" value="<%=cg_patdet_alt_id1_no %>" /> 
<input type=hidden name="cg_patdet_alt_id2_type" id="cg_patdet_alt_id2_type" value="<%=cg_patdet_alt_id2_type %>" /> 
<input type=hidden name="cg_patdet_alt_id2_no" id="cg_patdet_alt_id2_no" value="<%=cg_patdet_alt_id2_no %>" /> 
<input type=hidden name="cg_patdet_alt_id3_type" id="cg_patdet_alt_id3_type" value="<%=cg_patdet_alt_id3_type %>" /> 
<input type=hidden name="cg_patdet_alt_id3_no" id="cg_patdet_alt_id3_no" value="<%=cg_patdet_alt_id3_no %>" /> 
<input type=hidden name="cg_patdet_alt_id4_type" id="cg_patdet_alt_id4_type" value="<%=cg_patdet_alt_id4_type %>" /> 
<input type=hidden name="cg_patdet_alt_id4_no" id="cg_patdet_alt_id4_no" value="<%=cg_patdet_alt_id4_no %>" /> 
<input type=hidden name="cg_patdet_oth_altid_type" id="cg_patdet_oth_altid_type" value="<%=cg_patdet_oth_altid_type %>" /> 
<input type=hidden name="cg_patdet_oth_altid_no" id="cg_patdet_oth_altid_no" value="<%=cg_patdet_oth_altid_no %>" /> 
<input type=hidden name="citizen_flag" id="citizen_flag" value="<%=citizen_flag %>" /> 
<input type='hidden' name='siteSpecAgeVal' id='siteSpecAgeVal' value="<%=siteSpecAgeVal%>"><!-- Added Against V210211--><!--V210221 Modified-->
<input type=hidden name="reg_referral_flag" id="reg_referral_flag" value="<%=reg_referral_flag%>" /> <!-- Added Against V230616-->
...
<script>
	...
	{
		parent.frames[1].location.href="../../eBL/jsp/AddModifyPatFinDetails6.jsp?setlmt_ind=<%=str_slmt_ind%>&boolean_security=<%=boolSecurity%>&payerGrpBySetup=<%=payerGrpBySetup%>&adm_rec_flag=<%=str_adm_rec_flag%>&credit_doc_ref_reqd_yn=<%=credit_doc_ref_reqd_yn%>&CitizenYn=<%=strCitizenYn%>&<%=query_string%>&billingGroup=<%=billing_group%>&customerId=<%=strCustomerId%>";
	}
	else{
