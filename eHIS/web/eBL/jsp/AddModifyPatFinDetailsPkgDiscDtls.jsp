...
<% 
	...
	try
	{
		...
		int totRec=1;
	//	String appr_seq_no = request.getParameter("appr_seq_no") == null ? "" :request.getParameter("appr_seq_no") ;// IN 20964 //checklist
...
//		String bean_pkg_desc="";//checklist
		String pkgDtlsVals="";	
		pkgDtlsVals = request.getParameter("pkgDtlsVals") == null ? "" : request.getParameter("pkgDtlsVals");/* Added V180529-Aravindh/AMS-SCF-0667 */
		...
		try
		{		
			PreparedStatement pstmt = con.prepareStatement( " select nvl(no_of_decimal,2) from sm_acc_entity_param where acc_entity_id='ZZ'");
		...
		if(!pkg_disc_amt.equals(""))
			pkg_disc_amt = cf.formatCurrency(pkg_disc_amt,noofdecimal);
...
/*		String pkg_disc_res_sql = "select a.action_reason_desc description from bl_action_reason_lang_vw a,bl_action_reason_by_type b where a.action_reason_code  = b.action_reason_code and nvl(a.status,'X') <> 'S' and NVL(b.active_yn, 'N') = 'Y' and upper(language_id) = upper('"+locale+"') AND ACTION_TYPE='PD' and a.action_reason_code='"+pkg_disc_reason+"'";
	...
	//	Connection con	=	ConnectionManager.getConnection(request); //checklist
