			...
			<tr>
				...
				<%if(siteSpecRefPrice) { %>
					<% if("Y".equals(policyBean.getCoverRefPrice())) { %>
						<input type="checkbox" name='coverRefPriceHdr' id='coverRefPriceHdr' id='coverRefPriceHdr' onclick='unCheckAllLineLvlRefChkBox("hdr")' checked <%=totalDisabled %> />
					<% }
					else { %>
						<input type="checkbox" name='coverRefPriceHdr' id='coverRefPriceHdr' id='coverRefPriceHdr' onclick='unCheckAllLineLvlRefChkBox("hdr")' <%=totalDisabled %> />
					<% } %>
				...
				<!-- Added V190517-Aravindh/NMC-JD-CRF-0010/Starts -->
				<%if(siteSpecPolApprv) { %>
					<input type='checkbox' name='policyApprvdYN' id='policyApprvdYN' id='policyApprvdYN' <%="Y".equals(policyBean.getPolicyApproved())?"checked":"" %> disabled />
				...
				<td width='25%' nowrap class='fields' >
					<input type="checkbox" <%=totalDisabled %> name='ClaimsAgrmtExts' 
					<%="Y".equals(policyBean.getclaimsAgrmtExts())?"checked":"" %> disabled/>
