...
function setReportItemId() {
	...
	*/
function setBatchSelection(report_type,batch_sel) {
	var report_id = report_type.value;
	if((report_id == 'STBSTKS1') || (report_id == 'STBSTKS2') || (report_id == 'STBSTKS5')){
		 batch_sel.options.length = 1;
		 batch_sel.options[0].value = '';
		 batch_sel.options[0].text = '----- '+getLabel("Common.defaultSelect.label", "Common")+' -----';
		 batch_sel.disabled = true;
	}
	else
	{
		 batch_sel.options.length = 3;
		 batch_sel.options[0].value = 'A';
		 batch_sel.options[0].text = getLabel("eST.AllBatches.label", "ST");
		 batch_sel.options[1].value = 'Y';
		 batch_sel.options[1].text = getLabel("eST.SuspendedBatches.label", "ST");
		 batch_sel.options[2].value = 'N';
		 batch_sel.options[2].text = getLabel("eST.ActiveBatches.label", "ST");
		 batch_sel.disabled = false;
	}
