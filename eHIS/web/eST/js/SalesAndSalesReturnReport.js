...
async function searchCodeStore(obj) {
	...
	var formObj = document.documentformSalesAndSalesReturnReport;
	var xmlHttp = new XMLHttpRequest();
	var xmlDoc="";
	xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../eST/jsp/SalesAndSalesReturnReportValidate.jsp?patient_id=" + _patient_id + "&validate=PATIENT_NAME", false);
	xmlHttp.send(xmlDoc);
	...
	/*function PopulateSalesReturnDocTypeCode(_trn_type1) {
		...
		var xmlDoc="";
		xmlDoc.loadXML(getXMLString(null));
		xmlHttp.open("POST", "../../eST/jsp/SalesAndSalesReturnReportValidate.jsp?trn_type=" + _trn_type1 +"&validate=POPULATE_SALES_DOC_TYPE_CODE", false);
		xmlHttp.send(xmlDoc);
		responseText = xmlHttp.responseText;
		eval(responseText) ;
	}
	*/
	function PopulateSalesReturnDocTypeCode(_trn_type1) {
	    ...
	    var url = "../../eST/jsp/SalesAndSalesReturnReportValidate.jsp";
	    var params = "trn_type1=" + encodeURIComponent(_trn_type1) + "&validate=POPULATE_SALES_DOC_TYPE_CODE";
	...
	var xmlDoc="";
	xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../eST/jsp/SalesAndSalesReturnReportValidate.jsp?trn_type="+_trn_type+"&validate=POPULATE_RETURN_DOC_TYPE_CODE", false);
	xmlHttp.send(xmlDoc);
	responseText=xmlHttp.responseText;
	eval(responseText);
}*/
	function populateBothDocTypeCode(_trn_type2){
	    ...
	    var xmlDoc = getXMLString(null); 	   
            var url = "../../eST/jsp/SalesAndSalesReturnReportValidate.jsp";
	    var params = "trn_type2=" + encodeURIComponent(_trn_type2) + "&validate=POPULATE__BOTH_DOC_TYPE_CODE";
	...
	var xmlDoc="";
	xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../eST/jsp/SalesAndSalesReturnReportValidate.jsp?trn_type="+_trn_type2+"&validate=POPULATE_BOTH_DOC_TYPE_CODE", false);
	xmlHttp.send(xmlDoc);
	responseText=xmlHttp.responseText;
	eval(responseText);
}*/
...
/*	function run(){
		formObj = f_query_add_mod.document.formSalesAndSalesReturnReport;
		   ...
		   var startDate = formObj.p_from_date.value;
		    ...
		    var xmlHttp = new XMLHttpRequest();
		    var params = "start_date=${startDate}&end_date=${endDate}&validate=CHECK_DOC_DATE";
		    xmlHttp.open("POST", "../../eST/jsp/SalesAndSalesReturnReportValidate.jsp?${params}", false);
		    xmlHttp.send();
		    var responseText = xmlHttp.responseText;
		    eval(responseText);
		if(result){
...
function run() {
	...
	xmlHttp.open("POST",
			"../../eST/jsp/SalesAndSalesReturnReportValidate.jsp?${params}",
			false);
	xmlHttp.send();
	var responseText = xmlHttp.responseText;
	eval(responseText);
	if (daysDifference <= 90) {
		formObj.DT_FROM.value = convertDate(formObj.p_from_date.value, "DMY",
				formObj.p_language_id.value, "en");
		formObj.DT_TO.value = convertDate(formObj.p_to_date.value, "DMY",
				formObj.p_language_id.value, "en");
		if (formObj.p_from_date.value != "" && formObj.p_to_date.value != "") {
			if (!isBefore(formObj.DT_FROM.value, formObj.DT_TO.value, "DMY",
					'en'.value)) {
				alert(getMessage("FM_DATE_GR_TO_DATE", "ST"));
				formObj.p_to_date.focus();
				return;
		...
		var localename = formObj.p_language_id.value;
		if (formObj.p_from_date.value != "") {
			if (!isBefore(formObj.p_from_date.value, currentdate, "DMY",
					localename)) {
				alert(getMessage("ST_DOC_DT_NOT_GR_SYSDATE", "ST"));
				formObj.p_from_date.select();
				formObj.p_from_date.focus();
		...
		if (formObj.p_to_date.value != "") {
			if (!isBefore(formObj.p_to_date.value, currentdate, "DMY",
					localename)) {
				alert(getMessage("ST_DOC_DT_NOT_GR_SYSDATE", "ST"));
				formObj.p_to_date.select();
				formObj.p_to_date.focus();
