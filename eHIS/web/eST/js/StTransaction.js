...
function checkIsValidDocDate(doc_date, sys_date, curr_proc_month, curr_proc_year, messageFrame) {
	...
	if (!(checkDateWithSysDate(doc_date, sys_date))) {
		msg = getMessage("DATE_NOT_GREATER_SYSDATE", "ST");
...
//	 var retVal;
	...
	var arguments = "";
	 // retVal = window.showModalDialog("/eCIS/ph/jsp/SearchFrame.jsp",arguments,features);
	var retVal = "";
	retVal =await  window.showModalDialog("/eCIS/st/jsp/grnsearch.jsp", arguments, features);
//	alert(retVal);
	return retVal;
}
async function getIssueSearch() {
	var retVal;
	var dialogHeight = "30vh";
	var dialogWidth = "50vw";
	var dialogTop = "75";
	...
	var features = "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments = "";
	retVal = await window.showModalDialog("/eCIS/st/jsp/docDetailSearch.jsp", arguments, features);
	if (retVal == null) {
		retVal = "";
	}
	return retVal;
...
function getTotalQty(total_item_qty) {
	//var srb_header_total_item_qty	= parent.frames[0].document.forms[0].total_item_qty;
	var total_qty = total_item_qty.value; //srb_header_total_item_qty.value;
	if (total_qty == "") {
		total_qty = "0";
	}
	...
	var HTMLVal = "" + total_qty + "&nbsp;&nbsp;&nbsp;&nbsp;";
	parent.STStkLBTotalFrame.document.body.all.total_qty.innerHTML = HTMLVal;
	return true;
}
function reduceTotalQty(checkObj, item_qty, tblrow_id, noOfCols, total_item_qty_name) {
	if (checkObj.checked == true) {
		cancelRecord(tblrow_id, noOfCols);
	} else {
		activateRecord(tblrow_id, noOfCols);
	}
	var srb_header_total_item_qty = eval("parent.frames[0].document.forms[0]." + total_item_qty_name);
	var stbheader_item_qty = srb_header_total_item_qty.value;
	//alert(stbheader_item_qty);
	if (isNaN(parseFloat(stbheader_item_qty))) {
		srb_header_total_item_qty.value = "0";
	}
	if (stbheader_item_qty < 0 || stbheader_item_qty == "") {
		srb_header_total_item_qty.value = "0";
	}
	var checkFlag = false;
	checkFlag = checkObj.checked;

	//alert("before checkFlag : "+checkFlag );
	if (total_item_qty_name.indexOf("return") == -1) {
		if (checkFlag) {
			checkFlag = false;
		} else {
			checkFlag = true;
	...
	//alert("after checkFlag : "+checkFlag );
	if (checkFlag == false) {
		if (!(isNaN(parseFloat(stbheader_item_qty)) && isNaN(parseFloat(item_qty)))) {
			srb_header_total_item_qty.value = (parseFloat(stbheader_item_qty) - parseFloat(item_qty));
		} else {
			srb_header_total_item_qty.value = 0;
		}
	} else {
	...
	//var HTMLVal= ""+srb_header_total_item_qty.value;
	//parent.STStkLBTotalFrame.document.body.all.total_qty.innerHTML= HTMLVal;
	getTotalQty(srb_header_total_item_qty);
}
/**
  * Added by arul to invoke Item Batch Search screen on 18th May 2002
  */
...
async function confirmFinalize() {
	...
	var arguments = "";
	var url = "../../eST/jsp/ConfirmTransaction.jsp";
...
async function confirmCancel() {
	/** @user - ST_CANCEL_REQUEST */
	var retVal = "";
	var dialogHeight = "8vh";
	var dialogWidth = "19vw";
	var dialogTop = "230";
	var center = "1";
	var status = "no";
	var features = "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; scroll: no " + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments = "";
	var url = "../../eST/jsp/ConfirmCancel.jsp";
	retVal = await window.showModalDialog(url, arguments, features);
	if (retVal == null) {
		retVal = "";
	}
...
async function confirmAuthorize() {
	...
	var features = "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; scroll: no " + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments = "";
	var url = "../../eST/jsp/ConfirmAuthorize.jsp";
	retVal = await window.showModalDialog(url, arguments, features);
	if (retVal == null) {
		retVal = "";
	}
...
async function confirmDelete() {
	...
	var features = "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; scroll: no " + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments = "";
	var url = "../../eST/jsp/ConfirmDelete.jsp";
	retVal = await window.showModalDialog(url, arguments, features);
	if (retVal == null) {
		retVal = "";
	}
...
async function confirmPrinting() {
	...
	var features = "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; scroll: no " + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments = "";
	var url = "../../eST/jsp/ConfirmOnlinePrint.jsp";
	retVal = await window.showModalDialog(url, arguments, features);
	if (retVal == null) {
		retVal = "";
	}
...
async function confirmPrintView() {
	...
	var features = "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; scroll: no " + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments = "";
	var url = "../../eST/jsp/ConfirmPrintView.jsp";
	retVal = await window.showModalDialog(url, arguments, features);
	if (retVal == null) {
		retVal = "";
	}
...
async function confirmYesNo(str) 
{
	var retVal="";
	...
	var features	= "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; scroll: no " + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments	= "" ;
	var url = "../../eST/jsp/ConfirmAction.jsp?str=" + str ;
	retVal = await window.showModalDialog(url,arguments,features);
...
async function confirmReject() { 
	...
	var features = "dialogHeight:" + dialogHeight + "; dialogWidth:" + dialogWidth + "; scroll: no " + "; center: " + center + "; status: " + status + "; dialogTop :" + dialogTop;
	var arguments = "";
	var url = "../../eST/jsp/ConfirmReject.jsp";
	retVal = await window.showModalDialog(url, arguments, features);
	if (retVal == null) {
		retVal = "";
	}
