...
function loadPage() {
	...
	if (parent.MasterCodeListFrame.document.formTrnSummaryList.summary_code.value == "A") {
		...
		if (parent.MasterCodeListFrame.document.formTrnSummaryList.summary_code.value == "B") {
			parent.MasterCodePages.document.location.href = "../../eST/jsp/IssueSummaryReport.jsp";
		} else {
				...
				if (parent.MasterCodeListFrame.document.formTrnSummaryList.summary_code.value == "D") {
					...
					if (parent.MasterCodeListFrame.document.formTrnSummaryList.summary_code.value == "E") {
						...
						if (parent.MasterCodeListFrame.document.formTrnSummaryList.summary_code.value == "F") {
							parent.MasterCodePages.document.location.href = "../../eST/jsp/IssueReturnSummaryReport.jsp";
						} else {
...
async function searchCodeStore1(obj) { 
	//alert("searchCodeStore1"); 
	var user_name =  document.getElementById("p_user_name").value; 	
	var strFacilityParam = document.getElementById("p_strFacilityParam").value; 	 //MOHE-CRF-0084.1
	var result = false;
	var message = "";
	var flag = "";
	var function_id = "";
...
//MOHE-CRF-0084.1 
async function searchCodeStore2(obj) { 
	//alert("searchCodeStore1"); 
	var user_name =  document.getElementById("p_user_name").value; 	
	var strFacilityParam = document.getElementById("p_strFacilityParam").value; 	//MOHE-CRF-0084.1
	var result = false;
	...
	var dataValueArray = new Array();
	var dataTypeArray = new Array();
	var trn_type =document.forms[0].p_trn_type.value;//MOHE-CRF-0084.1   
	var report_id_value = document.forms[0].p_report_id_value.value;//END 
	dataNameArray[0] = "language_id";
	dataValueArray[0] = document.forms[0].p_language_id.value;
	dataTypeArray[0] = STRING;
	dataNameArray[1] = "facility_id";//tfs id: 22406
	dataValueArray[1] = document.forms[0].p_facility_id.value;
	dataTypeArray[1] = STRING;
	if(report_id_value=="STBSPBS2" || (trn_type == "CBL") ){//MOHE-CRF-0084.1 start 
		if(strFacilityParam =="N"){
			argumentArray[0] = "SELECT st.store_code code, mm.short_desc  description FROM  mm_store_lang_vw  mm, st_store st , st_acc_entity_param  WHERE mm.eff_status ='E'  AND  language_id  like  ? AND mm.facility_id LIKE  ?   AND  UPPER(st.store_code)  LIKE UPPER(?)  AND  UPPER(mm.short_desc)   LIKE UPPER(?)  AND  mm.store_code=st.store_code  ORDER BY 2 "; //modified for tfsid: 22406
		}else if (strFacilityParam == "Y"){
			argumentArray[0] = "SELECT st.store_code code, mm.short_desc  description FROM  mm_store_lang_vw  mm, st_store st , st_acc_entity_param  WHERE mm.eff_status ='E'  AND  language_id  like  ? AND mm.facility_id LIKE  ?   AND  UPPER(st.store_code)  LIKE UPPER(?)  AND  UPPER(mm.short_desc)   LIKE UPPER(?)  AND  mm.store_code=st.store_code AND st.store_code IN (SELECT STORE_CODE FROM st_user_access_for_store where user_id = '" +user_name+"' "+ ") ORDER BY 2 "; //modified for tfsid: 22406
		}
...
//MOHE-CRF-0084.1 
function setReportIdValue(obj){
	document.forms[0].p_report_id_value.value = obj.value; 
...
async function searchCodeTrnDocTypeFORGRN(obj) {
	var result = false;
	var message = "";
	var flag = "";
	var function_id = "";
...
function run() {
	if (f_query_add_mod.MasterCodePages.document.formrequestsummary) {
		...
		//Added by Rabbani #Inc no: 39379 (MMS-QH-CRF-0111) on 21-NOV-2013 
		if(formObj.p_item_class.checked) 
		formObj.p_report_id.value = "STBREQIC";
		else
		formObj.p_report_id.value = "STBREQSM";
		//ends
		...
		if (f_query_add_mod.MasterCodePages.document.formissuesummary) {
			...
			if (isValidFromToField(formObj.p_fm_item_code, formObj.p_to_item_code, STRING, getLabel("Common.item.label", "Common"), messageFrame)) {
				if (isValidFromToField(formObj.p_fm_iss_st, formObj.p_to_iss_st, STRING, getLabel("eST.IssuingStore.label", "ST"), messageFrame)) {
					if (isValidFromToField(formObj.p_fm_item_class_code, formObj.p_to_item_class_code, STRING, getLabel("Common.ItemClass.label", "Common"), messageFrame)) {
						if (isValidFromToField(formObj.p_fm_iss_to_st, formObj.p_to_iss_to_st, STRING, getLabel("Common.ReceivingStore.label", "Common"), messageFrame)) {
							if (isValidFromToField(formObj.p_fr_doc_type, formObj.p_to_doc_type, STRING, getLabel("Common.DocTypeCode.label", "Common"), messageFrame)) {
								if (isValidFromToField(formObj.p_fm_doc_no, formObj.p_to_doc_no, STRING, getLabel("Common.DocNo.label", "Common"), messageFrame)) {
			...
			//Added by Rabbani #Inc no: 39379 (MMS-QH-CRF-0111) on 21-NOV-2013 
			if(formObj.p_item_class.checked){
		    if(formObj.p_report_type.value == "STBISSS1") 
		    formObj.p_report_id.value = "STBISSIC1";
			else 
			formObj.p_report_id.value = "STBISSIC2";
			}else{
			if(formObj.p_report_type.value == "STBISSS1") 
		    formObj.p_report_id.value = "STBISSS1";
			else 
			formObj.p_report_id.value = "STBISSS2";
			}
		   //ends
			formObj.action="../../eCommon/jsp/report_options.jsp";
			formObj.target="messageFrame"
			...
			if (f_query_add_mod.MasterCodePages.document.formMFGsummary) {
				...
				if (f_query_add_mod.MasterCodePages.document.formadjustmentsummary) {
					...
					if (f_query_add_mod.MasterCodePages.document.formtransfersummary) {
						...
						//Added by Rabbani #Inc no: 39379 (MMS-QH-CRF-0111) on 21-NOV-2013 
						if(formObj.p_item_class.checked)
		                formObj.p_report_id.value = "STBTFRIC";
						else
		                formObj.p_report_id.value = "STBTFRSM";
						formObj.action="../../eCommon/jsp/report_options.jsp";
						formObj.target="messageFrame"
						formObj.submit();
					} else {
						if (f_query_add_mod.MasterCodePages.document.formproducttransfersummary) {
							...
							if (f_query_add_mod.MasterCodePages.document.formissuereturnsummary) {
								...
								//Added by Rabbani #Inc no: 39379 (MMS-QH-CRF-0111) on 21-NOV-2013 
			                  if(formObj.p_item_class.checked){
		                      if(formObj.p_report_type.value == "STBRETS1") 
		                         formObj.p_report_id.value = "STBRETIC1";
			                 else if(formObj.p_report_type.value == "STBRETS2")
			                     formObj.p_report_id.value = "STBRETIC2";
							  else
							  formObj.p_report_id.value = "STBRETIC3";
			                 }else{
			                if(formObj.p_report_type.value == "STBRETS1") 
		                      formObj.p_report_id.value = "STBRETS1";
			                else if(formObj.p_report_type.value == "STBRETS2") 
		                      formObj.p_report_id.value = "STBRETS2";
							  else
			                   formObj.p_report_id.value = "STBRETS3"; //Changed from STBISSS3 to STBRETS3 for HSA-SCF-0118 by B.Badmavathi
			                 }
		                 //ends
			                  formObj.action="../../eCommon/jsp/report_options.jsp";
			                  formObj.target="messageFrame"
