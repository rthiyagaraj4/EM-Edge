...
var formObj = "";
function reset() {
	var formObj = f_query_add_mod.document.formStockCardReport;
...
async function searchCodeItem(obj) {
	...
	argumentArray[3] = dataTypeArray;
	argumentArray[4] = "2,3";
	argumentArray[5] = encodeURIComponent(obj.value);
	argumentArray[6] = CODE_LINK;
	argumentArray[7] = CODE_DESC;
...
function run() {
	if (f_query_add_mod.document.formStockCardReport) {
		formObj = f_query_add_mod.document.formStockCardReport;
     ...
     if(formObj.p_trn_date_to.value != "" && formObj.p_trn_date_from.value != "" ){
       ...
       return ;	
		...
		if (isValidFromToField(formObj.p_fm_store_code, formObj.p_to_store_code, STRING,getLabel("Common.StoreCode.label", "Common"), messageFrame)) {
			...
			if (isValidFromToField(formObj.p_fm_MANUFACTURER_ID, formObj.p_to_MANUFACTURER_ID, STRING, getLabel("Common.ItemCode.label", "Common"), messageFrame)) {
