...
function reset() 
	{
		f_query_add_mod.document.RecalculateItemStockLevelCriteria.reset();
...
function run()
{
	var formObj=  f_query_add_mod.document.RecalculateItemStockLevelCriteria;
	var resultText	= formApply(formObj,ST_CONTROLLER);
	var errorPage	=	"../../eCommon/jsp/error.jsp";
    
    var obj1 = true;// f_query_add_mod.document.RecalculateItemStockLevelCriteria.p_process_option[0].status;
...
var blankObject = checkSTFields(arrayObject, names,messageFrame);	
	...
	//	blankObject.focus();	
		...
		if (isSTValidFromToField(formObj.p_fm_item_code, formObj.p_to_item_code, STRING, getLabel("Common.ItemCode.label","Common"), messageFrame,errorPage)) 
		if (isSTValidFromToField(formObj.p_fm_item_class, formObj.p_to_item_class, STRING, getLabel("Common.ItemClass.label","Common"), messageFrame,errorPage)) 
		{
		formObj.submit();		
		}
	}
