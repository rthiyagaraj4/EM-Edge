...
<body onMouseDown="CodeArrest()" onKeyDown="lockKey()">
	...
	<form name="MReportForm" id="MReportForm" action="../../eCommon/jsp/report_options.jsp" target="messageFrame">
		...
		<tr>
			<td align="right" width="25%" class="label"><fmt:message key="eOH.ConditionTypeFrom.Label" bundle="${oh_labels}"/> &nbsp;</td>
			<td align="left" width="25%" >
			<select name="p_condition_type_from" id="p_condition_type_from">
				<option value=""><fmt:message key="Common.all.label" bundle="${common_labels}"/></option>
				<% 
				try{
					pstmt=con.prepareStatement("SELECT CONDITION_TYPE,CONDITION_TYPE_DESC FROM OH_CONDITION_TYPE_LANG WHERE LANGUAGE_ID=? ORDER BY CONDITION_TYPE_DESC");
					pstmt.setString(1,locale);
					rs=pstmt.executeQuery();
					while(rs.next()){
						condition_type=rs.getString(1);
						condition_type_desc=rs.getString(2);
				%>	
						<option value= "<%=condition_type%>"> <%=condition_type_desc%> </option>
					<% 
					}%>
				<% 
				}catch(Exception e){
			...
			<td align="left" width="25%" >
			<select name="p_condition_type_to" id="p_condition_type_to">
				<option value=""><fmt:message key="Common.all.label" bundle="${common_labels}"/></option>
				<% 
				try{
					pstmt=con.prepareStatement("SELECT CONDITION_TYPE,CONDITION_TYPE_DESC FROM OH_CONDITION_TYPE_LANG WHERE LANGUAGE_ID=? ORDER BY CONDITION_TYPE_DESC");
	...
	<input type="hidden" name="report_id" id="report_id" value="OHRCNTYP">
	<input type="hidden" name="p_report_id" id="p_report_id" value="OHRCNTYP">
