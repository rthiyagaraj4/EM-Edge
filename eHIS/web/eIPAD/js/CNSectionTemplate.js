...
function showSectionTemplate()
{
	...
	var retVal			=	"";
	var dialogHeight	=	"37.5";//updated by Arvind @ 09-12-08
	var dialogWidth	=	"50";
	var dialogTop		=	"170";
	var status			=	"no";
	var arguments		=	"" ;
...
function callComponent()
{
	...
	if((template_type == null)|| (template_type == "") || (template_type == "B")){
		retVal = window.showModalDialog("../../eCA/jsp/CAComponentLookUpMain.jsp?desc_val="+target.value,arguments,features);	
		...
		retVal = CommonLookup(getLabel("Common.Component.label","Common"),argumentArray );
		/******************************/
	}
    ...
    if(document.forms[0].acceptOption.options[i].value!="R" && len2 <3  )
	{
		...
		else
		{
			if(retVal[2]=='List Item')
			{
				...
				document.getElementById("tablist").style.display = "inline";
				//document.getElementById("linkHistoryBtn").style.display = "none";
				document.getElementById("histDefYNTR").style.display = "none";
				document.getElementById("presentationCriteriaTR").style.display = "inline";
			...
			{
				document.getElementById("presentationCriteriaTR").style.display = "none";
				document.getElementById("numericPrefixTR").style.display = "inline";
				document.getElementById("histDefYNTR").style.display = "inline";
				//document.getElementById("linkHistoryBtn").style.display = "inline";
				document.getElementById("tablist").style.display = "inline"; //none to inline
			...
			{
				parent.CASectionTemplateToolbarFrame.document.CASectionTemplateToolbarForm.linking.style.display = "inline";
			}
			else
			{
				document.getElementById("presentationCriteriaTR").style.display = "none";
		...
		{	
			//document.getElementById("linkHistoryBtn").style.display = "none";
			parent.CASectionTemplateToolbarFrame.document.CASectionTemplateToolbarForm.linkHistoryBtn.style.display = "none";
			parent.CASectionTemplateToolbarFrame.document.CASectionTemplateToolbarForm.formulaDefinitionBtn.style.display='inline';
			document.getElementById("histDefYNTR").style.display = "none";
			document.getElementById("formulaResultTypeTR").style.display='inline';
			document.getElementById("formulaTypeTR").style.display='inline';
			document.getElementById("tablist").style.display = "inline"; 
...
function callConditionLookup()
{
		...
		{
				document.CASectionTemplateDetailForm.age_group_code_desc.value=retArray[1];
				document.CASectionTemplateDetailForm.age_group_code.value=retArray[0];				
		}
		else
		{
				document.CASectionTemplateDetailForm.age_group_code_desc.value="";
...
function showpopulateSections(sectionObject)
{
...
function populateSections()
{
	...
	if(retArray != null && retArray !="")	
	{
		document.forms[0].sect_name.value = retArray[1];
		document.forms[0].section.value = retArray[0];
...
//				document.getElementById("ok1").disabled			= false;
	...
	function selectValue(obj){
		//alert('to check :'+rem);
		var a=eval('document.all.'+rem+'.style.background="white"'); // c
		var a=eval('document.all.'+rem+'.style.color="black"');      // c 
		obj.style.background="yellow"; // c
		obj.style.color="black";      // c
//		obj.select();
	}
	...
	function formulaDateCheck(obj,str)
	{
		...
		if(obj.checked)
		{
		    ...
		    if(str != 'P' && window.confirm(msg) )
			{
			    ...
			    document.getElementById("mand").style.display='inline';
			}
		    else
			{
            if(str != 'P')
      ...
      //  alert(document.CASectionTemplateDetailForm.formula_definition.value);
		...
		if(document.CASectionTemplateDetailForm.formulaResultType.value == 'N' && obj.checked)
			document.getElementById("formulaResultUnitTR").style.display = "inline";
		else
			document.getElementById("formulaResultUnitTR").style.display = "none";
...
//	alert(old_hist_time_frame_val+old_hist_time_frame_unit);
...
async function openClinEventWin()
{
	...
	var sql = "SELECT a.SHORT_DESC, EVENT_CODE, EVENT_CODE_TYPE FROM ca_event_hist_rec_vw a where a.HIST_REC_TYPE=`"+hist_rec_type+"`AND  a.language_id = `"+locale+"` ";
	//IN040006 - End
	...
	retVal = await window.showModalDialog("../../eCA/jsp/SectionTemplateEventSearch.jsp?SQL="+sql+"&search_code="+search_code+"&text_Val="+target+"&search_desc="+search_desc+"&code_type="+code_type+"&title="+encodeURIComponent(title,"UTF-8")+"&dispDescFirst=dispDescFirst",arguments,features);	
	if(retVal != null)
	{
...
function setHiddenVal(obj)
{
	if(obj.value!= '')
	{
		...
		document.CASectionTemplateLinkHistoryForm.clinEventLookUpTxt.readOnly = false;
		document.CASectionTemplateLinkHistoryForm.mand_img.style.display = "inline";
	}
	else
	{
...
//		alert(document.CASectionTemplateDetailForm.title_text_code.value);
//		alert(document.CASectionTemplateDetailForm.title_text.value);
