/**     Purpose     : To Validate Date,Arrest View Source,Automatic Date format,Change UpperCase,and Date Check
    Written on: Sunday, September 03, 2000
    Time        : 11:31 AM
    Function    : CodeArrest(),ChangeUpperCase(Object),AumaticDateFormat(Object),CheckDate(Object),doDateCheck(From,To).
    Events  : 1.CodeArrest() - <Body OnMouseDown="CodeArrest()"></Body>
              2.ChangeUpperCase(Object)  - <input type ---- OnKeyPress="ChangeUpperCase(this)">
              3.AutomaticDateFormat(Object) - <input type --- OnKeyPress="AutomaticDateFormat(this)"  OnBlur="CheckDate(this)">
              4.Focusing(ObjectName) - <Body onLoad="Focusing('region_code')">
**/

//Function to Check String Range
...
//Function to Clear the MessageFrame
    function ClearMsg() {
        parent.messageFrame.document.location.href='../../eCommon/jsp/error.jsp?err_num= ';
    }

//Function to Set Focus
function Focusing(fld) {
    if ((!document.forms[0].item(fld).readOnly) && document.forms[0].item(fld).type !='hidden')
        document.forms[0].item(fld).focus();
}
...
function CodeArrest() {
    if((window.event.button == 2) || (window.event.button == 3) || (window.event.button == 4)) {
		/*
		  From LC/DS the getmessage will not work
		 */
		try {
...
/*
	This block creates a parameter url with the required fields only.Check
	This will reduce the amount of data being passed as parameters to webhelp.jsp
	- Vinod 3/4/2009
*/
		var href = document.location.href;
		var params = href.split('&');
		var paramurl = params[0];
/*
	Get the context path of the application from the page url
	E.g., 'eHIS in http://indbgl-app25:8899/eHIS/eSM/jsp/SiteParameterModify.jsp
*/
		var urlElements = paramurl.split('/');
		var contextPath	= urlElements[3];

		for (i=1;i<params.length ;++i )
		{
			//alert(params[i]);
			if (  (params[i].indexOf('option_id')>-1)
				||(params[i].indexOf('function_id')>-1)
				||(params[i].indexOf('functionid')>-1)    //"function id " from LC / DS
				||(params[i].indexOf('vo.functionId')>-1) //"function id " from LC / DS
				||(params[i].indexOf('appl_task_id')>-1)
				||(params[i].indexOf('module_id')>-1)
				||(params[i].indexOf('locale')>-1)
				)
			{

				paramurl = paramurl + '&' + params[i];

			}
		}
		/*
		 If there is no function_id passed in Query String then search for hidden variable 'function_id' in the form and append it to the url.
		 - Vinod 3/6/2009
		 */
		if ( paramurl.indexOf('function_id') == -1 )
		{
			/*
			  In few JSPs 'form' is not defined so the validation for the existence of form is required.
			  - Vinod 3/9/2009
			 */
			 if ( (document.forms[0]) && (document.forms[0].function_id) )
			 {
				paramurl = paramurl + '&' + 'function_id' + '='+ document.forms[0].function_id.value;
			 }
		}
		// alert(paramurl);
/*
 ...
 */
		...
		//if(BrowserVersion()>6){
			...
			//var helpWindow = window.showModalDialog( '../../../../'+contextPath+'/OnlineHelp/webhelp.jsp?url='+paramurl, 'eHIS', features );
		...
		//}else{
			var helpWindow = open( '../../../../'+contextPath+'/OnlineHelp/webhelp.jsp?url='+paramurl, 'eHIS', myFeatures );
		//}

	}else if (whichCode == 8 && e != "INPUT" && e != "TEXTAREA")
	{
		event.cancelBubble = true;
		event.returnValue = false;
		return false;
	}else if (whichCode == 13 && e != "TEXTAREA")
	{
		event.cancelBubble = true;
		event.returnValue = false;
		return false;
...
//Function to Change the font to uppercase
...
function getErrorMessage(mess,notes){


	var x = document.getElementById("errorTemp");
	x.value=mess;
}
 ...
 function CheckDate(Object2,focusflag) {
	 ...
	 console.log("----+++---called to check date::"+JSON.stringify(Object2));


	 var getValues =$(Object2).attr('data-dater');


		var getDate = $("#"+getValues).data("kendoDateTimePicker");

		 console.log("----+1+23+---called to check date::"+getDate);

		var getDateObj = getDate.value();


		 console.log("abcdefgh called to check date::"+getDateObj);


		 getDateObj=getString(getDateObj);
    ...
    if (ChkDate(Object2) == false) {
       // alert(getMessage("INVALID_DATE_FMT", "SM"));
    	...
    	getErrorMessage("INVALID_DATE_FMT","SM");
...
function getMessageIpad( message_ref, moduleId, param ) {
...
function isSpclChar(doc){

	var iChars = "!@#$%^&*()+=-[]\\\';,/{}|\":?";
	for (var i = 0; i < doc.value.length; i++) 
	{
	    if (iChars.indexOf(doc.value.charAt(i)) != -1)     
	    return false;
...
function allowValidNumberLocalIpad(fld, e, maxInt, deci,comp)
{
    ...
    var whichCode = keyCodes;

var spl =isSpclChar(fld);
    if(!spl){
    ...
    if(regexPattern.test(fld.value)){
        alert('Special characters are not allowed !');
        fld.value="";
        return false;
    }
   
// include code for restricting spl characters //
...
function checkValidRange1Ipad(numObj,numhidObj,minDigit,decDigit,short_desc_alert)
{
	...
	if(val.indexOf(".") != -1)
	{
		...
		{
			var msg = getMessageIpad('VARIABLE_DEC_DIGIT','CA');
//		alert("enter value if not in range 12");
	...
	if ( val != null && val != '')
	{
		...
		{
			var message_alert=getMessageIpad("MINIMUM_DIGITS","CA");
			message_alert = message_alert.replace('$',short_desc_alert);
//			console.log("msg"+"message_alert");
//			alert("enter value if not in range 34");

			

			console.log(message_alert+' '+minDigit);
...
//	min_max_obj	=	eval("document.RecClinicalNotesTemplateForm.min_max" + numhidObj);
	...
	min_max_obj=document.getElementsByName("min_max"+numhidObj)[0].value;




	//if (min_max_obj.length > 1 )
		min_max_value	=	min_max_obj;
	//else
	...
	{
		var msg = getMessageIpad("MIN_MAX_VALUE_RANGE","CA");
...
function DateTransform(transform,messageFrame) {

    form = transform[0];
    datefrom = transform[1];
    datefrom1 = transform[2];
    dateto = transform[3];
    dateto1 = transform[4];
    ...
    if ( doDateCheck(datefrom1,dateto1,messageFrame )){
        form.submit() ;
        return true;
    }else return false;
}
...
function CheckForSpecCharsAllCaseAlwSpc(event){
    var strCheck = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_* ';
    var whichCode = (window.Event) ? event.which : event.keyCode;
    key = String.fromCharCode(whichCode);  // Get key value from key code
    if (strCheck.indexOf(key) == -1) return false;  // Not a valid key
...
function CheckForSpecCharsAllCase(event){
    var strCheck = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_*';
    var whichCode = (window.Event) ? event.which : event.keyCode;
    key = String.fromCharCode(whichCode);  // Get key value from key code
    if (strCheck.indexOf(key) == -1) return false;  // Not a valid key
...
function CheckForSpecCharsAllCaseAlwSpcVal(event,val){
    var whichCode = (window.Event) ? event.which : event.keyCode;
    key = String.fromCharCode(whichCode);  // Get key value from key code
    if (val.indexOf(key) == -1) return false;  // Not a valid key
	return true ;
}

function CheckForSpecCharsAllNotAlwSpc(event,val){
    var whichCode = (window.Event) ? event.which : event.keyCode;
    key = String.fromCharCode(whichCode);  // Get key value from key code
    if (val.indexOf(key) == -1) return true;  // Not a valid key
	return false ;
}
...
/* Added By Dharma on Dec 1st against IN:052597 Start*/

function fillDecimal(obj,maxInt,deci){
	var objValue	= obj.value;
	if(objValue!="" && deci > 0){
		var len	= objValue.length;
		var dotIndex    =   objValue.indexOf(".");
		if(len==(parseInt(maxInt+1)) && dotIndex==maxInt){
			for(i=0;i<deci;i++){
				obj.value	= obj.value+"0";
			}
		}else if(dotIndex > maxInt){
			var new_str	=  objValue.substr(0, maxInt);
			new_str		= new_str +".";
			for(i=0;i<deci;i++){
				new_str		= new_str+"0";
			}
...
/* Added By Dharma on Dec 1st against IN:052597 Int*/
