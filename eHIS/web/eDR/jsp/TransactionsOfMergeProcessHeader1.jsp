					...
					if(!p_order_by.equals(""))	
		           {
					...
					if(rs != null)
					{
						...
						{	
							...
							<td colspan='2' class='label'><a name='<%=x%>' onClick='changeRowColor(this,3)' href='javascript:callDupGroupDetails("<%=p_patient_id%>","<%=p_dup_group_id%>","<%=p_pat_count%>","S")' id='_a<%=p_patient_id%>'>&nbsp;<%=p_patient_id%></a><br>
							<font size='1'><%=p_patient_line%> </font>
							</td>
							<td valign='top' class='label'><%=p_pat_count%></td>
					</tr>
					<%
						i++;
					}
