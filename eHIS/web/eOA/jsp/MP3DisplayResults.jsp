...
%>
<%@ page contentType="text/html;charset=UTF-8" import ="java.sql.*,java.text.*,webbeans.eCommon.*,java.util.*,eCommon.Common.*,eCommon.XSSRequestWrapper" %>
<%  
...
%>
	...
	<tr><td class='label' colspan='3'>&nbsp;</td></tr>
	<tr><td colspan=3 class='label' align='center'><b><fmt:message key="eOA.Scheduleisnotdefined.label" bundle="${oa_labels}"/></b></td></tr>
	<script>
...
%>
	...
	<tr>
		<td class='label' nowrap  ><b><%=care_locn_ind_desc%>/<%=P_O_CLINIC%></b>
		</td>
<%
	if(value_chk.equals("display1")){
		if(!alcn_criteria.equals("")) {%>
		...
		if(!alcn_criteria.equals("")) {%>
			<td class='label' width='30%'><a href="javascript:allocationDetails('<%=clinic_code3%>','<%=date%>','<%=practitioner_id3%>')"><b><fmt:message key="eOA.Allocation.label" bundle="${oa_labels}"/></b>
			</a></td>
		<%}else{%>
			<td class='label' width='30%'>&nbsp;</td>
		<%} 
...
<table border="1"  cellspacing='0' cellpadding='0' width='148%' id='results0'>
				<th align='left' width='15%'><div  id='head1'  class='myClass' nowrap ><fmt:message key="Common.fromtime.label" bundle="${common_labels}"/></div></th>
				<th align='left' width='15%'><div  id='head2'  class='myClass' nowrap ><fmt:message key="Common.totime.label" bundle="${common_labels}"/></div></th>
				<th align='left' width='30%'><div  id='head3'  class='myClass' nowrap ><fmt:message key="Common.details.label" bundle="${common_labels}"/></div></th>
				<th align='left' width='30%'><div  id='head4'  class='myClass' nowrap ><fmt:message key="eOA.SecondaryResources.label" bundle="${oa_labels}"/></div></th>
			 <!--Below line Added for this CRF Bru-HIMS-CRF-165.1 [IN:035659]   -->
...
if( ! u .equals("~")) 
		...
		if(z.equals("FL") || z.equals("OL") || z.equals("VL")|| z.equals("VT")){
			if(!(mode_find.equals("revise")) || mode_find.equals("")){
					...
					if(chk_blank_row_patient.equals("~")){ 
						...
						if(slots > 0)
							{%>
								<td width='15%' class="<%=classValue%>" onClick='col(this)'><%=u%><input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' > </td>
								<%slots--;
								same_appt = "Y";
							}
							else
							...
							else
							{
								if (z.equals("FL")&&(!normal_booking_allow_flag)){%>
								   <td width='15%' class="<%=classValue%>" onClick='col(this)'><%=u%></a> <input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' > </td>
								<%}else{
									if(blkStatus.equals("B")){%>
											<td width='15%'  class='OARED' title="<%=blkRemark%>"><%=u%></td>
									<%}else{
										if(P_O_TIME_TAB_TYPE.equals("2")&& groupApptYN.equals("Y") && groupCnt>0 && callingMode.equals("") && (from_page.equals("") || from_page.equals("schedule_appt"))){
											%>
									   <td width='15%' class="<%=classValue%>" onClick='col(this)'><a href="javascript:GroupappointmentSelect('','<%=o_appt_ref_no[init]%>')"><%=u%></a> <input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' > </td>
									<%
										}else{
										%>
									   <td width='15%' class="<%=classValue%>" onClick='col(this)'><a href=javascript:moveNext_results0('')><%=u%></a> <input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' > </td>
									<%}}
								}
							}
							}
								...
								<%}else{
									if(blkStatus.equals("B")){%>
										<td width='15%'  class='OARED' title="<%=blkRemark%>"><%=u%></td>
									<%}else{%>
										<td width='15%' class="<%=classValue%>" onClick='col(this)'><a href= javascript:moveNext_results0('')><%= u%><input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' > </td>
									<%}
								}
						}
					}
                   ...
                   <%} else{
						...
						<%}else{%>
							<%if(z1.equals("OL*F")){%>
								<td width='15%'  class='OAFORCED' onClick='col(this)' Title='Forced Appointment'><a href= javascript:moveNext_results0('F')><%=u%></a> <input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' >  </td>
							<%}else if(z1.equals("OL*")){%>
								
								<td width='15%'  class="<%=classValue%>" onClick='col(this)'><a href= javascript:moveNext_results0('NF')><%=u%></a> <input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' ></td>												
							<%}else if(z1.equals("OL*M")){%>	
								<td width='15%'  class="<%=classValue%>" onClick='col(this)'><a href= javascript:moveNext_results0('M')><%=u%></a> <input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' ></td>							
							<%}else{%>	
								<%if(z.equals("VL")){%>
									<td width='15%'  class="<%=classValue%>" onClick='col(this)'><a href= javascript:moveNext_results0('NF')><%=u%></a> <input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' ></td>
								<%}else{%>							
					...
					if(chk_blank_row_patient.equals("~")){
						...
						<td width='15%' class="<%=classValue%>" ><%= u%><input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' > </td>
					<%
					}
		   }
		}
		else if(z.equals("RN")){%>
		<td width='15%' class='OABLOCK' title =<%=reason%>><%= u%> <input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' ></td>
		<%}	
		else if(z.equals("BN")){%>
		...
		<td width='15%' class="<%=classValue%>"><input type='hidden'  name='o_appt_ref_no<%=init%>' id='o_appt_ref_no<%=init%>' value='<%=o_appt_ref_no[init]%>' ></td>
		<%}
...
if( ! u1 .equals("~"))
		...
		if(z.equals("FL") || z.equals("OL") || z.equals("VL") || z.equals("FN")|| z.equals("VT")){
			if(blkStatus.equals("B")){%>
				<td width='15%' class="OARED" title="<%=blkRemark%>"><%= u1%></td>
			<%}else{%>
				<%if(z1.equals("OL*F")){%>
					<td width='15%' class="OAFORCED" Title="Forced Appointment"><%=u1%></td>
				<%}else{%>
					<td width='15%' class="<%=classValue%>"><%=u1%></td>
				<%}%>
			<%}%>
		<%}
		else
			{
			if(z.equals("RN")){%>
			<td width='15%' class='OABLOCK' title ="<%=reason%>"><%= u1%></td>
			<%}
		  else{%>
			<td width='15%' class='OARED' title ="<%=reason%>"><%= u1%></td>
		  <%}
			}

		}
		else{%>
			<td width='15%' class="<%=classValue%>">&nbsp;</td>
		<%}
...
if(pat_id.equals("~"))
	...
	{
		if(z.equals("FL") || z.equals("OL") || z.equals("VL") || z.equals("FN")|| z.equals("VT")){%>
			<td width='30%' class="<%=classValue%>"><%=pat_id%>&nbsp;<%=u2%><% if(pat_cat[init].equals("V")){%><img src='../../eCommon/images/PI_VIP.gif'><%}%></td>
		<%}else
		{
					...
					{%>
						<td width='30%' class='OARED' title ="<%=reason%>">&nbsp;</td>
					<%}
		}
	}
	else
	{
		if(same_appt.equals("Y")){%>
			<td width='30%'  class="<%=classValue%>"><%=pat_name%></td>
		<%}	else{
			if(z.equals("BN")){%>
				<td width='30%' class="<%=classValue%>"><fmt:message key="Common.block.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.reason.label" bundle="${common_labels}"/>:<b><%=reason%></b></td>
			<%}else{%>				 
				<td width='30%' class="<%=classValue%>">&nbsp;</td>
			<%}
...
<%}%>
 ...
 <input type='hidden' name='sys_time' id='sys_time' value='<%= current_time %>' >
  ...
  <input type='hidden' name='clinic_code3' id='clinic_code3' value='<%= clinic_code3 %>' >
<input type="hidden" name="P_O_OBVISIT_FLAG" id="P_O_OBVISIT_FLAG" value="<%=P_O_OBVISIT_FLAG%>">

  <input type='hidden' name='pract_id2' id='pract_id2' value='<%=practitioner_id2 %>' >
  <input type='hidden' name='pract_id3' id='pract_id3' value='<%=practitioner_id3 %>' >
...
<input type="hidden" name='rd_appt_yn' id='rd_appt_yn' value='<%=rd_appt_yn%>'> <%/*this hidden field Added for this SCF [GHL-SCF-715]*/%>
...
finally
{
 ...
 if(value12.length() !=0)
	 {%>
		...
		parent.parent.frames[0].document.forms[0].decide.value='one'; parent.parent.frames[0].document.forms[0].ref_no.value='<%=value13%>';
		parent.parent.frames[0].document.forms[0].reason_for_revise.value='<%=value12%>';
		}
		</script>
	<% }
