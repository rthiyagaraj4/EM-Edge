...
function Focusing(fld) {
    ...
    if ((!document.getElementById(fld).readOnly) && (document.getElementById(fld).type !='hidden'))
    ...
    if((window.event.button == 2) || (window.event.button == 3) || (window.event.button == 4)) {
		...
		try {
          var error=getMessage("WELCOME","common");
			//alert(error); //COMMENTED FOR EDGE WORK COMPLETED FINISH WILL ENABLE
		}
		catch (error) {
...
/*
	This block creates a parameter url with the required fields only. 
...
function displayRemarksForOtherReason(obj)
{
	...
	if(others_id!=""){
		...
		{
			document.getElementById("remarks_labl").style.visibility = "visible";
			document.getElementById("remarks_fld").style.visibility = "visible";	
			//document.getElementById("remarks_img").style.visibility = "visible";//Commented by KAMATCHI S for ML-MMOH-CRF-1390	
		}
		else
		{
			document.getElementById("remarks_labl").style.visibility = "hidden";
			document.getElementById("remarks_fld").style.visibility = "hidden";	
			//document.getElementById("remarks_img").style.visibility = "hidden";//Commented by KAMATCHI S for ML-MMOH-CRF-1390	
		}
	}
}
/*End ML-MMOH-CRF-1114*/

//Below function added for MOHE-CRF-0113 US001
//Below function allow alphabets & numbers to paste and will not allow special characters while paste
function CheckSplCharsValidation(Obj){
	var str = Obj.value;
...
function CheckSplCharsforIdValidation(Obj){
	var str = Obj.value;
		strCheck = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
	for (i=0;i<str.length;i++){
		if (strCheck.indexOf(str.charAt(i)) == -1) 
...
function CheckForSpecCharsValidation1(event){
...
function CheckForSpecCharsValidation2(event){
    var strCheck = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    var whichCode = (window.Event) ? event.which : event.keyCode;
    key = String.fromCharCode(whichCode);  // Get key value from key code
	if (strCheck.indexOf(key) == -1) return false;  // Not a valid key
...
//Added by Himanshu for ML-MMOH-CRF-1930 (U3) on 19-04-2023
