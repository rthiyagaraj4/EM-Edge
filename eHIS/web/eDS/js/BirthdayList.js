...
function run(){
	...
	var flag =checkNull();
		if(flag==true){
			...
			var formObj=parent.content.f_query_rep.BirthdayList;
			...
			var fromDate = convertDate(formObj.date.value,"DMY",locale,"en");
			var kitchen = formObj.Kitchen.value;
			var wardCodeFrom=formObj.wardCodeFrom.value;
			var wardCodeTo=formObj.wardCodeTo.value;

			formObj.action="../../eCommon/jsp/report_options.jsp?p_module_id="+p_module_id+"&p_report_id="+p_report_id+"&p_facility_id="+p_facility_id+"&p_user_name="+p_user_name+"&facility_id="+facility_id+"&locale="+locale+"&param1="+fromDate+"&p_kitchen_code="+kitchen+"&param8="+wardCodeFrom+"&param9="+wardCodeTo;
			formObj.submit();
		}
...
function checkNull(){
	var flag;
 	...
 	var fromDate=formObj.date.value;
