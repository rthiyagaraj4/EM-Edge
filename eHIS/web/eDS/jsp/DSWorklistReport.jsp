...
<script src='../../eCommon/js/showModalDialog.js' language='JavaScript'></script>
	...
	<%
		...
		%>
	<form name="WorkListForm" id="WorkListForm" target="messageFrame" method="POST">
			...
			<tr>
				...
				<tr>
					...
					<td class='fields'><select name="mealTypeFrom" id="mealTypeFrom"><option value=""><fmt:message key="eOT.select.Label" bundle="${ot_labels}" /></option>
								...
								%>
					<td class="label"><fmt:message key="eOT.MealTypeTab.Label" bundle="${ot_labels}" /></td>
					<td class='fields'><select name="mealTypeTo" id="mealTypeTo"><option value=""><fmt:message key="eOT.select.Label" bundle="${ot_labels}" /></option>
									<%pstmt=conn.prepareStatement(sql);
									rst = pstmt.executeQuery();
									while(rst.next()){
									strMtype = rst.getString(1);
				...
				<tr>
					<td class="label"><fmt:message key="eOT.ItemType.Label" bundle="${ot_labels}" /></td>
					<td class='fields'><select name = "ItemTypeFrom">
					<option value=""><fmt:message key="eOT.select.Label" bundle="${ot_labels}" /></option>
					<%                     try
												{
												sql ="select ITEM_TYPE.ITEM_TYPE,ITEM_TYPE.short_Desc from DS_ITEM_TYPE_LANG_VW ITEM_TYPE where ITEM_TYPE.eff_Status ='E'	and ITEM_TYPE.language_Id like '"+locale+"'";
														pstmt=conn.prepareStatement(sql);
														rst = pstmt.executeQuery();
														while(rst.next()){
														strIttype = rst.getString(1);
														strItDesc =  rst.getString(2);
											%>
											<option value="<%=strIttype%>"><%=strItDesc%></option>
											<%} %>
					</select></td><%
											}
										  catch(Exception e){
								...
								%>
					<td class="label"><fmt:message key="eOT.ItemType.Label" bundle="${ot_labels}" />	</td>
					<td class='fields'><select name = "ItemTypeTo">
					<option value=""><fmt:message key="eOT.select.Label" bundle="${ot_labels}" /></option><%pstmt=conn.prepareStatement(sql);
									rst = pstmt.executeQuery();
									while(rst.next()){
									strIttype = rst.getString(1);
									...
									<%
									   ...
									   finally
									   {
											try{
												...
												System.err.println("Exception in DSWork List:"+e);
											}
										}
				...
				<tr><td>&nbsp;&nbsp;</td><td>&nbsp;&nbsp;</td><td>&nbsp;&nbsp;</td><td>&nbsp;&nbsp;</td></tr>
					...
					<hidden property="functionId" value="DSR_WORKLIST"/>
					...
					<input type="hidden" name="p_report_id" id="p_report_id" value="DSRWKLST">
					<input type="hidden" name="report_id" id="report_id" value="DSRWKLST"/>
