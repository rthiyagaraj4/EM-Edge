...
<form name='calender1' id='calender1'>
		...
		<td  align="left" class="Day" width="13%" nowrap><b><fmt:message key="Common.Position.label" bundle="${common_labels}"/>/<fmt:message key="Common.SHIFT.label" bundle="${common_labels}"/>/<fmt:message key="eRS.WorkPlace.label" bundle="${rs_labels}"/>&nbsp;&nbsp;&nbsp;&nbsp;</b><input type='hidden' name='u1' id='u1' value='u1'>
		</td>
...
{		
	for(int q = 0; q<14; q++)
		...
		%>
		<tr >
		<td class='WRKPChange' colspan=15 nowrap><b><%=pos_temp%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></td>
<%
	while (pos_temp.equals(pos) && flag.equals("1"))
	{ 
			%>
		<tr >
		<td class='ShiftChange' colspan=15 nowrap><b><%=sh_temp%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></td>
	<%
		...
		<%
					...
					cell_no1[y] = -1; %>
<tr>
		<td class='Total' width='3%' nowrap><b><%=wp_temp%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></td>
		<%
			    classval="EmptyCell";
				...
				%>
				<td class="<%=classval%>" width='3%' align='center' id=<%=kkk%> <%=borderc%> >&nbsp;<%=wp_disp%>&nbsp;
					<%
					if(total_sh[kkk]==0)
					{
					total_sh[kkk]=m[kkk];
		...
		%>
		<tr >
		<td class='Total'  nowrap><b><fmt:message key="eRS.TotalByShift.label" bundle="${rs_labels}"/>&nbsp;&nbsp;</b></td>
		<%
		...
		%>
		<td class="<%=classval%>"  width='3%' align='center' <%=borderc%> >&nbsp;<%=disp%>&nbsp;</td>
		<% 
			if(total_pos[kkk1]==0){
		...
		%>
		<td  class="<%=classval%>" width='3%'>&nbsp;<%=disp%>&nbsp; </td>
		<%
			}
		else{
			if(total_sh[kkk1] == total_sh11[kkk1])
			...
			%>
				<td class="<%=classval%>" id=<%=kkk1%> width='3%'>&nbsp;<%=disp%>&nbsp;
					<%			
		}
		}
		%> 
	...
	%>
	<tr >
		<td class='Total_POS'  nowrap><b><fmt:message key="eRS.TotalByPosition.label" bundle="${rs_labels}"/>&nbsp;&nbsp;</b></td>
		<%
		for(int kkk1=0;kkk1<14;kkk1++){
