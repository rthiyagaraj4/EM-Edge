...
/* Initialize Function specific end */
	...
	String privilege_yn			= "N"; String disabled= "";
...
//out.println("period_from"+period_from);
//out.println("period_to"+period_to);
...
//	String  spl_yn = bean.getSplYN(practitioner_id,resp_id) ;
	String slClassValue = "";
	//String classValue = "";
//	int total = 0;
	...
	//if(from!=null && from.equals("1")) {
	StatusDetail			= bean.getStatusByDetail(facility_id,source_type,source_code,practitioner_id,resp_id,priority,ord_cat,order_by, patientId, period_from,period_to,start,end,"PS",filter) ;
...
%>
<form name='spl_apprv_order_dtl' id='spl_apprv_order_dtl' action="../../eOR/jsp/SpecialApprovalOrdersBottomRightDtl.jsp" method="post">
...
<tr id="heading">
		<th class='columnheader' width='' nowrap align='left'><input type=checkbox name='select_all' id='select_all' onClick='clickAll()' disabled><fmt:message key="Common.Approve.label" bundle="${common_labels}"/></th>
	<%if((order_by.equals(" ")||(order_by.equals("P"))) && ord_cat_desc.equals("")){%>
		<th class='columnheader' width='' nowrap><fmt:message key="Common.category1.label" bundle="${common_labels}"/></th>
	<%}%>
	...
	<%}%>
		<th class='columnheader' width='' nowrap align='left'><fmt:message key="eOR.OrderedAt.label" bundle="${or_labels}"/></th>
		<th class='columnheader' width='' nowrap align='left'><fmt:message key="eOR.OrderCatalogs.label" bundle="${or_labels}"/></th>
		<th class='columnheader' width='' nowrap align='left'><fmt:message key="Common.OrderDetails.label" bundle="${common_labels}"/></th>
		<%
 ...
 <%
		if(!(priority.equals(" ")))
		{
			...
			{
				out.println("<tr><td class='ORRED' colspan=6>"+priority_desc+"</td></tr>");
			}else if(priority.equals("S")){
				out.println("<tr><td class='ORBROWN' colspan=6>"+priority_desc+"</td></tr>");
			}else if(priority.equals("R")){
				out.println("<tr align='left'><th class='columnheader'  colspan=6>"+priority_desc+"</th></tr>");
			}
		}
		if(!(ord_cat.equals(" ")))
		{
			out.println("<tr align='left'><th class='columnheader'  colspan=6>"+ord_cat_desc+"</th></tr>");
		}
		...
		//out.println("status"+StatusDetail.size());
		for( int k=0 ; k< StatusDetail.size() ; k++ ) 
		{
		 	//for( int i=0 ; i< StatusDetail.size() ; i++ ) {
				...
				{
					if(record[31] != null)
					total_query_records = bean.checkForNull((String)record[31],"0");
					...
					<tr  align="right">
						<td colspan="6">
							...
							<jsp:param name ="form_name" value= "spl_apprv_order_dtl" />  
							</jsp:include>
						</td>
					</tr>
					<%
				...
				order_category = (String)record[0];
				cont_order_ind = (String)record[24];
				//out.println(record[11]+record[12]);
					
				privilege_yn  = (String)record[29];
				if(privilege_yn!=null && privilege_yn.equals("Y"))
					disabled  = "";
				else
					disabled  = "DISABLED";	 

				//out.println(visit_admn_date);
				...
				{
					out.println("<tr align='left'><th class='columnheader' colspan=6 >"+record[1]+"</th></tr>");
					tmp_ord_cat = record[1];
				}
				if(!(tmp_urgency.equals(record[2])) && (!(order_by.equals(" ") ||(order_by.equals("O")))))
				{
					out.println("<tr align='left'><th class='columnheader' colspan=6 >"+record[2]+"</th></tr>");
					tmp_urgency = record[2];
				}
		%>

			<tr>
				<td  width='' nowrap id='status<%=i%>' class='<%=slClassValue%>'><input type=checkbox name='chk<%=i%>' id='chk<%=i%>' <%=disabled%>></td>
				...
				{ 
					SpecialDate 	=com.ehis.util.DateUtils.convertDate(record[9],"DMYHM","en",localeName);
				}
				else
				{
						SpecialDate=record[9];
				}
				...
				%>
				<td width='' nowrap class='<%=slClassValue%>'><font size=1><%=SpecialDate%></td> <!--  data of ordered AT -->
				<td width='' nowrap class='<%=slClassValue%>'><font size=1><a class="gridLink"  href='#' onClick="callMenuFunctions('View','<%=record[1]%>','<%=record[6]%>','<%=record[22]%>','<%=record[7]%>','<%=record[8]%>','<%=record[19]%>')">
					<%=record[10]%>
					</a></td><!--  data of ordered Catalogs -->
		

				<td width='' wrap class='<%=slClassValue%>'><font size=1><%=patientdetails%><BR><%=encounter_dtls%>
			...
			{
			%>
				<td class='<%=slClassValue%>' width='' nowrap><a class="gridLink"  href="javascript:callBilling('<%=i%>','<%=record[6]%>','<%=record[7]%>','<%=facility_id%>','<%=record[8]%>','<%=record[19]%>','<%=record[1]%>','<%=record[22]%>','<%=menu_text1%>','<%=record[6]%>','<%=record[30]%>')" title='Charge Details'> <FONT SIZE="1" ><fmt:message key="eOR.View/Edit.label" bundle="${or_labels}"/></FONT></a></td>
			<%
			}
			...
			%>
				...
				<input type="hidden" name="order_set_id<%=i%>" id="order_set_id<%=i%>" value="<%=bean.checkForNull(record[30],"")%>">
			</tr>
		<%
			i++;
			}
		%>
	<script>ondetail();</script>
</table>
</div>
...
<script>
parent.parent.parent.SpecialApprovalOrdersTop.document.aspl_apprv_order.search_btn.disabled = false;
</script>
</html>
<%
