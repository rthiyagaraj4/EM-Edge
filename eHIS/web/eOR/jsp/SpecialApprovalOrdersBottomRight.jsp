...
%>
<iframe name='SpecialApprovalOrdersBottomRight1' id='SpecialApprovalOrdersBottomRight1' src='SpecialApprovalOrdersBottomRightDtl.jsp?source_type=<%=source_type%>&source_code=<%=source_code%>&order_by=<%=order_by%>&priority=<%=priority%>&total_recs=<%=total_recs%>&ord_cat=<%=ord_cat%>&ord_cat_desc=<%=ord_cat_desc%>&priority_desc=<%=priority_desc%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>&filter=<%=filter%>' frameborder=0 scrolling='auto'  noresize style='height:80vh;width:99vw'></iframe>
