...
---------------------------------------------------------------------------------------------------------------
-->
<%@ page import="java.sql.*,java.util.*,java.text.*,java.net.*,webbeans.eCommon.*,eOR.*,eOR.Common.*,eCommon.XSSRequestWrapper" contentType="text/html;charset=UTF-8" %>
...
<form name='OrderableResult' id='OrderableResult' action="ORApptOrderSelectionResult.jsp?<%=request.getQueryString()%>"  method='post'>
<%
...
<%
		...
		{
%>
			<a class='gridLink' href='javascript:submitPrevNext(<%=(start-7)%>,<%=(end-7)%>)' text-decoration='none'><fmt:message key="Common.previous.label" bundle="${common_labels}"/></a>
...
<%
		...
		{
%>
			<a class='gridLink' href='javascript:submitPrevNext(<%=(start+7)%>,<%=(end+7)%>)' text-decoration='none'>&nbsp;<fmt:message key="Common.next.label" bundle="${common_labels}"/></a>
...
%>
			...
			<input type='checkbox' name="chk<%=ilCkeckedYN%>" id="chk<%=ilCkeckedYN%>" value="<%=option_id%>~~<%=short_desc%>~~<%=order_type_code1%>~~<%=order_category1%>~~<%=appt_ref_no%>" <%=checkProp%> onclick='getDescription(this,"<%=option_id%>","<%=counts%>");'> <!-- IN030284.1 -->
			...
			<input type="hidden" name="appt_ref_no<%=counts%>" id="appt_ref_no<%=counts%>" value="<%=appt_ref_no%>">
			</td></tr>
<%
...
%>
		...
		<input type=hidden name='qry_str' id='qry_str' value='<%=qry_str%>'>
