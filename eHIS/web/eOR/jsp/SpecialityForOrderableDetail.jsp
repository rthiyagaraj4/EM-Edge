...
<%@page import="webbeans.eCommon.ConnectionManager,eOR.Common.*, eOR.*,eCommon.XSSRequestWrapper" %>
...
%>
	...
	<td class='columnheader' width='75%'><fmt:message key="Common.description.label" bundle="${common_labels}"/></td>
	...
	try{
       ...
       tblrow_id	=	"tblrow_id"+i;
				...
				%>
				<tr id='<%=tblrow_id%>'>
				<td class='<%=classValue%>' ><%=((desc.equals(""))?"&nbsp;":desc)%>
				<%if(!spec_code.equals(""))
				      {
					%>
					<B><label class="label"><font COLOR='#A4AE00;'>[<fmt:message key="Common.speciality.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.specific.label" bundle="${common_labels}"/>]</font></label></B>
					<%	
						}
			    %>
				</td>
			...
			%>
				<tr id='<%=tblrow_id%>'>
				<%if(valid.equals("Y")){%>
					<td class='<%=classValue%>' ><b><%=((desc.equals(""))?"&nbsp;":desc)%></b>
					<%if(!spec_code.equals(""))
				      {
					%>
					<B><label class="label"><font COLOR='#A4AE00;'>[<fmt:message key="Common.speciality.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.specific.label" bundle="${common_labels}"/>]</font></label></B>
