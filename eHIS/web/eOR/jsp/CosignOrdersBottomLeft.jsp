...
//out.println("source_type"+source_type);
	...
	ArrayList CategoryData = bean.getStatusByCategory(properties, facility_id,source_type,source_code,practitioner_id,resp_id,patientId, period_from, period_to,"CS") ;
...
<table cellpadding=3 cellspacing=0 border=1 width="100%" class="grid">
	<tr>
		...
		%>
			...
			<tr><td class='<%=slClassValue%>'><font size=1><a class="gridLink"  href='CosignOrdersBottomRight.jsp?priority=<%=temp%>&priority_desc=<%=record[0]%>&total_recs=<%=record[1]%>&source_type=<%=source_type%>&source_code=<%=source_code%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>' target='CosignOrdersBottomRight'><%=record[0]%></a><b>(<%=Integer.parseInt(record[1])%>)</b></font></td></tr>
		...
		<%if(total !=0){%>
			...
			<a class="gridLink"  href='CosignOrdersBottomRight.jsp?order_by=P&total_recs=<%=total%>&source_type=<%=source_type%>&source_code=<%=source_code%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>' target='CosignOrdersBottomRight'><fmt:message key="Common.all.label" bundle="${common_labels}"/><B>(<%=total%>)</B></a>
	...
	<tr>
		...
		%>
				...
				<tr><td class='<%=slClassValue%>'><font size=1><a class="gridLink"  href='CosignOrdersBottomRight.jsp?ord_cat=<%=record[2]%>&ord_cat_desc=<%=record[0]%>&total_recs=<%=record[1]%>&source_type=<%=source_type%>&source_code=<%=source_code%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>' target='CosignOrdersBottomRight'><%=record[0]%></a><b>(<%=Integer.parseInt(record[1])%>)</b></font></td></tr>

		<%
			}
		%>
						<tr><td class='cagroup' bgcolor='#D5E2E5'><%if(total !=0){%><a class="gridLink"  href='CosignOrdersBottomRight.jsp?order_by=O&total_recs=<%=total%>&source_type=<%=source_type%>&source_code=<%=source_code%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>' target='CosignOrdersBottomRight'><%}%><fmt:message key="Common.all.label" bundle="${common_labels}"/><B>(<%=total%>)</B></a></font></td></tr>
</table>
