...
<table cellpadding=3 cellspacing=0 border=1 width="100%" class="grid">
	<tr>
		<th class='columnheader' align='left'><fmt:message key="eOR.StatusByPriority.label" bundle="${or_labels}"/></th>
	</tr>
		...
		%>
				...
				<tr><td class='<%=slClassValue%>'><font size=1><a class="gridLink"  href='SpecialApprovalOrdersBottomRight.jsp?priority=<%=record[2]%>&priority_desc=<%=record[0]%>&total_recs=<%=record[1]%>&source_type=<%=source_type%>&source_code=<%=source_code%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>&filter=<%=filter%>' target='SpecialApprovalOrdersBottomRight'><%=record[0]%></a><b>(<%=Integer.parseInt(record[1])%>)</b></font></td></tr>
		...
		<%if(total !=0){%>
			<tr><td class='cagroup' >
			<a class="gridLink"  href='SpecialApprovalOrdersBottomRight.jsp?order_by=P&source_type=<%=source_type%>&source_code=<%=source_code%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>&total_recs=<%=total%>&filter=<%=filter%>' target='SpecialApprovalOrdersBottomRight'><fmt:message key="Common.all.label" bundle="${common_labels}"/><B>(<%=total%>)</B></a>

			</td></tr>
		<%}%>

	<tr>
		<th class='columnheader' align='left'><fmt:message key="eOR.StatusByCategory.label" bundle="${or_labels}"/></th>
	</tr>
		<%
		total = 0;
		...
		%>
				...
				<tr><td class='<%=slClassValue%>'><font size=1><a class="gridLink"  href='SpecialApprovalOrdersBottomRight.jsp?ord_cat=<%=record[1]%>&ord_cat_desc=<%=record[3]%>&total_recs=<%=record[2]%>&source_type=<%=source_type%>&source_code=<%=source_code%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>&filter=<%=filter%>' target='SpecialApprovalOrdersBottomRight'><%=record[0]%></a><b>(<%=Integer.parseInt(record[2])%>)</b></font></td></tr>

		<%
			}
		%>
			<tr><td class='cagroup' ><%if(total !=0){%><a class="gridLink"  href='SpecialApprovalOrdersBottomRight.jsp?order_by=O&total_recs=<%=total%>&source_type=<%=source_type%>&source_code=<%=source_code%>&patientId=<%=patientId%>&period_from=<%=period_from%>&period_to=<%=period_to%>&filter=<%=filter%>' target='SpecialApprovalOrdersBottomRight'><%}%><fmt:message key="Common.all.label" bundle="${common_labels}"/><B>(<%=total%>)</B></a></font></td></tr>
</table>
