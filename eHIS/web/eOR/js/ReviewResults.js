...
--------------------------------------------------------------------------------------------------------------
?             100            ?           created						
06/07/2012    IN033610      Chowminya G	 Add Notification Alert in summary 	
18/07/2012    IN034064      Chowminya G	 Add Notification Alert in summary - include remarks	
...
16/09/2014	  IN051199		Ramesh G	Histopathology reports does not open up in All Result section & can be seen at View Result 
		...
		if(!isAfter(toDate.value,fromDate.value,"DMY",formObj.localeName.value)){//this function is available in eCommon/js/DateUtils.js
			...
			*/
			...
			else
			{
				...
				qry_string += "&get_task_applicability="+parent.ReviewResultsSearch.document.review_result_search.get_task_applicability.value;
				//qry_string += "&modal=Y";
				//alert(top.content.name);
				parent.ReviewResultsSearch.document.review_result_search.search_btn.disabled =true;
				parent.ReviewResultsHeader.location.href = "../jsp/ReviewResultsHeader.jsp?"+qry_string;
				parent.ReviewResultsResult.location.href = "../../eCommon/html/blank.html";
				parent.ReviewResultsBtn.location.href = "../jsp/ReviewResultsBtn.jsp";
			}
			/*
				ends here.			
			*/
...
async function clickAll(){
	if(document.review_results_hdg.select_all.checked == true){
	   var retVal = await clickHeader(document.review_results_hdg.select_all);
	   if (retVal != null) {
...
function completeReview(){
	...
	qry_string = "../jsp/ReviewResultsDetail.jsp?practitioner_id="+practitioner_id+"&priority="+priority+"&normalcy_ind="+normalcy_ind+"&result_type="+result_type+"&patientId="+patientId+"&event_class="+event_class;
	...
	if(parseInt(no_of_checked) == 0){
		...
		if( invalidCode != "" && invalidCode!=null){// in orMessageArray){ //"OR_STATUS_CHANGED"
			eval("parent.ReviewResultsResult.document.getElementById('status"+invalidCode+"')").className="orcancel";
...
async function viewClobData(accession_num){
		...
		//alert(finalString);
		//var retVals = window.showModalDialog("../../eOR/jsp/ReviewResultClobData.jsp?"+finalString,arguments,features);	//IN041228
		var retVals = await window.showModalDialog("../../eOR/jsp/ReviewResultClobDataMain.jsp?"+finalString,arguments,features); //IN041228
}
...
async function viewComments(i){
	...
	var features		= 'dialogHeight:' + dialogHeight + '; dialogWidth:' + dialogWidth +'; overflow: hiddend; status=no';
	var arguments;
	arguments 			= result ;
	hdg_name			= "";
	retVal 				= await window.showModalDialog("../../eOR/jsp/OrderEntryComments.jsp?hdg_name="+encodeURIComponent(hdg_name),arguments,features);
}
...
async function viewClobDataHtm(accession_num){
		...
		var arguments = "";
		//IN051199 Start.
		//var finalString	= "accession_num="+ accession_num;
...
function populateEventClass( obj ) {
	...
	xmlHttp.open( "POST", "ReviewResultsValidate.jsp?bean_id=" + bean_id + "&bean_name="+ bean_name+"&func_mode=EVENT_CLASS&localeName="+localeName, false ) ;
	xmlHttp.send( xmlDoc ) ;
...
async function clickHeader(obj) {
    if (obj.checked == true) {
		...
		var retVals = await window.showModalDialog("../../eOR/jsp/ReviewResultsRemarks.jsp",arguments,features);
...
function clickChk(rec_num){
	...
	//--[IN033610]
	//if((eval("parent.ReviewResultsResult.document.review_results_dtl.chk"+rec_num).checked == true ) && !(eval("parent.ReviewResultsResult.document.review_results_dtl.normal_ind"+rec_num).value == "Y"))//--[IN034064]
	if((eval("parent.ReviewResultsResult.document.review_results_dtl.chk"+rec_num).checked == true )){
		   ...
		   eval("parent.ReviewResultsResult.document.getElementById('remark_td"+rec_num+"')").innerHTML="<font size='1'><a class='gridLink' href=\"javascript:clickLine('"+rec_num+"')\">"+getLabel('Common.remarks.label','Common')+"</a></font>";			
	}else{
		//IN066119 starts
		if(eval("parent.ReviewResultsResult.document.review_results_dtl.headval"+rec_num)!=undefined){
			...
			temp=eval("document.getElementById('maingrp"+headvalue+"')");
			if(temp!=undefined)
		...
		//IN066119 ends
		 ...
		 eval("parent.ReviewResultsResult.document.getElementById('remark_td"+rec_num+"')").innerHTML="&nbsp";	
		 eval("parent.ReviewResultsResult.document.review_results_dtl.review_remarks"+rec_num).value="";
	}

}
//IN066119 starts
function checkgroup(rec_num){
	var formObj     = parent.ReviewResultsResult.document.review_results_dtl;
	var flag=false;
...
async function clickLine(rec_num){
    if(eval("parent.ReviewResultsResult.document.review_results_dtl.chk"+rec_num).checked == true ){
        ...
        eval("parent.ReviewResultsResult.document.review_results_dtl.chk"+rec_num).value = "Y";
		var review_remarks	= eval("parent.ReviewResultsResult.document.review_results_dtl.review_remarks"+rec_num).value;
		var retVals = await window.showModalDialog("../../eOR/jsp/ReviewResultsRemarks.jsp?&review_remarks="+encodeURIComponent(review_remarks),arguments,features);
		if (retVals != null) {
...
async function callForward() {
	...
	if (count <= 0) {
		...
		xmlStr 			+=" /></root>" ;
		xmlDoc = new DOMParser().parseFromString(xmlStr, "text/xml");
		xmlHttp.open( "POST", "ReviewResultsValidate.jsp?bean_id=" + bean_id + "&bean_name="+ bean_name+"&func_mode=FORWARD_PRACT&localeName="+localeName, false ) ;
		xmlHttp.send( xmlDoc ) ;
		responseText=xmlHttp.responseText ;
		//eval( responseText ) ;
		...
		var features 	= 'dialogHeight:' + dialogHeight + '; dialogWidth:' + dialogWidth +';dialogTop:'+dialogTop+';dialogLeft:'+dialogLeft+';dialogTitle:'+title+'; scroll=no; status=no';
	    var arguments 	= "";
	    var retVals = await window.showModalDialog("ReviewResultsForwardPractFrameset.jsp",arguments,features);
...
function searchPractitioner() 
{
	...
	parent.ReviewResultsForwardPract.document.location.href = "ReviewResultsForwardPract.jsp?" + finalString;
}
...
function chk(Obj){  //Added by Prasad on 23/2/2007
