...
/*******************************************************************/
function create() {
	f_query_add_mod.location.href = "../../eOR/jsp/ChartHorizontalAddModifyframe.jsp?mode=1&function_id="+function_id;
}
/******************************************************************************/
function query() {
f_query_add_mod.location.href="../../eOR/jsp/ChartHorizontalQueryCriteria.jsp?function_id="+function_id;
}
/******************************************************************************/
...
function apply() {
	...
	formObjDetail.chart_result_type.value = formObjHeader.chart_result_type.value;
	
	var fields = new Array ( formObjHeader.chart_result_type);
	...
	arrLegends[0]			=	encodeURIComponent(getLabel("Common.code.label","Common"));
	...
	var	result1	=	validateMandatoryFields(formObjDetail,arrFieldName,arrLegends,arrPKFieldNames,startIndex, objMessageFrame, errorPage);
	var long_flag=false;var short_flag= false;
	if(result1)
	{
		...
		{
			for(var i = 0 ; i < 10 ; i++){
				if(eval("formObjDetail.org_code"+i).value != ''){
					if(trimCheck(eval("formObjDetail.org_long_desc"+i).value)){
						long_flag= true;
					}else{
						long_flag= false;
						break;
					}
					if(trimCheck(eval("formObjDetail.org_short_desc"+i).value))
						short_flag= true;
					else{
						short_flag= false;
						break;
			...
			if(resultGlobal) {
				...
				//alert("invalid"+invalidCode);
				//alert(showDuplicates(formObjDetail,arrPKFieldNames,invalidCode,"0",messageFrame ));
...
function onSuccess() {
	...
	if(flagGlobal == "insert" ){
		headerDoc.chart_result_type.selectedIndex = 0;
		formObjDetail.reset();
	}
	}else{//Update Mode
	f_query_add_mod.document.location.href="../../eOR/jsp/ChartHorizontalAddModifyframe.jsp?"+detailDocUpdate.qry_str.value ;
	}
}
/******************************************************************************/
function reset() {
...
if(checkFieldsofMst( fields, names, messageFrame)) {
	/*	var flds=new Array(formObjDetailUpdate.org_long_desc,formObjDetailUpdate.org_short_desc);
		var name=new Array("Long Description","Short Description");
		if(!SpecialCharCheck(flds,name,messageFrame,"M","../../eCommon/jsp/MstCodeError.jsp")){
			return false
		}*/
