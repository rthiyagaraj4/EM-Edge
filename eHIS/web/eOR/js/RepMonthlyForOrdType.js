...
function run(){
	f_query_add_mod.RepMonthlyForOrdType.submit();
	alert('sdf');
}
...
function populateOrderTypes(ord_cat){
	...
	var bean_id = document.RepMonthlyForOrdType.bean_id.value ;
...
function addOrderType1List(code,value) {
	...
	element.value =  code ;
	element.text = value ;
	document.RepMonthlyForOrdType.p_from_order_type.add(element);
}

function clearL1List( docObj ) {
	var len = eval(docObj+".RepMonthlyForOrdType.p_from_order_type.options.length") ;
	for(var i=0;i<len;i++)
	{
		eval(docObj+".RepMonthlyForOrdType.p_from_order_type.remove(\"atc_class_l2_code\")") ;
	}
	var tp = "     --- "+getLabel("Common.all.label","Common")+" ---     " ;
	opt = eval(docObj+".createElement(\"OPTION\")") ;
	opt.text = tp ;
	opt.value = "*ALL" ;
	eval(docObj+".RepMonthlyForOrdType.p_from_order_type.add(opt)") ;
}
...
function addOrderType2List(code,value) {
	...
	element.value =  code ;
	element.text = value ;
	document.RepMonthlyForOrdType.p_to_order_type.add(element);
}

function clearL2List( docObj ) {
	var len = eval(docObj+".RepMonthlyForOrdType.p_to_order_type.options.length") ;
	for(var i=0;i<len;i++)
	{
		eval(docObj+".RepMonthlyForOrdType.p_to_order_type.remove(\"atc_class_l2_code\")") ;
	}
	var tp = "     --- "+getLabel("Common.all.label","Common")+" ---     " ;
	opt = eval(docObj+".createElement(\"OPTION\")") ;
	opt.text = tp ;
	opt.value = "*ALL" ;
	eval(docObj+".RepMonthlyForOrdType.p_to_order_type.add(opt)") ;
}
...
function reset() {
	f_query_add_mod.location.href = '../../eOR/jsp/RepMonthlyForOrdTypeQuery.jsp';
