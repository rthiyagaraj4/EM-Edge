...
public class MoheDepositRefundCheckTransaction {
	...
	public String l_eventType = "BL02";
	...
	public String TransRequestMethod(String actionKey) {
		String l_facility_id = "", l_patient_id = "", l_episode_type = "", l_episode_id = "", l_visit_id = "", l_unadjusted_deposit = "";
		...
		try{
			...
			l_app_msg = " ::: Inside DepositRefundCheckTransaction TransRequestMethod ::: ";
			...
			if (l_request_ref_no.indexOf("Error") >= 0) {
				l_errcd = "E";
				l_errmsg = "request_ref_no not available in request parameter -"+l_request_ref_no;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "+ " } ";				
			...
			if (l_request_type.indexOf("Error") >= 0) {
				l_errcd = "E";
				l_errmsg = "request_type not available in request parameter -"+l_request_type;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "+ " } ";
			...
			if (l_facility_id.indexOf("Error") >= 0) {
				l_errcd = "E";
				l_errmsg = "facility_id not available in request parameter -"+l_facility_id;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "+ " } ";	
			...
			l_app_msg = " ::: Request Data Patient ID : " + l_facility_id;
			if (l_debug_YN.equals("Y"))	System.out.println(l_app_msg);
			
			l_patient_id = XHJSONUtil.getJsonRequestString(l_in_JSON, "patient_id" );
			if (l_patient_id.indexOf("Error") >= 0) {
				l_errcd = "E";
				l_errmsg = "Patient ID not available in request parameter -"+l_patient_id;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "+ " } ";
			...
			l_app_msg = " ::: Request Data Patient ID : " + l_patient_id;
			if (l_debug_YN.equals("Y"))	System.out.println(l_app_msg);
	        ...
	        //l_episode_type = FetchStdCntrlSegMethod(l_segment_id, l_element_id, l_standard_code);
			...
			{
				l_errcd = "E";
				l_errmsg = "Unable to Fetch Episode Type -"+l_episode_type;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "+ " } ";
			...
			l_app_msg = "Retrieving Episode ID...";
			if (l_debug_YN.equals("Y")) System.out.println(l_app_msg);
			
			//l_episode_id = FetchStdCntrlSegMethod(l_segment_id, l_element_id, l_standard_code);
			...
			{
				l_errcd = "E";
				l_errmsg = "Unable to Fetch Episode ID -"+l_episode_id;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "+ " } ";
			...
			l_app_msg = "Retrieving Visit ID...";
			if (l_debug_YN.equals("Y")) System.out.println(l_app_msg);
			
			//l_visit_id = FetchStdCntrlSegMethod(l_segment_id, l_element_id, l_standard_code);
			...
			{
				l_errcd = "E";
				l_errmsg = "Unable to Fetch Visit ID -"+l_visit_id;
				l_response_err_json = " { " + " \"status\" : \"0\" , " + " \"message\" : \" " + l_errmsg + " \"  "+ " } ";
			...
			try{
				...
				String DepositRefundCheckQuery = "{ call XBAPI.DEPOSIT_REFUND_CHECK(?,?,?,?,?,?,?,?)}";
				dbConn = EMDataSource.getConnection();
				cstmt = dbConn.prepareCall(DepositRefundCheckQuery);
				...
				l_unadjusted_deposit = cstmt.getString(6);
				String p_msg = cstmt.getString(7);
				String p_status = cstmt.getString(8);
				
				if(p_status.equals("E")){
					l_errcd = "E";
				}
				
				l_app_msg = " ::: Unadjusted Deposit : "+l_unadjusted_deposit+" : msg : "+p_msg+" : status : "+p_status;
				if (l_debug_YN.equals("Y")) System.out.println(l_app_msg);
				
				
			}catch (Exception e) {
				e.printStackTrace();
				l_app_msg = " ::: Exception while calling Disposit Refund Check - "+ e.getMessage();
			    if (l_debug_YN.equals("Y")) System.out.println(l_app_msg);
			...
			if(l_errcd.equals("S") && !l_unadjusted_deposit.equals("") && l_unadjusted_deposit != null)
			{
				l_status = "1";
				l_status_desc = "Deposit Refund Check details successfully retrieved";
				...
				l_response_json.put("unadjusted_deposit", l_unadjusted_deposit);
			...
			if(l_errcd.equals("E")){
				l_status = "0";
				l_status_desc = "Deposit Refund Check details retrieved failed";
				l_response_json.put("request_ref_no", l_request_ref_no);
				l_response_json.put("status", l_status);
				l_response_json.put("status_desc", l_status_desc);
				l_response_json.put("unadjusted_deposit", "");
