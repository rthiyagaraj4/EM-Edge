...
{
    31,
    "T:\\BL\\handover\\PROC\\Blrpruna.pc"
};


static unsigned long sqlctx = 2038189661;
...
"select EPISODE_ID ,PATIENT_ID ,TO_CHAR(NVL(TOT_UNADJ_PREP_AMT,0),'999,999,9\
90.99') ,NVL(TOT_UNADJ_PREP_AMT,0) ,TO_CHAR(NVL(TOT_PREP_PAID_AMT,0),'999,999\
,990.99') ,NVL(TOT_PREP_PAID_AMT,0) ,TO_CHAR((NVL(TOT_PREP_PAID_AMT,0)-NVL(TO\
T_UNADJ_PREP_AMT,0)),'999,999,990.99') ,(NVL(TOT_PREP_PAID_AMT,0)-NVL(TOT_UNA\
DJ_PREP_AMT,0))  from BL_EPISODE_FIN_DTLS where (((((EPISODE_TYPE=:b0 and EPI\
SODE_ID between NVL(:b1,0) and NVL(:b2,99999999)) and PATIENT_ID between NVL(\
:b3,' ') and NVL(:b4,'~~~~~~~~~~~~~~~~~~~~')) and NVL(TOT_UNADJ_PREP_AMT,0)<:\
b5) and NVL(TOT_PREP_PAID_AMT,0)>0) and OPERATING_FACILITY_ID=:b6) order by E\
PISODE_ID,PATIENT_ID            ";
...
),'999,999,990.99') ,NVL(TOT_PREP_PAID_AMT,0) ,TO_CHAR((NVL(TOT_PREP_PAID_AMT\
,0)-NVL(TOT_UNADJ_PREP_AMT,0)),'999,999,990.99') ,(NVL(TOT_PREP_PAID_AMT,0)-N\
VL(TOT_UNADJ_PREP_AMT,0))  from BL_VISIT_FIN_DTLS where ((((((EPISODE_TYPE in\
 ('O','E') and EPISODE_ID between NVL(:b0,0) and NVL(:b1,99999999)) and VISIT\
_ID between NVL(:b2,0) and NVL(:b3,9999)) and PATIENT_ID between NVL(:b4,' ')\
 and NVL(:b5,'~~~~~~~~~~~~~~~~~~~~')) and NVL(TOT_UNADJ_PREP_AMT,0)<:b6) and \
NVL(TOT_PREP_PAID_AMT,0)>0) and OPERATING_FACILITY_ID=:b7) order by EPISODE_I\
D,VISIT_ID,PATIENT_ID            ";

 static const char *sq0007 = 
"select PATIENT_ID ,TO_CHAR(NVL(TOT_UNADJ_PREP_REF_AMT,0),'999,999,990.99') \
,NVL(TOT_UNADJ_PREP_REF_AMT,0)  from BL_PATIENT_FIN_DTLS where (PATIENT_ID be\
tween NVL(:b0,' ') and NVL(:b1,'~~~~~~~~~~~~~~~~~~~~') and NVL(TOT_UNADJ_PREP\
_REF_AMT,0)<:b2) order by PATIENT_ID            ";

 static const char *sq0008 = 
"select SUBSTR(SHORT_NAME_LOC_LANG,1,16) ,short_name  from MP_PATIENT_MAST w\
here PATIENT_ID=:b0           ";
...
{10,4274,0,0,0,
5,0,0,1,0,0,27,125,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/BLRPRUNA.c-arc   1.0   Apr 19 2007 17:06:50   vcm_th  $ sqlca.h 
 */
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/BLRPRUNA.c-arc   1.0   Apr 19 2007 17:06:50   vcm_th  $ sqlda.h 
 */
...
AM4 ,PARAM5 ,PARAM6 ,PARAM7 ,PARAM8 ,PARAM9 ,TO_CHAR(TO_NUMBER(PARAM9),'999,9\
99,990.99') into :b0,:b1,:b2,:b3,:b4,:b5,:b6,:b7,:b8,:b9,:b10  from SY_PROG_P\
ARAM where ((PGM_ID=:b11 and SESSION_ID=:b12) and PGM_DATE=:b13)";
   sqlstm.iters = (unsigned int  )1;
...

{
     ...
     sqlstm.sqladtp = &sqladt;
     sqlstm.sqltdsp = &sqltds;
     sqlstm.stmt = "begin declare str1 varchar2 ( 100 ) ; str2 varchar2 ( 6\
0 ) ; BEGIN blcommon . split_words ( :d_short_name_full , 30 , '*' , 1 , str1\
 , str2 ) ; IF str2 IS NOT NULL THEN :bl_patient_name := str2 ; END IF ; blco\
mmon . split_words ( :d_short_name_full , 30 , '*' , 2 , str1 , str2 ) ; IF s\
tr2 IS NOT NULL THEN :bl_patient_name1 := str2 ; END IF ; blcommon . split_wo\
...
4:MI') ,USER into :b0,:b1,:b2  from SY_ACC_ENTITY where ACC_ENTITY_ID=:b3";
    sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRPRUNA.LEGEND_'||LTRIM(RTRIM(TO_CHA\
