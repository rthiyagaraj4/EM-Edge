...
{
    31,
    "T:\\BL\\handover\\PROC\\BLRCSTRX.pc"
};


static unsigned long sqlctx = 2038191045;
...
"select CASH_COUNTER_CODE ,SHIFT_ID ,TO_CHAR(DOC_DATE,'DD/MM/YYYY') ,(CASH_A\
MOUNT+CREDIT_AMOUNT) TOTAL_AMOUNT ,COLLECT_AMOUNT ,DECODE(DOC_TYPE_CODE,'',''\
,((DOC_TYPE_CODE||'/')||DOC_NUM)) CASH_CTRL_RCPT ,TO_CHAR(CASH_CTRL_DOC_DATE,\
'DD/MM/YYYY')  from BL_CASH_CTRL_RCPT_HDR_VW where (((CASH_COUNTER_CODE betwe\
en DECODE(:b0,'LOWEST','!!',:b0) and DECODE(:b2,'HIGHEST','~~',:b2) and DOC_D\
ATE between TO_DATE(:b4,'DD/MM/YYYY') and TO_DATE(:b5,'DD/MM/YYYY')) and CASH\
_COUNTER_FACILITY_ID=:b6) and ((DOC_TYPE_CODE is null  and :b7='U') or (DOC_T\
YPE_CODE is  not null  and :b7='R'))) order by CASH_COUNTER_CODE,DOC_DATE,SHI\
FT_ID            ";

 static const char *sq0007 = 
"DESC ,B.SLMT_DOC_REMARKS ,SUBSTR(B.N\
ARRATION,1,40) ,DECODE(IP_ADM_TRF_DEP_IND,'A','Admission Depoist','T','Transf\
er Deposit','O','Other Deposit','S','Deposit Against Surgery','K','Patient De\
posit Against Package','N','Not Applicable') ,B.PATIENT_ID ,B.EPISODE_TYPE ,L\
TRIM(TO_CHAR(B.EPISODE_ID,'00000009')) ,B.VISIT_ID ,Decode(Nvl(CANCELLED_IND,\
'N'),'Y','B','A') CHK ,DECODE(CANCELLED_IND,'Y','CANCEL','')  from BL_RECEIPT\
_REFUND_HDRDTL_VW B ,BL_BILL_HDR A where ((((((B.CASH_COUNTER_FACILITY_ID=:b0\
 and A.OPERATING_FACILITY_ID(+)=B.OPERATING_FACILITY_ID) and A.DOC_TYPE_CODE(\
+)=B.BILL_DOC_TYPE_CODE) and A.DOC_NUM(+)=B.BILL_DOC_NUMBER) and B.DOC_DATE b\
etween to_date(:b1,'dd/mm/yyyy') and to_date(:b2,'dd/mm/yyyy')) and B.CASH_CO\
UNTER_CODE between DECODE(:b3,'LOWEST','!!',:b3) and DECODE(:b5,'HIGHEST','~~\
',:b5)) and ((B.CASH_CTRL_DOC_TYPE_CODE is null  and :b7='U') or (B.CASH_CTRL\
_DOC_TYPE_CODE is  not null  and :b7='R'))) order by 1,2,3,6            ";
...
43,0,0,3,0,0,27,206,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/BLRCSTRX.c-arc   1.0   Apr 19 2007 17:06:00   vcm_th  $ sqlca.h 
 */
 ...
 */ 
/*
 * $Header:   T:/eHIS-102/archives/eHIS2Tier/BL/BLRCSTRX.c-arc   1.0   Apr 19 2007 17:06:00   vcm_th  $ sqlda.h 
 */
...

{
 ...
 sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='BLRCSTRX' and S\
...
,'LOWEST') ,NVL(PARAM4,'HIGHEST') ,PARAM5 ,PARAM6 into :b0,:b1,:b2,:b3,:b4,:b\
5,:b6  from SY_PROG_PARAM where ((PGM_ID='BLRCSTRX' and SESSION_ID=:b7) and P\
GM_DATE=:b8)";
    sqlstm.iters = (unsigned int  )1;
...
YY HH24:MI') ,USER into :b0,:b1,:b2  from SY_ACC_ENTITY where ACC_ENTITY_ID=:\
b3";
    sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select LONG_DESC into :b0  from BL_CASH_COUNTER where (CASH\
...
r2 ) ; IF str2 IS NOT NULL THEN :cv_patient_name := str2 ; END IF ; blcommon \
. split_words ( :d_short_name_full , 30 , '*' , 2 , str1 , str2 ) ; IF str2 I\
S NOT NULL THEN :cv_patient_name1 := str2 ; END IF ; blcommon . split_words (\
 :d_short_name_full , 30 , '*' , 3 , str1 , str2 ) ; IF str2 IS NOT NULL THEN\
 :cv_patient_name1 := :cv_patient_name1 || str2 ; end if ; END ; END ;";
sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRCSTRX.LEGEND_'||LTRIM(RTRIM(TO_CHA\
