...
static unsigned long sqlctx = 36677003;
...
"DOC_REF_DATE ,C.SLMT_DOC_REMARKS ,C.APPROVAL_REF_NO ,B.BILL_NATURE_CODE ,D.C\
ASH_SLMT_FLAG ,NVL(B.BILL_STATUS,'A') ,nvl(B.PATIENT_ID,C.PATIENT_ID) ,b.cust_\
code  from BL_CANCELLED_BOUNCED_TRX A ,BL_BILL_HDR B ,bl_receipt_refund_dtl_vw\
 c ,bl_slmt_type D where (((((((((c.operating_facility_id=:b0 and a.cancelled_\
date between TO_DATE(:b3,'DD/MM/YYYY HH24:MI:SS') and TO_DATE(NVL(:b4,TO_CHAR(\
SYSDATE,'DD/MM/YYYY HH24:MI:SS')),'DD/MM/YYYY HH24:MI:SS')) and a.cash_counter\
_code=nvl(:b1,a.cash_counter_code)) and a.added_by_id=nvl(:b2,a.added_by_id)) \
and B.DOC_TYPE_CODE(+)=C.BILL_DOC_TYPE_CODE) and B.DOC_NUM(+)=C.BILL_DOC_NUMBE\
R) and C.DOC_TYPE_CODE=A.DOC_TYPE_CODE) and C.DOC_NUMBER=A.DOC_NUMBER) and C.D\
OC_SRNO=A.DOC_SRNO) and C.SLMT_TYPE_CODE=D.SLMT_TYPE_CODE) order by 1,2,10    \
        ";
...
36,0,0,2,0,0,30,243,0,0,0,0,0,1,0,
51,0,0,3,258,0,4,251,0,0,4,3,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
82,0,0,4,170,0,4,262,0,0,4,2,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,2,9,0,0,
113,0,0,5,3858,0,9,609,0,0,15,15,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,
188,0,0,5,0,0,13,677,0,0,29,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,2,
...
319,0,0,6,72,0,4,1450,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
342,0,0,7,144,0,4,1605,0,0,2,1,0,1,0,2,3,0,0,1,9,0,0,
365,0,0,8,0,0,31,1616,0,0,0,0,0,1,0,
380,0,0,9,147,0,6,1647,0,0,3,3,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,
};
...
struct { unsigned short len; unsigned char arr[101]; } l_approval_ref_no;
...
declare_cur()
{
   ...
   /*  EXEC SQL DECLARE DAILY_SCROLL CURSOR FOR 
	...
	WHERE
		...
		A.DOC_DATE BETWEEN 		     TO_DATE (:nd_login_datetime,'DD/MM/YYYY HH24:MI:SS') AND TO_DATE(NVL(:nd_logout_datetime,TO_CHAR(SYSDATE,'DD/MM/YYYY HH24:MI:SS')),'DD/MM/YYYY HH24:MI:SS') AND
...
/*
   ...
   /* EXEC SQL DECLARE DAILY_SCROLL CURSOR FOR 
	...
	WHERE
	    ...
	    /o AND nvl(B.bill_status,'A')<>'C' o/ /oCOMMENTED FOR AMS-SCF-0596o/
	...
	and     a.added_by_id = nvl(:nd_user_id,a.added_by_id)
	AND 	B.DOC_TYPE_CODE(+)= C.BILL_DOC_TYPE_CODE  
	AND 	B.DOC_NUM (+)= C.BILL_DOC_NUMBER
	AND 	C.DOC_TYPE_CODE = A.DOC_TYPE_CODE 
	AND	    C.DOC_NUMBER   =  A.DOC_NUMBER 
	AND		C.DOC_SRNO = A.DOC_SRNO /oAdded for AMS-SCF-0345 [IN:054636]o/
...
.BILL_STATUS,'A') ,B.PATIENT_ID ,b.cust_cod");
    sqlbuft((void **)0, 
      "e  from BL_BILL_HDR B where (((((nvl(BILL_PAID_AMT,0)=0 and nvl(TOT_R\
EFUND_AMT,0)=0) and B.OPERATING_FACILITY_ID=:b0) and B.CASH_COUNTER_CODE=NVL\
(:b1,B.CASH_COUNTER_CODE)) and B.ADDED_BY_ID=NVL(:b2,B.ADDED_BY_ID)) and B.A\
DDED_DATE between TO_DATE(:b3,'DD/MM/YYYY HH24:MI:SS') and TO_DATE(NVL(:b4,T\
O_CHAR(SYSDATE,'DD/MM/YYYY HH24:MI:SS')),'DD/MM/YYYY HH24:MI:SS')) union all\
 select B.DOC_TYPE_CODE BILL_DOC_TYPE ,B.DOC_NUM BILL_DOC_NUM ,TO_CHAR(A.Can\
celled_date,'DD/MM/YYYY HH24:MI') DT_TIME ,GROSS_AMT ,SERV_DISC_AMT ,BILL_AM\
T ,(((((NVL(DEPOSIT_ADJ_AMT,0)+NVL(PREPAY_ADJ_AMT,0))+NVL(SPL_AGENCY_DEP_ADJ\
_AMT,0))+NVL(BILL_TOT_ADJUST_AMT,0))+NVL(BILL_TOT_EXEMPT_AMT,0))+NVL(BILL_TO\
T_WRITE_OFF_AMT,0)) DEP_ADJ ,BILL_TOT_AMT ,BILL_TOT_OUTST_AMT ,A.Cancelled_d\
ate DOC_DATE ,C.RECPT_REFUND_IND ,C.DOC_TYPE_CODE RECPT_DOC_TYPE_CODE ,C.DOC\
_NUMBER RECPT_DOC_NUM ,C.RECPT_NATURE_CODE ,C.CUSTOMER_CODE ,A.CASH_COUNTER_\
FACILITY_ID ,A.CASH_COUNTER_CODE ,A.SHIFT_ID ,((-1)* C.DOC_AMT) doc_amt ,C.S\
LMT_TYPE_CODE ,C.SLMT_DOC_REF_DESC ,C.SLMT_");
    sqlstm.stmt = sq0005;
    sqlstm.iters = (unsigned int  )1;
		    ...
		    if (strcmp(l_bill_status.arr,"C")!=0)
			{
				...
				{ 
					...
					l_credit++;
						  ...
						  { 
							  if (strcmp(l_bill_status.arr,"C")!=0) /* COMMENTED FOR AMS-SCF-0596 ADDED BY DHANANJAY */
								{
							  l_due++;
							     } /* COMMENTED FOR AMS-SCF-0596 ADDED BY DHANANJAY  */
							  print_formated(l_bill_tot_outst_amt);
						  }
						  else
						  {
