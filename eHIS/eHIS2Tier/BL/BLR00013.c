...
{
    37,
    "C:\\temp\\IN7840\\New Folder\\BLR00013.PC"
};


static unsigned long sqlctx = 1049836989;
...
a.doc_date,'DD/MM/YYYY HH24:MI:SS') ,a.patient_id ,b.short_name ,c.short_desc \
,a.slmt_doc_ref_desc ,TO_CHAR(a.slmt_doc_ref_date,'DD/MM/YYYY') ,a.doc_amt ,a.\
ext_acct_code ,a.narration ,TO_CHAR(a.doc_date,'DD/MM/YYYY')  from bl_receipt_\
refund_HDRdtl_VW a ,mp_patient_mast b ,bl_slmt_type c where ((((((((b.patient_\
id(+)=a.patient_id and c.slmt_type_code=a.slmt_type_code) and a.operating_faci\
lity_id=:b0) and a.doc_date>=TO_DATE(:b1,'DD/MM/YYYY HH24:MI:SS')) and a.doc_d\
ate<=TO_DATE(:b2,'DD/MM/YYYY HH24:MI:SS')) and recpt_refund_ind='R') and NVL(c\
ancelled_ind,'N')<>'Y') and recpt_nature_code='MI') and a.SLMT_TYPE_CODE in (s\
elect SLMT_TYPE_CODE  from BL_SLMT_TYPE where nvl(CASH_SLMT_FLAG,'$')<>DECODE(\
:b3,'Y','A','#'))) order by a.doc_date,a.doc_type_code,a.doc_number           \
 ";
...
51,0,0,3,0,0,32,142,0,0,0,0,0,1,0,
66,0,0,4,193,0,4,162,0,0,6,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,
0,0,
105,0,0,5,91,0,2,183,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
128,0,0,6,0,0,32,193,0,0,0,0,0,1,0,
143,0,0,7,91,0,2,203,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
166,0,0,8,96,0,4,217,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
189,0,0,9,162,0,6,224,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
220,0,0,10,198,0,4,248,0,0,6,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,
9,0,0,
259,0,0,11,0,0,32,271,0,0,0,0,0,1,0,
274,0,0,12,857,0,9,313,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
305,0,0,13,0,0,32,318,0,0,0,0,0,1,0,
320,0,0,12,0,0,13,340,0,0,11,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,4,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
379,0,0,14,0,0,32,378,0,0,0,0,0,1,0,
394,0,0,12,0,0,15,387,0,0,0,0,0,1,0,
409,0,0,15,40,0,4,417,0,0,2,1,0,1,0,1,3,0,0,2,97,0,0,
432,0,0,16,44,0,4,433,0,0,2,1,0,1,0,1,9,0,0,2,9,0,0,
455,0,0,17,42,0,4,442,0,0,2,1,0,1,0,1,9,0,0,2,9,0,0,
478,0,0,18,73,0,4,525,0,0,5,4,0,1,0,1,97,0,0,1,9,0,0,1,97,0,0,1,9,0,0,2,9,0,0,
};


/*************************************************************
Author    :  Harish A   02/03/2004
Name      :  List of Misc. Receipts between specified dates
Project   :  ML
*************************************************************/
...
struct { unsigned short len; unsigned char arr[10]; } l_only_date;

struct { unsigned short len; unsigned char arr[30]; } l_prev_doc_date;
...
struct { unsigned short len; unsigned char arr[60]; } l_narration;
...
struct { unsigned short len; unsigned char arr[30]; } l_substr_narration;
...
char *argv[];
{
	...
	if(argc < 5) 
	{
		disp_message(ERR_MESG,"Usage BLR00013 uid/passwd session_id pgm_date facility_id");
		proc_exit();
	}
    ...
    strcpy(g_pgm_id,"BLR00013");
    strcpy(g_pgm_date,argv[3]);

	strcpy(filename,WORKING_DIR);
	strcat(filename,"BLR00013.lis");
...
00013' and SESSION_ID=TO_NUMBER(:b4)) and PGM_DATE=:b5)";
 sqlstm.iters = (unsigned int  )1;
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='BLR00013' and\
...
fetch_legend_value()
{
	for(i=1;i<=50;i++)
	{
		...
		/* EXEC SQL SELECT LTRIM(RTRIM('BLR00013.LEGEND_'||LTRIM(RTRIM(TO_CHAR(:i,'009')))))
		           INTO :l_pk_value
				   FROM dual; */ 

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00013.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
...
declare_cursor()
{
	...
	/* EXEC SQL DECLARE receipt_list_cur
	          CURSOR
			     FOR
...

{
 ...
 sqlstm.sqadto[2] = (unsigned short )0;
 sqlstm.sqtdso[2] = (unsigned short )0;
 sqlstm.sqhstv[3] = (         void  *)&l_patient_name;
 ...
 sqlstm.sqadto[3] = (unsigned short )0;
 sqlstm.sqtdso[3] = (unsigned short )0;
 sqlstm.sqhstv[4] = (         void  *)&l_slmt_desc;
 ...
 sqlstm.sqadto[4] = (unsigned short )0;
 sqlstm.sqtdso[4] = (unsigned short )0;
 sqlstm.sqhstv[5] = (         void  *)&l_slmt_doc_ref_desc;
 ...
 sqlstm.sqadto[5] = (unsigned short )0;
 sqlstm.sqtdso[5] = (unsigned short )0;
 sqlstm.sqhstv[6] = (         void  *)&l_slmt_doc_ref_date;
 ...
 sqlstm.sqhstv[7] = (         void  *)&l_doc_amt;
 ...
 sqlstm.sqhstv[9] = (         void  *)&l_narration;
 sqlstm.sqhstl[9] = (unsigned int  )62;
 ...
 sqlstm.sqhstv[10] = (         void  *)&l_only_date;
 ...
 if (sqlca.sqlcode < 0) goto err_exit;
	...
	l_narration.arr[l_narration.len]				= '\0';
	l_only_date.arr[l_only_date.len]				= '\0';
...
void do_report()  
{
	...
	{
		if((strlen(l_prev_doc_date.arr)!=0)&&(strcmp(l_prev_doc_date.arr,l_only_date.arr)!=0))
		{
			print_day_total();
		}
		...
		l_prev_doc_date.arr[0]='\0';
		strcpy(l_prev_doc_date.arr,l_only_date.arr);
		l_prev_doc_date.len = strlen(l_prev_doc_date.arr);
	}
	print_report_total();
	end_report();
}
...

{
 ...
 sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
	...
	fprintf(f1,"%-7.7s  %-18.18s %-19.19s  %-20.20s  %-40.40s  %-15.15s  %-20.20s  %-10.10s ",l_serial_no_char, l_receipt_no.arr,l_doc_date.arr,l_patient_id.arr,l_patient_name.arr,l_slmt_desc.arr,l_slmt_doc_ref_desc.arr,l_slmt_doc_ref_date.arr);
	print_formated(l_doc_amt);
	
	if(strlen(l_narration.arr)<=40)
	{
		fprintf(f1,"%-11.11s %-41.41s\n",l_ext_acct_code.arr,l_narration.arr);
		line_no = line_no+1;	
	}
	else if(strlen(l_narration.arr)>40)
	{
		l_substr_narration.arr[0]='\0';

		/* EXEC SQL SELECT SUBSTR(:l_narration,1,40)
		           INTO :l_substr_narration
				   FROM dual; */ 

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select SUBSTR(:b0,1,40) into :b1  from dual ";
  sqlstm.iters = (unsigned int  )1;
  ...
  sqlstm.sqlety = (unsigned short)256;
  sqlstm.occurs = (unsigned int  )0;
  sqlstm.sqhstv[0] = (         void  *)&l_narration;
  ...
  sqlstm.sqadto[0] = (unsigned short )0;
  sqlstm.sqtdso[0] = (unsigned short )0;
  sqlstm.sqhstv[1] = (         void  *)&l_substr_narration;
  sqlstm.sqhstl[1] = (unsigned int  )32;
  sqlstm.sqhsts[1] = (         int  )0;
  sqlstm.sqindv[1] = (         void  *)0;
  sqlstm.sqinds[1] = (         int  )0;
  ...
  sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
		...
		l_substr_narration.arr[l_substr_narration.len]='\0';

		fprintf(f1,"%-11.11s %-41.41s\n",l_ext_acct_code.arr,l_substr_narration.arr);

		l_substr_narration.arr[0]='\0';

		/* EXEC SQL SELECT SUBSTR(:l_narration,41)
		           INTO :l_substr_narration
				   FROM dual; */ 

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select SUBSTR(:b0,41) into :b1  from dual ";
  sqlstm.iters = (unsigned int  )1;
  ...
  sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
		...
		fprintf(f1,"%-41.41s\n",l_substr_narration.arr);
		line_no = line_no+2;	
	}
	l_day_total_doc_amt = l_day_total_doc_amt + l_doc_amt;
...
print_report_total()
{
	...
	fprintf(f1,"---------------\n");
	print_space(139);
	fprintf(f1,"%19.19s :",loc_legend[21]);
...
print_day_total()
{
	...
	fprintf(f1,"---------------\n");
	print_space(128);
	fprintf(f1,"%19.19s %10.10s :",loc_legend[20],l_prev_doc_date.arr);
	print_formated(l_day_total_doc_amt);
	fprintf(f1,"\n");
	print_space(160);
	fprintf(f1,"---------------\n");
	l_day_total_doc_amt=0;
...
print_page_title()
{
	...
	l_comp_name_end_point=l_start_point+(strlen(d_user.arr)+16)+strlen(loc_legend[2]);
	...
	/* EXEC SQL SELECT :loc_legend[6]||' '||:l_from_recpt_date||' '||:loc_legend[7]||' '||:l_to_recpt_date
...
4  from dual ";
 ...
 sqlstm.sqlety = (unsigned short)256;
 sqlstm.occurs = (unsigned int  )0;
 sqlstm.sqhstv[0] = (         void  *)loc_legend[6];
 ...
 sqlstm.sqadto[1] = (unsigned short )0;
 sqlstm.sqtdso[1] = (unsigned short )0;
 sqlstm.sqhstv[2] = (         void  *)loc_legend[7];
 ...
 sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
	...
	fprintf(f1,"%-14.14s: BLR00013",loc_legend[4]);
...
print_page_header()
{
	print_line(225);
	fprintf(f1,"\n");
	fprintf(f1,"%-7.7s  %-15.15s %-19.19s  %-20.20s  %-40.40s  %-15.15s  %-20.20s  %-10.10s  %14.14s  %-11.11s %-25.25s\n",loc_legend[8],loc_legend[9],loc_legend[11],loc_legend[12],loc_legend[13],loc_legend[14],loc_legend[15],loc_legend[16],loc_legend[17],loc_legend[18],loc_legend[19]);	
	print_line(225);
	fprintf(f1,"\n");
	line_no = line_no+3;
}
