...
{
    21,
    "C:\\10xdev\\Blrblreg.pc"
};


static unsigned long sqlctx = 139597923;
...
"CODE(:b14,'C','C','A',NVL(BILL_STATU\
S,'X')) or (NVL(BILL_STATUS,'X')=DECODE(:b14,'V',NVL(BILL_STATUS,'X')) and NV\
L(BILL_STATUS,'X')<>'C'))) and NVL(BILL_STATUS,'X')<>'S') and CASH_COUNTER_CO\
DE between NVL(ltrim(rtrim(substr(:b16,1,2))),'  ') and NVL(ltrim(rtrim(subst\
r(:b17,1,2))),'~~')) and OPERATING_FACILITY_ID between NVL(ltrim(rtrim(substr\
(:b18,1,2))),'  ') and NVL(ltrim(rtrim(substr(:b19,1,2))),'~~')) order by OPE\
RATING_FACILITY_ID,CASH_COUNTER_CODE,EPISODE_TYPE,PATIENT_ID,TRUNC(DOC_DATE),\
DOC_TYPE_CODE,DOC_NUM            ";

 static const char *sq0006 = 
"im(rtrim(substr(:b17,1,2))),'~~')) a\
nd OPERATING_FACILITY_ID between NVL(ltrim(rtrim(substr(:b18,1,2))),'  ') and\
 NVL(ltrim(rtrim(substr(:b19,1,2))),'~~')) group by OPERATING_FACILITY_ID,CAS\
H_COUNTER_CODE,DOC_TYPE_CODE,BILL_NATURE_CODE,NVL(BILL_STATUS,'X')           ";
...
{10,4274,0,0,0,
5,0,0,1,0,0,27,197,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...
AM4 ,PARAM5 ,PARAM6 ,PARAM7 ,PARAM2 ,PARAM3 ,PARAM8 ,PARAM9 ,PARAM10 ,PARAM11\
 ,PARAM12 ,PARAM13 ,PARAM14 ,PARAM15 ,PARAM16 ,PARAM17 ,PARAM18 into :b0,:b1,\
:b2,:b3,:b4,:b5,:b6,:b7,:b8,:b9,:b10,:b11,:b12,:b13,:b14,:b15,:b16,:b17,:b18,\
:b19,:b20  from SY_PROG_PARAM where ((PGM_ID=:b21 and SESSION_ID=:b22) and PG\
M_DATE=:b23)";
   sqlstm.iters = (unsigned int  )1;
...
S NOT NULL THEN :bl_patient_name1 := str2 ; END IF ; blcommon . split_words (\
...
time_2t ( to_date ( :nd_temp_date , 'DD/MM/RRRR HH24:MI' ) , :p_language_id )\
 ; END ;";
 sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "declare t_date date ; BEGIN :nd_loc_date := sm_convert_date\
_2t ( to_date ( :nd_temp_date , 'DD-MON-rr' ) , :p_language_id ) ; :nd_loc_da\
te := to_char ( to_date ( :nd_temp_date , 'DD-MON-rr' ) , 'DD-MON-rr' ) ; END\
 ;";
 sqlstm.iters = (unsigned int  )1;
