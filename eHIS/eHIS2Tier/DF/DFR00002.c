...
{
    38,
    "T:\\BL\\CRF-0120\\DF\\Check-In\\DFR00002.pc"
};


static unsigned long sqlctx = 1178026453;
...
") and TO_DATE(((A.PROCESSING_MONTH||\
'/')||A.PROCESSING_YEAR),'MM/YYYY') between to_date(:b6,'MM/YYYY') and TO_DAT\
E(:b7,'MM/YYYY')) and A.REVENUE_CATEGORY between NVL(:b8,'!!!!!!!!!!') and NV\
L(:b9,'~~~~~~~~~~')) and A.BLNG_GRP_ID between NVL(:b10,'!!') and NVL(:b11,'~\
~')) and NVL(A.REVENUE_CUST_CODE,'X') between NVL(:b12,'!!!!!!!!') and NVL(:b\
13,'~~~~~~~~')) and A.EPISODE_TYPE=DECODE(:b2,'A',A.EPISODE_TYPE,:b2)) group \
by A.PHYSICIAN_ID,B.SHORT_NAME,DECODE(:b0,'D',(((A.PHYSICIAN_ID||TO_CHAR(A.PR\
OCESSING_YEAR))||'/')||LPAD(TO_CHAR(A.PROCESSING_MONTH),2,'0')),((((TO_CHAR(A\
.PROCESSING_YEAR)||'/')||LPAD(TO_CHAR(A.PROCESSING_MONTH),2,'0'))||'/')||A.PH\
YSICIAN_ID)),((A.PROCESSING_MONTH||'/')||A.PROCESSING_YEAR),A.REVENUE_CATEGOR\
Y,C.SHORT_DESC,DECODE(:b1,'Y',DECODE(A.EPISODE_TYPE,'I','Inpatient','D','Dayc\
are','O','Outpatient','E','Emergency','R','External'),DECODE(:b2,'I','Inpatie\
nt','D','Daycare','O','Outpatient','E','Emergency','R','External','A','   All\
')) order by 3            ";
...
43,0,0,3,0,0,27,193,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...

{
 ...
 sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='DFR00002' and S\
...
ARAM7 ,PARAM8 ,PARAM9 ,PARAM10 ,PARAM11 ,PARAM12 ,PARAM13 ,PARAM14 ,PARAM15 i\
nto :b0,:b1,:b2,:b3,:b4,:b5,:b6,:b7,:b8,:b9,:b10,:b11,:b12,:b13,:b14  from SY\
_PROG_PARAM where ((PGM_ID='DFR00002' and SESSION_ID=:b15) and PGM_DATE=:b16)";
    sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('DFR00002.LEGEND_'||LTRIM(RTRIM(TO_CHA\
