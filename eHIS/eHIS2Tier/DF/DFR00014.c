...
{
    38,
    "T:\\BL\\CRF-0120\\DF\\Check-In\\DFR00014.pc"
};


static unsigned long sqlctx = 1178026421;
...
"der by ((((A.TRX_DOC_REF||'/')||A.TR\
X_DOC_REF_LINE_NUM)||'/')||A.TRX_DOC_REF_SEQ_NUM)            ";

 static const char *sq0005 = 
"select ((PROCESSING_MONTH||'/')||PROCESSING_YEAR) ,TRX_DOC_REF_SUBSEQ_NUM ,\
PHYSICIAN_ID ,GROSS_AMT ,DISCOUNT_AMT ,NET_REVENUE_AMT ,DR_REVENUE_AMT ,CONTR\
ACT_REF ,((to_char(CONTRACT_PERIOD_FROM,'DD/MM/YYYY')||' - ')||to_char(CONTRA\
CT_PERIOD_TO,'DD/MM/YYYY')) contract_period ,SERV_QTY  from DF_REVENUE_CHARGE\
S_FOLIO where (OPERATING_FACILITY_ID=:b0 and ((((TRX_DOC_REF||'/')||TRX_DOC_R\
EF_LINE_NUM)||'/')||TRX_DOC_REF_SEQ_NUM)=:b1) order by 1,2            ";
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('DFR00014.LEGEND_'||LTRIM(RTRIM(TO_CHA\
