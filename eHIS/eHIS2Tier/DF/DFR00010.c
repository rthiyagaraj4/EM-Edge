...
{
    39,
    "T:\\BL\\CRF-0120\\DF\\check-out\\DFR00010.pc"
};


static unsigned long sqlctx = 1939086891;
...
"select a.processing_month ,a.processing_year ,a.physician_id ,b.full_name ,\
a.contract_ref ,to_char(a.contract_period_from,'dd/mm/yyyy') ,to_char(a.contr\
act_period_to,'dd/mm/yyyy') ,a.revenue_amt_ytlm ,a.revenue_amt_cur_month ,a.s\
ettled_amt_ytlm ,a.settled_amt_cur_month ,a.pmnt_amt_ytlm_paid ,a.payable_amt\
_cur_month ,a.pmnt_amt_cur_month_paid ,a.pmnt_doc_type_code ,a.pmnt_doc_numbe\
r ,a.pmnt_doc_line_num ,to_char(a.pmnt_doc_date,'dd/mm/yyyy')  from df_monthl\
y_pmnt_by_contract a ,sy_physician_mast b where ((((a.operating_facility_id=:\
b0 and (a.processing_year||TO_CHAR(a.processing_month,'FM00')) between (:b1||\
TO_CHAR(:b2,'FM00')) and (:b3||TO_CHAR(:b4,'FM00'))) and a.physician_id betwe\
en NVL(:b5,'!!!!!!!!!!!!!!!') and NVL(:b6,'~~~~~~~~~~~~~~~')) and b.facility_\
id=a.operating_facility_id) and a.physician_id=b.physician_id) order by a.pro\
cessing_year,a.processing_month,a.physician_id,a.contract_ref,a.contract_peri\
od_from            ";
...
36,0,0,2,941,0,9,413,0,0,7,7,0,1,0,1,9,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,9,
0,0,1,9,0,0,
79,0,0,2,0,0,13,449,0,0,18,0,0,1,0,2,3,0,0,2,3,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
0,0,2,9,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,
166,0,0,2,0,0,15,487,0,0,0,0,0,1,0,
181,0,0,3,203,0,4,553,0,0,9,3,0,1,0,2,3,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,9,0,0,2,
9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
232,0,0,4,84,0,2,575,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
259,0,0,5,96,0,4,590,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
282,0,0,6,165,0,6,598,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
313,0,0,7,120,0,4,731,0,0,4,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,
};
...
ORDER BY a.processing_year, a.processing_month, a.physician_id, a.contract_ref, a.contract_period_from; */ 
...
print_rec()
{
	...
	{	
		if((temp_month != d_month) || (temp_year != d_year))
		{
			if(first != 0)
			{
				check_line(6);
			...
			fprintf(fp,"%s : %d/%d",loc_legend[24], d_month, d_year);
			next_line(1);
			next_line(1);
		...
		fprintf(fp,"%-20.20s %-15.15s %-15.15s ", d_contract_ref.arr,
								d_contract_from.arr, d_contract_to.arr);
		...
		m_share_rev_cur_tot += d_share_rev_cur_amt;
		m_settle_cur_tot	+= d_settle_cur_amt;
		m_payable_cur_tot	+= d_payable_cur_amt;
		m_paid_cur_tot		+= d_paid_cur_amt;
...
dr_change()
{
		...
		fprintf(fp,"%s  %s", d_doctor_id.arr, d_doctor_name.arr);
		next_line(1);
		next_line(1);
...
print_dr_total()
{
	...
	horz_skip(53);
	print_line('-',REP_WIDTH-53);
	next_line(1);

	fprintf(fp,"%50.50s %18s",loc_legend[25]," ");
...
print_month_total()
{
	...
	fprintf(fp,"%50.50s %18s",loc_legend[26]," ");

	print_formated(m_share_rev_cur_tot);
	horz_skip(16);
	print_formated(m_settle_cur_tot);
	horz_skip(16);
	print_formated(m_payable_cur_tot);
	print_formated(m_paid_cur_tot);
	next_line(1);
...
print_grand_total()
{
	...
	fprintf(fp,"%50.50s %18s",loc_legend[27]," ");
...
print_first_page()
{
	...
	fprintf(fp,"%30.30s %10.10s : %-d/%-d\n",loc_legend[10],loc_legend[8],nd_from_month,nd_from_year);
	fprintf(fp,"%41.41s : %-d/%-d\n\n",loc_legend[9],nd_to_month,nd_to_year);
...
print_column_title()
{
      fprintf(fp,"%-15.15s %-30.30s %5s %-31.31s %-31.31s %-15.15s %-15.15s %-15.15s %-30.30s",
	  loc_legend[6],loc_legend[7]," ",loc_legend[14],loc_legend[15],loc_legend[16],loc_legend[17],loc_legend[16], loc_legend[18]);

      next_line(1);

      fprintf(fp, "%-20.20s %-15.15s %-15.15s %15.15s %15.15s %15.15s %15.15s %15.15s %15.15s %15.15s %-20.20s %-10.10s",
	  loc_legend[11],loc_legend[12],loc_legend[13],loc_legend[19],loc_legend[20],loc_legend[19],loc_legend[20],loc_legend[19],loc_legend[20],loc_legend[20],loc_legend[21],loc_legend[22]);
...
open_files()
{
	...
	strcat(filename,"DFR00010.lis");

	if ((fp = fopen(filename,"w")) == NULL)
	{
		disp_message(ERR_MESG,"Error in opening file DFR00010.lis");
		proc_exit();
	}
}
...
get_sy_prog_param()
 {
    ...
    /* EXEC SQL SELECT	TO_NUMBER(PARAM1),TO_NUMBER(PARAM2),
			TO_NUMBER(PARAM3),TO_NUMBER(PARAM4),PARAM5,PARAM6
	 INTO	:nd_from_month, 
			:nd_to_month,
			:nd_from_year, 
			:nd_to_year,
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select TO_NUMBER(PARAM1) ,TO_NUMBER(PARAM2) ,TO_NUMBER(P\
ARAM3) ,TO_NUMBER(PARAM4) ,PARAM5 ,PARAM6 into :b0,:b1,:b2,:b3,:b4,:b5  from \
SY_PROG_PARAM where ((PGM_ID=:b6 and SESSION_ID=:b7) and PGM_DATE=:b8)";
    sqlstm.iters = (unsigned int  )1;
    ...
    sqlstm.sqadto[5] = (unsigned short )0;
    sqlstm.sqtdso[5] = (unsigned short )0;
    sqlstm.sqhstv[6] = (         void  *)&nd_curr_pgm_id;
...
fetch_legend_value()
{
	for(i=1;i<=300;i++)
	{
		...
		/* EXEC SQL SELECT LTRIM(RTRIM('DFR00010.LEGEND_'||
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('DFR00010.LEGEND_'||LTRIM(RTRIM(TO_CHA\
