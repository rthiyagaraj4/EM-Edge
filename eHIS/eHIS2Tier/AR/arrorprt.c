...
{
    38,
    "T:\\BL\\CRF-0120\\AR\\WIP\\Proc\\arrorprt.pc"
};


static unsigned long sqlctx = 1150830245;
...
Y') ,NARRATION ,ROWID  from AR_CASHIER where (((((((DOC_TYPE_CODE>=NVL(:b0,'  \
    ') and DOC_TYPE_CODE<=NVL(:b1,'~~~~~~')) and DOC_NUM>=NVL(:b2,0)) and DOC_\
NUM<=NVL(:b3,99999999)) and NVL(DOC_DATE,SYSDATE)>=NVL(TO_DATE(:b4,'DD/MM/YYYY\
'),to_date('00010101','YYYYDDMM'))) and NVL(DOC_DATE,SYSDATE)<=NVL(TO_DATE(:b5\
,'DD/MM/YYYY'),to_date('47123112','YYYYDDMM'))) and NVL(POSTED_FLAG,'N')='N') \
...
{10,4274,0,0,0,
5,0,0,1,193,0,4,206,0,0,9,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
0,0,2,9,0,0,1,9,0,0,1,9,0,0,
56,0,0,2,143,0,4,261,0,0,3,2,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,
83,0,0,3,91,0,2,282,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
106,0,0,4,0,0,30,297,0,0,0,0,0,1,0,
121,0,0,5,0,0,27,303,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
152,0,0,6,181,0,4,327,0,0,7,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
3,0,0,1,9,0,0,
195,0,0,7,615,0,9,400,0,0,6,6,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,
234,0,0,7,0,0,13,445,0,0,11,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
4,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
293,0,0,8,118,0,9,484,0,0,1,1,0,1,0,1,9,0,0,
312,0,0,8,0,0,13,492,0,0,1,0,0,1,0,2,9,0,0,
331,0,0,9,79,0,5,590,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
354,0,0,10,0,0,29,599,0,0,0,0,0,1,0,
};
...
PE_CODE=:b1 and DOC_NUM=:b2) and NVL(PRINT_FLAG,'N')='N') for update of PRINT_\
FLAG nowait ";
        sqlstm.iters = (unsigned int  )1;
