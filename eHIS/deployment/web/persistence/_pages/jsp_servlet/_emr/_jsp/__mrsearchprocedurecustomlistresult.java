...
public final class __mrsearchprocedurecustomlistresult extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/emr/jsp/MRSearchProcedureCustomListResult.jsp", 1722863946800L ,"10.3.6.0","Asia/Calcutta")) return true;
...
//if(cause_indicator==null)
 ...
 try
 {
	...
	String sql1 = "select count(*) from mr_term_group_dtl a,mr_term_code b where a.term_group_id=?  and a.term_set_id =? and a.term_set_id = b.term_set_id  and a.term_code = b.term_code";
	...
	sql.append("select a.term_code diag_code , b.short_desc diag_desc from mr_term_group_dtl a,mr_term_code b where a.term_group_id=?  and a.term_set_id =? and a.term_set_id = b.term_set_id  and a.term_code = b.term_code ");
...
if ( !(start <= 1) )
    out.println("<A HREF='../../eMR/jsp/SearchProcedureCustomListResult.jsp?from="+(start-14)+"&to="+(end-14)+"&p_diag_code="+diagcode+"&p_diag_scheme_desc="+diagdesc+"&Diag_code="+Diag_Code+"&term_set_id="+term_set_id+"' text-decoration='none'>Prev</A>");

if ( !( (start+14) > maxRecord ) )
    out.println("<A HREF='../../eMR/jsp/SearchProcedureCustomListResult.jsp?from="+(start+14)+"&to="+(end+14)+"&p_diag_code="+diagcode+"&p_diag_scheme_desc="+diagdesc+"&Diag_code="+Diag_Code+"&term_set_id="+term_set_id+"'  text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
   ...
   if ( start != 1 )
	...
	while (rset.next()&& i<=end )
	{
		...
		code_desc_param=Diag_code+"/"+Diag_desc;
		code_desc_param=code_desc_param.replace('>','`');
		code_desc_param=code_desc_param.replace(' ','%');
		out.println("<tr style='background-color:;' ><td  class='"+rowclass+"'>");
		out.println("<a   href=javascript:closewd('"+code_desc_param+"')>");		
