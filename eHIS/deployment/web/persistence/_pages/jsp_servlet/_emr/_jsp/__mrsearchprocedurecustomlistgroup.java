...
public final class __mrsearchprocedurecustomlistgroup extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/emr/jsp/MRSearchProcedureCustomListGroup.jsp", 1709118978282L ,"10.3.6.0","Asia/Calcutta")) return true;
...
try
{
  ...
  //String sql="SELECT diag_group_code, diag_group_desc FROM mr_diag_group WHERE  GROUP_TYPE='P' and diag_code_scheme=4 and  (practitioner_id = ? or practitioner_id is null) and (speciality_code is null or speciality_code = (select SPECIALTY_CODE from pr_encounter where facility_id=? and encounter_id=?))"; 
	...
	String sql="select a.term_group_id diag_group_code, a.term_group_desc  diag_group_desc, a.term_set_id from mr_term_group_hdr a, mr_term_set b,mr_term_set_for_facility c where a.term_set_id = b.term_set_id and a.eff_status = 'E' and b.proc_spec_yn = 'Y' and b.term_set_id = c.term_set_id and c.facility_id = '"+facility_id+"'";
...
if (rset!=null)
{
	while (rset.next()) {
		...
		term_set_id=rset.getString("term_set_id");
          ...
          out.println("<li><a  href=\"MRSearchProcedureCustomListResult.jsp?Diag_code="+Diag_groupcode+"&Diag_desc="+java.net.URLEncoder.encode(Diag_groupdesc)+"&p_diag_code="+diagcode+"&p_diag_scheme_desc="+java.net.URLEncoder.encode(diagdesc)+
			  "&cause_indicator="+cause_indicator+"&term_set_id="+term_set_id+"\"onClick=\"changeRowColor(tbl_row"+rowCount+")\" target='code_description'>");
