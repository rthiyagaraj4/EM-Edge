...
public final class __annualcensusofbedqueryresult extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/emr/jsp/AnnualCensusofBedQueryResult.jsp", 1722236085605L ,"10.3.6.0","Asia/Calcutta")) return true;
        return false;
    }
...
public static String checkForNull(String inputString, String defaultValue)
{
 ...
 String sStyle = checkForNull((String)session.getAttribute("PREFERRED_STYLE"), "IeStyle.css"); 
	...
	try
	{
		...
		String CountSql=" SELECT COUNT(*) TOTAL FROM MR_ANNUAL_CENSUS_OF_BED A " + sql ;
		...
		//Below query modified (added Reportid in where condtion) for this CRF PER-PD-209 
		String FinalSql=" select  to_char(A.census_year,'YYYY') CENSUS_YEAR,(SELECT group_desc from mr_report_grouping where group_code = A.GROUP_CODE and report_id =  'MRIPCSBD')GROUP_CODE,(SELECT group_desc from mr_report_grouping where group_code = A.SUB_GROUP_CODE and report_id =  'MRIPCSBD')SUB_GROUP_CODE, A.BED_CLASS_GROUP_CODE,A.COMM_BED_CNT,A.MALE_BED_CNT,A.FEMALE_BED_CNT,A.CHILD_BED_CNT,(SELECT group_desc from mr_report_grouping where group_code = a.BED_CLASS_GROUP_CODE and report_id =  'MRIPCSBD')bed_desc from MR_ANNUAL_CENSUS_OF_BED A " + sql ;
		...
		rs = stmt.executeQuery(FinalSql);
            ...
            _bw.write(_wl_block4Bytes, _wl_block4);
				...
				if ( !(start <= 1) )
					out.println("<A HREF='../../eMR/jsp/AnnualCensusofBedQueryResult.jsp?from="+(start-14)+"&to="+(end-14)+"&whereclause="+URLEncoder.encode(sql)+"'"+" text-decoration='none'>Previous</A>");
				if ( !( (start+14) > maxRecord ) )
					out.println("<A HREF='../../eMR/jsp/AnnualCensusofBedQueryResult.jsp?from="+(start+14)+"&to="+(end+14)+"&whereclause="+URLEncoder.encode(sql)+"'"+" text-decoration='none'>Next</A>");
