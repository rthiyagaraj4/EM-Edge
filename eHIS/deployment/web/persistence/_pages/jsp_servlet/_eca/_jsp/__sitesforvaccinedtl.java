...
public final class __sitesforvaccinedtl extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eca/jsp/SitesForVaccineDtl.jsp", 1721649538817L ,"10.3.6.0","Asia/Calcutta")) return true;
    ...
    private final static java.lang.String  _wl_block26 ="\" id=\"siteName";
...
response.addHeader("X-Content-Type-Options", "nosniff"); //MMS-ME-SCF-0085
	...
	eCA.SitesForVaccineBean sitesbean = (eCA.SitesForVaccineBean)getObjectFromBean("sitesbean","eCA.SitesForVaccineBean",session);
		...
		try
		{	sitesbean.clearArray(retArray);			
			con = ConnectionManager.getConnection(request);			
			maxRecord = sitesbean.getMaxRecord(vaccineId,searchText,con);
	...
	finalString = sitesbean.getConcatenatedString();
			if(finalString==null || finalString.equals(""))
			{
				finalString = "";
			}
		retArray = sitesbean.getQueryForSite(vaccineId,searchText,con,start,end);				
		if(retArray != null)
			{
			  if( retArray.size()!=0)
				{
					...
					//for(int k=0;k<arrayDesc.size();k++){
						//out.println("arrayDesc   "+arrayDesc.get(k));
					//}
					
					siteId  = (ArrayList)retArray.get(1);
					default_YN = (ArrayList)retArray.get(2);
					flag = (ArrayList)retArray.get(3);
					if(arrayDesc.size()==0 || siteId.size()==0|| flag.size()==0)
					{
						
						error_msg ="NO_RECORD_FOUND_FOR_CRITERIA";
						
						out.println("<script>alert(getMessage('"+error_msg+"','Common'));</script>");
						out.println("<script>parent.SitesForVaccineSearchFrame.document.sitesForVaccineSearchForm.searchtext.value=''</script>");
		...
		if ( !(start <= 1) )
			out.println("<A class='gridLink'  href='../../eCA/jsp/SitesForVaccineDtl.jsp?from="+(start-6)+"&siteDesc="+java.net.URLEncoder.encode(searchText)+"&vaccineId="+vaccineId+"&mode=modify"+"&to="+(end-6)+"'text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A>");
		if ( !( (start+6) > maxRecord ) )
			
			out.println("&nbsp;<A class='gridLink'  href='../../eCA/jsp/SitesForVaccineDtl.jsp?from="+(start+6)+"&vaccineId="+vaccineId+"&siteDesc="+java.net.URLEncoder.encode(searchText)+"&mode=modify"+"&to="+(end+6)+"'text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
					...
					for(rowCount=0;rowCount < arrayDesc.size();rowCount++)
					{
							...
							{								
								...
								else
								{
									...
									int descLength = sitesbean.getLength();
							...
							else
							{
								...
								for(index = 0;index < arrayCodeCmp.size();index++)
								{
									...
									siteName = (String)siteId.get(rowCount);								
									if(siteName.equalsIgnoreCase(tempString))
									{
            ...
            out.print( String.valueOf((String)siteId.get(rowCount)));
		...
		{
			//out.println("Exception in Sites For Vaccine List Block: :"+e.toString());//COMMON-ICN-0181
