...
public final class __specimenfororderablevalidate extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eor/jsp/SpecimenForOrderableValidate.jsp", 1732512598000L ,"10.3.6.0","Asia/Calcutta")) return true;
...
//	String class_name   = (String)hash.get("CLASS_NAME");
	...
	//count_code		=	stoken1.countTokens();
	}
	for(int j=fm_value; j<=to_value; j++){
		...
		if(tab_value.equals("S"))
		{
			if(update_value1!="")
			{
			multiRecSet.setBeanValue(j,update_value,update_value1);  //alteredRecord		=	multiRecSet.setBeanValue(j,update_value);
			}
			else
			{
		...
		else if(tab_value.equals("A"))
		{
			if(update_value1!="")
			{
			multiRecSet.setDBBeanValue(j,update_value,update_value1); //alteredRecord		=	multiRecSet.setDBBeanValue(j,update_value);
			}
			else
			{
