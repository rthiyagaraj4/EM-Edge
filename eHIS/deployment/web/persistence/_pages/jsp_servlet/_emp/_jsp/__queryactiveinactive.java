...
public final class __queryactiveinactive extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/emp/jsp/queryActiveInactive.jsp", 1709118699090L ,"10.3.6.0","Asia/Calcutta")) return true;
...
//	sql = "";
	...
	String reason_for_active_yn = request.getParameter("reason_for_active_yn");
...
//			sql = "where long_desc like '"+longdesc+"%'" ;
	...
	if ( !(reason_for_active_yn == null || reason_for_active_yn.equals("") || reason_for_active_yn.equals("B")) )
	{
		String strEffStatus="";
		if(reason_for_active_yn.equals("Y"))
			strEffStatus="Y";
		else if(reason_for_active_yn.equals("N"))
			strEffStatus="N";
		
		if(andCheck == 0 ){
			sbQuery.setLength(0);
			sbQuery.append("where reason_for_active_yn='"+strEffStatus+"'");
		}else if(!enabled.equals("B")){
			sbQuery.append(" and reason_for_active_yn='"+strEffStatus+"'");
		}
	}
...
/*	pstmt = conn.prepareStatement("select count(*) as total from mp_pat_inactivate_reason"+sbQuery.toString());
...
//	pstmt = conn.prepareStatement("select reason_code, long_desc, short_desc, eff_Status,reason_for_active_yn from mp_pat_inactivate_reason"+sbQuery.toString());
...
while ( rs.next() && i<=end )
{
	if(cnt==0)
	{
		...
		if ( !(start <= 1) )
			out.println("<td align ='right' id='prev'><A HREF='../../eMP/jsp/queryActiveInactive.jsp?from="+(start-14)+"&to="+(end-14)+"&whereclause="+URLEncoder.encode(sbQuery.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A></td>");
			
            _bw.write(_wl_block9Bytes, _wl_block9);

		//if ( !( (start+14) > maxRecord ) )
			out.println("<A HREF='../../eMP/jsp/queryActiveInactive.jsp?from="+(start+14)+"&to="+(end+14)+"&whereclause="+URLEncoder.encode(sbQuery.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
	...
	out.println("<a href='../../eMP/jsp/ActiveInactiveModify.jsp?reason_code="+ code + "' target='f_query_add_mod' >");
	out.println(code+"</a></td><td class='" + classValue + "'>");
	out.println( rs.getString("long_desc") );
	out.println("</td><td class='" + classValue + "'>");
	out.println(rs.getString("short_desc") );
	out.println("</td><td class='" + classValue + "' align='center'>");
	out.println(rs.getString("reason_for_active_yn") );
	out.println("</td><td class='" + classValue + "' align='center'>");
