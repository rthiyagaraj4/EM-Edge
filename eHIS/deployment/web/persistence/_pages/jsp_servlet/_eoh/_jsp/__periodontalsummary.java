...
public final class __periodontalsummary extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eoh/jsp/PeriodontalSummary.jsp", 1709119359255L ,"10.3.6.0","Asia/Calcutta")) return true;
	...
	//
		...
		try{
			con=ConnectionManager.getConnection(request);
				...
				//pstmt=con.prepareStatement("SELECT A.QUADRANT_SNO, A.TOOTH_NO, (SELECT B.NS_TOOTH_NO FROM OH_TOOTH_BY_NUMBERING_SYSTEM B WHERE B.NUMBERING_SYSTEM = ? AND B.TOOTH_NO = A.TOOTH_NO) NS_TOOTH_NO, COUNT(A.TOOTH_NO) TOOTH_COUNT, SUM(A.VALUE) GINGIVAL_STATUS_SUM  FROM  OH_RESTORATIVE_CHART_GING_STS A WHERE A.OPERATING_FACILITY_ID = ? AND A.PATIENT_ID=? AND A.CHART_NUM = ? AND NVL(STATUS,'X') != 'E' AND A.QUADRANT_SNO IS NOT NULL GROUP BY A.QUADRANT_SNO, A.TOOTH_NO");
				if(oh_chart_level.equals("E")){
					//Commented and Added by Sharon Crasta on 8/6/2009 for IN009739(SRR20056-CRF-0457)
					//pstmt=con.prepareStatement("SELECT A.QUADRANT_SNO, A.TOOTH_NO, (SELECT B.NS_TOOTH_NO FROM OH_TOOTH_BY_NUMBERING_SYSTEM B WHERE B.NUMBERING_SYSTEM = ? AND B.TOOTH_NO = A.TOOTH_NO) NS_TOOTH_NO, COUNT(A.TOOTH_NO) TOOTH_COUNT, SUM(A.VALUE) GINGIVAL_STATUS_SUM  FROM  OH_RESTORATIVE_CHART_GING_STS A WHERE A.PATIENT_ID=? AND A.CHART_NUM = ? AND NVL(STATUS,'X') != 'E' AND A.QUADRANT_SNO IS NOT NULL GROUP BY A.QUADRANT_SNO, A.TOOTH_NO");
					pstmt=con.prepareStatement("SELECT A.QUADRANT_SNO, A.TOOTH_NO, (SELECT B.NS_TOOTH_NO FROM OH_TOOTH_BY_NUMBERING_SYSTEM B WHERE B.NUMBERING_SYSTEM = ? AND B.TOOTH_NO = A.TOOTH_NO) NS_TOOTH_NO, (SELECT C.MAPPED_TOOTH_NO FROM OH_TOOTH C WHERE C.TOOTH_NO = A.TOOTH_NO) MAPPED_TOOTH_NO, COUNT(A.TOOTH_NO) TOOTH_COUNT, SUM(A.VALUE) GINGIVAL_STATUS_SUM, A.STATUS  FROM  OH_RESTORATIVE_CHART_GING_STS A WHERE A.PATIENT_ID=? AND A.CHART_NUM = ? AND NVL(STATUS,'X') != 'E' AND NVL(STATUS,'X') != 'Y'  AND NVL(STATUS,'X') != 'N' AND A.QUADRANT_SNO IS NOT NULL GROUP BY A.QUADRANT_SNO, A.TOOTH_NO, A.STATUS ORDER BY NS_TOOTH_NO ASC");
					...
					//Commented and Added by Sharon Crasta on 8/6/2009 for IN009739(SRR20056-CRF-0457)
					//pstmt=con.prepareStatement("SELECT A.QUADRANT_SNO, A.TOOTH_NO, (SELECT B.NS_TOOTH_NO FROM OH_TOOTH_BY_NUMBERING_SYSTEM B WHERE B.NUMBERING_SYSTEM = ? AND B.TOOTH_NO = A.TOOTH_NO) NS_TOOTH_NO, COUNT(A.TOOTH_NO) TOOTH_COUNT, SUM(A.VALUE) GINGIVAL_STATUS_SUM  FROM  OH_RESTORATIVE_CHART_GING_STS A WHERE A.OPERATING_FACILITY_ID = ? AND A.PATIENT_ID=? AND A.CHART_NUM = ? AND NVL(STATUS,'X') != 'E' AND A.QUADRANT_SNO IS NOT NULL GROUP BY A.QUADRANT_SNO, A.TOOTH_NO");
					pstmt=con.prepareStatement("SELECT A.QUADRANT_SNO, A.TOOTH_NO, (SELECT B.NS_TOOTH_NO FROM OH_TOOTH_BY_NUMBERING_SYSTEM B WHERE B.NUMBERING_SYSTEM = ? AND B.TOOTH_NO = A.TOOTH_NO) NS_TOOTH_NO, (SELECT C.MAPPED_TOOTH_NO FROM OH_TOOTH C WHERE C.TOOTH_NO = A.TOOTH_NO) MAPPED_TOOTH_NO, COUNT(A.TOOTH_NO) TOOTH_COUNT, SUM(A.VALUE) GINGIVAL_STATUS_SUM, A.STATUS  FROM  OH_RESTORATIVE_CHART_GING_STS A WHERE A.OPERATING_FACILITY_ID = ? AND A.PATIENT_ID=? AND A.CHART_NUM = ? AND NVL(STATUS,'X') != 'E' AND NVL(STATUS,'X') != 'Y' AND NVL(STATUS,'X') != 'N' AND A.QUADRANT_SNO IS NOT NULL GROUP BY A.QUADRANT_SNO, A.TOOTH_NO, A.STATUS ORDER BY NS_TOOTH_NO ASC");
				...
				while(rs.next()){
					ns_tooth_no = rs.getString("NS_TOOTH_NO");
				...
				//	}
					 //
					if(rs.getString("QUADRANT_SNO").equals("1")){
						if(q1_count == 0){
					...
					else if(rs.getString("QUADRANT_SNO").equals("2")){
						if(q2_count == 0){
					...
					else if(rs.getString("QUADRANT_SNO").equals("3")){
						if(q3_count == 0){
					...
					else if(rs.getString("QUADRANT_SNO").equals("4")){
						if(q4_count == 0){
