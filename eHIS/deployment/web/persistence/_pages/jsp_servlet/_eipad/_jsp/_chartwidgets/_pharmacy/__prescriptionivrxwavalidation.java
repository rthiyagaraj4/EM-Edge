...
public final class __prescriptionivrxwavalidation extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/eipad/jsp/chartwidgets/pharmacy/prescriptionIvRxWaValidation.jsp", 1709116948317L ,"10.3.6.0","Asia/Calcutta")) return true;
...
//clearing the mfr details and enabling admin form fields on change of fluid
		...
		if (bean.checkForStock()) {
            ...
            int fluid_adr_count = bean.getADRCount(patient_id,drug_code);   
			if (stockDetails.size() != 0)
				out.println("setVolumeDetails(1, '"+ stockDetails.get(0) + "', '" + stockDetails.get(1) + "', '" + bean.getUomDisplay((String)session.getValue("facility_id"),(String)stockDetails.get(1)) + "', '" +rec_no + "','"+ fluid_adr_count +"','"+fluid_stock_uom_desc+"');") ;				
			else
		...
		if(drug_count>0){
			...
			if(dfltFluidDetails != null && dfltFluidDetails.size()>0){
				...
				if(dfltFluidCode.equals(""))
					dfltFluidCode = (String)dfltFluidDetails.get(0);
				bean.setFluidDetails(dfltFluidCode,"","","",""); 
				HashMap fluid_det =(HashMap)bean.getFluidDetails();
		        String orderType = (String)fluid_det.get("ORDER_TYPE_CODE");
				String  fluid_stock_uom					= (String) fluid_det.get("FLUID_STOCK_UOM");
				String fluid_stock_uom_desc = bean.getUomDisplay((String)session.getValue("facility_id"),fluid_stock_uom);
			...
			if(drugList!=null && drugList.size()>0){
				for(int i=0;i<drugList.size();i++){
					...
					if(extdrugCode.equals(drug_code) && extsrlNo.equals(srl_no) ){											
						...
						drugDetails.put("buildMAR_yn",buildMAR_yn); 
					}
				}
			}
		...
		String rowNum = "";
		if (mode.equals("1"))
			out.println("setEndDate('"+ end_date +"','document.rxDetailsIvRxWaViewForm.ivRxEndDate');");
		else if (mode.equals("2"))
			out.println("setEndDate('"+ end_date +"','document.rxDetailsIvRxWaViewForm.ivRxEndDate');");
		else if (mode.equals("3"))
			out.println("setEndDate('"+ end_date +"','parent.f_detail.f_iv_admin.document.rxDetailsIvRxWaViewForm.ivRxEndDate');");
		...
		out.println("setEndDate('"+ end_date +"','document.rxDetailsIvRxWaViewForm.ivRxEndDate"+ rec_no +"');");		
	}
		...
		for(int i=1;i<=6;i++){			
			...
			if(!((String)hash.get("FLUID_CODE"+i)).equals("") && ((String)hash.get("SELECT"+i)).equals("Y")) {
				...
				if(((String)hash.get("mfr_yn"+i)).equals("Y")){
					...
					record.put("INFUSE_OVER_UNIT",((String)hash.get("INFUSE_OVER_UNIT")));
				}
...
//added for Billing CRF 0001
    ...
    //Added for  ML-BRU-CRF-072[Inc:29938] End
	...
	else if(validate.equals("completeIVRxDetails")){
		ArrayList AllFluids			= new ArrayList();	
		ArrayList newAllFluids			= new ArrayList();	
		String patient_class		= request.getParameter("patient_class");
		...
		bean.setAllergic((String)hash.get("allergic_yn"));
		bean.setBuildMAR_yn(buildMAR_yn); 
		bean.setOrderDate((String)request.getParameter("sys_date")); 
		bean.setTakeHomeMedication((String)hash.get("take_home_medication")); 
		...
		mfr_recsAll = new HashMap();
		mfr_recsAll = bean.getMFRRecs_fluids("");
		if(!((String)hash.get("FLUID_CODE")).equals("") && ((String)hash.get("SELECT")).equals("Y")) {
			if(mfr_recsAll != null && mfr_recsAll.size() > 0)
				mfr_recs = (HashMap)mfr_recsAll.get((String)hash.get("FLUID_CODE"));
			...
			if(AllFluids!=null && AllFluids.size()>0){
				for(int j=0; j<AllFluids.size(); j++){
					...
					if(record.get("DRUG_CLASS") != null && ((String)record.get("DRUG_CLASS")).equals("N"))
						narcotic_yn = "Y";
					if(((String)hash.get("FLUID_CODE")).equals((String)record.get("DRUG_CODE"))){
						if(record==null || record.size()==0){
							bean.setFluidDetails (((String)hash.get("FLUID_CODE")),"","","",""); // CRF-0062 NEWLY ADDED
							record	=	bean.getFluidDetails();
						}
						if(record.get("DRUG_CLASS") != null && ((String)record.get("DRUG_CLASS")).equals("N"))
							narcotic_yn = "Y";
						if(!bean.getLanguageId().equals("en")){
							record.put("START_DATE",com.ehis.util.DateUtils.convertDate(((String)hash.get("START_DATE")),"DMYHM",bean.getLanguageId(),"en"));
							record.put("END_DATE",com.ehis.util.DateUtils.convertDate(((String)hash.get("END_DATE")),"DMYHM",bean.getLanguageId(),"en"));
						}
						else{
							record.put("START_DATE",((String)hash.get("START_DATE")));
							record.put("END_DATE",((String)hash.get("END_DATE")));
						}
						temp_mfr_remarks="";
						temp_mfr_remarks=(String)hash.get("mfr_remarks");
						if(!temp_mfr_remarks.equals("") || temp_mfr_remarks!=null)
							temp_mfr_remarks=java.net.URLDecoder.decode(temp_mfr_remarks,"UTF-8");
						record.put("INFUSION_PERIOD_VALUE",((String)hash.get("INFUSION_PERIOD_VALUE_HRS")));
						record.put("INFUSION_PERIOD_VALUE_MIN",((String)hash.get("INFUSION_PERIOD_VALUE_MIN")));
						record.put("QTY_VALUE",((String)hash.get("volume")));
						record.put("DURN_VALUE",((String)hash.get("DURATION")));
						record.put("MFR_REMARKS",temp_mfr_remarks);
						record.put("BUILD_MAR_YN",((String)hash.get("buildMAR_yn")));
						record.put("PER_FACILITY_ID",((String)hash.get("PER_FACILITY_ID"))); // Added for Bru-HIMS-CRF-347 [IN:037862]
						record.put("DISP_LOCN_CODE",((String)hash.get("DISP_LOCN_CODE"))); // Added for Bru-HIMS-CRF-347 [IN:037862]
						record.put("DISP_LOCN_TYPE",((String)hash.get("DISP_LOCN_TYPE"))); // Added for Bru-HIMS-CRF-347 [IN:037862]
						record.put("PER_FACILITY_ID",((String)hash.get("PER_FACILITY_ID"))); // Added for Bru-HIMS-CRF-347 [IN:037862]
						record.put("ROOM_NUM",((String)hash.get("ROOM_NUM"))); //Added for MMS-QH-CRF-0080 [IN:038064]
						record.put("BED_NUM",((String)hash.get("BED_NUM"))); //Added for MMS-QH-CRF-0080 [IN:038064]
							if(((String)hash.get("mfr_yn")).equals("Y")){
							record.put("INFUSION_RATE","");
							record.put("INFUSE_OVER","");
							record.put("INFUSE_OVER_UNIT_SCH","");
							record.put("INFUSE_OVER_UNIT","");
							...
							record.put("INFUSION_RATE",((String)hash.get("INF_RATE")));
							record.put("INFUSE_OVER",((String)hash.get("INFUSION_PERIOD_VALUE")));
							record.put("INFUSE_OVER_UNIT_SCH",((String)hash.get("inf_rate_uom")));
							record.put("INFUSE_OVER_UNIT",((String)hash.get("INFUSE_OVER_UNIT")));
						}
						record.put("MFR_Recs",mfr_recs);
		...
		PersistenceHelper.putObjectInBean(pres_bean_id,pres_bean,request);		
	}
