...
public final class __loanreturngroupvalidate extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/ess/jsp/LoanReturnGroupValidate.jsp", 1732510709974L ,"10.3.6.0","Asia/Calcutta")) return true;
	...
	switch (functionType) {
		...
		case 2: 
			{	
					...
					out.println("clearListItems('parent.frameLoanReturnGroupDetail.document.formLoanReturnGroupDetail.bin_location_code'); ");
					ArrayList binLocation = bean.getBinLocations();
					for(int i=0; i< binLocation.size(); i++) {
						String[] record = (String[])binLocation.get(i);
						out.println("addListItem(\"parent.frameLoanReturnGroupDetail.document.formLoanReturnGroupDetail.bin_location_code\", \"" + record[0] + "\",\"" + record[1] + "\");");
					}
					binLocation = null;
					...
					if (remarks == null || remarks.equals("")) {
						...
						out.println("parent.frameLoanReturnGroupDetail.document.formLoanReturnGroupDetail.img_remarks.style.visibility = 'visible';");
						}else{
						out.println("parent.frameLoanReturnGroupDetail.document.formLoanReturnGroupDetail.img_remarks.style.visibility = 'hidden';");
						}
				//ENDS	
		...
		case 5: 
			{
					...
					out.println("setPatientName('"+patient_name+"','"+index+"');");
			}
			break;
		case 6: 
			{
					if (hash.size() > 0) {
						bean.updateSelectedTraysValues(hash);
		...
		case 11:
			{
					...
					//Added by Sakti to handle Special Character if any in remarks against inc#45463
					String remarks = discrepancy_details[5];
					...
					out.println("arrayObject[2] = '"+bean.checkForNull(remarks)+"';");
		...
		case 14 :
			{
				...
				out.println ("document.formLoanReturnGroupDiscrepancyDetail.charge_amt.value = \'"+(unitCost * itemQty)+"\'");
			}
			break;
		case 15 :
			{
		...
		case 17:
			{
				...
				out.println("document.formLoanReturnGroupDiscrepancyDetail.item_code.value=\""+bean.checkForNull((String)hmRecord.get("ITEM_CODE"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.write_off_reason_code.value=\""+bean.checkForNull((String)hmRecord.get("WRITE_OFF_REASON_CODE"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.remarks.value=\""+bean.checkForNull((String)hmRecord.get("REMARKS"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.discrepancy_qty.value=\""+bean.checkForNull((String)hmRecord.get("DISCREPANCY_QTY"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.patient_id.value=\""+bean.checkForNull((String)hmRecord.get("PATIENT_ID"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.item_returnable_yn.value=\""+bean.checkForNull((String)hmRecord.get("ITEM_RETURNABLE_YN"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.encounter_id.value=\""+bean.checkForNull((String)hmRecord.get("ENCOUNTER_ID"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.charge_option.value=\""+bean.checkForNull((String)hmRecord.get("CHARGE_OPTION"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.charge_amt.value=\""+bean.checkForNull((String)hmRecord.get("CHARGE_AMT"))+"\"");
				out.println("document.formLoanReturnGroupDiscrepancyDetail.rec_reference.value=\""+bean.checkForNull((String)hmRecord.get("REC_REFERENCE"))+"\"");
				}
			}
		...
		case 18:
			{
				...
				if(stCountRecord.equals("0"))
				{
					...
					out.println("parent.frameLoanReturnGroupDetail.document.formLoanReturnGroupDetail.discrepancy.disabled=true");
				}
			}
			break;
		...
		break;
		case 20: // This case will be executed on click of group search button added  to hold already added groupcode-trayno
		{

			if (hash.size() > 0) {
				bean.updateSelectedList(hash);
			}
		}
		break;
		default :
