...
public final class __requestgroupvalidate extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/ess/jsp/RequestGroupValidate.jsp", 1728455891041L ,"10.3.6.0","Asia/Calcutta")) return true;
	...
	RequestGroupListBean bean		=		(RequestGroupListBean)getBeanObject( "ssrequestGroupListBean", "eSS.RequestGroupListBean", request ) ;
	...
	try{
		if (validate.equals("DOC_TYPE_CODE")){
			...
			out.println(  "clearListItems(\"parent.frameRequestGroupDetail.document.formRequestGroupDetail.auth_group_code\");"  );
			if(alRecords==null) return;
			for (int i=0;i<alRecords.size();i++){
				HashMap hmRecord	=	(HashMap)alRecords.get(i);
				out.println(  "addListItem(\"parent.frameRequestGroupDetail.document.formRequestGroupDetail.auth_group_code\",\""+hmRecord.get("GROUP_CODE")+"\",\""+hmRecord.get("SHORT_DESC")+"\",\""+req_group_code+"\");"  );
			}
		}
			...
			if(bean.isGroupInArrayList(alRequestGroupList,req_group_code,Integer.parseInt(index)))
			...
			if(index.equals("-1")){
				...
				alRequestGroupList.add(hmListItems);
			}
			else{
				hmListItems.put("mode",((HashMap)alRequestGroupList.get(Integer.parseInt(index))).get("mode"));
				alRequestGroupList.set(Integer.parseInt(index),hmListItems);
			}
			...
			try{
				...
				//This is added by Sakti  if in group search window any one group is selected among all pages, need to add that group in list. If not, then  message shown
			...
			HashMap allMapvalue =(HashMap)bean.fetchRecord(bean.getSsRepositoryValue("SQL_SS_STERILIZED_QTY_SELECT"),parameters);
		...
		else if (validate.equals("printOnlineDoc")){//Adding start for AAKH-CRF-0057
			System.err.println("insideprintOnlineDoc@@@@@");
				String language_id = request.getParameter("language_id");
				String facility_id = bean1.getLoginFacilityId();
				String report_id="SSREQGRPFRM";
				String doc_no=request.getParameter("doc_no");
				 doc_type_code=request.getParameter("doc_type_code");
				 //System.err.println("doc_no@@@========"+doc_no+"doc_type_code@@@@@========"+doc_type_code+"report_id===="+report_id+"language_id==="+language_id+"facility_id==="+facility_id);
	...
	putObjectInBean("ssrequestGroupListBean",bean,request);
