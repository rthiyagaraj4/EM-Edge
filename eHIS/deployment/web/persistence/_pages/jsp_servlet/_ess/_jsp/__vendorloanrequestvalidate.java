...
public final class __vendorloanrequestvalidate extends  weblogic.servlet.jsp.JspBase  implements weblogic.servlet.jsp.StaleIndicator {
    ...
    public static boolean _staticIsStale(weblogic.servlet.jsp.StaleChecker sci) {
        if (sci.isResourceStale("/ess/jsp/VendorLoanRequestValidate.jsp", 1732511083611L ,"10.3.6.0","Asia/Calcutta")) return true;
    ...
    throws javax.servlet.ServletException, java.io.IOException {
        ...
        try {;
            ...
            java.util.ArrayList alVendorLoanRequestList= null;synchronized(session){
                alVendorLoanRequestList=(java.util.ArrayList)pageContext.getAttribute("alVendorLoanRequestList",javax.servlet.jsp.PageContext.SESSION_SCOPE);
                if(alVendorLoanRequestList==null){
                    alVendorLoanRequestList=new java.util.ArrayList();
                    pageContext.setAttribute("alVendorLoanRequestList",alVendorLoanRequestList,javax.servlet.jsp.PageContext.SESSION_SCOPE);
	...
	VendorLoanRequestListBean bean		=		(VendorLoanRequestListBean)getBeanObject( "VendorLoanRequestListBean", "eSS.VendorLoanRequestListBean", request ) ;
	VendorLoanRequestBean bean1			=		(VendorLoanRequestBean)getBeanObject( "VendorLoanRequestBean", "eSS.VendorLoanRequestBean", request ) ;
	...
	try{
		if (validate.equals("DOC_TYPE_CODE")){
			...
			if(!issue_store_code.equals("")){
				try{
					String stParameters[]={issue_store_code,req_group_code};
			...
			if(bean.isGroupInArrayList(alVendorLoanRequestList,req_group_code,Integer.parseInt(index)))
			...
			if(index.equals("-1")){
				...
				alVendorLoanRequestList.add(hmListItems);
			}
			else{
				hmListItems.put("mode",((HashMap)alVendorLoanRequestList.get(Integer.parseInt(index))).get("mode"));
				alVendorLoanRequestList.set(Integer.parseInt(index),hmListItems);
			}

			result	=	true;
			message	=	"Done";
		}
	    else if (validate.equals("CHECK_REQ_STORE")){
			try{
			         result			=	true;
		...
		}else if (validate.equals("ADD_GROUPS_TO_LIST")){
			try{
			...
			for(int i=0;i<total_rec;i++){
				...
				hmDtlRecord.put("TOTAL_QTY",(String)hash.get("total_qty"+i));
			...
			int iValidDocDate = bean1.isValidDocDate(doc_date);
			if(iValidDocDate==1){
				flag			=	"DOC_DT_NOT_LT_PROC_MTH";
				result			=	false;
			}else if(iValidDocDate==2){
				flag			=	"DOC_DT_NOT_GR_SYSDATE";
				result			=	false;
			}else{
				result			=	true;
				flag			= "";
			...
			String req_on_vendor_code = request.getParameter("req_on_vendor_code");
	...
	putObjectInBean("VendorLoanRequestListBean",bean,request);
