...
public class CentralizedMonitor extends JFrame implements ActionListener {
	...
	driverLabel = new JLabel("Database", JLabel.LEFT);
	driverField = new JTextField(20);
	driverField.setHorizontalAlignment(JTextField.LEFT);
...
*/
	...
	namePanel.add(userNameLabel, gbc);
	gbc.gridwidth = GridBagConstraints.REMAINDER;
	namePanel.add(userNameField, gbc);

	gbc.gridwidth = GridBagConstraints.RELATIVE;
	namePanel.add(passwordLabel, gbc);
	gbc.gridwidth = GridBagConstraints.REMAINDER;
	namePanel.add(passwordField, gbc);

	gbc.gridwidth = GridBagConstraints.RELATIVE;
	namePanel.add(driverLabel, gbc);
	gbc.gridwidth = GridBagConstraints.REMAINDER;
	namePanel.add(driverField, gbc);
	...
	connectionPanel.add(namePanel,BorderLayout.CENTER);
	...
	public void connect() {
		...
		if(dbInfoExists)
		{
			CentralizedMonitorApplication monitorApp = new CentralizedMonitorApplication(databaseurl, username, password);	
			if(monitorApp.isLoginSuccessfull) setVisible(false);
		}
	}
