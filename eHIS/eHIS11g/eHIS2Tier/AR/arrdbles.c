...
{
    78,
    "E:\\Work Gdoh\\ar-proc-source form team\\PROC_Files_10052017\\ARRDBLES\\arrdbles.pc"
};


static unsigned long sqlctx = 1096740483;
...
{10,4130,0,0,0,
5,0,0,1,513,0,4,216,0,0,25,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
...
120,0,0,2,249,0,4,329,0,0,9,3,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,3,0,0,1,9,0,0,
171,0,0,3,91,0,2,428,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
194,0,0,4,0,0,30,437,0,0,0,0,0,1,0,
209,0,0,5,555,0,9,660,0,0,12,12,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
272,0,0,6,577,0,9,688,0,0,12,12,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
335,0,0,7,329,0,9,731,0,0,1,1,0,1,0,1,9,0,0,
354,0,0,5,0,0,13,752,0,0,4,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
385,0,0,6,0,0,13,761,0,0,4,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
416,0,0,7,0,0,13,804,0,0,12,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
479,0,0,8,185,0,9,839,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
506,0,0,8,0,0,13,852,0,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,
533,0,0,9,0,0,27,1154,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
};
...
/*-- Y2K corrections. Year format changed from YY to YYYY VSK 14/11/1999 */
...
_ACC_ENTITY where ACC_ENTITY_ID=:b8";
 ...
 /*/// Printing The Customer Name & Alpha Name ////////*/

if (lctr > 56) print_head();

fprintf(fp,"%-10s  %-40.40s%-10s ",
ar_customer_cust_code.arr,ar_customer_long_name.arr,
ar_cust_alpha_code.arr);
/*-------------------------------------------*/             
        while(fetch_cust_trn())
        {
			  ...
			  strcpy(to_ym.arr,xty.arr);
			  strcat(to_ym.arr,xtm.arr);

			  strcpy(fm_ym.arr,xfy.arr);
			  strcat(fm_ym.arr,xfm.arr);
...
open_files()     
{
   ...
   if ((fp = fopen(filename,"w")) == NULL)
   {
		...
		err_mesg(text_msg,0,"");
		//err_mesg("\nError while opening File arrdbles.lis\n",0,"");
        proc_exit();
   }
...
"\n\nMDL : AR                                               %-30s                                                %-16s\n",
  ...
  */
   fprintf(fp,"%cM",ESC);
   print_hospital_name(149,"AR",hosp_name.arr,date_time.arr);

     fprintf(fp,"OPR : %-10s \n", user_id.arr);
  fprintf(fp,
"REP : %-8s                                         %-28s                                                PAGE : %4d\n",
    g_pgm_id,"DEBTOR'S LEDGER - SUMMARY BY PERIOD",++pctr);
fprintf(fp,
"-----------------------------------------------------------------------------------------------------------------------------------------------------\n");
...
"\fMDL : AR                                               %-30s                                                %-16s\n",
  ...
  */
  ...
  fprintf(fp,"REP : %-8s                                         %-28s                                                PAGE : %4d\n",
    g_pgm_id,"DEBTOR'S LEDGER - SUMMARY BY PERIOD",++pctr);
  fprintf(fp,"                                                     FOR THE PERIOD FROM %s/%s TO %s/%s\n",
//    open_bal_date.arr,close_bal_date.arr);
   xfm.arr,xfy.arr,xtm.arr,xty.arr);

fprintf(fp,"-----------------------------------------------------------------------------------------------------------------------------------------------------\n");
fprintf(fp,"CUSTOMER    CUSTOMER NAME                            ALPHA            OPENING BALANCE             DEBIT AMT          CREDIT AMT      CLOSING BALANCE\n");
fprintf(fp,"CODE                                                 CODE             AS OF %-2s/%-4s                                                  AS OF %-2s/%-4s \n",xfm.arr,xfy.arr,xtm.arr,xty.arr);
fprintf(fp,"-----------------------------------------------------------------------------------------------------------------------------------------------------\n\n");
  fflush(fp);
  lctr = 10;
}
...
print_end()
{
  ...
  char s_tot_dr_amt[20], s_tot_cr_amt[20], s_tot_bal_amt[20],s_tot_open_amt[20];
  ...
  if(g_tot_print_flag) {
	 ...
	 sprintf(s_tot_bal_amt,"%16.2f",g_tot_bal_amt);
      ...
      if (nd_no_of_decimal==3)
         sprintf(s_tot_open_amt,"%16.3f",tot_open_amt);
      else 
	     sprintf(s_tot_open_amt,"%16.2f",tot_open_amt);

      ltrim(s_tot_open_amt);
      format_amt(s_tot_open_amt,nd_no_of_decimal);
...
fprintf(fp,"-----------------------------------------------------------------------------------------------------------------------------------------------------\n");
	  ...
	  if (g_tot_bal_amt>0)
	      fprintf(fp,"                                 GRAND TOTAL  ---------->          %16s      %16s    %16s    %16s\n\n",s_tot_open_amt,s_tot_dr_amt,s_tot_cr_amt,s_tot_bal_amt);
	  
	  else if (g_tot_bal_amt==0)  
	   	  fprintf(fp,"                                 GRAND TOTAL  ---------->          %16s      %16s    %16s    %16s\n\n",s_tot_open_amt,s_tot_dr_amt,s_tot_cr_amt,s_tot_bal_amt);
	 	  
	  else
	     fprintf(fp,"                                 GRAND TOTAL  ---------->          %16s      %16s    %16s    %16sCR\n\n",s_tot_open_amt,s_tot_dr_amt,s_tot_cr_amt,s_tot_bal_amt);
...
/*--------- Added to get the sum of debits & Credits -------*/
...
*/
fprintf(fp,
"\n                                                OPENING BALANCE AS OF %-2d/%-4d        --------->               ",
nd_fm_post_month,nd_fm_post_year);
if (opening_amt >= 0)
{
   if (nd_no_of_decimal==3)
...
print_footer()
{
  ...
  if (lctr > 56) print_head();
/* Marked for deletion on 22/12/1998 ------*/
/*
fprintf(fp,
"\n                                                CLOSING BALANCE AS OF %-2d/%-4d        --------->               ",
nd_to_post_month,nd_to_post_year);
*/
/*------ Added on 22/12/1998 to print the summary details --*/

//   Getting the Opening balance for the customer
...
else
{
   ...
   sprintf(out_str1,"%20s",out_str);
   fprintf(fp,"%-20sCR",out_str1);
}
     tot_open_amt += opening_amt;
/*----------------------------------------------------*/
...
//   {
      if (nd_no_of_decimal==3)
         sprintf(out_str,"%16.3f",tot_db_amt);
      else
         sprintf(out_str,"%16.2f",tot_db_amt);
...
//   {
   ...
   //   fprintf(fp,"%20s"," ");
      if (nd_no_of_decimal==3)
         sprintf(out_str,"%16.3f",tot_cr_amt);
      else
	 sprintf(out_str,"%16.2f",tot_cr_amt);
