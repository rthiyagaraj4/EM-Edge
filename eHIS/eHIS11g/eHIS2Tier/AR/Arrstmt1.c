...
{
    78,
    "E:\\Work Gdoh\\ar-proc-source form team\\PROC_Files_10052017\\ARRSTMT1\\Arrstmt1.pc"
};


static unsigned long sqlctx = 1098316419;
...
_DOC where ((OPERATING_FACILITY_ID=:b1 and CUST_CODE=:b2) and NVL(DOC_DATE,BAS\
E_DATE)<=TO_DATE(:b0,'DD/MM/YYYY')) order by DOC_DATE,DOC_TYPE_CODE,DOC_NUM   \
         ";
...
UNT  from AR_CUST_TRN where (((((operating_facility_id=:b0 and CUST_CODE=:b1) \
and ORG_DOC_TYPE_CODE=:b2) and ORG_DOC_NUM=:b3) and (RPAD(ORG_DOC_TYPE_CODE,6,\
' ')||TO_CHAR(ORG_DOC_NUM))<>(RPAD(DOC_TYPE_CODE,6,' ')||TO_CHAR(DOC_NUM))) an\
d DOC_DATE<=TO_DATE(:b4,'DD/MM/YYYY'))           ";
...
201,0,0,5,166,0,4,455,0,0,6,1,0,1,0,2,3,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,3,0,0,1,
9,0,0,
240,0,0,6,1084,0,9,625,0,0,14,14,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
311,0,0,7,1110,0,9,631,0,0,14,14,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
382,0,0,6,0,0,13,667,0,0,12,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,1,0,0,2,1,0,0,2,9,0,0,
445,0,0,7,0,0,13,685,0,0,12,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,1,0,0,2,1,0,0,2,9,0,0,
508,0,0,8,943,0,9,720,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
539,0,0,8,0,0,13,787,0,0,31,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
...
678,0,0,11,310,0,4,898,0,0,6,5,0,1,0,2,4,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,
717,0,0,9,119,0,9,940,0,0,1,1,0,1,0,1,9,0,0,
736,0,0,9,0,0,13,945,0,0,1,0,0,1,0,2,9,0,0,
755,0,0,10,0,0,13,969,0,0,5,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,
790,0,0,10,359,0,9,1066,0,0,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
825,0,0,12,0,0,27,1276,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
};
...
if(d_misc_customer_flag == 'Y') {
    ...
    /* EXEC SQL SELECT NVL(SUM(NVL(AMOUNT,0)),0) 
	     ...
	     FROM   AR_CUST_TRN
	     WHERE operating_facility_id=:nd_facility_id /oadded for gdoh-crf-107.6o/ 
		 and   CUST_CODE = :d_cust_cust_code
	     AND    ORG_DOC_TYPE_CODE = :d_ar_pend_doc_type_code
	     AND    ORG_DOC_NUM       = :d_ar_pend_doc_num
	     AND    RPAD(ORG_DOC_TYPE_CODE,6,' ')||TO_CHAR(ORG_DOC_NUM) <>
...
RN where (((((operating_facility_id=:b1 and CUST_CODE=:b2) and ORG_DOC_TYPE_CO\
DE=:b3) and ORG_DOC_NUM=:b4) and (RPAD(ORG_DOC_TYPE_CODE,6,' ')||TO_CHAR(ORG_D\
OC_NUM))<>(RPAD(DOC_TYPE_CODE,6,' ')||TO_CHAR(DOC_NUM))) and DOC_DATE<=TO_DATE\
(:b5,'DD/MM/YYYY'))";
    sqlstm.iters = (unsigned int  )1;
    ...
    sqlstm.sqadto[2] = (unsigned short )0;
    sqlstm.sqtdso[2] = (unsigned short )0;
    sqlstm.sqhstv[3] = (         void  *)&d_ar_pend_doc_type_code;
    ...
    sqlstm.sqadto[3] = (unsigned short )0;
    sqlstm.sqtdso[3] = (unsigned short )0;
    sqlstm.sqhstv[4] = (         void  *)&d_ar_pend_doc_num;
