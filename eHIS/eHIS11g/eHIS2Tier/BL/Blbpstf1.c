...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/Blbpstf1.pc"
};


static unsigned int sqlctx = 1288464579;
...
"PE_CODE ,DOC_NUMBER ,BILL_DOC_TYPE_CODE ,BILL_DOC_NUM ,TO_CHAR(BILL_DOC_DATE\
,'DD/MM/YYYY') ,TRX_FINALIZE_IND ,FINALIZED_BY_ID ,TO_CHAR(FINALIZED_DATE,'DD/\
MM/YYYY') ,NVL(TRX_POSTED_IND,'N') ,POSTED_BY_ID ,TO_CHAR(POSTED_DATE,'DD/MM/Y\
YYY') ,LAST_ADJUSTED_BY_ID ,TO_CHAR(LAST_ADJUSTED_DATE,'DD/MM/YYYY') ,ADDED_BY\
_ID ,MODIFIED_BY_ID ,TO_CHAR(ADDED_DATE,'DD/MM/YYYY') ,TO_CHAR(MODIFIED_DATE,'\
DD/MM/YYYY') ,((PACKAGE_IND||PACKAGE_SERVICE_CODE)||BLNG_SERV_CODE) ,CASH_CTRL\
_DOC_TYPE_CODE ,CASH_CTRL_DOC_NUMBER ,CASH_CTRL_DOC_DATE ,CASH_COUNTER_CODE ,S\
HIFT_ID ,CUST_CODE  from BL_PAT_CHARGES_TEMP where (((((((((NVL(TRX_POSTED_IND\
,'N')='N' and TRX_DATE<=TO_DATE((:b0||' 23:59'),'DD/MM/YYYY HH24:MI')) and NVL\
(TRX_FINALIZE_IND,'N')='Y') and CONFIRMED_YN='Y') and EPISODE_TYPE in ('O','E'\
)) and PATIENT_ID=PATIENT_ID) and EPISODE_ID=EPISODE_ID) and VISIT_ID=VISIT_ID\
) and AVAILED_IND in ('Y','U','C')) and OPERATING_FACILITY_ID=:b1) order by TR\
X_DOC_REF,PACKAGE_IND,PACKAGE_SERVICE_CODE,BLNG_SERV_CODE,ACCT_DEPT_CODE      \
      ";

 static const char *sq0012 = 
"select BL_PKG_CTRL_ACC_INT_CODE  from BL_BILL_TYPE_INT where ((BILL_TYPE_COD\
E=:b0 and TO_DATE(:b1,'DD/MM/YYYY') between NVL(EFF_FROM_DATE,TO_DATE('01/01/1\
900','DD/MM/YYYY')) and NVL(EFF_TO_DATE,SYSDATE)) and ACC_ENTITY_CODE in (sele\
ct ACC_ENTITY_CODE  from SY_ACC_ENTITY where ACC_ENTITY_ID=:b2))           ";
...
"select DISCOUNT_ACC_INT_CODE  from BL_TYPE_FOR_SERV_GRP where (((BLNG_GRP_ID\
=:b0 and BILL_TYPE_CODE=:b1) and TO_DATE(:b2,'DD/MM/YYYY') between NVL(EFF_FRO\
M_DATE,TO_DATE('01/01/1900','DD/MM/YYYY')) and NVL(EFF_TO_DATE,SYSDATE)) and A\
CC_ENTITY_CODE in (select ACC_ENTITY_CODE  from SY_ACC_ENTITY where ACC_ENTITY\
_ID=:b3))           ";

 static const char *sq0010 = 
"select INCOME_ACC_INT_CODE  from BL_TYPE_FOR_BLNG_SERV where (((BLNG_SERV_CO\
DE=:b0 and BILL_TYPE_CODE=:b1) and TO_DATE(:b2,'DD/MM/YYYY') between NVL(EFF_F\
...
BL_PATIENT_CHARGES_FOLIO_I1 +*/ ROWID  from BL_PATIENT_CHARGES_FOLIO where (((\
((((((((((TRX_DOC_REF=:b0 and TRX_DOC_REF_LINE_NUM=TRX_DOC_REF_LINE_NUM) and T\
RX_DOC_REF_SEQ_NUM=TRX_DOC_REF_SEQ_NUM) and NVL(TRX_POSTED_IND,'N')='N') and C\
ONFIRMED_YN='Y') and TRX_DATE<=TO_DATE((:b1||' 23:59'),'DD/MM/YYYY HH24:MI')) \
and NVL(TRX_FINALIZE_IND,'N')='Y') and EPISODE_TYPE in ('O','E')) and PATIENT_\
ID=PATIENT_ID) and EPISODE_ID=EPISODE_ID) and VISIT_ID=VISIT_ID) and AVAILED_I\
ND in ('Y','U','C')) and NVL(TRX_FINALIZE_IND,'X')='Y') and OPERATING_FACILITY\
_ID=:b2) for update of TRX_DOC_REF nowait ";
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,237,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
36,0,0,2,22,0,1,327,0,0,0,0,0,1,0,
...
120,0,0,6,155,0,4,407,0,0,7,4,0,1,0,2,3,0,0,2,3,0,0,2,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,
163,0,0,7,279,0,4,430,0,0,8,1,0,1,0,2,3,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,1,0,0,2,
1,0,0,2,1,0,0,1,9,0,0,
210,0,0,8,101,0,4,476,0,0,2,1,0,1,0,2,1,0,0,1,9,0,0,
233,0,0,9,2042,0,9,651,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
256,0,0,9,0,0,13,814,0,0,77,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
...
579,0,0,12,307,0,9,979,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
606,0,0,12,0,0,13,986,0,0,1,0,0,1,0,2,9,0,0,
625,0,0,13,304,0,9,1001,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
652,0,0,13,0,0,13,1021,0,0,7,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,
695,0,0,11,330,0,9,1053,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
726,0,0,11,0,0,13,1060,0,0,1,0,0,1,0,2,9,0,0,
745,0,0,13,304,0,9,1077,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
772,0,0,13,0,0,13,1097,0,0,7,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,
815,0,0,10,332,0,9,1125,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
846,0,0,10,0,0,13,1129,0,0,1,0,0,1,0,2,9,0,0,
865,0,0,13,304,0,9,1147,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
892,0,0,13,0,0,13,1167,0,0,7,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,
935,0,0,15,244,0,4,1215,0,0,5,3,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,
970,0,0,16,925,0,3,1228,0,0,27,27,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
...
1093,0,0,17,933,0,3,1318,0,0,27,27,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,
0,0,1,9,0,0,1,9,0,0,1,4,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,
1216,0,0,18,244,0,4,1427,0,0,5,3,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,
1251,0,0,19,925,0,3,1437,0,0,27,27,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,
0,0,1,9,0,0,1,9,0,0,1,4,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,
1374,0,0,20,30,0,43,1530,0,0,0,0,0,1,0,
1389,0,0,21,801,0,5,1537,0,0,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1424,0,0,14,664,0,9,1576,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
1451,0,0,22,0,0,29,1626,0,0,0,0,0,1,0,
1466,0,0,23,22,0,1,1635,0,0,0,0,0,1,0,
1481,0,0,24,685,0,5,1791,0,0,6,6,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,
};
...
struct { unsigned short len; unsigned char arr[30]; } nd_pre_doc_date;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "update BL_PARAMETERS  set LAST_FOLIO_POST_DATE=TO_DATE(:b0,'\
DD/MM/YYYY'),MODIFIED_BY_ID=USER,MODIFIED_DATE=SYSDATE,MODIFIED_AT_WS_NO=:b1,M\
ODIFIED_FACILITY_ID=:b2 where (NVL(LAST_FOLIO_POST_DATE,TO_DATE(:b0,'DD/MM/YYY\
Y'))<=TO_DATE(:b0,'DD/MM/YYYY') and operating_facility_id=:b2)";
 sqlstm.iters = (unsigned int  )1;
...
declare_cur()
{
    ...
    /* EXEC SQL DECLARE LOCK_TRX_REF_CUR CURSOR FOR
	     SELECT /o+INDEX(BL_PATIENT_CHARGES_FOLIO BL_PATIENT_CHARGES_FOLIO_I1) USE BL_PATIENT_CHARGES_FOLIO_I1o/
	     ROWID
...
,SERV_QTY ,ROUND(BASE_CHARGE_AMT,6) ,ORG_GROSS_CHARGE_AMT ,ORG_DISC_AMT ,ORG\
_NET_CHARGE_AMT ,UPD_GROSS_CHARGE_AMT ,UPD_DISC_AMT ,UPD_NET_CHARGE_AMT ,ADJ\
_GROSS_CHARGE_AMT ,ADJ_DISC_AMT ,ADJ_NET_CHARGE_AMT ,MODULE_ID ,SOURCE_DOC_R\
EF ,TO_CHAR(SOURCE_DOC_DATE,'DD/MM/YYYY') ,TO_CHAR(SERVICE_DATE,'DD/MM/YYYY'\
) ,TO_NUMBER(TO_CHAR(SERVICE_DATE,'MM')) ,TO_NUMBER(TO_CHAR(SERVICE_DATE,'YY\
YY')) ,ACCT_DEPT_CODE ,ORIG_DEPT_CODE ,NVL(PHYSICIAN_ID,'000000000000000') ,\
BLNG_GRP_ID ,BLNG_CLASS_CODE ,DAY_TYPE_CODE ,TIME_TYPE_CODE ,DISC_PERC ,ADDL\
_FACTOR_NUM ,NVL(BED_CLASS_CODE,'XX') BED_CLASS_CODE ,BILL_TYPE_CODE ,FOLIO_\
REMARKS ,DF_SERVICE_IND ,DF_UPD_FLAG ,DOC_TY");
     sqlstm.stmt = sq0009;
     sqlstm.iters = (unsigned int  )1;
...

{
     ...
     sqlstm.sqhstl[73] = (unsigned int  )32;
...
NTERFACE_DATE,GL_INTERFACE_FLAG,OPERATING_FACILITY_ID,CASH_CTRL_DOC_TYPE,CASH_\
CTRL_DOC_NUMBER,CASH_CTRL_DOC_DATE,ADDED_BY_ID,MODIFIED_BY_ID,ADDED_DATE,MODIF\
IED_DATE,ADDED_AT_WS_NO,ADDED_FACILITY_ID,MODIFIED_AT_WS_NO,MODIFIED_FACILITY_\
ID,CUST_CODE) values (:b0,:b1,:b2,:b3,:b4,:b5,:b6,nvl(:b7,TO_CHAR(TO_DATE(:b8,\
...
ll ,null ,'N',:b18,:b19,:b20,:b21,USER,USER,SYSDATE,SYSDATE,:b22,:b18,:b22,:b1\
8,:b26)";
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqadto[18] = (unsigned short )0;
 sqlstm.sqtdso[18] = (unsigned short )0;
 sqlstm.sqhstv[19] = (         void  *)&nd_pre_doc_type;
 sqlstm.sqhstl[19] = (unsigned int  )10;
 sqlstm.sqhsts[19] = (         int  )0;
 sqlstm.sqindv[19] = (         void  *)0;
 sqlstm.sqinds[19] = (         int  )0;
 sqlstm.sqharm[19] = (unsigned int  )0;
 sqlstm.sqadto[19] = (unsigned short )0;
 sqlstm.sqtdso[19] = (unsigned short )0;
 sqlstm.sqhstv[20] = (         void  *)&nd_pre_doc_num;
 ...
 sqlstm.sqadto[20] = (unsigned short )0;
 sqlstm.sqtdso[20] = (unsigned short )0;
 sqlstm.sqhstv[21] = (         void  *)&nd_pre_doc_date;
...
L_INTERFACE_DATE,GL_INTERFACE_FLAG,OPERATING_FACILITY_ID,CASH_CTRL_DOC_TYPE,CA\
SH_CTRL_DOC_NUMBER,CASH_CTRL_DOC_DATE,ADDED_BY_ID,MODIFIED_BY_ID,ADDED_DATE,MO\
DIFIED_DATE,ADDED_AT_WS_NO,ADDED_FACILITY_ID,MODIFIED_AT_WS_NO,MODIFIED_FACILI\
TY_ID,CUST_CODE) values (:b0,:b1,:b2,:b3,:b4,:b5,:b6,nvl(:b7,TO_CHAR(TO_DATE(:\
...
)* :b17),null ,null ,'N',:b18,:b19,:b20,:b21,USER,USER,SYSDATE,SYSDATE,:b22,:b\
18,:b22,:b18,:b26)";
    sqlstm.iters = (unsigned int  )1;
...
,null ,null ,'N',:b18,:b19,:b20,:b21,USER,USER,SYSDATE,SYSDATE,:b22,:b18,:b22,\
:b18,:b26)";
    sqlstm.iters = (unsigned int  )1;
...
{
     /* EXEC SQL UPDATE 
				 /o+INDEX(BL_PATIENT_CHARGES_FOLIO BL_PATIENT_CHARGES_FOLIO_I1) USE BL_PATIENT_CHARGES_FOLIO_I1o/ 
...

{
     ...
     sqlstm.sqladtp = &sqladt;
     sqlstm.sqltdsp = &sqltds;
     sqlstm.stmt = "update  /*+ INDEX(BL_PATIENT_CHARGES_FOLIO BL_PATIENT_CH\
ARGES_FOLIO_I1) USE BL_PATIENT_CHARGES_FOLIO_I1 +*/ BL_PATIENT_CHARGES_FOLIO  \
set DF_UPD_FLAG=DECODE(DF_SERVICE_IND,'D','Y',DF_UPD_FLAG),TRX_POSTED_IND='Y',\
POSTED_BY_ID=USER,POSTED_DATE=SYSDATE,MODIFIED_BY_ID=USER,MODIFIED_DATE=SYSDAT\
E,MODIFIED_AT_WS_NO=:b0,MODIFIED_FACILITY_ID=:b1,UNPOSTED_REASON=null  where (\
(((((((((((TRX_DOC_REF=:b2 and TRX_DOC_REF_LINE_NUM=TRX_DOC_REF_LINE_NUM) and \
TRX_DOC_REF_SEQ_NUM=TRX_DOC_REF_SEQ_NUM) and TRX_POSTED_IND='N') and TRX_DATE<\
=TO_DATE((:b3||' 23:59'),'DD/MM/YYYY HH24:MI')) and NVL(TRX_FINALIZE_IND,'N')=\
'Y') and CONFIRMED_YN='Y') and OPERATING_FACILITY_ID=:b1) and EPISODE_TYPE in \
('O','E')) and PATIENT_ID=PATIENT_ID) and EPISODE_ID=EPISODE_ID) and VISIT_ID=\
VISIT_ID) and AVAILED_IND in ('Y','U','C'))";
     sqlstm.iters = (unsigned int  )1;
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "update  /*+ INDEX(BL_PATIENT_CHARGES_FOLIO BL_PATIENT_CHAR\
GES_FOLIO_I1) USE BL_PATIENT_CHARGES_FOLIO_I1 +*/ BL_PATIENT_CHARGES_FOLIO  se\
t UNPOSTED_REASON=:b0,TRX_POSTED_IND='N',MODIFIED_BY_ID=user,MODIFIED_DATE=SYS\
DATE,MODIFIED_AT_WS_NO=:b1,MODIFIED_FACILITY_ID=:b2 where ((((((((TRX_DOC_REF=\
:b3 and TRX_DOC_REF_LINE_NUM=TRX_DOC_REF_LINE_NUM) and OPERATING_FACILITY_ID=:\
b2) and TRX_DOC_REF_SEQ_NUM=TRX_DOC_REF_SEQ_NUM) and NVL(TRX_POSTED_IND,'X')='\
N') and TRX_DATE<=TO_DATE((:b5||' 23:59'),'DD/MM/YYYY HH24:MI')) and NVL(TRX_F\
INALIZE_IND,'N')='Y') and CONFIRMED_YN='Y') and (EPISODE_TYPE not  in ('O','E'\
