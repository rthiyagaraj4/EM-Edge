...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLRDSPLT.PC"
};


static unsigned int sqlctx = 1288467387;
...
"3,'!!!!!!!!!!!!!!!!!!!!') and nvl(:b4,'~~~~~~~~~~~~~~~~~~~~')) and nvl(FINAL\
IZED_YN,'N')=Decode(:b5,'N','N',Decode(:b6,'BLTCVFNL','N','Y'))) and nvl(CANCE\
LLED_YN,'N')<>'Y')           ";
...
"select B.DOC_TYPE_CODE ,B.DOC_NUMBER ,((B.DOC_TYPE_CODE||'/')||TO_CHAR(B.DOC\
_NUMBER)) ,to_char(B.DOC_DATE,'DD/MM/YYYY') ,B.DOC_AMT ,substr(B.PRINT_TEXT,1,\
45)  from BL_CUST_DISPATCH_LET_DETAIL B where (((B.OPERATING_FACILITY_ID=:b0 a\
nd DOC_REF_NUM=:b1) and NVL(CANCELLED_YN,'N')<>'Y') and NVL(REJECTED_BY_PAYER_\
YN,'N')<>'Y') order by to_char(B.DOC_DATE,'DD/MM/YYYY'),B.DOC_TYPE_CODE,B.DOC_\
NUMBER            ";
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,233,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...
param6,'Y') ,nvl(param9,'BLRDSPLT') into :b0,:b1,:b2,:b3,:b4,:b5,:b6  from sy_\
prog_param where ((pgm_id=:b7 and session_id=:b8) and pgm_date=:b9)";
 sqlstm.iters = (unsigned int  )1;
...
l(NO_OF_DECIMAL,2) ,USER ,TO_CHAR(SYSDATE,'DD/MM/YYYY') into :b0,:b1,:b2,:b3,:\
b4,:b5  from sy_acc_entity_lang_vw where (acc_entity_id=:b6 and LANGUAGE_ID=:b\
7)";
 sqlstm.iters = (unsigned int  )1;
...

{
          ...
          sqlstm.sqladtp = &sqladt;
          sqlstm.sqltdsp = &sqltds;
          sqlstm.stmt = "select CONTACT_NAME ,CONTACT_NAME_TITLE ,NVL(DETAIL\
_DESC,LONG_NAME) into :b0,:b1,:b2  from AR_CUSTOMER_LANG_VW where (CUST_CODE=:\
...

{
        ...
        sqlstm.sqladtp = &sqladt;
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "select NVL(DETAIL_DESC,LONG_NAME) into :b0  from AR_C\
USTOMER_LANG_VW where (Cust_Code=:b1 and Language_Id=:b2)";
        sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select count(rowid) into :b0  from BL_CUST_DISPATCH_LET_DET\
AIL where ((OPERATING_FACILITY_ID=:b1 and DOC_REF_NUM=:b2) and (:b3='N' or (:b\
3='Y' and print_text is null )))";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select (count(rowid)* 2) into :b0  from BL_CUST_DISPATCH_LE\
T_DETAIL where ((OPERATING_FACILITY_ID=:b1 and DOC_REF_NUM=:b2) and (:b3='Y' a\
nd print_text is  not null ))";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRDSPLT.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "begin :no_of_lines := bl_spell_number . print_in_words ( :lv\
_facility_id , abs ( nvl ( :d_tot_outst_amt , 0 ) ) , :no_of_characts , :d_bas\
e_currency , :subcur , :l_decimal , :amt_in_words1 , :amt_in_words_Sep1 ) ; EN\
D ;";
 sqlstm.iters = (unsigned int  )1;
