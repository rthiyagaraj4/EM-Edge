...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR00097.PC"
};
...
"select a.BANK_CODE ,b.BANK_NAME ,a.BANK_IN_BATCH_DOC_TYPE ,a.BANK_IN_BATCH_D\
OC_NO ,a.BANK_IN_AMT ,((a.BANK_IN_BATCH_DOC_TYPE||'/')||a.BANK_IN_BATCH_DOC_NO\
) docno ,to_char(a.bank_in_date,'dd/mm/yyyy') ,a.bank_in_doc_no  from bl_bank_\
in_doc a ,sy_acc_entity c ,bl_bank_name b where ((((((a.operating_facility_id=\
:b0 and c.acc_entity_id=a.operating_facility_id) and a.bank_code=b.bank_code(+\
)) and nvl(a.bank_code,'!!!!')=:b1) and a.BANK_IN_BATCH_DOC_TYPE=decode(:b2,'!\
!!!',a.BANK_IN_BATCH_DOC_TYPE,:b2)) and to_char(a.BANK_IN_BATCH_DOC_NO)=decode\
(:b4,'!!!!',to_char(a.BANK_IN_BATCH_DOC_NO),:b4)) and a.BANK_IN_BATCH_DOC_DATE\
=:b6) order by a.SLMT_TYPE_CODE,a.BANK_IN_BATCH_DOC_DATE,a.BANK_IN_BATCH_DOC_T\
YPE,a.BANK_IN_BATCH_DOC_NO            ";

 static const char *sq0007 = 
"select a.BANK_IN_BATCH_DOC_TYPE ,a.BANK_IN_BATCH_DOC_NO ,a.BANK_IN_BATCH_DOC\
_DATE ,a.EXT_ACCT_CODE ,ACCT_CODE_AMT ,ACCT_DESC  from BL_BANK_IN_BY_ACCT_CODE\
 a ,BL_EXT_ACCT_CODE b ,sy_acc_entity c where (((((a.OPERATING_FACILITY_ID=c.a\
cc_entity_id and a.operating_facility_id=:b0) and b.ACC_ENTITY_CODE=c.acc_enti\
ty_code) and a.EXT_ACCT_CODE=b.ACCOUNT_CODE) and a.BANK_IN_BATCH_DOC_TYPE=:b1)\
 and to_char(a.BANK_IN_BATCH_DOC_NO)=:b2) order by EXT_ACCT_CODE            ";
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,107,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
36,0,0,2,135,0,4,147,0,0,7,3,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,
0,0,1,9,0,0,
79,0,0,3,84,0,2,169,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
106,0,0,4,73,0,4,179,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
129,0,0,5,0,0,30,204,0,0,0,0,0,1,0,
144,0,0,6,738,0,9,257,0,0,7,7,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,
187,0,0,6,0,0,15,265,0,0,0,0,0,1,0,
202,0,0,7,464,0,9,274,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,4,0,0,
229,0,0,7,0,0,15,282,0,0,0,0,0,1,0,
244,0,0,6,0,0,13,305,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,2,9,
0,0,2,9,0,0,2,9,0,0,
291,0,0,8,216,0,4,338,0,0,5,3,0,1,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,1,4,0,0,
326,0,0,9,212,0,4,358,0,0,4,2,0,1,0,2,9,0,0,2,9,0,0,1,9,0,0,1,4,0,0,
357,0,0,7,0,0,13,386,0,0,6,0,0,1,0,2,9,0,0,2,4,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,9,
0,0,
396,0,0,10,192,0,4,532,0,0,7,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,1,9,0,0,
439,0,0,11,96,0,4,683,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
462,0,0,12,162,0,6,690,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
};
...

{
  ...
  from SY_PROG_PARAM where ((PGM_ID=:b4 and SESSION_ID=:b5) and PGM_DATE=:b6)";
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select to_char(min(doc_date),'dd/mm/yyyy') ,to_char(max(doc_\
date),'dd/mm/yyyy') into :b0,:b1  from bl_receipt_refund_dtl where ((operating\
_facility_id=:b2 and bank_in_batch_doc_type=:b3) and bank_in_batch_doc_no=:b4)";
 sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select distinct s.SLMT_TYPE_CODE ,LONG_DESC into :b0,:b1  f\
rom bl_slmt_type s ,BL_BANK_IN_BY_ACCT_CODE b where ((b.SLMT_TYPE_CODE=s.SLMT_\
TYPE_CODE and b.BANK_IN_BATCH_DOC_TYPE=:b2) and b.BANK_IN_BATCH_DOC_NO=:b3)";
  sqlstm.iters = (unsigned int  )1;
...
_CHAR(SYSDATE,'DD/MM/RRRR:HH24:MI:SS') ,USER ,tel_num ,fax_num into :b0,:b1,:b\
2,:b3,:b4,:b5  from SY_ACC_ENTITY where ACC_ENTITY_ID=:b6";
    sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00097.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
