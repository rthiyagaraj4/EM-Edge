...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/Blrsrind.pc"
};


static unsigned int sqlctx = 1288468923;
...
"')) order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE,TRUNC(TRX_DATE),B.TR\
X_DOC_REF            ";

 static const char *sq0007 = 
"_ID,A.CUST_CODE,B.BLNG_SERV_CODE,TRUNC(TRX_DATE),B.TRX_DOC_REF            ";

 static const char *sq0008 = 
"cility_id,'X')=NVL(:b13,'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV\
_CODE,TRUNC(TRX_DATE),B.TRX_DOC_REF            ";

 static const char *sq0009 = 
",'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE,TRUNC(TRX_DATE),B\
.TRX_DOC_REF            ";
...
"select decode(:b0,'en',SHORT_NAME,SHORT_NAME_LOC_LANG)  from MP_PATIENT_MAST\
 where PATIENT_ID=:b1           ";
...
138,0,0,0,0,0,27,319,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...
_CHARGES_FOLIO B where ((((((((((((((((A.CUST_CODE=:b1 and A.BLNG_GRP_ID=:b2) \
and A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE) and A.DOC_NUM=B.BILL_DOC_NUM) and B.\
BILL_DOC_TYPE_CODE is  not null ) and B.BILL_DOC_NUM is  not null ) and A.PATI\
...
L(:b13,'X')) order by A.CUST_CODE,A.BLNG_GRP_ID,B.BLNG_SERV_CODE ";
    sqlstm.iters = (unsigned int  )1;
...
HARGES_FOLIO B where (((((((((((((((A.BLNG_GRP_ID=:b1 and A.DOC_TYPE_CODE=B.BI\
LL_DOC_TYPE_CODE) and A.DOC_NUM=B.BILL_DOC_NUM) and B.BILL_DOC_TYPE_CODE is  n\
ot null ) and B.BILL_DOC_NUM is  not null ) and A.PATIENT_ID between :b2 and :\
b3) and A.DOC_TYPE_CODE between :b4 and :b5) and A.DOC_NUM between :b6 and :b7\
) and TRUNC(A.DOC_DATE) between to_date(:b8,'DD/MM/YYYY') and to_date(:b9,'DD/\
MM/YYYY')) and B.BLNG_SERV_CODE between :b10 and :b11) and NVL(B.PACKAGE_TRX_I\
ND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C') \
and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(a.operating_facility_id,'X')=N\
VL(:b12,'X')) and NVL(b.operating_facility_id,'X')=NVL(:b12,'X')) order by A.C\
UST_CODE,A.BLNG_GRP_ID,B.BLNG_SERV_CODE ";
  sqlstm.iters = (unsigned int  )1;
...
NT_CHARGES_FOLIO B where (((((((((((((((((A.CUST_CODE=:b1 and A.BLNG_GRP_ID=:b\
2) and A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE) and A.DOC_NUM=B.BILL_DOC_NUM) and\
 B.BILL_DOC_TYPE_CODE is  not null ) and B.BILL_DOC_NUM is  not null ) and A.E\
PISODE_TYPE=:b3) and A.PATIENT_ID between :b4 and :b5) and A.DOC_TYPE_CODE bet\
ween :b6 and :b7) and A.DOC_NUM between :b8 and :b9) and TRUNC(A.DOC_DATE) bet\
ween to_date(:b10,'DD/MM/YYYY') and to_date(:b11,'DD/MM/YYYY')) and B.BLNG_SER\
V_CODE between :b12 and :b13) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.BI\
LL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE_C\
ODE,'X')<>'I') and NVL(a.operating_facility_id,'X')=NVL(:b14,'X')) and NVL(b.o\
perating_facility_id,'X')=NVL(:b14,'X')) order by A.CUST_CODE,A.BLNG_GRP_ID,B.\
BLNG_SERV_CODE ";
      sqlstm.iters = (unsigned int  )1;
...
=B.BILL_DOC_TYPE_CODE) and A.DOC_NUM=B.BILL_DOC_NUM) and B.BILL_DOC_TYPE_CODE \
is  not null ) and B.BILL_DOC_NUM is  not null ) and A.EPISODE_TYPE=:b2) and A\
.PATIENT_ID between :b3 and :b4) and A.DOC_TYPE_CODE between :b5 and :b6) and \
A.DOC_NUM between :b7 and :b8) and TRUNC(A.DOC_DATE) between to_date(:b9,'DD/M\
M/YYYY') and to_date(:b10,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between :b11 and\
 :b12) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') an\
d NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(\
a.operating_facility_id,'X')=NVL(:b13,'X')) and NVL(b.operating_facility_id,'X\
')=NVL(:b13,'X')) order by A.CUST_CODE,A.BLNG_GRP_ID,B.BLNG_SERV_CODE ";
      sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRSRIND.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
