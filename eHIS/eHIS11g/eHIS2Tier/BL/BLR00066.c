...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR00066.PC"
};


static unsigned int sqlctx = 1288460971;
...
"select a.patient_id ,c.short_name patient_name ,((a.doc_type_code||'-')||to_\
char(a.doc_number)) rcpt_no ,to_char(a.doc_date,'DD/MM/YYYY') rcpt_date ,a.doc\
_amt Deposit_paid ,a.EPISODE_TYPE ,a.EPISODE_ID ,a.VISIT_ID  from BL_RECEIPT_R\
EFUND_HDRDTL_VW a ,BL_PATIENT_LEDGER B ,MP_PATIENT_MAST C where (((((((((a.REC\
PT_REFUND_IND='R' and B.ORG_DOC_TYPE_CODE=A.DOC_TYPE_CODE) and B.ORG_DOC_NUM=A\
.DOC_NUMBER) and A.RECPT_NATURE_CODE in ('AD','PR')) and B.AD_PR_ADJ_BILL_DOC_\
TYPE is null ) and B.DOC_OUTST_AMT<0) and to_char(a.DOC_DATE,'YYYY/MM/DD') bet\
ween to_char(TO_DATE(:b0,'DD/MM/YYYY'),'YYYY/MM/DD') and to_char(TO_DATE(:b1,'\
DD/MM/YYYY'),'YYYY/MM/DD')) and a.EPISODE_TYPE=decode(:b2,'A',a.episode_type,:\
b2)) and NVL(a.CANCELLED_IND,'N')<>'Y') and a.PATIENT_ID=c.PATIENT_ID) order b\
y 1,2            ";
...
R(BILL_DOC_NUMBER))=:b0) and NVL(CANCELLED_IND,'N')<>'Y') order by 1          \
  ";
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,110,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...

{
 ...
 :b0,:b1,:b2,:b3  from SY_PROG_PARAM where ((PGM_ID=:b4 and SESSION_ID=:b5) an\
d PGM_DATE=:b6)";
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00066.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "select to_char(VISIT_REGN_DATE_TIME,'DD/MM/YYYY') into :b0\
  from OP_VISIT where ((facility_id=:b1 and episode_id=:b2) and visit_id=:b3)";
   sqlstm.iters = (unsigned int  )1;
