...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/blr00078.pc"
};
...
|' ')||'(')||a.RECOUPED_VR_NO)||')') ,'251' LEGEND_DEPT ,a.DOC_AMT ,to_char(a.\
DOC_DATE,'dd/mm/yyyy')  from BL_CASH_REFUND_CYCLE_TRN a ,BL_CASH_REFUND_CASHBO\
OK_CB B where ((((((a.operating_facility_id=:b0 and a.operating_facility_id=b.\
operating_facility_id) and a.RECOUPED_VR_NO=b.CB_RECOUPED_VR_NO) and a.OB_TRN_\
YN='N') and a.RECOUPED_VR_NO is  not null ) and trunc(b.CB_DATE)<=trunc(to_dat\
e(:b1,'DD/MM/YYYY'))) and (nvl(a.CHQ_RECEIVED_YN,'N')='N' or (nvl(a.CHQ_RECEIV\
ED_YN,'N')='Y' and trunc(a.CHQ_RECEIVED_DATE)>trunc(to_date(:b1,'DD/MM/YYYY'))\
)))           ";

 static const char *sq0008 = 
"select to_char(VR_DATE,'dd/mm/yyyy') ,((((to_char(VR_DATE,'dd/mm/yyyy')||' '\
)||'(')||RECOUPED_VR_NO)||')') vno ,((((('('||' ')||chq_no)||' ')||TO_CHAR(chq\
_date,'DD\\MM\\YYYY'))||')') chkno ,'251' LEGEND_DEPT ,DOC_AMT ,DOC_DATE  from\
 bl_cash_refund_cycle_trn where (((operating_facility_id=:b0 and nvl(CHQ_RECEI\
VED_YN,'N')='Y') and CHQ_RECEIVED_DATE<=trunc(to_date(:b1,'DD/MM/YYYY'))) and \
(nvl(CHQ_CLEARED_YN,'N')='N' or (nvl(CHQ_CLEARED_YN,'N')='Y' and CHQ_CLEARED_D\
ATE>=trunc(to_date(:b1,'DD/MM/YYYY')))))           ";

 static const char *sq0009 = 
"select to_char(doc_date,'dd/mm/yyyy') ,null  ,'251' LEGEND_DEPT ,abs(doc_amt\
) ,null   from bl_recpt_refund_can_bou_dtl_vw where ((((operating_facility_id=\
:b0 and doc_date>=TO_DATE((NVL(:b1,'01/01/1900')||' 00:00:00'),'DD/MM/YYYY HH2\
4:MI:SS')) and doc_date<=NVL(TO_DATE((:b2||' 23:59:59'),'DD/MM/YYYY HH24:MI:SS\
'),SYSDATE)) and RECPT_REFUND_IND='F') and cash_slmt_flag='Y') order by doc_da\
te            ";
...
468,0,0,0,0,0,27,1029,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...

{
 ...
 ((PGM_ID=:b2 and SESSION_ID=:b3) and PGM_DATE=:b4)";
 sqlstm.iters = (unsigned int  )1;
...
,CB_CHQ_IN_HAND_AMT ,CB_CASH_IN_HAND_AMT into :b0,:b1,:b2,:b3,:b4  from BL_CAS\
H_REFUND_CASHBOOK_CB where to_char(CB_DATE,'DD/MM/YYYY')=:b5";
 sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select to_char((to_date(:b0,'dd/mm/yyyy')+1),'dd/mm/yyyy') i\
nto :b1  from dual ";
 sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select sum(abs(doc_amt)) into :b0  from bl_recpt_refund_can_\
bou_dtl_vw where ((((operating_facility_id=:b1 and doc_date>=TO_DATE((NVL(:b2,\
'01/01/1900')||' 00:00:00'),'DD/MM/YYYY HH24:MI:SS')) and doc_date<=NVL(TO_DAT\
E((:b3||' 23:59:59'),'DD/MM/YYYY HH24:MI:SS'),SYSDATE)) and RECPT_REFUND_IND='\
F') and cash_slmt_flag='Y')";
 sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select acc_entity_name ,BASE_CURRENCY ,BASE_CURRENCY_DEC in\
to :b0,:b1,:b2  from sy_acc_entity where acc_entity_id=:b3";
  sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select acc_entity_name_loc_lang ,BASE_CURRENCY into :b0,:b1\
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00078.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
