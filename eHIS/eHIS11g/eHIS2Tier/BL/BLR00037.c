...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR00037.PC"
};
...
"select to_char(A.BOUNCED_DATE,'dd/mm/yyyy') DOCDATE ,A.DOC_TYPE_CODE ,A.DOC_\
NUMBER DOC_NO ,A.DOC_SRNO ,b.doc_amt AMOUNT ,EXT_ACCT_CODE ACC_CODE  from bl_c\
ancelled_bounced_trx a ,bl_receipt_refund_dtl_VW b where (((((((((a.doc_type_c\
ode=b.doc_type_code and a.doc_number=b.doc_number) and a.doc_srno=b.doc_srno) \
and b.RECPT_REFUND_IND='R') and NVL(b.cancelled_ind,'N')='N') and NVL(A.cancel\
led_ind,'N')='N') and NVL(A.BOUNCED_YN,'N')='Y') and b.CASH_SLMT_FLAG not  in \
('A')) and to_char(a.BOUNCED_DATE,'YYYYMM')=to_char(to_date(:b0,'YYYYMM'),'YYY\
YMM')) and A.CASH_COUNTER_FACILITY_ID=:b1) order by 1            ";
...
ummary_tran_by_acct a ,BL_EXT_ACCT_CODE b where (((a.ext_acct_code=b.account_c\
ode and OPERATING_FACILITY_ID=:b0) and to_char(trx_date,'YYYYMM')<=to_char(to_\
date(:b1,'YYYYMM'),'YYYYMM')) and to_char(trx_date,'YYYY')=to_char(to_date(:b1\
,'YYYYMM'),'YYYY')) group by nvl(b.cashbk_main_acct_seq,9) order by 3         \
   ";
...
om bl_summary_tran_by_acct a ,BL_EXT_ACCT_CODE b where ((a.ext_acct_code=b.acc\
ount_code and OPERATING_FACILITY_ID=:b0) and to_char(trx_date,'YYYYMM')=to_cha\
r(to_date(:b1,'yyyymm'),'YYYYMM')) group by nvl(b.cashbk_main_acct_seq,9) orde\
r by 3            ";

 static const char *sq0007 = 
"select EXT_ACCT_CODE ,sum(NVL(B.DOC_AMT,0))  from bl_cancelled_bounced_trx a\
 ,bl_receipt_refund_dtl_VW b where (((((((((a.doc_type_code=b.doc_type_code an\
d a.doc_number=b.doc_number) and a.doc_srno=b.doc_srno) and b.RECPT_REFUND_IND\
='R') and NVL(b.cancelled_ind,'N')='N') and NVL(A.cancelled_ind,'N')='N') and \
NVL(A.BOUNCED_YN,'N')='Y') and b.CASH_SLMT_FLAG not  in ('A')) and to_char(a.B\
OUNCED_DATE,'YYYYMM')=to_char(to_date(:b0,'YYYYMM'),'YYYYMM')) and A.CASH_COUN\
TER_FACILITY_ID=:b1) group by ext_acct_code order by 1 desc             ";
...
{12,4274,178,0,0,
5,0,0,0,0,0,27,208,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
36,0,0,2,0,0,30,256,0,0,0,0,0,1,0,
51,0,0,8,96,0,4,342,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
74,0,0,9,162,0,6,349,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
105,0,0,3,128,0,9,368,0,0,0,0,0,1,0,
120,0,0,4,609,0,9,372,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
143,0,0,6,547,0,9,376,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
170,0,0,5,484,0,9,380,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
193,0,0,7,538,0,9,384,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
216,0,0,3,0,0,13,395,0,0,2,0,0,1,0,2,9,0,0,2,3,0,0,
239,0,0,5,0,0,13,413,0,0,3,0,0,1,0,2,4,0,0,2,4,0,0,2,3,0,0,
266,0,0,6,0,0,13,455,0,0,3,0,0,1,0,2,4,0,0,2,4,0,0,2,3,0,0,
293,0,0,4,0,0,13,480,0,0,6,0,0,1,0,2,9,0,0,2,9,0,0,2,3,0,0,2,3,0,0,2,4,0,0,2,9,
0,0,
332,0,0,7,0,0,13,501,0,0,2,0,0,1,0,2,9,0,0,2,4,0,0,
355,0,0,10,69,0,4,989,0,0,2,1,0,1,0,1,9,0,0,2,9,0,0,
378,0,0,11,174,0,4,1053,0,0,6,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
1,9,0,0,
417,0,0,12,129,0,6,1078,0,0,1,1,0,1,0,2,3,0,0,
436,0,0,13,141,0,4,1257,0,0,2,1,0,1,0,2,9,0,0,1,3,0,0,
};
...
struct { unsigned short len; unsigned char arr[20]; } nd_forthe_month;
...
struct { unsigned short len; unsigned char arr[11]; } bl_ext_acct_dept_code;
...
struct { unsigned short len; unsigned char arr[10]; } l_month_year;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLR00037.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
...

{
    ...
    sqlstm.occurs = (unsigned int  )0;
    sqlstm.sqcmod = (unsigned int )0;
    sqlstm.sqhstv[0] = (         void  *)&nd_forthe_month;
...

{
 ...
 sqlstm.sqadto[0] = (unsigned short )0;
 sqlstm.sqtdso[0] = (unsigned short )0;
 sqlstm.sqhstv[1] = (         void  *)&nd_forthe_month;
 ...
 sqlstm.sqadto[1] = (unsigned short )0;
 sqlstm.sqtdso[1] = (unsigned short )0;
 sqlstm.sqhstv[2] = (         void  *)&nd_forthe_month;
...

{
      ...
      sqlstm.sqadto[2] = (unsigned short )0;
      sqlstm.sqtdso[2] = (unsigned short )0;
      sqlstm.sqhstv[3] = (         void  *)&bl_doc_srno;
      ...
      sqlstm.sqhstv[5] = (         void  *)&bl_acct_code;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select to_char(to_date(:b0,'yyyymm'),'Mon.YYYY') into :b1  f\
rom DUAL ";
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqadto[0] = (unsigned short )0;
 sqlstm.sqtdso[0] = (unsigned short )0;
 sqlstm.sqhstv[1] = (         void  *)&l_month_year;
