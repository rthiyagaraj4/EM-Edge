...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/Blrdlrev.pc"
};
...
"select NVL(A.ORG_NET_CHARGE_AMT,0) ,A.BLNG_GRP_ID ,TO_CHAR(A.TRX_DATE,'YYYYM\
MDD') ,A.EPISODE_TYPE ,A.OPERATING_FACILITY_ID  from BL_PATIENT_CHARGES_FOLIO \
A ,BL_BLNG_PRT_GRP_HDR B where (((((A.OPERATING_FACILITY_ID between NVL(:b0,'!\
!') and NVL(:b1,'~~') and A.TRX_DATE between TO_DATE(:b2,'YYYYMMDD') and TO_DA\
TE(:b3,'YYYYMMDD')) and NVL(A.TRX_FINALIZE_IND,'N')='Y') and NVL(A.CONFIRMED_Y\
N,'N')='Y') and A.PRT_GRP_HDR_CODE=B.PRT_GRP_HDR_CODE) and NVL(B.INCOME_YN,'N'\
)='Y') order by A.OPERATING_FACILITY_ID,EPISODE_TYPE,BLNG_GRP_ID,TRUNC(TRX_DAT\
E)            ";
...

{
 ...
 ,PARAM5 ,PARAM6 ,PARAM7 ,PARAM8 ,PARAM9 ,PARAM16 ,PARAM17 into :b0,:b1,:b2,:b\
3,:b4,:b5,:b6,:b7,:b8,:b9,:b10,:b11  from SY_PROG_PARAM where ((PGM_ID='BLRDLR\
EV' and PGM_DATE=:b12) and SESSION_ID=TO_NUMBER(:b13))";
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "delete  from SY_PROG_PARAM  where ((PGM_ID='BLRDLREV' and\
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "select ACC_ENTITY_NAME into :b0  from SY_ACC_ENTITY_lang_v\
w where (ACC_ENTITY_ID=:b1 and language_id=:b2)";
   sqlstm.iters = (unsigned int  )1;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select TO_CHAR(TO_DATE(:b0,'YYYYMMDD'),'DD/MM/YYYY') into :b\
1  from dual ";
 sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRDLREV.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
...
data where (language_id='en' and Pk_value=:b1)";
  sqlstm.iters = (unsigned int  )1;
