...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/Blrmncss.pc"
};


static unsigned int sqlctx = 1288467907;
...
'Y',DECODE(A.SLMT_TYPE_CODE,'CO',0,DOC_AMT),0)) CASH ,sum(DECODE(B.CASH_SLMT_F\
LAg,'B',DOC_AMT,0)) CHEQUE ,sum(DECODE(B.CASH_SLMT_FLAG,'C',DOC_AMT,0)) AUTOPA\
Y ,sum(DECODE(B.CASH_SLMT_FLAG,'A',DOC_AMT,0)) CREDIT ,sum(DECODE(A.SLMT_TYPE_\
CODE,'CO',DOC_AMT,0)) CO ,CASH_COUNTER_CODE ,CASH_COUNTER_FACILITY_ID ,OPERATI\
NG_FACILITY_ID  from BL_RECEIPT_REFUND_HDRDTL_VW A ,BL_SLMT_TYPE_LANG_VW B whe\
re (((((A.SLMT_TYPE_CODE=B.SLMT_TYPE_CODE(+) and language_id=:b0) and TRUNC(A.\
DOC_DATE) between TO_DATE(:b1,'DD/MM/YYYY') and TO_DATE(:b2,'DD/MM/YYYY')) and\
 NVL(A.CANCELLED_IND,'N')<>'Y') and CASH_COUNTER_FACILITY_ID between NVL(:b3,'\
  ') and NVL(:b4,'~~')) and CASH_COUNTER_CODE between NVL(:b5,'  ') and NVL(:b\
6,'~~')) group by A.CASH_COUNTER_FACILITY_ID,A.CASH_COUNTER_CODE,A.OPERATING_F\
ACILITY_ID,TRUNC(A.DOC_DATE)           ";
...
9,0,0,1,9,0,0,
231,0,0,6,158,0,4,574,0,0,5,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,9,0,0,
266,0,0,7,96,0,4,617,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
289,0,0,8,165,0,6,624,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
320,0,0,9,143,0,6,645,0,0,3,3,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,
347,0,0,10,145,0,6,719,0,0,3,3,0,1,0,2,9,0,0,1,9,0,0,1,9,0,0,
};
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRMNCSS.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
...
_2t ( to_date ( :nd_temp_date1 , 'DD/MM/YYYY HH24:MI:SS' ) , :p_language_id ) \
; END ;";
 sqlstm.iters = (unsigned int  )1;
