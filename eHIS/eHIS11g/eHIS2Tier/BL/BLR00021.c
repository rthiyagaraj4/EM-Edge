...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/BLR00021.PC"
};
...
"operating_facility_id=:b0) and d.adjust_exempt_ind=:b1) and c.reason_code be\
tween nvl(:b2,'!!!!') and nvl(:b3,'~~~~')) and :b5='Y') and to_char(a.cancelle\
d_date,'MM/YYYY')=:b4) order by r_code,d_date            ";
...
129,0,0,6,2259,0,9,310,0,0,14,14,0,1,0,1,9,0,0,1,1,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,1,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,1,0,0,1,9,0,0,1,9,0,0,1,1,0,0,1,9,0,0,
200,0,0,5,468,0,9,318,0,0,8,8,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,1,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,1,0,0,
247,0,0,6,0,0,15,326,0,0,0,0,0,1,0,
262,0,0,6,0,0,13,356,0,0,12,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,9,0,0,
325,0,0,5,0,0,13,392,0,0,1,0,0,1,0,2,9,0,0,
344,0,0,7,206,0,4,827,0,0,7,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,
9,0,0,1,9,0,0,
387,0,0,8,132,0,4,857,0,0,3,2,0,1,0,1,9,0,0,2,9,0,0,1,9,0,0,
414,0,0,9,176,0,4,881,0,0,4,2,0,1,0,2,9,0,0,2,3,0,0,1,9,0,0,1,9,0,0,
445,0,0,10,489,0,6,895,0,0,4,4,0,1,0,1,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
476,0,0,11,96,0,4,984,0,0,2,1,0,1,0,1,3,0,0,2,9,0,0,
499,0,0,12,162,0,6,991,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,1,9,0,0,
530,0,0,13,199,0,6,1040,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,
557,0,0,14,193,0,6,1062,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,2,9,0,0,
};
...
d a.operating_facility_id=:b0) and a.adjust_exempt_ind=:b1) and c.reason_cod\
e between nvl(:b2,'!!!!') and nvl(:b3,'~~~~')) and to_char(a.doc_date,'MM/YY\
YY')=:b4) and :b5='N') and  not exists (select 'x'  from bl_bill_adj_dtl_can\
c where ((((operating_facility_id=:b0 an");
 sqlbuft((void **)0, 
   "d a.doc_type_code=doc_type_code) and a.doc_num=doc_num) and a.doc_slno=d\
oc_slno) and to_char(cancelled_date,'MM/YYYY')=:b4))) union select a.cancell\
ed_date d_date ,to_char(a.cancelled_date,'DD/MM/YYYY') doc_date ,c.reason_co\
de r_code ,b.patient_id patient_id ,b.episode_type episode_type ,((b.episode\
_id||'/')||to_char(b.visit_id)) enctr_id ,((adj_doc_type_code||'/')||to_char\
(adj_doc_num)) document ,to_char(adj_doc_date,'DD/MM/YYYY') a_date ,nvl(d.do\
c_amt,0) doc_amt ,nvl(b.bill_tot_amt,0) tot_amt ,(nvl(b.bill_paid_amt,0)-nvl\
(b.tot_refund_amt,0)) paid_amt ,a.added_by_id added_by  from bl_bill_adj_dtl\
_canc a ,BL_BILL_DCP_DTL_VW b ,bl_bill_adj_hdr_canc c ,bl_bill_adj_dtl d whe\
re (((((((((((((d.adj_doc_type_code=b.doc_type_code and d.adj_doc_num=b.doc_\
num) and a.operating_facility_id=c.operating_facility_id) and a.doc_type_cod\
e=c.doc_type_code) and a.doc_num=c.doc_num) and a.operating_facility_id=d.op\
erating_facility_id) and a.doc_type_code=d.doc_type_code) and a.doc_num=d.do\
c_num) and a.doc_slno=d.doc_slno) and a.");
 sqlstm.stmt = sq0006;
 sqlstm.iters = (unsigned int  )1;
