...
{
    41,
    "Y:\\trunk\\eHIS11g\\eHIS2Tier/BL/blrsrins.pc"
};


static unsigned int sqlctx = 1288469043;
...
"select A.CUST_CODE ,A.BLNG_GRP_ID ,SUBSTR(B.BLNG_SERV_CODE,1,2) ,SUBSTR(B.BL\
NG_SERV_CODE,1,3) ,B.BLNG_SERV_CODE ,B.ORG_GROSS_CHARGE_AMT ,B.ORG_DISC_AMT ,B\
.ORG_NET_CHARGE_AMT  from BL_BILL_HDR A ,BL_PATIENT_CHARGES_FOLIO B where ((((\
((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and A.DOC_NUM=B.BILL_DOC_NUM) a\
nd A.BLNG_GRP_ID=:b0) and A.CUST_CODE=:b1) and A.PATIENT_ID between :b2 and :b\
3) and A.DOC_TYPE_CODE between :b4 and :b5) and A.DOC_NUM between :b6 and :b7)\
 and TRUNC(A.DOC_DATE) between to_date(:b8,'DD/MM/YYYY') and to_date(:b9,'DD/M\
M/YYYY')) and B.BLNG_SERV_CODE between :b10 and :b11) and NVL(B.PACKAGE_TRX_IN\
D,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C') a\
nd NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(a.operating_facility_id,'X')=NV\
L(:b12,'X')) and NVL(b.operating_facility_id,'X')=NVL(:b12,'X')) order by A.BL\
NG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE            ";

 static const char *sq0007 = 
"select A.CUST_CODE ,A.BLNG_GRP_ID ,SUBSTR(B.BLNG_SERV_CODE,1,2) ,SUBSTR(B.BL\
NG_SERV_CODE,1,3) ,B.BLNG_SERV_CODE ,B.ORG_GROSS_CHARGE_AMT ,B.ORG_DISC_AMT ,B\
.ORG_NET_CHARGE_AMT  from BL_BILL_HDR A ,BL_PATIENT_CHARGES_FOLIO B where ((((\
(((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and A.DOC_NUM=B.BILL_DOC_NUM) an\
d A.BLNG_GRP_ID=:b0) and A.PATIENT_ID between :b1 and :b2) and A.DOC_TYPE_CODE\
 between :b3 and :b4) and A.DOC_NUM between :b5 and :b6) and TRUNC(A.DOC_DATE)\
 between to_date(:b7,'DD/MM/YYYY') and to_date(:b8,'DD/MM/YYYY')) and B.BLNG_S\
ERV_CODE between :b9 and :b10) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.B\
ILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE_\
CODE,'X')<>'I') and NVL(a.operating_facility_id,'X')=NVL(:b11,'X')) and NVL(b.\
operating_facility_id,'X')=NVL(:b11,'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,B\
.BLNG_SERV_CODE            ";
...
and A.BLNG_GRP_ID=:b0) and A.CUST_CODE=:b1) and A.EPISODE_TYPE=:b2) and A.PATI\
ENT_ID between :b3 and :b4) and A.DOC_TYPE_CODE between :b5 and :b6) and A.DOC\
_NUM between :b7 and :b8) and TRUNC(A.DOC_DATE) between to_date(:b9,'DD/MM/YYY\
Y') and to_date(:b10,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between :b11 and :b12\
) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and NVL\
(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(a.ope\
rating_facility_id,'X')=NVL(:b13,'X')) and NVL(b.operating_facility_id,'X')=NV\
L(:b13,'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE            ";
...
.ORG_NET_CHARGE_AMT  from BL_BILL_HDR A ,BL_PATIENT_CHARGES_FOLIO B where ((((\
((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and A.DOC_NUM=B.BILL_DOC_NUM) a\
nd A.BLNG_GRP_ID=:b0) and A.EPISODE_TYPE=:b1) and A.PATIENT_ID between :b2 and\
 :b3) and A.DOC_TYPE_CODE between :b4 and :b5) and A.DOC_NUM between :b6 and :\
b7) and TRUNC(A.DOC_DATE) between to_date(:b8,'DD/MM/YYYY') and to_date(:b9,'D\
D/MM/YYYY')) and B.BLNG_SERV_CODE between :b10 and :b11) and NVL(B.PACKAGE_TRX\
_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C'\
) and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(a.operating_facility_id,'X')\
=NVL(:b12,'X')) and NVL(b.operating_facility_id,'X')=NVL(:b12,'X')) order by A\
.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE            ";
...
138,0,0,0,0,0,27,330,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
...
7  from SY_PROG_PARAM where ((PGM_ID='BLRSRINC' and SESSION_ID=:b18) and PGM_D\
ATE=:b19)";
...
_CHARGES_FOLIO B where ((((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and \
A.DOC_NUM=B.BILL_DOC_NUM) and A.BLNG_GRP_ID=:b1) and A.CUST_CODE=:b2) and A.PA\
TIENT_ID between :b3 and :b4) and A.DOC_TYPE_CODE between :b5 and :b6) and A.D\
OC_NUM between :b7 and :b8) and TRUNC(A.DOC_DATE) between to_date(:b9,'DD/MM/Y\
YYY') and to_date(:b10,'DD/MM/YYYY')) and B.BLNG_SERV_CODE between :b11 and :b\
12) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and N\
VL(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NVL(a.o\
perating_facility_id,'X')=NVL(:b13,'X')) and NVL(b.operating_facility_id,'X')=\
NVL(:b13,'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE ";
    sqlstm.iters = (unsigned int  )1;
...
HARGES_FOLIO B where (((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE and A.D\
OC_NUM=B.BILL_DOC_NUM) and A.BLNG_GRP_ID=:b1) and A.PATIENT_ID between :b2 and\
 :b3) and A.DOC_TYPE_CODE between :b4 and :b5) and A.DOC_NUM between :b6 and :\
b7) and TRUNC(A.DOC_DATE) between to_date(:b8,'DD/MM/YYYY') and to_date(:b9,'D\
D/MM/YYYY')) and B.BLNG_SERV_CODE between :b10 and :b11) and NVL(B.PACKAGE_TRX\
_IND,'X')='N') and NVL(A.BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C'\
...
NT_CHARGES_FOLIO B where (((((((((((((((A.DOC_TYPE_CODE=B.BILL_DOC_TYPE_CODE a\
nd A.DOC_NUM=B.BILL_DOC_NUM) and A.BLNG_GRP_ID=:b1) and A.CUST_CODE=:b2) and A\
.EPISODE_TYPE=:b3) and A.PATIENT_ID between :b4 and :b5) and A.DOC_TYPE_CODE b\
etween :b6 and :b7) and A.DOC_NUM between :b8 and :b9) and TRUNC(A.DOC_DATE) b\
etween to_date(:b10,'DD/MM/YYYY') and to_date(:b11,'DD/MM/YYYY')) and B.BLNG_S\
ERV_CODE between :b12 and :b13) and NVL(B.PACKAGE_TRX_IND,'X')='N') and NVL(A.\
BILL_STATUS,'X')<>'C') and NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE\
_CODE,'X')<>'I') and NVL(a.operating_facility_id,'X')=NVL(:b14,'X')) and NVL(b\
.operating_facility_id,'X')=NVL(:b14,'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,\
B.BLNG_SERV_CODE ";
      sqlstm.iters = (unsigned int  )1;
...
d A.DOC_NUM=B.BILL_DOC_NUM) and A.BLNG_GRP_ID=:b1) and A.EPISODE_TYPE=:b2) and\
...
and NVL(B.trx_status,'~')<>'C') and NVL(A.BILL_TRX_TYPE_CODE,'X')<>'I') and NV\
L(a.operating_facility_id,'X')=NVL(:b13,'X')) and NVL(b.operating_facility_id,\
'X')=NVL(:b13,'X')) order by A.BLNG_GRP_ID,A.CUST_CODE,B.BLNG_SERV_CODE ";
      sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "select LTRIM(RTRIM(('BLRSRINS.LEGEND_'||LTRIM(RTRIM(TO_CHAR\
...
_report where report_id='BLRSRINS'";
 sqlstm.iters = (unsigned int  )1;
