...
int totalRecords=0;
	...
	<p align='right'>
		...
		<%
			...
			{   /*Below line modified for this incident [44249]*/
		%>
			<td align ='right' id='next' style='visibility:hidden'><a href="../../eOA/jsp/TransferCancelAppointmentQueryResult.jsp?operation_mode=Next&start_index=<%=(lower_limit+maxDispRecord)%>&end_index=<%=(upper_limit+maxDispRecord)%>&oper_stn_id=<%=oper_stn_id%>&called_for=<%=called_for%>&practcode=<%=practcode%>&gender=<%=gender%>&clinic_type=<%=clinic_type%>&res_type=<%=res_type%>&patientid=<%=patientid%>&patname=<%=patname%>&cliniccode=<%=cliniccode%>&appttodate=<%=DateUtils.convertDate(appttodate,"DMY","en",locale)%>&apptfromdate=<%=DateUtils.convertDate(apptfromdate,"DMY","en",locale)%>&function_id=<%=function_id%>&group_chk=<%=group_chk%>" ><fmt:message key="Common.next.label" bundle="${common_labels}"/></a></td>
		<%
			}	
		}else
		{
	...
	<td class='COLUMNHEADER' nowrap>&nbsp;</td>
		<%}%>
			...
			<td class='COLUMNHEADER' nowrap><fmt:message key="Common.OrderID.label" bundle="${common_labels}"/></td>
		</tr>
		<% }
	/*	try
		{
			...
			String episode_type="";
				...
				<% if(forced_yn.equals("N")){ %>
						...
						%>
						<td class='<%=classValue%>' onMouseOver='hideToolTip()' onClick="displayToolTip('<%=tfr_appt_yn%>','<%=tfr_appt_across_catg_yn%>','<%=canc_appt_yn%>','<%=apptrefno%>','<%=cliniccode1%>','<%=practitionerid%>','<%=apptdate%>',	'<%=overbookedyn%>','<%=apptslabfromtime%>','<%=apptslabtotime%>','<%=appt_day1.trim()%>','<%=visit_type_ind%>','<%=team_id%>','N','<%=resourceClass%>','<%=locnType%>','<%=patientid%>','<%=book_appt_across_catg_yn%>','<%=book_appt_yn%>','<%=book_appt_wo_pid_yn%>','<%=obook_appt_yn%>','<%=create_wait_list_yn%>','<%=time_table_type%>','<%=referral_id%>','','<%=clinic_name%>','<%=appt_category%>','<%=speciality_code%>','<%=rd_appt_yn%>','<%=group_id%>')" nowrap>
				...
				<td  class = '<%=classValue%>'  nowrap><%=apptrefno%></td>
					...
					/*Below Line Modified for this Incident [IN34988]*/
					%>
					<td  title = 'Forced Appointment' class = 'OAFORCED' onClick="displayToolTip('<%=tfr_appt_yn%>','<%=tfr_appt_across_catg_yn%>','<%=canc_appt_yn%>','<%=apptrefno%>','<%=cliniccode1%>','<%=practitionerid%>','<%=apptdate%>','<%=overbookedyn%>','<%=apptslabfromtime%>','<%=apptslabtotime%>','<%=appt_day1.trim()%>','<%=visit_type_ind%>','<%=team_id%>','F','<%=resourceClass%>','<%=locnType%>','<%=patientid%>','<%=book_appt_across_catg_yn%>','<%=book_appt_yn%>','<%=book_appt_wo_pid_yn%>','<%=obook_appt_yn%>','<%=create_wait_list_yn%>','<%=time_table_type%>','<%=referral_id%>','','<%=clinic_name%>','<%=appt_category%>','<%=speciality_code%>','<%=rd_appt_yn%>','<%=group_id%>')" nowrap> 
				...
				<td class="<%=classValue%>" nowrap onClick=hideToolTip() style="background-color:<%=minpat_indicator%>;cursor:pointer;" onMouseOut="hideTooltip(t2)" onMouseOver="showTooltip(event,'<%=div_val%>',t2,'<%=disply_title%>','<%=group_name%>')">
				<%}else{%>
				...
				<td class="<%=classValue%>" nowrap >	<%=patname%>,<%=gender%>,<%=dis_age%></td>
			...
			<td nowrap class='<%=classValue%>'><%=apptslabfromtime%></td>
				...
				<td nowrap class='<%=classValue%>'><%=order_id%>&nbsp;</td>
				</tr>
					<%
				rowcolor++;		
				cnt++;
