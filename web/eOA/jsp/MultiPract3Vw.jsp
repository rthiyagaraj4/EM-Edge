...
%>
<html>
<iframe name='imageing' id='imageing' src='../../eOA/jsp/MP3DisplayDate.jsp?clinic_code1=<%=clinic_code1%>&practitioner_id1=<%=practitioner_id1%>&clinic_code2=<%=clinic_code2%>&callingMode=<%=callingMode%>&valid_date=<%=valid_date%>&practitioner_id2=<%=practitioner_id2%>&clinic_code3=<%=clinic_code3%>&practitioner_id3=<%=practitioner_id3%>&Date=<%=Date%>&Date1=<%=Date1%>&Date2=<%=Date2%>&value12=<%=value12%>&value13=<%=value13%>&old_clinic=<%=old_clinic%>&old_date=<%=old_date%>&old_from_time=<%=old_from_time%>&old_to_time=<%=old_to_time%>&pre_pat_alcn_catg_code=<%=pre_pat_alcn_catg_code%>&ca_patient_id=<%=ca_patient_id%>&ca_mode=<%=ca_mode%>&mode_find=<%=mode_find%>&clinic_type1=<%=clinic_type1%>&res_type1=<%=res_type1%>&clinic_type2=<%=clinic_type2%>&res_type2=<%=res_type2%>&clinic_type3=<%=clinic_type3%>&res_type3=<%=res_type3%>&oper_stn_id=<%=oper_stn_id%>&alcn_criteria=<%=alcn_criteria%>&visit_limit_rule=<%=visit_limit_rule%>&slot_appt_ctrl=<%=slot_appt_ctrl%>&override_no_of_slots_yn=<%=override_no_of_slots_yn%>&req_id=<%=req_id%>&install_yn_val=<%=install_yn_val%>&create_wait_list_yn=<%=create_wait_list_yn%>&visit_flag_OP=<%=visit_flag_OP%>&capture_fin_dtls_yn=<%=capture_fin_dtls_yn%>&from_page=<%=from_page%>&noshow_ctrl_by_splty_or_clinic=<%=noshow_ctrl_by_splty_or_clinic%>&min_ela_per_resc_noshow_in_day=<%=min_ela_per_resc_noshow_in_day%>&contact_num_reqd_yn=<%=contact_num_reqd_yn%>&from_facility_id=<%=from_facility_id%>&from_encounter_id=<%=from_encounter_id%>&speciality=<%=speciality%>&clinic_old=<%=clinic_old%>&practitioner_old=<%=practitioner_old%>&practitioner_type=<%=practitioner_type%>&pref_start_date=<%=pref_start_date%>&pref_end_date=<%=pref_end_date%>&care_locn_ind=<%=care_locn_ind%>&visittype=<%=visittype%>&no_dwm=<%=no_dwm%>&dwm_desc=<%=dwm_desc%>&or_catalogue_code=<%=or_catalogue_code%>&no_of_noshow_appts_for_alert=<%=no_of_noshow_appts_for_alert%>&per_chk_for_no_show_alert=<%=per_chk_for_no_show_alert%>&position=<%=position%>&addtl_splty=<%=addtl_splty%>&rd_appt_yn=<%=rd_appt_yn%>' frameborder=no scrolling=no noresize style='height:6vh;width:97vw'></iframe><!--rd_appt_yn included for the incident 40965-->
	<iframe name='main' id='main'  src='../../eOA/jsp/MP3Display6Frames.jsp?clinic_code1=<%=clinic_code1%>&practitioner_id1=<%=practitioner_id1%>&clinic_code2=<%=clinic_code2%>&callingMode=<%=callingMode%>&practitioner_id2=<%=practitioner_id2%>&clinic_code3=<%=clinic_code3%>&practitioner_id3=<%=practitioner_id3%>&Date=<%=Date%>&Date1=<%=Date1%>&Date2=<%=Date2%>&value12=<%=value12%>&value13=<%=value13%>&old_clinic=<%=old_clinic%>&old_practitioner=<%=old_practitioner%>&old_date=<%=old_date%>&old_from_time=<%=old_from_time%>&old_to_time=<%=old_to_time%>&pre_pat_alcn_catg_code=<%=pre_pat_alcn_catg_code%>&ca_patient_id=<%=ca_patient_id%>&ca_mode=<%=ca_mode%>&mode_find=<%=mode_find%>&clinic_type1=<%=clinic_type1%>&res_type1=<%=res_type1%>&clinic_type2=<%=clinic_type2%>&res_type2=<%=res_type2%>&clinic_type3=<%=clinic_type3%>&res_type3=<%=res_type3%>&or_catalogue_code=<%=or_catalogue_code%>&order_id=<%=order_id%>&order_line_num=<%=order_line_num%>&order_catalog_code=<%=order_catalog_code%>&oper_stn_id=<%=oper_stn_id%>&alcn_criteria=<%=alcn_criteria%>&visit_limit_rule=<%=visit_limit_rule%>&slot_appt_ctrl=<%=slot_appt_ctrl%>&override_no_of_slots_yn=<%=override_no_of_slots_yn%>&req_id=<%=req_id%>&install_yn_val=<%=install_yn_val%>&create_wait_list_yn=<%=create_wait_list_yn%>&visit_flag_OP=<%=visit_flag_OP%>&capture_fin_dtls_yn=<%=capture_fin_dtls_yn%>&from_page=<%=from_page%>&noshow_ctrl_by_splty_or_clinic=<%=noshow_ctrl_by_splty_or_clinic%>&min_ela_per_resc_noshow_in_day=<%=min_ela_per_resc_noshow_in_day%>&from_facility_id=<%=from_facility_id%>&from_encounter_id=<%=from_encounter_id%>&contact_num_reqd_yn=<%=contact_num_reqd_yn%>&no_of_noshow_appts_for_alert=<%=no_of_noshow_appts_for_alert%>&per_chk_for_no_show_alert=<%=per_chk_for_no_show_alert%>&rd_appt_yn=<%=rd_appt_yn%>' frameborder=no scrolling=no noresize style='height:94vh;width:97vw'></iframe><!--rd_appt_yn included for the incident 40965-->
