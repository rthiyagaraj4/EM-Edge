...
function  prevRecs(start,end,whereclause,fromPag){
	...
	if(flag==false){
		//parent.f_query_add_mod.location.href="../../eOA/jsp/HolidayQueryResult<MAP NAME=".jsp">	<AREA SHAPE="" HREF="" COORDS="" ALT=""></MAP>?mode=<%=fmode%>&from=<%=start-14%>&to=<%=end-14%>&whereclause=<%=URLEncoder.encode(sql.toString())%>&fromPage=<%=currPage%>&link=prev&selCheckBoxes=";
...
<form name='HolidayQueryResult' id='HolidayQueryResult' method='post' action='../../servlet/eOA.HolidayServlet' target='messageFrame'>
	...
	<th class='COLUMNHEADER' ><fmt:message key="Common.Location.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.description.label" bundle="${common_labels}"/></th>
	<th class='COLUMNHEADER' ><fmt:message key="Common.HolidayDate.label" bundle="${common_labels}"/></th>
	<th class='COLUMNHEADER' ><fmt:message key="eOA.HolidayDay.label" bundle="${oa_labels}"/></th>
	<th class='COLUMNHEADER' ><fmt:message key="Common.ReasonForHoliday.label" bundle="${common_labels}"/></th>
	<th class='COLUMNHEADER' ><fmt:message key="Common.selectall.label" bundle="${common_labels}"/><input type='checkbox' name='selectall' id='selectall' onClick='selectAll_S(this);' <%=checked[0]%>></th>	
	</tr>
	<% }
		...
		String classValue = "" ;
			...
			<a href='javascript:qryModify("<%=clti%>","<%=cliniccode%>","<%=holiday_date%>","<%=java.net.URLEncoder.encode(reason)%>","<%=holiday%>","<%=java.net.URLEncoder.encode(locn_type_desc)%>","<%=facilityId%>")'><%=locn_type%>
			&nbsp;</a><input type=hidden name='locnCode' id='locnCode' value='<%=clti%>'></td>	
			<td class='<%=classValue%>' >
			<a href='javascript:qryModify("<%=clti%>","<%=cliniccode%>","<%=holiday_date%>","<%=java.net.URLEncoder.encode(reason)%>","<%=holiday%>","<%=java.net.URLEncoder.encode(locn_type_desc)%>","<%=facilityId%>")'>
			<%=cliniccode%></a><input type=hidden name='clinicCode' id='clinicCode' value='<%=cliniccode%>'></td>	
			...
			<a href='javascript:qryModify("<%=clti%>","<%=cliniccode%>","<%=holiday_date%>","<%=java.net.URLEncoder.encode(reason)%>","<%=holiday%>","<%=java.net.URLEncoder.encode(locn_type_desc)%>","<%=facilityId%>")'><%=holiday_date_disp%></a><input type=hidden name='holidayDate' id='holidayDate' value='<%=holiday_date%>'></td>
			<td class='<%=classValue%>'>
			<a href='javascript:qryModify("<%=clti%>","<%=cliniccode%>","<%=holiday_date%>","<%=java.net.URLEncoder.encode(reason)%>","<%=holiday%>","<%=java.net.URLEncoder.encode(locn_type_desc)%>","<%=facilityId%>")'> 
			<%=holiday%>&nbsp;</a></td>
			<td class='<%=classValue%>'>
			<a href='javascript:qryModify("<%=clti%>","<%=cliniccode%>","<%=holiday_date%>","<%=java.net.URLEncoder.encode(reason)%>","<%=holiday%>","<%=java.net.URLEncoder.encode(locn_type_desc)%>","<%=facilityId%>")'> 
			<%=reason%></a></td>
			<td class='<%=classValue%>' align='center'><input type='checkbox' id='check_Box<%=k%>' name='checkBox' onClick='javascript:checkBoxOnClick(this,<%=k%>);'  <%=checked[k]%>></td>	</tr>
			<%
			k=k+1;
			i++;
			maxRecord++;
...
<input type='hidden' name='fmode' id='fmode' value='<%=fmode%>'>
...
<input type='hidden' name='clinic_desc' id='clinic_desc' value='<%=p_clinic_desc%>'>
<input type='hidden' name='holiday_date' id='holiday_date' value='<%=holiday_date1%>'>
<input type='hidden' name='locn_type' id='locn_type' value='<%=p_locn_type%>'>
