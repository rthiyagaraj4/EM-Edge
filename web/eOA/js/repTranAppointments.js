...
function chng_report_name(obj)
{
 if (obj.value=="N")
    document.forms[0].p_report_id.value = "OABNRTR";
 else
    document.forms[0].p_report_id.value = "OABTRAPT";
...
/*function searchCode(obj,target,obj1)
	{
			...
			if(obj.name=="locn")
			{
				...
				sql="select clinic_code, long_desc from op_clinic_lang_vw where facility_id=`"+facilityid+"` and level_of_care_ind = `A` and language_id=`"+locale+"` and CARE_LOCN_TYPE_IND=nvl(`"+p_location_type1+"`,CARE_LOCN_TYPE_IND)";
