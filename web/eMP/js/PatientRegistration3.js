...
async function callChangePatApply() 
{
...
if(f_query_add_mod.patient_sub.document.getElementById('n_contact_ma_area_code').value=="")
	f_query_add_mod.patient_sub.document.getElementById('contact1_res_area').value="";
}
...
if ( canSave == true && errorsThere == false ) {
	...
	if(f_query_add_mod.patient_sub.document.getElementById('abha_pat_id') !=null && f_query_add_mod.patient_sub.document.getElementById('abha_pat_id').value!="" && f_query_add_mod.patient_sub.document.getElementById('ndhm_appl_YN').value=="Y" ){
					...
					xmlHttp.open('POST','../../eMP/jsp/MPIntermediate.jsp?action=chkDuplicateABHAId&abha_id='+abha_id,false);
					xmlHttp.send(xmlDoc);
...
{ 
	...
	if(f_query_add_mod.patient_sub.PatRegForm.date_of_birth.value != null && f_query_add_mod.patient_sub.document.getElementById('Birth_Date').value == ""){
		f_query_add_mod.patient_sub.document.getElementById('Birth_Date').value = f_query_add_mod.patient_sub.PatRegForm.date_of_birth.value+" 00:00:00";
	}	
	var bir_date =""; // added by mujafar for KDAH-CRF-370.1 
...
if ( canSave == true && errorsThere == false ) {
	...
	if(f_query_add_mod.patient_sub.document.getElementById('abha_pat_id') !=null && f_query_add_mod.patient_sub.document.getElementById('abha_pat_id').value!="" && f_query_add_mod.patient_sub.document.getElementById('ndhm_appl_YN').value=="Y" ){
			...
			if(f_query_add_mod.patient_sub.document.getElementById('nat_id_exp_date')){
			if(f_query_add_mod.patient_sub.document.getElementById('nat_id_exp_date').value=="" && f_query_add_mod.patient_sub.document.getElementById('nat_id_reqd_all_series').value=='Y')
			{
				canSave=checkForNull(f_query_add_mod.patient_sub.document.getElementById('nat_id_prompt').value +" "+getLabel('Common.expiryDate.label','common'));
			}
			}
			   if ( canSave == false )
                errorsThere = true;
		   if(f_query_add_mod.patient_sub.document.getElementById('sex').value=="")
			...
			if ( canSave == false )
                errorsThere = true;
			//End by Kamatchi S for AAKH-CRF-0145
...
bir_date = birthDate; //added by mujafar for KDAH-CRF-370.1 
			...
			/* below line add by mano for ML-MMOH-CRF-0736 */
          ...
          //Below Added by Suji keerthi on 26-Nov-2019 for KDAH-CRF-0522
            if  ( (f_query_add_mod.patient_sub.document.getElementById('pat_cat_code')) && (f_query_add_mod.patient_sub.document.getElementById('pat_cato_reqd_yn').value == 'Y') )
			{
				if(f_query_add_mod.patient_sub.document.getElementById('pat_cat_code').value=="")
				{
					canSave=checkForNull(getLabel('Common.category.label','common'));
  ...
  // CHECKING FOR PATIENT NAME
            ...
            if ( f_query_add_mod.patient_sub.document.getElementById('alt_id1_no') != null && tempflagchk==false)
            {
				...
				//	if( isNatIdAltIdMandVisaType == "false"){
						...
						//Maheshwaran K added for AAKH-CRF-0168 as on 22-10-2022
					if (isNatIdOrAltIdMandVisaType == "true" && f_query_add_mod.patient_sub.document.getElementById('visa_type').value != "V"){
				//	if( isNatIdAltIdMandVisaType == "false"){
							canSave = checkForNull1(f_query_add_mod.patient_sub.document.getElementById('alt_id1_no'),f_query_add_mod.patient_sub.document.getElementById('altId1Desc').value);
					}
            ...
            if ( f_query_add_mod.patient_sub.document.getElementById('alt_id2_no') != null )
            {
				...
				//	if( isNatIdAltIdMandVisaType == "false"){
					...
					if( isNatIdOrAltIdMandVisaType == "true" && f_query_add_mod.patient_sub.document.getElementById('alt2_gif').style.visibility=='visible'){
				//	if( isNatIdAltIdMandVisaType == "false"){
                        canSave = checkForNull1(f_query_add_mod.patient_sub.document.getElementById('alt_id2_no'),f_query_add_mod.patient_sub.document.getElementById('altId2Desc').value);
					}
					if( isNatIdOrAltId1MandRes == "true"){
...
if(f_query_add_mod.patient_sub.document.getElementById('isAutoPopulateNOKDOBAppl').value == 'true' && f_query_add_mod.patient_sub.document.getElementById('contact1_birth_date') && f_query_add_mod.patient_sub.document.getElementById('contact1_birth_date').value!="") {
	if (!f_query_add_mod.patient_sub.isBeforeNow(f_query_add_mod.patient_sub.document.getElementById('contact1_birth_date').value,"DMY",localeName)) 
	{
		...
		canSave = false;
			...
			/*Added by Suji keerthi on 27-Nov-2019 for KDAH-CRF-0522*/
...
{ 
	if(f_query_add_mod.patient_sub.document.getElementById('pan_holder_name').value=="" && f_query_add_mod.patient_sub.document.getElementById('pan_status').value=="") // pan_status
		{ 
...
if(f_query_add_mod.patient_sub.document.getElementById('chkPanValidate').value=="true")
{
if(canSave==true && errorsThere == false )	
{
	if(f_query_add_mod.patient_sub.document.getElementById('alt_id3_no').value)
				...
				if(f_query_add_mod.patient_sub.PatRegForm.appt_yn.value=='Appt' || f_query_add_mod.patient_sub.PatRegForm.appt_yn.value=='IPADMIT')
                {
                    ...
                    { 
							 ...
							 else
                             {
								...
								for(i=0;i<frm.elements.length;i++)
								{
									if(frm.elements[i].type=='checkbox')
									{
										...
										{
											s += "<input type='hidden' name='"+frm.elements[i].name+"' value='Y'>"
										}
									 }
									else 
									{
										s += "<input type='hidden' name='"+frm.elements[i].name+"' value=\""+(frm.elements[i].value == null ?'':frm.elements[i].value)+"\">"
									}
								}
                     ...
                     else
                     {
                        ...
                        { 
							var s = "<html><link rel='stylesheet' type ='text/css' href='../../eCommon/html/"+sStyle+"'></link><head><META HTTP-EQUIV='Content-Type'  CONTENT='text/html; CHARSET=UTF-8'></head><body class='message' onKeyDown='lockKey()'><form name='PatRegForm' id='PatRegForm' method='post'  action='../../servlet/eMP.PatientRegistrationServlet' target='messageFrame'>";
...
// calculate_age function added by mujafar for KDAH-CRF-370.1 
