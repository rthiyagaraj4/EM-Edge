...
<%@ page import ="java.sql.*,java.util.*,java.net.*,java.text.*,webbeans.eCommon.*,com.ehis.util.*,javax.servlet.http.*" contentType="text/html;charset=UTF-8" %>
...
String sSQL = "SELECT"
	...
	rs = pstmt.executeQuery();
				...
				while(rs.next() )
				{
					...
					if ( cnt==0 )
					{%>
					<table border="1" width="<%=tab_width%>" cellspacing='0' cellpadding='0' align='center' id='t'>
						<!--Added by Ashwini on 16-Mar-2017 for GDOH-CRF-0144-->
						<tr>
						<td colspan='3' class='CAFIRSTNOSELECT'><fmt:message key="Common.Registeredby.label" bundle="${common_labels}"/>: <%=added_by_name%>
						&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<fmt:message key="Common.Registrationdatetime.label" bundle="${common_labels}"/>: <%=regn_date%></td>
						</tr>
						<!--End GDOH-CRF-0144-->
						<td class='columnheader'><fmt:message key="eMP.FieldName.label" bundle="${mp_labels}"/></td>
		...
		<script language="JavaScript">
			if(<%=pat_exists%> && <%=status%>){				parent.frames[2].location.href="../../eCommon/jsp/pline.jsp?Patient_ID=<%=java.net.URLEncoder.encode(pat_id)%>";
			}
			else
			{
