...
<%@ page contentType="text/html;charset=UTF-8" import="java.sql.*,webbeans.eCommon.*,java.util.StringTokenizer,java.util.*,com.ehis.util.*,java.io.* ,eCommon.Common.*,eXH.*, eCommon.XSSRequestWrapper" %>
...
%>
	<%if(family_org_id_accept_yn.equals("Y"))
		...
		<tr>
			...
			<td  class='label'  width='<%=header_width%>'><fmt:message key="Common.Organization.label" bundle="${common_labels}"/></td>
...
%>
	...
	<%
		 ...
		 if (ndhm_appl_YN.equals("Y")){ %>
				...
				<tr><td class='label'><fmt:message key="eMP.ABHANumber.label" bundle="${mp_labels}"/> </td>
				<td class="data" ><%=unique_no%></td>
				</tr>
		<% }%> 
...
<a name='addr' id='addr'></a>
       <table cellpadding='4' cellspacing='1' border ='0' width='99%' align='center'>
                <tr>
                    ...
                    <table cellpadding='3' cellspacing='0' border='0' height='100%' width='<%=tab_width2%>'>
                        <tr>
                            <td class='COLUMNHEADER'>&nbsp;</td>
                            <td  class='COLUMNHEADER' colspan=''><fmt:message key="eMP.residenceaddress.label" bundle="${mp_labels}"/></td>
							<%
							 
							if(alterAddressApplicable && increasedaddressLength){%>
                            <td  class='COLUMNHEADER'><fmt:message key="eMP.AlternateAddress.label" bundle="${mp_labels}"/></td>
							<%}%>
...
<tr>
	...
	<%//Below line added fro this ML-MMOH-CRF-0601 and ML-MMOH-CRF-860.2
	if(alterAddressApplicable){	%>
	<td class='data' ><%=alt_contact_name%></td>
	<%}%>
...
<tr>
		...
		<td  class='label' width='15%'><%=contactModeTwo%></td>
		<td  class='data' width='34%'>
		<%if(isVirtualConsultationVisit) {%><%=contact2TelephoneCode%>&nbsp;<%}%><!-- Added by lakshmanan for ML-MMOH-CRF-1930 US005 -->
		<%=Orn_Tel_No%></td>
</tr>

<tr>
      <td  class='label'><fmt:message key="Common.email.label" bundle="${common_labels}"/></td>
      <td  class='data' width='<%=email_width%>' style='<%=email_word_wrap%>' ><%=email_id%></td>
...
<tr>
		<td  class='label' width="<%=nextofkin_width%>" ><fmt:message key="Common.name.label" bundle="${common_labels}"/></td>
		<td  class='data' width="<%=nextofkin_width%>" style="<%=nextofkin_word_wrap%>"><%=Nkin_Contact_Name%></td>
		<%=nKin_Addr_val%>
 </tr>
...
<tr><td>
		<table cellspacing='1' cellpadding='4' width='100%' border=0>
			...
			<tr>
					<td  class='label' width="<%=nextofkin_width%>" ><fmt:message key="Common.name.label" bundle="${common_labels}"/></td>
					<td  class='data' width="<%=nextofkin_width%>" style="<%=nextofkin_word_wrap%>" ><%=Fton_Contact_Name%>&nbsp;</td>
					<%=nKin_Addr_val%>
			</tr>
...
<tr><th  width="5%" class='columnheader'>&nbsp;</th><th  width="20%" class='columnheader'><fmt:message key="Common.employerdetails.label" bundle="${common_labels}"/>&nbsp;</th><td class='COLUMNHEADERCENTER' colspan='2' width="40%"><fmt:message key="Common.employeedetails.label" bundle="${common_labels}"/></td></tr>
<tr>
		<td  class='label' width="24%">&nbsp;</td>
		<td class='DATA' width="24%">&nbsp;</td>
		<td class='DATA' width="24%">&nbsp;</td>
