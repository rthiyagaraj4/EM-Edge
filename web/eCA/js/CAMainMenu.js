...
/** Modified by Kishore Kumar N on 27/11/2008 for the CRF-0387*/
//var tempMainMenuCols3 = "30%,0%,*,0%";
//var tempMainMenuCols3 = "200px,0%,*,0%";//Commented for IN061885
var tempMainMenuCols3 = "220px,0%,*,0%"; //Modified for IN061885
/** Ends here*/
var tempMainMenuCols4 = "0%,0%,*,0%";
var clinicianMain = "8%,*,5%,0%";  //"10%,*,5%,0%";
...
function Node(id, pid, name, url, title, target, icon, iconOpen, open,function_type,only_option_id) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
	...
	this.only_option_id = only_option_id;
};
...
dTree.prototype.add = function(id, pid, name, url, title, target, icon, iconOpen, open,function_type,only_option_id) {
	//alert('id='+id+'pid='+pid+'name='+name+'url='+url+'title='+title+'icon='+icon+'iconOpen='+iconOpen+'open='+open+'function_type='+function_type);
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, title, target,icon, iconOpen, open,function_type,only_option_id);
};
	...
	if (this.config.useIcons) {
		...
		if (this.root.id == node.pid) {
			...
			if(node.pid!=-1)
			{
				...
				//alert(node.function_type)
				/*str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" />';*/

				if(node.function_type == 'R'){
					//alert('R');
					str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" />';
				}else if(node.function_type == 'H'){
					//alert('V');
					//str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" />';
...
dTree.prototype.s = function(id) 
{
	...
	//parent.Navigation.document.frmMenu.txtCurrentPosition.value = id;
...
//	loadOrUnloadMenus(parent.CommonToolbar.HideAndShow);
	//alert(document.HomePage.homepageValue);
	if(top.content.CommonToolbar != null)
	{
		if(top.content.CommonToolbar.HideAndShowLookup != null)
			top.content.CommonToolbar.HideAndShowLookup.value = '';
	}
	...
	//{
		...
		{	
			//if((cn.only_option_id=="FLOW_SHEET")||(cn.only_option_id=="CLINICAL_NOTES")||(cn.only_option_id=="MEDICAL_CERTFICATE")||(cn.only_option_id=="DISCHARGE_SUMMARY")||(cn.only_option_id=="REFERRAL_LETTER")||(cn.only_option_id=="BY_LOCATION")) //commented for IN061898
			//IN051294 start
			//if((cn.only_option_id=="FLOW_SHEET")||(cn.only_option_id=="CLINICAL_NOTES")||(cn.only_option_id=="MEDICAL_CERTFICATE")||(cn.only_option_id=="DISCHARGE_SUMMARY")||(cn.only_option_id=="REFERRAL_LETTER")||(cn.only_option_id=="BY_LOCATION")||(cn.only_option_id=="CA_TASK_LIST")) // Added tasklist for IN061898
				//if((cn.only_option_id=="FLOW_SHEET")||(cn.only_option_id=="CLINICAL_NOTES")||(cn.only_option_id=="MEDICAL_CERTFICATE")||(cn.only_option_id=="DISCHARGE_SUMMARY")||(cn.only_option_id=="REFERRAL_LETTER")||(cn.only_option_id=="BY_LOCATION")||(cn.only_option_id=="CA_TASK_LIST")||(cn.only_option_id=="PHYSICIAN_NOTES"))//IN069654
			if((cn.only_option_id=="FLOW_SHEET")||(cn.only_option_id=="CLINICAL_NOTES")||(cn.only_option_id=="MEDICAL_CERTFICATE")||(cn.only_option_id=="DISCHARGE_SUMMARY")||(cn.only_option_id=="REFERRAL_LETTER")||(cn.only_option_id=="BY_LOCATION")||(cn.only_option_id=="CA_TASK_LIST")||(cn.only_option_id=="PHYSICIAN_NOTES")||(cn.only_option_id=="FLOW_SHEET_NEW")) // Added IN069654
			//IN051294 End
			{
				...
				xmlHttp.open( "POST","../../eCA/jsp/CAMainFunctionFilterCriteria.jsp?function_id="+cn.only_option_id, false);
				xmlHttp.send(xmlDoc);
				responseText=xmlHttp.responseText;
				responseText = trimString(responseText);
				eval(responseText);
				parent.CommonToolbar.filter_by.style.display = '';
				//top.content.mainFrameSet.rows = '7.75%,*,5%,0%';commented by channaveer B for issue: 21268
			    top.content.mainFrameSet.rows = '8%,90%,5%,0%';//added by channaveer B for issue: 21268
			...
			else
			{
				...
				top.content.mainFrameSet.rows = '8%,*,5%,0%';
			}
		}
	//}
};
...
function changeTree(toggleButton){
	...
	{	
		document.getElementById('exp_collapse_img').innerHTML="<IMG style='cursor:pointer' id = 'toggleTreeCollapse' SRC='../../eCommon/images/close-all.gif'  BORDER=0 ALT=\""+getLabel("Common.closeAll.label","common")+"\" onClick=javascript:changeTree(this)>";
		obj.openAll();
	}
	else if(toggleButton.id == 'toggleTreeCollapse')
	{
		document.getElementById('exp_collapse_img').innerHTML="<IMG SRC='../../eCommon/images/expand-all.gif' id='toggleTreeExpand' style='cursor:pointer' BORDER='0' ALT=\""+getLabel("Common.openAll.label","common")+"\" onClick=javascript:changeTree(this);>";
		obj.closeAll();
	}
}
...
function loadTitleName(obj){
	...
	//parent.CommonToolbar.alertFrame(obj);
}
