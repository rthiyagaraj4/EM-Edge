...
{	
	document.waittimeanalysisreport.loca.value="";
	document.waittimeanalysisreport.locn_code.value="";	
}
...
function reset(){
  ...
  var formObj = parent.content.f_query_add_mod.waittimeanalysisreport;
  parent.content.f_query_waittimeanalysreportresult.location.href = '../../eCommon/html/blank.html';
  formObj.reset();
...
function callPatientSearch(){
	...
	if(pat_id != null)
	{
			...
			var frmObj=document.waittimeanalysisreport;
			resetValues();
	}
}	
...
async function patientEncounterDetailNew()
{
	...
	var arguments	= "" ;
	var patId = document.waittimeanalysisreport.patientId.value;
	
	if(patId !="")
	{
		var retVal =await window.showModalDialog("../../eOR/jsp/RefusalOrderEncounterFrameSet.jsp?called_from=generic_atc&mode="+MODE_INSERT+"&patientId="+document.waittimeanalysisreport.patientId.value,arguments,features);
...
{  
	...
	var locale			= document.waittimeanalysisreport.locale.value;
...
{  
	...
	var locale			= document.waittimeanalysisreport.locale.value;
	var facilityId		= document.waittimeanalysisreport.p_facility_id.value;
...
function getPractCode(obj)
 {
 	if(obj.value != ''){
		...
		document.waittimeanalysisreport.practitioner_id.value = "";
 		document.waittimeanalysisreport.pctr.value = "";
	}		
 } 
 
function getPractitionerId13(){
 	
	var patientId	=	document.waittimeanalysisreport.patientId.value;
	var encounterId		=	document.waittimeanalysisreport.encounterId.value;
	var messageFrameReference = parent.messageFrame;
	var	fields		=	new Array ( document.waittimeanalysisreport.patientId, document.waittimeanalysisreport.encounterId) ;
	...
	var target = document.waittimeanalysisreport.pctr;
 	...
 	{
 		document.waittimeanalysisreport.practitioner_id.value = retArray[0];
 		document.waittimeanalysisreport.pctr.value = retArray[1];
 	}else{
		document.waittimeanalysisreport.practitioner_id.value = "";
 		document.waittimeanalysisreport.pctr.value = "";
	}
...
function getResults(){
		...
		var patientId		=	document.waittimeanalysisreport.patientId.value;
		var encounterId		=	document.waittimeanalysisreport.encounterId.value;
		var note_group		=	document.waittimeanalysisreport.note_group_id.value;
		var note_type		=	document.waittimeanalysisreport.note_type.value;
		var practitioner_id	=	document.waittimeanalysisreport.practitioner_id.value;
		var p_fm_Date		=	document.waittimeanalysisreport.p_fm_Date.value;
		var p_to_Date		=	document.waittimeanalysisreport.p_to_Date.value
		var messageFrameReference = parent.messageFrame;
		var	fields		=	new Array ( parent.f_query_add_mod.document.waittimeanalysisreport.patientId, parent.f_query_add_mod.document.waittimeanalysisreport.encounterId) ;
		var names		=	new Array (getLabel("Common.patientId.label","Common"),getLabel("Common.encounterid.label","COMMON"));
		...
		parent.f_query_waittimeanalysreportresult.location.href = "../../eCA/jsp/WaitingTimeAnalysisReportResults.jsp?"+query_string;
		
 }
 async function showpreview(calledFromOption){	 
	
	var selectedRowValue  = "";	
	var selectedRow = parent.f_query_waittimeanalysreportresult.document.getElementsByName('selectedRow'); 	
	...
	if(selectedRowValue ==""){
		...
		var accessionNum = parent.f_query_waittimeanalysreportresult.document.getElementById("accessionNum"+selectedRowValue).value;
		var noteType = parent.f_query_waittimeanalysreportresult.document.getElementById("noteType"+selectedRowValue).value;
		var recordedFacility = parent.f_query_waittimeanalysreportresult.document.getElementById("recordedFacility"+selectedRowValue).value;
		
		var patient_id	=	document.waittimeanalysisreport.patientId.value;
		var encounter_id		=	document.waittimeanalysisreport.encounterId.value;
		var dialogTop	= "80";
		var dialogHeight= "40" ;
		var dialogWidth	= "60" ;
		var arguments = "";
		...
		var retVal =await window.showModalDialog("../../eCA/jsp/WaitingTimeAnalysisReportPrintPreview.jsp?patient_id="+patient_id+"&encounter_id="+encounter_id+"&called_from=&accessionNum="+accessionNum+"&noteType="+noteType+"&recordedFacility="+recordedFacility+"&calledFromOption="+calledFromOption,arguments,features);
	}
