		...
		<table class='grid' cellspacing=0 cellpadding=0 width='100%'>
			...
			<tr id='tableTRId<%=count%>'>
				<td class="CAHIGHERLEVELCOLOR">
					...
					&nbsp;
					<font size=1><%=(String)resultMap.get("Event_Date_Time")%>
			<!-- 24094 starts -->		 <% 
					 String Modifiedyn=(String)resultMap.get("Modified_Date_YN");
					 if(Modifiedyn.equals("Y")){%>
					&nbsp;<img src='../../eCA/images/modifiedresult.png'/>
					<%} %> <!-- 24094 ends -->	
					</font>&nbsp;&nbsp; (<%=(String)resultMap.get("Note_Group_Desc")%>) - <%=(String)resultMap.get("Note_Type_Desc")%> </font>
				</td>
			</tr>
			<tr>
				<td>
			...
			<%-- <input type='hidden' name='modified_date<%=count%>' id='modified_date<%=count%>' value='<%=(String)resultMap.get("Modified_Date")%>'/><!-- 24094 starts --> --%>	
			<input type='hidden' name='modifiedyn<%=count%>' id='modifiedyn<%=count%>' value='<%=(String)resultMap.get("Modified_Date_YN")%>'/><!-- 24094 starts -->	
