...
*/ 
%>
<%@page import="java.sql.*,java.io.*,webbeans.eCommon.*,java.util.*,java.awt.image.BufferedImage,javax.imageio.ImageIO,sun.misc.BASE64Decoder,eCA.*,eCommon.XSSRequestWrapper" contentType="text/html;charset=UTF-8"%>
...
%>
	...
	<body onLoad='makeActive("<%=operation_mode%>");' onMouseDown="CodeArrest()" onKeyDown="lockKey()" onmousemove = "" onmouseout= "" class='CONTENT'>
	...
	<td class='<%=classValue%>' colspan = '3'>
		<table cellpadding = '0' cellspacing = '0' width='100%'>
			<tr>
				<td class='<%=classValue%>' width="80%">
				<a href="javascript:showNotes(tblrow_id0,'<%=operation_mode%>','')" id='notes_view' class='ACTIVE'><b><fmt:message key="eCA.NotesView.label" bundle="${ca_labels}"/></b></a>
				</td>
				<!-- IN058147 Start. -->
				<td class='<%=classValue%>' width="20%" align="right"> 
...
<%
	try
	{
		...
		%>
		<input type="hidden" name="previousNote_accession_num" id="previousNote_accession_num" value="<%=previousNote_accession_num %>"/>
		<input type="hidden" name="call_for_copy_previous_sec" id="call_for_copy_previous_sec" value="<%=call_for_copy_previous_sec %>"/><!--IN063931-->
		<input type="hidden" name="call_for_copy_previous_note" id="call_for_copy_previous_note" value="<%=call_for_copy_previous_note %>"/><!--IN063931-->
		<input type="hidden" name="tempcurr_sec_hdg_code" id="tempcurr_sec_hdg_code" value="<%=tempcurr_sec_hdg_code %>"/><!--IN063931-->
		<input type='hidden' name='Sex' id='Sex' value='<%=Sex%>'><!--IN072879-->	
		<input type='hidden' name='speciality_name' id='speciality_name' value='<%=speciality_name%>'><!--IN072879-->
		<input type='hidden' name='location_code' id='location_code' value='<%=location_desc%>'><!--IN072879-->
		
		<input type="hidden" name="accession_num_auto" id="accession_num_auto" value="<%=accession_num_auto %>"/><!--IN071264 Starts-->
		<%
		//IN052940 End.
...
//			query_section_details = " select sec_hdg_code,1 DOC_EXISTS,replace(sec_hdg_desc,'''','') sec_hdg_desc, sec_content_type, sec_result_linked_rec_type, NVL(child_sec_hdg_code,sec_hdg_code) child_sec_hdg_code, replace(child_sec_hdg_desc,'''','') child_sec_hdg_desc, child_sec_content_type, child_sec_result_link_rec_type, CHIEF_COMPLAINT_SEC_YN, IMAGE_LINKED_YN,'Y' CONTENT_EDITABLE_YN,nvl(child_sec_result_link_rec_type,sec_result_linked_rec_type) result_link_type,nvl(child_sec_content_type,sec_content_type) content_type  from ca_note_section_view where note_type = ?  order by note_sec_seq_num, child_seq_num ";
		...
		// Test Code For Tracing Bean object added on 17/07/2008 By Sridhar Reddy
			...
			while(rs.next())
			{
				...
				//	traceBeanOBJ.append("sectionBean   Object  :   "+sectionBean+"PatientSection key   ***"+resultKey+"***login_user_id  =  "+login_user_id+"**encounter_id**"+encounter_id+"**patient_id**"+patient_id+"**SectionContent in Bean in RecClinicalNotesSection    ***"+sectionBean.getSectionContent(sec_hdg_code,child_sec_hdg_code)+"***");
					...
					if(!(sec_hdg_code.equals(prev_sec_hdg_code)))
					{
						...
						%>
						<tr id='<%=tblrow_id%>' >
							<td class='<%=classValue%>'  width="1%">
							<b class='gridLink' onclick = "javascript:expandCollapse('<%=cnt%>','<%=sec_hdg_code%>')" name = "no_of_sub_sections_<%=sec_hdg_code%>_anchor" id = "no_of_sub_sections_<%=sec_hdg_code%>_anchor" style="visibility:hidden;cursor:pointer;color:blue" >-</b>
							</td>						
							<td class='<%=classValue%>' colspan='2'  >	<%=sec_content_type.equals("S")?"<b>":""%>&nbsp;
								<a class='gridLink' id='<%=tblrow_id%>_anchor' <%=anchorColor%>  href='javascript:showSectionDetails(<%=tblrow_id%>,"<%=sec_hdg_code%>","<%=child_sec_hdg_code%>","<%=sec_hdg_desc%>","<%=child_sec_hdg_desc%>","<%=image_linked_yn%>","<%=result_link_yn%>","<%=operation_mode%>","<%=content_type%>","<%=chief_complaint_sec_yn%>","<%=result_link_type%>","<%=sec_content_type%>","<%=cnt%>","","<%=selection_type%>","<%=accession_num_auto%>");' ><%=sec_hdg_desc%> <%=(chief_complaint_sec_yn.equals("Y"))?"(<b><i style='color:red' >c</i></b>)":"" %><%=sec_content_type.equals("S")?"</b>":""%><!-- Added for IN071264 -->
								</a >
								<input type="hidden" name="no_of_sub_sections_<%=sec_hdg_code%>" id="no_of_sub_sections_<%=sec_hdg_code%>" value="">
								<%
								...
								{//IN071264 Starts
									if("Y".equals(auto_save_yn) && ""!=image_content_tmpupdate_as){
										...
										%>
										<input type='hidden' name='img_<%=sec_hdg_code%>_<%=child_sec_hdg_code%>' id='img_<%=sec_hdg_code%>_<%=child_sec_hdg_code%>' value = "<%=image_content_tmpupdate_as%>">
										(<B style="color:red" ><I>i</I></B>)							
										<%image_content_tmpupdate_as="";
									}else{//IN071264 Ends
								%>
								<input type='hidden' name='img_<%=sec_hdg_code%>_<%=child_sec_hdg_code%>' id='img_<%=sec_hdg_code%>_<%=child_sec_hdg_code%>' value = "<%=image_content%>">
								(<B style="color:red" ><I>i</I></B>)							
								<%
								}//IN071264 
								}
					...
					if(sec_content_type.equals("S"))
					{
						...
						if(child_sec_hdg_desc.equals(""))
						{
							%>
								<tr id='<%=tblrow_id%>' >
									...
									<td class='<%=classValue%>' style="color:red"  colspan = '2'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&lt;<%="No Sub Section Exists"%>&gt;</td>
								</tr>
							<%
						}
						else
						{
							...
							<tr id='<%=tblrow_id%>' >
								<td class='<%=classValue%>'>&nbsp;&nbsp;</td>
								<td class='<%=classValue%>'>&nbsp;&nbsp;&nbsp;&nbsp;</td>
								<td class='<%=classValue%>'><a id='<%=tblrow_id%>_anchor'  href='javascript:showSectionDetails(<%=tblrow_id%>,"<%=sec_hdg_code%>","<%=child_sec_hdg_code%>","<%=sec_hdg_desc%>","<%=child_sec_hdg_desc%>","<%=image_linked_yn%>","<%=result_link_yn%>","<%=operation_mode%>","<%=content_type%>","<%=chief_complaint_sec_yn%>","<%=result_link_type%>","<%=sec_content_type%>","<%=cnt%>","<%=sub_section_counter%>","<%=output_form_exists_yn%>","<%=accession_num_auto %>");' ><%=child_sec_hdg_desc%><!--IN071264-->
							</a>
							<%
							if(image_linked_yn.equals("Y"))
							{
								...
								%>
									<input type='hidden' name='img_<%=sec_hdg_code%>_<%=child_sec_hdg_code%>' id='img_<%=sec_hdg_code%>_<%=child_sec_hdg_code%>' value = "<%=image_content%>">(<B style="color:red" ><I>i</I></B>)
								<%
								}//IN071264
							}
							%>
			...
			{%>
				<tr id='<%=tblrow_id%>'>
					<td style="background-color:#FFDEAD" colspan='3'><a id='<%=tblrow_id%>_anchor' href='javascript:showSectionDetails(<%=tblrow_id%>,"*AmmendNotes*","*AmmendNotes*","Addendum To Note","","","","<%=operation_mode%>","","","","","","","<%=output_form_exists_yn%>","<%=accession_num_auto%>");'>&nbsp;&nbsp;&nbsp;<b>-&nbsp;<fmt:message key="eCA.AddendumToNote.label" bundle="${ca_labels}"/></b></a></td><!--IN071264-->
				</tr>
...
%>
<script>
	setTimeout("parent.RecClinicalNotesToolbarFrame.document.forms[0].noteCopiedYN.value='<%=noteCopiedYN%>'",500);//IN068105
</script>
<%
}
//IN068105
...
%> 
<script>
		var counts = '<%=cnt_ofMainSections%>'.split("|");
		var sec_hdg_codes = '<%=ids_of_mainSections%>'.split("|");
		var ii = 0;
...
/*		var noOfSec = "<%=cntSec%>";
			var operationMode = "<%=operation_mode%>";
			//alert("operationMode"+operationMode);
			if ((parseInt(noOfSec) == 1) && (operationMode==""))
			{											showSectionDetails("<%=tblrow_id%>","<%=sec_hdg_code%>","<%=child_sec_hdg_code%>","<%=sec_hdg_desc%>","<%=child_sec_hdg_desc%>","<%=image_linked_yn%>","<%=result_link_yn%>","<%=operation_mode%>","<%=content_type%>","<%=chief_complaint_sec_yn%>","<%=result_link_type%>","<%=sec_content_type%>","<%=cnt%>","","<%=output_form_exists_yn%>");			
			}		*/
</script>
