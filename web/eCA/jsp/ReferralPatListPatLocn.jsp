...
<script>
	 function callResult(obj, obj1,current_pos)
	 {
		if(obj1 != "")
		{
			...
			top.content.workAreaFrame.PatResultFr.LocnResult.location.href = '../../eCA/jsp/ReferralPatListResult.jsp?<%=queryString%>&locn_type='+obj+'&loc_code='+obj1;
		}
	}
</script>
</head>
...
%>
						<td class='<%=className%>' align="center" onMouseOver='callOnMouseOver(this);' onMouseOut='callOnMouseOut(this);'style='cursor:pointer;'onclick="displayLoc(this);callResult('<%=pat_type%>','<%=locn_code%>','<%=labelCounter%>');"  width='15%' align='center'><a href="javascript:callResult('<%=pat_type%>','<%=locn_code%>','<%=labelCounter%>');"><%=locn_short_desc%></a></td>
<%
					}
...
<%		
					...
					if(flagLastYN.equals("true"))
					{
						...
						locn_code		=	(String)hLocn.get("LOCN_CODE");
%>
						<td class='<%=className%>' align="center" onMouseOver='callOnMouseOver(this);' onMouseOut='callOnMouseOut(this);''style='cursor:pointer;' onclick="displayLoc(this);callResult('<%=pat_type%>','<%=locn_code%>','<%=labelCounter%>');"  width='15%' align='center'><a href="javascript:callResult('<%=pat_type%>','<%=locn_code%>','<%=tempLabelCounter%>');"><%=locn_short_desc%></a></td>
<%
					}
%>
				...
				<input type="hidden" name="pat_type" id="pat_type" value="<%=pat_type%>">
