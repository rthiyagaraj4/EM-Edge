...
<%
	...
	<tr ><td width='85%' class='CAGROUP' ><b><font size='2'><fmt:message key="Common.searchresult.label" bundle="${common_labels}"/></b></td>
	</tr>
	</table>
	<table class='grid' width='100%'  id='record'>
	<tr>
		<th class='columnheadercenter' width="1%" ><div  id='head0'  class='myClass' nowrap width='3%'>&nbsp;&nbsp;<fmt:message key="Common.all.label" bundle="${common_labels}"/><input type='checkbox' name='ctrlChkBox' id='ctrlChkBox' onClick="javascript:perform(this);" ></div></th>
			...
			if(fields.size()>0)
			{
				...
				for(int i=0;i<fields.size();++i)
				{
					...
					%>
						...
						<th class='columnheadercenter' width='<%=col_width%>%'><div  id='head2' class='myClass' nowrap><fmt:message key="Common.EventType.label" bundle="${common_labels}"/></div></th>
					<%
					}
					else if(col_field.equals("ALLERGEN"))
					{
					%>
						<th class='columnheadercenter' width='<%=col_width%>%'><div  id='head3' class='myClass' nowrap ><fmt:message key="eMR.Allergen.label" bundle="${mr_labels}"/></div></th>
					<%
					}
					else if(col_field.equals("ONSETDATE"))
					{
					%>
						<th class='columnheadercenter' width='<%=col_width%>%' ><div  id='head4' class='myClass' nowrap ><fmt:message key="Common.onsetdate.label" bundle="${common_labels}"/></div></th>
					<%
					}
					else if(col_field.equals("Status"))
					{
					%>
						<th class='columnheadercenter' width='<%=col_width%>%'><div  id='head5' class='myClass' nowrap ><fmt:message key="Common.status.label" bundle="${common_labels}"/></div></th>
					<%
					}
	...
	<tr>
		<th width='3%'><div  id='head0'  class='myClass' nowrap width='3%'><fmt:message key="Common.all.label" bundle="${common_labels}"/><input type='checkbox' name='ctrlChkBox' id='ctrlChkBox' onClick="javascript:perform(this);" ></div></th>
		<th width='3%'><div  id='head1'  class='myClass' nowrap width='3%'>&nbsp;&nbsp;</div></th> -->
		<th class='columnheadercenter' width='15%'><div  id='head2' class='myClass' nowrap width='24%'><fmt:message key="Common.EventType.label" bundle="${common_labels}"/></div></th>
		<th class='columnheadercenter' width='15%'><div  id='head3' class='myClass' nowrap width='25%'><fmt:message key="eMR.Allergen.label" bundle="${mr_labels}"/></div></th>
		<th class='columnheadercenter' width='15%'><div  id='head4' class='myClass' nowrap width='25%'><fmt:message key="Common.onsetdate.label" bundle="${common_labels}"/></div></th>
		<th class='columnheadercenter' width='15%'><div  id='head5' class='myClass' nowrap width='23%'><fmt:message key="Common.status.label" bundle="${common_labels}"/></div></th>
	</tr>
	<% }
		%>
		</table>
...
/*					out.println("<script>");
	 ...
	 %>
		...
		<tr id='first' width=''><td colspan='6'>
			...
			<tr>
			<td class="<%=classValue%>" align='left'><input type="checkbox" name="chkbox<%=k%>" id="chkbox<%=k%>" onClick="ReassaignAlrg(this);" value="<%=k%>" >
			<input type='hidden' name='checking_value<%=k%>' id='checking_value<%=k%>' value='<%=z%>'>
				<%k++;
				%>
			...
			for(int n=0;n<fields.size();++n)
					{
						...
						{
						%>
						<td  class='<%=classValue%>' nowrap align='left'width='<%=col_width%>%'><%=adv_event_type%></td>
						...
						{
						%>
							<td   class='<%=classValue%>'  align='left' nowrap width='<%=col_width%>%' id='header'>
							<%=allergen%>
							</td>
						<%
						}
						...
						{
						%>
						 <td  class='<%=classValue%>' nowrap  align='left' width='<%=col_width%>%' ><%=onset_date_disply%> 
						 <%
						...
						<%if(cnt_recurring.equals("0")){%>
							<td  class='<%=classValue%>'  align='left' nowrap width='<%=col_width%>%' ><%=status%></td>
							<%}else {%>
						<td  class='<%=classValue%>'  align='left' nowrap width='<%=col_width%>%'	><%=status%>(<%=cnt_recurring%>)<img src='../../eCommon/images/mandatory.gif' ></img></td>
						<%}%>
		...
		<tr id='third<%=m%>' Style="display;visibility:visible"><td class='<%=classValue%>'  >&nbsp;</td><td class='<%=classValue%>' >&nbsp;</td>
			<td class=<%=classValue%> width='' nowrap><b><fmt:message key="Common.ReactionDate.label" bundle="${common_labels}"/></b></td>
			<td class=<%=classValue%> width='' nowrap ><b><fmt:message key="eMR.reactiondesc.label" bundle="${mr_labels}"/></b></td>
			<td class=<%=classValue%> width='' nowrap><b><fmt:message key="Common.Severity.label" bundle="${common_labels}"/></b></td>
			<td class=<%=classValue%> width='' nowrap><b><fmt:message key="eMR.SiteOfReaction.label" bundle="${mr_labels}"/></b></td>
		</tr>
			
			<tr id='second<%=m%>' Style="display;visibility:visible" ><td class='<%=classValue%>'  >&nbsp;</td><td class='<%=classValue%>' >&nbsp;</td>
			<td  class='<%=classValue%>' width='' nowrap ><%=reaction_date1_disply%></td>
			

			<% if(adv_reac_code.equals("Oth")) 
			{ %>
			<td  class='<%=classValue%>' width=''  nowrap><%=others_reaction%></td>
			<%} else { 
				
			%>
			<td  class='<%=classValue%>' width='' nowrap ><%=reac_desc%></td>
			<%}%>
			<td  class='<%=classValue%>' width='' nowrap ><%=severity_two%></td>
			<td  class='<%=classValue%>' width=''  nowrap><%=reaction_site%></td>
			...
			<td class=<%=classValue%>  width='' nowrap><fmt:message key="Common.ReactionDate.label" bundle="${common_labels}"/></td>
			<td class=<%=classValue%>  width=''  nowrap><fmt:message key="eMR.reactiondesc.label" bundle="${mr_labels}"/></td>
			<td class=<%=classValue%> width='' nowrap ><fmt:message key="Common.Severity.label" bundle="${common_labels}"/></td>
			<td class=<%=classValue%> width='' nowrap ><fmt:message key="eMR.SiteOfReaction.label" bundle="${mr_labels}"/></td>
		</tr></table>
		</td></tr>
		...
		<tr id='first' >
			<td class="<%=classValue%>" ><input type="checkbox" name="chkbox<%=k%>" id="chkbox<%=k%>" onClick="ReassaignAlrg(this);" value="<%=k%>" >
			<input type='hidden' name='checking_value<%=k%>' id='checking_value<%=k%>' value='<%=z%>'>
				<%k++;%>
		...
		<tr id='third<%=m%>' Style="display;visibility:visible">
			...
			<td  class='<%=classValue%>' nowrap ></td>
			<td  class='<%=classValue%>' nowrap></td>
			<td class=<%=classValue%> nowrap><fmt:message key="Common.ReactionDate.label" bundle="${common_labels}"/></td>
			<td class=<%=classValue%>   nowrap><fmt:message key="eMR.reactiondesc.label" bundle="${mr_labels}"/></td>
			<td class=<%=classValue%>  nowrap><fmt:message key="Common.Severity.label" bundle="${common_labels}"/></td>
			<td class=<%=classValue%>  nowrap><fmt:message key="eMR.SiteOfReaction.label" bundle="${mr_labels}"/></td>
		</tr>
		<tr id='second<%=m%>' Style="display;visibility:visible" >
			<td  class='<%=classValue%>' nowrap ></td>
			<td  class='<%=classValue%>' nowrap ></td>
