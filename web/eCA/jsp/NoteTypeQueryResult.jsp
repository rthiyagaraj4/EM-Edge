...
ResultSet rset = null;
ResultSet rs = null;
isClinician = eCommon.Common.CommonBean.isSiteSpecific(conn, "CA","CLINICIAN_TO_THERAPIST");//IN067236
...
<%
if ( !(start <= 1) )
    out.println("<A class='gridLink' HREF='../../eCA/jsp/NoteTypeQueryResult.jsp?from="+(start-pageCnt)+"&to="+(end-pageCnt)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A>");

if ( !( (start+pageCnt) > maxRecord ) )
    out.println("&nbsp;<A class='gridLink' HREF='../../eCA/jsp/NoteTypeQueryResult.jsp?from="+(start+pageCnt)+"&to="+(end+pageCnt)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
%>
</td>
</tr>
</table>
...
<th class='columnheadercenter' align='left' width='20%'><fmt:message key="Common.code.label" bundle="${common_labels}"/></th>
<th class='columnheadercenter' align='left' width='30%'><fmt:message key="Common.description.label" bundle="${common_labels}"/></th>
<th class='columnheadercenter' align='left' width='25%'><fmt:message key="eCA.NoteGroup.label" bundle="${ca_labels}"/></th>
<%
if(license_rights.equals("FULL") || license_rights.equals("BASIC"))
{
%>
<th class='columnheadercenter' align='left' width='30%'><fmt:message key="Common.Class.label" bundle="${common_labels}"/></th>
<%}%>
<th class='columnheadercenter' align='left' width='20%'><fmt:message key="Common.enabled.label" bundle="${common_labels}"/></th>
<th class='columnheadercenter' align='left' width='20%'><fmt:message key="Common.Confidential.label" bundle="${common_labels}"/></th>
      ...
      while ( rset.next() && i<=end  )
{
	...
	out.println("<tr><td align=='left' class='gridData'>");
	...
	//IN067236 starts
	else if(event_class1.equals("CLI$")) {
		if(isClinician)
			event_class_desc="Therapist";
		else
			 event_class_desc="Clinician";//IN067236ends
	}
	...
	out.println("<a class='gridLink' href='../../eCA/jsp/NoteTypeAddModify.jsp?mode=update&note_type="+ URLEncoder.encode(note_type1)+"' target='f_query_add_mod' >");
    out.println(note_type1 + "</a></td><td class='gridData'>");
