...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="MATRIX">
	    ...
	    <LEGEND><b><xsl:value-of select="@MATRIX-DESC"/></b></LEGEND>
	    <table border="1" width="98%" cellspacing="0" cellpadding="0" style="margin-left:5;margin-right:5;margin-top:5;margin-bottom:5;">
    ...
    <xsl:template match="MATRIX-TH">
        ...
        <tr>                         
            <td align="right" width="5%" border="1">
            </td>
    ...
    <xsl:template match="MATRIX-ROW">
        <tr>
            <td align="left" width="21%"  style = "BACKGROUND-COLOR:#9499C7;color:black;font-size:10pt;font-weight:bold;">
    ...
    <xsl:template match="GRID">
        ...
        <FIELDSET style="margin-left:5;margin-right:5;margin-top:5;margin-bottom:5;">
	<LEGEND> <b><xsl:value-of select="@DESCRIPTION"/></b></LEGEND>
	<table border="1" width="98%" cellspacing="0" cellpadding="0" style="margin-left:5;margin-right:5;margin-top:5;margin-bottom:5;">
            <xsl:attribute name="ID"><xsl:value-of select="@NAME"/></xsl:attribute>
            <xsl:apply-templates/>
        </table>
  ...
  <xsl:template match="GRID-TH">
    ...
    <tr>
        ...
        <xsl:for-each select ="TH">
        <td class='COMMONTOOLBARFUNCTION'><center>
            <xsl:attribute name="colspan"><xsl:value-of select="@COLSPAN"></xsl:value-of></xsl:attribute>
            <xsl:value-of select=".">
            </xsl:value-of>
        </center></td>
        </xsl:for-each>
        <td class='COMMONTOOLBARFUNCTION' width="3%" border="1">
  ...
  <!--=======================================================================-->
   ...
   <xsl:param name="LANGID"/>	
	    ...
	    <xsl:attribute name="class">label</xsl:attribute>
	   <!-- <b><font color='navy'><xsl:value-of select="@UNIT"/></font></b>-->
            <image src="../../eCA/images/MCT_calcResult.gif" title='Click here to find the formula result' style="cursor:hand" align="center" WIDTH="15"  HEIGHT="18" >
		<xsl:choose>
			<xsl:when test="@EDITABLE = 'Y' or @EDITABLE = null">
  ...
  <xsl:template match="LIST-BOX" >
    <xsl:if test="not(@GRID-COMP|@MATRIX-COMP)">
        <td align="right" style="padding-top:5;padding-bottom:5;">
            <xsl:attribute name="class">label</xsl:attribute>
	    <xsl:attribute name="title"><xsl:value-of select ="@HINT_MESSAGE" /></xsl:attribute>
            <xsl:value-of select="@PROMPT"/>&#160;
            <xsl:if test="@NOWRAP='true'">
 ...
 <!--   <xsl:choose>
  ...
  <xsl:otherwise>-->
      ...
      <td align="left" style="padding-top:5;padding-bottom:5;" >
            <xsl:if test="@GRID-COMP = 'Y'">
			...
			<!-- ONBLUR LOOKUP FUNCTION END -->
					<script>
						function lookUp<xsl:value-of select="@NAME"/>(str)
						{
							...
							var encounter_id ="";
							var patient_id ="";
							var MAST_TYPE_REF="<xsl:value-of select="@DISCRID"/>";
							var tempTitle = "";
							var speciality_code = "";
							var facility_id = "";
							...
							var action_url = "<xsl:value-of select="@LOOKUP_URL"/>?std_comp_id=<xsl:value-of select="@STD_COMP_ID"/>&amp;title="+tempTitle+"&amp;encounter_id="+encounter_id+"&amp;patient_id="+patient_id+"&amp;MAST_TYPE_REF="+MAST_TYPE_REF+"&amp;speciality_code="+speciality_code+"&amp;facility_id="+facility_id+"";
							var returnArray		=	new Array();
							var dialogHeight	=	"25" ;
							var dialogWidth		=	"50" ;
							var status			=	"no";
							...
							{							
								//document.getElementById(str).focus();
								eval("document.all."+str).value =returnArray[1];
								// document.all.<xsl:value-of select="@NAME"/>.value = unescape(returnArray[1]);
								// document.all.<xsl:value-of select="@NAME"/>_desc.value = returnArray[0];							
					...
					<input type="image" src = "../../eCA/images/Lookup.gif"  align="center" WIDTH="15"  HEIGHT="18" style="padding-top: 3px;background: url(../../eCA/images/Lookup.gif) no-repeat center;background-size: contain; border: none; width: 15px; min-height: 18px; cursor: pointer;" > 
						<xsl:if test="@GRID-COMP='Y'">
   ...
   <xsl:template name="LIST-BOX-WTD" match="LIST-BOX-WTD">
	   ...
	   <xsl:choose>
				<xsl:when test="@STD_COMP_RESULT_TYPE ='L'">
					...
					<image src = "../../eCA/images/Lookup.gif"  style="padding-top: 3px;background: url(../../eCA/images/Lookup.gif) no-repeat center;background-size: contain; border: none; width: 15px; min-height: 18px; cursor: pointer;"> 
						<xsl:attribute name="onClick">
	   ...
	   <xsl:if test="@STD_COMP_RESULT_TYPE !='L'">
	    <!--<xsl:if test="@LIST_PRESENTATION ='S'">-->
	   <select>
                <xsl:if test="@GRID-COMP = 'Y'">
                    <xsl:attribute name="id">list_box_component_of_<xsl:value-of select="@GRID-NAME"/><xsl:value-of select="../@COLINDEX"/></xsl:attribute>
                </xsl:if>
  ...
  <xsl:template match="SHORT-TEXT">
   ...
   <xsl:template name="SHORT-TEXT-WTD" match="SHORT-TEXT-WTD">  
   	<textarea rows="1">            
	    ...
	    <xsl:attribute name="ONBLUR">return allowMaximumCharacters(this)</xsl:attribute>
	    <xsl:attribute name="ONKEYPRESS">return allowMaximumCharacters(this)</xsl:attribute>
            <xsl:value-of select="."/>
        </textarea>
   </xsl:template>
  <!--===========================================================================================-->
  ...
  <xsl:template match="LONG-TEXT">    
       ...
       <td align="left" >
	       ...
	       <image src="../../eCA/images/MCT_LinkDoc.gif" title='Click here for result Lookup' style="cursor:hand" align="center" WIDTH="15"  HEIGHT="18" >
   ...
   <xsl:template name="DATE-TIME-NUMERIC-WTD" match="DATE-TIME-NUMERIC-WTD">    
       ...
       <input>
		 <xsl:attribute name="title">
            ...
            <xsl:if test="@GRID-COMP='Y'">
                ...
                <xsl:attribute name="name"><xsl:value-of select="@NAME"/></xsl:attribute>                
                <xsl:if test="@READONLY='true' or @EDITABLE='N'">
                    <xsl:attribute name="readonly"></xsl:attribute>
                    <xsl:attribute name="onKeyDown">return restrictBackSpace(this)</xsl:attribute>
                </xsl:if>
                <xsl:if test="@DISABLED='true'  ">
		...
		<xsl:choose>
			<xsl:when test="@COMPTYPE='D'">
				...
				<xsl:choose>
				<xsl:when test="@ONBLURREQ='Y'">
				<xsl:attribute name="onBlur">return getFormulaResult1(this,'<xsl:value-of select="@DISCRID"/>','<xsl:value-of select="@SECSEQ"/>','<xsl:value-of select="@INPUT"/>','<xsl:value-of select="@MINGESPARAMVAL"/>','<xsl:value-of select="@MAXGESPARAMVAL"/>','<xsl:value-of select="@GESUNIT"/>')</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="@EDITABLE = 'Y' and @COMPID ='C_MC01CORRECTDEDD'">
							<xsl:attribute name="onBlur">return CheckValidDate(this,'<xsl:value-of select="@EDD"/>','<xsl:value-of select="@CREDDPARAMVAL"/>','<xsl:value-of select="@MINGESPARAMVAL"/>','<xsl:value-of select="@MAXGESPARAMVAL"/>','<xsl:value-of select="@GESUNIT"/>')</xsl:attribute>						
						</xsl:when>
						<xsl:otherwise>
							<!--<xsl:attribute name="onBlur">return CheckDate(this)</xsl:attribute>--><!-- Commented for IN071708-->
							<xsl:attribute name="onBlur">return CheckDateNew(this)</xsl:attribute><!-- Added for IN071708-->
						</xsl:otherwise>
					</xsl:choose>
				</xsl:otherwise>
	 		       </xsl:choose>
			...
			<xsl:when test="@COMPTYPE='I'">					
				<xsl:attribute name="onBlur">return checkValidRange(this)</xsl:attribute>
				<xsl:attribute name="OnKeyPress">return allowValidNumber(this,event,<xsl:value-of select="@SIZE"/>,0)</xsl:attribute>
				<!--<xsl:attribute name="OnMouseOver">return displaytt(this)</xsl:attribute>-->
				<xsl:attribute name="value"><xsl:value-of select="@VALUE"/></xsl:attribute>
			</xsl:when>
			<xsl:when test="@COMPTYPE='N'">					
				<xsl:attribute name="onBlur">return checkValidRange(this)</xsl:attribute>	
				<xsl:attribute name="OnKeyPress">return checkValid(this,event,<xsl:value-of select="@SIZE"/>,<xsl:value-of select="@DEC_DIGIT"/>)</xsl:attribute>
				<xsl:attribute name="value"><xsl:value-of select="@VALUE"/></xsl:attribute>
			</xsl:when>
		</xsl:choose>
         </input>
	  ...
	  <!--shaiju added -->	
		...
		<xsl:apply-templates select="LINK-HISTORY-BUTTON"/>
	      ...
	      <xsl:otherwise>
			<xsl:if test="@COMPTYPE='D' or @COMPTYPE ='E'">  
			<xsl:if test="@EDITABLE='Y' or @EDITABLE =null ">  			
			<img src = "../../eCommon/images/CommonCalendar.gif">
				<xsl:if test="@GRID-COMP='Y'">
				    <xsl:attribute name="id">calender_component_of_<xsl:value-of select="@GRID-NAME"/><xsl:value-of select="../@COLINDEX"/></xsl:attribute>
				</xsl:if>
