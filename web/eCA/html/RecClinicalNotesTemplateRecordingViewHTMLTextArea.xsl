        ...
        <head>            
            ...
            <body>
				...
				<form>
					...
					<xsl:attribute name="id"><xsl:value-of select="@FORM-NAME"/></xsl:attribute>
	 	...
	 	<script>
			...
			setTimeout(function(){ //IN055628
				for(i=0;i&lt;len;i++){
					...
					document.getElementById(input.id).contentWindow.document.body.contextmenu = "False";
  ...
  <xsl:otherwise>-->
   ...
   <xsl:template name="LIST-BOX-WTD" match="LIST-BOX-WTD">
	   ...
	   <select>
				...
				<!-- <xsl:attribut name="class">easyui-combobox</xsl:attribute>-->
                <xsl:if test="@DISABLED='true'">
                    <xsl:attribute name="disabled"></xsl:attribute>
                </xsl:if>
			...
			<table  width="100%" border="0">
				<tr>
					...
					<td align="left" valign="bottom" nowrap="nowrap" width="10%">
						<xsl:if test="@RECORD-TYPE!=''">
							<input type="button" class="button" value="Link Doc" name = "" onClick="" >
								...
								<!--IN061907 START-->
								<!--<xsl:attribute name="onClick">showLinkDocumentsHTMLFromTemplate('<xsl:value-of select="@RECORD-TYPE"/>','<xsl:value-of select="@NAME"/>')</xsl:attribute>-->
					...
					<td align="left" nowrap="nowrap" width="65%">
						<xsl:if test="@MANDATORY='Y'">
							&#32;
							<IMAGE valign="bottom"  SRC="../../eCommon/images/mandatory.gif"/>
						</xsl:if>
...
<!--<xsl:attribute name="onBlur">chkMaxLenForLongTest('<xsl:value-of select="@NAME"/>','<xsl:value-of select="@PROMPT"/>')</xsl:attribute> 68801 Twinkle-->		
<xsl:attribute name="onBlur">chkMaxLenForLongTest('<xsl:value-of select="@NAME"/>','<xsl:value-of select="@PROMPT"/>','','<xsl:value-of select="@DISCRID"/>')</xsl:attribute>           
...
<!-- <xsl:attribute name="onBlur">chkMaxLenForHTMLLongTest('<xsl:value-of select="@NAME"/>','<xsl:value-of select="@PROMPT"/>')</xsl:attribute> 68801 by Twinkle-->	
 <xsl:attribute name="onBlur">chkMaxLenForHTMLLongTest('<xsl:value-of select="@NAME"/>','<xsl:value-of select="@PROMPT"/>','','<xsl:value-of select="@DISCRID"/>')</xsl:attribute>
       ...
       <input>
		...
		<xsl:choose>
			...
			<xsl:if test="not(@GRID-COMP)"> 
				...
				<!--<xsl:attribute name="onBlur">if(CheckDate(this)){getFormulaResult();<xsl:value-of select="@ONBLUR"/>}</xsl:attribute>--><!--Commented for IN071708 -->
				<xsl:attribute name="onBlur">if(CheckDateNew(this)){getFormulaResult();<xsl:value-of select="@ONBLUR"/>}</xsl:attribute><!--Added for IN071708 --><!--INC61905-->
