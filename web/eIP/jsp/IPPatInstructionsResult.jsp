<!DOCTYPE html>
<%--
	FileName		: IPPatInstructionsResult.jsp Included for the CRF - RUT-CRF-013 23396
...
{ 
  ...
  nursing_unit_code=(request.getParameter("nursing_unit_code")==null)?"":request.getParameter("nursing_unit_code");  
	
		if(mode.equals("add"))
		{
			if(!(serial_no.equals("") && instruction_id.equals("") && instruction_desc.equals("")) )		
			{	
          //modified by N Munisekhar on 09-May-2012 against [IN:39901]			
			boolean ser=true;
			boolean inst=true;
		...
		{ 			
			...
			sqlmodify = "select INSTRUCTION_ID, AM_GET_DESC.AM_PAT_INSTRUCTIONS(INSTRUCTION_ID, '"+locale+"','1') description, DISPLAY_ORDER, DEFAULT_LINKED_YN from IP_NURS_PAT_INSTRUCTIONS where FACILITY_ID=? and nursing_unit_code=? order by DISPLAY_ORDER";
...
<form name='PatInstructionsResult_form' id='PatInstructionsResult_form'>
	<table border='1' cellpadding='0' cellspacing='0' width='101%' align='center'>
		<tr>
			...
			<th width='65%'><fmt:message key="eAM.InstructionDescript.label" bundle="${am_labels}"/></th>
			<th width='13%'><fmt:message key="eAM.InstructionID.label" bundle="${am_labels}"/></th>
			<th width='12%' nowrap><fmt:message key="Common.Default.label" bundle="${common_labels}"/>&nbsp;-&nbsp;<fmt:message key="Common.defaultSelect.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.to.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="Common.print.label" bundle="${common_labels}"/></th>
		</tr>
		<%
			...
			{  
					...
					out.println("<td class='" + classValue + "' align='center' width ='7%' nowrap><center><input type=checkbox name=default_yn "+check_default+"  onClick=deafult_check(this,'"+j+"')></center></td></tr>");
