<%@page import="java.util.*,java.sql.*,java.util.List,eIPAD.*,eIPAD.HealthObjects.*,eIPAD.sfs.*,java.io.*,java.net.*,java.text.*,webbeans.eCommon.*,webbeans.eCommon.ConnectionManager,com.ehis.util.*" contentType="text/html;charset=UTF-8" %>
<%
	...
	if (session.getValue("ca_practitioner_id") != null
			&& session.getValue("ca_practitioner_id") != "") {
		sclinicianId = (String) session.getValue("ca_practitioner_id");
		oSFSContext.setSclinicianId(sclinicianId);
	}
	if (request.getParameter("sPractionerId") != null
			&& request.getParameter("sPractionerId") != "") {
	...
	if (request.getParameter("cmbCriteria") != null
			&& request.getParameter("cmbCriteria") != "") {
		sSearchCriteria = (String) request.getParameter("cmbCriteria");
		oSFSContext.setsSearchCriteria(sSearchCriteria);
	}
	if (request.getParameter("txtSrchCriteria") != null
			&& request.getParameter("txtSrchCriteria") != "") {
...
function openPatient()
 ...
 function srchclose(strCode,strDesc,objTr)
 {
	...
	window.parent.Practitioner(strCode,strDesc);
	//alert(objTr);
 }
 function dummyFn()
 {
...
<body onload = "LoadDefaultValues()">
  ...
  <div id = "divSrch"  data-role ="none" class="dvApp" >
    <span class = "spanDvsrch">Practitioner</span> 
  ...
  <td align = "left" style="width:2%"></td>
   ...
   <%
   	...
   	List<SFSUnit> objPractitionerDetails = new ArrayList<SFSUnit>();

   	try {
   		if (strPostBack != null && strPostBack.equals("true")) {
			// set the type before getting the data
			oSFSContext.setType(SFSContext.PRACTITIONER);
   			objPractitionerDetails = bean.getSFSUnit(oSFSContext);
   		}

   		nListSize = objPractitionerDetails.size();

   		SFSUnit oPractitioner = null;
   		if (nListSize > 0) {
   			for (nLoopCount = 0; nLoopCount < nListSize; nLoopCount++) {
   				oPractitioner = objPractitionerDetails.get(nLoopCount);
   				if (oPractitioner != null) {
   %>
     <table class = "tbIpContent2" cellpadding = "0" cellspacing = "0"  border = "0">
      <tr class = "trSrchRow<%=nCssCount%>"  onclick = "srchclose('<%=oPractitioner.getsCode()%>','<%=oPractitioner.getsDescription()%>',this)">
        <td style="width:30%" align = "left" valign = "center">
          <span class = "srchLabel"><%=oPractitioner.getsCode()%></span>
        </td>
        <td style="width:70%" align = "left" valign = "center">
            <span class = "srchLabel"><%=oPractitioner.getsDescription()%></span>
        </td>
