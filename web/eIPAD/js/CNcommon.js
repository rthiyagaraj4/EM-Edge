...
async function PatientSearch(jsp_name,window_styl,close_yn,win_top,win_height,win_width,act_yn,srr,register_button_yn,func_act,dr_function_id,patient_class) {
...
////*** Start Function to Show the patient Image in a Modal Window Added by ChandraSekhar M  ***///
	...
	async function showPatientImage(patient_id) {		
			...
			var arguments = "" ;			 
			var imageWindow = await window.showModalDialog( '../../eMP/jsp/ViewPatientImage.jsp?patient_id='+patient_id+"&BrowserVersion="+BrowserVersion()+"&title=Photo", 'eHISMain', myFeatures ) ;	
	}
...
//Start
function MDSCompliance(mr_mds_recorded)
	{
	...
	window.showModalDialog("../../eMR/jsp/MDSCompliance.jsp?mr_mds_recorded="+encodeURIComponent(mr_mds_recorded)+"&title="+title,arguments,features);
	}	
//End	
