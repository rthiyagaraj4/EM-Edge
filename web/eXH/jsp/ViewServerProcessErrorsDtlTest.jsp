...
<tr>
	...
	<td class="COLUMNHEADER"><a id='PROC_ID'  href="javascript:callSort('PROC_ID');"><font color=white><fmt:message key="Common.Process.label" bundle="${common_labels}"/></font></td>
   	<td class="COLUMNHEADER"><a id='FAILED_PROC_ID' href="javascript:callSort('FAILED_PROC_ID');" ><font color=white><fmt:message key="eXH.FailedProcedure.Label" bundle="${xh_labels}"/></font></td>
...
<a id="link" href='#' onClick='funShow(this)' Facility_Id='<%=arrCol.get(0)+""%>' Facility_Name='<%=arrCol.get(1)+""%>' Process_Id='<%=arrCol.get(2)+""%>' Failed_Proc='<%=arrCol.get(3)+""%>' Msg_No='<%=arrCol.get(4)+""%>' Appl_msg='<%=arrCol.get(5)+""%>' Err_msg='<%=arrCol.get(6)+""%>' Msg_date='<%=arrCol.get(7)+""%>' Appl_key='<%=arrCol.get(8)+""%>'>
&nbsp;<%=facility%>
</a>
</b>
</td>
<td class='<%=strTDClass%>' align="center">&nbsp;<%=process%></td>
<td class='<%=strTDClass%>' align="center">&nbsp;<%=failed_proc%></td>
