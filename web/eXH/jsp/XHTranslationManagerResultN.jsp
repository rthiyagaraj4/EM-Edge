...
<!--					 <td class=data style='<%=classValue%>' >&nbsp;</td>  -->
				  ...
				  value='<%=rs.getString((String)keyName)==null?"":rs.getString((String)keyName)%>'  maxlength='<%=pkLen.get(startVal)%>' 
				  size=20 > </td>
				<!--  ext_pk1_value
				 <td class='fields' style='<%=classValue%>' >
 				 <INPUT TYPE="text" name="pkVal<%=startVal%><%=colCount%>" id="pkVal<%=startVal%><%=colCount%>"   onblur='return makeValidString(this);'
				 maxlength='<%=pkLen.get(startVal)%>'  value='<%=rs.getString((String)pkval.get(startVal))==null?"":rs.getString((String)pkval.get(startVal))%>' size=40 > </td> 
				 -->
				 <% 
					 pkCnt++;}
	}
...
for( startVal=0;startVal<pkval.size();startVal++)
  {
  %>
<input type=hidden name=pk<%=colCount%><%=startVal%> id=pk<%=colCount%><%=startVal%> value='<%=pkData[startVal]%>'>  
<%
	}
%>
<td class=data><b><%=columnPrompt.get(colDispCount)%></b></td>

<td class=data ><b><%=(rs.getString((String)colValue.get(colDispCount))==null)? "":(rs.getString((String)colValue.get(colDispCount)))%></b></td>
<%
			...
			<input type="hidden" name="engLang<%=engColCount%><%=colCount%>" id='<%="engLang"+(colDispCount)+"0"%>' value='<%=rs.getString((String)columnVal.get(colDispCount))==null?"":rs.getString((String)columnVal.get(colDispCount))%>'> 
	 	 
	 <%engColCount++ ;
		 rs.next();
		 %>
				<td class='fields' style='<%=classValue%>' ><INPUT TYPE="text" name="desc<%=colDispCount%>0" id="desc<%=colDispCount%>0"  onblur='return makeValidString(this);'
				 maxlength='<%=columnLen.get(colDispCount)%>' value='<%=rs.getString((String)columnVal.get(colDispCount))==null?"":rs.getString((String)columnVal.get(colDispCount))%>' size=60 > </td> 
		<%
		 
%>
		<%---<td class=data><INPUT TYPE="text" name="desc<%=colDispCount%>0" id="desc<%=colDispCount%>0" onblur='return makeValidString(this);'	size=60 maxlength='<%=columnLen.get(colDispCount)%>' value='<%=rs.getString((String)columnVal.get(colDispCount))==null?"":rs.getString((String)columnVal.get(colDispCount))%>' >  </td>----%>
<% /*  }
	} */
	%>
</tr>
 ...
 <%
	...
	if(lang.equals("en"))
	 ...
	 <%engColCount++ ;}else {
		 ...
		 %>
				 <td class='fields' style='<%=classValue%>' ><INPUT TYPE="text" NAME=desc<%=colCount1++%><%=colCount%> ID=desc<%=colCount1++%><%=colCount%> onblur='return makeValidString(this);'
				 maxlength='<%=columnLen.get(startVal)%>' value='<%=rs.getString((String)columnVal.get(startVal))==null?"":rs.getString((String)columnVal.get(startVal))%>' size=50 > </td> 
		<%
		}
