...
<tr>
	<td class='COLUMNHEADER'><a name='MESSAGE_ID' id='MESSAGE_ID' href="javascript:callSort('MESSAGE_ID');" ><font color=white><fmt:message key="eXH.MessageID.Label" bundle="${xh_labels}"/></font></td> 
	<td class='COLUMNHEADER'><a name='FACILITY_ID' id='FACILITY_ID'  href="javascript:callSort('FACILITY_ID');" ><font color=white><fmt:message key="Common.facility.label" bundle="${common_labels}"/></font></td>
   	<td class='COLUMNHEADER'><a name='PROCESS_DATE_TIME' id='PROCESS_DATE_TIME' href="javascript:callSort('PROCESS_DATE_TIME');" ><font color=white><fmt:message key="Common.startdatetime.label" bundle="${common_labels}"/></font></td>
	<td class='COLUMNHEADER'><a name='EVENT_TYPE' id='EVENT_TYPE'  href="javascript:callSort('EVENT_TYPE');" ><font color=white><fmt:message key="Common.Event.label" bundle="${common_labels}"/></font></td>
	<td class='COLUMNHEADER'><a id='PROCESS_ID' id='PROCESS_ID'  href="javascript:callSort('PROCESS_ID');" ><font color=white><fmt:message key="Common.ProcessID.label" bundle="${common_labels}"/></font></td>
    <td class='COLUMNHEADER'><a name='PROCESSED_BY' id='PROCESSED_BY'  href="javascript:callSort('PROCESSED_BY');" ><font color=white><fmt:message key="eXH.ProcessBy.Label" bundle="${xh_labels}"/></font></td>
	<td class='COLUMNHEADER'><a name='END_DATE_TIME' id='END_DATE_TIME'  href="javascript:callSort('END_DATE_TIME');" ><font color=white><fmt:message key="Common.enddatetime.label" bundle="${common_labels}"/></font></td>
	<td class='COLUMNHEADER'><a name='COMM_RETRIES' id='COMM_RETRIES'  href="javascript:callSort('COMM_RETRIES');" ><font color=white><fmt:message key="eXH.Retries.Label" bundle="${xh_labels}"/></font></td>
	<td class='COLUMNHEADER'><a name='STATUS' id='STATUS'  href="javascript:callSort('STATUS');" ><font color=white><fmt:message key="Common.status.label" bundle="${common_labels}"/></font></td>
...
%>
<tr>
<td <%=strTDClass%> align="center">&nbsp;<a href='#' 
onClick='funShow(this)' MESSAGEID='<%=arrCol.get(0)+""%>' FACILITYID='<%=arrCol.get(1)+""%>'  FACILITYNAME='<%=arrCol.get(2)+""%>' PROCESSDATETIME='<%=arrCol.get(3)+""%>' EVENTTYPE='<%=arrCol.get(4)+""%>' 
PROCESSID='<%=arrCol.get(5)+""%>'  PROCESSEDBY='<%=arrCol.get(6)+""%>' ENDDATETIME='<%=arrCol.get(7)+""%>'
COMMRETRIES='<%=arrCol.get(8)+""%>' MSGSTATUS ='<%=arrCol.get(9)+""%>' MESSAGE_TEXT ='<%=arrCol.get(11)+""%>'
STATUS_TEXT ='<%=arrCol.get(10)+""%>' 
><%=msgid%></a></td>
<td <%=strTDClass%> align="center">&nbsp;<%=((String)arrCol.get(2)).equals("null")?"":arrCol.get(2)%></td>
<td <%=strTDClass%> align="center">&nbsp;<%=""+strProcessDate%></td>
<td <%=strTDClass%> align="center">&nbsp;<%=((String)arrCol.get(4)).equals("null")?"":arrCol.get(4)%></td>
<td <%=strTDClass%> align="center">&nbsp;<%=((String)arrCol.get(5)).equals("null")?"":arrCol.get(5)%></td>
<td <%=strTDClass%> align="center">&nbsp;<%=((String)arrCol.get(6)).equals("null")?"":arrCol.get(6)%></td>
<td <%=strTDClass%> align="center">&nbsp;<%=""+strendDate%></td>
<td <%=strTDClass%> align="center">&nbsp;<%=((String)arrCol.get(8)).equals("null")?"":arrCol.get(8)%></td>
<td <%=strTDClass%> align="center">&nbsp;<%=""+int_status%></td>
</tr>
<%
}
}
