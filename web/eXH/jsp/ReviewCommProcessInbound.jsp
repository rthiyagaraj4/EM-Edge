...
<%
 ...
 String comm_client=request.getParameter("comm_client"); if(comm_client==null) comm_client="";
   //System.out.println(" sql_query inbound(1) :"+sql_query );
  if( sql_query==null){
...
" TO_CHAR(X.LAST_COMM_DATE_TIME,'DD/MM/YYYY HH:MI:SS')LAST_COMM_DATE_TIME,X.LAST_OUTBOUND_RETRIES LAST_OUTBOUND_RETRIES,"+
 ...
 " WHERE X.CLIENT_ID='"+comm_client+"' AND X.PROCESS_TYPE='I' "+
 " AND X.FACILITY_ID='*A' AND X.APPLICATION_ID=Y.APPLICATION_ID AND Y.LANGUAGE_ID='"+locale+"' "+
 " AND (X.APPLICATION_ID, X.START_DATE_TIME)  IN (SELECT APPLICATION_ID , MAX(START_DATE_TIME) FROM XH_COMM_PROCESS "+
 " WHERE CLIENT_ID='"+comm_client+"' AND PROCESS_TYPE ='I' GROUP BY  APPLICATION_ID) "+
 " UNION "+
 ...
 " WHERE X.IN_USE_YN='Y'AND Y.INBOUND_YN='Y' AND X.CLIENT_ID  = Y.CLIENT_ID AND X.CLIENT_ID='"+comm_client+"' "+ 
 " AND Y.IN_USE_YN='Y' AND X.COMM_TYPE='I' AND X.APPLICATION_ID=Z.APPLICATION_ID AND Z.LANGUAGE_ID='"+locale+"' AND X.FACILITY_ID='*A' "+
 " AND (X.APPLICATION_ID,'"+comm_client+"') NOT IN (SELECT APPLICATION_ID,CLIENT_ID FROM XH_COMM_PROCESS WHERE PROCESS_TYPE='I') ";
  //System.out.println(" sql_query inbound :"+sql_query ); 
...
<%  
	...
	try
	{
      con =  ConnectionManager.getConnection(request);
		 ...
		 pstmt=con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE , ResultSet.CONCUR_READ_ONLY);		 rs = pstmt.executeQuery(sql2); 
		 rs.last();
		 maxRecord=rs.getRow();
		 rs.beforeFirst();
		 if (maxRecord==0)
...
alert(getMessage('XH1021','XH'));
 ...
 <p>
<form name="review_communprocess_inbound_form" id="review_communprocess_inbound_form">
...
<input type="hidden" name="comm_type" id="comm_type" value='I'/>
...
<input type="hidden" name="mode" id="mode" value="W"/>	
	...
	<%  
	if ( !(start <= 1) )
	out.println("<A HREF='../../eXH/jsp/ReviewCommProcessInbound.jsp?facility="+facility+"&comm_client="+comm_client+
...
<td nowrap class='<%=strTDClass%>' >&nbsp;<%=PROCESS_ID%></font></td>
<td nowrap class='<%=strTDClass%>' >&nbsp;<%=START_DATE_TIME%></font></td>
...
<td nowrap class='<%=strTDClass%>' align=center><input type=radio name=c1 id=c1 onClick="validatecheck(this)" appl_Id='<%=APPLICATION_ID%>' st_date_time='<%=START_DATE_TIME%>' tot_msg='<%=TOTAL_MESSAGES%>' last_comm='<%=LAST_COMM_DATE_TIME%>' retries='<%=LAST_OUTBOUND_RETRIES%>' last_msg_id='<%=LAST_MESSAGE_ID%>' process_id='<%=PROCESS_ID%>' process_status='<%=PROCESS_STATUS%>' appl_Name='<%=APPLICATION_NAME%>'></td>
<%
i++;
p_rec_cnt++;
}//endwhile         
