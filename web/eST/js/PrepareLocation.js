...
function create() {
	f_query_add_mod.location.href = "../../eST/jsp/PrepareLocationAddModifyMultiple.jsp?mode=" + MODE_INSERT + "&function_id=" + function_id;
}
function query() {
	f_query_add_mod.location.href = "../../eST/jsp/PrepareLocationQueryCriteria.jsp?mode=" + MODE_MODIFY + "&function_id=" + function_id;
}
function apply() {
	var arrPKFieldNames = new Array();
	arrPKFieldNames[0] = "code_";
	var formObj = f_query_add_mod.document.formPrepareLocation;
	var errorPage = "../../eCommon/jsp/error.jsp";
	var obj = null;
if(f_query_add_mod.document.formPrepareLocation==undefined){
	messageFrame.location.href = "../../eCommon/jsp/error.jsp?err_num=" ;
	return
	}
	if (f_query_add_mod.document.formPrepareLocation.mode.value == MODE_MODIFY) {
	...
	if (formObj.mode.value == MODE_INSERT) {
		...
		if (validateResult) {
			eval(formApply(f_query_add_mod.document.formPrepareLocation, ST_CONTROLLER));
			if (result) {
				onSuccess();
			}
...
function reset() {
	if (f_query_add_mod.document.formPrepareLocation != null && f_query_add_mod.document.formPrepareLocation.mode.value == 2) {
		f_query_add_mod.document.formPrepareLocation.reset();
	}
	if (f_query_add_mod.document.formPrepareLocation != null && f_query_add_mod.document.formPrepareLocation.mode.value == 1) {
...
function onSuccess() {
	var mode = f_query_add_mod.document.formPrepareLocation.mode.value;
	if (mode == MODE_INSERT) {
		reset();
	} else {
...
function Modify(obj) {
	var prepare_location_code = obj.cells[0].innerText;
	document.location.href = "../../eST/jsp/PrepareLocationAddModify.jsp?mode=" + MODE_MODIFY + "&function_id=" + parent.function_id + "&prepare_location_code=" + prepare_location_code;
}
function checkbatchid(obj, count) {
	if (eval("document.formPrepareLocation.auto_gen_batch_yn_" + count).checked == true) {
		eval("document.formPrepareLocation.auto_gen_batch_yn_" + count).value = "Y";
		eval("document.formPrepareLocation.batch_id_" + count).disabled = false;
		eval("document.formPrepareLocation.batch_series_" + count).disabled = false;
	} else {
		eval("document.formPrepareLocation.auto_gen_batch_yn_" + count).value = "N";
		eval("document.formPrepareLocation.batch_id_" + count).value = "";
		eval("document.formPrepareLocation.batch_id_" + count).disabled = true;
		eval("document.formPrepareLocation.batch_series_" + count).value = "";
		eval("document.formPrepareLocation.batch_series_" + count).disabled = true;
	}
}
function checksinglebatchid() {
	if (document.formPrepareLocation.auto_gen_batch_yn_0.checked == true) {
		document.formPrepareLocation.auto_gen_batch_yn_0.value = "Y";
		document.formPrepareLocation.batch_id_0.disabled = false;
	} else {
		document.formPrepareLocation.auto_gen_batch_yn_0.value = "N";
		document.formPrepareLocation.batch_id_0.value = "";
		document.formPrepareLocation.batch_id_0.disabled = true;
	}
}
function checkForNumber(batch_id) {
	if (parseInt(document.formPrepareLocation.batch_id_0.value) < parseInt(batch_id)) {
		alert(getMessage("BATCH_ID_GREATER_THAN_PREVIOUS", "ST"));
		document.formPrepareLocation.batch_id_0.value = batch_id;
		document.formPrepareLocation.batch_id_0.focus();
	}
}
function resetColorCode(obj) {
	var arrPKFieldNames = new Array();
	var formObj = parent.f_query_add_mod.document.formPrepareLocation;
	arrPKFieldNames[0] = "code_";
