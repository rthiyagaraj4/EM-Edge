...
*/
%>
<%@ page import ="webbeans.eCommon.*, java.sql.*, java.util.Properties, java.util.ArrayList,java.util.HashMap, eST.Common.StRepository, eCommon.Common.OptionValues,eST.Common.*, eST.* ,java.sql.*"
...
%>
		...
		<td class='WHITE'>
			...
			<%  if(bean.getTransaction_Type().equalsIgnoreCase("ISS")){ %>	<!-- MO-CRF-20174 -->
				<th ><fmt:message key="eST.ForRejectOrCancel.label" bundle="${st_labels}"/></th>
				<% } %>
			</tr>
			<%
				i								=		start;
			...
			%>
				...
				<% if(bean.getTransaction_Type().equalsIgnoreCase("ISS")) {  %> <!-- MO-CRF-20174 -->
				<td class="<%=classValue%>"><input type="checkbox" name="reject_status_<%=i%>" id="reject_status_<%=i%>" value="Y" <%=reject_status_checked%> class='<%=noDisplay%>' onClick='alterBean1(this,"<%=i%>")'> </td>
				<% }  %>
			</tr>
