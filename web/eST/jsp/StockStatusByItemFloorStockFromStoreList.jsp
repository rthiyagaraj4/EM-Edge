...
%>
		...
		<title><fmt:message key="eST.StorewiseFloorStockConsumption.label" bundle="${st_labels}"/></title>
	</head>
	<%
    ...
    try{
		...
		%>
			...
			<table border="1" cellpadding="0" cellspacing="0" width="99%" align="center" id='tb1'>
				<th align="center"><fmt:message key="Common.IssueStore.label" bundle="${common_labels}"/></th>
			    ...
			    %>
				<tr>
						<td id="period_<%=intCount%>" class="<%=className%>" align="left"><%=(String)data.get(0)%></td>
						<td id="qty_<%=intCount%>" class="<%=className%>" onClick="displyFloorStores('<%=item_code%>','<%=data.get(1)%>','<%=move_month%>','<%=move_year%>','<%=intCount%>','<%=store_code_from%>','<%=store_code_to%>', 'DisplyToStores', '<%=dflt_facility_id%>');"  style="TEXT-ALIGN:right;color:blue;cursor:pointer"><%=data.get(2)%></td>
						<%if(strCostAccess.equals("Y")){%>
						<td id="cost_<%=intCount%>" class="<%=className%>" onClick="displyFloorStores('<%=item_code%>','<%=data.get(1)%>','<%=move_month%>','<%=move_year%>','<%=intCount%>','<%=store_code_from%>','<%=store_code_to%>', 'DisplyToStores', '<%=dflt_facility_id%>');" style="TEXT-ALIGN:right;color:blue;cursor:pointer"><%=bean.setNumberFormat((String)data.get(3),noOfDecimalsForCost)%></td>
						<%}%>
			   </tr>
			<%
				}
		...
		<input type="hidden" name="cost_acc" id="cost_acc" value="<%=strCostAccess%>" >
<%	
	}
