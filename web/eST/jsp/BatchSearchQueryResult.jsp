...
*/
%> 
<%@ page import ="eST.BatchSearchBean,eCommon.Common.*,java.util.HashMap,java.util.ArrayList,java.lang.*,webbeans.eCommon.*,java.text.NumberFormat,java.text.DecimalFormat,java.sql.*" contentType="text/html;charset=UTF-8"%> <% //ADDED FOR GHL-CRF-0499 %>
...
// Shaiju added to show the expired batches Ends
		...
		%>
			...
			<tr>
				...
				<th  class='CAGROUP_HEADING'><font size=1><b><fmt:message key="Common.BinLocation.label" bundle="${common_labels}"/></b></font></th>
				<th  class='CAGROUP_HEADING'><font size=1><b>
				<fmt:message key="Common.PurchaseUOM.label" bundle="${common_labels}"/></b></font></th>
				<%if(batch_search_pur_uom_qoh_appl){ %><!-- added for MO-CRF-20175 -->
				<th  class='CAGROUP_HEADING'><font size=1><b><fmt:message key="eST.PurchaseUOMQOH.label" bundle="${st_labels}"/></b></font></th>
				<th  class='CAGROUP_HEADING'><font size=1><b><fmt:message key="eST.PurchaseUOMAvlQty.label" bundle="${st_labels}"/></b></font></th>
				<%} %>
				<th  class='CAGROUP_HEADING'><font size=1><b>
...
<%             if(searchFunction.equals("Y")){
 ...
 %>
             <th  class='CAGROUP_HEADING'><font size=1><b><fmt:message key="Common.Qty.label" bundle="${common_labels}"/></b></font></th>
<%}%>
		...
		<%
				...
				<td colspan=30 
						class='ITEMSELECT'><font size=1><b><fmt:message key="Common.item.label" bundle="${common_labels}"/> :</b> 
								<%=CommonBean.checkForNull((String)dataList.get("ITEM_DESC"),"")%>&nbsp
									;&nbsp; <b><fmt:message key="eST.ExpiryItems.label" bundle="${st_labels}"/> : </b><%=expiry_yn%>&nbsp;&nbsp;<b><fmt:message key="Common.Store.label" bundle="${common_labels}"/> : 
								</b><%=CommonBean.checkForNull((String)dataList.get("STORE_DESC"),"")%>
				</font></td>
			</tr>
			<%}else{%>
			 <tr>
				<td colspan=30  class='ITEMSELECT'><font size=1><b><fmt:message key="Common.item.label" bundle="${common_labels}"/> :</b> <%=CommonBean.checkForNull((String)dataList.get("ITEM_DESC"),"")%>&nbsp;&nbsp;<b><fmt:message key="eST.ExpiryItems.label" bundle="${st_labels}"/> : </b><%=expiry_yn%>&nbsp;&nbsp; <b><fmt:message key="Common.Store.label" bundle="${common_labels}"/> :</b> <%=CommonBean.checkForNull((String)dataList.get("STORE_DESC"),"")%></b></font></td>
			</tr> 
			<%}%>
			<%}%>
		 ...
		 <tr id="tr_<%=chkCount%>">
			...
			%>
				...
				<%}else{%>
				<td class="<%=stlightgreen%>"   bgcolor="#FFC6AA" title= "<%=tool%> " 
			  ...
			  <%
				...
				%>
				<td class="<%=classvalue%>" ><font size=1><%=CommonBean.checkForNull(challanNo,"&nbsp;")%>
				...
				<!--Added  by Rabbani #inc no :35661 (PMG2012-CRF-0012) on 23/11/2012 -->
				<td class="<%=classvalue%>" ><font size=1><%=CommonBean.checkForNull((String)dataList.get("PUR_UOM_DESC"),"&nbsp;")%></font></td>
				<% if (batch_search_pur_uom_qoh_appl){ %><!-- MO-CRF-20175 -->
				<td class="<%=classvalue%>" ><font size=1><%=dataList.get("PUR_UOM_QOH")%></font>&nbsp;</td>
				<td class="<%=classvalue%>" ><font size=1><%=dataList.get("PUR_UOM_AVL_QTY")%></font>&nbsp;</td> 
				<%} %> 
		...
		<%
			...
			<tr>
				...
				<b><fmt:message key="Common.Store.label" bundle="${common_labels}"/> : 
				</b><%=CommonBean.checkForNull((String)dataList.get("STORE_DESC"),"")%>
			</font></td>
			</tr>
			<%}else{%>
...
<%                  if(called_from.equals("NEWSCREEN")){//Added for TH-KW-CRF-0043
%>
                        <td class="<%=classvalue%>" ><input type=text name="sel_qty_<%=count%>" id="sel_qty_<%=count%>" id ="sel_qty_<%=count%>" size=10 maxlength=12 class="NUMBER" onKeyPress="return isValidNumber(this,event,7,0);" onBlur = "checkQty(this,'<%=count%>','<%=temp_conv%>');" value="<%=sel_qty%>" >&nbsp;</td>
<%}%>
						<td  class="<%=classvalue%>"><input type=checkbox onClick="return checkAllowed(this,'<%=count%>');" name="check<%=count%>" <%=sel_check%>></font></td><!-- Parameters passed for TH-KW-CRF-0043 -->
