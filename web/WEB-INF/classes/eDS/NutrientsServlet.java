...
public class NutrientsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("insert".equals(mode)){
			...
			if(!(commonChk.checkForPk(request, response, DlQuery.DL_NUTRIENTS_CHECK_PK, request.getParameter("nutrients")))){
	...
	private int update(HttpServletRequest request,HttpServletResponse response) {
		...
		int err_no = 0;
		try{
			nutrients = request.getParameter("updNutrients");
			longDesc = request.getParameter("long_Desc");
			shortDesc = request.getParameter("short_Desc");
			uom = request.getParameter("uom");
			nutrientConversion = request.getParameter("nutrient_Conversion");
			energyConversion = request.getParameter("energy_Conversion");
			...
			pstmt = con.prepareStatement(DlQuery.DL_UPDATE_NUTRIENTS);
			pstmt.setString(1, longDesc);
			pstmt.setString(2, shortDesc);
			pstmt.setString(3, uom);
			pstmt.setString(4, nutrientConversion);
			pstmt.setString(5, energyConversion);
			pstmt.setString(6, enabledYn);
			pstmt.setString(7, login_user);
			pstmt.setString(8, facility_id);
			pstmt.setString(9, client_ip_address);
			pstmt.setString(10, nutrients);
			rslt = pstmt.execute();
			err_no = 1;
			con.commit();
			}
			catch(Exception e){
				System.err.println("eDS Exception Nutrients->"+e);
				err_no = 2;
			}
			finally{
				try{
	...
	private int insert(HttpServletRequest request, HttpServletResponse response) {
		...
		int err_no = 0;
		try{
			nutrients = request.getParameter("nutrients");
			longDesc = request.getParameter("long_Desc");
			shortDesc = request.getParameter("short_Desc");
			uom = request.getParameter("uom");
			nutrientConversion = request.getParameter("nutrient_Conversion");
			...
			con = ConnectionManager.getConnection(request);
			pstmt = con.prepareStatement(DlQuery.DL_INSERT_NUTRIENTS);
			pstmt.setString(1, nutrients);
			pstmt.setString(2, longDesc);
			pstmt.setString(3, shortDesc);
			pstmt.setString(4, uom);
			pstmt.setString(5, nutrientConversion);
			pstmt.setString(6, energyConversion);
			pstmt.setString(7, enabledYn);
