...
/**
 * Servlet implementation class MealRejCompServlet
 */
public class MealRejCompServlet extends javax.servlet.http.HttpServlet {
...
protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	...
	//String mealrejection="";	//Common-ICN-0044
	session = request.getSession(false);
	this.prop = (java.util.Properties) session.getValue("jdbc");
	locale = prop.getProperty("LOCALE");
	if(operation.equals("insertMealRejection")){
		try{			
			request.setCharacterEncoding("UTF-8");
			response.setContentType("text/html;charset=UTF-8");
			this.out = response.getWriter();
			MessageManager mm = new MessageManager();
			insertStatus_SuccessFlag=insertMealRejection(request,response);		
			if(insertStatus_SuccessFlag){
					System.err.println("95,insertStatus_SuccessFlag==="+insertStatus_SuccessFlag);	
			...
			e.printStackTrace();
			System.err.println("SQLException in MealRejCompServlet.java::::" + e.toString());
		} catch (ParseException e) {
			e.printStackTrace();
			System.err.println("ParseException in MealRejCompServlet.java::::" + e.toString());
		}
	}
}

private boolean insertMealRejection(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException, ParseException
{
		int flag=0;
...
{	
	...
	String acknowledge1="";
	String MealRejection_insert ="";
	//int count=Integer.parseInt(request.getParameter("RejectionListcount"));	//Common-ICN-0044
	...
	//JSONArray acknowledged_byJSON = json.getJSONArray("acknowledged_by");
	//JSONArray acknowledged_date_timeJSON = json.getJSONArray("acknowledged_date_time");	
	...
	System.err.println("146,acknowledge==="+acknowledge+",i==="+i);
	if(acknowledge.equalsIgnoreCase("Yes")){
		MealRejection_insert="insert into ds_meal_complaint_acknowledge Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
	Date servDate = new Date(sdf.parse(serving_date).getTime());
	pstmt = conn.prepareStatement(MealRejection_insert);
	System.err.println("162,MealRejection_insert==="+MealRejection_insert);
	System.err.println("178,acknowledge==="+acknowledge);
		...
		//conn.commit();
			if (flag > 0)
			{
				...
				System.err.println("231,if,flag======"+flag);
				System.err.println("232,if,isUpdateSuccess======"+isUpdateSuccess);
				
			}/* else{
				isUpdateSuccess = false;
				System.err.println("236,else,flag======"+flag);
				System.err.println("237,else,isUpdateSuccess======"+isUpdateSuccess);
	...
	{
		e.printStackTrace();
		System.err.println("Exception in MealRejCompServlet.java========="+e);
	}
	finally {	
	try{		
		...
		if(conn!=null) ConnectionManager.returnConnection(conn);
			...
			System.err.println("Final Exception in MealRejCompServlet.java========"+e);
		}
	}
	
		return "Operation completed Successfully!";*/
}catch(Exception e){
			System.err.println("Exception in MealRejCompServlet.java::::" + e.toString());
			e.printStackTrace();
			isUpdateSuccess = false;
	}finally{	
	try{		
	...
	System.err.println("isUpdateSuccess MealRejCompServlet.java::::" + isUpdateSuccess);
