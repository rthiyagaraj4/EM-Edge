...
public class ComplaintsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("insert".equals(mode)){
			...
			if(!(commonChk.checkForPk(request, response, DlQuery.DL_COMPLAINTS_CHECK_PK, request.getParameter("complaints")))){
	...
	private int update(HttpServletRequest request,HttpServletResponse response) {
		...
		int err_no = 0;
		try{
			complaints = request.getParameter("updComplaints");
			...
			pstmt = con.prepareStatement(DlQuery.DL_UPDATE_COMPLAINTS);
			...
			pstmt.setString(7, complaints);
			rslt = pstmt.execute();
			err_no = 1;
			con.commit();
			}
			catch(Exception e){
				System.err.println("eDS Exception complaints->"+e);
				err_no = 2;
			}
			finally{
				try{
	...
	private int insert(HttpServletRequest request, HttpServletResponse response) {
		...
		int err_no = 0;
		try{
			complaints = request.getParameter("complaints");
			longDesc = request.getParameter("long_Desc");
			shortDesc = request.getParameter("short_Desc");
			enabledYn = request.getParameter("enabled_Yn");
			...
			con = ConnectionManager.getConnection(request);
			pstmt = con.prepareStatement(DlQuery.DL_INSERT_COMPLAINTS);
