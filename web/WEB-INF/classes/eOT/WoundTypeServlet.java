...
public class WoundTypeServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	String login_user="";
	String table_name="OT_WOUND_TYPE";
	...
	private HashMap populateTabData(HttpServletRequest req, HttpServletResponse res){
		...
		if("insert".equals(mode)){
			String wound_type_code=checkForNull(req.getParameter("wound_type_code"));
			String long_desc=checkForNull(req.getParameter("long_desc"));
			String wound_text=checkForNull(req.getParameter("wound_text"));
			String status = checkForNull(req.getParameter("status"));
			if(status.equals("")){
				status="D";
			}
			tabdata.put("wound_type_code",wound_type_code);
			tabdata.put("long_desc",long_desc);
			tabdata.put("wound_text",wound_text);
			...
			String long_desc=checkForNull(req.getParameter("long_desc"));
			String long_desc_1=checkForNull(req.getParameter("long_desc1"));
			String wound_text=checkForNull(req.getParameter("wound_text"));
			String wound_text_1=checkForNull(req.getParameter("wound_text1"));
			if(long_desc.equals("")){
				long_desc=long_desc_1;
			}
			if(wound_text.equals("")){
				wound_text=wound_text_1;
			}
			String status = checkForNull(req.getParameter("status"));
			if(status.equals("")){
				status="D";
	...
	private void insert(HttpServletRequest req, HttpServletResponse res){
		...
		String dupflds[]={"wound_type_code"};
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		String wound_type_code=checkForNull(req.getParameter("wound_type_code1"));
		HashMap tabData = populateTabData(req,res);
		HashMap condflds=new HashMap();
		condflds.put("wound_type_code",wound_type_code);
		Object argArray[]  = new Object[4];
		argArray[0] = p;
		argArray[1] = tabData;				   
		argArray[2] = condflds;
