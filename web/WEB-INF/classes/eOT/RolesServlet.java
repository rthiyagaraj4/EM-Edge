...
public class RolesServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	private HashMap populateTabData(HttpServletRequest req, HttpServletResponse res){
		...
		if("insert".equals(mode)){
			String role_id=checkForNull(req.getParameter("role_id"));
			String role_desc=checkForNull(req.getParameter("role_desc"));
			String role_type=checkForNull(req.getParameter("role_type_desc"));
			String appl_to_ot_slate=checkForNull(req.getParameter("appl_to_ot_slate"),"N");
			String status = checkForNull(req.getParameter("status"),"D");
			tabdata.put("role_id",role_id);
			tabdata.put("role_desc",role_desc);
			tabdata.put("role_type",role_type);
			tabdata.put("appl_to_ot_slate",appl_to_ot_slate);
			...
			String role_desc=checkForNull(req.getParameter("role_desc"),checkForNull(req.getParameter("role_desc1")));
			String role_type=checkForNull(req.getParameter("role_type_desc"),checkForNull(req.getParameter("role_type_desc1")));
			String appl_to_ot_slate=checkForNull(req.getParameter("appl_to_ot_slate"),"N");
			String status = checkForNull(req.getParameter("status"),"D");
			tabdata.put("role_desc",role_desc);
			tabdata.put("role_type",role_type);
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		String role_id=checkForNull(req.getParameter("role_id1"));
		HashMap tabData = populateTabData(req,res);
		HashMap condflds=new HashMap();
		condflds.put("role_id",role_id);
		Object argArray[]  = new Object[4];
		argArray[0] = p;
		argArray[1] = tabData;				   
		argArray[2] = condflds;
