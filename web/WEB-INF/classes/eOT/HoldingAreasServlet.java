...
public class HoldingAreasServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	String login_user="";
	String table_name="OT_HOLDING_AREAS";
...
//		String locale = this.p.getProperty("LOCALE");
	...
	private HashMap populateTabData(HttpServletRequest req, HttpServletResponse res){
		...
		if("insert".equals(mode)){
			String holding_area_code=checkForNull(req.getParameter("holding_area_code"));
			String holding_area_desc=checkForNull(req.getParameter("holding_area_desc"));
			...
			tabdata.put("holding_area_code",holding_area_code);
			tabdata.put("holding_area_desc",holding_area_desc);
			...
			String holding_area_desc=checkForNull(req.getParameter("holding_area_desc"),checkForNull(req.getParameter("holding_area_desc1")));
			String facility_id=checkForNull(req.getParameter("facility_id"));
			String status = checkForNull(req.getParameter("status"),"D");
			String chargeable_yn = checkForNull(req.getParameter("chargeable_yn"),"N");
			tabdata.put("holding_area_desc",holding_area_desc);
	...
	private void insert(HttpServletRequest req, HttpServletResponse res){
		...
		String dupflds[]={"holding_area_code","operating_facility_id"};
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		String holding_area_code=checkForNull(req.getParameter("holding_area_code1"));
		HashMap tabData = populateTabData(req,res);
		HashMap condflds=new HashMap();
		condflds.put("operating_facility_id",facility_id);
		condflds.put("holding_area_code",holding_area_code);
		Object argArray[]  = new Object[4];
		argArray[0] = p;
		argArray[1] = tabData;				   
		argArray[2] = condflds;
