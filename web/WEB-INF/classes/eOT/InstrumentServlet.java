...
public class InstrumentServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	String login_user="";
	String table_name="OT_INSTRUMENT";
	...
	private HashMap populateTabData(HttpServletRequest req, HttpServletResponse res){
		...
		if("insert".equals(mode)){
			String instr_code=checkForNull(req.getParameter("instrument_code"));
			String ss_group_code=checkForNull(req.getParameter("ss_group_code"));
			String short_desc=checkForNull(req.getParameter("short_desc"));
			String location=checkForNull(req.getParameter("location"));//Added by lakshmi against CRF-0058
...
//Added by Muthu for RUT-CRF-0057
			tabdata.put("instr_code",instr_code);
			tabdata.put("long_desc",long_desc);
			tabdata.put("short_desc",short_desc);
			tabdata.put("location",location);//Added by lakshmi against CRF-0058
			tabdata.put("ss_group_code",ss_group_code);
...
//Added by Muthu for RUT-CRF-0057
			...
			String ss_group_code=checkForNull(req.getParameter("ss_group_code"),req.getParameter("ss_group_code1") );
			String long_desc=checkForNull(req.getParameter("long_desc"),req.getParameter("long_desc1") );
			String short_desc=checkForNull(req.getParameter("short_desc"),req.getParameter("short_desc1") );
			//Modified by Rajesh to store empty Location
			//String location=checkForNull(req.getParameter("location"),req.getParameter("location1"));//Added by lakshmi against CRF-0058
			String location=checkForNull(req.getParameter("location"));
			//Modified by Rajesh to store empty Location
			String status = checkForNull(req.getParameter("status"),"D");
//Added by Muthu for RUT-CRF-0057
...
//Added by Muthu for RUT-CRF-0057
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		String instr_code=checkForNull(req.getParameter("instr_code1"));
		HashMap tabData = populateTabData(req,res);
		HashMap condflds=new HashMap();
		condflds.put("instr_code",instr_code);
		Object argArray[]  = new Object[4];
		argArray[0] = p;
		argArray[1] = tabData;				   
		argArray[2] = condflds;
