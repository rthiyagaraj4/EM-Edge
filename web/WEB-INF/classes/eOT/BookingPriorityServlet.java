...
public class BookingPriorityServlet extends javax.servlet.http.HttpServlet{
	PrintWriter out;
	HttpSession session;
	...
	String login_user="";
	//String table_name="OT_BOOKING_PRIORITY";
	String table_name="OT_BOOKING_PRIORITY";
	...
	private HashMap populateTabData(HttpServletRequest req, HttpServletResponse res){
		...
		if("insert".equals(mode)){
			...
			String priority_desc=checkForNull(req.getParameter("priority_desc"));
			String status = checkForNull(req.getParameter("status"),"D");
			tabdata.put("priority_code",priority_code);
			tabdata.put("priority_desc",priority_desc);
			...
			String priority_desc=checkForNull(req.getParameter("priority_desc"),checkForNull(req.getParameter("priority_desc1")));
			String status = checkForNull(req.getParameter("status"),"D");
			tabdata.put("priority_desc",priority_desc);
			tabdata.put("status",status);
			tabdata.put("modified_by_id",login_user);
	...
	private void modify(HttpServletRequest req, HttpServletResponse res){
		String priority_code=checkForNull(req.getParameter("priority_code1"));
		HashMap tabData = populateTabData(req,res);
		HashMap condflds=new HashMap();
		condflds.put("priority_code",priority_code);
		Object argArray[]  = new Object[4];
		argArray[0] = p;
		argArray[1] = tabData;				   
		argArray[2] = condflds;
