...
public class GenerateCS_RA extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleCS_RA js =new JobScheduleCS_RA();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createCS_RA function which will create the JSON file*/
			GenerateCS_RA Gjson= new GenerateCS_RA();

			String CS_RA_fileCreated_flag =Gjson.createCS_RA(facility_id,from_date,to_date);

			String error="";
			if(CS_RA_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (CS_RA_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (CS_RA_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + CS_RA_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createCS_RA(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='CS_RA'");
				...
				/*The View which is created for this createCS_RAJson purpose*/
...
String sqlQuery		  = "SELECT * FROM MR_CS_RA_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				
				//String sqlQuery		  = "SELECT * FROM MR_CS_RA_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('06/10/2021','dd/mm/yyyy') AND TO_DATE ('06/10/2021', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String pat_class="";
				 while(rset!=null && rset.next()) 	{  	 // looping each ip encounter
					...
					outpatient.append("\"visitingSpecialist\": "+checkForNull(rset.getString("visitingSpecialist"))+" ,");
					outpatient.append("\"medicalCheckup\": \""+checkForNull(rset.getString("medicalCheckup_OP"))+"\" ,");
					...
					StringBuffer radiology= new StringBuffer(" \"radiology\": {	");
					radiology.append("\"mainDiagnosis\": \""+checkForNull(rset.getString("mainDiagnosis"))+"\",");
					radiology.append("\"refDisciplineCode\": \""+checkForNull(rset.getString("refDisciplineCode"))+"\",");
					radiology.append("\"refSubSpecialityCode\": \""+checkForNull(rset.getString("refSubSpecialityCode"))+"\",");
					radiology.append("\"refSpecialityCode\": \""+checkForNull(rset.getString("refSpecialityCode"))+"\",");
					radiology.append("\"refReferralToCode\": \""+checkForNull(rset.getString("refReferralToCode"))+"\",");
					radiology.append("\"refProgramCode\": \""+checkForNull(rset.getString("refProgramCode"))+"\",");
					radiology.append("\"nature\": "+checkForNull(rset.getString("nature"))+",");
					radiology.append("\"priority\": "+checkForNull(rset.getString("priority"))+",");
					radiology.append("\"externalReport\": "+checkForNull(rset.getString("externalReport"))+",");
					radiology.append("\"procedureOnly\": "+checkForNull(rset.getString("procedureOnly"))+",");
					radiology.append("\"medicalCheckup\": "+checkForNull(rset.getString("medicalCheckup"))+",");
					radiology.append("\"refCategoryCaseCode\": \""+checkForNull(rset.getString("refCategoryCaseCode"))+"\",");
					
					pstmt_1 =  con.prepareStatement("select MR_CS_RA_DTLS("+enc_id+") RA_DTLS from dual");
					
					
					StringBuffer examinations= new StringBuffer(" \"examinations\": [	");
					StringBuffer performedBy =new StringBuffer("");
					rset_1 = pstmt_1.executeQuery();
					String RA_dtls="";
					while(rset_1!=null && rset_1.next()){
					
						RA_dtls=checkForNull(rset_1.getString("RA_DTLS"));
					}
					
					System.out.println("RA_dtls-->"+ RA_dtls);
					temp_tkn=null;
					temp_tkn_inner=null;
					
					temp_tkn		= new StringTokenizer(RA_dtls,"@" );
					
					StringBuffer pf_by=new StringBuffer("");
					int kk=0;
					while (temp_tkn.hasMoreTokens()){
						...
						while (temp_tkn_inner.hasMoreTokens()){
							...
							examinations.append("{");
							examinations.append("\"examinationNo\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"examinationDateTime\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"refModalityCode\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"refRegionCode\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"refOrderablesCode\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"ivContrastMedia\": "+temp_tkn_inner.nextToken()+",");
							examinations.append("\"complication\": "+temp_tkn_inner.nextToken()+",");
							examinations.append("\"refComplicationCode\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"completionDate\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"reportingDate\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"rejectedFilm\": "+temp_tkn_inner.nextToken()+",");
							examinations.append("\"noOfFilmRejected\": \""+temp_tkn_inner.nextToken()+"\",");
							examinations.append("\"noOfFilmUsed\": \""+temp_tkn_inner.nextToken()+"\"}");
							
							
							pf_by.append("'"+temp_tkn_inner.nextToken()+"'");
						...
						System.out.println("pf_by-->"+ pf_by.toString());
						
						if(pf_by.charAt(pf_by.length()-1)==','){
							pf_by.delete(pf_by.length()-1, pf_by.length());
						}
						
						System.out.println("pf_by  final -->"+ pf_by.toString());
						
						performedBy= new StringBuffer(" \"performedBy\": [{	");
						
						//pstmt_2 =  con.prepareStatement("select  replace(practitioner_name,'@','**')||'#'||  NVL (( SELECT ext_pk_id FROM xh_oth_appl_data_skey WHERE table_id = 'AM_PRACT_TYPE'  AND pk_value =pract_type  AND application_id = 'SMRP'),'00') perform_dtls from AM_PRACTITIONER where practitioner_id in ("+pf_by.toString()+") ||'#'||replace(nvl(APC_NO,'**') ,'    ','')");
						
						pstmt_2 =  con.prepareStatement("select  replace(practitioner_name,'@','') ||'#'||NVL (( SELECT ext_pk_id FROM xh_oth_appl_data_skey WHERE table_id = 'AM_PRACT_TYPE'  AND pk_value =pract_type  AND application_id = 'SMRP'),'00') ||'#'|| replace(nvl(APC_NO,'**') ,'    ','') perform_dtls from AM_PRACTITIONER where practitioner_id in ("+pf_by.toString()+")");
						//pstmt_2.setString(1,pf_by.toString());
						rset_2 = pstmt_2.executeQuery();
						String pf_dtls="";
						String holder="";
						while(rset_2!=null && rset_2.next()){
						
							pf_dtls=checkForNull(rset_2.getString("perform_dtls"));
						
							System.out.println("pf_dtlspf_dtls-->"+ pf_dtls);
							
							StringTokenizer temp_tkn_pfmd=null;						
							temp_tkn_pfmd		= new StringTokenizer(pf_dtls,"#" );
							
																			
							while (temp_tkn_pfmd.hasMoreTokens()){
								System.out.println("610");
								holder=temp_tkn_pfmd.nextToken();
								if(holder.equals("**")){
									holder="";
								}
								performedBy.append("\"fullName\": \""+holder+"\",");
								holder=temp_tkn_pfmd.nextToken();
								if(holder.equals("**")){
									holder="";
								}
								performedBy.append("\"refDesignationCode\": \""+holder+"\",");
								...
								performedBy.append("\"regNumber\": \""+holder+"\" ");
					...
					radiology.append(examinations.toString());
					radiology.append(",");
					radiology.append(performedBy.toString());
					radiology.append("}}}");
					...
					clinicalsupport.append(radiology.toString());
					//clinicalsupport.append(examinations.toString());
					//clinicalsupport.append(performedby.toString());
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_CS_RA.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- CS_RA-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File CS_RAjson = new File (dir);
						FileWriter file = new FileWriter(CS_RAjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("CS_RAjson.getPath()-->"+CS_RAjson.getPath());
						//System.out.println("CS_RAjson.getAbsolutePath()-->"+CS_RAjson.getAbsolutePath());
						//System.out.println("CS_RAjson.getName()-->"+CS_RAjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_RA",CS_RAjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  CS_RA generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_RA","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createCS_RA function Inside try:"+e.toString());
						System.out.println("Error while file obj  CS_RA generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_RA","IO Error. Unable to create File for CS_RA generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for CS_RA generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_RA","No data for CS_RA generation");
				}
	}	 //check in function control				

		
		}catch (Exception e) {
			  System.err.println("Exception in createCS_RA function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_RA",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleCS_RA extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleCS_RA = new JobScheduleCS_RA();
    ...
    timer.scheduleAtFixedRate(JobScheduleCS_RA, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobScheduleCS_RA, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createCS_RA(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleCS_RA Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
