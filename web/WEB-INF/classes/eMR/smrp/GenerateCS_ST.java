...
public class GenerateCS_ST extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleCS_ST js =new JobScheduleCS_ST();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createCS_ST function which will create the JSON file*/
			GenerateCS_ST Gjson= new GenerateCS_ST();

			String CS_ST_fileCreated_flag =Gjson.createCS_ST(facility_id,from_date,to_date);

			String error="";
			if(CS_ST_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (CS_ST_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (CS_ST_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + CS_ST_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createCS_ST(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='CS_ST'");
				...
				/*The View which is created for this createCS_STJson purpose*/
				...
				String sqlQuery		  = "SELECT * FROM MR_CS_ST_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				
				//String sqlQuery		  = "SELECT * FROM MR_CS_ST_VW  where rownum<3";// WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('06/10/2021','dd/mm/yyyy') AND TO_DATE ('06/10/2021', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String pat_class="";
				 ...
				 while(rset!=null && rset.next()) 	{  	 // looping each ip encounter
					...
					StringBuffer speechTherapy= new StringBuffer(" \"speechTherapy\": {	");
					speechTherapy.append("\"refMainDiagnosisCodes\": "+checkForNull(rset.getString("refMainDiagnosiscode"))+",");
					speechTherapy.append("\"mainDiagnosis\": \""+checkForNull(rset.getString("mainDiagnosis"))+"\",");
					speechTherapy.append("\"refDisciplineCode\": \""+checkForNull(rset.getString("refDisciplineCode"))+"\",");
					speechTherapy.append("\"refSpecialityCode\": \""+checkForNull(rset.getString("refSpecialityCode"))+"\",");
					speechTherapy.append("\"refSubSpecialityCode\": \""+checkForNull(rset.getString("refSubSpecialityCode"))+"\",");
					speechTherapy.append("\"refDiagnosisCategoryCode\": \""+checkForNull(rset.getString("refDiagnosisCategoryCode"))+"\",");				
					speechTherapy.append("\"refProgramCode\": \""+checkForNull(rset.getString("refProgramCode"))+"\",");
					speechTherapy.append("\"refsubbprogramcode\": \""+checkForNull(rset.getString("refsubprogramcode"))+"\",");
					speechTherapy.append("\"refsubsubprogramcode\": \""+checkForNull(rset.getString("refsubsubprogramcode"))+"\",");
					speechTherapy.append("\"refsubsubsubprogramcode\": \""+checkForNull(rset.getString("refsubsubsubprogramcode"))+"\",");					
					speechTherapy.append("\"otherProgram\": \""+checkForNull(rset.getString("otherProgram"))+"\",");					
					speechTherapy.append("\"refActivityCode\": \""+checkForNull(rset.getString("refActivityCode"))+"\",");
					speechTherapy.append("\"onset\": \""+checkForNull(rset.getString("onset"))+"\",");
					speechTherapy.append("\"isGroupSession\": \""+checkForNull(rset.getString("isGroupSession"))+"\",");
					...
					pstmt_1 =  con.prepareStatement("select MR_CS_ST_DTLS("+enc_id+",'A') ST_DTLS from dual");
					
					rset_1 = pstmt_1.executeQuery();
					String ST_dtls="";
					while(rset_1!=null && rset_1.next()){
					
						ST_dtls=checkForNull(rset_1.getString("ST_DTLS"));
					}
					
					
					System.out.println("Assessment ST_dtls-->"+ ST_dtls);
					
					temp_tkn=null;
					temp_tkn_inner=null;
					
					temp_tkn		= new StringTokenizer(ST_dtls,"@" );
					...
					pstmt_1 =  con.prepareStatement("select MR_CS_ST_DTLS("+enc_id+",'I') ST_DTLS from dual");
					rset_1 = pstmt_1.executeQuery();
					ST_dtls="";
					while(rset_1!=null && rset_1.next()){
					...
					System.out.println("Intervention ST_dtls-->"+ ST_dtls);
						...
						while (temp_tkn_inner.hasMoreTokens()){
							...
							if(!activity_type.equals("00") && !catalog_codes.equals("00")){	
								...
								if (! olc_act_typ.equals(activity_type)) {
									...
									interventions.append("{");
									interventions.append("\"refDataCode\": \""+catalog_codes+"\",");
									interventions.append("\"values\": []");	
									interventions.append("}");
									olc_act_typ= activity_type;
								}else{	
					...
					} // outer for interventions 
					
					if (hasIntervention.equals("true")){
						//therapists.append(",");
						interventions.append("]}");
					}
					
					
					speechTherapy.append("\"hasAssessment\": "+hasAssessment+",");
					speechTherapy.append("\"hasIntervention\": "+hasIntervention+",");
					
					speechTherapy.append("\"hasDischarge\": "+checkForNull(rset.getString("hasDischarge"))+",");
					speechTherapy.append("\"refEpisodeDischargeTypeCode\": \""+checkForNull(rset.getString("refEpisodeDischargeTypeCode"))+"\",");
					speechTherapy.append("\"otherDischarge\": \""+checkForNull(rset.getString("otherDischarge"))+"\",");
					
					
					therapists.append("]");
					assessments.append("]");
					interventions.append("]");
					clinicalsupport.append(speechTherapy.toString());
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_CS_ST.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- CS_ST generate servlet-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File CS_STjson = new File (dir);
						FileWriter file = new FileWriter(CS_STjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("CS_STjson.getPath()-->"+CS_STjson.getPath());
						//System.out.println("CS_STjson.getAbsolutePath()-->"+CS_STjson.getAbsolutePath());
						//System.out.println("CS_STjson.getName()-->"+CS_STjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_ST",CS_STjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  CS_ST generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_ST","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createCS_ST function Inside try:"+e.toString());
						System.out.println("Error while file obj  CS_ST generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_ST","IO Error. Unable to create File for CS_ST generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for CS_ST generation-->"+filename);
					fileCreated="0";
					//smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_ST","No data for CS_ST generation");
				}
	}	 //check in function control				

		
		}catch (Exception e) {
			  System.err.println("Exception in createCS_ST function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_ST",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleCS_ST extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleCS_ST = new JobScheduleCS_ST();
    ...
    timer.scheduleAtFixedRate(JobScheduleCS_ST, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobScheduleCS_ST, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createCS_ST(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleCS_ST Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
