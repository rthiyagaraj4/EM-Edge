...
public class GenerateCS_PATH extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleCS_PATH js =new JobScheduleCS_PATH();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createCS_PATH function which will create the JSON file*/
			GenerateCS_PATH Gjson= new GenerateCS_PATH();

			String CS_PATH_fileCreated_flag =Gjson.createCS_PATH(facility_id,from_date,to_date);

			String error="";
			if(CS_PATH_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (CS_PATH_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (CS_PATH_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + CS_PATH_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createCS_PATH(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='CS_PATH'");
				...
				/*The View which is created for this createCS_PATHJson purpose*/
				...
				String sqlQuery		  = "SELECT * FROM MR_CS_PATH_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				
				//String sqlQuery		  = "SELECT * FROM MR_CS_PATH_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('17/10/2021','dd/mm/yyyy') AND TO_DATE ('17/10/2021', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' and rownum<3 ORDER BY VISIT_ADM_DATE_TIME";
				...
				String fl_name=filename+"_CSPATH";
				...
				String pat_class="";
				 while(rset!=null && rset.next()) 	{  	 // looping each ip encounter
					...
					StringBuffer pathology= new StringBuffer(" \"pathology\": {	");
					pathology.append("\"refSourceOfReferralCode\": \""+checkForNull(rset.getString("refSourceOfReferralCode"))+"\",");
					pathology.append("\"refSubSpecialityCode\": \""+checkForNull(rset.getString("refSubSpecialityCode"))+"\",");
					pathology.append("\"noOfSpeciment\": \""+checkForNull(rset.getString("noOfSpeciment"))+"\",");
					
					
					pstmt_1 =  con.prepareStatement("select MR_CS_PATH_DTLS("+enc_id+") PATH_DTLS from dual");
					System.out.println("enc_id-->"+ enc_id);
					
					StringBuffer tests= new StringBuffer(" \"tests\": [	");
					StringBuffer performedBy =new StringBuffer("");
					rset_1 = pstmt_1.executeQuery();
					String PATH_dtls="";
					while(rset_1!=null && rset_1.next()){
					
						PATH_dtls=checkForNull(rset_1.getString("PATH_dtls"));
					}
					
					System.out.println("PATH_dtls-->"+ PATH_dtls);
					temp_tkn=null;
					temp_tkn_inner=null;
					
					temp_tkn		= new StringTokenizer(PATH_dtls,"@" );
					...
					while (temp_tkn.hasMoreTokens()){
						...
						while (temp_tkn_inner.hasMoreTokens()){
							...
							tests.append("{");
							tests.append("\"testGroup\": \""+temp_tkn_inner.nextToken()+"\",");
							tests.append("\"refPathologyTestCode\": \""+temp_tkn_inner.nextToken()+"\",");
							tests.append("\"refPathologyMethodCode\": \""+filler+"\",");
							tests.append("\"refLocationCode\": \"01\",");
							tests.append("\"noOfSlide\": \""+filler+"\",");
							tests.append("\"noOfTissueBlock\": \""+filler+"\"}");
						}  // each test
					...
					pathology.append(tests.toString());
					...
					clinicalsupport.append(pathology.toString());
				...
				filename=filename+"_CSPATH.json";
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_CS_PATH.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- CS_PATH SERV-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File CS_PATHjson = new File (dir);
						FileWriter file = new FileWriter(CS_PATHjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("CS_PATHjson.getPath()-->"+CS_PATHjson.getPath());
						//System.out.println("CS_PATHjson.getAbsolutePath()-->"+CS_PATHjson.getAbsolutePath());
						//System.out.println("CS_PATHjson.getName()-->"+CS_PATHjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_PATH",CS_PATHjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  CS_PATH generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_PATH","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createCS_PATH function Inside try:"+e.toString());
						System.out.println("Error while file obj  CS_PATH generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_PATH","IO Error. Unable to create File for CS_RA generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for CS_PATH generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_PATH","No data for CS_PATH generation");
				}
	}	 //check in function control				

		
		}catch (Exception e) {
			  System.err.println("Exception in createCS_PATH function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_PATH",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleCS_PATH extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleCS_PATH = new JobScheduleCS_PATH();
    ...
    timer.scheduleAtFixedRate(JobScheduleCS_PATH, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobScheduleCS_PATH, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createCS_PATH(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleCS_PATH Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
