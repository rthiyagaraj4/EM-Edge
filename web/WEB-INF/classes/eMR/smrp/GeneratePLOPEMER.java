...
public class 	GeneratePLOPEMER extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobSchedulePLOPEMER js =new JobSchedulePLOPEMER();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createPLOPEMER function which will create the JSON file*/
			GeneratePLOPEMER Gjson= new GeneratePLOPEMER();

			String PLOPEMER_fileCreated_flag =Gjson.createPLOPEMER(facility_id,from_date,to_date);

			String error="";
			if(PLOPEMER_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (PLOPEMER_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (PLOPEMER_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + PLOPEMER_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createPLOPEMER(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				/*The View which is created for this PLOPEMERJson purpose*/
				...
				*/
				...
				String sqlQuery		  = "SELECT * FROM MR_PLOP_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999 and patient_class = 'EM' and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String fl_name=filename+"_PLETD";
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_PLOPEMER.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- PLOPDEMER-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						//System.out.println("json content-->"+str_head.toString());
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File PLOPEMERjson = new File (dir);
						FileWriter file = new FileWriter(PLOPEMERjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("PLOPEMERjson.getPath()-->"+PLOPEMERjson.getPath());
						//System.out.println("PLOPEMERjson.getAbsolutePath()-->"+PLOPEMERjson.getAbsolutePath());
						//System.out.println("PLOPEMERjson.getName()-->"+PLOPEMERjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPEMER",PLOPEMERjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  PLOPEMER generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPEMER","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createPLOPEMER function Inside try:"+e.toString());
						System.out.println("Error while file obj  PLOPEMER generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPEMER","IO Error. Unable to create File for PLOPEMER generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for PLOPEMER generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPEMER","No data for PLOPEMER generation");
				}
				
			}	 //check in function control				
		
		}catch (Exception e) {
			  System.err.println("Exception in createPLOPEMER function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"PLOPEMER",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobSchedulePLOPEMER extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobSchedulePLOPEMER = new JobSchedulePLOPEMER();
    ...
    con =   ConnectionManager.getConnection();
		...
		rset =  stmt.executeQuery("select nvl(to_char(LAG_TIME,'HH24'),'00') hr,nvl(to_char(LAG_TIME,'mi'),'00') min from MR_SMRP_CONFIG where SMRP_ID='PLOPEMER'");
		String hr_lag = "00";
		String min_lag = "00";
		if(rset!=null && rset.next()){
    ...
    timer.scheduleAtFixedRate(JobSchedulePLOPEMER, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
    //timer.scheduleAtFixedRate(JobSchedulePLOPEMER, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createPLOPEMER(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobSchedulePLOPEMER Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
