...
public class GenerateCS_SW extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleCS_SW js =new JobScheduleCS_SW();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createCS_SW function which will create the JSON file*/
			GenerateCS_SW Gjson= new GenerateCS_SW();

			String CS_SW_fileCreated_flag =Gjson.createCS_SW(facility_id,from_date,to_date);

			String error="";
			if(CS_SW_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (CS_SW_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (CS_SW_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + CS_SW_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createCS_SW(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='CS_SW'");
				...
				/*The View which is created for this createCS_SWJson purpose*/
				...
				String sqlQuery		  = "SELECT * FROM MR_CS_SW_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				
				//String sqlQuery		  = "SELECT * FROM MR_CS_SW_VW  where rownum<3";// WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('06/10/2021','dd/mm/yyyy') AND TO_DATE ('06/10/2021', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String pat_class="";
				 ...
				 while(rset!=null && rset.next()) 	{  	 // looping each ip encounter
					...
					pstmt_1 =  con.prepareStatement("select MR_CS_SW_DTLS("+enc_id+") SW_DTLS from dual");
					...
					while(rset_1!=null && rset_1.next()){
						...
						SW_dtls=checkForNull(rset_1.getString("SW_DTLS"));
					}
					
					System.out.println("SW_dtls-->"+ SW_dtls);
					...
					temp_tkn		= new StringTokenizer(SW_dtls,"@" );
					
					int kk=0;
					StringTokenizer practtkn=null;
					
					StringBuffer officers =new StringBuffer(" \"officers\": [	");
					...
					while (temp_tkn.hasMoreTokens()){
						...
						while (temp_tkn_inner.hasMoreTokens()){
							...
							if (kk==0){
								ord_id=temp_tkn_inner.nextToken();
								ord_dt=temp_tkn_inner.nextToken();
							}else{
							...
							remarks=temp_tkn_inner.nextToken();
								...
								if(holder.equals("00")){
									officers.append("\"fullName\": \"\",");	
								}else{
									officers.append("\"fullName\": \""+practtkn.nextToken()+"\",");
								}
								
								if(remarks.equals("**"))
									remarks="";
								
								officers.append("\"remarks\": \""+remarks+"\",");
								holder=practtkn.nextToken();
								if(holder.equals("**")){
									officers.append("\"regNumber\": \"\"}");
								}else{
									officers.append("\"regNumber\": \""+holder+"\"}");
								}
					...
					StringBuffer socialWork= new StringBuffer(" \"socialWork\": {	");
					socialWork.append("\"refNo\": \""+ord_id+"\",");
					socialWork.append("\"mswNo\": \""+checkForNull(rset.getString("mswNo"))+"\",");
					socialWork.append("\"therapeuticSupportCaseResponseDate\": \""+ord_dt+"\",");
					socialWork.append("\"refDisciplineCode\": \""+checkForNull(rset.getString("refDisciplineCode"))+"\",");
					socialWork.append("\"refSpecialityCode\": \""+checkForNull(rset.getString("refSpecialityCode"))+"\",");
					socialWork.append("\"refSubSpecialityCode\": \""+checkForNull(rset.getString("refSubSpecialityCode"))+"\",");
					socialWork.append("\"mainDiagnosis\": \""+checkForNull(rset.getString("mainDiagnosis"))+"\",");
					socialWork.append("\"refProgramCode\": \""+checkForNull(rset.getString("refProgramCode"))+"\",");				
					socialWork.append("\"otherProgram\": \""+checkForNull(rset.getString("otherProgram"))+"\",");
					
					
					
					StringBuffer caseCategories= new StringBuffer(" \"caseCategories\": [	");
					caseCategories.append("{");
					caseCategories.append("\"refDataCode\":  \"\",");
					caseCategories.append("\"values\": [],");					
					caseCategories.append("\"caseCategories\":[]}],");
					
					StringBuffer helpPractices= new StringBuffer(" \"helpPractices\": [	");
					helpPractices.append("{");
					helpPractices.append("\"refDataCode\":  \"\",");
					helpPractices.append("\"value\": \"\",");		
					
					helpPractices.append("\"medicalTreatment\":{");
						helpPractices.append("\"facilityCode\": \"\",");
						helpPractices.append("\"dietarySupplement\": \"\",");
							helpPractices.append("\"sourcesOfFinance\":  [	{");
								helpPractices.append("\"refSourceOfFinancialCode\": \"\",");
								helpPractices.append("\"totalApplied\": \"\",");
								helpPractices.append("\"totalPaid\": \"\",");
								helpPractices.append("\"totalApproved\": \"\"}]},");
					helpPractices.append("\"sourcesOfFinance\":  [	{");
					helpPractices.append("\"refSourceOfFinancialCode\": \"\",");
					helpPractices.append("\"totalApplied\": \"\",");
					helpPractices.append("\"totalPaid\": \"\",");
					...
					helpPractices.append("\"helpPractices\":[],");	
					
					helpPractices.append("\"helpPracticeAppliedDate\": \"\",");
					helpPractices.append("\"helpPracticePassedDate\": \"\",");
					
					StringBuffer therapeuticSupports= new StringBuffer(" \"therapeuticSupports\": [	");
					
					therapeuticSupports.append("\"therapeuticSupports\":[");
					therapeuticSupports.append("{");
					therapeuticSupports.append("\"refDataCode\":  \"\",");
					therapeuticSupports.append("\"values\": [],");					
					therapeuticSupports.append("\"therapeuticSupports\":[]}],");
					
					StringBuffer processInterventions= new StringBuffer(" \"processInterventions\": [	");
					
					processInterventions.append("\"processInterventions\":[");
					processInterventions.append("{");
					processInterventions.append("\"refProcessInterventionCodes\":  \"\",");
					processInterventions.append("\"otherProcessIntervention\":  \"\",");
					processInterventions.append("\"givenInterventionDate\":  \"\"}],");
					
					processInterventions.append("\"reportReceivedDate\":  \"\",");
					processInterventions.append("\"quotationReceivedDate\":  \"\",");
					processInterventions.append("\"patientDocumentReceivedDate\":  \"\",");
					processInterventions.append("\"socialReportPreparedDate\":  \"\",");
					processInterventions.append("\"validationSupportDate\":  \"\",");
					processInterventions.append("\"completedDocumentReceivedDate\":  \"\",");
					processInterventions.append("\"documentSentToTBPDate\":  \"\",");
					
					
					officers.append("]");
					
				
					//socialWork.append(caseCategories.toString());
					//socialWork.append(helpPractices.toString());
					
					//socialWork.append(therapeuticSupports.toString());
					//socialWork.append(processInterventions.toString());
					
					
					socialWork.append(officers.toString());
					
					clinicalsupport.append(socialWork.toString());
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_CS_SW.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- CS_SW generate srvlt-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File CS_SWjson = new File (dir);
						FileWriter file = new FileWriter(CS_SWjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("CS_SWjson.getPath()-->"+CS_SWjson.getPath());
						//System.out.println("CS_SWjson.getAbsolutePath()-->"+CS_SWjson.getAbsolutePath());
						//System.out.println("CS_SWjson.getName()-->"+CS_SWjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_SW",CS_SWjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  CS_SW generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_SW","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createCS_SW function Inside try:"+e.toString());
						System.out.println("Error while file obj  CS_SW generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_SW","IO Error. Unable to create File for CS_SW generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for CS_SW generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_SW","No data for CS_SW generation");
				}
	}	 //check in function control				

		
		}catch (Exception e) {
			  System.err.println("Exception in createCS_SW function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_SW",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleCS_SW extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleCS_SW = new JobScheduleCS_SW();
    ...
    timer.scheduleAtFixedRate(JobScheduleCS_SW, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobScheduleCS_SW, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createCS_SW(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleCS_SW Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
