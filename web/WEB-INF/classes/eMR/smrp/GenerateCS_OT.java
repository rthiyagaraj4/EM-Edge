...
public class GenerateCS_OT extends HttpServlet 
{ 
	...
	public void init(ServletConfig config) throws ServletException	
	{
		...
		JobScheduleCS_OT js =new JobScheduleCS_OT();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException 
	{
		...
		/*This will make call to createCS_OT function which will create the JSON file*/
			GenerateCS_OT Gjson= new GenerateCS_OT();

			String CS_OT_fileCreated_flag =Gjson.createCS_OT(facility_id,from_date,to_date);

			String error="";
			if(CS_OT_fileCreated_flag.equals("1")){
				error= "SMRP_FILE_GENERATED";
			}else if (CS_OT_fileCreated_flag.equals("0")){
				error= "SMRP_FILE_NOT_GENERATED";
			}else if (CS_OT_fileCreated_flag.equals("2")){
			...
			res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error, "UTF-8" ) + "&err_value=" + CS_OT_fileCreated_flag );
...
/*This function will Create the Json based on the Parameters*/
	public String createCS_OT(String facility_id,String start_date,String end_date)					
	{ 
		...
		try {			
			...
			if(dir.equals("") && def_duration==0) {
				...
				def_duration=1;
				stmt =  con.createStatement();
				rset =  stmt.executeQuery("select DURATION def_duration from MR_SMRP_CONFIG where SMRP_ID='CS_OT'");
				...
				/*The View which is created for this createCS_OTJson purpose*/
				...
				String sqlQuery		  = "SELECT * FROM MR_CS_OCTH_VW  WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('"+start_date+"','dd/mm/yyyy') AND TO_DATE ('"+end_date+"', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				
				//String sqlQuery		  = "SELECT * FROM MR_CS_OCTH_VW  where rownum<6";// WHERE trunc(DISCHARGE_DATE_TIME) BETWEEN TO_DATE ('06/10/2021','dd/mm/yyyy') AND TO_DATE ('06/10/2021', 'dd/mm/yyyy') + 0.99999  and FACILITY_CODE='"+facility_id+"' ORDER BY VISIT_ADM_DATE_TIME";
				...
				String pat_class="";
				 while(rset!=null && rset.next()) 	{  	 // looping each ip encounter
					...
					StringBuffer occupationalTherapy= new StringBuffer(" \"occupationalTherapy\": {	");
					occupationalTherapy.append("\"mainDiagnosis\": \""+checkForNull(rset.getString("mainDiagnosis"))+"\",");
					occupationalTherapy.append("\"refDisciplineCode\": \""+checkForNull(rset.getString("refDisciplineCode"))+"\",");
					occupationalTherapy.append("\"refSpecialityCode\": \""+checkForNull(rset.getString("refSpecialityCode"))+"\",");
					occupationalTherapy.append("\"refSubSpecialityCode\": \""+checkForNull(rset.getString("refSubSpecialityCode"))+"\",");
					occupationalTherapy.append("\"refProgramCode\": \""+checkForNull(rset.getString("refProgramCode"))+"\",");
					occupationalTherapy.append("\"refSubProgramCode\": \""+checkForNull(rset.getString("refSubProgramCode"))+"\",");
					
					System.out.println("enc_id-->"+ enc_id);
					pstmt_1 =  con.prepareStatement("select MR_CS_OCTH_DTLS("+enc_id+") OCTH_DTLS from dual");
					...
					while(rset_1!=null && rset_1.next()){
						...
						OCTH_dtls=checkForNull(rset_1.getString("OCTH_DTLS"));
					}
					
					System.out.println("OCTH_dtls-->"+ OCTH_dtls);
					temp_tkn=null;
					temp_tkn_inner=null;
					
					temp_tkn		= new StringTokenizer(OCTH_dtls,"@" );
					...
					while (temp_tkn.hasMoreTokens()){
						...
						while (temp_tkn_inner.hasMoreTokens()){
							...
							if(activity_type.equals("00")){	
								assessments.append("{");
								assessments.append("\"refAssessmentInterventionCode\": \"351\",");
								assessments.append("\"refAssessmentCodes\": [],");
								assessments.append("\"refInterventionCodes\": [],");
								if(aasmt_inter.equals("A")){									
									assessments.append("\"assessmentOthers\":[ \""+catalog_desc+"\"],");
									assessments.append("\"interventionOthers\": [] }");
								}else{
									assessments.append("\"assessmentOthers\":[],");
									assessments.append("\"interventionOthers\": [\""+catalog_desc+"\"] }");
								}
							}
							...
							if(!activity_type.equals("00") && !catalog_codes.equals("00")){	
								assessments.append("{");
								assessments.append("\"refAssessmentInterventionCode\": \""+activity_type+"\",");
								if(aasmt_inter.equals("A")){									
									assessments.append("\"refAssessmentCodes\":[ \""+catalog_codes+"\"],");
									assessments.append("\"refInterventionCodes\": [],");
									
								}else{									
									assessments.append("\"refAssessmentCodes\":[],");
									assessments.append("\"refInterventionCodes\": [\""+catalog_codes+"\"],");
					...
					therapists.append("]");
					assessments.append("]");
					clinicalsupport.append(occupationalTherapy.toString());
				...
				if(i>0){
					...
					try{
						...
						//FileWriter file = new FileWriter(dir+"\\"+filename+"_CS_OT.json");/*File is Created with Json object as data in specific folder*/
						System.out.println("<-- CS_OT generate srvlt-->"+str_head.toString());
						//System.out.println("filename-->"+filename);
						dir=dir+filename;
						System.out.println("dir-->"+dir);
						File CS_OTjson = new File (dir);
						FileWriter file = new FileWriter(CS_OTjson);/*File is Created with Json object as data in specific folder*/
						//System.out.println("CS_OTjson.getPath()-->"+CS_OTjson.getPath());
						//System.out.println("CS_OTjson.getAbsolutePath()-->"+CS_OTjson.getAbsolutePath());
						//System.out.println("CS_OTjson.getName()-->"+CS_OTjson.getName());
						...
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_OT",CS_OTjson.getAbsolutePath());
						file.flush();
						file.close();

						if(dir.equals("")){
							fileCreated="2";
							 System.out.println("Error while file obj  CS_OT generation. No dir path-->"+filename);
							 smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_OT","File creation path is not defined");
						}

					} catch (IOException e) {
						fileCreated="2";
						System.err.println("Exception in createCS_OT function Inside try:"+e.toString());
						System.out.println("Error while file obj  CS_OT generation-->"+filename+"-->"+e.toString());
						smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_OT","IO Error. Unable to create File for CS_OT generation");
						e.printStackTrace();
					}
				}else{ // no data for file generation
					System.out.println("No data for CS_OT generation-->"+filename);
					fileCreated="0";
					smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_OT","No data for CS_OT generation");
				}
	}	 //check in function control					
		
		}catch (Exception e) {
			  System.err.println("Exception in createCS_OT function outside try:"+e.toString());
			  smrpLogger.logSMRPStatus(con,fileCreated,filename,start_date,"CS_OT",e.toString());
			  fileCreated="0";
			  e.printStackTrace();
		}finally{
			try{
...
public static class JobScheduleCS_OT extends TimerTask {

 /*The main method will be invoked when Server is Started*/
  public  void main (String... arguments ) {
    TimerTask JobScheduleCS_OT = new JobScheduleCS_OT();
    ...
    timer.scheduleAtFixedRate(JobScheduleCS_OT, getTomorrowMorning12am(hr,mn), fONCE_PER_DAY);
	//timer.scheduleAtFixedRate(JobScheduleCS_OT, getTomorrowMorning12am(), fONCE_PER_DAY);
  }
...
try
 ...
 while(rset!=null && rset.next())
    ...
    Gjson.createCS_OT(rset.getString("facility_id"),"","");
		...
		{
			  System.err.println("Exception in run method of JobScheduleCS_OT Class:::"+e.toString());
			  e.printStackTrace();
		}
  }
