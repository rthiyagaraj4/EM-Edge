...
public class CreateUserDesktopServlet extends javax.servlet.http.HttpServlet implements SingleThreadModel{
	...
	public synchronized void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException{
		...
		operation = req.getParameter("function_name")==null?"insert":req.getParameter("function_name");
...
private void insertCode(HttpServletRequest req, HttpServletResponse res){
	...
	// String modifiedDate = addedDate ;
		...
		summ_show_by= req.getParameter("summ_show_by")==null?"H":req.getParameter("summ_show_by");
		...
		try{
			...
			pstmt.setString(17,summ_show_by);
			...
			if(resulFlag){
				...
				if(bUpdated)
				{
					con.commit();
					error_value			= "1" ;
					error				= getMessage(locale, "RECORD_INSERTED","SM");
				}
				else
				{
					con.rollback();
					error_value			= "0" ;
					error				= getMessage(locale, "FAILED_TRANSACTION","SM");
				}
...
private void modifyCode(HttpServletRequest req, HttpServletResponse res){
	...
	// String modifiedDate = addedDate ;
		...
		try{
			...
			String Sqlupdate= "update SM_DESKTOP_SUM_HDR set " +
						"DESKTOP_SUMMARY_NAME= ?,  " +
						"DESKTOP_SUMMARY_TYPE= ?,  " +
						"SYS_USER_DEF_IND	=?,  " +
						"DESKTOP_LAYOUT_TYPE =?, "+
						"NAVIGATION_PANEL_TYPE =? ,	"+
						"EFF_STATUS=? ,	"+
			            "DFLT_CONTENT_TYPE=? ,"+
				        "DFLT_MODULE_ID=? ,	"+
						"DFLT_FUNCTION_ID=? ,	"+
						...
						"DESKTOP_SUMMARY_AREA=?  "+
				        "where DESKTOP_SUMMARY_ID=? ";


           pstmt=con.prepareStatement(Sqlupdate);
			pstmt.setString(1,name.trim());
			pstmt.setString(2,desktop_summary_type.trim());
			pstmt.setString(3,"U");
			pstmt.setString(4,"01");
			pstmt.setString(5,"01");
			pstmt.setString(6,effectiveStatus);
			pstmt.setString(7,"E");
			pstmt.setString(8,module_id);
			pstmt.setString(9,Function_id);
			...
			pstmt.setString(16,summ_show_by);
			pstmt.setString(17,code.trim());
			int resCount=pstmt.executeUpdate();
