...
public class OperationalStatusEntityServlet extends javax.servlet.http.HttpServlet implements SingleThreadModel
{
	PrintWriter out;
	Properties p;
	...
	private void insertUpdateOperValues(HttpServletRequest req, HttpServletResponse res)
	{
		try
		{
			...
			try
			{
				pstmt = con.prepareStatement("SELECT count(*) FROM SM_MODULES_ACCT_ENTITY WHERE ACCT_ENTITY_ID = '"+entity+"' ");
				...
				if(rec_count == 0 && operational_modules.length() > 0)
				{
					...
					tabledata.put("ACCT_ENTITY_ID",entity);
					...
					String dupfields[]={"MODULE_ID","ACCT_ENTITY_ID"};
					...
					while(smodtoken.hasMoreTokens())
					{
						...
						String table_name="SM_MODULES_ACCT_ENTITY";
						argArray[0] = p;
					...
					else if(rec_count > 0 && operational_modules.length() > 0)
				{
					...
					while(smodtoken.hasMoreTokens())
					{
						...
						condfields.put("ACCT_ENTITY_ID",entity);
						condfields.put("MODULE_ID",smodtoken.nextToken());
