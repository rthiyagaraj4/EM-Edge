...
---------------------------------------------------------------------------------------------------------------
1            V210902             23250        BL-Regression-MMS-Register Visit -Payer     Mohana priya K
2			 V211221			 26886		Alpha-PMG2021-MMS-CRF-0004/01-All payer		Mohanapriya K
3			 V220908							TH-KW-CRF-0093 			                  Namrata Charate
4			 V230315							MMS-DM-CRF-0209.5						  Namrata Charate
...
public class BLFinancialDetailsServlet extends javax.servlet.http.HttpServlet
{
...
//			int total_records=Integer.parseInt(str_total_records);
			...
			String str_mcn_for_policy=req.getParameter("rtn_mcn_for_policy");  //V220908
			System.err.println("str_mcn_for_policy "+str_mcn_for_policy); //V220908
			String str_reg_ref_id=req.getParameter("rtn_reg_ref_id");  //V230315
			System.err.println("str_reg_ref_id "+str_reg_ref_id); //V230315
			String str_userId_for_priv=req.getParameter("rtn_userId_for_priv");  //V230707
			System.err.println("str_userId_for_priv "+str_userId_for_priv); //V230707
			String str_reasonCode_for_priv=req.getParameter("rtn_reasonCode_for_priv");  //V230707
			System.err.println("str_reasonCode_for_priv "+str_reasonCode_for_priv); //V230707
...
// -------------------------------------------------------------------------------------------------
			...
			System.out.println("BLFinancialDetailsServlet.java:::rtn_payerEligibilityParamSession...."+rtn_payerEligibilityParamSession);
			...
			System.out.println("BLFinancialDetailsServlet.java:::rtn_responseCodeDesc...."+rtn_responseCodeDesc);
...
//Added V190801 Gayathri/GHL-CRF-0523
			String rtn_patBedCategory=req.getParameter("rtn_patient_bed_category");			
			if(rtn_patBedCategory == null) rtn_patBedCategory="";
...
/****************END******************************************************************/
			...
			tabdata.put("mcn_for_policy",str_mcn_for_policy);  //V220908
			tabdata.put("reg_ref_id",str_reg_ref_id);  //V230315
			tabdata.put("userId_for_priv",str_userId_for_priv);  //V230707
			tabdata.put("reasonCode_for_priv",str_reasonCode_for_priv);  //V230707
...
/* Added for incorporating the Billing Classification Enhancement */
			...
			tabdata.put("rtn_payerEligibilityParamSession",rtn_payerEligibilityParamSession);
			tabdata.put("rtn_responseCode",rtn_responseCode);
			tabdata.put("rtn_responseCodeDesc",rtn_responseCodeDesc);
			tabdata.put("rtn_statuDescription",rtn_statuDescription);
			tabdata.put("rtn_validityPeriod",rtn_validityPeriod);
			tabdata.put("rtn_validityPeriodDesc",rtn_validityPeriodDesc);
			tabdata.put("rtn_eligibilityCode",rtn_eligibilityCode);
			tabdata.put("rtn_transactionId",rtn_transactionId);
			tabdata.put("rtn_responseDateTime",rtn_responseDateTime);
...
/*********ADDED BY KARTHIK TO INCLUDE INSURANCE POLICY DETAILS INCLUDE_POLICY_DETAILS_CRF ****************************************************/
			...
			if(policyBLInclusionFlag!=null && policyBLInclusionFlag.equals("Y")){			
				...
				while(st1.hasMoreTokens()){
					...
					if( beanInstance != null)
					{
					//Added try catch block against V210902 Starts
					try{
						...
						System.err.println("exception handled in BLfinancialDetlsServ ,743");
						//pcyNo="";
						excep.printStackTrace();
					}
				//Added try catch block against V210902 Ends
...
/* Karthik added the code to add claim code starts   - IN50355 */	
			...
			String str_clinic_code=req.getParameter("rtn_clinic_code");
			String str_site_spec=req.getParameter("rtn_site_spec");
			String str_module_id=req.getParameter("rtn_calling_module_id");
			
			
			System.err.println("Inside BLFinancialDetailsServlet str_clinic_code :: "+str_clinic_code);
			...
			while(strPriorityTok.hasMoreTokens()){
				...
				if(arrClaimCredit.length >0){
					...
					/*Added Below Condition by Rajesh V for MMS-MD-SCF-0077*/
					if(temp_membership_number == null || "null".equals(temp_membership_number)){
...
//			tabdata.put("total_records",str_total_records);
			...
			tabdata.put("rtn_patBedCategory", rtn_patBedCategory);//Added V190801 Gayathri/GHL-CRF-0523
			if("true".equalsIgnoreCase(str_site_spec)){
				if("OP".equalsIgnoreCase(str_module_id)||"AE".equalsIgnoreCase(str_module_id)){
					session.setAttribute("str_clinic_code",str_clinic_code); 		
				}
			}
			
			System.err.println("finDetClose :"+finDetClose);
			System.err.println("finDetClose :"+finDetClose+"clinic_code "+str_clinic_code+" str_site_spec"+str_site_spec+" str_module_id "+str_module_id);
...
/*********ADDED BY RAM GHL-CRF-1.2****************************************************/
				...
				session.removeAttribute("rtn_patBedCategory");//Added V190801 Gayathri/GHL-CRF-0523
/********************END**************************************************************/
...
//			System.out.println("fin_dtls in Submit After Session Removal:"+(HashMap)session.getAttribute("financial_details"));
		...
		out.println("<script src='../eCommon/js/showModalDialog.js' language='JavaScript'></script><script>window.close();</script>");//NMC-JD-CRF-0054
		//out.println("<script>parent.parent.document.getElementById('dialog_tag').close();</script>");
//		out.println("<script>");
//		out.println("const diagElement = parent.parent.document.getElementsByTagName('dialog');");
//		out.println("var returnValue;"); // Declare returnValue
//		out.println("for (var i = 0; i < diagElement.length; i++) {");
//		out.println("    for (var j = 0; j < diagElement[i].children.length; j++) {");
...
//		out.println("            if (dialogSrc.includes('AddModifyPatFinDetailsMain.jsp')) {");
