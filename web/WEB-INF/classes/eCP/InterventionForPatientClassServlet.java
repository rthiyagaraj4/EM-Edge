...
import eCP.InterventionForPatientClass.*;

public class InterventionForPatientClassServlet extends javax.servlet.http.HttpServlet 
	{
	PrintWriter out;
   ...
   public void doGet(HttpServletResponse res) throws javax.servlet.ServletException,IOException
	...
	private void SpecialityAddModify(HttpServletRequest req, HttpServletResponse res)
	{
		try
		{
			...
			patient_class = req.getParameter("patient_class");
		...
		if(from != null && to != null)
					...
					if(!(InterventionPatient.containsObject(checkedOnes)))
				 			...
				 			for(int i=0;i<InterventionPatient.getSize();i++)
				 			{
				 				tabdata.put( ("chk"+i), (String)InterventionPatient.getObject(i) );
				 				
				 			}

					String slCount	=	""+InterventionPatient.getSize();
		...
		Object homeObject = context.lookup("java:comp/env/InterventionForPatientClass");
		InterventionForPatientClassHome home = (InterventionForPatientClassHome) PortableRemoteObject.narrow(homeObject ,InterventionForPatientClassHome.class);
		InterventionForPatientClassRemote remote = home.create();
		java.util.HashMap results = remote.SpecialityAddModify(p,tabdata) ;*/
				...
				local_ejbs = true;
				Object obj = com.ehis.eslp.ServiceLocator.getInstance().getHome("java:comp/env/InterventionForPatientClass", eCP.InterventionForPatientClass.InterventionForPatientClassHome.class, local_ejbs);
		...
		{
			session.removeAttribute("InterventionPatient");
		}
	}//E.O.Method
} 
