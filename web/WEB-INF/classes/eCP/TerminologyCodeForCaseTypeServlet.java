...
import eCP.TerminologyCodeForCaseType.*;

public class TerminologyCodeForCaseTypeServlet
...
public void doPost(HttpServletRequest request, HttpServletResponse response) throws javax.servlet.ServletException,IOException	
	...
	try
		{
            ...
            HashMap tabdata=new HashMap();
            RecordSet	TermCodeForCaseType	=	null;
            TermCodeForCaseType	=	(webbeans.eCommon.RecordSet)	session.getAttribute("TermCodeForCaseType");
				...
				for( int i=(Integer.parseInt(from)); i<=(Integer.parseInt(to)); i++){
					...
					if(TermCodeForCaseType != null){
						for(int j=0;j<TermCodeForCaseType.getSize();j++){
							...
							TabList=(ArrayList)TermCodeForCaseType.getObject(j);
							String temp_code=(String)TabList.get(0);
						...
						String mand = request.getParameter("mandatory_yn"+i)==null?"N":request.getParameter("mandatory_yn"+i);
						TabList.add(mand);
						String nature = request.getParameter("nature"+i)==null?"":request.getParameter("nature"+i);
						char nat=' ';
						if(!nature.equals(""))
							nat=nature.charAt(0);
						switch(nat){
						...
						select_val=request.getParameter("nature"+i)==null?"":request.getParameter("nature"+i);
						TabList.add(select_val);
						TermCodeForCaseType.putObject(TabList);
					}
 				}
 			}
			int size=TermCodeForCaseType.getSize();
			...
			tabdata.put("locale",locale);
				...
				local_ejbs = true;
				Object obj = com.ehis.eslp.ServiceLocator.getInstance().getHome("java:comp/env/TerminologyCodeForCaseType", eCP.TerminologyCodeForCaseType.TerminologyCodeForCaseTypeHome.class, local_ejbs);
	...
	{
		session.removeAttribute("TermCodeForCaseType");
	}
}//end of insert
}
