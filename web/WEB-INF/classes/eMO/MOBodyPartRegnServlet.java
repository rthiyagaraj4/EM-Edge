...
import eMO.BodyPartRegistration.*;
import webbeans.eCommon.*;
import com.ehis.util.*;

public class MOBodyPartRegnServlet extends HttpServlet {
	...
	String refTypeBrghtByPoliceDisable = "";
...
//    java.text.SimpleDateFormat dateFormat = new java.text.SimpleDateFormat( "dd/mm/yyyy" ) ;
	...
	public synchronized void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException{
			...
			body_received_date          = DateUtils.convertDate(body_received_date,"DMYHM",locale,"en");
			...
			assign_date_time            = DateUtils.convertDate(assign_date_time,"DMYHM",locale,"en");
		    ...
		    organ                       = checkForNull(req.getParameter("organ"));
		    ...
		    body_part_obtained_from     = checkForNull(req.getParameter("obt_from"));
			...
			//Added by Ashwini on 08-Aug-2017 for ML-MMOH-CRF-0748
			refTypeBrghtByPoliceDisable = req.getParameter("refTypeBrghtByPoliceDisable");
			
			if(!police_received_date.equals(""))
			police_received_date            = DateUtils.convertDate(police_received_date,"DMYHM",locale,"en");			

			if(!body_claiming_date_time.equals(""))
			body_claiming_date_time            = DateUtils.convertDate(body_claiming_date_time,"DMYHM",locale,"en");
			
			if(!burial_app_date.equals(""))
			burial_app_date            = DateUtils.convertDate(burial_app_date,"DMYHM",locale,"en");	


			if(mode.equalsIgnoreCase("modify"))
				{updateMOBodyPartRegn(res); 
				}else
				{
				insertMOBodyPartRegn(res);
				}
...
public static String checkForNull(String inputString, String defaultValue)
{
 ...
 private void  insertMOBodyPartRegn( HttpServletResponse res){
  try{
		...
		tabdata.put("body_part_obtained_from",body_part_obtained_from);
		...
		Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome("java:comp/env/BodyPartRegistration",MOBodyPartRegnHome.class,local_ejbs);
		...
		results =(HashMap)busObj.getClass().getMethod("insertBodyPartRegn", paramArray).invoke(busObj, argArray); 
         inserted = ( ((Boolean) results.get( "status" )).booleanValue() ) ;
		error = (String) results.get("error") ;
        registration_no=(String) results.get("registration_no") ;
		error_value  = "0" ;
		if ( inserted )
		 { error_value = "1" ;
			 doOnlineReports(facilityId,registration_no);			 
		 out.println("<script>parent.messageFrame.location.href='../eCommon/jsp/error.jsp?err_num="+error+"&err_value=1'</script>");
		 }
		  else
		 { 
			//Added by Ashwini on 08-Aug-2017 for ML-MMOH-CRF-0748
			if(refTypeBrghtByPoliceDisable.equals("true"))
			 {
			 out.println("<script>parent.DisableRefTypeHlthCare1();</script>");
			 }
			error_value = "0" ;
			//res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error,"UTF-8" ) + "&err_value=" + error_value );
			out.println("<script>parent.messageFrame.location.href='../eCommon/jsp/error.jsp?err_num="+error+"&err_value=0'</script>");
		 }
...
private void doOnlineReports(String facility_id,String registration_no) throws ServletException, IOException, SQLException
    {
        ...
        htmlFor.append("<script language = 'JavaScript'> async function bodyPartReport(){" );
        ...
        htmlFor.append("  var getUrl        =   '../../eCommon/jsp/InternalReportsPrint.jsp?P_function_id=MO_BODY_PART_REGN&step=1&sqlString="+java.net.URLEncoder.encode(sqlString)+"&reportParamNames="+reportParamNames+"&reportParamValues="+reportParamValues+"&Patient_ID="+patient_id+"';");
        ...
        htmlFor.append("bodyPartReport(); </script>"  );
        out.println(htmlFor.toString());
		htmlFor.setLength(0);
 }


 private void  updateMOBodyPartRegn( HttpServletResponse res){
  try{
	...
	paramArray[1] = tabdata.getClass();
	java.util.HashMap results=new HashMap();
    results =(HashMap)busObj.getClass().getMethod("updateBodyPartRegn", paramArray).invoke(busObj, argArray); 
	...
	if ( inserted )
	{ 
		out.println("<script>alert('"+error+"');parent.onSuccess();</script>");
		/*error_value = "1" ;
	 res.sendRedirect("../eCommon/jsp/error.jsp?err_num=" + java.net.URLEncoder.encode( error,"UTF-8" ) + "&err_value=" + error_value );
	 */
	}
