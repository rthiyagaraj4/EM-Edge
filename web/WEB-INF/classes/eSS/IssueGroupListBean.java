...
import java.util.*;
import java.io.*;
public class IssueGroupListBean extends eSS.Common.SsTransaction implements Serializable { 
...
//
	...
	/*public String getIssue_group_code_List(){
		ArrayList alParameters	=	new ArrayList(2);
		alParameters.add(getAuth_group_code());
		alParameters.add(getAuth_group_code());
		return getListOptionTag( getListOptionArrayList(getSsRepositoryValue("SQL_SS_ALTERNATE_GROUP_SELECT_LISTS"),alParameters), getIssue_group_code());
	}*/
	//Modified by Sakti against BRU-HIMS-CRF-375 to add group_for_store condition
	...
	public boolean isTrayInArrayList(ArrayList alIssueGroupList, String stTrayNo, String stIssueGroupCode, int exceptionalIndex){
		boolean result=false;
		for (int i=0;i<alIssueGroupList.size();i++) {
			if (exceptionalIndex == i) continue;
			if ( ((HashMap)alIssueGroupList.get(i)).get("tray_no").toString().equals(stTrayNo)&&((HashMap)alIssueGroupList.get(i)).get("issue_group_code").toString().equals(stIssueGroupCode) ) {
				result=true;
				break;
			}
		}
	...
	public void initialize(HashMap hmIssueGroupListItem){
		setPatient_id		((String)hmIssueGroupListItem.get("patient_id"));
		setBin_location_desc((String)hmIssueGroupListItem.get("bin_location_desc"));
		setTray_no  		((String)hmIssueGroupListItem.get("tray_no"));
		setExpiry_date		((String)hmIssueGroupListItem.get("expiry_date"));
		setReq_group_code	((String)hmIssueGroupListItem.get("req_group_code"));
		setPatient_name     ((String)hmIssueGroupListItem.get("patient_name"));
		setIssue_group_code	((String)hmIssueGroupListItem.get("issue_group_code"));
		setBin_location_code((String)hmIssueGroupListItem.get("bin_location_code"));
		setReq_group_desc  	((String)hmIssueGroupListItem.get("req_group_desc"));
		setIssue_group_desc((String)hmIssueGroupListItem.get("issue_group_desc"));
		setIssue_cost		((String)hmIssueGroupListItem.get("issue_cost"));
		setAuth_group_code((String) hmIssueGroupListItem.get("auth_group_code"));
		setAuth_group_desc((String) hmIssueGroupListItem.get("auth_group_desc"));
	}
