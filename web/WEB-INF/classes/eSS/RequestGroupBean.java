...
import eSS.RequestGroup.*;
import javax.servlet.http.HttpSession;

public class RequestGroupBean extends SsTransaction implements Serializable { 
	...
	private String module_id;// Added For OT CRF 35660	Bru-HIMS-CRF-268.1 by Ganga
	...
	public String getAuth_Group_Desc() {
		...
		{
				//String search_type = (String)htTrays.get("search_type");		
				int i=0;
	...
	public HashMap validate(){
		...
		try {
			...
			if (!isDocNoAutoGenYN(getDoc_type_code())) {
				...
				if (!fetchRecord(getSsRepositoryValue("SQL_SS_REQUEST_HDR_COUNT"),alParameters).get("TOTAL").toString().equals("0")) {
			...
			//alRequestGroupList	=	(ArrayList)session.getAttribute("alRequestGroupList");

			/*if((alRequestGroupList.size()==0)||(getRecords_to_delete().indexOf("N")==-1)){
				throw new Exception("MIN_ONE_DETAIL_REQUIRED");
			}*/
			
			if((DataList.size()==0)||(getRecords_to_delete().indexOf("N")==-1)){
				throw new Exception("MIN_ONE_DETAIL_REQUIRED");
			}
	...
	public HashMap insert(){
		...
		try { 
            ...
            alHdrInsertData.add(getTrn_type() );  //Added by Rabbani #INC no:32068 on 06/12/2012	
			...
			alHdrInsertData.add(getLoginAtWsNo());    
			alHdrInsertData.add(getLoginFacilityId());
			alHdrInsertData.add(getModuleId()); //Added For OT CRF 35660	Bru-HIMS-CRF-268.1 by Ganga 
			
			 //Added for 36422	Bru-HIMS-CRF-268.2 Ganga
			alHdrInsertData.add(getAuthorized()); //Finalized_YN
			alHdrInsertData.add("N"); //AutoReq_YN

			//alRequestGroupList	=	(ArrayList)session.getAttribute("alRequestGroupList");
			

			for (int i=0;i<DataList.size() ;i++ ) {//Changed FROM alRequestGroupList TO DataList against BRU-HIMS-CRF-373
			...
			hmSQLMap.put( "HdrInsertSQL",		getSsRepositoryValue("SQL_SS_REQUEST_HDR_INSERT")			);
			hmSQLMap.put( "DtlInsertSQL",		getSsRepositoryValue("SQL_SS_REQUEST_DTL_INSERT")			);
			hmSQLMap.put( "NextDocNoUpdateSQL",	getSsRepositoryValue("SQL_SY_ACC_ENTITY_DOC_TYPE_UPDATE")	);
			hmSQLMap.put( "UpdateGroupForStoreSQL",	getSsRepositoryValue("SQL_SS_GROUP_FOR_STORE_PEND_REQ_QTY_UPDATE"));
			...
			System.err.println("Requestgroupbean Insert hmTableData"+hmTableData);
		}

		return requestGroupMethodCall(hmTableData, hmSQLMap, FUNCTION_INSERT);
	}
	...
	public HashMap modify() {
		...
		alHdrModifyData.add(getReq_on_store_code ()		);
		...
		alHdrModifyData.add(getAuthorized()); //Added for 36422	Bru-HIMS-CRF-268.2 Ganga
		...
		hmSQLMap.put( "HdrModifySQL", getSsRepositoryValue("SQL_SS_REQUEST_HDR_UPDATE")	);
		hmSQLMap.put( "DtlModifySQL", getSsRepositoryValue("SQL_SS_REQUEST_DTL_UPDATE")		);
		hmSQLMap.put( "DtlInsertSQL", getSsRepositoryValue("SQL_SS_REQUEST_DTL_INSERT")		);
		hmSQLMap.put( "DtlDeleteSQL", getSsRepositoryValue("SQL_SS_REQUEST_DTL_DELETE")		);
		hmSQLMap.put( "UpdateGroupForStoreSQL",	getSsRepositoryValue("SQL_SS_GROUP_FOR_STORE_PEND_REQ_QTY_UPDATE"));
		hmSQLMap.put( "UpdateGroupForStoreReduceSQL",	getSsRepositoryValue("SQL_SS_GROUP_FOR_STORE_PEND_REQ_QTY_REDUCE_UPDATE"));
		hmSQLMap.put( "RequestQtySelectSQL",	getSsRepositoryValue("SQL_SS_REQUEST_DTL_REQ_QTY_SELECT"));

		hmTableData.put( "properties",			getProperties() );
		hmTableData.put( "HdrModifyData",		alHdrModifyData	);
		hmTableData.put( "DtlModifyData",		alDtlModifyData	);

		return requestGroupMethodCall(hmTableData, hmSQLMap, FUNCTION_UPDATE);
	}
	...
	public HashMap delete(){
		...
		hmSQLMap.put( "HdrDeleteSQL", getSsRepositoryValue("SQL_SS_REQUEST_HDR_DELETE")	);
		hmSQLMap.put( "DtlDeleteSQL", getSsRepositoryValue("SQL_SS_REQUEST_DTL_DELETE_GROUP")	);
		hmSQLMap.put( "UpdateGroupForStoreReduceSQL",getSsRepositoryValue("SQL_SS_GROUP_FOR_STORE_PEND_REQ_QTY_REDUCE_UPDATE"));
		...
		return requestGroupMethodCall(hmTableData, hmSQLMap, FUNCTION_DELETE);
	}

	public HashMap requestGroupMethodCall(HashMap hmTableData, HashMap hmSQLMap, int functionID ){

		HashMap		hmResult	=	new HashMap()	;
		RequestGroupRemote	requestGroupRemote		= null ;
		hmResult.put(RESULT, FALSE ) ;
		hmResult.put(MESSAGE, "Initial Message" ) ;

		try {
			Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome( getSsRepositoryValue("JNDI_SS_REQUEST_GROUP") ,RequestGroupHome.class,getLocalEJB());
		...
		paramArray[1] = hmSQLMap.getClass();
			...
			switch (functionID)
			{
				...
				default:
					hmResult.put( "message", "Invalid Function ID passed to requestGroupMethodCall()") ;
			}
...
//Adding start for AAKH-CRF-0057
			...
			try {
				if( requestGroupRemote != null )
					requestGroupRemote.remove() ;
			}
			catch( Exception exception) {
	...
	public void loadData() throws Exception {
		...
		HashMap hmRecord		= 	fetchRecord(getSsRepositoryValue("SQL_SS_REQUEST_HDR_SELECT_SINGLE"),alParameters);
...
//Added by Sakti against BRU-HIMS-CRF-373
	public void loadDtlData() {
		...
		try {
			...
			stSQL = eSS.Common.SsRepository.getSsKeyValue("SQL_SS_REQUEST_DTL_SELECT_MULTIPLE");
			listRecord			=		fetchRecords(stSQL.toString(),alParameters);
			
			for(int i=0;i<listRecord.size();i++){
				HashMap dataList		=		(HashMap)listRecord.get(i);

				dataList.put("GROUP_CODE",(String)dataList.get("REQ_GROUP_CODE"));
				...
				dataList.put("AVLBL_STER_QTY",(String)dataList.get("STERILIZED_BLOCKED_QTY")); 
				dataList.put("AUTH_GROUP_DESC",(String)dataList.get("AUTH_GROUP_DESC"));
				dataList.put("AUTH_GROUP_CODE",(String)dataList.get("AUTH_GROUP_CODE"));
				dataList.put("SURGERY_TYPE",checkForNull((String)dataList.get("SURGERY_TYPE_DESC"),""));
				dataList.put("SURGERY_NAME",checkForNull((String)dataList.get("SURGERY_NAME_DESC"),""));
				dataList.put("mode","2");
...
public String getAuthGroupCode(String dflt_code,String auth_group,String req_by_store_code,String req_on_store_code) {
		...
		alOptionValues = getListOptionArrayList(eSS.Common.SsRepository.getSsKeyValue("SQL_SS_ALTERNATE_GROUP_SELECT_LIST"), alParam,false);
...
public String getGroupType(){//MMOH-CRF-1661 
