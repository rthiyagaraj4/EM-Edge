...
import eSS.LoanAcknowledgement.*;

public class LoanAcknowledgementBean extends eSS.Common.SsTransaction implements
		Serializable {
	private String store_code = "";
	private String to_store_code = "";
	private String trn_type = "ISL";
	...
	public String getTrn_type_List() {
		String trn_types = "";
		try {
			HashMap hmTemp = (HashMap) fetchRecord("SELECT ISL_ACK_REQ_YN, RTL_ACK_REQ_YN FROM SS_FACILITY_PARAM WHERE FACILITY_ID = ?",getLoginFacilityId());
			if ((checkForNull((String) hmTemp.get("ISL_ACK_REQ_YN"), "N")).equals("Y")) {
				trn_types = trn_types + "'ISL',";
			}
			if ((checkForNull((String) hmTemp.get("RTL_ACK_REQ_YN"), "N")).equals("Y")) {
				trn_types = trn_types + "'RTL',";
			}
		...
		return getListOptionTag(getListOptionArrayList("SELECT   TRN_TYPE, SHORT_DESC FROM SS_TRN_TYPE_LANG_VW TRN WHERE TRN_TYPE IN ("+ trn_types + ") AND LANGUAGE_ID = ? ORDER BY DECODE (TRN_TYPE, 'ISL', 1, 'RTL', 2)",getLanguageId()), "ISL");
	}

	public String getDoc_type_code_List() {
		// String chkParameters[] =
		// {getTrn_type(),getLoginFacilityId(),getLanguageId()};
		ArrayList arraylist = new ArrayList();
		arraylist.add(getTrn_type());
		arraylist.add(getLoginFacilityId());
		arraylist.add(getLanguageId());
		// return
		// getListOptionTag(getListOptionArrayList(getSsRepositoryValue("SS_SQL_SS_ACKNOWLEDGE_DOC_TYPE_CODE_SELECT"),chkParameters));
	...
	public String getStore_code_List() {
		...
		if ((getAction_type().equals("A")))
			return getListOptionTag(getListOptionArrayList(getSsRepositoryValue("SS_SQL_LOAN_ACKNOWLEDGE_STORE_LIST_ACK_SELECT"),arraylist));
		else
			return getListOptionTag(getListOptionArrayList(getSsRepositoryValue("SS_SQL_LOAN_ACKNOWLEDGE_STORE_LIST_CFM_SELECT"),arraylist));
	}
	...
	public ArrayList getStoreArrayList() throws Exception {
		...
		arraylist.add(getTrn_type());
		if ((getAction_type().equals("A")))
			return fetchRecords(getSsRepositoryValue("SS_SQL_LOAN_ACKNOWLEDGE_STORE_LIST_ACK_SELECT"),arraylist);

		else
			return fetchRecords(getSsRepositoryValue("SS_SQL_LOAN_ACKNOWLEDGE_STORE_LIST_CFM_SELECT"),arraylist);
	}
	...
	// Added by Sakti for checkbox key validation
	...
	/*
	 * public HashMap validate() throws Exception{ HashMap hmReturn =
	 * getMessageHashMap(true); String adj_doc_type_code = (String)fetchRecord(
	 * "SELECT DEF_ADJ_DOC_TYPE_CODE DOC_TYPE_CODE FROM ST_FACILITY_PARAM WHERE FACILITY_ID = ? AND MATERIAL_GROUP_CODE = 'ALL'"
	 * , getLoginFacilityId()).get("DOC_TYPE_CODE"); if
	 * (checkForNull(adj_doc_type_code).equals("")) { hmReturn.put(RESULT,
	 * FALSE); hmReturn.put(MESSAGE,
	 * getSTMessage(getLanguageId(),"DEF_ADJ_DOC_TYPE_CODE_NOT_SELECTED","ST"));
	 * return hmReturn; } HashMap hmRecord =
	 * fetchRecord(getSsRepositoryValue("SQL_SY_ACC_ENTITY_DOC_TYPE_SELECT"
	 * ),adj_doc_type_code); if
	 * (checkForNull((String)hmRecord.get("NEXT_DOC_NO")).equals("")) {
	 * hmReturn.put(RESULT, FALSE); hmReturn.put(MESSAGE,
	 * getSTMessage(getLanguageId(),"SYS_GEN_DOC_NO_NOT_SELECTED","ST")); }
	 * return hmReturn; }
	 */
	public HashMap insert() {
		// String doc_no = "";
		//String confirm = "Y"; //Comented by sakti as this is unused against inc#48061
		String confirm_date = "";
		Boolean exist = false;
		...
		try {
			...
			if ((getAction_type().equals("A"))) { // Added by Sakti for action type A
				if (alCheckedItems.size() > 0) {// Added by Sakti for multiple check box
					...
					for (int l = 0; l < alCheckedItems.size(); l++) {
						...
						if (hmFinalTFRExpRecords.containsKey(docNoValue + "~" + docSeqNo)) {
							for (int i = 0; i < alTFRDtlRecords.size(); i++) {
								...
								if (hmFinalTFRExpRecords.containsKey((String) hmDtlRecords.get("DOC_NO") + "~"	+ (String) hmDtlRecords.get("SEQ_NO"))) {
									...
									if (dtl_doc_number.equals(docNoValue)&& dtl_seq_number.equals(docSeqNo)) {
										...
										seq_no = Integer.parseInt((String) (((HashMap) fetchRecord("SELECT nvl(MAX(SEQ_NO),0) SEQ_NO FROM SS_LOAN_ACKNOWLEDGE_TRN_HDR WHERE FACILITY_ID = ? AND DOC_NO = ? AND DOC_TYPE_CODE=? ",					alParam)).get("SEQ_NO")));
										++seq_no;
										...
										for (int j = 0; j < alTmpExpRecords.size(); j++) {
											...
											if (!(hmTemp.get("ACKNOWLEDGE_STATUS")).equals("P")) { // Pending records not to consider
												...
												if (getTrn_type().equals("ISL")) {
													...
													// SS_LOAN__ISSUE_DTL_ACK_STATUS_UPDATE DATA ENDS
							...
							// This is to reduce pending ack qty from group for store table only for Issue
										...
										// SS_LOAN_RETURN_HDR_FINALIZE_UPDATE DATA STARTS
							...
							for (int i = 0; i < alTFRDtlRecords.size(); i++) {
								...
								if (hmFinalTFRExpRecords.containsKey((String) hmDtlRecords.get("DOC_NO")+ "~"+ (String) hmDtlRecords.get("SEQ_NO"))) { // doc_no and seq_no comparision between exp and detail record starts
									...
									if (dtl_doc_no.equals(docNoValue) && dtl_seq_no.equals(docSeqNo)) { // doc_no and seq_no comparision between checked record and detail record starts
										...
										if (getTrn_type().equals("ISL")) {
											...
											if (getTrn_type().equals("ISL")) {
												...
												alDtlData.add(hmDtlTemp);
												// SS_LOAN_ISSUE_DTLS_ACK_STATUS_UPDATE DATA ENDS

												
												if (getAction_type().equals("C")) {
													++reduce_blocked_qty;
													alTemp = new ArrayList();
													alTemp.add(""+ reduce_blocked_qty);// Blocked Quantity
													alTemp.add(""+ reduce_blocked_qty);// Reject Quantity
													alTemp.add(getLoginById());
													alTemp.add(getLoginAtWsNo());
													alTemp.add(getLoginFacilityId());
										...
										if (getTrn_type().equals("RTL")) {
											...
											alTemp.add((String) hmDtlRecords.get("DOC_NO"));
											alHdrData.add(alTemp);
											// SS_LOAN_RETURN_HDR_FINALIZE_UPDATE DATA ENDS
		...
		hmSQLMap.put("SS_SQL_SS_LOAN_ACKNOWLEDGE_TRN_HDR_INSERT",getSsRepositoryValue("SS_SQL_LOAN_ACKNOWLEDGE_TRN_HDR_INSERT"));
		hmSQLMap.put("SS_SQL_SS_LOAN_ACKNOWLEDGE_TRN_DTL_INSERT",getSsRepositoryValue("SS_SQL_LOAN_ACKNOWLEDGE_TRN_DTL_INSERT"));
		hmSQLMap.put("SS_SQL_SS_LOAN_ACKNOWLEDGE_TRN_HDR_UPDATE",getSsRepositoryValue("SS_SQL_LOAN_ACKNOWLEDGE_TRN_HDR_UPDATE"));
		...
		hmSQLMap.put("SQL_SS_TRAY_DTL_UPDATE_FROM_LOAN_ACKNOWLEDGE",getSsRepositoryValue("SQL_SS_TRAY_DTL_UPDATE_FROM_LOAN_ACKNOWLEDGE"));
		hmSQLMap.put("SQL_SS_TRAY_DTL_UPDATE_FROM_LOAN_CONFIRM",getSsRepositoryValue("SQL_SS_TRAY_DTL_UPDATE_FROM_LOAN_CONFIRM"));
		...
		try {
			Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome(getSsRepositoryValue("JNDI_SS_LOAN_ACKNOWLEDGEMENT"),LoanAcknowledgementLocalHome.class, getLocalEJB());
	...
	public HashMap stockSQL() {
		...
		HashMap hmSQLMap = new HashMap();
		if (getTrn_type().equals("ISL")) {
			String SQL_SS_LOAN_ISSUE_DTL_ACKNOWLEDGE_YN_FRLG_UPDATE = "UPDATE ss_loan_issue_dtl SET ACK_STATUS	= ? ,modified_by_id = ?,	modified_date =	sysdate, modified_at_ws_no = ?,	modified_facility_id = ?  WHERE facility_id = ? AND doc_type_code = ? AND doc_no = ? and tray_no=? and issue_group_code=?	";
			hmSQLMap.put("IssueDtlUpdateSQL",SQL_SS_LOAN_ISSUE_DTL_ACKNOWLEDGE_YN_FRLG_UPDATE);
			hmSQLMap.put("stSQLReqDtlUpdate",getSsRepositoryValue("SQL_SS_LOAN_REQUEST_DTL_UPDATE_FROM_CONFIRM"));
			hmSQLMap.put("stSQLReqDtlUpdateforissueqty",getSsRepositoryValue("SQL_SS_LOAN_REQUEST_DTL_ISSUE_QTY_UPDATE"));

		} else if (getTrn_type().equals("RTL")) {

			hmSQLMap.put("stSQLHdrUpdate",getSsRepositoryValue("SQL_SS_LOAN_RETURN_HDR_FINALIZE_UPDATE"));
			String SQL_SS_LOAN_RETURN_DTL_ACK_STATUS_YN_FLAG_UPDATE = "UPDATE ss_loan_return_dtl SET ACK_STATUS	= ? ,modified_by_id = ?,	modified_date =	sysdate, modified_at_ws_no = ?,	modified_facility_id = ?  WHERE facility_id = ? AND doc_type_code = ? AND doc_no = ? and tray_no=? and return_group_code=?	";
			hmSQLMap.put("stSQLDtlUpdate",SQL_SS_LOAN_RETURN_DTL_ACK_STATUS_YN_FLAG_UPDATE);
		}

		return hmSQLMap;
	}

	public Boolean getLoanAckReqd() {
		String ack_reqd = "N";
		try {
			HashMap hmTemp = (HashMap) fetchRecord("SELECT ISL_ACK_REQ_YN, RTL_ACK_REQ_YN FROM SS_FACILITY_PARAM WHERE FACILITY_ID = ?",getLoginFacilityId());
			if ((checkForNull((String) hmTemp.get("ISL_ACK_REQ_YN"), "N")).equals("Y")) {
