...
import eSS.CancelLoanRequestGroup.*;


public class CancelLoanRequestBean extends eSS.Common.SsTransaction implements Serializable { 
	...
	//private CancelLoanRequestListBean		cancelLoanRequestListBean	;
...
//		alParam.add(login_by_id);
		return getListOptionTag( getListOptionArrayList(getSsRepositoryValue("SQL_SS_STORE_ISL_SELECT_LIST"), alParam), getReq_on_store_code());
	}
	...
	public String getReq_by_store_code_List(){
		...
		alParam.add(login_facility_id);
		alParam.add(login_by_id);
		return getListOptionTag( getListOptionArrayList(getSsRepositoryValue("SQL_SS_STORE_RQL_SELECT_LIST"), alParam), getReq_by_store_code());
	}
	...
	public void setAll(Hashtable htRecordSet){
		...
		updateSelectedValues(htRecordSet);
		...
		setcancelAll(checkForNull((String) htRecordSet.get("cancelAll"),"N"));
	...
	public HashMap validate(){
		...
		if(getCancelled_remarks().length()>200){
	...
	public HashMap modify() {
		...
		CancelLoanRequestGroupRemote cancelLoanRequestGroupRemote = null ;
	try {
		...
		hmSQLMap.put( "ModifySQL",		getSsRepositoryValue("SQL_SS_LOAN_REQUEST_HDR_UPDATE_CANCEL")	);
		hmSQLMap.put( "UpdateGroupForStoreSQL",	getSsRepositoryValue("SQL_SS_GROUP_FOR_STORE_PEND_REQ_QTY_REDUCE_UPDATE"));
		hmSQLMap.put( "DeleteRequestDetailSQL",	getSsRepositoryValue("SQL_SS_LOAN_REQUEST_DTL_DELETE_GROUP_CODE_WISE"));
		...
		hmTableData.put ("cancelAll",getcancelAll());
			...
			Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome(SsRepository.getSsKeyValue("JNDI_SS_CANCEL_LOAN_REQUEST_GROUP") ,CancelLoanRequestGroupHome.class,getLocalEJB());
		...
		if( ((Boolean) hmResult.get("result")).booleanValue() )	
			...
			try {
				...
				if(cancelLoanRequestGroupRemote != null)
					cancelLoanRequestGroupRemote.remove() ;
			}
			catch(Exception ee) {
	...
	public void loadData() throws Exception {
		...
		HashMap hmRecord		= 	fetchRecord(getSsRepositoryValue("SQL_SS_LOAN_REQUEST_HDR_SELECT_SINGLE_CANCEL"),alParameters);
