        ...
        try
        {
			...
			String tags[] = { "SHORT-TEXT", "LONG-TEXT", "SHORT-TEXT-WTD",
                    "LONG-TEXT-WTD", "TEXT", "DATE-TIME-NUMERIC",
                     "DATE-TIME-NUMERIC-WTD","IMAGE-TAG"}; //"IMAGE-TAG" added for IN069484
	


					if (!XMLContent.equals("") && !(XMLContent.equals("&nbsp;")))
					{				
...
//			Document doc  = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new ByteArrayInputStream(XMLContent.toString().getBytes()));
			...
			//                            boolean flag = false;
										...
										if(strCompType.equals("D") || strCompType.equals("E") || strCompType.equals("A") ||strCompType.equals("G"))
										//IN069484 End.
										{
...
//d									if (flag)
	...
	// following method can be ignored
	public StringBuffer tempDateConvert(StringBuffer XMLContent,String locale)
	{
		...
		{			
			...
			sbXPathDefn.append("/USER-TEMPLATE/ROW/COL/DATE-TIME-NUMERIC[@COMPTYPE='D' or @COMPTYPE='E' or @COMPTYPE='A']");
			...
			if(nlDTNodes != null)
			{
				...
				for(nCurIdx = 0; nCurIdx < nDTNodesLen ; nCurIdx++)
				{
					...
					ndAttNode		= nnmpAttr.getNamedItem("COMPTYPE");
					strCompType		= ndAttNode.getNodeValue();
				
					if(strCompType.equals("A") )
					{
						ndAttNode		= nnmpAttr.getNamedItem("RESULTTYPE");
						strResultType	= ndAttNode.getNodeValue();
					}

					ndAttNode		= nnmpAttr.getNamedItem("VALUE");
					
					if( ndAttNode != null && (strCompType.equals("D") || strCompType.equals("E") || strCompType.equals("A") ) )
					{
						strCompValue	= ndAttNode.getNodeValue();
						
						if(strCompValue != null && !strCompValue.equals(""))
						{
							if(strCompType.equals("D"))
								strCompValue = com.ehis.util.DateUtils.convertDate(strCompValue,"DMY",locale,"en");
							else if(strCompType.equals("E") )
								strCompValue = com.ehis.util.DateUtils.convertDate(strCompValue,"DMYHM",locale,"en");
							else if(strCompType.equals("A") )
							{
								if(strResultType.equals("D") )
								{
									if(strCompValue.length() > 10)
										strCompValue = com.ehis.util.DateUtils.convertDate(strCompValue,"DMYHM",locale,"en");
									else
										strCompValue = com.ehis.util.DateUtils.convertDate(strCompValue,"DMY",locale,"en");
								}
							...
							ndAttNode.setNodeValue(strCompValue);
							nnmpAttr.setNamedItem(ndAttNode);
						}
					}
				}
			}
