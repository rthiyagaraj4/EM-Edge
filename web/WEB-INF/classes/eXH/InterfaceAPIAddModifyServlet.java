 ...
 *
 */
public class InterfaceAPIAddModifyServlet extends HttpServlet
{
	HttpServletRequest req=null;
	/**
	 * Do post method which implements the functionalities for InterfaceAPI setup
	...
	public void doPost (HttpServletRequest request, HttpServletResponse response)throws IOException, ServletException 
	{
		...
		String api_desc=XHDBAdapter.checkNull(request.getParameter("api_desc"));
		String api_type=XHDBAdapter.checkNull(request.getParameter("api_type"));
		String exec_nature=request.getParameter("exec_nature");	
...
//		String lang_id=XHDBAdapter.checkNull(request.getParameter("lang_id"));
		...
		arrColumn.add(api_desc);
		arrColumn.add(api_type);
 ...
 /**
	 * callEJB method is used to call InterfaceAPIAddModifyServlet setup related database procedures
	...
	public HashMap callEJB(HashMap details_hm,String locale )
   {
    ...
    try
	{
      ...
      catch(Exception exp)
      {
		 ...
		 System.out.println("Error in calling getconnection method of InterfaceAPIAddModifyServlet  :"+exp.toString());
	...
	try
	{
	 ...
	 ostmt = connection.prepareCall("{ call Xg_prc.xh_api(?,?,?,?,?,?,?,?,?) }" ); 

		  String  p_mode=(String)row_arr.get(0);
   	      String  api_code=(String)row_arr.get(1);
		  String  api_desc=(String)row_arr.get(2);
		  String  api_type=(String)row_arr.get(3);
		  String  exec_nature=(String)row_arr.get(4);
...
//  	 		 System.out.println("Call EJB api_code : "+api_code);
		 ostmt.setString(3,api_desc);
//	 		 System.out.println("Call EJB api_desc : "+api_desc);
		 ostmt.setString(4,api_type);
//	 		 System.out.println("Call EJB api_type : "+api_type);
		 ostmt.setString(5,exec_nature);
//  	 		 System.out.println("Call EJB exec_nature : "+exec_nature);
...
//			    status = ((String) mesg.get("message"));			
				status = "Child record found in xh_api_details for the data_group_id, cannot delete xh_api record";
		}
        else
		{
                connection.rollback();
	...
	{ 
		 System.out.println("Error in calling dbrelated statements in callejb method of InterfaceAPIAddModifyServlet  :"+ex.toString());
	...
	}catch(Exception ex){ System.out.println("Error in closing dbrealted statements in callejb method of InterfaceAPIAddModifyServlet :"+ex.toString());
		   ex.printStackTrace(System.err); }
