...
public class EMExternalUpdates extends HttpServlet
{
    ...
    { 
		try
		{
			...
			String locale = "en";
			if(session != null) {
				locale = ((Properties) session.getAttribute("jdbc")).getProperty("LOCALE");
			}
	...
	public String saveExternalAppCallDtls(Map<String, String> reqParams, String locale) {
		...
		try
		{
			...
			//	pstmt.setTimestamp(4, convertStrToTimestamp(reqParams.get("P_ENCNTR_DATE"), reqParams.get("callFrom")));
				...
				qry = "UPDATE xh_external_app_simulator SET DOCUMENT_ID = ?, SUB_DOCUMENT_TYPE = ?, DOC_CREATION_DATE = ?, DOC_FILE_NAME = ? WHERE PATIENT_ID = ? AND ENCOUNTER_ID = ? AND FACILITY_ID = ? and encounter_date = (select max(encounter_date) from XH_EXTERNAL_APP_SIMULATOR where patient_id = ? and encounter_id = ?)";
			...
			//	pstmt.setString(7, reqParams.get("orderID"));
				pstmt.setString(7, reqParams.get("facilityID"));
				pstmt.setString(8, reqParams.get("patientID"));
				pstmt.setString(9, reqParams.get("encounterID"));
			}
			...
			//	statusMessage = XHUtil.getMessage("1", locale);
				statusMessage = "Wrong data... go to hell !!!";
			}
		}
		catch (Exception exp) {
		...
		//	else statusMessage = XHUtil.getMessage("1", locale);
			else statusMessage = "Wrong data... go to hell !!!";
		}
		finally {
