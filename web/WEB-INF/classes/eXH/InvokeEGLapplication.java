...
public class InvokeEGLapplication extends HttpServlet {
    ...
    { 
		...
		// added by thiru
			...
			else if ("ABDM".equals(callType)) {
				/*
				 * Added by Gopinath on 24/08/2023 for GHL-CRF-0657.2 This method is
				 * used for mapping Health Id to the patient
				 */
				responseTxt = processABDMRequest(reqParams);
				PrintWriter resp = response.getWriter();
				resp.flush();
				resp.write(responseTxt);
				resp.close();
			}
			else if ("SCANABHA".equals(callType)) {
				responseTxt = searchScanedABHADetails(reqParams);
				PrintWriter resp = response.getWriter();
				resp.flush();
				resp.write(responseTxt);
				resp.close();
		...
		else if ("FetchQueueNumber".equals(callType))
		{
			if(isDebugYN) 
				...
				resp.flush();
				resp.close();
		} else if ("NOTIFYEVENT".equals(callType)){
				System.out.println("Calling NOTIFYEVENT...");
				responseTxt = updateNotifyEvent(request, response);
				PrintWriter resp = response.getWriter();
				resp.write(responseTxt);
				resp.flush();
				resp.close();
			}
			else if ("SINGLESIGNON".equals(callType)){
				/*
				 * Added by Kranthi on 18/10/2023 for ML-MMOH-CRF-1844
				 */
				System.out.println("Calling SINGLESIGNON...");
				responseTxt = processActiveDirRequest(reqParams);
				PrintWriter resp = response.getWriter();
				resp.write(responseTxt);
				resp.flush();
				resp.close();
...
// Added by Apoorva for MOHE-CRF-0020 - This is used for getting the check status flag
	...
	private String updateNotifyEvent(HttpServletRequest request, HttpServletResponse response) {
		
		if (isDebugYN)
			System.out.println(" NotificationEventServlet updateData :::");
		...
		try {
			...
			String checkedEventsId = XHDBAdapter.checkNull(request.getParameter("checkedEventsId"));
			String[] checkedList = checkedEventsId.split(",");
			dbConn = ConnectionManager.getConnection(request);
			boolean updated = true;
			
			if (isDebugYN)
				System.err.println("checkedList.length========= "+checkedList.length);
			
			for (int i = 0; i < checkedList.length; i++) {
				//System.err.println("i ======================= " + i);
				try {
					if(!XHDBAdapter.checkNull(checkedList[i]).equals("")){
						String[] appElementId = checkedList[i].split("\\$\\!\\^");
						
						if (isDebugYN)
							System.out.println("checkedList["+i+"]: "+appElementId[0]+" "+appElementId[1]+" "+appElementId[2]);
						
						String updateQuery = "UPDATE XH_APPL_ELEMENT  Set In_Use_Yn ='"+appElementId[2]+"' Where Application_Id = '"+appElementId[0]+"'  And    Element_Id = '"+appElementId[1]+"'";
			...
			System.err.println("Error in NotificationEventServlet updateData :: " + e);
			e.printStackTrace();
		}finally{
			XHUtil.closeDBResources(rs, pstmt, dbConn);
	...
	private String processABDMRequest(Map<String, String> reqParams)
			throws Exception {

		if(isDebugYN)
			System.out.println("ReParams: " + reqParams.toString());

		String request_type = XHDBAdapter.checkNull(reqParams.get("request_type"));
		String request_option = XHDBAdapter.checkNull(reqParams.get("request_option"));
		String request_data = XHDBAdapter.checkNull(reqParams.get("request_data"));
		String responseTxt = "";
		String p_response = "";
		String errorCode = "";
		String errorMsg = "";
		...
		try {
			...
			ostmt = dbConn.prepareCall("{ CALL XHNDHM.NDHM_REQUEST_NEW(?, ?, ?, ?, ?, ?) }");
			ostmt.setString(1, request_type);
			ostmt.setString(2, request_option);
			ostmt.setString(3, request_data);
			ostmt.registerOutParameter(4, java.sql.Types.CLOB);
			ostmt.registerOutParameter(5, java.sql.Types.VARCHAR);
			ostmt.registerOutParameter(6, java.sql.Types.VARCHAR);
			ostmt.execute();
			p_response = ostmt.getString(4);
			errorCode = ostmt.getString(5);
			errorMsg = ostmt.getString(6);

			
			responseTxt = p_response + "$!^" + errorCode + "$!^ " + errorMsg;
			
			if(isDebugYN){
				System.out.println("(InvokeEGLapplication:processABDMRequest) p_response " + p_response);
				System.out.println("(InvokeEGLapplication:processABDMRequest) errorCode " + errorCode);
				System.out.println("(InvokeEGLapplication:processABDMRequest) errorMsg " + errorMsg);
				System.out.println("(InvokeEGLapplication:processABDMRequest) responseTxt " + responseTxt);
			}

		} catch (Exception exp) {
			System.out.println("(InvokeEGLapplication:processABDMRequest) Exception at executing procedure");
			exp.printStackTrace(System.err);
		} finally {
			XHUtil.closeDBResources(null, ostmt, dbConn);
		}
	...
	private String searchScanedABHADetails(Map<String, String> reqParams) {
		String response="";
		Connection dbConn = null;
		CallableStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			String mobile = reqParams.get("mobileNumber");
			String flag =reqParams.get("usedFlag");
			
			if(isDebugYN)
				System.out.println("(InvokeEGLapplication:processABDMRequest) name..."+name+" mobile..."+mobile+" flag..."+flag);

			if(!name.equals("")&&!mobile.equals("")){
				sql = "SELECT PATIENT_NAME,SEX,TO_CHAR(DATE_OF_BIRTH,'DD/MM/YYYY') DATE_OF_BIRTH,PATIENT_ID,CONTACT1_NO FROM XH_PATIENT_EXT_INFO WHERE "
	                + "APPLICATION_ID = 'ACONNECT' AND UPPER(PATIENT_NAME) LIKE ? AND CONTACT1_NO=? ";
				
				pstmt = dbConn.prepareCall(sql);
				
				if(!flag.equals("A")){
					sql += "AND PRIM_SEC_FLAG=?"; 
					pstmt = dbConn.prepareCall(sql);
					pstmt.setString(3, flag);
				}
		
				String mode = reqParams.get("searchMode");
				if(mode.equals("S")){			
			...
			}else if(!reqParams.get("name").equals("")&&reqParams.get("mobileNumber").equals("")){
				sql = "SELECT PATIENT_NAME,SEX,TO_CHAR(DATE_OF_BIRTH,'DD/MM/YYYY') DATE_OF_BIRTH,PATIENT_ID,CONTACT1_NO FROM XH_PATIENT_EXT_INFO WHERE "
		                + "APPLICATION_ID = 'ACONNECT' AND UPPER(PATIENT_NAME) LIKE ? ";
				pstmt = dbConn.prepareCall(sql);
				...
				sql = "SELECT PATIENT_NAME,SEX,TO_CHAR(DATE_OF_BIRTH,'DD/MM/YYYY') DATE_OF_BIRTH,PATIENT_ID,CONTACT1_NO FROM XH_PATIENT_EXT_INFO WHERE "
		                + "APPLICATION_ID = 'ACONNECT' AND CONTACT1_NO=? ";
				pstmt = dbConn.prepareCall(sql);
			...
			response="[";
			while (rs.next()) {
				response+="{\"Name\":\""+rs.getString("PATIENT_NAME")+"\",\"Gender\":\""+rs.getString("SEX")+"\",\"DOB\":\""+rs.getString("DATE_OF_BIRTH")+"\",\"ABHA ID\":\""+rs.getString("PATIENT_ID")+"\",\"Mobile Number\":\""+rs.getString("CONTACT1_NO")+"\"},";
			}
			...
			response+="]";
			if(isDebugYN){
				System.out.println("(InvokeEGLapplication:searchScanedABHADetails) response " + response.toString());
			}

		} catch (Exception exp) {
			System.out.println("(InvokeEGLapplication:searchScanedABHADetails) Exception at executing query...");
			exp.printStackTrace(System.err);
		} finally {
			XHUtil.closeDBResources(rs, pstmt, dbConn);
		}
	...
	// Added by Kranthi for ML-MMOH-CRF-1844 on 18/10/2023 - This is used for
	// Single SignOn request
	private String processActiveDirRequest(Map<String, String> reqParams) throws Exception {
		...
		String request_type = XHDBAdapter.checkNull(reqParams.get("request_type"));
		String request_option = XHDBAdapter.checkNull(reqParams.get("request_option"));
		String username = XHDBAdapter.checkNull(reqParams.get("Username"));
		String password = XHDBAdapter.checkNull(reqParams.get("Password"));
		String responseTxt = "";
		String p_response = "";
		String errorCode = "";
		String errorMsg = "";
		...
		try {
			...
			ostmt = dbConn.prepareCall("{ CALL XHACTIVEDIR.GET_ACTIVEDIR_USER(?, ?, ?, ?, ?, ?, ?, ?) }");
			ostmt.setString(1, request_type);
			ostmt.setString(2, request_option);
			ostmt.setString(3, username);
			ostmt.setString(4, password);
			ostmt.registerOutParameter(5, java.sql.Types.VARCHAR);
			ostmt.registerOutParameter(6, java.sql.Types.CLOB);
			ostmt.registerOutParameter(7, java.sql.Types.VARCHAR);
			ostmt.registerOutParameter(8, java.sql.Types.VARCHAR);
			ostmt.execute();
			...
			if(isDebugYN){
				System.out.println("(InvokeEGLapplication:processSSORequest) p_response " + p_response);
				System.out.println("(InvokeEGLapplication:processSSORequest) errorCode " + errorCode);
				System.out.println("(InvokeEGLapplication:processSSORequest) errorMsg " + errorMsg);
				System.out.println("(InvokeEGLapplication:processSSORequest) responseTxt " + responseTxt);
			}

		} catch (Exception exp) {
			System.out.println("(InvokeEGLapplication:processSSORequest) Exception at executing procedure");
			exp.printStackTrace(System.err);
		} finally {
			XHUtil.closeDBResources(null, ostmt, dbConn);
		}
