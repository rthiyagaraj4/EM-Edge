 ...
 */
public class ElementIDXXXServlet extends HttpServlet
{
	/**
...
/*		Statement stmt = con.createStatement();
		...
		String query = "SELECT  ELEMENT_ID, ELEMENT_NAME,IN_USE_YN FROM XH_ELEMENT_CROSSREF where IN_USE_YN='Y' order by ELEMENT_NAME asc ";
		ResultSet rs = stmt.executeQuery(query);
		while(rs.next())
		{
			String elementID = 	rs.getString("ELEMENT_ID");
			System.out.println("SERV "+elementID);
			String validationType = req.getParameter(elementID+"_validationtype");
			if(validationType != null)
			{
				if( (("h").equalsIgnoreCase(validationType)))
				{
				System.out.println(validationType+" ---validation");
				}
				else if( (("s").equalsIgnoreCase(validationType)))
				{
				System.out.println(validationType+" ---validation");
				}
				else if( (("u").equalsIgnoreCase(validationType)))
				{
				System.out.println(validationType+" ---validation");
				}
			}
       ...
       XHApplicationDerivationSetupControllerBean xhBean=XHApplicationDerivationSetupControllerBean.getBean(XHApplicationElementControllerBean.strBeanName,req,session);
...
//		System.out.println(strData[i][0]+" "+strData[i][0]+" "+strData[i][1]+" "+strData[i][2]+" "+strData[i][3]+" "+strData[i][4]+" "+strData[i][5]+" "+strData[i][6]);
	    oraclecallablestatement.setString(1,"i");
		...
		oraclecallablestatement.setString(6,strData[i][4]);		 
		oraclecallablestatement.setInt(7,Integer.parseInt(strData[i][4]));
		oraclecallablestatement.setString(8,strData[i][4]);
	...
	//	System.out.println("Status XXX "+status);
        if("0".equals(status))
		{
                con.commit();
