...
public class RejectRequestBean extends StTransaction implements Serializable { 
...
public void setUser_id(String user_id) {
	...
	public String getRequestByStore() {
		String sql = eST.Common.StRepository.getStKeyValue("SQL_ST_CANCEL_REQUEST_REQ_BY_STORE_SELECT"); //MO-CRF-20174    
	...
	public String getRequestOnStore() {
		String sql = eST.Common.StRepository.getStKeyValue("SQL_ST_REJECT_REQUEST_REQ_BY_STORE_SELECT"); // MO-CRF-20174  
		java.util.ArrayList sqlParam = new java.util.ArrayList (1);
		sqlParam.add(getLoginFacilityId());
		sqlParam.add(getLanguageId());
		sqlParam.add(getLoginById()); //mo-crf-20174
		return getListValues(sql, sqlParam,"");
	}
...
*/
	...
	hmRemarks.put(key.toString(),new RejectRequestRemarksBean(doc_type_code, doc_no, (String)remarksData.get("remarks"),   (String)remarksData.get("remarks_code_1")));
	setCancelled_remarks( (String)remarksData.get("remarks") );
  ...
  //setCancelled_remarks( (String)remarksData.get("remarks_code_1") ); //Added by Rabbani #Inc no:33352 on 18/06/12 
	...
	public String getRemarks(String key) {
		if (hmRemarks.containsKey(key)) {
			RejectRequestRemarksBean rejectRequestRemarksBean = (RejectRequestRemarksBean) hmRemarks.get(key);  
			return rejectRequestRemarksBean.getRemarks(); 
		}  
		return "";
	}
	...
	public String getRemarks_code(String key) {  
		if (hmRemarks.containsKey(key)) {
			RejectRequestRemarksBean rejectRequestRemarksBean = (RejectRequestRemarksBean) hmRemarks.get(key);
			return rejectRequestRemarksBean.getRemarks_code();
		}
		return "";
	}
	...
	public void updateItemRemarks(java.util.Hashtable remarksData) {
	  ...
	  hmItemRemarks.put(key.toString(),new RejectRequestRemarksBean(doc_type_code, doc_no, (String)remarksData.get("remarks"),   (String)remarksData.get("remarks_code_1")));
	}
	public String getItemRemarks(String key) {
		if (hmItemRemarks.containsKey(key)){
			RejectRequestRemarksBean rejectRequestRemarksBean = (RejectRequestRemarksBean) hmItemRemarks.get(key);
			return rejectRequestRemarksBean.getDtlRemarks();
		}
		return ""; 
	}
	...
	public String getItemRemarks_code(String key) {
		if (hmItemRemarks.containsKey(key)) {
			RejectRequestRemarksBean rejectRequestRemarksBean = (RejectRequestRemarksBean) hmItemRemarks.get(key);
			return rejectRequestRemarksBean.getDtl_Remarks_code(); 
		}
		return "";
	}
	...
	public HashMap validate()
	{
		...
		RejectRequestRemarksBean remarksBean = new RejectRequestRemarksBean() ;
		...
		for (int i=0;i<alCheckedItems.size(); i++)
		{
			...
			{
				  remarksBean = (RejectRequestRemarksBean) hmRemarks.get(key);
				  remarks = remarksBean.getRemarks();     
                  remarks = remarks.substring(remarks.lastIndexOf("=")+1) ;

			}else{
         			hmRemarks.put(key,"Rejected as per request");
...
//---ends---
	...
	public void loadData(String facility_id,String doc_type_code, String doc_no,String item_code,String item_desc) {
        ...
        PreparedStatement pstmt = null;
        ResultSet resultSet = null;
		RejectRequestDetailBean rejectRequestDetailBean;
		...
		try {
         ...
         //   pstmt.setString(1, loginFacilityID);
	        ...
	        while (resultSet != null && resultSet.next()) {
	        	rejectRequestDetailBean = new RejectRequestDetailBean(resultSet);
				alItems.add(rejectRequestDetailBean);
			}	   
        } 
		catch (Exception e) {
            e.printStackTrace();
	...
	{
		System.out.println("modify-RejectRequestBean.java");  
		HashMap map = new HashMap() ;
		map.put( "result", new Boolean( false ) ) ;
		HashMap hmResult = new HashMap();
		hmResult.put( "result", new Boolean( false ) ) ;
		ArrayList aldocdtl  = new ArrayList();
		//HashMap hmTableData = new HashMap() ;  //COMMON-ICN-0118
		...
		PreparedStatement pstmt_update       = null;//MO-CRF-20174
		PreparedStatement pstmt_update_pending = null;  //MO-CRF-20174
		ArrayList<String> alParam4 = new ArrayList<String>();//MO-CRF-20174
		int result = 0; 
		int result2= 0; //MO-CRF-20174
		int result3= 0;//MO-CRF-20174
		try{ 
		connection = getConnection();
		pstmt_update = connection.prepareStatement("UPDATE st_request_hdr SET REJECTED_BY_ID = ? , request_status = ?, modified_by_id = ? ,	modified_date =	sysdate	, modified_at_ws_no	= ?	, modified_facility_id = ?, REJECTED_REMARKS_DESC = ?	WHERE facility_id =	?  AND doc_type_code = ?  AND doc_no = ?");//MO-CRF-20174
		pstmt_update_pending = connection.prepareStatement("UPDATE st_request_dtl SET pending_item_qty = pending_item_qty - NVL (?, 0) , modified_by_id = ? ,	modified_date =	sysdate	, modified_at_ws_no	= ?	, modified_facility_id = ? , REJECT_REMARKS = ?, REJECT_REMARKS_DESC = ?	WHERE item_code = ? AND doc_no = ? AND doc_type_code = ?"); //MO-CRF-20174
...
//			String item_code =  ""; //st.nextToken();//MO-CRF-20174  
			...
			if( ( hmRemarks.get(key) != null ) &&  (hmRemarks.get(key) != ""))
			{
			   // Rabbani # Bru-HIMS-CRF-118[INC NO:30077]
				...
				remarks     = "Rejected as per request";
				remarks_desc = getRemarks(key);
			  }
			ArrayList<String> alParam = new ArrayList<String>();
			ArrayList<String> alParam1 = new ArrayList<String>();//MO-CRF-20174 
//			ArrayList<String> alParam2 = new ArrayList<String>();
			int doc_count = 0;
			try{
			...
			pstmt_update.setString(1, login_by_id);  //MO-CRF-20174
			pstmt_update.setString(2, "R");  
			pstmt_update.setString(3, login_by_id);  
			pstmt_update.setString(4, login_at_ws_no);  
			pstmt_update.setString(5, login_facility_id);  
			pstmt_update.setString(6, checkForNull(remarks_desc)); 
			pstmt_update.setString(7, reqfacility_id);  
			pstmt_update.setString(8, docTypeCode);  
			pstmt_update.setString(9, ""+docNo);  
			alCheckedDocNumbers.add(""+docNo);	
			result = pstmt_update.executeUpdate(); 
			//String itemCd= "";//MO-CRF-20174
			if(result>0){
				try{
					alParam1.add(login_facility_id);
					alParam1.add(docTypeCode);  
					alParam1.add(""+docNo);    
					datadtl = getItemListDtl(reqfacility_id,docTypeCode,docNo); 	//MO-CRF-20174	//changed login facility id to reqfacility_id Added for MO-GN-5584
					System.out.println("datadtl: "+datadtl);   
					System.out.println("hmRemarks: "+hmRemarks); 
					System.out.println("hmItemRemarks: "+hmItemRemarks);  
										
					if(datadtl!=null){
						for(int k=0;i<datadtl.size();i++){
							String item_Code=(String) datadtl.get(i);   
							...
							while(resultSet != null && resultSet.next()) {
								...
								System.out.println("reqByStoreCode: "+reqByStoreCode+"--pendingItemQty: "+pendingItemQty+"--itemCode: "+itemCode);  
								//updating request_dtl remarks
							  ...
							  if( ( hmItemRemarks.get(key) != null ) &&  (hmItemRemarks.get(key) != ""))
							  {
								...
								//revert pending_qty in st_request_dtl
								pstmt_update_pending.setString(1, pendingItemQty);  //MO-CRF-20174 START
								pstmt_update_pending.setString(2,login_by_id);  
								pstmt_update_pending.setString(3, login_at_ws_no);  
								pstmt_update_pending.setString(4, login_facility_id);  
								pstmt_update_pending.setString(5,checkForNull(dtl_remarks_code)); // remarks code
								pstmt_update_pending.setString(6,checkForNull(dtl_remarks_desc)); // remarks desc  
								pstmt_update_pending.setString(7, itemCode);  
								pstmt_update_pending.setString(8, ""+docNo);  
								pstmt_update_pending.setString(9, ""+docTypeCode);  
								result2 = pstmt_update_pending.executeUpdate(); 
							}//MO-CRF-20174 END 
							 closeResultSet(resultSet);  //COMMON-ICN-0118
							closeStatement(pstmt);  //COMMON-ICN-0118
						}
		...
		System.out.println("alDtlCheckedItems: "+alDtlCheckedItems);
					 ...
					 String  key1 = doc_type_code_value+"-"+doc_No+"-"+item_code; 
						if( ( hmItemRemarks.get(key1) != null ) &&  (hmItemRemarks.get(key1) != ""))
						{
						 ...
						 ArrayList<String> alParamtr = new ArrayList<String>();
							...
							try{
								alParam4.add(login_facility_id);
								alParam4.add(doc_type_code_value);  
								alParam4.add(""+doc_No);    
								/*datadtl = getItemListDtl(login_facility_id,doc_type_code_value,doc_No); 	//MO-CRF-20174	
								System.out.println("datadtl: "+datadtl);   */
								System.out.println("hmRemarks: "+hmRemarks); 
								System.out.println("hmItemRemarks: "+hmItemRemarks);  
										...
										while(resultSet != null && resultSet.next()) {
											...
											System.out.println("reqByStoreCode: "+reqByStoreCode+"--pendingItemQty: "+pendingItemQty+"--itemCode: "+itemCode);  
											//revert pending_qty in st_request_dtl
											pstmt_update_pending.setString(1, pendingItemQty);  //MO-CRF-20174
											pstmt_update_pending.setString(2,login_by_id);  
											pstmt_update_pending.setString(3, login_at_ws_no);  
											pstmt_update_pending.setString(4, login_facility_id);  
											pstmt_update_pending.setString(5, itemCode);  
											pstmt_update_pending.setString(6, ""+doc_No);  
											pstmt_update_pending.setString(7, ""+doc_type_code_value);  
											result2 = pstmt_update_pending.executeUpdate(); 
										}//MO-CRF-20174*/
								pstmt = connection.prepareStatement(StRepository.getStKeyValue("SQL_ST_REVERT_DTL_SELECT_CANCEL_REQUEST"));//MO-CRF-20174
				                ...
				                while (resultSet != null && resultSet.next()) {
									String pendingItemQty = resultSet.getString("PENDING_ITEM_QTY");
				                    String itemcode = resultSet.getString("item_code"); 
				                    System.out.println("--pendingItemQty: "+pendingItemQty+"--itemCode: "+itemcode); 	
				                  //revert pending_qty in st_request_dtl
				                	//revert pending_qty in st_request_dtl
									pstmt_update_pending.setString(1, pendingItemQty);  //MO-CRF-20174
									pstmt_update_pending.setString(2,login_by_id);  
									...
									pstmt_update_pending.setString(6,checkForNull(dtl_remarks_desc)); // remarks desc   
									pstmt_update_pending.setString(7, itemcode);  
									pstmt_update_pending.setString(8, ""+doc_No);  
									pstmt_update_pending.setString(9, ""+doc_type_code_value);  
									result3 = pstmt_update_pending.executeUpdate(); 
							 }
							...
							connection.commit();//END 
	                         ...
	                         System.out.println("Before resultset close");
								closeResultSet(resultSet);
				                closeStatement(pstmt);
				                closeStatement(pstmt_update_pending); 
							 System.out.println("after resultset close");
		alLanguageData.add(getLanguageId());

		System.out.println("alModifyData: "+alModifyData);  
		tabData.put( "ModifyData",	alModifyData ) ;
		tabData.put( "properties",	getProperties() ) ;
		tabData.put("LanguageData",alLanguageData); 
	...
	public java.util.HashMap modify1() {
...
//	    SingleTableHandlerHome home = null;
	...
	//MO-CRF-20174  
	/*public java.util.ArrayList getItemListDtl(String login_facility_id,String docTypeCode,int doc_no) throws Exception { 
		String chkParameters[] = {login_facility_id,docTypeCode,""+doc_no}; 
		ArrayList result = new ArrayList();
		//result = fetchRecords(eST.Common.StRepository.getStKeyValue("SQL_MM_ITEM_CLASS_FOR_STORE_SELECT_LIST"),chkParameters);
		result = fetchRecords("SELECT ITEM_CODE  FROM ST_REQUEST_DTL WHERE FACILITY_ID = ? AND DOC_TYPE_CODE = ? AND DOC_NO = ? ",chkParameters);
		System.out.println("getItemListDtl - result: "+result);    
		return result;
}	*/  
	//MO-CRF-20174  
	public ArrayList getItemListDtl(String login_facility_id,String docTypeCode,int doc_no) throws Exception{ 
		//String itemCode = ""; 
...
//		HashMap hmRecord					=	new HashMap();  
		...
		ResultSet resultSet = null;
		String sql = "";
		sql   = "SELECT ITEM_CODE  FROM ST_REQUEST_DTL WHERE FACILITY_ID = ? AND DOC_TYPE_CODE = ? AND DOC_NO = ? AND PENDING_ITEM_QTY > 0";
		
	try {
		conn = getConnection();
		pstm = conn.prepareStatement(sql);
		pstm.setString(1, login_facility_id);
		pstm.setString(2, docTypeCode);
		pstm.setString(3, ""+doc_no);
		resultSet = pstm.executeQuery(); 
		while(resultSet!=null && resultSet.next()){//MO-CRF-20174
			arrItemList.add(resultSet.getString("ITEM_CODE")==null?"":resultSet.getString("ITEM_CODE")); 
		}
...
System.out.println("arrItemList: "+arrItemList);  
return arrItemList ;   
}
