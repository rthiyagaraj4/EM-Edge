...
public class CurrentAndHistoryDAOImpl extends GenericDAOImpl implements CurrentAndHistoryDAO {
	public CurrentAndHistoryResponse getDiscreteValues(VitalsRequest oVitalsReq)
	{
		CurrentAndHistoryResponse oCurrentAndHistoryResponse = new CurrentAndHistoryResponse();
		...
		List<CurrentAndHistoryDM> lstCurrentAndHistoryDM = new ArrayList<CurrentAndHistoryDM>();
		...
		if(oVitalsReq != null && oVitalsReq.getChartId() != null && !oVitalsReq.getChartId().equals("") )
		{
			...
			if(oPatContext != null)
			{
				ArrayList<DiscreteValue> discreteValueList;
				Map<CurrentAndHistoryDM,ArrayList<DiscreteValue>> discreteMeasureMap = new LinkedHashMap<CurrentAndHistoryDM, ArrayList<DiscreteValue>>();
			...
			//	sqlStr.append("  AND a.event_code IN ( SELECT a.discr_msr_id discr_msr_id  FROM ca_chart_section_comp a, am_discr_msr discr_msr, am_discr_msr_ref_rng_num b  WHERE a.chart_id = '" + chartId+"'  AND discr_msr.discr_msr_id = a.discr_msr_id AND discr_msr.ch_appl_yn = 'Y'  AND a.discr_msr_id = b.discr_msr_id(+)))  WHERE rnum < 5  order by dummydate desc ");
				
				if(oVitalsReq.getFilterByEventCode() == null || oVitalsReq.getFilterByEventCode().equals(""))
				{
					sqlStr.append("  AND a.event_code IN ( SELECT a.discr_msr_id discr_msr_id  FROM ca_chart_section_comp a, am_discr_msr discr_msr, am_discr_msr_ref_rng_num b  WHERE a.chart_id = '" + chartId+"'  AND discr_msr.discr_msr_id = a.discr_msr_id AND discr_msr.ch_appl_yn = 'Y'  AND a.discr_msr_id = b.discr_msr_id(+)) ");
				}
				else
				{
					if(oVitalsReq.getFilterByEventCode().indexOf(",") == -1)
					{
						sqlStr.append(" AND a.event_code = '" + oVitalsReq.getFilterByEventCode() + "' ");
					}
					else
					{
						sqlStr.append(" AND a.event_code in ( " );
				...
				sqlStr.append( " ) ");
				if(oVitalsReq.isRankRequired())
				{
					sqlStr.append(" WHERE rnum < 5 ");
				}
				//sqlStr.append(" order by dummydate desc ");
				//sqlStr.append( " ORDER BY event_code,rankk ");
				sqlStr.append(" order by dummydate desc,rankk ");
				try
				{
					...
					if(resultSet != null)
					{
						...
						{
							CurrentAndHistoryDM oCurrAndHistDM = new CurrentAndHistoryDM();
							DiscreteValue oDiscreteValue = new DiscreteValue();
							
							oCurrAndHistDM.setEventCode(checkForNull(resultSet.getString("event_code")));
							oCurrAndHistDM.setEventDescription(checkForNull(resultSet.getString("event_desc")));
							oCurrAndHistDM.setNormalHigh(checkForNull(resultSet.getString("normal_high")));
							oCurrAndHistDM.setNormalLow(checkForNull(resultSet.getString("normal_low")));
							oCurrAndHistDM.setCriticalLow(checkForNull(resultSet.getString("critical_low")));
							oCurrAndHistDM.setCriticalHigh(checkForNull(resultSet.getString("critical_high")));
							oCurrAndHistDM.setUOM(checkForNull(resultSet.getString("uom")));
							oCurrAndHistDM.setHistRecType(checkForNull(resultSet.getString("hist_rec_type")));
							oCurrAndHistDM.setEventGroupDesc(checkForNull(resultSet.getString("event_group_desc")));
							
							oDiscreteValue.setResultNum(checkForNull(resultSet.getString("result_num")));
							...
							oDiscreteValue.setRank(checkForNull(resultSet.getString("rankk")));
							if(oDiscreteValue.getRank().equals("1"))
							{
								oCurrAndHistDM.setCurrentDiscreteValue(oDiscreteValue);
							}
							if(discreteMeasureMap.containsKey(oCurrAndHistDM))
							{
								//discreteMeasureMap.get(oCurrAndHistDM).add(oDiscreteValue);
								//if(!oDiscreteValue.getRank().equals("1"))
								//{
									discreteMeasureMap.get(oCurrAndHistDM).add(oDiscreteValue);
								//}
							...
							{
								discreteValueList = new ArrayList<DiscreteValue>();
								//if(!oDiscreteValue.getRank().equals("1"))
								//{
									discreteValueList.add(oDiscreteValue);
									////discreteMeasureMap.put(oCurrAndHistDM, discreteValueList);
								//}
								discreteMeasureMap.put(oCurrAndHistDM, discreteValueList);
							}
						...
						Iterator<Entry<CurrentAndHistoryDM,ArrayList<DiscreteValue>>> iter = discreteMeasureMap.entrySet().iterator();
						if(iter != null)
						{
							while (iter.hasNext()) {
								Map.Entry<CurrentAndHistoryDM,ArrayList<DiscreteValue>> mEntry = (Map.Entry<CurrentAndHistoryDM,ArrayList<DiscreteValue>>)iter.next();
								CurrentAndHistoryDM oCurrentAndHistoryDMEntry = (CurrentAndHistoryDM)mEntry.getKey();
								ArrayList<DiscreteValue> discreteValueListEntry = (ArrayList<DiscreteValue>)mEntry.getValue();
							//	oCurrentAndHistoryDMEntry.setLstHistoryValues(discreteValueListEntry);
								oCurrentAndHistoryDMEntry.setDiscreteValuesList(discreteValueListEntry);
								lstCurrentAndHistoryDM.add(oCurrentAndHistoryDMEntry);
					...
					oCurrentAndHistoryResponse.setLstCurrentAndHistoryDM(lstCurrentAndHistoryDM);
				}
				catch(Exception e)
				{
					e.printStackTrace();
		...
		oCurrentAndHistoryResponse.getErrorsList().add(oErrInfo);
		return oCurrentAndHistoryResponse;
	}
