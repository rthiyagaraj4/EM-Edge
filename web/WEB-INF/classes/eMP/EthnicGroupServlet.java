...
public class EthnicGroupServlet extends HttpServlet
{

    public EthnicGroupServlet()
    {
        ...
        throws ServletException, IOException
    {
        ...
        try
        {
            ...
            if(mode.equals("insert"))
                insertEthnicGroup(httpservletrequest, httpservletresponse);
            if(mode.equals("modify"))
                modifyEthnicGroup(httpservletrequest, httpservletresponse);
        }
        catch(Exception exception)
        {
            // out.println(exception.toString());
        	exception.printStackTrace(); // Added by lakshmanan for security issue ID 174094330 on 05-09-2023
        }
    }
        ...
        throws ServletException, IOException
    {
        try
        {
            ...
            out.println("<html><head><link rel='stylesheet' type ='text/css' href='../eCommon/html/IeStyle.css'></link><script language='javascript' src='../eMP/js/EthnicGroup.js'></script></head><frameset rows='42,*,50'><frame name='commontoolbarFrame'\tsrc='" + s2 + "' frameborder=0 scrolling='no' noresize><frame name='f_query_add_mod' src='../eCommon/html/blank.html' frameborder=0><frame name='messageFrame' src='../eCommon/jsp/MstCodeError.jsp' frameborder=0 noresize scrolling='auto'></frameset></html> ");
        }
        catch(Exception exception) { }
    }

    private void modifyEthnicGroup(HttpServletRequest httpservletrequest, HttpServletResponse httpservletresponse)
    {
        try
        {
            occupationClassCode = httpservletrequest.getParameter("ETHNIC_GROUP_CODE");
            ...
            tabData.put("modified_facility_id", s4);
            tabData.put("modified_at_ws_no", s6);

            HashMap hashtable1 = new HashMap();
            hashtable1.put("RACE_CODE", occupationClassCode);
        ...
        catch(Exception exception)
        {
           ...
           // out.println(occupationClassCode + " " + longDesc + " " + shortDesc + " " + effectiveDateFrom + " " + effectiveDateTo + " " + effectiveStatus);
        	exception.printStackTrace(); // Added by lakshmanan for security issue ID 169541308 on 05-09-2023
        }
    }

    private void insertEthnicGroup(HttpServletRequest httpservletrequest, HttpServletResponse httpservletresponse)
    {
        try
        {
            occupationClassCode = httpservletrequest.getParameter("ETHNIC_GROUP_CODE");
			...
			HashMap tabData = new HashMap();
            tabData.put("RACE_CODE", occupationClassCode);
            tabData.put("LONG_DESC", longDesc);
            tabData.put("SHORT_DESC", shortDesc);
            tabData.put("EFF_STATUS", effectiveStatus);
        ...
        catch(Exception exception)
        {
        	...
        	exception.printStackTrace(); // Added by lakshmanan for security issue ID 169534068 on 05-09-2023
        }
    }
