...
public class SectionServlet extends javax.servlet.http.HttpServlet
{
	...
	private void modifyService(HttpServletRequest req, HttpServletResponse res)
	{
		try
		{
			...
			final java.util.Hashtable results = singleTabHandlerManagerRemote.update(p,tabdata,condflds,"am_dept_section");*/
			...
			String table_name="am_dept_section";
		...
		catch ( Exception e )
		{
			...
			out.println( sectioncode+ " "+longdesc + " "+shortdesc + " "+ effectiveStatus );
		}
	}
	...
	private void insertService(HttpServletRequest req, HttpServletResponse res)
	{
		try
		{
			...
			String dupflds[]={"section_code","dept_code"};
			...
			final SingleTabHandlerManagerHome singleTabHandlerManagerHome  = (SingleTabHandlerManagerHome) PortableRemoteObject.narrow(homeObject ,SingleTabHandlerManagerHome.class);
			final SingleTabHandlerManagerRemote singleTabHandlerManagerRemote = singleTabHandlerManagerHome.create();
			final java.util.Hashtable results = singleTabHandlerManagerRemote.insert(p,tabdata,dupflds,"am_dept_section");*/
		...
		catch ( Exception e )
		{
			out.println("<h3> Exception raise by Servlet ...So Record Not Modified </h3> Foll: Error Received : "+e.toString());
			out.println( "Values sent are : <br>" ) ;
			out.println( deptcode+ " "+sectioncode+ " "+longdesc + " "+shortdesc +" "+effectiveStatus );
		}
	}
