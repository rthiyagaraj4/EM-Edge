 ...
 */
public class PatientTypeServlet extends HttpServlet{
	PrintWriter out;
	...
	public synchronized void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException,IOException{
		...
		try	{
			...
			if ( operation.equals("insert") )   insertPatientType(req, res);
			if ( operation.equals("modify"))    modifyPatientType(req, res);
		}catch(Exception e){
	...
	private void modifyPatientType(HttpServletRequest req, HttpServletResponse res){
		try{
			...
			condflds.put("patient_type",patienttype);
...
/*
			...
			final java.util.HashMap results = singleTabHandlerManagerRemote.update(p,tabdata,condflds,"am_patient_type");
*/
		...
		/*	Object argArray[] = new Object[1];			
			...
			hmParams.put("table_name","am_patient_type");
			argArray[0] = hmParams;
			...
			paramArray[0] = hmParams.getClass();*/
				...
				//argArray[0] =hashmap;
				String table_name="am_patient_type";
				argArray[0] =p;
				argArray[1] =tabdata;
				argArray[2] =condflds;
		...
		//	hmParams.clear();
			...
			out.println( patienttype+ " "+longDesc + " "+shortDesc + " "+ effectiveStatus );
			e.printStackTrace();
		}
	}

	private void insertPatientType(HttpServletRequest req, HttpServletResponse res){
		try	{
			patienttype=req.getParameter("patient_type");
			longDesc = req.getParameter("long_desc");
			shortDesc = req.getParameter("short_desc");
			...
			HashMap tabdata=new HashMap();
			tabdata.put("patient_type",patienttype);
			tabdata.put("long_desc",longDesc);
			tabdata.put("short_desc",shortDesc);
			tabdata.put("eff_status",effectiveStatus);
...
/*
			...
			final java.util.HashMap results = singleTabHandlerManagerRemote.insert(p,tabdata,dupflds,"am_PATIENT_type");
*/
			boolean local_ejbs = false;
			ServletContext context = getServletConfig().getServletContext();
			if ((context.getInitParameter("LOCAL_EJBS") !=null) && (context.getInitParameter("LOCAL_EJBS").equalsIgnoreCase("TRUE"))) 
