...
public class GenderGroupServlet extends HttpServlet{
	PrintWriter out;
	Properties p;
	String agegroupcode;
	String longDesc ;
	...
	public synchronized void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException,IOException{
		...
		try{
			...
			if ( operation.equals("insert") )   insertGender(req, res);
			if ( operation.equals("modify"))    modifyGender(req, res);
		}catch(Exception e){
	...
	private void modifyGender(HttpServletRequest req, HttpServletResponse res){
		try{	
			agegroupcode=req.getParameter("gender_group_code");
			longDesc = req.getParameter("long_desc");
			shortDesc = req.getParameter("short_desc");
			gender=req.getParameter("gender");
			minage=Integer.parseInt(req.getParameter("min_age").trim());
			maxage=Integer.parseInt(req.getParameter("max_age").trim());
			agerangeunit=req.getParameter("age_unit");
				
			agerangeunit1=req.getParameter("age_unit1");
			if(agerangeunit1==null) agerangeunit1 = "";
			...
			tabdata.put("min_age",new Integer(minage));
			tabdata.put("max_age",new Integer(maxage));
			tabdata.put("age_unit",agerangeunit);
			tabdata.put("AGE_UNIT_MAX",agerangeunit1);
			...
			condflds.put("age_group_code",agegroupcode);
...
/*			
			...
			final java.util.HashMap results = singleTabHandlerManagerRemote.update(p,tabdata,condflds,"am_age_group");
*/
		...
		/*	Object argArray[] = new Object[1];			
			...
			hmParams.put("table_name","am_age_group");
			argArray[0] = hmParams;
			...
			paramArray[0] = hmParams.getClass();*/
				...
				//argArray[0] =hashmap;
				String table_name="am_age_group";
				argArray[0] =p;
				argArray[1] =tabdata;
				argArray[2] =condflds;
...
//			hmParams.clear();
			...
			out.println( agegroupcode+ " "+longDesc + " "+shortDesc + " "+ effectiveStatus );
			e.printStackTrace();
		}
	}

	private void insertGender(HttpServletRequest req, HttpServletResponse res){
		try{			
			agegroupcode=req.getParameter("gender_group_code");
			longDesc = req.getParameter("long_desc");
			shortDesc = req.getParameter("short_desc");
			gender=req.getParameter("gender");
			minage=Integer.parseInt(req.getParameter("min_age"));
			maxage=Integer.parseInt(req.getParameter("max_age"));
			agerangeunit=req.getParameter("age_unit");
		...
		//	tabdata.put("facility_id",facilityId);
			tabdata.put("age_group_code",agegroupcode);
			tabdata.put("long_desc",longDesc);
			tabdata.put("short_desc",shortDesc);
			tabdata.put("gender",gender);
			tabdata.put("min_age",new Integer(minage));
			...
			//String dupflds[]={"facility_id","age_group_code"};
			String dupflds[]={"age_group_code"};
...
/*			InitialContext context = new InitialContext();
			...
			final java.util.HashMap results = singleTabHandlerManagerRemote.insert(p,tabdata,dupflds,"am_age_group");
*/
			boolean local_ejbs = false;
			ServletContext context = getServletConfig().getServletContext();
			if ((context.getInitParameter("LOCAL_EJBS") !=null) && (context.getInitParameter("LOCAL_EJBS").equalsIgnoreCase("TRUE"))) 
