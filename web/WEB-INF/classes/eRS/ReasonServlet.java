...
public class ReasonServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws javax.servlet.ServletException,IOException{
		...
		try{
			...
			reason_code=req.getParameter("reason_code")==null ? "":req.getParameter("reason_code");
			reason_desc=req.getParameter("reason_desc")==null ? "":req.getParameter("reason_desc");
			activity_type_code=req.getParameter("activity_type_code")==null ? "":req.getParameter("activity_type_code");
	...
	public void insertOperation(HttpServletResponse res)throws Exception{
		try{
			...
			tabData.put("activity_type_code",activity_type_code);
			...
			/*	InitialContext context = new InitialContext();
				...
				final java.util.HashMap results = singleTabHandlerManagerRemote.insert(p,tabData,dupflds,"rs_reason");
				*/
	...
	public void modifyOperation(HttpServletRequest req,HttpServletResponse res)	throws Exception{
		...
		try{
			...
			activity_type_code1=req.getParameter("activity_type_code1")==null ? "":req.getParameter("activity_type_code1");
			HashMap tabData=new HashMap();

			tabData.put("reason_desc",reason_desc);
			tabData.put("activity_type_code",activity_type_code1);
			tabData.put("eff_status",eff_status);
			tabData.put("MODIFIED_BY_ID", added_by_id);
			tabData.put("MODIFIED_DATE", addeddate);
			tabData.put("MODIFIED_AT_WS_NO", client_ip_address);
			...
			condflds.put("reason_code",reason_code);
			...
			/*	InitialContext context = new InitialContext();
				...
				final SingleTabHandlerManagerHome singleTabHandlerManagerHome  = (SingleTabHandlerManagerHome) PortableRemoteObject.narrow(homeObject ,SingleTabHandlerManagerHome.class);
				final SingleTabHandlerManagerRemote singleTabHandlerManagerRemote = singleTabHandlerManagerHome.create();
				final java.util.HashMap results = singleTabHandlerManagerRemote.update(p,tabData,condflds,"rs_reason");
			*/
				boolean local_ejbs = false;
				ServletContext context = getServletConfig().getServletContext();
				if ((context.getInitParameter("LOCAL_EJBS") !=null) && (context.getInitParameter("LOCAL_EJBS").equalsIgnoreCase("TRUE"))) 
