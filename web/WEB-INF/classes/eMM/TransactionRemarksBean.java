...
public class TransactionRemarksBean extends MmAdapter implements Serializable
{	protected String trn_remarks_code			= "";
	...
	public TransactionRemarksBean()throws Exception
	{ 
		try
		{
			doCommon();
...
//********* * Set Methods Start **************************
	public void setMasterCode (String trn_remarks_code)
	{
		this.trn_remarks_code = trn_remarks_code;
	}
...
//*************************************************************************
	...
	{	
		...
		for(int i=0; i<10;i++)
		{
			String trn_remark_code = "trn_remarks_code"  + i ;
			String remarks_desc = "remarks_desc"  + i ;
			String module_id = "module_id"  + i ;
			String trn_type = "trn_type"  + i ;
			ArrayList insertRow = new ArrayList() ;
			ArrayList whereRow = new ArrayList() ;
			if(!
				  (  (((String)hashMultiple.get(trn_remark_code))).trim().equals("")
										||
					(((String)hashMultiple.get(remarks_desc))).trim().equals("")
										||
					(((String)hashMultiple.get(module_id))).trim().equals("")
										||
					(((String)hashMultiple.get(trn_type))).trim().equals("")
				  )
			  )
			{
				/***** 4 values per row from the AddModify form  */
				insertRow.add( (String)hashMultiple.get(  ("module_id" + i)         ) ) ;
				insertRow.add( (String)hashMultiple.get(  ("trn_type" + i)         ) ) ;
				insertRow.add( (String)hashMultiple.get(  ("trn_remarks_code" + i)    )  ) ;
				insertRow.add( (String)hashMultiple.get(  ("remarks_desc" + i)         ) ) ;
				...
				whereRow.add( (String)hashMultiple.get(  ("trn_remarks_code" + i) )  ) ;				
				whereRow.add( (String)hashMultiple.get(  ("trn_type" + i) )  ) ;
				whereRow.add( (String)hashMultiple.get(  ("module_id" + i) )  ) ;
		...
		try
		{
			sqlMap.put( "SelectSQL", MmRepository.getMmKeyValue( "SQL_MM_TRN_REMARKS_CODE_EXISTS") );
			sqlMap.put( "InsertSQL", MmRepository.getMmKeyValue( "SQL_MM_TRN_REMARKS_INSERT") );
...
//			System.out.println("Map = "+map); 
	...
	public HashMap modify()
	{
		...
		ArrayList LanguageData = new ArrayList() ;
		ModifyData.add((String)hashMultiple.get(  "remarks_desc"  ));
		...
		ModifyData.add(trn_remarks_code);
		LanguageData.add(getLanguageId());
	...
	//	SingleTableHandlerHome home = null ;
		...
		try
			{
				sqlMap.put( "ModifySQL", MmRepository.getMmKeyValue( "SQL_MM_TRN_REMARKS_UPDATE") );
...
public void loadData() throws Exception {
		...
		try {
			...
			pstmt = connection.prepareStatement(MmRepository.getMmKeyValue( "SQL_MM_TRN_REMARKS_MODIFY"));
			pstmt.setString( 1, trn_remarks_code);
			pstmt.setString( 2, getLanguageId());
			resultSet = pstmt.executeQuery();
			if ((resultSet != null) && (resultSet.next())) {		
				setMasterCode(resultSet.getString("trn_remarks_code"));
				setRemarks(resultSet.getString("Remarks_desc"));
				setEffStatus(resultSet.getString("eff_status"));			
				setModuleID(resultSet.getString("module_id"));			
				setTRNType(resultSet.getString("trn_type"));			
