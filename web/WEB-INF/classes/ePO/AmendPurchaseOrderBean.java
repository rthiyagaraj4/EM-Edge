...
public class AmendPurchaseOrderBean extends ePO.Common.PoTransaction implements Serializable{
	...
	private ArrayList approveItemKeysAdded;
	...
	public void setAprvl_ref_no(String aprvl_ref_no) {
		 this.aprvl_ref_no = aprvl_ref_no;
	}
	public String getAprvl_ref_no() {
		 return aprvl_ref_no;
	}
	...
	public void setRevn_no(String revn_no) {
		 this.revn_no = revn_no;
	}
	
	public String getRevn_no() {
		 return revn_no;
	}
	...
	public String setSupp_address_2()
	{
	...
	public String getPurchase_type_List() {
		   ...
		   return getListOptionTag(getListOptionArrayList(getPoRepositoryValue("SQL_PO_PURCHASE_TYPE_SELECT"),arr_language),""); 
    }
	...
	public String getRequestFromPurchaseUnit() {
		...
		return getListOptionTag(getListOptionArrayList(getPoRepositoryValue("SQL_PO_PURCHASE_UNIT_LIST_FOR_PUR_AMEND"),sqlParam),""); 
	...
	public String getRequestFromPurchaseUnit(String pur_unit) {
		...
		sqlParam.add(getLoginById());
		sqlParam.add(getLoginFacilityId());
		return getListOptionTag(getListOptionArrayList(getPoRepositoryValue("SQL_PO_PURCHASE_UNIT_LIST_FOR_PUR_AMEND"),sqlParam),pur_unit); 
	...
	public String getRequestToSupp() {
		...
		ArrayList alParameters = new ArrayList();
		alParameters.add("en");
		return getListOptionTag(getListOptionArrayList(getPoRepositoryValue("SQL_PO_SUPPLIER_LIST"),alParameters),""); 
...
//	public HashMap getAcctDesc(String acc_no ,String commit_code,String dept_id){
	...
	public AmendPurchaseOrderDetailBean getItemDetail(String item_code) {
		return (AmendPurchaseOrderDetailBean) hmItemDetail.get(item_code);
	}
	...
	public java.util.HashMap addDetailRecord(java.util.Hashtable detailRecord) {
		...
		hmItemDetail.put(item_code, new AmendPurchaseOrderDetailBean(detailRecord));
		...
		try {
			...
			if(itemKeysAdded.contains(item_code)){
			approveItemKeysAdded.add(item_code);
			}else{
...
public java.util.HashMap insert() {
		...
		AmendPurchaseOrderDetailBean requestDetailBean;
		hmResult.put("result", new Boolean(false));
		ArrayList final_dtl=new ArrayList();
		ArrayList delivery_dtl=new ArrayList();
		...
		alHdrInsertRecord.add(getFinalize_yn().equals("Yes")?appvl_yn:"NE");//PO_STATUS
			...
			for (int i=0;i<itemKeys.size();i++) {
					 ...
					 item_code = (String)itemKeys.get(i);
						...
						if(hmExpRecords.containsKey(item_code)){
							...
							requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(item_code);
					...
					int detailSize = checkedItemKeys.size();
						...
						for (int index=0;index<detailSize;index++) {
							...
							requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(checkedItemKeys.get(index));
	...
	//end for Bill delivery_schedule
		...
		hmTableData.put("InsertBILLDELVData",alInsertBillDeliveryRecord);
		hmTableData.put("UpdateData",alUpdateData);
		hmTableData.put("LANGUAGE_DATA",	allanguageData);
		hmTableData.put("FINAL_DTL",final_dtl);
		hmTableData.put("DELIVERY_DTL",delivery_dtl);
			
		AmendPurchaseOrderRemote requestRemote = null;

		try {
			Object home = com.ehis.eslp.ServiceLocator.getInstance().getHome("java:comp/env/AmendPurchaseOrder", AmendPurchaseOrderHome.class,getLocalEJB());
...
//load Bill to Delivery to data
	...
	public void loadData(String purchase_type_code, String po_no) {
		...
		//load PO hdr data	
            ...
            while (resultSet != null && resultSet.next()) {
				...
				setRevn_no(resultSet.getString("PO_REVN_NO"));
			...
			String item_code = "";
            while (resultSet != null && resultSet.next()) {
				requestDetailBean = new AmendPurchaseOrderDetailBean(resultSet);
	...
	AmendPurchaseOrderRemote requestRemote = null;
		...
		AmendPurchaseOrderDetailBean requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(key);
		...
		if (itemKeysAdded != null && itemKeysAdded.size()>0) {
			...
			if(checkedItemKeys.contains(itemKeysAdded.get(index))){
				requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(itemKeysAdded.get(index));
	...
	//	if (itemKeysAdded != null && itemKeysAdded.size()>0) {
		if (checkedItemKeys != null && checkedItemKeys.size()>0) {
			...
			*/
			...
			hmSQLMap.put("UpdateCOMMTSQL", ePO.Common.PoRepository.getPoKeyValue("SQL_PO_COMMT_CODE_UPDATE_FINALIZE_YES_AMEND_INSERT"));
...
// Update Blocked Budget Amount in  PO_COMMTTMENT_ACCOUNT
	...
	if(getFinalize_yn().equals("Yes")){
		...
		if(getFinalize_yn().equals("Yes") && getPo_mode().equals("R")){
		
			// Update Request Detail 
			req_dtl=(ArrayList)newReqDetPOAQtyUpdate();
			System.out.println("req_dtl in po bean modify = "+req_dtl);
			hmSQLMap.put("UpdateREQDTLSQL", ePO.Common.PoRepository.getPoKeyValue("SQL_PO_REQ_DTL_PO_QTY_AMEND_UPDATE"));
			hmTableData.put("UpdateREQDTLData",req_dtl);
			
			// Update Request Header
			req_hdr=(ArrayList)newReqHdrUpdate();
			System.out.println("req_hdr in po bean modify = "+req_hdr);
	...
	/*
		...
		// Update Request Detail po_blocked_qty	
			...
			System.out.println("req_dtl in po bean insert = "+req_dtl);
	...
	//get Approval YN	
		...
		if (allItemKeys != null && allItemKeys.size() > 0) {
			...
			for (int index=0;index<detailSize;index++) {
				...
				requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(allItemKeys.get(index));
		...
		for (int index=0;index<detailSize;index++) {
			...
			alUpdateDtlRecord.add(checkForNull(requestDetailBean.getEff_from_dt())); 
		...
		HashMap hashRevNo=new HashMap();
		String rev_no="";
		
		
		try{
			ArrayList chkParameters=new ArrayList();
			chkParameters.add(getDoc_no());
			hashRevNo		=	fetchRecord(getPoRepositoryValue("SQL_ST_PO_ORDER_REVN_NO"),chkParameters);
			rev_no=(String)hashRevNo.get("PO_REVN_NO");
		}catch(Exception e){
			e.printStackTrace();
		}
		
		alModifyData.add(""+(rev_no));//PO_REVN_NO	
		alModifyData.add(getDoc_date_value());
		alModifyData.add(getDoc_ref());
		alModifyData.add(getFinalize_yn().equals("Yes")?"RA":"RA");
						...
						if(hmExpRecords.containsKey(item_code)){
								if(sourceArrayList.size()>0){
								  ...
								  for (int j=0;j<sourceArrayList.size();j++) {
									...
									requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(hmExpRecord.get("ITEM_CODE"));		
									ArrayList alInsertDeliveryRecord        =       new ArrayList();
									alInsertDeliveryRecord.add(getDoc_no()); // PO_NO
									alInsertDeliveryRecord.add(hmExpRecord.get("ITEM_CODE")); // ITEM_CODE
...
public ArrayList newDevyInsert() {
		
		AmendPurchaseOrderDetailBean requestDetailBean;
		int detailSize = checkedItemKeys.size();
		ArrayList alDevyInsertRecord = new ArrayList(detailSize);
	try{
			for (int index=0;index<detailSize;index++) {
		if((checkedItemKeys.get(index) == null) || (checkedItemKeys.get(index).equals(""))) 
		continue;
		requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(checkedItemKeys.get(index));
			...
			alDevyRecord.add(requestDetailBean.getItem_code());//ITEM_CODE
			alDevyRecord.add(""+(index+1));//PO_SRL_NO
			alDevyRecord.add(""+(index+1));//DELV_SRL_NO
			alDevyRecord.add(requestDetailBean.getOrder_qty());//DELV_QTY
			alDevyRecord.add(requestDetailBean.getDel_date());//DELV_DATE
			alDevyRecord.add("ND");//DELV_STATUS
			alDevyRecord.add(login_by_id);
			...
			alDevyRecord.add(login_at_ws_no);
			alDevyRecord.add(login_facility_id);
			alDevyInsertRecord.add(alDevyRecord);		
...
public ArrayList newInsert() {
	...
	try{
		...
		requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(checkedItemKeys.get(index));
			...
			if(getPo_mode().equals("D")){	
				alDtlRecord.add(" ");// REQ_FACILITY_ID
				alDtlRecord.add(" ");// REQ_STORE_CODE
			}else if(getPo_mode().equals("R")){	
			...
			//alDtlRecord.add(requestDetailBean.getOrder_qty());//REQ_GEN_UOM_QTY
			alDtlRecord.add(requestDetailBean.getUnit_price());//UNIT_PRICE
...
public void updateDeliveryRecords(String item_code, Hashtable htExpRecords){
	...
	public java.util.HashMap addDetailRecordNew(java.util.HashMap detailRecord) {
		...
		hmItemDetail.put(itemcode_reqno, new AmendPurchaseOrderDetailBean(detailRecord));
		hashResult.put("result",new Boolean(true));
		hashResult.put("message","");
		return hashResult;
	}
	...
	public ArrayList checkApprovedYN() {
		...
		AmendPurchaseOrderDetailBean requestDetailBean;
		int detailSize = approveItemKeysAdded.size();
		ArrayList alAppYNRecord = new ArrayList(detailSize);
		try{
		
		for (int index=0;index<detailSize;index++) {
		if((approveItemKeysAdded.get(index) == null) || (approveItemKeysAdded.get(index).equals(""))) 
		continue;
		requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(approveItemKeysAdded.get(index));
			ArrayList alAppRecord = new ArrayList();
...
public HashMap newOverride() {
	...
	try{
		...
		continue;
		requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(dummyItemKeys.get(index));
			String amount = "";			
				Double amt;	
...
public ArrayList newReqDetPOAQtyUpdate() {
	...
	//	java.util.Collections.reverse(checkedItemKeys);
		...
		requestDetailBean = (AmendPurchaseOrderDetailBean) hmItemDetail.get(checkedItemKeys.get(index));
			...
			String prev_po_qty = checkForNull((String)fetchRecord(getPoRepositoryValue("SQL_PO_REQ_DTL_SELECT_PO_QTY"),alBlockedRecord).get("PO_QTY"),"0");
			alReqDtlRecord.add(prev_po_qty);//PO_QTY
