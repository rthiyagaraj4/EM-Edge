...
public class STPORequestStatusBean extends ePO.Common.PoTransaction implements Serializable {
...
//31/05/12
	public String getPODocTypeCodes(){
		return getPODefaultDocumentTypeList("PORQ");	
	}
	private String getPODefaultDocumentTypeList(String trn_type){
		...
		alParameters.add(trn_type);
		alParameters.add(getLanguageId());
		return getListOptionTag(getListOptionArrayList(getPoRepositoryValue("SQL_PO_TRN_DOC_TYPE_SELECT_LIST"),alParameters));
	}
    //ends
	...
	public String getRequestByStore() {
		String sql = ePO.Common.PoRepository.getPoKeyValue("SQL_PO_STORE_SELECT2");
...
public java.util.ArrayList getOnDepartmentRequestStatus(String req_by_store) throws Exception {
		...
		result = fetchRecords(ePO.Common.PoRepository.getPoKeyValue("SQL_PO_AM_DEPT_SELECT1"),chkParameters);	
		return result;
