...
function reset() {
	f_query_add_mod.document.formRep_TrayUsageStatistics.reset();
}
function loadPage()
{
}
function run()
{
   var formObj=parent.f_query_add_mod.document.formRep_TrayUsageStatistics;
	...
	if (isValidFromToField(formObj.p_fm_group_code, formObj.p_to_group_code, STRING, "Group", messageFrame)) 
		if (isValidFromToField(formObj.p_fm_store_code, formObj.p_to_store_code, STRING, "Store", messageFrame))
		   if (isValidFromToField(formObj.p_fm_doc_date1, formObj.p_to_doc_date1, DATE, "Cut Off Date", messageFrame)) 
...
async function searchCodeStoreMaster(obj) {
	...
	dataValueArray [0]=document.formRep_TrayUsageStatistics.p_facility_id.value;
	dataTypeArray  [0]=STRING
	argumentArray[0] = document.formRep_TrayUsageStatistics.SQL_SS_STORE_LOOKUP.value;
		dataNameArray  [1]="LANGUAGE_ID"
	dataValueArray [1]=document.formRep_TrayUsageStatistics.locale.value;
...
async function searchCodeGroupMaster(obj) {
	...
	argumentArray[0] = document.formRep_TrayUsageStatistics.SQL_SS_GROUP_LOOKUP.value;
	dataNameArray  [0]="LANGUAGE_ID"
	dataValueArray [0]=document.formRep_TrayUsageStatistics.locale.value;
	dataTypeArray  [0]=STRING;
...
var function_id = "";
	...
	argumentArray[0] = document.formRep_TrayUsageStatistics.SQL_SS_SURGERY_TYPE_LOOKUP.value;
	dataNameArray  [0]="LANGUAGE_ID"
	dataValueArray [0]=document.formRep_TrayUsageStatistics.locale.value;
	dataTypeArray  [0]=STRING;
