...
function reset() {
	...
	f_query_add_mod.document.formWriteOffdetail.reset();
}
...
function run()
{
	formObj= f_query_add_mod.document.formWriteOffdetail;
		if( isValidFromToField(formObj.p_fm_document_type,formObj.p_to_document_type,STRING,"Doc Type" ,  messageFrame )  )
		if( isValidFromToField(formObj.p_fm_store_code,formObj.p_to_store_code,STRING,"Store" ,  messageFrame )  )
		if( isValidFromToField( formObj.p_fm_document_no,formObj.p_to_document_no,LONG,"Doc No" , messageFrame )  )
...
async function searchCode( objCode,label ) {
	...
	if(objCode.name=="p_fm_document_type"  || objCode.name=="p_to_document_type"){
		argumentArray[0] = document.formWriteOffdetail.SQL_SS_DOC_TYPE_ADJ_LIST_LOOKUP.value;
	}
	if(objCode.name=="p_fm_store_code" || objCode.name=="p_to_store_code" ){
		argumentArray[0] =document.formWriteOffdetail.SQL_SS_STORE_LOOKUP.value;
	}

	dataNameArray[0]	= "facility_id";
	dataValueArray[0]	= document.formWriteOffdetail.P_facility_id.value;
	dataTypeArray[0]	= STRING;

	dataNameArray  [1]="LANGUAGE_ID"
	dataValueArray [1]=document.formWriteOffdetail.locale.value;
