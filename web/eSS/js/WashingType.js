...
*/
function create() {
	f_query_add_mod.location.href="../../eSS/jsp/WashingTypeAddModifyMultiple.jsp?mode="+MODE_INSERT+"&function_id="+function_id; 
}

function query() {
	f_query_add_mod.location.href="../../eSS/jsp/WashingTypeQueryCriteria.jsp?function_id="+function_id+"&mode="+MODE_MODIFY;
}
   ...
   if( mode == MODE_MODIFY )
    {
        ...
        var obj=f_query_add_mod.document.formWashingType;
...
function proceedOnApply() {
    ...
    url = url.toLowerCase();
    if ( (url.indexOf("blank.html")==-1) && (url.indexOf("washingtypequery")==-1) )
        return true;
    else
        return false;
}
...
function reset() {
	if (f_query_add_mod.document.formWashingType != null) {
...
function onSuccess() {
	var mode = f_query_add_mod.document.formWashingType.mode.value;
	if( mode == MODE_INSERT ) 
		reset();
	else 
		f_query_add_mod.document.location.reload();
...
function Modify(obj){
	...
	document.location.href="../../eSS/jsp/WashingTypeAddModify.jsp?mode="+MODE_MODIFY+"&function_id="+parent.function_id+"&washing_type="+code1;
}

function resetColorCode(obj){
    var arrPKFieldNames = new Array();
    var formObj = parent.f_query_add_mod.document.formWashingType;
    arrPKFieldNames[0] = "code_";
