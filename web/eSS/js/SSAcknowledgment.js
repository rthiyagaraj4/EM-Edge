...
function create(){
	f_query_add_mod.location.href="../../eSS/jsp/SSAcknowledgmentFrame.jsp?mode="+MODE_INSERT+"&function_id="+function_id; 
}
function reset() {
...
if(frmObjlist){
	...
	xmlDoc = new DOMParser().parseFromString(getEnhancedXMLString(frmObjlist), "text/xml");
	xmlHttp.open("POST", "../../eSS/jsp/SSAcknowledgmentValidate.jsp?validate=UPDATE_EXP_DETAIL&bean_name=" + bean_name + "&bean_id=" + bean_id+"&doc_no="+frmObjlist.doc_no.value+"&seq_no="+frmObjlist.seq_no.value+"&req_doc_no="+frmObjlist.req_doc_no.value, false);
	xmlHttp.send(xmlDoc);
	responseText = xmlHttp.responseText;
	repsonseText = responseText.replace(/<\/?[^>]+(>|$)|\s+|\r?\n/g, "");
	//alert(responseText);
	eval(responseText);
		...
		var total_records = eval(frmObjlist.total_records.value);
			...
			if(exist){
					...
					eval(responseText);
						if(result){
						//f_query_add_mod.location.reload();
							onSuccess(); //Added by Sakti against BRU-HIMS-CRF-376
						}
					}else {
						messageFrame.location.href = "../../eCommon/jsp/error.jsp?err_num=";
						return;
					...
					//f_query_add_mod.location.reload();
			...
			}else if(formObjhdr.action_type.value=="C" && formObjhdr.trn_type.value=="ISS" ) {
										...
										if(doIt) {
											...
											var retVal = window.showModalDialog("../../eSS/jsp/IssueGroup.jsp?mode="+MODE_MODIFY+"&req_doc_type_code="+frmObjlist.req_doc_type_code.value+"&req_doc_no="+frmObjlist.req_doc_no.value+"&function_id="+function_id+"&doc_no="+frmObjlist.doc_no.value+"&doc_type_code="+frmObjlist.doc_type_code.value+"&Re_Issue="+Re_Issue,arguments,features);
...
function searchVals(){
	...
	if(blankObject==true){
		...
		eval(responseText);
		//alert(responseText);
		parent.AcknowledgeDetailframe.document.location.href = "../../eSS/jsp/SSAcknowledgmentDetail.jsp?trn_type="+formObj.trn_type.value+"&action_type="+formObj.action_type.value+"&mode="+formObj.mode.value;
...
//Added by Sakti against BRU-HIMS-CRF-376 to store list form value
	...
	/*if(formAcknowledgeDetail.action_type.value=="C")
...
function showRemarks(i) {
	...
	//rem = window.showModalDialog("../../eSS/jsp/SSAcknowledgeRemarks.jsp?remarks=" + eval("document.formAcknowledgeList.remarks_"+i).value+"&doc_no="+doc_no+"&seq_no="+seq_no+"&group_code="+group_code+"&tray_no="+tray_no+"&expiry_date="+expiry_date+"&bin_location_code="+bin_location_code+"&doc_type_code="+doc_type_code, "", "dialogHeight:10; dialogWidth:30; center:0; status: no; dialogLeft :520; dialogTop :330;");
	
	//Added by Rabbani #Inc no:37244  on 09-Jan-2013
	rem = window.showModalDialog("../../eSS/jsp/SSAcknowledgeRemarks.jsp?remarks="+encodeURIComponent(remarks,"UTF-8")+"&doc_no="+doc_no+"&seq_no="+seq_no+"&group_code="+group_code+"&tray_no="+tray_no+"&expiry_date="+expiry_date+"&bin_location_code="+bin_location_code+"&doc_type_code="+doc_type_code, "", "dialogHeight:10; dialogWidth:30; center:0; status: no; dialogLeft :520; dialogTop :330;");
...
function updateRemarks() {
	...
	//if (!setTextAreaMaxLength(formObject.remarks, 400)) { //commented by Rabbani for allowing 400 characters on 14/12/2012
	 if (!setTextAreaMaxLength(formObject.remarks, 401)) { //Added by Rabbani for allowing 400 characters on 14/12/2012
...
function setTrnType(obj){
	...
	xmlDoc = new DOMParser().parseFromString(getXMLString(null), "text/xml");
	//xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../eSS/jsp/SSAcknowledgmentValidate.jsp?validate=TRNTYPE&trn_type="+obj.value, false);
	xmlHttp.send(xmlDoc);
	responseText = xmlHttp.responseText;
...
function setActionType(obj){
	...
	xmlDoc = new DOMParser().parseFromString(getXMLString(null), "text/xml");
	//xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../eSS/jsp/SSAcknowledgmentValidate.jsp?validate=ACTIONTYPE&action_type="+obj.value, false);
	xmlHttp.send(xmlDoc);
	responseText = xmlHttp.responseText;
	eval(responseText.replace(/<\/?[^>]+(>|$)|\s+|\r?\n/g, ""));
	//alert(responseText);
...
function restoreToolbar(){
    ...
    xmlHttp.open("POST","SSAcknowledgmentValidate.jsp?validate=toolBar&"+toolBarString,false);
    xmlHttp.send(xmlDoc);
...
function StoreMendateImageChk(){
	if(document.formAcknowledgeHeader.action_type.value=="A"){
		document.formAcknowledgeHeader.querySelector('#fm_store_img').style.visibility = "hidden";
		document.formAcknowledgeHeader.querySelector('#to_store_img').style.visibility = "visible";
		}else{
		document.formAcknowledgeHeader.querySelector('#fm_store_img').style.visibility = "visible";
		document.formAcknowledgeHeader.querySelector('#to_store_img').style.visibility = "hidden";
	}
}
//Added by Sakti against BRU-HIMS-CRF-376
function showReturnRemarks(return_remarks) {
	...
	if( return_remarks != "") {
	var rem = "";
	rem = window.showModalDialog("../../eSS/jsp/ReturnGroupRemarks.jsp?remarks=" + return_remarks, "", "dialogHeight:10; dialogWidth:28; center:1; status: no; dialogLeft :520; dialogTop :330;");
...
function showReturnDiscrepancyRemarks(i, doc_no, doc_type_code, facility_id) {
	
	var  group_code			= eval("document.formAcknowledgeList.ret_group_code_"+i).value;
	var tray_no				= eval("document.formAcknowledgeList.tray_no_"+i).value;
	var group_desc			= eval("document.formAcknowledgeList.group_desc_"+i).value;
	
	
	var rem = "";
	rem = window.showModalDialog("../../eSS/jsp/ReturnDiscrepancyRemarks.jsp?group_desc="+encodeURIComponent(group_desc,"UTF-8")+"&group_code="+group_code+"&tray_no="+tray_no+"&doc_no="+doc_no+"&doc_type_code="+doc_type_code+"&facility_id="+facility_id, "", "dialogHeight:30; dialogWidth:70; center:1; status: no;  dialogTop :265;");
}
//Added ends
...
//Tray discrepancy and Return Discrepancy Remarks popup display and hide logic Started
 function showPopup(i,count_tray_disc,count_ret_disc,called_from){
	 show=1;
	 ...
	 if(count_tray_disc>0){
		 ...
		 document.getElementById("ack_disc").style.fontWeight='bold';//Added for TH-KW-CRF-0026
	 }else{
	 ...
	 if(count_ret_disc>0){
	  ...
	  document.formAcknowledgeList.rec_no.value =i;
...
function showWindow(doc_no, doc_type_code, facility_id,count_tray_disc,count_ret_disc){
	...
	//} 
	var trn_type				= document.formAcknowledgeList.trn_type.value;
	i =document.formAcknowledgeList.rec_no.value;//Added for ICN-38140 point2
	//alert("i=="+i);
	var called_from ="ACK";	
		if(trn_type == "ISL") {
		var  group_code			= eval("document.formAcknowledgeList.group_code_"+i).value;
	...
	if (event.target.style.fontWeight=="bold"){
		...
		if(opt=='A'){//Addign start for TH-KW-CRF-0026
			...
			var xmlHttp = new XMLHttpRequest();
				...
				xmlHttp.open("POST", "../../eSS/jsp/SSAcknowledgmentValidate.jsp?group_code="+group_code+"&tray_no="+tray_no+ "&validate=setGroup", false);
				xmlHttp.send(xmlDoc);
				responseText=xmlHttp.responseText;
				responseText = responseText.replace(/<\/?[^>]+(>|$)|\s+|\r?\n/g, "");
				eval(responseText);
				...
				var arguments	= "" ;
				var parameters = "?group_code="+group_code +"&tray_no="+tray_no;
				retVal = window.showModalDialog("../../eSS/jsp/AcknowledgmentDiscrepancyFrames.jsp"+parameters,arguments,features);
				
		}else{//Adding end for TH-KW-CRF-0026
		var url = "";
...
}//Tray discrepancy and Return Discrepancy Remarks popup display and hide logic ends

function addToDiscrepancyList(called_from){//Adding start for TH-KW-CRF-0026
	var formObj=  document.formAckDiscrepancyDetail;
	...
	eval(responseText);
	if( result ){
		resetAckDiscrepancyDetail();
		parent.frameAckDiscrepancyList.location.href=parent.frameAckDiscrepancyList.location.href;
	}
	else{
...
function preserveDiscrepancyDetails(){
	var xmlHttp = new XMLHttpRequest();
	var xmlDoc="";
	xmlDoc.loadXML(getEnhancedXMLString(document.formAckDiscrepancyList));
	xmlHttp.open("POST", "../../eSS/jsp/SSAcknowledgmentValidate.jsp?validate=PRESERVE_DISCREPANCY", false);
	xmlHttp.send(xmlDoc);
	responseText=xmlHttp.responseText;
}

function resetAckDiscrepancyDetail(){
	var formObj=document.formAckDiscrepancyDetail;
...
function discrepancyListModify(index){
	parent.frameAckDiscrepancyDetail.location.href="../../eSS/jsp/AcknowledgmentDiscrepancyDetail.jsp?index="+index;//+"&function_id="+parent.function_id;
}//Addign end for TH-KW-CRF-0026
