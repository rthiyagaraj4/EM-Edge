		...
		<form name="formReturnInstrumentsHeader" id="formReturnInstrumentsHeader"onReset="FocusFirstElement();">
			<table border="0" cellpadding="0" cellspacing="0" width='100%' height='100%' align='center'>
				<tr>
					...
					<td class="label">&nbsp;&nbsp;<input type="text" class="NUMBER" value="<%=bean.getDoc_no()%>" name="doc_no" maxLength="8" size="8" onKeyPress="return isValidInteger();" onBlur="checkIntegerFormat(this);" <%=codeDisabled%> <%=docNoDisabled%>><%=bean.getMandatoryImageTag()%></td>
					<td  class="label"><fmt:message key="eSS.DocDate.label" bundle="${ss_labels}"/></td>
					<td class="label">&nbsp;&nbsp;<input type="text" value="<%=com.ehis.util.DateUtils.convertDate(bean.getDoc_date(),"DMY","en",locale)%>" name="doc_date"  maxLength="10" size="10" <%=disabled%> onBlur="CheckDate(this,'<%=locale%>');">&nbsp;<img src="../../eCommon/images/CommonCalendar.gif" onClick="return showCalendar('doc_date');"></img><%=bean.getMandatoryImageTag()%></td>
				</tr>			
				<tr>
					<td  class="label"><fmt:message key="eSS.ReceivingStore.label" bundle="${ss_labels}"/></td>
					<td class="label">&nbsp;&nbsp;<select name="rec_store_code" id="rec_store_code" onBlur="populateItem();" <%=disabled%> <%=codeDisabled%>><%=bean.getRec_store_code_List()%></select><%=bean.getMandatoryImageTag()%></td>
					<td  class="label"><fmt:message key="Common.patientId.label" bundle="${common_labels}"/></td>
					<td class="label">&nbsp;&nbsp;<input type="text" value="<%=bean.getPatient_id()%>" name="patient_id"  maxLength="20" size="20" onBlur="searchPatientName()" <%=disabled%> <%=codeDisabled%>><input type="button" class='BUTTON' value="?" name="patient_search" id="patient_search" onclick="searchPatient();" <%=disabled%> <%=codeDisabled%>></td>
					<td  class="label"><fmt:message key="Common.PatientName.label" bundle="${common_labels}"/></td>
					<td class="label">&nbsp;&nbsp;<input type="text" value="<%=bean.getPatient_name()%>" name="patient_name"  maxLength="25" size="25"  disabled></td>
				</tr>			
				<tr>
					<td  class="label"><fmt:message key="eSS.ReturningStore.label" bundle="${ss_labels}"/></td>
					<td class="label">&nbsp;&nbsp;<select name="ret_store_code" id="ret_store_code" <%=disabled%> <%=codeDisabled%> onBlur="populateItem();"><%=bean.getRet_store_code_List()%></select><%=bean.getMandatoryImageTag()%></td>
				...
				<tr>
					...
					<td class="label" colspan='5'>&nbsp;<input type="checkbox" value="Y" name="entry_completed_yn" id="entry_completed_yn" <%=disabled%> <%=bean.getChecked(bean.getEntry_completed_yn())%> ></td>			
				</tr>
			</table>
