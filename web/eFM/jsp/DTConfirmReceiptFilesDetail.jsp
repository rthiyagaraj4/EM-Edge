...
%>
					...
					<A HREF onMouseover="changeCursor(this)" onClick="showEncounter('<%=encounterid%>','<%=patientid%>','<%=fileno%>')"><font size=1><%=encounterid%></font></A></td>
<%	
					...
					{
%>
					   <a href='javascript:GetShowRemarks("ReqComm<%=i%>","<%=java.net.URLEncoder.encode(p_req_remarks)%>","R")'>Remarks</a></font></td>
<%					
					}
					...
					{
%>
					   <a href='javascript:GetShowRemarks("txtComm<%=i%>","<%=java.net.URLEncoder.encode(p_iss_remarks)%>","IR")'>Remarks</a></font></td>
<%					
					}
