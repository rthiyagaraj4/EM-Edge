...
if(frmObj.theatre.value ==''|| frmObj.theatre.value==null)
	...
	if(change_from_date ==''|| change_from_date==null)
	 ...
	 if(change_to_date ==''|| change_to_date==null)
	   error+=msgArray[0]+" "+getLabel('Common.BookingDate.label','Common')+" "+msgArray[1]; 
	   if(error.length > 0){
		alert (error);
		return false;
	   ...
	   if(!CompareDateFields(frmObj)){
	  	return false;
			...
			parent.f_query_add_mod.location.href ="../../eOT/jsp/ManageIOSchDtl.jsp?"+param;
	   }
}
...
function searchBookingQryOperation(obj,target){
	var tcode = obj.value;
	...
	var sql="SELECT SHORT_DESC DESCRIPTION,OPER_CODE CODE  FROM OT_OPER_MAST WHERE NVL(STATUS,'E') = 'E' AND upper(SHORT_DESC) like upper(?) AND upper(OPER_CODE) like upper(?) ";
	var frmObj=document.forms[0];
...
function reset() {
	 if(f_query_add_mod.document.location.href.indexOf("ManageIOSearch.jsp") != -1)
		  return false;
	 else
