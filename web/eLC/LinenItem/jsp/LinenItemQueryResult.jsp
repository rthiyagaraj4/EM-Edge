...
<body OnMouseDown="CodeArrest()" onKeyDown="lockKey()">
	<ibaHTMLEl:form action="/LinenItemPageAction">
			...
			<ibaLogic:equal name="PageForm" value="0"
				...
				<script> 
					...
					parent.content.location.href=contextPath+"/LinenItemAction.do?method="+queryMethod+"&vo.functionId="+'<%=request.getParameter("vo.functionId")%>'
			</script>
			</ibaLogic:equal>
			...
			<ibaLogic:notEqual name="PageForm" value="0"
				...
				<tr>
					<th>
						<ibaBean:message key="com.iba.ehis.lc.linenType.linenItemCode"
							bundle="lcLabels" />
					</th>
			...
			<ibaLogic:iterate id="result" name="PageForm"
				...
				<tr >
					<td class="<%=style %>"> 
						<a
							href="javascript:parent.loadResult('<ibaBean:write name="result" property="linenItem" />')"><ibaBean:write
								name="result" property="linenItem" />
						</a>
					</td>
					<td class="<%=style %>">
						<ibaBean:write name="result" property="longDesc" />
					</td>
					<td class="<%=style %>">
						<ibaBean:write name="result" property="shortDesc" />
					</td>
					<td class="<%=style %>" align="center">
						<ibaLogic:equal value="Y" property="disposableItem" name="result">
							<img src='framework/images/enabled.gif'></img>
						</ibaLogic:equal>
						<ibaLogic:equal value="N" property="disposableItem" name="result">
