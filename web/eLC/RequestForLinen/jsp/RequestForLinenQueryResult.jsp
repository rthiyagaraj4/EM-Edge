			...
			%>
			 ...
			 <!-- 	<td class="<%=style%>" align="left">
				...
				<td class="<%=style%>" align="left" >
					...
					<td class="<%=style%>" align="left">
						<ibaLogic:equal value="W" property="sourceType" name="result">
							<ibaBean:message key="com.iba.ehis.lc.ward" bundle="lcLabels" />
						</ibaLogic:equal>
						<ibaLogic:equal value="C" property="sourceType" name="result">
							<ibaBean:message key="com.iba.ehis.lc.clinic" bundle="lcLabels" />
						</ibaLogic:equal>
						<ibaLogic:equal value="D" property="sourceType" name="result">
							<ibaBean:message key="com.iba.ehis.lc.department" bundle="lcLabels" />
						</ibaLogic:equal>
					</td>
					<td class="<%=style%>" align="right" width='10%'>
						
						<a href='<%=request.getContextPath()%>/eLC/RequestForLinen/jsp/link.jsp?srcCode="+<ibaBean:write name="result" property="sourceCode" />+"&sourceType="+<ibaBean:write name="result" property="sourceType"/>+"&requestedQty="+<ibaBean:write name="result" property="requestedQty" bundle="lcLabels" />+"&requestNum="+<ibaBean:write name="result" property="requestNo" />+"&requestDate="+<ibaBean:write name="result" property="requestDate" bundle="lcLabels" />+"&raisedBy="+<ibaBean:write name="result" property="raisedBy" />+"&requestStatus="+<ibaBean:write name="result" property="requestStatus" />+"&linenItem="+<ibaBean:write name="result" property="linenItem" />+"&reasonCode="+<ibaBean:write name="result" property="reasonCode" />+"&cancelledBy="+<ibaBean:write name="result" property="cancelledBy" />+"&cancelledDate="+<ibaBean:write name="result" property="cancelledDate" bundle="lcLabels" />' onclick="return show_hide_box(this,100,35,'2px dashed')" >
					...
					<td class="<%=style%>" class="label" align='left'> 
						<div id="dt<%=rowId%>" class="<%=style%>" />
					...
					<script>	
					 // var date=document.getElementById("requestDate_<%=rowId%>").value
						IBADateValidator.convertDateJS('<ibaBean:write name="result" property="requestDate" bundle="lcLabels"/>',"en","<%=locale%>","dd/MM/yyyy HH:mm",callBackReqDate);
						function callBackReqDate(conDate){
							document.getElementById("dt<%=rowId%>").innerText=conDate;
						} 
					</script>		

					<td class="<%=style%>" align="left">
						<ibaBean:write name="result" property="raisedBy" />
					</td>
					<td class="<%=style%>" align="left">
						...
						<ibaLogic:equal value="PI" property="requestStatus" name="result">
							<ibaBean:message key="com.iba.ehis.lc.requestForLinen.partiallyissued" bundle="lcLabels" />
						</ibaLogic:equal>
						<ibaLogic:equal value="IS" property="requestStatus" name="result">
