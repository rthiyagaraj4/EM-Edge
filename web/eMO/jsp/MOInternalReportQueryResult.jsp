<!DOCTYPE html>
<%@ page  import= "webbeans.eCommon.*,java.util.*,java.sql.*,java.text.*,java.net.*,com.ehis.util.*" contentType="text/html;charset=UTF-8" %>
 ...
 <%
        ...
        try
		 ...
		 %> <table cellspacing=0 cellpadding=0 align='center' width='100%' border='1' cellpadding=0 cellspacing=0>
			...
			<th><fmt:message key="eMO.Body.label" bundle="${mo_labels}"/><br><fmt:message key="eMO.Receipt.label" bundle="${mo_labels}"/><br><fmt:message key="Common.datetime.label" bundle="${common_labels}"/></th>
...
<%
	...
	<%}%>
		...
		<td class='<%=classValue%>'  <%=colordisplay%> > <font size=1><%=by_recd_dt%></td>
		<td class='<%=classValue%>' align='right'  <%=colordisplay%>> <font size=1><%=dis_pid%></td>
		
		<%if(brought_dead_yn.equals("Y")){%>
		<td  style='background-color:black;'>
		<a name='desc1' style="color:white" href='javascript:showAllowedOperations1("<%=dis_acode%>","<%=dis_rno%>","<%=facility_id%>","<%=dis_pid%>","<%=exportation_doc_iss_date%>")' > <font size=1><%=dis_pname%></a></td>
		<%}else{%>
		<td class='<%=classValue%>' <%=colordisplay%>>
		<a name='desc1' href='javascript:showAllowedOperations1("<%=dis_acode%>","<%=dis_rno%>","<%=facility_id%>","<%=dis_pid%>","<%=exportation_doc_iss_date%>")' > <font size=1><%=dis_pname%></a></td>
			
		<%}%>
		<td class='<%=classValue%>'  <%=colordisplay%> align='left'> <font size=1><%=dis_age%></td>
		...
		<td class='<%=classValue%>' align='left' <%=colordisplay%>> <font size=1><%=BODY_PART_OBTAINED_FROM_DESC%></td>
		<td class='<%=classValue%>' align='left' <%=colordisplay%>> <font size=1><%=bur_perm_no%></td>
