...
function reset() 
{
	if(f_query_add_mod.document.repReqForPostmortem)
	{
	   f_query_add_mod.location.href = '../../eMO/jsp/MORepRequestForPostmortem.jsp?param=';
	}
}
/*function run() 
{ 	
...
/*function calljsp1(obj)
{
	...
	if(az!='')
	{
		f_query_add_mod.location.href = '../../eMO/jsp/MORepRequestForPostmortem.jsp?param='+az+'&first=N';
	}
}*/
async function searchCode(obj,target)
{
	...
	sql="select REGISTRATION_NO code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"'  and PM_YN='Y'  and upper(REGISTRATION_NO) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}
	else if(obj.name=="patient_search2"||obj.name=="patient_search3")
	{
     ...
     fieldLegend["DESC_LEGEND"] = getLabel("Common.PatientName.label","Common");
    tit= getLabel("eMO.DeceasedPatients.label","MO") + "/" + getLabel("Common.BodyParts.label","Common");
	sql="select patient_id code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"'  and PM_YN='Y'  and upper(patient_id) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}
	...
	if(retVal != null && retVal != "" )
	{
		...
		var arr=ret1.split(",");
		target.value= arr[0];
			//parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepRequestForPostmortem.jsp?param='+arr[0];
	}
...
/*function calljsp(obj)
{
	...
	if(az!='')
	{
		parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepRequestForPostmortem.jsp?param='+az+'&first=N';
	}
}*/
