...
function reset() 
{
	if(f_query_add_mod.document.repConsentForClinicPM)
	{
	   f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForClinicalPM.jsp?param=';
	}
}
/*function run() 
{ 	alert();
...
/*function calljsp1(obj)
{
	...
	if(az!='')
	{
		f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForClinicalPM.jsp?param='+az+'&first=N';
	}
}*/
...
async function searchCode(obj,target)
{
	...
	if(obj.name=="patient_search2"||obj.name=="patient_search3")
	{
		...
		fieldLegend["CODE_LEGEND"] = getLabel("Common.patientId.label","Common");
		sql="select patient_id code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name)) Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and CONSENT_FORM_CODE is not null and upper(Patient_Id) like upper(?) and upper(patient_name) like upper(?) order by 2";
			
	
	
	}else if(obj.name=="patient_search"||obj.name=="patient_search1")
	{
       	fieldLegend["CODE_LEGEND"] = getLabel("Common.registrationno.label","Common");
        fieldLegend["DESC_LEGEND"] = getLabel("Common.PatientName.label","Common");
		tit= getLabel("eMO.DeceasedPatients.label","MO") + "/" + getLabel("Common.BodyParts.label","Common");	
		fieldLegend["CODE_LEGEND"] = getLabel("Common.patientId.label","Common");
		sql="select REGISTRATION_NO code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name)) Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and CONSENT_FORM_CODE is not null and upper(REGISTRATION_NO) like upper(?) and upper(patient_name) like upper(?) order by 2";
	...
	if(retVal != null && retVal != "" )
	{
		...
		if(target.name == 'P_FM_REGISTRATION_NO')
		{	
		document.forms[0].P_FM_REGISTRATION_NO.value= arr[0];
			//parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForClinicalPM.jsp?param='+arr[0];
	}	
	else if(target.name=='P_TO_REGISTRATION_NO')
	{	
		document.forms[0].P_TO_REGISTRATION_NO.value= arr[0];
	}else if(target.name == 'P_FM_PATIENT_ID')
		{	
		document.forms[0].P_FM_PATIENT_ID.value= arr[0];
			//parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForClinicalPM.jsp?param='+arr[0];
	}	
	else if(target.name=='P_TO_PATIENT_ID')
	{	
		document.forms[0].P_TO_PATIENT_ID.value= arr[0];
	}
}
}
/*function calljsp(obj)
{
	...
	if(az!='')
	{
		parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForClinicalPM.jsp?param='+az+'&first=N';
	}
}*/
