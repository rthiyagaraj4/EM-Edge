...
function reset() 
{
	if(f_query_add_mod.document.repExpDoc)
	{
		f_query_add_mod.location.href = '../../eMO/jsp/MOrepExportationDocument.jsp?param=';
	}
}
...
if (f_query_add_mod.CheckString(getLabel('Common.patientId.label','Common'), f_query_add_mod.document.forms[0].P_FM_PATIENT_ID,f_query_add_mod.document.forms[0].P_TO_PATIENT_ID, messageFrame))
		...
		f_query_add_mod.document.repExpDoc.submit() ;


/*	chkman1(f_query_add_mod.document.forms[0].patient_id);
	...
	if (valid=='Y')
	{
	var field = new Array (f_query_add_mod.document.repExpDoc.patient_id);
	var name= new Array("Registration Number");
	if(f_query_add_mod.document.repExpDoc)
	{ */
	//	if(f_query_add_mod.checkFields(field, name, messageFrame))
		   
     /*}
	}*/
 }

/*function chkman1(obj)
{
  if(obj.value!='')
  {
    f_query_add_mod.location.href = '../../eMO/jsp/MOrepExportationDocument.jsp?param='+obj.value;
  }
...
/*function chkman(obj)
 {
  if(obj.value!='')
	  {
	   parent.f_query_add_mod.location.href = '../../eMO/jsp/MOrepExportationDocument.jsp?param='+obj.value;
	  }
}*/
...
async function searchCode(obj,target)
{
	...
	{   fieldLegend["CODE_LEGEND"] = getLabel("Common.registrationno.label","Common");
		...
		sql="select REGISTRATION_NO code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and EXPORTATION_DOC_ISS_DATE is not null  and upper(REGISTRATION_NO) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}
	else if(obj.name=="patient_search2"||obj.name=="patient_search3")
	{   fieldLegend["CODE_LEGEND"] = getLabel("Common.patientId.label","Common");
        fieldLegend["DESC_LEGEND"] = getLabel("Common.PatientName.label","Common");
		tit= getLabel("eMO.DeceasedPatients.label","MO") + "/" + getLabel("Common.BodyParts.label","Common");
		sql="select Patient_id code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and EXPORTATION_DOC_ISS_DATE is not null  and upper(Patient_id) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}
	argumentArray[0] =sql;
