...
function reset() 
{
	if(f_query_add_mod.document.repConsentOrgDonNoDonCard)
	{
	   f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForOrgDon.jsp?param=';
	}
}
/*function run() 
{ 	
...
/*function calljsp1(obj)
{
	...
	if(az!='')
	{
		f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForOrgDon.jsp?param='+az+'&first=N';
	}
}*/
async function searchCode(obj,target)
{
	...
	if(obj.name=="patient_search"||obj.name=="patient_search1")
	{
		...
		sql="select registration_no code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and CONSENT_FORM_CODE2 is not null and upper(registration_no) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}else
	{
		tit= getLabel("eMO.DeceasedPatients.label","MO") + "/" + getLabel("Common.BodyParts.label","Common");		
		fieldLegend["CODE_LEGEND"] = getLabel("Common.patientId.label","Common");
        fieldLegend["DESC_LEGEND"] = getLabel("Common.PatientName.label","Common");
		sql="select Patient_Id code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and CONSENT_FORM_CODE2 is not null and upper(Patient_Id) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}
	argumentArray[0] =sql;
	...
	if(retVal != null && retVal != "" )
	{
		...
		//if(target.name == 'patient_id')
			target.value= arr[0];
			//parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForOrgDon.jsp?param='+arr[0];
	}
...
/*function calljsp(obj)
{
	...
	if(az!='')
	{
		parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepConsentForOrgDon.jsp?param='+az+'&first=N';
	}
}*/
