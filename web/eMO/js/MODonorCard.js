...
function reset() 
{
	if(f_query_add_mod.document.repConsentForOrgDonCard)
	{
	   f_query_add_mod.location.href = '../../eMO/jsp/MORepDonorCard.jsp?param=';
	}
}
...
if (f_query_add_mod.CheckString(getLabel('eMO.DonorCardNo.label','MO'), f_query_add_mod.document.forms[0].P_FM_DONOR_CARD_NO,f_query_add_mod.document.forms[0]. P_TO_DONOR_CARD_NO, messageFrame))	
	...
	/*if(f_query_add_mod.document.forms[0].patient_id.value=='' && f_query_add_mod.document.forms[0].valid.value=='Y')
		{
			var errors='APP-000001 Patient ID Cannot be Blank'; 
			messageFrame.document.location.href="../../eCommon/jsp/error.jsp?err_num="+errors ;
		}
...
/*function calljsp1(obj)
{
	...
	if(az!='')
	{
		f_query_add_mod.location.href = '../../eMO/jsp/MORepDonorCard.jsp?param='+az+'&first=N';
	}
}*/
async function searchCode(obj,target)
{
	...
	{  
		...
		sql="select registration_no code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name)) Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and donor_card_no is not null and upper(registration_no) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}else if(obj.name=="patient_search2"||obj.name=="patient_search3")
	{
		...
		sql="select Patient_Id code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name)) Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and donor_card_no is not null and upper(Patient_Id) like upper(?) and upper(patient_name) like upper(?) order by 2";
	
	}else if(obj.name=="patient_search4"||obj.name=="patient_search5")
	{
		tit= getLabel("eMO.DeceasedPatients.label","MO") + "/" + getLabel("Common.BodyParts.label","Common");	
		fieldLegend["CODE_LEGEND"] = getLabel("eMO.DonorCardNo.label","MO");
        fieldLegend["DESC_LEGEND"] = getLabel("Common.PatientName.label","Common");

		sql="select DONOR_CARD_NO code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name)) Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"' and donor_card_no is not null and upper(DONOR_CARD_NO) like upper(?) and upper(patient_name) like upper(?) order by 2";
	...
	if(retVal != null && retVal != "" )
	{
		...
		var arr=ret1.split(",");
			...
			target.value= arr[0];
			//parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepDonorCard.jsp?param='+arr[0];
	}
...
/*function calljsp(obj)
{
	...
	if(az!='')
	{
		parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepDonorCard.jsp?param='+az+'&first=N';
	}
}*/
