...
function reset() 
{
	if(f_query_add_mod.document.repAutopsyReport)
	{
	   f_query_add_mod.location.href = '../../eMO/jsp/MORepAutopsyReport.jsp?param=';
	}
}
/*function run() 
{ 	
...
return false;
	...
	//calljsp1(f_query_add_mod.document.forms[0].registration_no);
	
	/*if(f_query_add_mod.document.forms[0].registration_no.value=='' && f_query_add_mod.document.forms[0].valid.value=='Y')
		{
	...
	if (valid=='Y')
	{
		if(f_query_add_mod.document.forms[0].registration_no.value=='')
		{
			var errors='APP-000001 Registration No Cannot be Blank'; 
			messageFrame.document.location.href="../../eCommon/jsp/error.jsp?err_num="+errors ;
		}
...
/*function calljsp1(obj)
{
	...
	if(az!='')
	{
		f_query_add_mod.location.href = '../../eMO/jsp/MORepAutopsyReport.jsp?param='+az+'&first=N';
	}
}*/
...
async function searchCode(obj,target)
{
	...
	{//tit="Deceased Patients/Body Parts "
	...
	sql="select REGISTRATION_NO code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"'  and PM_YN='Y' and postmortem_status='E'  and upper(REGISTRATION_NO) like upper(?) and upper(patient_name) like upper(?) order by 2";
	...
	sql="select patient_id code, decode('"+localeName+"', 'en', patient_name, nvl(patient_name_loc_lang, patient_name))  Description from mo_mortuary_regn_vw where  facility_id='"+facilityId+"'  and PM_YN='Y' and postmortem_status='E' and upper(patient_id) like upper(?) and upper(patient_name) like upper(?) order by 2";
	}
	argumentArray[0] =sql;
	...
	if(retVal != null && retVal != "" )
	{
		...
		var arr=ret1.split(",");
			...
			target.value= arr[0];
			//parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepAutopsyReport.jsp?param='+arr[0];
	}
...
/*function calljsp(obj)
{
	...
	if(az!='')
	{
		parent.f_query_add_mod.location.href = '../../eMO/jsp/MORepAutopsyReport.jsp?param='+az+'&first=N';
	}
}
*/
...
function firstfocus()
{
	if(document.forms[0].first.value=='')
	   document.forms[0].registration_no.focus();
}
