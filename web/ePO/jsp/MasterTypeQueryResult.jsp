...
<%
	...
	try{
		...
		master_code = CommonBean.checkForNull(master_code)+'%';
		long_desc = CommonBean.checkForNull(long_desc)+'%';
		eff_status = CommonBean.checkForNull(eff_status);
		master_type = CommonBean.checkForNull(master_type,"%");

		HashMap sqlMap = new HashMap();
		sqlMap.put("sqlData", PoRepository.getPoKeyValue("SQL_PO_MASTER_TYPE_QUERYCRIT"));
		HashMap funcMap = new HashMap();

		//The fields are going to be display
		ArrayList displayFields = new ArrayList();
		displayFields.add("master_desc");
	...
	//	System.out.println("sqlMap in Master qry res = "+sqlMap);
	//	System.out.println("displayFields in Master qry res = "+displayFields);
	//	System.out.println("chkFields in Master qry res = "+chkFields);
		
		ArrayList result=(ArrayList)CommonBean.getQueryResultPage(pageContext,sqlMap, funcMap, request);
		
	//	System.out.println("result in Master qry res = "+result);
		...
		<%
			...
			for(int recCount=2; recCount<result.size(); recCount++)
			{
				...
				if( !master_desc.equals((String)records.get(0)) ) {
				master_desc = (String)records.get(0) ;
				out.println("<tr><td class='ITEMSELECT' align='left' colspan='4'><b>"+master_desc+"</b></td></tr>");
				}
	...
	%>
		...
		out.println(CommonBean.setForm ( request ,"../../ePO/jsp/MasterTypeQueryResult.jsp", searched) );
	}
	catch(Exception e)
	{
