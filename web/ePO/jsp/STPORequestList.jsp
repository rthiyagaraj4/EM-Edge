<!DOCTYPE html>
<%@ page import="ePO.STPORequestBean, ePO.STPORequestDetailBean,java.sql.*,java.util.*,webbeans.eCommon.*,java.io.UnsupportedEncodingException,
	...
	<body >
		<form name='formPORequestList' id='formPORequestList'>
			<table name ="tblItemList" border='1' width='100%' cellpadding='0' cellspacing='0' >
 			<tr>
				<th width='2%'><fmt:message key="ePO.S.label" bundle="${po_labels}"/></th>
				...
				<th nowrap><fmt:message key="ePO.ReqQty.label" bundle="${po_labels}"/></th>
				<th nowrap><fmt:message key="Common.uom.label" bundle="${common_labels}"/></th>
				<th nowrap><fmt:message key="ePO.RequiredByDate.label" bundle="${po_labels}"/></th>
				<th nowrap><fmt:message key="ePO.ItemRemarks.label" bundle="${po_labels}"/></th>
			</tr>
			<%
				...
				<tr id="tr_<%=i%>">
					...
					<td class="<%=className%>" nowrap align='left'><%=bean.checkForNull(bean.getUOMDesc(detailBean.getPurUomCode()),"&nbsp;")%></td>
					<td class="<%=className%>" nowrap align='right'><%=bean.checkForNull(detailBean.getRequired_by_date(),"&nbsp;")%>&nbsp;</td>
					<td class="<%=className%>" nowrap align='center' onClick = "showRemarks1('<%=remarks%>','disabled')" onMouseOver="changeCursor(this);"><font class='HYPERLINK'><fmt:message key="Common.remarks.label" bundle="${common_labels}"/></font></td>
				</tr>
