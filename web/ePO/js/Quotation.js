...
function create() {
	f_query_add_mod.location.href = "../../ePO/jsp/QuotationFrame.jsp?mode=" + MODE_INSERT + "&function_id=" + function_id;
}
...
function query() {
	f_query_add_mod.location.href = "../../ePO/jsp/QuotationQueryFrame.jsp?function_id=" + function_id;
}
...
async function deleterecord() {
	if( f_query_add_mod.QuotHeaderFrame==undefined){
		messageFrame.location.href = "../../eCommon/jsp/error.jsp?error_value=0&err_num=";
	}else{
	var formHeader = f_query_add_mod.QuotHeaderFrame.document.formQuotationHeader;
...
function addItemRecord() {
	...
	var arrayObject = new Array(formObject.item_desc,formObject.quotation_qty,formObject.quotation_uom,formHeader.quotation_type,formHeader.supplier_code,formHeader.currency_code,formHeader.exc_rate);
	var names = new Array(getLabel("Common.item.label", "Common"),getLabel("ePO.QuotationQty.label", "PO"),getLabel("ePO.QuotationUOM.label", "PO"),getLabel("ePO.QuotType.label", "PO"),getLabel("ePO.Supplier.label", "PO"),getLabel("ePO.TransactionCurrency.label", "PO"),getLabel("ePO.ExchangeRate.label", "PO"));
	...
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name + "&function_type=" + function_type, false);
	xmlHttp.send(xmlDom);
	...
	parent.QuotListFrame.document.location.href = "../../ePO/jsp/QuotationList.jsp?index=" + index  +"&mode=" + MODE_INSERT + "&supp_code=" + supp_code + "&currency_code=" + currency_code +"&local_currency="+ local_currency  ;
	formObject.add.value = getLabel("Common.Add.label", "Common");
	formHeader.supplier_code.disabled = true;
	formHeader.quotation_type.disabled = true;
	...
	//	document.getElementById("batch_td").disabled = true;
		...
		var currency_code = formHeader.currency_code.value;
		document.location.href = "../../ePO/jsp/QuotationDetails.jsp?mode=" + MODE_INSERT +  "&to_supplier=" + supplier_code + "&currency_code=" + currency_code +"&local_currency="+ local_currency + "&function_id=" + parent.parent.function_id ;
...
function updateCheckStatus(obj_item_code) {
	var check_status = "";
	var formObj = document.formQuotationList;
	assignBean(formObj);
	...
	var xmlDoc="";
	xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?item_code=" + obj_item_code.value + "&check_status=" + check_status + "&bean_id=" + formObj.bean_id.value + "&bean_name=" + formObj.bean_name.value + "&function_type=UPDATECHECKSTATUS", false);
	xmlHttp.send(xmlDoc);
...
function displayItemDetail(idx) {
	...
	var item_code = eval("document.formQuotationList.item_code_" + idx + ".value");
	var po_no  =  eval("document.formQuotationList.po_no_" + idx + ".value");
	...
	parent.QuotDetailFrame.document.location.href = "../../ePO/jsp/QuotationDetails.jsp?mode=" + MODE_MODIFY + "&function_id=" + parent.parent.function_id + "&item_code=" + item_code  + "&po_no=" + po_no + "&request_no=" + request_no +"&delv_date="+ delv_date + "&to_supplier=" + to_supplier + "&currency_code=" + currency_code +"&local_currency="+ local_currency +"&edit=" + document.formQuotationList.mode.value;
	hide = 'yes';
	resizeFrames();
}
...
function resetDetail() {
	document.location.href = "../../ePO/jsp/QuotationDetails.jsp?mode=" + MODE_INSERT + "&function_id=" + parent.parent.function_id;
...
async function apply() {
	...
	var formObject = f_query_add_mod.QuotListFrame.document.formQuotationList;
...
/*		
	...
	*/
	...
	formObject.quotation_type.value = formHeader.quotation_type.value;
...
function onSuccess() {
	var formObject = f_query_add_mod.QuotHeaderFrame.document.formQuotationHeader;
...
function Modify(obj) {
	...
	var quot_no = obj.cells[1].innerText;
	var quotation_type_code = obj.cells[4].innerText;
	
	parent.document.location.href = "../../ePO/jsp/QuotationFrame.jsp?mode=" + MODE_MODIFY + "&function_id=" + parent.parent.function_id  + "&quotation_no="+quot_no+ "&quotation_type_code="+quotation_type_code ;
...
function onLoadCurrCode(obj_currency,local_currency){
		...
		if(parent.QuotDetailFrame.document.getElementById("currency_label_1")!=undefined){
			parent.QuotDetailFrame.document.getElementById("currency_label_1").innerHTML =  obj_currency ; 
			parent.QuotDetailFrame.document.getElementById("currency_label_2").innerHTML =  obj_currency ; 
			parent.QuotDetailFrame.document.getElementById("currency_label_3").innerHTML =  obj_currency ; 
			parent.QuotDetailFrame.document.getElementById("currency_label_4").innerHTML =  obj_currency ; 
		}	
		if(parent.QuotListFrame.document.getElementById("currency_label_5")!=undefined){
			parent.QuotListFrame.document.getElementById("currency_label_5").innerHTML =  obj_currency  ; 
			parent.QuotListFrame.document.getElementById("currency_label_6").innerHTML =  obj_currency  ; 
			parent.QuotListFrame.document.getElementById("currency_label_7").innerHTML =  obj_currency  ;
			parent.QuotListFrame.document.getElementById("currency_label_8").innerHTML =  local_currency  ;
		}
...
function calculateValue(){
		...
		var unit_price = formObject.quotation_price.value ; 
...
function LoadTrade(item_code){
	if(item_code!=""){
		...
		var xmlDoc="";
		xmlDoc.loadXML(getXMLString(null));
		xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name + "&item_code=" + item_code + "&function_type=LOADTRADEID", false);
		xmlHttp.send(xmlDoc);
		responseText = xmlHttp.responseText;
	//	alert(responseText);
		eval(responseText);
...
function MoreItemDetails1(bean_id, bean_name, mode,item_code,item_desc,quotation_qty,quot_uom_desc,quot_uom_code) {
	
	function_id = document.forms[0].function_id.value;

//	alert("mode = "+mode);
	var tit = "TITLE";
	var retVal = window.showModalDialog("../../ePO/jsp/QuotMoreDetails.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name +  "&function_id=" + function_id + "&mode=" + mode + "&item_code=" + item_code +"&quot_qty=" + quotation_qty +"&temp_uom=" + quot_uom_desc +"&temp_uom_code=" + quot_uom_code+"&item_desc=" + item_desc, "", "dialogHeight:25; dialogWidth: 60; right: 1; status: no; dialogTop :300;scroll:no");
	
	
}

function MoreItemDetails(bean_id, bean_name, mode) {

	var formObj = document.formQuotationDetail;
	function_id = document.forms[0].function_id.value;

//	alert("mode jjj= "+mode);
	var tit = "TITLE";
	var retVal = window.showModalDialog("../../ePO/jsp/QuotMoreDetails.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name +  "&function_id=" + function_id + "&mode=" + mode + "&item_code=" + formObj.item_code.value +"&quot_qty=" + formObj.quotation_qty.value +"&temp_uom=" + formObj.temp_delv_uom_desc.value +"&temp_uom_code=" + formObj.quotation_uom.value+"&item_desc=" + formObj.item_desc.value, "", "dialogHeight:25; dialogWidth: 60; right: 1; status: no; dialogTop :300;scroll:no");
	
	
}


function exitMoreItemDetails(){

	var formObj = document.formQuotMoreDetails;
	
	var xmlHttp = new XMLHttpRequest();
	var xmlDoc="";
	xmlDoc.loadXML(getEnhancedXMLString(formObj));
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?bean_id=" + formObj.bean_id.value + "&bean_name=" + formObj.bean_name.value+ "&item_code=" + formObj.item_code.value + "&function_type=SETMOREITEMVALUES", false);
	xmlHttp.send(xmlDoc);
	responseText = xmlHttp.responseText;
	//	alert(responseText);
	eval(responseText);
...
function SelectionCriteria1(bean_id, bean_name, mode,item_code,item_desc,quot_qty,quot_uom_desc,quot_uom_code) {
...
//	var formObj = document.formQuotationDetail;
	...
	//alert("mode = "+mode);
	var tit = "TITLE";
	var retVal = window.showModalDialog("../../ePO/jsp/QuotationSelectionCriteria.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name +  "&function_id=" + function_id + "&mode=" + mode + "&item_code=" + item_code +"&quot_qty=" + quot_qty +"&temp_uom=" + quot_uom_desc +"&temp_uom_code=" + quot_uom_code+"&item_desc=" + item_desc + "&mode_modify="+ mode_modify, "", "dialogHeight:15; dialogWidth: 40; right: 1; status: no; dialogTop :300;scroll:yes");
	
	
}


function SelectionCriteria(bean_id, bean_name, mode) {
	...
	//alert("mode = "+mode);
	var tit = "TITLE";
	var retVal = window.showModalDialog("../../ePO/jsp/QuotationSelectionCriteria.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name +  "&function_id=" + function_id + "&mode=" + mode + "&item_code=" + formObj.item_code.value +"&quot_qty=" + formObj.quotation_qty.value +"&temp_uom=" + formObj.temp_delv_uom_desc.value +"&temp_uom_code=" + formObj.quotation_uom.value+"&item_desc=" + formObj.item_desc.value +"&mode_modify="+ mode_modify, "", "dialogHeight:15; dialogWidth: 40; right: 1; status: no; dialogTop :300;scroll:yes");
	
	
}


function exitSelectionCriteria(){

	var formObj = document.QuotationSelectionCriteriaForm;
	
	for(var j=0;j<formObj.totalRecords.value;j++){
		var master_code     = eval("document.QuotationSelectionCriteriaForm.master_code_"+j).value;
		
		if(eval("document.QuotationSelectionCriteriaForm.selection_status_"+j).checked){
			eval("document.QuotationSelectionCriteriaForm.selection_status_"+j).value = "Y";
		}else{
			eval("document.QuotationSelectionCriteriaForm.selection_status_"+j).value = "N";
			}
		
		for(var k = 0; k < formObj.totalRecords.value; k++) {
			var master_code1     = eval("document.QuotationSelectionCriteriaForm.master_code_"+k).value;
				if(master_code!="" && master_code1 != "" && master_code==master_code1 && j!=k){
	...
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?bean_id=" + formObj.bean_id.value + "&bean_name=" + formObj.bean_name.value+ "&item_code=" + formObj.item_code.value + "&function_type=SETSELECTIONCRITERIAVALUES", false);
	xmlHttp.send(xmlDoc);
	responseText = xmlHttp.responseText;
	eval(responseText);
	window.returnValue = result;
...
function EvaluationFactor1(bean_id, bean_name, mode,item_code,item_desc,quot_qty,quot_uom_desc,quot_uom_code) {

//	var formObj = document.formQuotationDetail;
	function_id = document.forms[0].function_id.value;
	var mode_modify = 2;
	var tit = "TITLE";
	var retVal = window.showModalDialog("../../ePO/jsp/QuotEvaluationFactor.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name +  "&function_id=" + function_id + "&mode=" + mode + "&item_code=" + item_code +"&quot_qty=" + quot_qty +"&temp_uom=" + quot_uom_desc +"&temp_uom_code=" + quot_uom_code+"&item_desc=" + item_desc +"&mode_modify="+ mode_modify, "", "dialogHeight:15; dialogWidth: 50; right: 1; status: no; dialogTop :300;scroll:yes");
...
function EvaluationFactor(bean_id, bean_name, mode) {

	var formObj = document.formQuotationDetail;
	function_id = document.forms[0].function_id.value;
	var mode_modify = 1;
	var tit = "TITLE";
	var retVal = window.showModalDialog("../../ePO/jsp/QuotEvaluationFactor.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name +  "&function_id=" + function_id + "&mode=" + mode + "&item_code=" + formObj.item_code.value +"&quot_qty=" + formObj.quotation_qty.value +"&temp_uom=" + formObj.temp_delv_uom_desc.value +"&temp_uom_code=" + formObj.quotation_uom.value+"&item_desc=" + formObj.item_desc.value + "&mode_modify="+ mode_modify, "", "dialogHeight:15; dialogWidth: 50; right: 1; status: no; dialogTop :300;scroll:yes");
	
	
}


function exitEvaluationFactor(){

	var formObj = document.EvaluationFactorForm;
	
	for(var j=0;j<formObj.totalRecords.value;j++){
	
		var master_code     = eval("document.EvaluationFactorForm.master_code_"+j).value;
		/*if(eval("document.EvaluationFactorForm.recommended_"+j).checked)
			eval("document.EvaluationFactorForm.recommended_"+j).value = "Y";
		else
			eval("document.EvaluationFactorForm.recommended_"+j).value = "N";
		*/	
		for(var k = 0; k < formObj.totalRecords.value; k++) {
			var master_code1     = eval("document.EvaluationFactorForm.master_code_"+k).value;
				if(master_code!="" && master_code1 != "" && master_code==master_code1 && j!=k){
					alert(getMessage("DUPLICATE_NOT_POSSIBLE", "PO"));
					return;
				}
	...
	var xmlDoc="";
	xmlDoc.loadXML(getEnhancedXMLString(formObj));
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?bean_id=" + formObj.bean_id.value + "&bean_name=" + formObj.bean_name.value+ "&item_code=" + formObj.item_code.value + "&function_type=SETEVALUATAIONFACTORVALUES", false);
	xmlHttp.send(xmlDoc);
	responseText = xmlHttp.responseText;
	eval(responseText);
	window.returnValue = result;
...
function AdditionalDetails(bean_id, bean_name, mode) {
	...
	var formObj = document.formQuotationHeader;
	function_id = document.forms[0].function_id.value;
	var tit = "TITLE";
	var retVal = window.showModalDialog("../../ePO/jsp/QuotationAdditionalDetails.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name +  "&function_id=" + function_id + "&mode=" + mode + "&supp_code=" + formObj.supplier_code.value , "", "dialogHeight:25; dialogWidth: 70; center: 1; status: no; dialogTop :300;scroll:no");
...
var formObj = document.formQuotAdditionalDetails;

	var result = checkQuotValidate();
	if(!result){
	...
	var xmlDoc="";
		...
		xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?bean_id=" + formObj.bean_id.value + "&bean_name=" + formObj.bean_name.value +  "&contact_person=" + formObj.contact_person.value +  "&pmnt_term=" + formObj.pmnt_term.value +  "&advt_date=" + formObj.advt_date.value +  "&terms_conditions=" + formObj.terms_conditions.value +    "&delivery_mode=" + formObj.delivery_mode.value +  "&quotation_remarks=" + formObj.quotation_remarks.value +  "&open_date=" + formObj.open_date.value +  "&closed_date=" + formObj.closed_date.value +  "&approved_date=" + formObj.approved_date.value +  "&drug_purchase_date=" + formObj.drug_purchase_date.value +  "&clerk_date=" + formObj.clerk_date.value +  "&initial_eval_date=" + formObj.initial_eval_date.value +  "&qc_res_date=" + formObj.qc_res_date.value +  "&checked_date=" + formObj.checked_date.value +  "&meeting_date=" + formObj.meeting_date.value +  "&after_meeting_date=" + formObj.after_meeting_date.value +  "&submitted_date=" + formObj.submitted_date.value+ "&function_type=SETADDITIONALDETAILS", false);
		xmlHttp.send(xmlDoc);
		responseText = xmlHttp.responseText;
		eval(responseText);
		window.close();
...
function ItemRemarks(remarks,codedisabled) {
	...
	var rem = "";
	rem = window.showModalDialog("../../ePO/jsp/DOItemRemarks.jsp?function_id=" + parent.parent.function_id + "&remarks=" +remarks + "&codedisabled=" +codedisabled, "", "dialogHeight:10; dialogWidth:30; center:0; status: no; dialogLeft :75; dialogTop :250;");
	if (rem != null && codedisabled=="") {
...
function loadCurrency(obj_supp) {
	...
	//alert('loadCurrency');
	var formObj = document.formQuotationHeader;
	formObj.supplier_code.value = obj_supp.value;
	var xmlHttp = new XMLHttpRequest();
	var xmlDoc="";
	xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?function_type=LOADCURRENCY&supplier=" + obj_supp.value + "&localCurrency=" + formObj.local_currency.value + "&bean_id=" + formObj.bean_id.value + "&bean_name=" + formObj.bean_name.value, false);
	xmlHttp.send(xmlDoc);
	responseText = xmlHttp.responseText;
	//alert(responseText);
	eval(responseText);
...
function getUOMDesc(obj_uom) {
	...
	var xmlDoc="";
	xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?function_type=GETUOMDESC&uom_code=" + obj_uom.value + "&bean_id=" + formObj.bean_id.value + "&bean_name=" + formObj.bean_name.value, false);
	xmlHttp.send(xmlDoc);
	responseText = xmlHttp.responseText;
	eval(responseText);
...
function searchCodeItem(obj) {
	...
	//var arrayObject = new Array(formHeader.quotation_type, formHeader.supplier_code);
	//var names = new Array(getLabel("ePO.QuotType.label", "PO"),getLabel("ePO.Supplier.label", "PO"));
	
		var arrayObject = new Array(formHeader.quotation_type,formHeader.supplier_code,formHeader.currency_code,formHeader.exc_rate);
	var names = new Array(getLabel("ePO.QuotType.label", "PO"),getLabel("ePO.Supplier.label", "PO"),getLabel("ePO.TransactionCurrency.label", "PO"),getLabel("ePO.ExchangeRate.label", "PO"));
	...
	if ((returnedValues != null) && (returnedValues != "")) {
		...
		document.formQuotationDetail.conv_fact.value = "1";
		document.formQuotationDetail.quotation_qty.focus();
	...
	if(formObject.item_code.value!=""){
		...
		var xmlDoc="";
		xmlDoc.loadXML(getXMLString(null));
		xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name + "&item_code=" + formObject.item_code.value + "&function_type=MM_UOM", false);
		xmlHttp.send(xmlDoc);
		responseText = xmlHttp.responseText;
		eval(responseText);
		}
...
function resizeFrames(){	
	...
	if (parent.QuotationFrmS)
	{
		fullFrame = parent.QuotationFrmS;
	}

	if(hide == 'no')
	{
		if (parent.QuotBlankFrame)
		{
			if(parent.QuotBlankFrame.document.getElementById('hideImg'))
			{
				parent.QuotBlankFrame.document.getElementById('hideImg').src='../../eST/images/POright.png'
				fullFrame.cols = '0px,14px,*';
				parent.QuotListFrame.document.getElementById('quotList').style.width = "1340px";
				hide='yes';
			}	
		}	
	}
	else if(hide == 'yes')
	{
		if (parent.QuotBlankFrame){
			if(parent.QuotBlankFrame.document.getElementById('hideImg'))
			{
				parent.QuotBlankFrame.document.getElementById('hideImg').src='../../eST/images/POleft.png'			
				fullFrame.cols = '350px,14px,*';
				parent.QuotListFrame.document.getElementById('quotList').style.width = "980px";
				hide='no';
			}
		}
	}
...
function checkValidateUpto(obj_validDate){
	...
	if (expDate.length > 0 && valDate.length > 0) {
		...
		//	alert("Valid upto date should be greater than or equal to system date");
...
//	var arrayObject = new Array();
	...
	assignBean(f_query_add_mod.QuotHeaderFrame.document.formQuotationHeader);
	var xmlDom = new ActiveXObject("Microsoft.XMLDom");
	var xmlHttp = new XMLHttpRequest();
	xmlStr = getXMLString(null);
	xmlDom.loadXML(xmlStr);
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?bean_id=" + bean_id + "&bean_name=" + bean_name + "&function_type=CHECKFORAUTHORIZE", false);
	xmlHttp.send(xmlDom);
	responseText = xmlHttp.responseText;
//	alert(responseText);
	eval(responseText);
...
function setAuthorize(authorizeValue) {
	f_query_add_mod.QuotHeaderFrame.document.formQuotationHeader.authorize_yn.value = authorizeValue;
}
...
function checkUOMConvFactor() {
	...
	var xmlDoc="";
	xmlDoc.loadXML(getXMLString(null));
	xmlHttp.open("POST", "../../ePO/jsp/QuotationValidate.jsp?uom_code=" + formObj.uom.value + "&pur_uom_code=" + formObj.temp_uom_code.value + "&bean_id=" + formObj.bean_id.value + "&bean_name=" + formObj.bean_name.value + "&function_type=CHECKUOMCONVFACTOR" + "&item_code=" + formObj.item_code.value, false);
	xmlHttp.send(xmlDoc);
	var responseText = xmlHttp.responseText;
	eval(responseText);
	if (!result) {
...
function loadExRate(){
		...
		if(parent.QuotHeaderFrame.document.getElementById("exc_rate") != undefined ){
			var formHeader = parent.QuotHeaderFrame.document.formQuotationHeader;
...
function loadExRateHdr(){
		...
		if(parent.QuotHeaderFrame.document.getElementById("exc_rate") != undefined ){
			var formHeader = parent.QuotHeaderFrame.document.formQuotationHeader;
			var formList = parent.QuotListFrame.document.formQuotationList;
			var exc_rate = formHeader.exc_rate.value;
			var net_poAmt = formList.net_poAmt.value ;
			var netLC;
...
function checkQuotValidate(){
	var formObject = document.formQuotAdditionalDetails;
	
	var toDateArray;
	var fromDateArray;
	var toDate = formObject.closed_date.value;
	var fromDate = formObject.open_date.value;
	if (fromDate.length > 0 && toDate.length > 0) {
		...
		//	alert("Closed date should be greater than or equal to Open date");
			alert(getMessage("CLOSED_DATE_GR_OPEN_DATE","PO"));  
				formObject.closed_date.select();
				formObject.closed_date.focus();
				return true;
		}
...
function checkValidateQryDate(){
	...
	var formObject = document.formQuotationQueryCriteria;
	...
	if (fromDate.length > 0 && toDate.length > 0) {
		...
		//	alert("To date should be greater than or equal to From date");
			...
			document.formQuotationQueryCriteria.submit();
		}
...
function sampleChange(obj_sample_type){
	if(obj_sample_type.value == "Y")
		document.formQuotMoreDetails.all.sample_qty.disabled=false;
	else
		document.formQuotMoreDetails.all.sample_qty.disabled=true;
}
...
function maxWeightageLimit(obj_value,index){

		var formObject = document.EvaluationFactorForm;
		var weightage = eval ("document.EvaluationFactorForm.weightage_"+index+".value");
				
		if(weightage > 100){
		 //  alert("Weightage Percentage should be less than 100");
		   alert(getMessage("WEIGHTAGE_PER_LS_100","PO"));   
		   eval ("document.EvaluationFactorForm.weightage_"+index+".focus()");
		}
		
}


function addRowSel(tableID){ 
			var formObject = document.QuotationSelectionCriteriaForm;
			var table = document.getElementById('mySelTable');
			...
			switch(newcell.childNodes[0].type) {
				...
				case "checkbox":
					...
					newcell.setAttribute("align", "center");//Added By Sakti Sankar
                    break;	
                }
			}
					
			
			formObject.totalRecords.value = rowCount;
...
function setStatus(obj,i){
	if(obj.value != null && obj.value != "")
		eval(document.getElementById("selection_status_" + i)).checked = true;
	else	
		eval(document.getElementById("selection_status_" + i)).checked = false;
}
