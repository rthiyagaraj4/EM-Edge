		...
		%>
			...
			<%
				if(language_direction.equals("R")){%>
					<td class=data><INPUT TYPE="text" name="desc<%=colDispCount%>0" id="desc<%=colDispCount%>0" onblur='return makeValidString(this);'	size=50 maxlength='<%=columnLen.get(colDispCount)%>' value='<%=rs.getString((String)columnVal.get(colDispCount))==null?"":rs.getString((String)columnVal.get(colDispCount))%>' dir='RTL'>  </td>
				<%}else{%>
			...
			<%
				...
				for(startVal=0;startVal<columnVal.size();startVal++){
					...
					<%}else{
						if(language_direction.equals("R")){%>
							<td class='fields' style='<%=classValue%>' ><INPUT TYPE="text" NAME=desc<%=colCount1++%><%=colCount%>  onkeypress='cheeckText(event)' onblur='return makeValidString(this);' maxlength='<%=columnLen.get(startVal)%>' value="<%=rs.getString((String)columnVal.get(startVal))==null?"":rs.getString((String)columnVal.get(startVal))%>" size=70 dir='RTL'></td> 
						<%}else{%>
							<td class='fields' style='<%=classValue%>' ><INPUT TYPE="text" NAME=desc<%=colCount1++%><%=colCount%>  onkeypress='cheeckText(event)' onblur='return makeValidString(this);' maxlength='<%=columnLen.get(startVal)%>' value="<%=rs.getString((String)columnVal.get(startVal))==null?"":rs.getString((String)columnVal.get(startVal))%>" size=70></td> 
						<%}
					}
				}	 
			%>
