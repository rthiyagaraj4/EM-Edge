...
request.setCharacterEncoding("UTF-8");	
	...
	if ( !(grade_code == null || grade_code.equals("")) )
		{
		sql.setLength(0);
		sql.append("where upper(grade_code) like upper('"+grade_code+"%')");
		andCheck = 1;
		}
...
int maxRecord = 0;
String strsql2="select grade_code,long_desc,short_desc,eff_status from am_grade "+sql.toString();
...
<%
if ( !(start <= 1) )
	out.println("<td align ='right' id='prev'><A HREF='../../eAM/jsp/GradeQuery.jsp?function_id="+function_id+"&from="+(start-14)+"&to="+(end-14)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A></td>");

	out.println("<td align ='right' id='next' style='visibility:hidden'><A HREF='../../eAM/jsp/GradeQuery.jsp?function_id="+function_id+"&from="+(start+14)+"&to="+(end+14)+"&whereclause="+URLEncoder.encode(sql.toString())+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A></td>");
...
<%}
	...
	<td class=<%=classValue%>><a href='../../eAM/jsp/GradeAdd.jsp?function_id=<%=function_id%>&grade_code=<%=code%>' target='f_query_add_mod' ><%=code%></a></td>
