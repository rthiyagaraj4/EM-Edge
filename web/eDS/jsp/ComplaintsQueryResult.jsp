...
try{
    ...
    String sql = " ";
    String complaints = request.getParameter("complaints")==null?"":request.getParameter( "complaints" ).trim().toUpperCase();
	...
	int cnt=0;
		...
		if ( !(complaints == null || complaints.equals("")) ){
	        sql = sql + " where upper(complaint_code) like upper('"+complaints +"%')";
			cnt++;
        }
...
ResultSet rs = null;
	...
	try{
		...
		String strsql="select count(*) as total from ds_complaints "+sql;
		String strsql1="select complaint_code,long_desc,short_desc,eff_status from ds_complaints "+sql;
		...
		System.err.println("Exception in ComplaintsQueryResult.jsp"+e);
		e.printStackTrace();
	}
%>
...
<%
if ( !(start <= 1) )
    out.println("<A HREF='../../eDS/jsp/ComplaintsQueryResult.jsp?from="+(start-14)+"&ph_module_yn="+ph_module_yn+"&to="+(end-14)+"&whereclause=" + URLEncoder.encode(sql) + "'" + " text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.previous.label","common_labels")+"</A>");

if ( !( (start+14) > maxRecord ) )
    out.println("<A HREF='../../eDS/jsp/ComplaintsQueryResult.jsp?from="+(start+14)+"&ph_module_yn="+ph_module_yn+"&to="+(end+14)+"&whereclause="+URLEncoder.encode(sql)+"'"+" text-decoration='none'>"+com.ehis.util.BundleMessage.getBundleMessage(pageContext,"Common.next.label","common_labels")+"</A>");
%>
</td>
</tr>
</table>
...
<th class="columnheadercenter" nowrap>
	<fmt:message key="eDS.ComplaintReasonCode.Label" bundle="${ds_labels}"/><!-- Changed Against PMG2013-CRF-0009.1-US1-->
</th>
<th class="columnheadercenter" nowrap>
      ...
      while ( rset.next() && i<=end  )
{
	...
	String mode="modify";
	out.println("<a href='../../eDS/jsp/ComplaintsAddModify.jsp?mode="+mode+"&complaints="+ complaints+"' target='f_query_add_mod' >");
    out.println(complaints+"</a></td>");
