...
function populateLocation(location_type){
	...
	xmlHttp.open( "POST", "PendingAppointmentValidate.jsp?bean_id=" + bean_id + "&default_val=''&validate=ord_cat&localeName="+localeName, false ) ;
...
//alert(retVal[0]+"=="+retVal[1]);
	if(retVal != null && retVal != ""){
		document.cosign_order.locn.value=retVal[0];
...
function authorise(){
	...
	var frmSum=parent.parent.parent.frames[0].document.cosign_order.frmSum.value

	qry_string = "../jsp/PendingAppointmentBottom.jsp?location_type="+locn_type+"&locn="+locn+"&patientId="+patientId+"&period_from="+period_from+"&period_to="+period_to+"&frmSum="+frmSum;
...
if(CheckDate(Object2)==false)
 ...
 function  callAppointmentBooking1(obj,patient_id,location_code,location_type,ordering_practitioner,practitioner_name,specialty_code,order_id,order_line_num,catalog_code,start_date_time,period_to,catalog_code_dc,orderd_at)
  {			 
			 while (practitioner_name.indexOf("~##") != -1)
			 {
	...
	if(obj.checked==true)
  {
	  if(catalog_code=="OT")
	  {
		/*  var orderd_at=orderd_at;
		  alert("orderd_at"+orderd_at)
		 orderd_at=  orderd_at.split(" ");
		 alert(orderd_at[0]);
		  orderd_at=orderd_at[0]; */
		 callOTAppointment(order_id, ordering_practitioner,practitioner_name, period_to)
	 ...
	 {	   		
		  ...
		  //alert("patient_id"+patient_id+"location_code"+location_code+"ordering_practitioner"+ordering_practitioner+"order_id"+order_id+"order_line_num"+order_line_num+"catalog_code_dc"+catalog_code_dc+"period_to"+period_to)	 ;
		 var retvalue= OABookAppointment(patient_id, location_code, "", ordering_practitioner, "", order_id, order_line_num, catalog_code_dc, period_to,"","","","","","","","");
         //callAppointmentBooking(patient_id,location_code,location_type,ordering_practitioner,specialty_code,order_id,order_line_num,catalog_code,start_date_time,catalog_code_dc)
			...
			parent.parent.parent.CosignOrderTop.document.cosign_order.search_btn.disabled = true;
			parent.parent.parent.CosignOrderTop.document.cosign_order.submit();
   }
}
