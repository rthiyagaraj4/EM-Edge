...
<tr id="heading">
		...
		<td class=columnheader width='' nowrap colspan="2"><fmt:message key="Common.EventStatus.label" bundle="${common_labels}"/></td>
		...
		<tr>
			...
			<td width='' nowrap class='<%=slClassValue%>'><font size=1><%=record[2]%>
				...
				{%>
					...
					<a class='gridLink' href='#' onClick="getFile('<%=histrectype%>','OR','<%=record[28]%>','<%=record[3]%>','<%=temp_img%>','<%=patient_id%>','<%=record[29]%>','<%=record[8]%>','<%=encounter_id%>)"><img src="../../eCA/images/<%=temp_img%>.gif" width='16' height='16' border=0 alt='<%=temp_img%>'></a>
...
<%
			...
			<td width='' nowrap class="<%=slClassValue%>">
				...
				<input type='hidden' name='min_value_<%=i%>' id='min_value_<%=i%>' value='<%=record1[6].trim()%>'>
				<input type='hidden' name='max_value_<%=i%>' id='max_value_<%=i%>' value='<%=record1[7].trim()%>'>
...
%>
			...
			{
			%>
			<td nowrap class="<%=slClassValue%>">
				<a class='gridLink' href='#' onclick='uploadDocuments("<%=i%>")'>Upload</a>
			</td>
			<%
			}
			%>
...
<input type=hidden name='report_srl_no' id='report_srl_no' value='<%=order_line_num%>'>
...
<Input type='hidden' name='ordCatLinked' id='ordCatLinked' value='<%=ordCatLinked%>'>
<Input type='hidden' name='from' id='from' value='<%=from%>'>
<Input type='hidden' name='option_id' id='option_id' value='<%=option_id%>'>
<!--Added By Sridhar Reddy on 01/060/2010 for PMG20089-CRF-0804-->
<Input type='hidden' name='result_modify_yn' id='result_modify_yn' value='<%=result_modify_yn%>'>
<!--End-->
</table>
</div>
<%
