		...
		<tr>
			...
			<% if((appt_reqd_yn.equals("Y")) && (!appt_ref_num.equals("")) && (!appt_date_time.equals(""))){%><br>
			   <% if( (order_category.equals("RD") && rd_install.equals("Y")) || 	(order_category.equals("OT") && ot_install.equals("Y"))	 ) { %>
				...
				<%  } else { %>
					<a class="gridLink" href="javascript:viewOACompleteApptData(escape('<%=i%>'))">Appt.Dtls</a>
				<%  } %>
			...
			if(record[29].equals("*ALL")){
				ChargeTypeDesc="All";
			}else{
				ChargeTypeDesc=(String)bean.getChargeTypeDesc(record[14],record[29]);
			}

			if(record[15].equalsIgnoreCase("Y")){				
				%>
			<a class="gridLink" href="javascript:ChangeChargeType(escape('<%=i%>'))"><%=ChargeTypeDesc%></a>
			<!-- <input type=checkbox name='chk<%=i%>' id='chk<%=i%>' onClick="checkOrderable('<%=i%>')"> -->
			<%}else{%>
			<%=ChargeTypeDesc%>
			<%}%>
				<INPUT TYPE="hidden" name="charge_type_code<%=i%>" id="charge_type_code<%=i%>" value="<%=record[29]%>">
				<INPUT TYPE="hidden" name="charge_type_desc<%=i%>" id="charge_type_desc<%=i%>" value="<%=ChargeTypeDesc%>">
			</td>
		</tr>
