...
<form name="place_order_pract_bottom" id="place_order_pract_bottom">
	...
	<th class="columnheader" width='75%'><%=disp_label%></th>
	...
	try{
		if(tabVal.equals("S"))
		{
			...
			for(int i=fm_disp; i<=to_disp; i++)
			{
				...
				%>
				<tr id='<%=tblrow_id%>'>
				<td class='<%=classValue%>' align='left'><%=((long_desc.equals(""))?"&nbsp;":long_desc)%></td>
		...
		else if(tabVal.equals("A"))
		{
			...
			<%if(valid.equals("Y"))
			{%>
				<td class='<%=classValue%>' align='left'><b><%=((long_desc.equals(""))?"&nbsp;":long_desc)%></b></td>
			<%
			}
			else
			{%>
	...
	<input type="hidden" name="allowed_type" id="allowed_type" value="<%=allowed_type%>">
</body>
</html>		
<%
