	 ...
	 <%
						...
						report_srl_no	= value_arr[0];
							...
							else
							{
								...
								{		
									...
									if(value_arr[8] != null && (!(value_arr[8].equals(""))) && !"".equals(CommonBean.checkForNull(value_arr[20],"").trim()))
									{
										...
										%>
										<PRE CLASS="<%=textClassValue%>" style="word-wrap:break-word;"><%=value_arr[8]%></PRE>
										<%
										/*if(value_arr[8].length() > 40)
										{
										%>
										<textarea rows="5" cols="90"  CLASS = "<%=textClassValue%>" READONLY title = "<%=toolTipTextArea%>" ondblclick="viewClobData('<%=value_arr[21]%>')"><%=value_arr[8]%></textarea>
										<%								 
										}else{
										%>
											<%=value_arr[8]%>
							...
							{	
								...
								//Bru-HIMS-CRF-195.3 - IN049578[IN047205] - End	
								%>
									<a class="gridLink"  href="javascript:getExtLink('<%=patient_id%>','<%=ext_encounter_id%>','<%=accession_num%>','<%=orderId%>','<%=perfFacilityId%>','<%=locale%>','<%=login_user_id%>','<%=resp_id%>','<%=login_at_ws_no%>','<%=ord_type_code%>','<%=op_episode_visit_num%>','<%=attend_practitioner_id%>','<%=assign_care_locn_type%>','<%=assign_care_locn_code%>','<%=visit_adm_date%>','<%=epis_type%>','<%=location_code%>','<%=ext_image_appli_id%>','<%=ext_image_obj_id_intf%>','<%=ext_image_source%>','<%=ext_srce_doc_ref_no%>','<%=ext_srce_doc_ref_line_no%>')"><img src='../../eCommon/images/ExternalDocument.PNG' title='External Link' width='16' height='16' border=0 alt='Result'></a>  <!-- IN47847 Added title to img tag --> <!-- IN049589 Passing peforming facility_id[perfFacilityId] instead of login facility_id[facility_id] -->
								<%
								}	//Bru-HIMS-CRF-195.3 -  IN049578[IN047205]
							}
							// added for IN047204 - End
				...
				else if(study_id.equals("") && !param_def_str.contains("~STUDY_ID`"))
				{
					...
					ext_img_display="Result";
							...
							if ( (ext_image_appl_id != null && (!ext_image_appl_id.equals("")))) 
							{
								...
								%>
									<%-- <a class="gridLink"  href="javascript:getFile('<%=hist_rec_type%>','<%=contr_sys_id%>','<%=ext_image_obj_id%>','<%=contr_sys_event_code%>','<%=ext_image_appl_id%>','<%=patient_id%>')"><img src='../../eCA/images/<%=ext_image_appl_id%>.gif'  width='16' height='16' border=0 alt='Result'></a>  <!--[IN033152] --> --%>
									 <%if(ext_img_disable.equals("disabled")) {%>
									 <a class="gridLink"><img src='../../eCA/images/<%=ext_image_appl_id%>.gif'  width='16' height='16' border=0 <%=ext_img_disable%> alt='<%=ext_img_display%>'></a>  <!--[IN033152] -->
									 <%}else{ %>
									<a class="gridLink"  href="javascript:getFile('<%=hist_rec_type%>','<%=contr_sys_id%>','<%=ext_image_obj_id%>','<%=contr_sys_event_code%>','<%=ext_image_appl_id%>','<%=patient_id%>')"><img src='../../eCA/images/<%=ext_image_appl_id%>.gif' width='16' height='16' border=0 alt='<%=ext_img_display%>'></a>  <!--7902-->
								...
								//IN050936 included patient_id
								%>	
								<%-- <a class="gridLink"  href="javascript:getFile('<%=hist_rec_type%>','<%=contr_sys_id%>','<%=accession_num%>','<%=contr_sys_event_code%>','<%=ext_image_appl_id%>','<%=patient_id%>')"><img src='../../eCA/images/<%=ext_image_appl_id%>.gif' width='16' height='16' border=0 alt='Result'></a>  <!--[IN033152] --> --%>						
									 <%if(ext_img_disable.equals("disabled")) {%>
									 <a class="gridLink"><img src='../../eCA/images/<%=ext_image_appl_id%>.gif'  width='16' height='16' border=0 <%=ext_img_disable%> alt='<%=ext_img_display%>'></a>  <!--[IN033152] -->
									 <%}else{ %>
									 <a class="gridLink"  href="javascript:getFile('<%=hist_rec_type%>','<%=contr_sys_id%>','<%=accession_num%>','<%=contr_sys_event_code%>','<%=ext_image_appl_id%>','<%=patient_id%>')"><img src='../../eCA/images/<%=ext_image_appl_id%>.gif' width='16' height='16' border=0 alt='<%=ext_img_display%>'></a>  <!--7902--> 
								<%
									}
								}
			...
			out.println("<tr><td colspan='"+(panels.size()+2)+"' class='OR_QRYEVENBORDER'>&nbsp;</td></tr>");%><!--Modified Colspan for IN063813-->
			...
			<Input name="order_category" id="order_category" type="hidden" value="<%=ord_cat%>"><!-- IN039857-->
			<Input name="ord_typ_desc" id="ord_typ_desc" type="hidden" value="<%=print_details[7]%>"><!-- IN039857-->
			<Input name="cont_order_ind" id="cont_order_ind" type="hidden" value="<%=cont_order_ind%>"><!-- IN053439-->
			<Input name="performing_facility_id" id="performing_facility_id" type="hidden" value="<%=perfFacilityId%>"><!-- IN054720 -->
			</table>
	<%
			}
...
<Input name="view_bean_id" id="view_bean_id" type="hidden" value="<%=bean_id%>">
<Input name="view_bean_name" id="view_bean_name" type="hidden" value="<%=bean_name%>">
<Input name="patient_id" id="patient_id" type="hidden" value="<%=patient_id%>">
<Input name="orderId" id="orderId" type="hidden" value="<%=orderId%>">
<Input name="facility_id" id="facility_id" type="hidden" value="<%=facility_id%>">
