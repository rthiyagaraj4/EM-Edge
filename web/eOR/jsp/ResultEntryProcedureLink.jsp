...
<%
	...
	//for(int i=0; i<2; i++){
		...
		//{
				...
				%>
				<tr ><td colspan="4" class='CAHIGHERLEVELCOLOR'><%=record[15]%>
				<% 
				if(record[7].equals("M"))
				{
				%>
				...
				%><input type='hidden'  name='hid<%=record[5]%><%="-"+i%>' id='hid<%=record[5]%><%="-"+i%>'  value='<%=record[5]%>'></td>
				</tr>
				<%
			...
			<td width='40%' id='desc' class='<%=classValue%>'><font size=1><%=record[11]%></font></td>

			<td width='4%' class='<%=classValue%>'>
				<input type='hidden'  name='ordercatforeach-<%=i%>' id='ordercatforeach-<%=i%>'  value='<%=record[5]%>'><input type="checkbox" name="proc_chk<%=j%><%=k%>" id="proc_chk<%=j%><%=k%>" <%=checked%>  <%//=eHISFormatter.ChkBoxChecked("Y",record[9])%> <%=disabled%>>
				&nbsp;
...
<input type=hidden name='numberOfGroup' id='numberOfGroup' value='<%=l%>'>
<input type=hidden name='ordCatdisplink' id='ordCatdisplink' value='<%=displink%>'>
