...
<!-- 		<script language="JavaScript" src="../../eCommon/js/messages.js"></script>	
 -->		<!--  -->
		...
		<script language="javascript" src="../../eOR/js/RepOrderStat.js"></script>
<script src='../../eCommon/js/showModalDialog.js' language='JavaScript'></script>
	...
	<%
		...
		String bean_id      = "RepOrderStatBean" ;
		String bean_name    = "eOR.RepOrderStatBean";
		RepOrderStatBean  bean = (RepOrderStatBean)getBeanObject( bean_id,  bean_name, request ) ; 
		bean.setLanguageId(localeName);
		ArrayList facilityDetails = bean.getFacilityDetails();		//IN055319
		
		bean.clear() ;
		
		
		%>
	<form name="repOrderStatForm" id="repOrderStatForm" target="messageFrame">
	<table width='100%' align='center' valign='top'>
		<tr align="left"><th class='columnheader'><fmt:message key="eOR.OrderStatistics.label" bundle="${or_labels}"/>
		</th></tr>
		<%
			/*Thai date validations 16/02/2007 start*/
			String to_date = "";
		...
		<table border=0 width='100%' cellPadding="3" cellSpacing="3"  align='center' >
			...
			<td class='fields'><input type=textbox size=11 maxlength=10 name='p_fr_date' id='p_fr_date' onBlur='chkDate(this)'   style="cursor='hand'" value=''><input type='image' src="../../eCommon/images/CommonCalendar.gif"  onClick="return showCalendarValidate('p_fr_date');" ><img src="../../eCommon/images/mandatory.gif"></img></td>
			<td class='label'><fmt:message key="Common.to.label" bundle="${common_labels}"/></td><td class='fields'>
			<input type=textbox size=11 maxlength=10 name='p_to_date' id='p_to_date' onBlur='chkDate(this)'  style="cursor='hand'" value=''><input type='image' src="../../eCommon/images/CommonCalendar.gif"  onClick="return showCalendarValidate('p_to_date');" ><img src="../../eCommon/images/mandatory.gif"></img>
			</td>
		</tr>
		...
		<tr>
			...
			<!-- IN030303 starts -->
			<!-- <td  class='fields' colspan='3' ><select name="p_ord_cat" id="p_ord_cat">-->
	...
	<%
		...
		<td  class='fields' ><Select name='ordering_facility' id='ordering_facility' onchange="setOrderFaci(this)">
			   <Option value=''>---<fmt:message key="Common.all.label" bundle="${common_labels}"/>---</Option>
			   <%for (int i=0;i<facilityDetails.size();i++) {
				String [] facility = (String[])facilityDetails.get(i);
			   %>
			   <Option value='<%=facility[1]%>' ><%=facility[0]%></Option>
			   <%}%>
		</Select></TD>
		<!--IN055319 Ends-->
		</tr>
		...
		<tr>
			...
			<td class='fields'><select name='p_locn_type' id='p_locn_type'  onChange='populateSourceLocation(this)'><!--IN055594-->
			<option value='' ><fmt:message key="Common.all.label" bundle="${common_labels}"/></option>
	...
	<input type="hidden" name="p_report_id" id="p_report_id" value="ORORDSTA">
	<input type="hidden" name="p_facility_id" id="p_facility_id" value='<%=facility_id%>'>
	<input type="hidden" name="facility_id" id="facility_id" value=''>  	<!-- IN055319 -->
