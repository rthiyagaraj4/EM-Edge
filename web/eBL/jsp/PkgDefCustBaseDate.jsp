...
<!--<div id="divHeadSep" style="position: absolute;">-->
	...
	<tr><td class='COLUMNHEADER'  colspan='12' align='LEFT'><fmt:message key="eBL.CUSTOMER_GROUP.label" bundle="${bl_labels}"/>&nbsp;: &nbsp;<%=cust_grp_desc%>&nbsp;,&nbsp;&nbsp;&nbsp;&nbsp;<fmt:message key="Common.Customer.label" bundle="${common_labels}"/>&nbsp;:&nbsp;<%=cust_desc%>&nbsp;&nbsp;&nbsp;</td>
	</tr>
		<tr>
...
<td  class='fields' ><input type='text' name='fromDate_<%=i%>' id='fromDate_<%=i%>' <%=modifyStatus%>  size='10' maxlength="10"  value="<%=fromDate%>"   onBlur="custValidateDate(this,this,toDate_<%=i%>,'<%=i%>');showCustBasePrice(this,'<%=i%>');" ><img id = 'fromDate_img_<%=i%>' name="fromDate_img_<%=i%>"	 <%=modifyStatus%>  src="../../eCommon/images/CommonCalendar.gif" onClick="return showCalendar('fromDate_<%=i%>');"   /><img src='../../eCommon/images/mandatory.gif'/><input type='hidden' name='oldPeriodKey<%=i%>' id='oldPeriodKey<%=i%>' value='<%=(cust_grp_code+"~~"+cust_code+"~~"+fromDate)%>'></td>


			<td  class='fields' ><input type='text' name='toDate_<%=i%>' id='toDate_<%=i%>'  <%=modifyStatus%>  id='toDate_<%=i%>' size='10' maxlength="10"  value="<%=toDate%>"  onBlur="custValidateDate(this,fromDate_<%=i%>,this,'<%=i%>');" ><img id = 'toDate_img_<%=i%>' name="toDate_img_<%=i%>"	 src="../../eCommon/images/CommonCalendar.gif" onClick="return showCalendar('toDate_<%=i%>');" ></td>

			<td  class='fields' nowrap="nowrap">
			
			<input type='text' name='pkgPriceClsCode_<%=i%>' id='pkgPriceClsCode_<%=i%>' size='8'	<%=modifyStatus%>  maxlength="8"  value="<%=pkgPriceClsCode%>" onBlur="callCustPriceClass(this,'code','PRICE_CLASS','<%=i%>')" > <input type='text' name='pkgPriceClsDesc_<%=i%>' id='pkgPriceClsDesc_<%=i%>' size='10'	<%=modifyStatus%>  maxlength="10"  value="<%=pkgPriceClsDesc%>" onBlur="callCustPriceClass(this,'desc','PRICE_CLASS','<%=i%>')" ><input type='button' class='button' name='pkgPriceCls_<%=i%>' id='pkgPriceCls_<%=i%>'	 <%=modifyStatus%>  value='?' onClick="callCustPriceClass('','','PRICE_CLASS','<%=i%>')" tabindex='0'/>

			</td>
				
			<td  class='fields' nowrap="nowrap"><input type='text'	<%=modifyStatus%>  name='oPkgPriceClsCode_<%=i%>' size='8' maxlength="8"  value="<%=oPkgPriceClsCode%>" onBlur="callCustPriceClass(this,'code','OUT_PRICE_CLASS','<%=i%>')" />
			<input type='text' name='oPkgPriceClsDesc_<%=i%>' id='oPkgPriceClsDesc_<%=i%>' size='10'	<%=modifyStatus%>  maxlength="10"  value="<%=oPkgPriceClsDesc%>" onBlur="callCustPriceClass(this,'desc','OUT_PRICE_CLASS','<%=i%>')"/><input type='button' class='button' name='OPkgPriceCls_<%=i%>' id='OPkgPriceCls_<%=i%>' value='?'	  <%=modifyStatus%>  onClick="callCustPriceClass('','','OUT_PRICE_CLASS','<%=i%>')" tabindex='0'/>
		</td>
		<td align="left" nowrap><input type='text' name='glSmry<%=i%>' id='glSmry<%=i%>' size='6' <%=modifyStatus%>  maxlength="6" value="<%=glSmry%>" <%=glSmryYN.equals("N")?"disabled":""%> onBlur="custGlSmryCodeLookUp(glSmryDesc<%=i%>,this,'<%=i%>')"/><input type='text' name='glSmryDesc<%=i%>' id='glSmryDesc<%=i%>' size='15' maxlength="15"	<%=modifyStatus%>  <%=glSmryYN.equals("N")?"disabled":""%> value="<%=glSmryDesc%>" onBlur="custGlSmryDescLookUp(this,glSmry<%=i%>,'<%=i%>')"/><INPUT type='hidden' name='glSmry_temp<%=i%>' id='glSmry_temp<%=i%>' value=''><INPUT type='hidden' name='glSmryDesc_temp<%=i%>' id='glSmryDesc_temp<%=i%>' value=''><input type='button' class='button' name='glSmryLookUp<%=i%>' id='glSmryLookUp<%=i%>' value='?'	 <%=modifyStatus%>  onClick="custGlSmryLookUp(glSmryDesc<%=i%>,glSmry<%=i%>,'<%=i%>');"  <%=glSmryYN.equals("N")?" disabled ":""%>/>
		</td>
		<td align="left" nowrap><input type='text' name='glSmryDisc<%=i%>' id='glSmryDisc<%=i%>' size='6' <%=modifyStatus%>  maxlength="6" value="<%=glSmryDisc%>" <%=glSmryYN.equals("N")?"disabled":""%> onBlur="custGlSmryDiscCodeLookUp(glSmryDiscDesc<%=i%>,this,'<%=i%>')"/><input type='text' name='glSmryDiscDesc<%=i%>' id='glSmryDiscDesc<%=i%>' size='15' maxlength="15"	<%=modifyStatus%>  <%=glSmryYN.equals("N")?"disabled":""%> value="<%=glSmryDiscDesc%>" onBlur="custGlSmryDiscDescLookUp(this,glSmryDisc<%=i%>,'<%=i%>')"/><INPUT type='hidden' name='glSmryDisc_temp<%=i%>' id='glSmryDisc_temp<%=i%>' value=''><INPUT type='hidden' name='glSmryDiscDesc_temp<%=i%>' id='glSmryDiscDesc_temp<%=i%>' value=''><input type='button' class='button' name='glSmryDiscLookUp<%=i%>' id='glSmryDiscLookUp<%=i%>' value='?'	 <%=modifyStatus%>  onClick="custGlSmryDiscLookUp(glSmryDiscDesc<%=i%>,glSmryDisc<%=i%>,'<%=i%>');"  <%=glSmryYN.equals("N")?" disabled ":""%>/>
		</td>

		<!-- Added by muthu against 31860 on 5/3/2012 -->
		<td align="left" style="white-space: nowrap">
		<input type='checkbox' name='CopyBase<%=i%>' id='CopyBase<%=i%>' value="" onclick="CopyBaseChk()">
		</td>
	...
	%>	
		<td>
		<input type='checkbox' name='VATapp<%=i%>' id='VATapp<%=i%>' id='VATapp<%=i%>' value="<%=VATapp%>"  <%=VATapp.equals("Y")?"checked":""%>	<%=VATSetup %><%=modifyStatus%>  onClick="enableRuleCode_Cust(this,'<%=i%>')">
		</td>
			<%
		...
		<input type='text' name='VATpercent<%=i%>' id='VATpercent<%=i%>' id='VATpercent<%=i%>' size='6' maxlength="6"  value="<%=VATpercent%>" <%=modifyStatus%> <%=VATapplCheck%> nowrap  onBlur="ruleCodeLookup_cust(VATpercent<%=i%>,'<%=i%>')"  />
		<input type='button' class='button' name='VATpercent_temp<%=i%>' id='VATpercent_temp<%=i%>' id='VATpercent_temp<%=i%>' value='?' <%=modifyStatus%> <%=VATapplCheck%> nowrap onClick="ruleCodeLookup_cust(VATpercent<%=i%>,'<%=i%>')"/>
...
<td>
	...
	<option value='S' <%=asperpolicy%>><fmt:message key="eBL.AS_PER_POLICY.label" bundle="${bl_labels}"/></option>
	<option value='C' <%=patientcoverage%>><fmt:message key="eBL.PATIENT_COVERAGE.label" bundle="${bl_labels}"/></option>
	<option value='P' <%=payercoverage%>><fmt:message key="eBL.PAYER_COVERAGE.label" bundle="${bl_labels}"/></option>
	</select>
	</td>
<%}else{
}%>
<!--//Added V180122-Gayathri/MMS-DM-CRF-0118/Ends -->
		...
		<!--<input type='radio'  name='select_<%=i%>' id='select_<%=i%>'  value="off"  onClick='showCustBasePrice(this,"<%=i%>")'/>-->
			<a name='select_<%=i%>' href='#' onClick='showCustBasePrice(this,"<%=i%>")'><fmt:message key="eBL.Dtls.label" bundle="${bl_labels}"/></a>
...
<td  class='fields' ><input type='text' name='fromDate_0' id='fromDate_0'	  <%=modifyStatus%>  id='fromDate_0' size='10' maxlength="10"  value=""   onBlur="custValidateDate(this,this,toDate_0,'0');showCustBasePrice(this,'0');" ><img id = 'fromDate_img_0' name="fromDate_img_0"	  <%=modifyStatus%>  src="../../eCommon/images/CommonCalendar.gif" onClick="return showCalendar('fromDate_0');"   /><img src='../../eCommon/images/mandatory.gif'/><input type='hidden' name='oldPeriodKey0' id='oldPeriodKey0' value=''></td>

			<td  class='fields' ><input type='text' name='toDate_0' id='toDate_0' <%=modifyStatus%>  id='toDate_0' size='10' maxlength="10"  value=""  onBlur="custValidateDate(this,fromDate_0,this,'0');" ><img id = 'toDate_img_0' name="toDate_img_0"		 <%=modifyStatus%>  src="../../eCommon/images/CommonCalendar.gif" onClick="return showCalendar('toDate_0');" ></td>

			<td  class='fields' nowrap="nowrap">
			
			<input type='text' name='pkgPriceClsCode_0' id='pkgPriceClsCode_0' size='8' maxlength="8" <%=modifyStatus%>  value="" onBlur="callCustPriceClass(this,'code','PRICE_CLASS','0')" > <input type='text' name='pkgPriceClsDesc_0' id='pkgPriceClsDesc_0' size='10' maxlength="10" 	 <%=modifyStatus%>  value="" onBlur="callCustPriceClass(this,'desc','PRICE_CLASS','0')" ><input type='button' class='button' name='pkgPriceCls_0' id='pkgPriceCls_0' value='?'   <%=modifyStatus%>  onClick="callCustPriceClass('','','PRICE_CLASS','0')" tabindex='0'/>

			</td>
				
			<td  class='fields' nowrap="nowrap"><input type='text' name='oPkgPriceClsCode_0' id='oPkgPriceClsCode_0'	<%=modifyStatus%>  size='8' maxlength="8"  value="" onBlur="callCustPriceClass(this,'code','OUT_PRICE_CLASS','0')" /><input type='text' name='oPkgPriceClsDesc_0' id='oPkgPriceClsDesc_0' size='10' maxlength="10"  value=""	<%=modifyStatus%>  onBlur="callCustPriceClass(this,'desc','OUT_PRICE_CLASS','0')"/><input type='button' class='button' name='OPkgPriceCls_0' id='OPkgPriceCls_0' value='?'	<%=modifyStatus%>  onClick="callCustPriceClass('','','OUT_PRICE_CLASS','0')" tabindex='0'/>
		</td>
		<td align="left" nowrap>
		<input type='text' name='glSmry0' id='glSmry0' size='6' maxlength="6" value=""	 <%=modifyStatus%>  <%=glSmryYN.equals("N")?"disabled":""%> onBlur="custGlSmryCodeLookUp(glSmryDesc0,this,'0')"/><input type='text' name='glSmryDesc0' id='glSmryDesc0' size='15' maxlength="15"  <%=modifyStatus%>  <%=glSmryYN.equals("N")?"disabled":""%> value="" onBlur="custGlSmryDescLookUp(this,glSmry0,'0')"/><INPUT type='hidden' name='glSmry_temp0' id='glSmry_temp0' value=''><INPUT type='hidden' name='glSmryDesc_temp0' id='glSmryDesc_temp0' value=''><input type='button' class='button' name='glSmryLookUp0' id='glSmryLookUp0'	<%=modifyStatus%>  value='?' onClick="custGlSmryLookUp(glSmryDesc0,glSmry0,'0');"  <%=glSmryYN.equals("N")?" disabled ":""%>/></td>
		<td align="left" nowrap>
		<input type='text' name='glSmryDisc0' id='glSmryDisc0' size='6' maxlength="6" value=""	 <%=modifyStatus%>  <%=glSmryYN.equals("N")?"disabled":""%> onBlur="custGlSmryDiscCodeLookUp(glSmryDiscDesc0,this,'0')"/><input type='text' name='glSmryDiscDesc0' id='glSmryDiscDesc0' size='15' maxlength="15"  <%=modifyStatus%>  <%=glSmryYN.equals("N")?"disabled":""%> value="" onBlur="custGlSmryDiscDescLookUp(this,glSmryDisc0,'0')"/><INPUT type='hidden' name='glSmryDisc_temp0' id='glSmryDisc_temp0' value=''><INPUT type='hidden' name='glSmryDiscDesc_temp0' id='glSmryDiscDesc_temp0' value=''><input type='button' class='button' name='glSmryDiscLookUp0' id='glSmryDiscLookUp0'	<%=modifyStatus%>  value='?' onClick="custGlSmryDiscLookUp(glSmryDiscDesc0,glSmryDisc0,'0');"  <%=glSmryYN.equals("N")?" disabled ":""%>/></td>
	...
	<%if("Y".equals(VATapplicable)) {%>
	<td>
		<input type='checkbox' name='VATapp0' id='VATapp0' id='VATapp0' value=""  <%=VATapp.equals("Y")?"checked":""%>	<%=modifyStatus%> onClick="enableRuleCode_Cust(this,'0')">
		</td>
			<%
			if("Y".equals(VATapp)){
				VATapplCheck="";
		...
		%>
		<td style="white-space: nowrap" width="6%">
		<input type='text' name='VATpercent0' id='VATpercent0' id='VATpercent0' size='6' maxlength="6"  value="" <%=modifyStatus%> <%=VATapplCheck%> onBlur="ruleCodeLookup_cust(VATpercent0,'0')"  />
		<input type='button' class='button' name='VATpercent_temp0' id='VATpercent_temp0' id='VATpercent_temp0' value='?' <%=modifyStatus%> <%=VATapplCheck%> onClick="ruleCodeLookup_cust(VATpercent0,'0')"/>
...
<%
	...
	<!--//Added V180122-Gayathri/MMS-DM-CRF-0118/Ends -->
		...
		<a name='select_0' href='#' onClick='showCustBasePrice(this,"0")'><fmt:message key="eBL.Dtls.label" bundle="${bl_labels}"/></a>
