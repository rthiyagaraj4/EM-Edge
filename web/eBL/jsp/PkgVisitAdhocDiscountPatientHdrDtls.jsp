...
//String sqlCount = "SELECT COUNT (*) total FROM bl_package_encounter_dtls WHERE  operating_facility_id = ? AND patient_id = ? AND package_code = ? AND package_seq_no = ?";//unused variable
...
<Script src=\"../../eCommon/js/showModalDialog.js\" language=\"JavaScript\"></Script>
	...
	<body onKeyDown="lockKey()" OnMouseDown='CodeArrest()'>
		<form name='patLineForm' id='patLineForm'>
			...
			<TR> <td id='tdLine1' nowrap>&nbsp;<span class='valueClassRow1' title='packageSeqNo'><fmt:message key="eBL.packageSeqNo.label" bundle="${bl_labels}"/>: <%=packageSeqNo%></span>&nbsp;
			<span class='valueClassRow1' title='packagedesc'><fmt:message key='Common.description.label' bundle='${common_labels}'/> : <%=packagelongDesc%></span>&nbsp;
			<%if(episodeType.equals("O")){%>
			<span class='valueClassRow1' title=''><fmt:message key='Common.EpisodeType.label' bundle='${common_labels}'/> :<fmt:message key='Common.OP.label' bundle='${common_labels}'/></span>
			<%}else  if(episodeType.equals("I")){%>
			<span class='valueClassRow1' title=''><fmt:message key='Common.EpisodeType.label' bundle='${common_labels}'/> :<fmt:message key='Common.IP.label' bundle='${common_labels}'/></span>
			<%}else if(episodeType.equals("D")){	%>
			<span class='valueClassRow1' title=''><fmt:message key='Common.EpisodeType.label' bundle='${common_labels}'/> :<fmt:message key='Common.daycare.label' bundle='${common_labels}'/></span>
			<%}%>
			</td></tr>
			</table>
		</form>
