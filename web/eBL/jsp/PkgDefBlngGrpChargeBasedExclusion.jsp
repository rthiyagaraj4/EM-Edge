...
<div id="tbl-container" STYLE="overflow: auto; width: 100%; height: 85px;   padding:3px; margin: 0px">
	...
	<thead>
	<%if( ss){%>
	<tr><td class='COLUMNHEADER'  colspan='6' align='left'><fmt:message key="eBL.BillingGroup.label" bundle="${bl_labels}"/>&nbsp;: &nbsp;<%=blng_grp_desc%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<fmt:message key="Common.fromdate.label" bundle="${common_labels}"/>&nbsp;: &nbsp;<%=fromDate%>&nbsp;&nbsp;&nbsp;<fmt:message key="Common.BillingService.label" bundle="${common_labels}"/>&nbsp;: &nbsp;<%=service_desc%>&nbsp;&nbsp;&nbsp;</td></tr>
	<%}else{%>
		<tr><td class='COLUMNHEADER'  colspan='6' align='left'><fmt:message key="eBL.BillingGroup.label" bundle="${bl_labels}"/>&nbsp;: &nbsp;<%=blng_grp_desc%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<fmt:message key="Common.fromdate.label" bundle="${common_labels}"/>&nbsp;: &nbsp;<%=fromDate%>&nbsp;&nbsp;&nbsp;<fmt:message key="Common.BillingService.label" bundle="${common_labels}"/>&nbsp;: &nbsp;<%=service_desc%>&nbsp;&nbsp;&nbsp;</td></tr>
	<%}%>
	<tr >
...
%>
			...
			<td  class='fields'><input type='text' name='cbCode_<%=i%>' id='cbCode_<%=i%>' <%=modifyStatus%>  id='cbCode_<%=i%>'size='10' maxlength="10"  value="<%=itemCode%>"   onBlur='blngGrpCallServices(document.forms[0].cbCode_<%=i%>,document.forms[0].cbDesc_<%=i%>,"BLNG_GRP_CHARGE_BASED_ITEM",charge_type<%=i%>,"<%=i%>");'><input type='hidden' name='cbCodePrev_<%=i%>' id='cbCodePrev_<%=i%>' value="<%=itemCode%>"/><input type='text' name='cbDesc_<%=i%>' id='cbDesc_<%=i%>' id='cbDesc_<%=i%>'size='30'  <%=modifyStatus%>  maxlength="100"  value="<%=itemDesc%>"   onBlur='blngGrpCallServices(document.forms[0].cbCode_<%=i%>,document.forms[0].cbDesc_<%=i%>,"BLNG_GRP_CHARGE_BASED_ITEM",charge_type<%=i%>,"<%=i%>");'><input type='hidden' name='cbDescPrev_<%=i%>' id='cbDescPrev_<%=i%>' value="<%=itemDesc%>"/><input type='button' class='button'	<%=modifyStatus%>  name='serviceLookUp_<%=i%>' value='?' onClick="blngGrpCallServices(document.forms[0].cbCode_<%=i%>,document.forms[0].cbDesc_<%=i%>,'BLNG_GRP_CHARGE_BASED_ITEM',charge_type<%=i%>,'<%=i%>');" tabindex='0'></td>

			<td  class='fields' >
			<input type='checkbox' name='inEx_<%=i%>' id='inEx_<%=i%>' id='inEx_<%=i%>'  value="<%=inExcl%>" <%=modifyStatus%>  <%=inSel%>   onclick="blngGrpExcludeItem(this,'<%=i%>');changeSplitReqd(this,'exclude','<%=i%>');IncludeHomeMedicationBlngGrpExclude(this,cbCode_<%=i%>,charge_type<%=i%>,'<%=i%>');"/>
		   	</td>
			<td  class='fields' >
		...
		<%if(ss){%>	
			...
			<input type='checkbox'   <%=modifyStatus%> name='includeHomeMedication<%=i%>' id='includeHomeMedication<%=i%>' value='<%=includeHomeMedication %>' <%="Y".equals(includeHomeMedication)?" checked ":""%> <%="Y".equals(enableDisParam)?"":"disabled" %>  onclick="checkInclHomeMedi(this); IncludeHomeMedicationUnchkExclude(this,'blngGrpchargeexclude','<%=i%>');">
		...
		<%}else{%>
			...
			<td  class='fields'>
			<input type='checkbox' name='delCode_<%=i%>' id='delCode_<%=i%>' value='N'  <%=modifyStatus%>  onclick="chkBlngGrpDeleteService(this,'<%=i%>')"/>
		   	</td>
		...
		<%}
			...
			<td  class='fields'><input type='text' name='cbCode_0' id='cbCode_0'	<%=modifyStatus%>  id='cbCode_0'size='10' maxlength="10"  value=""   onBlur="blngGrpCallServices(document.forms[0].cbCode_0,document.forms[0].cbDesc_0,'BLNG_GRP_CHARGE_BASED_ITEM','M','0');"><input type='hidden' name='cbCodePrev_0' id='cbCodePrev_0' value=""/><input type='text' name='cbDesc_0' id='cbDesc_0' id='cbDesc_0'size='30' maxlength="100" <%=modifyStatus%>  value=""   onBlur="blngGrpCallServices(document.forms[0].cbCode_0,document.forms[0].cbDesc_0,'BLNG_GRP_CHARGE_BASED_ITEM','M','0');"><input type='hidden' name='cbDescPrev_0' id='cbDescPrev_0' value=""/><input type='button'  <%=modifyStatus%>  class='button' name='serviceLookUp_0' value='?' onClick="blngGrpCallServices(document.forms[0].cbCode_0,document.forms[0].cbDesc_0,'BLNG_GRP_CHARGE_BASED_ITEM','M','0')" tabindex='0'></td>

			<td  class='fields' >
			<input type='checkbox' name='inEx_0' id='inEx_0' value="N"  <%=modifyStatus%>   onclick="blngGrpExcludeItem(this,'0');changeSplitReqd(this,'exclude','0');IncludeHomeMedicationBlngGrpExclude(this,cbCode_0,charge_type0,'0');"/>
		   	</td>
			<td  class='fields' >
		...
		<%if(ss){%>	
			<td class='fields' nowrap WIDTH=''>
			<input type='checkbox'   <%=modifyStatus%> name='includeHomeMedication0' id='includeHomeMedication0' value='<%=includeHomeMedication %>' <%="Y".equals(includeHomeMedication)?" checked ":""%> <%="Y".equals(enableDisParam)?"":"disabled" %>  onclick="checkInclHomeMedi(this); IncludeHomeMedicationUnchkExclude(this,'blngGrpchargeexclude','0');">
		...
		<%} %>
			...
			<td  class='fields'>
			<input type='checkbox' name='delCode_0' id='delCode_0' value='N'   <%=modifyStatus%>  onclick="chkBlngGrpDeleteService(this,'0')"/>
		   	</td>
