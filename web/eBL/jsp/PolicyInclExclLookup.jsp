	...
	%>
      	...
      	<thead>  
      		<tr>
      			...
      			<td class='COLUMNHEADER' colspan="22" align="center"><fmt:message key="eBL.CoverageDetails.label" bundle="${bl_labels}"/></td>
      		</tr>
      		<tr>
				...
				<% } %>
				<!-- MMS-QH-CRF-0170-US-4  PALANINARAYANAN 9/7/2020 ENDS-->
				<td class='COLUMNHEADER' nowrap><fmt:message key="eBL.Gross/Net.label" bundle="${bl_labels}"/></td> <!--210727-->
				...
				<td class='COLUMNHEADER' nowrap><fmt:message key="eBL.DeleteRecord.label" bundle="${bl_labels}"/></td> <!--210727-->
			 </tr>
			 <tr>
				...
				<td nowrap>
					<input type='text' name='age_groupFilter' id='age_groupFilter' id='age_groupFilter' length='10' value='<%=ageGroupDescFilter %>'  size='4' onblur='if(this.value.length>0){ callCommonValidation(5,this,hdn_age_groupFilter); } else{ fnClearCode(hdn_age_groupFilter); }'>
				...
				<td nowrap>
					<input type='text' id='clinic_codeFilter' name='clinic_codeFilter' id='clinic_codeFilter' value='<%=clinicFilter %>' size='4' onblur='if(this.value.length>0 && clinicFilter.value!=""){ callBGDescCode(1,clinicFilter,clinic_descFilter,clinic_codeFilter); } else{ fnClearCode(clinic_descFilter); }'> 
					<input type='text' id='clinic_descFilter' name='clinic_descFilter' id='clinic_descFilter' value='<%=clinicDescFilter %>' size='20' maxlength='60' onblur='if(this.value.length>0 && clinicFilter.value!=""){ callBGDesc(1,clinicFilter,clinic_descFilter,clinic_codeFilter); } else{ fnClearCode(clinic_codeFilter); }'>
				...
				<td nowrap>
					<input type='text' id='spec_codeFilter' name='spec_codeFilter' id='spec_codeFilter' size='4' value='<%=specFilter %>' onblur='if(this.value.length>0){ callCommonValidationSpecCode(spec_descFilter,spec_codeFilter,clinicFilter,clinic_codeFilter); } else{ fnClearCode(spec_descFilter); }'>
					<input type='text' id='spec_descFilter' name='spec_descFilter' id='spec_descFilter' size='20' maxlength='60' value='<%=specDescFilter %>' onblur='if(this.value.length>0){ callCommonValidationSpec(spec_descFilter,spec_codeFilter,clinicFilter,clinic_codeFilter); } else{ fnClearCode(spec_codeFilter); }'>
				...
				<td></td>
				<!-- Added V190326-Dhananjay/MMS-DM-CRF-0158/Starts -->
				<% if ("Y".equals(Site_across_enc)) { %> <td></td> <% } %>
				<!-- Added V190326-Dhananjay/MMS-DM-CRF-0158/Ends -->
				<td nowrap>
			...
			%>	
			 ...
			 "<%=policyBean1.getClinicNursCode()%>","<%=policyBean1.getSpecCode()%>","<%=policyBean1.getInclExclCriteria()%>","<%=policyBean1.getBillServCode()%>","<%=policyBean1.getStartDate()%>","inclExclCoverageLookup")'>
				...
				<%	if("Y".equals(SiteSpecific)){
      				%>
				<td nowrap class='fields'>
					<%=checkForNull(policyBean1.getInclhomeMedi()) %>
				</td>
				<%} %>
				<td nowrap class='fields'>	
