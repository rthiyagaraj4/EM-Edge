...
<form name='add_modify_pat_fin_det_insmain' id='add_modify_pat_fin_det_insmain' method='post' >
	...
	<input type=hidden name="userId_for_priv<%=totalRecords%>" id="userId_for_priv<%=totalRecords%>" value="" />  <!-- V230707 -->
	<input type=hidden name="reasonCode_for_priv<%=totalRecords%>" id="reasonCode_for_priv<%=totalRecords%>" value="" />  <!-- V230707 -->
	<input type=hidden name="offline_Eligibility_Check<%=totalRecords%>" id="offline_Eligibility_Check<%=totalRecords%>" value="" />  <!-- V240301 -->
	<input type=hidden name="offline_Eligibility_Dt<%=totalRecords%>" id="offline_Eligibility_Dt<%=totalRecords%>" value="" />  <!-- V240301 -->
	<input type=hidden name="offline_Eligibility_Code<%=totalRecords%>" id="offline_Eligibility_Code<%=totalRecords%>" value="" />  <!-- V240301 -->
	<%
		totalRecords++;
		}
		}
	...
	<td class=<%=classval%> width='14%'><b id="policy_coverage_basis_<%=totalRecords%>"></b></td>
	<td class=<%=classval%> width='12%' ><div id="apprd_amt_leg_disp<%=totalRecords%>" style=<%=v_display_hide_apprvd_amt%> ><fmt:message key="eBL.APPROVED_AMT.label" bundle="${bl_labels}"/></div></td>
	<td class=<%=classval%> width='7%'><div id="apprd_amt_val_disp<%=totalRecords%>" style=<%=v_display_hide_apprvd_amt%> ><input type="text" maxlength='10' size="7" name='credit_apprd_amt<%=totalRecords%>' style='text-align:right'  value='<%=mcredit_apprd_amt.get(i)%>' onFocus='check(<%=totalRecords%>);' readonly></div></td>
	<td  class=<%=classval%> width='12%'><div id="apprd_days_leg_disp<%=totalRecords%>" style=<%=v_display_hide_apprvd_days%> ><fmt:message key="eBL.APPROVED_DAY.label" bundle="${bl_labels}"/></div></td>
	<td class=<%=classval%> width='12%'><div id="apprd_days_val_disp<%=totalRecords%>" style=<%=v_display_hide_apprvd_days%> ><input type="text" maxlength='4' size="4" name='credit_apprd_days<%=totalRecords%>' style='text-align:right'  value='<%=mcredit_apprd_days.get(i)%>' onFocus='check(<%=totalRecords%>);' readonly></div></td>
	<td class=<%=classval%> width='10%' ><fmt:message key="eBL.POLICY_EFFECTIVE_FROM.label" bundle="${bl_labels}"/></td>
	<td class=<%=classval%> width='10%'><input type='text'  name='policy_eff_frm_date<%=totalRecords%>' size='10' maxlength='10' value='<%=policy_eff_frm_date_conv%>'  onFocus='check(<%=totalRecords%>);' readonly>
	<td class=<%=classval%> width='12%' ><fmt:message key="eBL.POLICY_EFFECTIVE_TO.label" bundle="${bl_labels}"/></td>
	<td class=<%=classval%> width='11%'><input type='text'  name='policy_eff_to_date<%=totalRecords%>' size='10' maxlength='10' value='<%=policy_eff_to_date_conv%>'  onFocus='check(<%=totalRecords%>);' readonly></td>
	</tr>
	<input type=hidden name='adj_rule_ind<%=totalRecords%>' id='adj_rule_ind<%=totalRecords%>' value="<%=madj_rule_ind.get(i)%>">
	<input type=hidden name='adj_perc_amt_ind<%=totalRecords%>' id='adj_perc_amt_ind<%=totalRecords%>' value="<%=madj_perc_amt_ind.get(i)%>">
	<input type=hidden name='adj_perc_amt_value<%=totalRecords%>' id='adj_perc_amt_value<%=totalRecords%>' value="<%=madj_perc_amt_value.get(i)%>">
...
<%	if(eligibilityCheckYN.equals("Y")){ %>
	<input type=hidden name="offline_Eligibility_Check<%=totalRecords%>" id="offline_Eligibility_Check<%=totalRecords%>" value="<%=moffline_Eligibility_Check.get(i)%>" />  <!-- V240301 -->	
	<input type=hidden name="offline_Eligibility_Dt<%=totalRecords%>" id="offline_Eligibility_Dt<%=totalRecords%>" value="<%=moffline_Eligibility_Dt.get(i)%>" />  <!-- V240301 -->
	<input type=hidden name="offline_Eligibility_Code<%=totalRecords%>" id="offline_Eligibility_Code<%=totalRecords%>" value="<%=moffline_Eligibility_Code.get(i)%>" />  <!-- V240301 -->
	<%}else{ %>
	<input type=hidden name="offline_Eligibility_Check<%=totalRecords%>" id="offline_Eligibility_Check<%=totalRecords%>" value="" />  <!-- V240301 -->
	<input type=hidden name="offline_Eligibility_Dt<%=totalRecords%>" id="offline_Eligibility_Dt<%=totalRecords%>" value="" />  <!-- V240301 -->
	<input type=hidden name="offline_Eligibility_Code<%=totalRecords%>" id="offline_Eligibility_Code<%=totalRecords%>" value="" />  <!-- V240301 -->	
	...
	<input type=hidden name="offline_Eligibility_Check<%=totalRecords%>" id="offline_Eligibility_Check<%=totalRecords%>" value="<%=output1[55]%>" />  <!-- V240301 -->
	<input type=hidden name="offline_Eligibility_Dt<%=totalRecords%>" id="offline_Eligibility_Dt<%=totalRecords%>" value="<%=output1[56]%>" />  <!-- V240301 -->
	<input type=hidden name="offline_Eligibility_Code<%=totalRecords%>" id="offline_Eligibility_Code<%=totalRecords%>" value="<%=output1[57]%>" />  <!-- V240301 -->
	<%
			totalRecords++;
		}
		else
	...
	%>
		<jsp:include page="../../eBL/jsp/AddModifyPatFinDetailsInsBodyExtn2.jsp" >
