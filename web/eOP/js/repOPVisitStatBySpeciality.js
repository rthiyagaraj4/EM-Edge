 function run() 
{
	var fields1 = new Array(  f_query_add_mod.document.forms[0].p_date_from,
							 f_query_add_mod.document.forms[0].p_date_to );
	var names1 = new Array(getLabel("eOP.StatisticsDateFrom.label","OP"), getLabel("eOP.StatisticsDateTo.label","OP"));
...
function reset() 
{
	//Modified by Arthi on 30-May-2022 for MOHE-CRF-0134
...
async function searchCode(obj,target)
{
	...
	if(obj.name=="tovtype" || obj.name=="fromvtype")
	{
		...
		argumentArray[0]="select visit_type_code code, short_desc description from op_visit_type_lang_vw where (('"+p_faclid+"' like ? and language_id='"+locale+"' and facility_id in (select facility_id from sm_facility_for_user where appl_user_id like ? intersect select facility_id from sm_facility_for_resp where resp_id like ? )) or ('" + p_faclid + "' != ? and facility_id like ?)) and upper(visit_type_code) like upper(nvl(?,visit_type_code)) and upper(short_desc) like upper(nvl(?,short_desc)) order by 2";
