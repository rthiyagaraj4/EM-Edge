...
<%@ page import ="java.sql.*,java.util.*,java.text.*,webbeans.eCommon.ConnectionManager,com.ehis.util.*,eCommon.XSSRequestWrapper" contentType="text/html;charset=UTF-8"%>
			...
			if(rs1 != null) {				
			 ...
			 %>
					<script>
						...
						document.getElementById("prevsubservice").innerHTML='<%=p_subservice_desc%>';						
			          }
					</script>
				<%
			if(rs1 != null) rs1.close();
...
%>
			<td  align='left'><input type='text' name='pract_name' id='pract_name' maxlength='30' onChange="onBlurCallPractitionerSearch(document.forms[0].preact_butt,pract_name);" value="<%=temp_pract%>" <%=disable_pract%> onkeypress="return(Chkval(this,event,0))"><input type='button' name='preact_butt' id='preact_butt' <%=disable_pract%> value='?' class='button' onClick="callPractSearch(this,pract_name);"><img src='../../eCommon/images/mandatory.gif'></img>
			<%
				if(!module_id.equals("CA"))
			{
				%>
			<input type='hidden' name='pract_id' id='pract_id' value='<%=p_cur_practitioner_id%>'>
					<%}else
			{%>
			...
			else
			{
			  ...
			  p_cur_pract_type = "";
					...
					%>
			<td  align='left' class='QUERYDATA' id='practtype'><%=p_cur_pract_type_desc%></td>
...
<%			
	...
	<td id='queue_no_div' style='visibility:hidden;display:none' ><input type='text' name='queue_no' id='queue_no' value = '<%=queue_num%>' size='15' onKeyPress='return Check_SpecCharsValidationOP(event);' onPaste='return checkspecialandalphanumericOP(event);' onblur = 'chkValue(this);' maxlength='15' ><img src='../../eCommon/images/mandatory.gif'></td>
	</tr>
	...
	<td class='label' ><fmt:message key="eOA.SecondaryResources.label" bundle="${oa_labels}"/>
	</td>
	<td>
	</td>
	<td>
	<input type='button' class='button' name='sec_res_btn' id='sec_res_btn' value='<fmt:message key="Common.Additional.label" bundle="${common_labels}"/>&nbsp;<fmt:message key="eOA.SecondaryResources.label" bundle="${oa_labels}"/>' onClick="callSecondaryResource('<%=p_cur_locn_code%>','<%=appt_id%>','','SecRes');">
	<input type='hidden' name='sec_res_lst' id='sec_res_lst' value='<%=practList%>'/>
	</td>
	</tr>
...
<script>
callSecondaryResource('<%=p_cur_locn_code%>','<%=appt_id%>','<%=practList%>');
</script><%}
...
%>
    ...
    <table cellpadding='0' cellspacing='0' border=0 width='95%' >
        <tr class="white">
            <td  class='white' align='right'>
			   <input type='button'  class='button' name='ok' id='ok' value='<fmt:message key="Common.record.label" bundle="${common_labels}"/>' onClick='apply()'><input type='button' align='left' class='button' name='close' id='close' value='<fmt:message key="Common.close.label" bundle="${common_labels}"/>' onClick='exitContact()'>
            </td>
        </tr>
    </table>
    </td>
	...
	%>
		...
		<input type='hidden' name='exist_pract_id' id='exist_pract_id' id='exist_pract_id' value='<%=p_pract_id%>'	 /> 
