...
public class linechart
{
    ...
    public static void drawgridline(Image image, int k, int l, int i1, int j1, Color color, int k1)
    {
    ...
    public static void Draw3Dgrid(Image image, Color color, int k, int l, int i1, int j1, int k1, int l1, 
            int i2, int j2)
    {
        pad = image.getGraphics();
        pad.setColor(color);
        ...
        for(int k2 = 0; k2 < l1; k2++)
            drawgridline(image, i1 + i3 + k2 * l, j1 - i3 - j3, i1 + i3 + k2 * l, j1 - i3, color, i2);

        for(int l2 = 0; l2 < k1; l2++)
        {
            drawgridline(image, i1 + i3, j1 - i3 - l2 * k, i1 + i3 + k3, j1 - i3 - l2 * k, color, i2);
            drawgridline(image, i1, j1 - l2 * k, i1 + i3, j1 - i3 - l2 * k, color, 1);
        }

    }

    public static void Drawline(Image image, int k, int l, int i1, int j1, Color color)
    {
        pad = image.getGraphics();
        pad.setColor(color);
        pad.drawLine(k, l, i1, j1);
    ...
    public static void Draw2Dgrid(Image image, Color color, int k, int l, int i1, int j1, int k1, int l1, 
            int i2)
    {
        pad = image.getGraphics();
        pad.setColor(color);
        ...
        for(int j2 = 0; j2 < l1; j2++)
            drawgridline(image, i1 + j2 * l, j1 - l2, i1 + j2 * l, j1, color, i2);

        for(int k2 = 0; k2 < k1; k2++)
            drawgridline(image, i1, j1 - k2 * k, i1 + i3, j1 - k2 * k, color, i2);
    ...
    public static void Draw3Daxis(Image image, Color color, int k, int l, int i1, int j1, int k1, int l1, 
            Color color1)
    {
        pad = image.getGraphics();
        int i2 = (7 * l1) / 10;
    ...
    public static void DrawPoint(Image image, int k, int l, Color color, int ai[], boolean flag)
    {
        pad = image.getGraphics();
        pad.setColor(color);
        switch(ai[0])
        ...
        case 2: // '\002'
            pad.drawLine(k - ai[1] / 2, l - ai[1] / 2, k + ai[1] / 2, l + ai[1] / 2);
            pad.drawLine(k - ai[1] / 2, l + ai[1] / 2, k + ai[1] / 2, l - ai[1] / 2);
            break;
    ...
    public static void linechart(Image image, double ad[][], boolean aflag[][], boolean aflag1[][], String as[], int ai[], int ai1[][], Image aimage[], 
            double d, double d1, Color acolor[], int ai2[][], boolean aflag2[], 
            boolean aflag3[], int ai3[], String as1[], int ai4[][], Font afont[], Color acolor1[], String as2[], 
            Color acolor2[], int ai5[][], double ad1[][], String as3[], Font afont1[])
    {
        int k = ai3[4];
        pad = image.getGraphics();
        for(i = 0; i < 5; i++)
        ...
        if(aflag3[1])
            if(aflag3[0])
                Draw3Dgrid(image, acolor1[13], ai3[2], k, ai3[5], ai3[6], ai3[1], ai3[0], ai3[11], ai3[7]);
            else
                Draw2Dgrid(image, acolor1[13], ai3[2], k, ai3[5], ai3[6], ai3[1], ai3[0], ai3[11]);
        if(aflag3[2])
        {
            ...
            if(d < 0.0D)
                ...
                yTemp = ai3[6];
            if(aflag3[0])
                Draw3Daxis(image, acolor1[14], ai3[5], yTemp, k * ai3[0], (ai3[2] * ai3[1] - ai3[6]) + yTemp, ai3[6] - yTemp, ai3[7], acolor1[15]);
            else
                DrawAxis(image, acolor1[14], ai3[5], yTemp, k * ai3[0], (ai3[2] * ai3[1] - ai3[6]) + yTemp, ai3[6] - yTemp);
        }
        if(as1[10] != "")
            DrawLabel(image, as1[10], afont[10], acolor1[10], ai4[10][0], ai4[10][1], 0);
        if(as1[11] != "")
            DrawLabel(image, as1[11], afont[11], acolor1[11], ai4[11][0], ai4[11][1], 0);
        if(as1[12] != "")
            DrawLabel(image, as1[12], afont[12], acolor1[12], ai4[12][0], ai4[12][1], 1);
        for(i = 0; i < ai3[0]; i++)
            if(as[i] != "")
                DrawLabel(image, as[i], afont[14], acolor1[17], ai3[5] + 5 + i * k, ai3[10] + ai[i], ai3[9]);
        ...
        if(aflag3[3])
            for(i = 0; i < ai3[1] + 1; i++)
            {
                ...
                DrawLabel(image, sTemp, afont[13], acolor1[17], ai3[5] - 5, ai3[6] - i * ai3[2], 4);
            }
        ...
        if(aflag3[5])
        {
            ...
            if(as1[13] != null)
            {
                ...
                pad.drawString(as1[13], (l + j1 / 2) - fontmetrics.stringWidth(as1[13]) / 2, i1);
                i1 += 10;
            }
            for(i = 0; i < ai3[3]; i++)
            {
        ...
        for(i = 1; i < ai3[0]; i++)
            for(j = 0; j < ai3[3]; j++)
                ...
                {
                    yTemp = ai3[6] - (int)Math.round((dTemp * (ad[i - 1][j] - d)) / d1);
                    yTemp2 = ai3[6] - (int)Math.round((dTemp * (ad[i][j] - d)) / d1);
                    xTemp = ai3[5] + (i - 1) * k + k / 2;
                    xTemp2 = ai3[5] + i * k + k / 2;
                    if(aflag3[0])
                        Draw3Dline(image, xTemp, yTemp, xTemp2, yTemp2, ai3[7], acolor[j], aflag3[4]);
                    else
        ...
        for(i = 0; i < ai3[0]; i++)
            for(j = 0; j < ai3[3]; j++)
                if(!aflag[i][j])
                {
                    yTemp = ai3[6] - (int)Math.round((dTemp * (ad[i][j] - d)) / d1);
                    xTemp = ai3[5] + i * k + k / 2;
        ...
        for(i = 0; i < 10; i++)
            if(ai5[i][1] > 0)
            {
                yTemp = ai3[6] - (int)Math.round((dTemp * (ad1[i][0] - d)) / d1);
                yTemp2 = ai3[6] - (int)Math.round((dTemp * (ad1[i][1] - d)) / d1);
                xTemp = ai3[5] + (ai5[i][1] - 1) * k + k / 2;
                xTemp2 = ai3[5] + (ai5[i][2] - 1) * k + k / 2;
                pad.setColor(acolor2[i]);
                pad.drawLine(xTemp, yTemp, xTemp2, yTemp2);
                DrawLabel(image, as3[i], afont1[i], acolor2[i], xTemp2, yTemp2 - 5, 0);
            }
        ...
        for(i = 10; i < 20; i++)
            if(ai5[i][1] > 0)
            {
                ...
                xTemp = ai3[5] + (ai5[i][1] - 1) * k + k / 2;
                xTemp2 = ai3[5] + (ai5[i][2] - 1) * k + k / 2;
                drawgridline(image, xTemp, yTemp, xTemp2, yTemp, acolor2[i], ai5[i][0]);
                DrawLabel(image, as3[i], afont1[i], acolor2[i], xTemp, yTemp - 5, 0);
            }
        ...
        for(i = 0; i < 10; i++)
            if(as1[i] != "")
                DrawLabel(image, as1[i], afont[i], acolor1[i], ai4[i][0], ai4[i][1], 0);
            ...
            for(j = 0; j < ai3[3]; j++)
                if(!aflag[i][j])
                {
                    ...
                    if(aflag1[i][j])
                    {
                        ...
                        sTemp = FormatStr(sTemp, -1, 2);
                        DrawTextBox(image, sTemp, afont[13], acolor1[17], acolor1[19], xTemp + 5, yTemp - 5);
                        DrawLabel(image, sTemp, afont[13], acolor1[17], xTemp + 5, yTemp - 5, 0);
                    }
                }
    ...
    public static void DrawAxis(Image image, Color color, int k, int l, int i1, int j1, int k1)
    {
        pad = image.getGraphics();
        pad.setColor(color);
        pad.drawLine(k, l, k + i1, l);
    ...
    public static void Draw3Dline(Image image, int k, int l, int i1, int j1, int k1, Color color, boolean flag)
    {
        pad = image.getGraphics();
        int l1 = (7 * k1) / 10;
        xpoints1 = new int[4];
    ...
    public static void DrawTextBox(Image image, String s, Font font, Color color, Color color1, int k, int l)
    {
        pad = image.getGraphics();
        pad.setFont(font);
        FontMetrics fontmetrics = pad.getFontMetrics();
        pad.setColor(color1);
        pad.fillRect(k - 4, l - fontmetrics.getHeight(), fontmetrics.stringWidth(s) + 8, fontmetrics.getHeight() + 4);
        pad.setColor(color);
        pad.drawRect(k - 4, l - fontmetrics.getHeight(), fontmetrics.stringWidth(s) + 8, fontmetrics.getHeight() + 4);
    }

    public static void DrawLabel(Image image, String s, Font font, Color color, int k, int l, int i1)
    {
        pad = image.getGraphics();
        pad.setColor(color);
        pad.setFont(font);
