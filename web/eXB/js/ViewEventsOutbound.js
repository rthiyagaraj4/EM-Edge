...
function funSubmit()
{
	var act = '../../eXB/jsp/ViewEventseBillResult.jsp';
...
//	alert(parent.frames[0].name);
	...
	this.document.forms[0].action = "../../eXB/jsp/ViewEventsOutboundHeader.jsp?editquery='Y'&function_id="+function_id;
...
{	
//	alert("execObjName : "+execObjName);
	document.forms[0].action = "../../eXB/jsp/"+ execObjName +".jsp";		
	this.document.forms[0].target = "f_query_add_mod_dtl";
...
function onChangeCommMode1()
{
	...
	if(comm_mode == "outbound")
	{
		...
		if(msgStatus == "H") document.getElementById("msg_status").options[6] = new Option("Review Pending","H",false, true);		
		else document.getElementById("msg_status").options[6] = new Option("Review Pending","H",false, false);
...
function initSetup()
{
	...
	// Commnented as per the new changes
	//	document.getElementById("inEventStatusDiv").style.display="none";
	//	document.getElementById("outEventStatusDiv").style.display="block";		
...
/*
		...
		if(msgStatus == "H") 		document.getElementById("msg_status").options[6] = new Option("Review Pending","H",false, true);		
		else  document.getElementById("msg_status").options[6] = new Option("Review Pending","H",false, true);
*/

		if(msgStatus == "H") 		document.getElementById("msg_status").options[0] = new Option("Review Pending","H",false, true);		
		else  document.getElementById("msg_status").options[0] = new Option("Review Pending","H",false, true);
		...
		var queryEventType = "SELECT DISTINCT me.EVENT_TYPE code, me.EVENT_NAME name FROM xh_application_event_type M,XH_EVENT_TYPE_LANG_VW me "+
					...
					" AND m.event_type IN (SELECT EVENT_TYPE FROM XH_DATA_GROUP_DETAIL WHERE DATA_GROUP_ID IN "+
					" (SELECT DATA_GROUP_ID FROM xh_protocol_link xpl,xh_protocol xp WHERE xp.protocol_id=xpl.protocol_id and xp.INTERFACE_MODULE_ID='"+moduleID+"'))";
	// Commneted as part of new changes		
		eval(sendRequest(queryEventType,'eventtype',getLabel("Common.all.label","common"),"both"));
...
//	linkset[0] = viewMsgLink;
	...
	auditDetailStr = '<a href="#" onClick='+'\''+'goToAuditLog("'+applicationId+'","'+applicationName+'","'+facilityId+'","'+facilityName+'","'+msgId+'","'+eventType+'","'+eventName+'","'+purge_status+'","O");'+'\''+'>'+auditLabel+'</a>';
	...
	if(audtiAut == "true") 
	{		
		var tabName = "xh_application_message_audit";
		if(purge_status == "OFFLINE") tabName = "XH_OFFLINE_APPL_MESSAGE_AUDIT";
		else if(purge_status == "PURGE") tabName = "XH_PURGE_APPL_MESSAGE_AUDIT";
...
//	linkset[0] = viewMsgLink;
	...
	auditDetailStr = '<a href="#" onClick='+'\''+'goToAuditLog("'+applicationId+'","'+applicationName+'","'+facilityId+'","'+facilityName+'","'+msgId+'","'+eventType+'","'+eventName+'","'+purge_status+'","I");'+'\''+'>'+auditLabel+'</a>';
...
//**********************************************************************************************/
function goToAuditLog(applicationId,applicationName,facilityId,facilityName,msgId,eventType,eventName,purge_status,commMode)
{
	...
	window.showModalDialog("../../eXH/jsp/ViewMessageAuditLogMain.jsp?"+url,arguments,features);  
}
...
function goToMsgReview(applicaionID,facilityId,facilityName,msgID,applicationName,protocolMode,sub_module,srlNo,protocol_link_id,purge_status,position)
{
...
function viewMessage(applicationId,applicationName,facilityId,facilityName,msgId,eventType,sub_module,
...
function messageViewer(applicationId,facilityId,msgId,eventType,protocol_link_id,protocolMode,sub_module,srlNo,purge_status,applicationName,facilityName,position)
{		
