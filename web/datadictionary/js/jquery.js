		...
		jQuery.each(elems, function(i, elem){
			...
			if ( typeof elem === "string" ) {
				...
				if ( !jQuery.support.leadingWhitespace && /^\s/.test( elem ) )
					div.insertBefore( context.createTextNode( elem.match(/^\s*/)[0] ), div.firstElementChild );
...
jQuery.each({
	...
	siblings: function(elem){return jQuery.sibling(elem.parentNode.firstElementChild,elem);},
	children: function(elem){return jQuery.sibling(elem.firstElementChild);},
	...
	filter: {
		CHILD: function(elem, match){
			...
			if ( parent && (!parent[ doneName ] || !elem.nodeIndex) ) {
				...
				for ( var node = parent.firstElementChild; node; node = node.nextSibling ) {
					if ( node.nodeType == 1 ) {
...
(function(){
	...
	form.innerHTML = "<input name='" + id + "' id='" + id + "'/>";
...
(function(){
	...
	if ( div.firstElementChild.getAttribute("href") !== "#" ) {
...
jQuery.event = {
	...
	fix: function(event) {
		...
		if ( !event.target )
			event.target = event.target || document; // Fixes #1925 where srcElement might not be defined either
...
(function(){
	...
	root.insertBefore( script, root.firstElementChild );
...
jQuery.offset = {
	initialize: function() {
		...
		innerDiv = container.firstElementChild, checkDiv = innerDiv.firstElementChild, td = innerDiv.nextSibling.firstElementChild.firstElementChild;
