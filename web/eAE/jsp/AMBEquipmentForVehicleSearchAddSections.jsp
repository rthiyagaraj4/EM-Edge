	...
	<body onscroll='scrollTitle()' onload="fxheaderInit('dat1',190);" OnMouseDown="CodeArrest()" onKeyDown="lockKey()">
		<form name = 'secForNoteTypeSearchSectionsForm'>
			<table cellpadding='5' cellspacing='0' border='0' width='100%' align='center'>
				<tr id='trcoll'>
					<th align='center' colspan='3'><fmt:message key="eAE.AvailableEquipments.label" bundle="${ae_labels}"/></th>
				</tr>
				<tr id='trcoll'>
					<td class = 'label' align='right' width='65%'><fmt:message key="eAE.EquipmentsStartsWith.label" bundle="${ae_labels}"/>&nbsp; <input type='text' name='searchText' id='searchText' value='<%=searchText%>'></td>
...
<Script src=\"../../eCommon/js/showModalDialog.js\" language=\"JavaScript\"></Script>
		...
		%>
		<script>
		parent.sectionsFrame.location.href = '../../eAE/jsp/AMBEquipmentForVehicleAMB.jsp?note_type=<%=note_type%>';
		</script>
		<%
		}
		%>
		<script>
		/*	var tempVar = <%=tempCount%> / 9;
			if(<%=start%> == 0)
				document.getElementById("prevRecs").style.visibility = 'hidden';
			else
				document.getElementById("prevRecs").style.visibility = 'visible';
		
			if(<%=count%> < 9 || <%=countForNext%> == tempVar)
				document.getElementById("nextRecs").style.visibility = 'hidden';
			else
