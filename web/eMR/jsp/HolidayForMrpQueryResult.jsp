...
<form name='MRPHolidayQueryResult' id='MRPHolidayQueryResult' method='post' action='../../servlet/eMR.MRPHolidayServlet' target='messageFrame'>
	...
	<% }
		...
		String classValue = "" ;
			...
			<td class='<%=classValue%>' >
			<a href='javascript:modifyMRPHoliday("<%=fs_locn_holiday_date%>","<%=java.net.URLEncoder.encode(reason)%>","<%=holiday%>","<%=facilityId%>")'><%=holiday_date_disp%></a><input type=hidden name='holidayDate' id='holidayDate' value='<%=fs_locn_holiday_date%>'></td>
			<td class='<%=classValue%>'>
			<a href='javascript:modifyMRPHoliday("<%=fs_locn_holiday_date%>","<%=java.net.URLEncoder.encode(reason)%>","<%=holiday%>","<%=facilityId%>")'> 
			<%=holiday%>&nbsp;</a></td>
			<td class='<%=classValue%>'>
			<a href='javascript:modifyMRPHoliday("<%=fs_locn_holiday_date%>","<%=java.net.URLEncoder.encode(reason)%>","<%=holiday%>","<%=facilityId%>")'> 
			<%=reason%></a></td>
			<td class='<%=classValue%>' align='center'><input type='checkbox' id='check_Box<%=k%>' class='checkBox' name='checkBox' onClick='javascript:checkBoxOnClick(this,<%=k%>);'  ></td>	</tr>
			<%
			k=k+1;
			i++;
			maxRecord++;
