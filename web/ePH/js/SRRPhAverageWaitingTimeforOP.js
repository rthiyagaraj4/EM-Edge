...
function run() {
	...
	var frmObject = f_query_add_mod.document.AveWaitingTimeforOPCriteria;
	//alert("Report is Under Process");
	if(f_query_add_mod.document.AveWaitingTimeforOPCriteria) {
		var fields= new Array (frmObject.p_frm_date1,frmObject.p_to_date1,frmObject.p_frm_time,frmObject.p_to_time,frmObject.p_frm_disp_locn,frmObject.p_to_disp_locn,frmObject.p_int_option);
		...
		if(checkFields(fields,names,messageFrame)){	
			var locale = frmObject.p_language_id.value;
				...
				var P_FRM_DISPN_DATE=convertDate(frmObject.p_frm_date1.value,"DMY",locale,"en"); //Added Variables for the incident num:==By Sandhya
				var P_TO_DISPN_DATE=convertDate(frmObject.p_to_date1.value,"DMY",locale,"en");
			
			frmObject.p_frm_date.value=P_FRM_DISPN_DATE;
			frmObject.p_to_date.value=P_TO_DISPN_DATE;
			
			f_query_add_mod.document.AveWaitingTimeforOPCriteria.submit();
		}
...
{
	var formObj =document.AveWaitingTimeforOPCriteria;
...
function clearList(){
	...
	var disp_loc_len =document.AveWaitingTimeforOPCriteria.p_frm_disp_locn.options.length;
	for(var i=0;i<disp_loc_len;i++) {
		AveWaitingTimeforOPCriteria.p_frm_disp_locn.remove("p_frm_disp_locn") ;
		AveWaitingTimeforOPCriteria.p_to_disp_locn.remove("p_to_disp_locn") ;
	}
...
function addOption(code,desc){
	...
	//opt.text = desc ;
	opt.text = decodeURIComponent(desc,"UTF-8");   //Added decodeUri component to desc regarding incident 23698 in spira on 08/Sept/2010
	
	opt.value = code ;	
	document.AveWaitingTimeforOPCriteria.p_frm_disp_locn.add(opt);

	var opt1 = document.createElement("OPTION") ;
	if(desc == "A")
		desc = "------"+getLabel("Common.defaultSelect.label","Common")+"-------------";
	opt1.text = desc ;
	opt1.text = decodeURIComponent(desc,"UTF-8");   //Added decodeUri component to desc regarding incident 23698 in spira on 08/Sept/2010
	opt1.value = code ;	
	
	document.AveWaitingTimeforOPCriteria.p_to_disp_locn.add(opt1);
...
function callLocationLookup(locn_txt) {
	...
	var formobj=document.AveWaitingTimeforOPCriteria
		if (document.AveWaitingTimeforOPCriteria)      
		{	
			formobj=document.AveWaitingTimeforOPCriteria
		}
...
function clearLocation(){
		...
		AveWaitingTimeforOPCriteria.p_locn.value = '';
		AveWaitingTimeforOPCriteria.locn_text.value = '';
...
function chkbill()
{
		/*alert("document.AveWaitingTimeforOPCriteria.p_frm_bill_grp.value======69" +document.AveWaitingTimeforOPCriteria.p_frm_bill_grp.value);
		alert("document.AveWaitingTimeforOPCriteria.p_to_bill_grp.value=====70" +document.AveWaitingTimeforOPCriteria.p_to_bill_grp.value);*/
	if(document.AveWaitingTimeforOPCriteria.billing_from.value==null || document.AveWaitingTimeforOPCriteria.billing_from.value=="")
	{
		
		document.AveWaitingTimeforOPCriteria.p_frm_bill_grp.value="";
	}
	if(document.AveWaitingTimeforOPCriteria.billing_to.value==null || document.AveWaitingTimeforOPCriteria.billing_to.value=="")
	{
		
		document.AveWaitingTimeforOPCriteria.p_to_bill_grp.value="";
	}
