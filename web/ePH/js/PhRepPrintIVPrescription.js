...
function reset(obj) {
	f_query_add_mod.frmPrescriptionQueryCriteria.reset();
	f_query_add_mod.frmPrescriptionQueryCriteria.btnPatientID.disabled=false;
...
function callPatientSearch(){
	...
	var formObj=document.frames.frmPrescriptionQueryCriteria;
...
if(pat_id=="") {
	...
	if(patient_id !="" || orig_order_id != ""){
		if(iv_type!="" ){
			parent.messageFrame.location.href="../../eCommon/jsp/error.jsp?err_num=";
			parent.f_query_results.location.href="../../ePH/jsp/PhRepPrintIVPrescriptionResult.jsp?patient_id="+patient_id+"&orig_order_id="+orig_order_id+"&order_date_from="+order_date_from+"&encounter_id="+encounter_id+"&pres_practitioner_id="+pres_practitioner_id+"&locn_type="+locn_type+"&locn_code"+locn_code+"&iv_type="+iv_type;
			parent.f_query_status.location.href="../../ePH/jsp/PhRepPrintPrescriptionStatus.jsp";
		}else{
			
			alert(getMessage("PRES_NO_OR_IV_TYPE_NOT_BLANK"));
		...
		if(formObj.order_id.value == "" && formObj.patient_id.value == ""){
			...
			parent.messageFrame.document.location.href="../../eCommon/jsp/error.jsp?err_num="+getMessage("PRES_NO_OR_PATIENT_ID_NOT_BLANK") ;
	...
	var locn_code=formObj.locn_code.value;
		...
		parent.f_query_results.location.href="../../ePH/jsp/PhRepPrintIVPrescriptionResult.jsp?patient_id="+patient_id+"&orig_order_id="+orig_order_id+"&order_date_from="+order_date_from+"&encounter_id="+encounter_id+"&pres_practitioner_id="+pres_practitioner_id+"&locn_type="+locn_type+"&locn_code"+locn_code;
		parent.f_query_status.location.href="../../ePH/jsp/PhRepPrintPrescriptionStatus.jsp";
}
}

function SubmitLink(from,to,patient_id,orig_order_id,order_date_from,order_date_to,encounter_id,pres_practitioner_id){
	var obj=document.frames.frmPrintPrescriptionReportResult;
	obj.from.value=from;
	obj.to.value=to;
...
function showPrintReport(order_id,patient_class,locn_code,iv_type){
	...
	parent.f_query_add_mod.frmPrescriptionQueryCriteria.submit();
