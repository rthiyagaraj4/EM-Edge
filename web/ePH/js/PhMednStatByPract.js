...
function run() {
	...
	if(frmObjectOfDispLocnCriteria != null || frmObjectOfDispLocnCriteria != undefined){
		if(f_query_add_mod.document.formPhMednStatByDispLocnCriteria) {
			...
			if(checkFields( fields,names,messageFrame) ) {
				...
				var flag=checkSysDate(frmObjectOfDispLocnCriteria, frmObjectOfDispLocnCriteria.P_DATE_FROM,frmObjectOfDispLocnCriteria.P_DATE_TO,sysdate, locale);
				if(flag){
	...
	if(frmObject != null || frmObject != undefined){//End of Modification on date for Medication Order Statistics By Dispense Location 
		...
		if(f_query_add_mod.document.formPhMednStatByPractCriteria) {
			var fields= new Array (frmObject.p_select_by,frmObject.dt_from_1,frmObject.dt_to_1,frmObject.p_group_by,frmObject.p_report_by);
			...
			if(checkFields(fields,names,messageFrame) ) {
				if( f_query_add_mod.doDateCheck( f_query_add_mod.document.formPhMednStatByPractCriteria.dt_from_1, f_query_add_mod.document.formPhMednStatByPractCriteria.dt_to_1, messageFrame ) ) {
					if(f_query_add_mod.CheckString(getLabel("Common.practitioner.label","Common"), f_query_add_mod.document.formPhMednStatByPractCriteria.pract_id, f_query_add_mod.document.formPhMednStatByPractCriteria.tpract_id, messageFrame) ) {
							var locale=frmObject.language_id.value;
							frmObject.dt_from.value=convertDate(frmObject.dt_from_1.value,'DMY',locale,"en");
							frmObject.dt_to.value=convertDate(frmObject.dt_to_1.value,'DMY',locale,"en");
...
function reset() {
    ...
    //f_query_add_mod.formPhMednStatByPractCriteria.reset();
     ...
     if(f_query_add_mod.document.formPhMednStatByPractCriteria!=undefined)//if condition for BRUSIM Adhoc Testing
...
//To Check Date
function checkSysDate(formObj, from,to,systemdate, locale){
