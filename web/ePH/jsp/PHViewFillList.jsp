...
<%}else{%>
		<TD class="BODYCOLORFILLED" align="left" nowrap>&nbsp;<img src='../../ePH/images/altenate.jpg' height='15' width='15' align='top' onclick="altDrugSearch('<%=disp_locn_code%>','<%=ord_drug_qty%>','<%=fill_int_start_date_time%>','<%=fill_int_end_date_time%>','<%=ord_drug_code%>','<%=order_id%>','<%=Proc_Type%>','<%=i%>')">
		</TD>
    <%}%>
...
<%}else{%>
		<TD class="BODYCOLORFILLED" style="BORDER-STYLE: none solid none none" ALIGN="left" nowrap><font size="1"  style="cursor:pointer;color:blue" title="Batch Details" onclick="showBatchDetails('<%=i%>','<%=order_id%>');" ><label id="disp_qty_id_<%=i%>" name="disp_qty_<%=i%>" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=disp_qty%></label>
...
<%
		...
		<TD class="BODYCOLORFILLED" ALIGN="right"><input type="checkbox" name="ip_fill_chk_<%=i%>" id="ip_fill_chk_<%=i%>" value="<%=ip_fill_chk_value%>" <%=chk_dsbl%> <%=ip_fill_chked%> onclick="checkOne(this)"></TD>
		</TR>
<%
		}
%>
